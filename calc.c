...
void GetMinMaxText(double X,double Y,double FontSize,int32 FontNr,double Rotation,
...
//  DrawFilledPolygon(PolygonObject,4);
  if (CheckPolygonCompleetlyOutsidePolygon(PolygonPointObject,PolygonObject)!=1) return 3;
...
{
  char   code,*str2;
  int32  cnt,cnt2,cnt3,cnt4,NrPolyLines,lengte,count,oldx,oldy,
         SegmentCount;
...

{
  ...
  double x1,y1,x2,y2,x3,y3,x4,y4,x5,y5,hoek1,hoek2,hoek,hoek_inc,
  ...
  if (mode==0) {
    if (max(x2,y2)>(400*2540)) NrSegments=64;
    if (max(x2,y2)>(1000*2540)) NrSegments=128;
  } else {
    if (max(x2,y2)>(100*2540)) NrSegments=64;
    if (max(x2,y2)>(250*2540)) NrSegments=128;
    if (max(x2,y2)>(500*2540)) NrSegments=256;
  }
...

{
  ...
  ObjectRecord *Object,*Object1,*Object2,*Object3,NewObject1,NewObject2,
  ...
  if (count!=2) {
    MessageBoxUTF8(GEOMWindow,SC(0,"Two objects should be selected"),SC(1,"Error"),MB_APPLMODAL|MB_OK);
    return 0;
  }
  if ((Object1->ObjectType==OBJECT_TEXT)
     ||
     (Object1->ObjectType==OBJECT_TEXT)) {
    return 0;
  }
/*
    ...
    case OBJECT_LINE:
      switch (Object2->ObjectType) {
        ...
        case OBJECT_POLYGON:
          ...
          if ((ObjectPolygon2->NrSubPolygons>0) 
             &&
             (ObjectPolygon2->NrVerticesMainPolygon>0))  {
...
// *******************************************************************************************************
    case OBJECT_POLYGON:
      switch (Object2->ObjectType) {
        case OBJECT_POLYGON:
          ...
          if ((ObjectPolygon1->NrSubPolygons>0) 
             &&
             (ObjectPolygon1->NrVerticesMainPolygon>0))  {
  ...
  if (MinDistance<0.0) MinDistance=0.0;
  if (Units==0) {
...
int32 GetDimensionTextFromLine(double x1,double y1,double x2,double y2,
                               ObjectRecord *ObjectText2,int32 mode)
