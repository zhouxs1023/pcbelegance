...
{
  int32  cnt2,Mirror;
  double xmin,ymin,xmax,ymax,xmin2,ymin2,xmax2,ymax2,x1,y1,x2,y2,
         MaxCl,Clearance,Thickness,Thickness2;
  ObjectRecord *Object;
...
//  StringWrite("Outline objects component ");
//  NrWrite(CompNr);
//  StringWriteln(" calculated");
//  StringWrite("Pins objects component ");
//  NrWrite(CompNr);
//  StringWriteln(" calculated");
...
// *******************************************************************************************************
  ...
  if ((mode & 2)==0) {
    ...
    if ((Comp->PlacementWidth>1900e5)
       &&
       (Comp->PlacementHeight>1900e5)) {
      ok=1;
...
//  double  xmin,ymin,xmax,ymax;
  int32  cnt,Layer,cnt2,TextMode,TextAlignment,Mirror,FontNr,
         MaxCountX,NrLines,Changed,BoardOutlineSizeChanged;
  char   str[MAX_LENGTH_STRING];
  double  x1,y1,x2,y2,x4,y4,MinX1,MinY1,MaxX1,MaxY1,dikte,lengte,
          MinX2,MinY2,MaxX2,MaxY2,Thickness,Rotation,
          Xmax,Xmin,Ymax,Ymin;
  ObjectRecord *Object;
  ...
  *MaxY=-1000000000.0;
  ...
  for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
    ...
    if ((ObjectLine->Info & OBJECT_NOT_VISIBLE) == 0) {
      ...
      x2=ObjectLine->X2;
      y2=ObjectLine->Y2;
//      Thickness=ObjectLine->LineThickNess*0.5;
      Thickness=0.0;
...
/*
  for (cnt=0;cnt<Design.NrObjectCircles;cnt++) {
    ...
    if ((ObjectCircle->Info & OBJECT_NOT_VISIBLE) == 0) {
      ...
      y2=ObjectCircle->Diam;
//      Thickness=ObjectLine->LineThickNess*0.5;
      Thickness=0.0;
      if (ObjectCircle->Layer==BOARD_OUTLINE_LAYER) {
        *MinX=min(*MinX,x1-x2*0.5-Thickness);
        *MaxX=max(*MaxX,x1+x2*0.5+Thickness);
        *MinY=min(*MinY,y1-y2*0.5-Thickness);
        *MaxY=max(*MaxY,y1+y2*0.5+Thickness);
...
/*
  for (cnt=0;cnt<Design.NrObjectTexts;cnt++) {
    ...
    if ((ObjectText->Info & OBJECT_NOT_VISIBLE) == 0) {
      ...
      GetMinMaxText(x1,y1,x2,0,TextRotation,TextAlignment,0,ObjectText->Text);
//      Thickness=ObjectLine->LineThickNess*0.5;
      Thickness=0.0;
      MinX2=min(MinX2,TextMinX-Thickness);
      MaxX2=max(MaxX2,TextMaxX+Thickness);
      MinY2=min(MinY2,TextMinY-Thickness);
      MaxY2=max(MaxY2,TextMaxY+Thickness);
      Changed=1;
    }
  }
*/
    ...
    if ((Comp->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      if (0) {
        if ((!InRangePCB(MinX2))
           ...
           ||
           (!InRangePCB(MaxY2))) {
          sprintf(str,SC(0,"%.1f,%.1f  %.1f,%.1f mm  %s"),
                      MinX2/100000.0,MinY2/100000.0,
  ...
  if (0) {
    sprintf(str,SC(2,"%.1f,%.1f  %.1f,%.1f mm"),
                MinX2/100000.0,MinY2/100000.0,
                MaxX2/100000.0,MaxY2/100000.0);
    MessageBoxOwn(PCBWindow,str,SC(1,"Message"),MB_APPLMODAL|MB_OK);
  }
...
void ShapePinsToObject(CompRecord *Comp,double OffsetX,double OffsetY,
...
{
  int32           PinOffset,MemPos,CompLayer,SoldLayer,
                  NrPins,PinNr,Rotation,NetPinCount,MemPos2,MemDiv,
                  PolygonVertices,NrLayers,MirroredLayer,CompObjectCount,
                  ShapePos,PinsMemPos,Layer,MemPosComp,
                  ShapeInfo,NrPinShapes,ShapeType,ShapeNr,Mirror,NetNr;
  double          x,y,Width,Height,OX,OY,OriginX,OriginY,hulp,RotationAngle,
...
/*
  MemPosOld=MemPos;
  bufp=&(ShapesMem[MemPos]);
  for (cnt=0;cnt<MemDiv;cnt++) {
    crc = (crc >> 8) ^ crc32_table[(crc & 0xFF) ^ bufp[cnt]];
  }
  if (ShapeCrcs[ShapeNr]==0) {
    ShapeCrcs[ShapeNr]=crc;
  } else {
...
//    PinText=&(CompsMem[PinStringPos]);
    ...
    while (NrPinShapes>0) {
      ...
      if (ObjectArrayNr==0) Object=&((*Objects)[NrObjects]);
      else Object=&((*Objects2)[NrObjects2]);
...
#ifdef _DEBUG
      if (stricmpOwn(Comp->Name,"z100")==0) {
//        sprintf(str,"z100 pin nr %d, mempos=%d\n",PinNr,MemPos);
//        OutputDebugStr(str);
      }
#endif
      NrPinShapes--;
      ...
      if (ObjectInclude) {
#ifdef _DEBUG
        if ((InRangeSpecial(Object->x1,2.54e5,0.05e5))
           &&
           (InRangeSpecial(Object->y1,11.05e5,0.05e5))) {
          ok=1;
        }
        ...
        if (ObjectArrayNr==0) {
          if (NrObjects>=MaxNrObjects-1) {
            if (AllocateMemObjects(MaxNrObjects+1024)==0) NrObjects++;
          } else NrObjects++;
        } else {
          if (NrObjects2>=MaxNrObjects2-1) {
            if (AllocateMemObjects2(MaxNrObjects2+128)==0) NrObjects2++;
          } else NrObjects2++;
        }
        CompObjectCount++;
      }
...
{
  int32 MemPos,*ShapeInfoP,NrLines,ShapeInfo2,Rotation,ShapeInfo,TextLength,
        ShapeNr,Mirror;
...

{
  ...
  ShapeNr=(int32)Comp->ShapeNr;
  if (ShapeNr==-1) return sizeof(CompRecord);
  MemPos=(*Shapes)[ShapeNr].ShapePos;
  Shape=(ShapeRecord *)&(ShapesMem[MemPos]);
...
{
  int32 MemPos,NrLines,Rotation,ShapeType,TextLength,ShapeMirror,NewMirror,
        *ShapeTypeP,PolygonVertices,ShapePos,*PolygonPointer,ShapeNr,Mirror,ShapeInfo2;
  double hulp,x1,y1,x2,y2,x3,y3,x4,y4,OX,OY,OriginX,OriginY,RotationAngle,
...
//        TextRotation=((GetRotationFromComp((int32)y2)) & 7)*45.0;
        ...
        if (RotationAngle2>360) RotationAngle2-=360;
        if (RotationAngle2<0) RotationAngle2+=360;
        Rotation=GetRotationFromFloat(RotationAngle);
        switch (Rotation) {
          case 0:
            break;
...
{
  int32 MemPos,NrLines,Rotation,ShapeType,TextLength,PolygonVertices,
        ShapeMirror,*ShapeTypeP,ShapePos,*PolygonPointer,
        NewMirror,ShapeNr,Mirror,ShapeInfo2;
  double hulp,x1,y1,x2,y2,x3,y3,x4,y4,OX,OY,OriginX,OriginY,RotationAngle,
         RotationAngle2,TextRotation;
  ShapeLinesArray *ShapeLines;
  ObjectRecord *Object;
...
void ShapeOtherToObject(CompRecord *Comp,double OffsetX,double OffsetY,double TempRotation,
...
*/
  int32 MemPos,NrLayers,NrLines,Rotation,Layer,PolygonVertices,ShapePos,ShapeNr,
        ShapeMirror,NewMirror,Mirror,ShapeType;
  double hulp,x,y,OriginX,OriginY,Width,Height,RotationAngle,
         x1,y1,x2,y2,x3,y3,x4,y4,RotationAngle2,TextRotation;
  ShapeLinesArray *ShapeLines;
  ObjectRecord *Object;
  PadRecord *Pad;
  uint8 y4a;
...
//        TextRotation=((GetRotationFromComp((int32)y2)) & 7)*45.0;
      ...
      case OBJECT_POLYGON:
        ...
        Object->y1=y+OffsetY+OriginY;
#ifdef _DEBUG
        if ((InRange9(Object->x1,3.5e5))
           &&
           (InRange9(Object->y1,244.8e5))) {
          ok=1;
        }
#endif
        Object->RotationAngle=RotationAngle;
...
double DistancePointToConnection(double px,double py,double ConnectionX1,double ConnectionY1,double ConnectionX2,double ConnectionY2)
...
*/
  ...
  if (ConnectionX2==ConnectionX1) {
    ...
    if ((py>=miny)
       ...
       (py<=maxy)
       &&
       (DistCentre<DistConnection1)
       &&
       (DistCentre<DistConnection2)) return DistCentre;
...
*/
  ...
  if (InRange(TraceX2,TraceX1)) {
    ...
    if ((py>=miny)
       ...
       (py<=maxy)
       &&
       (DistCentre<DistTrace1)
       &&
       (DistCentre<DistTrace2)) return DistCentre;
...
double DistancePointToArc(double px,double py,double x1,double y1,double x2,double y2,
...
*/
  ...
  if (mode==1) {
    if (InRange(divx,0.0)) {
      if (InRange(divy,0.0)) return -1;
      else {
        if (divy<0.0) return 0;
        else return 16;
      }
    }
    if (InRange(divy,0.0)) {
      if (divx<0.0) return 8;
      else return 24;
    }
    if (divx<0) {
      if (divy<0) {
  ...
  if (InRange(divx,0)) {
    if (InRange(divy,0)) return -1;
    else {
      if (X1SmallerThenX2(divy,0)) return 0;
      else return 16;
    }
  }
  if (InRange(divy,0)) {
    if (X1SmallerThenX2(divx,0)) return 8;
    else return 24;
  }
  if ((InRange(divx,divy))
     &&
     (X1SmallerThenX2(divx,0))) return 4;
  if ((InRange(divx,divy))
     &&
     (X1GreaterThenX2(divx,0))) return 20;
  if ((InRange(divx,-divy))
     &&
     (X1SmallerThenX2(divx,0))) return 12;
  if ((InRange(divx,-divy))
     &&
     (X1GreaterThenX2(divx,0))) return 28;
//  if (mode==1) return -2;
...
// *******************************************************************************************************
/*
void GetMinMaxText(double X,double Y,double FontSize,int32 FontNr,int32 Rotation,
...
void GetMinMaxText2b(double X,double Y,double FontSize,int32 FontNr,double Rotation,
                     int32 Alignment,int32 Mirror,WCHAR *Str)
...
{
  int32  Layer,TraceInfo,ViaInfo,CircleMode;
  int32  cnt,cnt2,NrTracesDeleted,NrViasDeleted,AreaFillCount;
  ...
  for (cnt=0;cnt<Design.NrObjectRects;cnt++) {
    ...
    if (((ObjectRect->Info & (OBJECT_NOT_VISIBLE)) == 0)
      ...
      minx=min(minx,ObjectRect->CentreX-ObjectRect->Width/2);
      miny=min(miny,ObjectRect->CentreY-ObjectRect->Height/2);
      maxx=max(maxx,ObjectRect->CentreX+ObjectRect->Width/2);
      maxy=max(maxy,ObjectRect->CentreY+ObjectRect->Height/2);
      Found=1;
    }
  }
  for (cnt=0;cnt<Design.NrObjectArcs;cnt++) {
    ...
    if (((ObjectArc->Info & (OBJECT_NOT_VISIBLE)) == 0)
      ...
      minx=min(minx,ObjectArc->CentreX-ObjectArc->Width/2);
      miny=min(miny,ObjectArc->CentreY-ObjectArc->Height/2);
      maxx=max(maxx,ObjectArc->CentreX+ObjectArc->Width/2);
      maxy=max(maxy,ObjectArc->CentreY+ObjectArc->Height/2);
      Found=1;
    }
  }
  if (!Found) {
  ...
  Design.BoardOriginX=minx;
  Design.BoardOriginY=miny;
  Design.BoardWidth=maxx-minx;
  Design.BoardHeight=maxy-miny;
  return 0;
}
*/
...
{
  if (Layer<32) {
    if ((DrawLayerCode[Layer] & 0x10) == 0x10) return -1;
    return 0;
  }
  if ((Layer>=ROUTING_KEEPOUT_LAYER)
     &&
    ...
    if ((Layer==0)
       &&
       (!OkToDrawRoutingKeepoutBottom)) return -1;
    if ((Layer==Design.NrBoardLayers-1)
       &&
       (!OkToDrawRoutingKeepoutTop)) return -1;
    if ((Layer>0)
       &&
       (Layer<Design.NrBoardLayers-1)
       &&
       (!OkToDrawRoutingKeepoutInner)) return -1;
    return 0;
  }

  switch (Layer) {
    case SILKSCREEN_TOP:
      if (!OkToDrawSilkScreenTop) return -1;
      break;
    case SILKSCREEN_BOTTOM:
      if (!OkToDrawSilkScreenBottom) return -1;
      break;
    case INFO_LAYER:
      if (!OkToDrawInfoObjects) return -1;
      break;
    case INFO_LAYER2:
      if (!OkToDrawInfo2Objects) return -1;
      break;
    case INFO_LAYER3:
      if (!OkToDrawInfo3Objects) return -1;
      break;
    case INFO_LAYER4:
      if (!OkToDrawInfo4Objects) return -1;
      break;
    case BOARD_OUTLINE_LAYER:
      if (!OkToDrawBoardOutline) return -1;
      break;
    case SOLD_MASK_BOTTOM:
      if (DrawSoldMaskBottomMode==0) return -1;
      break;
    case SOLD_MASK_TOP:
      if ((Design.NrBoardLayers==1)
         ||
         (DrawSoldMaskTopMode==0)) return -1;
      break;
    case PASTE_MASK_BOTTOM:
      if (DrawPasteMaskBottomMode==0) return -1;
      break;
    case PASTE_MASK_TOP:
      if ((Design.NrBoardLayers==1)
         ||
         (DrawPasteMaskTopMode==0)) return -1;
      break;
    case DRILL_LAYER:
      if (DrawDrillMode==0) return -1;
      break;
...

{
  ...
  NrSegments=32;
  if (Thickness>(400*2540)) NrSegments=64;
  if (Thickness>(1000*2540)) NrSegments=128;
  Step=128/NrSegments;
  switch (CircleMode) {
    case 1:
      Start=0;
...
int32 ArcToLineSegments(double x1,double y1,double Width,double Height,
                        double x2a,double y2a,double x2b,double y2b,double *LineSegments,int32 mode)

{
  int32  NrSegments,cnt2,count,SegmentCount;
  double x3,y3,x4,y4,x5,y5,hoek1,hoek2,hoek,hoek_inc,
         Length,sinx,sinx2,lengte1,lengte2;
#ifdef _DEBUG
  ...
  if (max(Width,Height)>10e5) NrSegments*=2;
  if (max(Width,Height)>25e5) NrSegments*=4;
  if (max(Width,Height)>100e5) NrSegments*=8;
  ...
  if ((InRange(x2a,x2b))
    ...
    if (hoek2<hoek1) hoek2+=ANGLE_360;
  }
...
*/
  ...
  sinx=sin(hoek);
  sinx2=sinx*sinx;
  Length=(Width*0.5)*(Height*0.5)*sqrt(1/(SQR((Height*0.5))*(1-sinx2)+SQR((Width*0.5))*sinx2));
  x5=Length*cos(hoek);
  y5=Length*sin(hoek);
...
int32 GetDimensionTextFromLine(double x1,double y1,double x2,double y2,
                               ObjectTextRecord2 *ObjectText2,int32 mode)
    ...
    if ((mode & 8)==0) {
      if ((mode & 4)==4) {
        if (Length<(TextMaxX-TextMinX)+ObjectText2->FontHeight*2.0) {
          ...
          cy=y1+sin(Angle)*(Length+dx*0.5+ObjectText2->FontHeight*2.0)+ObjectText2->FontHeight*0.1;
...
/*
int32 TextStringToLineSegments(double x,double y,double Size,
                               int32 Rotation,int32 Alignment,int32 Mirror,LPSTR str,
                               double *LineSegments)

{
  char   code,*str2;
  int32  ok,cnt,cnt2,cnt3,cnt4,NrPolyLines,lengte,count,oldx,oldy,hulp,
         SegmentCount,xx1,yy1,xx2,yy2,hx,hy;
  double  x1d,y1d,x2d,y2d,x1,y1,x2,y2,hulp2,lengte2,NewLine,PointX[20],PointY[20];
...
int32 TextStringToLineSegments2(double x,double y,double Size,
                                double Rotation,int32 Alignment,int32 Mirror,LPSTR str,
                                double *LineSegments)
...

{
  ...
  if ((InRange(ObjectArc->Width,ObjectArc->Height))
     &&
...

{
  char  str[MAX_LENGTH_STRING];
/*
  int32 ObjectType,Test;
  int32 Layer;
  double x1,y1,x2,y2,x3,y3,x4,y4,Clearance,
        minx,miny,maxx,maxy;
  double Thickness;
  uint8  *Address;
  int32 ObjectType2;
  ...
  int32 Mirror,Dummy;
  int32 TraceNr;
  int32 Info,Info2,NetNr,CompNr,PinNr,CompNr2;
...

{
  ...
  if (CheckPolygonCompleetlyOutsidePolygon(PolygonPointObject,PolygonObject)!=1) return 1;
...
int32 RectTestArc2(double x1,double y1,double x2,double y2,
                  double x3,double y3,double x4,double y4,
                  double Thickness)

{
  ...
  if ((ArcXmax<SearchMinX)
     ||
     (ArcXmin>SearchMaxX)
     ||
     (ArcYmax<SearchMinY)
     ||
     (ArcYmin>SearchMaxY)) return 0;


  if ((ArcXmax<SearchMaxX)
     &&
     (ArcXmin>SearchMinX)
     &&
     (ArcYmax<SearchMaxY)
     &&
     (ArcYmin>SearchMinY)) return 1;
...
{
  int32           PinOffset,MemPos,cnt,PolygonVertices,
                  NrPins,PinNr,ShapePos,NrLayers,MemPosComp,
                  Layer,ShapeInfo,NrPinShapes,ShapeType,ShapeNr,Mirror;
  int32            Found;
...
{
  int32           PinOffset,MemPos,cnt,cnt2,NrPins,MemPosComp,
                  NrPinShapes,ShapeType,ShapeNr;
  ShapePadRecord  *ShapePad;
  PadRecord       *Pad;
  ShapeRecord     *Shape;
...

{
  ...
  for (cnt2=0;cnt2<NrObjects;cnt2++) {
    Object=&((*Objects)[cnt2]);
    if (Object->Layer==-1) return -1;      // Pads on top/bottom
    if (Layer!=-1) {
      if (Object->Layer!=Layer) return -1; // Pads on top/bottom
    } else {
      Layer=Object->Layer;
    }
  }
...

{
  ...
  char  str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],
        str5[MAX_LENGTH_STRING],str6[MAX_LENGTH_STRING],str7[MAX_LENGTH_STRING],
        str8[MAX_LENGTH_STRING];
  ...
  if ((Object->NetNr>=0)
    ...
    if ((Object->NetNr==32767)
       ||
       (Object->NetNr==-1)) {
      strcpy(str2,SC(348,"Unconnected"));
    }
  }
  if (Object->Layer!=-1) {
    GetLayerText(Object->Layer,LayerText,16+4);
  } else {
    strcpy(LayerText,SC(350,"All"));
  }
  ...
  switch (Object->ObjectType) {
    case DRILL_UNPLATED:
      ...
      sprintf(str3,SC(1254,"%s,%s (drill unplated %s)"),str4,str5,str6);
      break;
    case DRILL:
      GetUnitsValue2(Units,Object->x1,str4,2);
      GetUnitsValue2(Units,Object->y1,str5,2);
      GetUnitsValue2(Units,Object->x2,str6,2);
      sprintf(str3,SC(349,"%s,%s (drill %s)"),str4,str5,str6);
      break;
    case PIN_PUT_THROUGH_ROUND:
      GetUnitsValue2(Units,Object->x1,str4,2);
      GetUnitsValue2(Units,Object->y1,str5,2);
      ...
      sprintf(str3,SC(351,"%s,%s (%s / drill %s)"),str4,str5,str6,str7);
      break;
    case PIN_SMD_ROUND:
      GetUnitsValue2(Units,Object->x1,str4,2);
      GetUnitsValue2(Units,Object->y1,str5,2);
      GetUnitsValue2(Units,Object->x2,str6,2);
      sprintf(str3,"%s,%s (%s)",str4,str5,str6);
      break;
    case PIN_LINE_HOR:
    case PIN_LINE_VER:
    case PIN_LINE_DIAG1:
    case PIN_LINE_DIAG2:
      ...
      sprintf(str3,SC(353,"PINTRACE %s,%s - %s,%s (%s)"),str4,str5,str6,str7,str8);
      break;
    case PIN_LINE_ALL_ANGLE:
      GetUnitsValue2(Units,Object->x1,str4,2);
      GetUnitsValue2(Units,Object->y1,str5,2);
    ...
    case PIN_ARC:
      ...
      GetUnitsValue2(Units,Object->y2,str7,2);
      GetUnitsValue2(Units,Object->Thickness,str8,2);
      sprintf(str3,SC(1223,"PINARC %s,%s (%s,%s)"),str4,str5,str6,str8);
      break;
    case PIN_SMD_POLYGON:
      GetUnitsValue2(Units,Object->x1,str4,2);
      GetUnitsValue2(Units,Object->y1,str5,2);
      sprintf(str3,SC(357,"POLYGON (%s,%s"),str4,str5);
      break;
    case PIN_PUT_THROUGH_SQUARE:
      GetUnitsValue2(Units,Object->x1,str4,2);
      GetUnitsValue2(Units,Object->y1,str5,2);
    ...
    case PIN_SMD_RECT:
      ...
      GetUnitsValue2(Units,Object->x2,str6,2);
      GetUnitsValue2(Units,Object->y2,str7,2);
      sprintf(str3,"%s,%s (%s x %s)",str4,str5,str6,str7);
      break;
    case PIN_PUT_THROUGH_POLYGON:
      GetUnitsValue2(Units,Object->x1,str4,2);
      GetUnitsValue2(Units,Object->y1,str5,2);
      GetUnitsValue2(Units,Object->y2,str6,2);
      sprintf(str3,SC(360,"POLYGON THROUGH HOLE (%s,%s), drill %s"),str4,str5,str6);
      break;
  }
