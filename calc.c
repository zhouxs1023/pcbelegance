...
{
  int32 MemPos,MemPos2,cnt,cnt2,PowerPinMem,PinMem,PinBusMem,
        Rotation,SymbolNr,NrPinBusses,
        NrPowerPins,NrPins,NrPartsPerPackage,
        PackagePartNr,pos,pos2,LengthIdent,
        LengthValue,NrPowerPinOverRules,
...
// Pin
  ...
  if ((Mode & 1)==0) {
    ...
    while (cnt>0) {
      ...
      switch (Rotation) {
        case 0:
          if (ObjectMirrorX==1) x1=-x1;
          if (ObjectMirrorY==1) y1=-y1;
          break;
        case 1:  //  90
          hulp=x1;
          x1=-y1;
      ...
      if ((NrObjects+1>=MaxNrObjects)
         &&
         (AllocateMemObjects(MaxNrObjects+128)!=0)) return;
...
{
  int32     result,Designfp,cnt,cnt3,ok,Time1,
            LoadResult,Pos,Length,TotalSymbolsMem,NrErrors,InstancePos,
            SymbolNr,MemSize,NrSymbolNames;
  char      str[MAX_LENGTH_STRING],SheetName[MAX_LENGTH_STRING],
            LibName[MAX_LENGTH_STRING],SymbolNames[512][64];
...
//      memmove(&Instance,&OldInstance,sizeof(OldInstanceRecord));
    ...
    for (cnt3=0;cnt3<NrSymbolNames;cnt3++) {
      if (stricmpUTF8(SymbolNames[cnt3],Instance.SymbolName)==0) SymbolNr=cnt3;
    }
    if (SymbolNr==-1) {
