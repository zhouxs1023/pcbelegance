...
{
  double xx1,yy1,xx2,yy2,xx3,yy3,xx2a,yy2a,Xmin,Ymin,Xmax,Ymax,
        Rotation,MinX2,MinY2,MaxX2,MaxY2;
  char  str[MAX_LENGTH_STRING],TextStr[1024],TextStr2[MAX_LENGTH_STRING];
  int32 CircleMode,NrLines,cnt2,cnt3,lengte,AddMode,LinePos[16],TextAlignment,
        TextRotation,Length;
  ...
  switch (Object->ObjectType) {
    ...
    case SYMBOL_CIRCLE:
      ...
      Object->maxx=xx1;
      Object->maxy=yy1;
      if ((CircleMode & 0x0c) > 0) Object->minx=xx1-xx2a;
      if ((CircleMode & 0x03) > 0) Object->maxx=xx1+xx2a;
      if ((CircleMode & 0x09) > 0) Object->miny=yy1-xx2a;
      if ((CircleMode & 0x06) > 0) Object->maxy=yy1+xx2a;
      break;
    case SYMBOL_ARC:
      xx2a=xx2*0.5;
      Object->minx=xx1-xx2a;
      ...
      while (cnt3<Length+1) {
        if ((TextStr[cnt3]=='\r')
          ...
          if (cnt3-cnt2>0) {
            ...
            if ((Rotation==0.0)
               ||
               (InRangeSpecial(Rotation,90.0,0.01))) {
              if (Rotation==0.0) {
...
{
  int32 MemPos,MemPos2,cnt,cnt2,PowerPinMem,InfoBlockMem,PinMem,
        NrPinBusses,NrPowerPins,NrPins,NrPartsPerPackage,PinBusMem,
        SymbolNr,PinTextRotation,PinRot,Rotation,
        PackagePartNr,FontNr,LineMode,Alignment,CircleMode,InstanceInfo,
        NrPowerPinOverRules,pos,pos2,LengthIdent,LengthValue,
        ObjectMirrorX,ObjectMirrorY,Version30,Version10;
  double hulp,x1,y1,x2,y2,x3,y3,x4,y4,OX,OY,Thickness,OriginX,OriginY,
        FontHeight;
  ...
  PinRecord *Pin;
  PinBusRecord *PinBus;
  LPSTR PinText,PowerPinText,PowerPinNetName,LabelText,InstanceAttrBuf,
        PowerPinOverRuleNetName[64],PowerPinOverRuleNewNetName[64];
  SymbolsPosRecord *SymbolPos;
...
//  Instance=(InstanceRecord *)&((*Instances)[InstanceNr]);
  ...
  if (SymbolNr==-1) {
    ...
    if (NrObjects==MaxNrObjects-1) {
      if (AllocateMemObjects(MaxNrObjects+512,1)==0) NrObjects++;
    } else NrObjects++;
    Object=&((*Objects)[NrObjects]);
    ...
    switch (Rotation) {
      case 0:
        if ((ObjectMirrorX==1)
          ...
          if (ObjectMirrorX==1) {
            x1=-x1;
            if (PinTextRotation==0) Alignment=TextMirrorX[Alignment];
            if (PinTextRotation==1) Alignment=TextMirrorY[Alignment];
          }
          if (ObjectMirrorY==1) {
            y1=-y1;
            if (PinTextRotation==0) Alignment=TextMirrorY[Alignment];
            if (PinTextRotation==1) Alignment=TextMirrorX[Alignment];
          }
        }
        break;
      case 1:  //  90
...
//          if (PinTextRotation==1) Alignment=TextMirrorY[Alignment];
    ...
    if (((Pin->NameInfo & TEXT_NOT_VISIBLE)==0)
       ||
       ((Mode & 1)==1)) {
      Object=&((*Objects)[NrObjects]);
      if ((Mode & 1)==0) {
...
//    FontHeight=Pin->FontHeight;
    if (((PinBus->NameInfo & TEXT_NOT_VISIBLE)==0)
       ||
       ((Mode & 1)==1)) {
      FontNr=0;
...
{
  int32  cnt,cnt2,cnt3,lengte,NrLines,AddMode,Length,
         LinePos[16],TextAlignment,TextRotation,Changed;
  char   PinBusStrings[16][64],TextString[1024],TextStr[1024],
         str[MAX_LENGTH_STRING];
  ...
  if (!EditingSymbol) {
    ...
    for (cnt=0;cnt<Design.NrNetLabels;cnt++) {
      ...
      if ((NetLabel->Info & OBJECT_NOT_VISIBLE) == 0) {
        ...
        GetMinMaxText(NetLabel->ConnectX+NetLabel->TextX,NetLabel->ConnectY+NetLabel->TextY,
                      1.0,0,TextRotation,TextAlignment,NetLabel->Name);
        MinX2=min(MinX2,TextMinX);
        MaxX2=max(MaxX2,TextMaxX);
        MinY2=min(MinY2,TextMinY);
        MaxY2=max(MaxY2,TextMaxY);
        ...
        while (cnt3<Length+1) {
          if ((TextStr[cnt3]=='\r')
            ...
            if (cnt3-cnt2>0) {
              ...
              if ((Rotation2==0.0)
                 ||
                 (InRangeSpecial(Rotation2,90.0,0.01))) {
                if (Rotation2==0.0) {
...

{
  ...
  if (InRange(x1,x2)) {
    ...
    if (D>=0) {
      y11=(-B+sqrt(D))/2.0;
      y22=(-B-sqrt(D))/2.0;
      if ((X1SmallerThenX2(y11,max(y1,y2)))
         &&
         (X1GreaterThenX2(y11,min(y1,y2)))) return 1;
      if ((X1SmallerThenX2(y22,max(y1,y2)))
         &&
         (X1GreaterThenX2(y22,min(y1,y2)))) return 1;
    }
    return 0;
  }
  ...
  if (InRange(y1,y2)) {
    ...
    if (D>=0) {
      x11=(-B+sqrt(D))/2.0;
      x22=(-B-sqrt(D))/2.0;
      if ((X1SmallerThenX2(x11,max(x1,x2)))
         &&
         (X1GreaterThenX2(x11,min(x1,x2)))) return 1;
      if ((X1SmallerThenX2(x22,max(x1,x2)))
         &&
         (X1GreaterThenX2(x22,min(x1,x2)))) return 1;
    }
    return 0;
  }
...
//  LineCrossY=-1000000.0;
  ...
  if (InRange(LineX2,LineX1)) {
    if (InRange(LineY3,LineY4)) {
      ...
      if (InRange(LineX4,LineX3)) return 0;
...
{
  if ((X1GreaterThenX2(min(x1,x2),max(x3,x4)))
     ||
     (X1GreaterThenX2(min(y1,y2),max(y3,y4)))
     ||
     (X1SmallerThenX2(max(x1,x2),min(x3,x4)))
     ||
     (X1SmallerThenX2(max(y1,y2),min(y3,y4)))) return 0;
...
//  All angle bus
    ...
    if (mode==0) {
      if (TestLineConnectedToCircle(x1,y1,x2,y2,x3,y3,0.04)) return 1;
      if (TestLineConnectedToCircle(x1,y1,x2,y2,x4,y4,0.04)) return 1;
      if (TestLineConnectedToCircle(x3,y3,x4,y4,x1,y1,0.04)) return 1;
      if (TestLineConnectedToCircle(x3,y3,x4,y4,x2,y2,0.04)) return 1;
    } else {
      if (LineCrosses(x1,y1,x2,y2,x3,y3,x4,y4)) return 1;
    }
    return 0;
  }
  if (InRange(y1,y2)) {
...
// *************************************************************************************
    ...
    if (InRange(y3,y4)) return 1;   //  Horizontal bus
...

{
  ...
  if ((Instance->Info & (OBJECT_NOT_VISIBLE|OBJECT_PROTECTED)) != 0) return -1;

  if ((EditingSymbol)
     &&
     (!EditingSheetSymbol)) Check=1;

  if ((!EditingSymbol)
     &&
     ((Instance->Info & SHEET_SYMBOL) == 0)
     &&
     ((Instance->RefInfo & TEXT_NOT_VISIBLE) == 0)) Check=1;
...
//        if (TextRotation==1) Alignment=TextMirrorX[Alignment];
  ...
  GetMinMaxText(Instance->OriginX+x1,Instance->OriginY+y1,
                1.0,0,PinTextRotation,Alignment,Instance->Reference);
  return 0;
}
...

{
  ...
  if (EditingSymbol) Check=1;
  if ((Instance->ValueInfo & TEXT_NOT_VISIBLE) == 0) Check=1;
...
//        if (TextRotation==1) Alignment=TextMirrorX[Alignment];
  ...
  GetMinMaxText(Instance->OriginX+x1,Instance->OriginY+y1,
                1.0,0,PinTextRotation,Alignment,Instance->Value);
  return 0;
}
...

{
  ...
  if ((X1SmallerThenX2(LineXmax,SearchMinX))
     ||
     (X1GreaterThenX2(LineXmin,SearchMaxX))
     ||
     (X1SmallerThenX2(LineYmax,SearchMinY))
     ||
     (X1GreaterThenX2(LineYmin,SearchMaxY))) return 0;


  if ((X1SmallerThenX2(LineXmax,SearchMaxX))
     &&
     (X1GreaterThenX2(LineXmin,SearchMinX))
     &&
     (X1SmallerThenX2(LineYmax,SearchMaxY))
     &&
     (X1GreaterThenX2(LineYmin,SearchMinY))) return 3;


  if ((X1SmallerThenX2(LineX1,SearchMaxX))
     &&
     (X1GreaterThenX2(LineX1,SearchMinX))
     &&
     (X1SmallerThenX2(LineY1,SearchMaxY))
     &&
     (X1GreaterThenX2(LineY1,SearchMinY))) return 1;

  if ((X1SmallerThenX2(LineX2,SearchMaxX))
     &&
     (X1GreaterThenX2(LineX2,SearchMinX))
     &&
     (X1SmallerThenX2(LineY2,SearchMaxY))
     &&
     (X1GreaterThenX2(LineY2,SearchMinY))) return 2;

  if ((InRange(LineX1,LineX2))
     ||
     (InRange(LineY1,LineY2))) return 3;
...
{
  double  Rect1Xmin,Rect1Xmax,Rect1Ymin,Rect1Ymax,
          Rect2Xmin,Rect2Xmax,Rect2Ymin,Rect2Ymax;
...
// #define X1GreaterThenX2(x1,x2) ( (x1>x2+0.01) ? (1) : (0) )
  ...
  if ((X1SmallerThenX2(Rect1Xmax,Rect2Xmin))
     ||
     (X1SmallerThenX2(Rect1Ymax,Rect2Ymin))
     ||
     (X1GreaterThenX2(Rect1Xmin,Rect2Xmax))
     ||
     (X1GreaterThenX2(Rect1Ymin,Rect2Ymax))) return 0;

  if ((NotInRange(Line1X1,Line1X2))
     &&
     (NotInRange(Line1Y1,Line1Y2))) return 0;

  if ((NotInRange(Line2X1,Line2X2))
     &&
     (NotInRange(Line2Y1,Line2Y2))) return 0;

  if ((InRange(Line1X1,Line1X2))
     &&
     (InRange(Line2X1,Line2X2))) return 1;

  if ((InRange(Line1Y1,Line1Y2))
     &&
     (InRange(Line2Y1,Line2Y2))) return 1;
    ...
    if ((RedefinedPinBus->Info & OBJECT_NOT_VISIBLE) == 0) {
      if ((stricmpUTF8(RedefinedPinBus->Reference,Instance->Reference)==0)
        ...
        for (cnt2=0;cnt2<PinBusObject->Info3;cnt2++) {
          ...
          if (res>LineNr) strcat(str,"\\");
          else
            if (cnt2>0) strcat(str,",");
          strcat(str,str2);
          LineNr=res;
        }
...

{
  ...
  *Distance=length;
  if (InRangeSpecial(x,0.0,0.01)) { // Vertical line
    if (y>0.0) {  // Vertical line to top
      hoek=ANGLE_90;
    } else {      // Vertical line to bottom
      hoek=ANGLE_270;
    }
  } else {
    r1=y/x;
    hoek=atan(r1);
    if (r1>0.0) {
      if (x<0.0) hoek+=ANGLE_180;
    } else {
      if (x>0.0) hoek+=ANGLE_360;
      else hoek+=ANGLE_180;
    }
  }
  if (hoek>ANGLE_360-0.001) hoek-=ANGLE_360;
  *Angle=(hoek*180.0/PI);
}
...
*/
  ...
  if (LineX2==LineX1) {
    ...
    if ((py>=miny)
       ...
       (py<=maxy)
       &&
       (DistCentre<DistLine1)
       &&
       (DistCentre<DistLine2)) return DistCentre;
...

{
  ...
  for (cnt=0;cnt<min(MaxNrNetLabels,Design.NrNetLabels);cnt++) {
    ...
    if ((NetLabel->Info & OBJECT_NOT_VISIBLE) == 0) {
      ...
      if ((InRangeSpecial(x1,x2,0.01))
         &&
         (InRangeSpecial(y1,y2,0.01))) {
        return cnt;
      }
    }
  }
...

{
  ...
  for (cnt=0;cnt<Design.NrWires;cnt++) {
    Wire=&((*Wires)[cnt]);
    if ((Wire->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      if ((InRange(x,Wire->X1))
         &&
         (InRange(y,Wire->Y1))) {
        count++;
      }
      if ((InRange(x,Wire->X2))
         &&
         (InRange(y,Wire->Y2))) {
        count++;
      }
