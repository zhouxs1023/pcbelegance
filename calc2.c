...

{
  ...
  if (hulp<=((CircleThickness1+CircleThickness2)*0.5)) return 1;
  return 0;
}
...
{
  double  CircleXmin,CircleXmax,CircleYmin,CircleYmax,
         RectXmin,RectXmax,RectYmin,RectYmax,xa,ya;
  ...
  if ((CircleXmax<RectXmin)
     ||
     (CircleXmin>RectXmax)
     ||
     (CircleYmax<RectYmin)
     ||
     (CircleYmin>RectYmax)) return 0;

  if ((CircleXmax<=RectXmax)
     &&
     (CircleXmin>=RectXmin)
     &&
     (CircleYmax<=RectYmax)
     &&
     (CircleYmin>=RectYmin)) return 1;
...
//  RectTestCircleOutline(x1,y1,x2,255);
  ...
  if (rxmin>=0) {
    ...
    if (((CircleY-ya>=RectYmin)
       &&
       (CircleY-ya<=RectYmax))
       ||
       ((CircleY+ya>=RectYmin)
       &&
       (CircleY+ya<=RectYmax))) return 1;
  }
  if (rxmax>=0) {
  ...
  if (rymin>=0) {
    ...
    if (((CircleX-xa>=RectXmin)
       &&
       (CircleX-xa<=RectXmax))
       ||
       ((CircleX+xa>=RectXmin)
       &&
       (CircleX+xa<=RectXmax))) return 1;
  }
  if (rymax>=0) {
...
{
  int32 SymbolNr,cnt2,InstanceRotation,ObjectMirrorX,ObjectMirrorY,
        TextRotation,Alignment,MemPos,lengte;
