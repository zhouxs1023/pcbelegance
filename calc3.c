...
// ***************************************************************************************
  ...
  for (cnt=0;cnt<Design.NrComps;cnt++) {
    ...
    if ((Comp->Info & OBJECT_NOT_VISIBLE) == 0) {
      ...
      if ((Object->maxx>=Comp->BoardPosMinX-0.1e5)
         &&
         (Object->minx<=Comp->BoardPosMaxX+0.1e5)
         &&
         (Object->maxy>=Comp->BoardPosMinY-0.1e5)
         &&
         (Object->miny<=Comp->BoardPosMaxY+0.1e5)) {
...
// *******************************************************************************************************
  for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
    ...
    if (((ObjectLine->Info & (OBJECT_NOT_VISIBLE)) == 0)
      ...
      if ((max(xx1,xx2)+yy2a>=Object->minx)
         &&
         (min(xx1,xx2)-yy2a<=Object->maxx)
         &&
         (max(yy1,yy2)+yy2a>=Object->miny)
         &&
         (min(yy1,yy2)-yy2a<=Object->maxy)) {
...
// *******************************************************************************************************
  for (cnt=0;cnt<Design.NrObjectRects;cnt++) {
    ...
    if (((ObjectRect->Info & (OBJECT_NOT_VISIBLE)) == 0)
       ...
       ((ObjectRect->Layer<32)
       ||
       ((ObjectRect->Layer>=ROUTING_KEEPOUT_LAYER)
       &&
      ...
      if ((xx1+xx2a>=Object->minx)
         &&
         (xx1-xx2a<=Object->maxx)
         &&
         (yy1+yy2a>=Object->miny)
         &&
         (yy1-yy2a<=Object->maxy)) {
        ...
        if ((ObjectRect->Layer>=ROUTING_KEEPOUT_LAYER)
           &&
           (ObjectRect->Layer<ROUTING_KEEPOUT_LAYER+32)) {
...
// *******************************************************************************************************
  for (cnt=0;cnt<Design.NrObjectArcs;cnt++) {
    ...
    if (((ObjectArc->Info & (OBJECT_NOT_VISIBLE)) == 0)
       ...
       ((ObjectArc->Layer<32)
       ||
       ((ObjectArc->Layer>=ROUTING_KEEPOUT_LAYER)
       &&
      ...
      if ((xx1+yy2a>=Object->minx)
...
#ifdef _DEBUG
        if ((InRange9(CheckObject.x1,182.9e5))
           &&
           (InRange9(CheckObject.y1,-6.6e5))) {
          ok=1;
        }
#endif
        CheckObject.Thickness=Thickness;
        ...
        if ((ObjectArc->Layer==DRILL_LAYER)
          ...
          if ((ObjectArc->Layer>=ROUTING_KEEPOUT_LAYER)
             &&
             (ObjectArc->Layer<ROUTING_KEEPOUT_LAYER+32)) {
  ...
  if (mode==1) {
    for (cnt=0;cnt<Design.NrAreaFills;cnt++) {
      ...
      if ((AreaFill->Info & (OBJECT_NOT_VISIBLE|POWERPLANE)) == 0) {
        if ((AreaFill->NetNr!=ObjectNetNr)
           &&
           ((Object->Layer==-1) || (AreaFill->Layer==Object->Layer))) {
          MakePolygonFromObject(Object,PolygonObject,AreaFill->Clearance,0.0,1,0);
          if ((PolygonObject->minx<AreaFill->maxx)
             &&
             (PolygonObject->miny<AreaFill->maxy)
             &&
             (PolygonObject->maxx>AreaFill->minx)
             &&
             (PolygonObject->maxy>AreaFill->miny)) {
...

{
  ...
  double ObjectClearance,TestClearance,xx1,yy1,xx2,yy2,xx2a,yy2a,
         CheckObjectX2,CheckObjectY2;
  ...
  for (cnt=0;cnt<NrCheckObjects;cnt++) {
    ...
    if ((ObjectToCheck->NetNr!=Object->NetNr)
       &&
       ((ObjectToCheck->Layer==-1)
       ||
       (ObjectLayer==-1)
       ||
       (ObjectLayer==ObjectToCheck->Layer))) {
...
#ifdef _DEBUG
      if ((InRange9(ObjectToCheck->x1,158.75e5))
         &&
         (InRange9(ObjectToCheck->y1,101.66e5))) {
        ok=1;
      }
#endif
...
// *******************************************************************************************************
      switch (ObjectToCheck->ObjectType & ~1) {
        ...
        case OBJECT_ARC:
          ...
          if ((xx1+xx2+yy2a>=Object->minx)
             &&
             (xx1-xx2-yy2a<=Object->maxx)
             &&
             (yy1+yy2+yy2a>=Object->miny)
             &&
             (yy1-yy2-yy2a<=Object->maxy)) {
            if (ObjectsConnected(ObjectToCheck,&CheckObject)) {
              return cnt;
            }
          }
...

{
  ...
  for (cnt=0;cnt<Design.NrAreaFills;cnt++) {
    ...
    if (((AreaFill->Info & (OBJECT_NOT_VISIBLE|POWERPLANE)) == POWERPLANE)
       &&
       (AreaFill->Layer==Layer)) Found=cnt;
  }
  if (Found!=-1) return 1;
  return 0;
}
...

{
  ...
  if (Layer==0) {
    switch (mode & 15) {
      case 0:
        sprintf(TextStr,SC(3,"Layer %i (Bottom)"),Layer2);
        break;
      case 1:
        sprintf(TextStr,SC(4,"Layer %i\t\tBottom"),Layer2);
        break;
      case 2:
        sprintf(TextStr,SC(5,"Layer %i\tBottom"),Layer2);
        break;
      case 3:
        sprintf(TextStr,SC(6,"(Bottom)"));
        break;
      case 4:
        sprintf(TextStr,SC(7,"Bottom %d"),Layer2);
        break;
      case 5:
      case 6:
        sprintf(TextStr,SC(8,"Bottom layer %d"),Layer2);
        break;
      case 7:
        break;
      case 8:
        sprintf(TextStr,SC(9,"Bottom_%d"),Layer2);
        break;
    }
    return 0;
  }
  if (Design.NrBoardLayers>1) {
    if (Layer==Design.NrBoardLayers-1) {
      switch (mode & 15) {
        case 0:
          sprintf(TextStr,SC(10,"Layer %i (Top)"),Layer2);
          break;
        case 1:
          sprintf(TextStr,SC(11,"Layer %i\t\tTop"),Layer2);
          break;
        case 2:
          sprintf(TextStr,SC(12,"Layer %i\tTop"),Layer2);
          break;
        case 3:
          sprintf(TextStr,SC(13,"(Top)"));
          break;
        case 4:
          sprintf(TextStr,SC(14,"Top %d"),Layer2);
          break;
        case 5:
        case 6:
          sprintf(TextStr,SC(15,"Top layer %d"),Layer2);
          break;
        case 7:
          break;
        case 8:
          sprintf(TextStr,SC(16,"Top_%d"),Layer2);
          break;
      }
      return 0;
    }
  ...
  if ((Design.NrBoardLayers>2)
    ...
    if (Found!=-1) {
      ...
      switch (mode & 15) {
        case 0:
          sprintf(TextStr,SC(17,"Layer %i (Powerplane %s)"),Layer2,Net->Name);
          break;
        case 1:
          sprintf(TextStr,SC(18,"Layer %i\t\tPowerplane %s"),Layer2,Net->Name);
          break;
        case 2:
          sprintf(TextStr,SC(19,"Layer %i\tPowerplane %s"),Layer2,Net->Name);
          break;
        case 3:
          sprintf(TextStr,SC(20,"(Inner layer %i [ Powerplane %s ])"),Layer2,Net->Name);
          break;
        case 4:
          sprintf(TextStr,SC(21,"Inner%i"),Layer2);
          break;
        case 5:
          sprintf(TextStr,SC(22,"Powerplane (%s) layer %i"),Net->Name,Layer2);
          break;
        case 6:
          sprintf(TextStr,SC(23,"Inner layer %i ( Powerplane %s )"),Layer2,Net->Name);
          break;
        case 8:
          sprintf(TextStr,SC(24,"Inner_%i"),Layer2);
          break;
      }
    } else {
      switch (mode & 15) {
        ...
        case 1:
        case 2:
          sprintf(TextStr,SC(25,"Layer %i"),Layer2);
          break;
        case 3:
          sprintf(TextStr,SC(26,"(Inner layer %i)"),Layer2);
          break;
        case 4:
          sprintf(TextStr,SC(21,"Inner%i"),Layer2);
          break;
        case 5:
        case 6:
          sprintf(TextStr,SC(27,"Inner layer %i"),Layer2);
          break;
        case 7:
          break;
        case 8:
  ...
  switch (Layer) {
    case SILKSCREEN_TOP:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(28,"Silkscreen top"));
          break;
        case 1:
          strcpy(LayerText,SC(29,"Silkscreen\tTop"));
          break;
        case 5:
          strcpy(LayerText,SC(30,"SilkscreenTop"));
          break;
        case 4:
          strcpy(LayerText,SC(31,"Silkscreen top layer"));
          break;
        case 2:
          break;
      }
    ...
    case SILKSCREEN_TOP_REFS:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(1181,"Silkscreen top refs)"));
          break;
        case 1:
          strcpy(LayerText,SC(1189,"Silkscreen\tTop\tRefs"));
          break;
        case 5:
          strcpy(LayerText,SC(1183,"SilkscreenTopRefs"));
          break;
        case 4:
          strcpy(LayerText,SC(31,"Silkscreen top layer"));
          break;
    ...
    case SILKSCREEN_TOP_VALUES:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(1182,"Silkscreen top (values)"));
          break;
        case 1:
          strcpy(LayerText,SC(1190,"Silkscreen\tTop\tValues"));
          break;
        case 5:
          strcpy(LayerText,SC(1184,"SilkscreenTopValues"));
          break;
        case 4:
          strcpy(LayerText,SC(31,"Silkscreen top layer"));
          break;
    ...
    case SILKSCREEN_BOTTOM:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(32,"Silkscreen bottom"));
          break;
        case 1:
          strcpy(LayerText,SC(33,"Silkscreen\tBottom"));
          break;
        case 5:
          strcpy(LayerText,SC(34,"SilkscreenBottom"));
          break;
        case 4:
          strcpy(LayerText,SC(35,"Silkscreen bottom layer"));
          break;
        case 2:
          break;
      }
    ...
    case SILKSCREEN_BOTTOM_REFS:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(1185,"Silkscreen bottom (refs)"));
          break;
        case 1:
          strcpy(LayerText,SC(1191,"Silkscreen\tBottom\tRefs"));
          break;
        case 5:
          strcpy(LayerText,SC(1187,"SilkscreenBottomRefs"));
          break;
        case 4:
          strcpy(LayerText,SC(35,"Silkscreen bottom layer"));
          break;
    ...
    case SILKSCREEN_BOTTOM_VALUES:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(1186,"Silkscreen bottom (values)"));
          break;
        case 1:
          strcpy(LayerText,SC(1192,"Silkscreen\tBottom\tValues"));
          break;
        case 5:
          strcpy(LayerText,SC(1188,"SilkscreenBottomValues"));
          break;
        case 4:
          strcpy(LayerText,SC(35,"Silkscreen bottom layer"));
          break;
    ...
    case INFO_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 5:
          strcpy(LayerText,SC(36,"Info1"));
          break;
        case 1:
          strcpy(LayerText,SC(37,"Info layer 1"));
          break;
        case 2:
          break;
        case 3:
          strcpy(LayerText,SC(38,"Info objects 1"));
          break;
        case 4:
          strcpy(LayerText,SC(37,"Info layer 1"));
          break;
    ...
    case INFO_LAYER2:
      switch (mode & 0x0F) {
        case 0:
        case 5:
          strcpy(LayerText,SC(39,"Info2"));
          break;
        case 1:
        case 4:
          strcpy(LayerText,SC(40,"Info layer 2"));
          break;
        case 2:
          break;
        case 3:
          strcpy(LayerText,SC(41,"Info objects 2"));
          break;
      }
      break;
    case INFO_LAYER3:
      switch (mode & 0x0F) {
        case 0:
        case 5:
          strcpy(LayerText,SC(42,"Info3"));
          break;
        case 1:
        case 4:
          strcpy(LayerText,SC(43,"Info layer 3"));
          break;
        case 2:
          break;
        case 3:
          strcpy(LayerText,SC(44,"Info objects 3"));
          break;
      }
      break;
    case INFO_LAYER4:
      switch (mode & 0x0F) {
        case 0:
        case 5:
          strcpy(LayerText,SC(45,"Info4"));
          break;
        case 1:
        case 4:
          strcpy(LayerText,SC(46,"Info layer 4"));
          break;
        case 2:
          break;
        case 3:
          strcpy(LayerText,SC(47,"Info objects 4"));
          break;
      }
      break;
    case PLACEMENT_OUTLINE_TOP:
      switch (mode & 0x0F) {
        ...
        case 1:
        case 3:
          strcpy(LayerText,SC(48,"Placement outline top"));
          break;
        case 4:
          strcpy(LayerText,SC(49,"Placement outline top layer"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(50,"PlacementOutlineTop"));
          break;
      }
      break;
    case PLACEMENT_OUTLINE_BOTTOM:
      switch (mode & 0x0F) {
        ...
        case 1:
        case 3:
          strcpy(LayerText,SC(1122,"Placement outline bottom"));
          break;
        case 4:
          strcpy(LayerText,SC(1123,"Placement outline bottom layer"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(1124,"PlacementOutlineBottom"));
          break;
      }
      break;
    case COMP_OUTLINE_LAYER:
      switch (mode & 0x0F) {
        ...
        case 1:
        case 3:
          strcpy(LayerText,SC(51,"Component outline top"));
          break;
        case 2:
          break;
        case 4:
          strcpy(LayerText,SC(52,"Component outline layer"));
          break;
        case 5:
          strcpy(LayerText,SC(53,"ComponentOutlineTop"));
          break;
      }
      break;
    case COMP_OUTLINE_LAYER+1:
      switch (mode & 0x0F) {
        ...
        case 1:
        case 3:
          strcpy(LayerText,SC(54,"Component outline bottom"));
          break;
        case 2:
          break;
        case 4:
          strcpy(LayerText,SC(55,"Component outline bottom layer"));
          break;
        case 5:
          strcpy(LayerText,SC(56,"ComponentOutlineBottom"));
          break;
      }
      break;
    case COMP_REF_LAYER:
      switch (mode & 0x0F) {
        ...
        case 3:
        case 4:
          strcpy(LayerText,SC(57,"Component references"));
          break;
        case 2:
          break;
      }
    ...
    case COMP_VALUE_LAYER:
      switch (mode & 0x0F) {
        ...
        case 3:
        case 4:
          strcpy(LayerText,SC(58,"Component values"));
          break;
        case 2:
          break;
      }
    ...
    case BOARD_OUTLINE_LAYER:
      switch (mode & 0x0F) {
        ...
        case 1:
        case 3:
          strcpy(LayerText,SC(59,"Board outline"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(60,"BoardOutline"));
          break;
        case 4:
          strcpy(LayerText,SC(61,"Board outline layer"));
          break;
      }
      break;
    case BOARD_OUTLINE_KEEPOUT_LAYER:
      switch (mode & 0x0F) {
        ...
        case 1:
        case 3:
          strcpy(LayerText,SC(1119,"Board outline keepout"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(1118,"BoardOutline keepout"));
          break;
        case 4:
          strcpy(LayerText,SC(1119,"Board outline keepout"));
          break;
    ...
    case SOLD_MASK_BOTTOM:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(62,"Solder mask bottom"));
          break;
        case 1:
          strcpy(LayerText,SC(63,"Solder mask\tBottom"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(64,"SolderMaskBottom"));
          break;
        case 4:
          strcpy(LayerText,SC(65,"Solder mask bottom layer"));
          break;
      }
      break;
    case SOLD_MASK_TOP:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(66,"Solder mask top"));
          break;
        case 1:
          strcpy(LayerText,SC(67,"Solder mask\tTop"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(68,"SolderMaskTop"));
          break;
        case 4:
          strcpy(LayerText,SC(69,"Solder mask top layer"));
          break;
      }
      break;
    case PASTE_MASK_BOTTOM:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(70,"Paste mask bottom"));
          break;
        case 1:
          strcpy(LayerText,SC(71,"Paste mask\tBottom"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(72,"PasteMaskBottom"));
          break;
        case 4:
          strcpy(LayerText,SC(73,"Paste mask bottom layer"));
          break;
      }
      break;
    case PASTE_MASK_TOP:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(74,"Paste mask top"));
          break;
        case 1:
          strcpy(LayerText,SC(75,"Paste mask\tTop"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(76,"PasteMaskTop"));
          break;
        case 4:
          strcpy(LayerText,SC(77,"Paste mask top layer"));
          break;
      }
      break;
    case ASSEMBLY_BOTTOM_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(1312,"Assembly bottom"));
          break;
        case 1:
          strcpy(LayerText,SC(1313,"Assembly\tBottom"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(1314,"AssemblyBottom"));
          break;
        case 4:
          strcpy(LayerText,SC(1315,"Assembly bottom layer"));
          break;
      }
      break;
    case ASSEMBLY_TOP_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(1316,"Assembly top"));
          break;
        case 1:
          strcpy(LayerText,SC(1317,"Assembly\tTop"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(1318,"AssemblyTop"));
          break;
        case 4:
          strcpy(LayerText,SC(1319,"Assembly top layer"));
          break;
      }
      break;
    case BOTTOM_COMP_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(1333,"Components bottom"));
          break;
        case 1:
          strcpy(LayerText,SC(1334,"Components\tBottom"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(1335,"ComponentsBottom"));
          break;
        case 4:
          strcpy(LayerText,SC(1336,"Components bottom layer"));
          break;
      }
      break;
    case TOP_COMP_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 3:
          strcpy(LayerText,SC(1337,"Components top"));
          break;
        case 1:
          strcpy(LayerText,SC(1338,"Components\tTop"));
          break;
        case 2:
          break;
        case 5:
          strcpy(LayerText,SC(1339,"ComponentsTop"));
          break;
        case 4:
          strcpy(LayerText,SC(1340,"Components top layer"));
          break;
      }
      break;
    case DRILL_LAYER:
      switch (mode & 0x0F) {
        case 0:
          strcpy(LayerText,SC(78,"Drills"));
          break;
        case 1:
          strcpy(LayerText,SC(79,"Drill holes"));
          break;
        case 2:
          break;
        case 3:
          strcpy(LayerText,SC(80,"Drill holes (plated)"));
          break;
        case 4:
          strcpy(LayerText,SC(81,"Drill layer"));
          break;
        case 5:
          strcpy(LayerText,SC(82,"Drill"));
          break;
      }
      break;
    case DRILL_UNPLATED_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(83,"Drill unplated"));
          break;
        case 2:
          break;
        case 3:
          strcpy(LayerText,SC(84,"Drill holes (unplated)"));
          break;
        case 4:
          strcpy(LayerText,SC(85,"Drill unplated layer"));
          break;
        case 5:
          strcpy(LayerText,SC(86,"DrillUnplated"));
          break;
      }
      break;
    case VIA_DRILL_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(87,"Drill vias"));
          break;
        case 2:
          break;
        case 3:
    ...
    case SHAPE_PINS_BOTTOM:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(88,"Component pins bottom"));
          break;
        case 2:
          break;
        case 3:
          ...
          break;
        case 4:
          strcpy(LayerText,SC(1111,"Component pins bottom layer"));
          break;
      }
      break;
    case SHAPE_PINS_TOP:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(89,"Component pins top"));
          break;
        case 2:
          break;
        case 3:
          ...
          break;
        case 4:
          strcpy(LayerText,SC(1112,"Component pins top layer"));
          break;
      }
      break;
    case SHAPE_PINS_INNER:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(90,"Component pins inner"));
          break;
        case 2:
          break;
        case 3:
          ...
          break;
        case 4:
          strcpy(LayerText,SC(1113,"Component pins inner layers"));
          break;
      }
      break;
    case VIA_LAYER:
    ...
    case NET_PINS_LAYER:
      switch (mode & 0x0F) {
        ...
        case 3:
        case 4:
          strcpy(LayerText,SC(92,"Net component pins"));
          break;
      }
      break;
    case NET_PINS_LAYER2:
      switch (mode & 0x0F) {
        ...
        case 3:
        case 4:
          strcpy(LayerText,SC(93,"Net component pins powerplane"));
          break;
      }
      break;
    case CONNECTIONS_LAYER:
      switch (mode & 0x0F) {
        ...
        case 3:
        case 4:
          strcpy(LayerText,SC(94,"Guides (Connections)"));
          break;
      }
      break;
    case GRID_LAYER:
    ...
    case SWAP_GATE_LAYER:
      switch (mode & 0x0F) {
        ...
        case 3:
        case 4:
          strcpy(LayerText,SC(96,"Swappable gate pins"));
          break;
        case 2:
          break;
      }
    ...
    case SWAP_PINS_LAYER:
      switch (mode & 0x0F) {
        ...
        case 3:
        case 4:
          strcpy(LayerText,SC(97,"Swappable pins"));
          break;
        case 2:
          break;
      }
    ...
    case BACKGROUND_LAYER:
      switch (mode & 0x0F) {
        ...
        case 3:
        case 4:
          strcpy(LayerText,SC(98,"Background"));
          break;
      }
      break;
    case CLEARANCE_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(99,"Clearances"));
          break;
        case 2:
          break;
        case 3:
    ...
    case ERROR_LAYER:
      switch (mode & 0x0F) {
        ...
        case 1:
        case 2:
          strcpy(LayerText,SC(100,"Error"));
          break;
        case 3:
        case 4:
          strcpy(LayerText,SC(101,"Design rule errors"));
          break;
      }
      break;
    case WARNING_LAYER:
      switch (mode & 0x0F) {
        ...
        case 1:
        case 2:
          strcpy(LayerText,SC(102,"Warning"));
          break;
        case 3:
        case 4:
          strcpy(LayerText,SC(103,"Design rule warnings"));
          break;
      }
      break;
    case BUTTON_INFO_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(104,"Button info"));
          break;
        case 2:
          break;
        case 3:
    ...
    case POLYLINE_LAYER:
      switch (mode & 0x0F) {
        ...
        case 2:
        case 3:
          strcpy(LayerText,SC(105,"Polyline"));
          break;
      }
      break;
    case UNCONNECTED_PADS_BOTTOM_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(106,"Unconnected pads bottom"));
          break;
        case 2:
          break;
        case 3:
          ...
          break;
        case 4:
          strcpy(LayerText,SC(1115,"Unconnected pads bottom layer"));
          break;
      }
      break;
    case UNCONNECTED_PADS_TOP_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(107,"Unconnected pads top"));
          break;
        case 2:
          break;
        case 3:
          ...
          break;
        case 4:
          strcpy(LayerText,SC(1116,"Unconnected pads top layer"));
          break;
      }
      break;
    case UNCONNECTED_PADS_INNER_LAYER:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(108,"Unconnected pads inner"));
          break;
        case 2:
          break;
        case 3:
          ...
          break;
        case 4:
          strcpy(LayerText,SC(1117,"Unconnected pads inner layers"));
          break;
      }
      break;
    case ROUTING_KEEPOUT_INNER:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(109,"Routing keepout inner layers"));
          break;
        case 2:
          break;
        case 3:
        case 4:
          strcpy(LayerText,SC(1114,"Routing keepout inner layers"));
          break;
      }
      break;
    case ROUTING_KEEPOUT_TOP:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(123,"Routing keepout top layer"));
          break;
        case 2:
          break;
        case 3:
    ...
    case ROUTING_KEEPOUT_BOTTOM:
      switch (mode & 0x0F) {
        case 0:
        case 1:
          strcpy(LayerText,SC(122,"Routing keepout bottom layer"));
          break;
        case 2:
          break;
        case 3:
      ...
      switch (mode & 0x0F) {
        case 0:
          if (Layer<32) {
            ...
            if ((Layer>=ROUTING_KEEPOUT_LAYER)
               &&
               (Layer<ROUTING_KEEPOUT_LAYER+32)) {
              if ((mode & 0x10)==0) {
              ...
              if (Layer==ROUTING_KEEPOUT_LAYER) {
                sprintf(LayerText,SC(110,"Routing keepout bottom %d"),NewLayer);
              }
              if (Design.NrBoardLayers>1) {
                if (Layer==ROUTING_KEEPOUT_LAYER+Design.NrBoardLayers-1) {
                  sprintf(LayerText,SC(111,"Routing keepout top %d"),NewLayer);
                }
              }
              if (CheckIfInnerLayer(Layer-ROUTING_KEEPOUT_LAYER)) {
                sprintf(LayerText,SC(112,"Routing keepout inner %d"),NewLayer);
              }
            }
          }
          break;
        case 1:
          if (Layer<32) {
            ...
            if ((Layer>=ROUTING_KEEPOUT_LAYER)
              ...
              if (Layer==ROUTING_KEEPOUT_LAYER) {
                sprintf(LayerText,SC(113,"Routing keepout\tBottom %d"),Layer-ROUTING_KEEPOUT_LAYER-1);
              }
              if (Design.NrBoardLayers>1) {
                if (Layer==ROUTING_KEEPOUT_LAYER+Design.NrBoardLayers-1) {
                  sprintf(LayerText,SC(114,"Routing keepout\tTop %d"),Layer-ROUTING_KEEPOUT_LAYER-1);
                }
              }
              if (CheckIfInnerLayer(Layer-ROUTING_KEEPOUT_LAYER)) {
                sprintf(LayerText,SC(115,"Routing keepout\tInner %d"),Layer-ROUTING_KEEPOUT_LAYER-1);
              }
            }
          }
          break;
        ...
        case 5:
          if (Layer<32) {
            ...
            if ((Layer>=ROUTING_KEEPOUT_LAYER)
              ...
              if (Layer==ROUTING_KEEPOUT_LAYER) {
                sprintf(LayerText,SC(1154,"RoutingKeepoutBottom_%d"),NewLayer);
              }
              if (Design.NrBoardLayers>1) {
                if (Layer==ROUTING_KEEPOUT_LAYER+Design.NrBoardLayers-1) {
                  sprintf(LayerText,SC(117,"RoutingKeepoutTop_%d"),NewLayer);
                }
              }
              if (CheckIfInnerLayer(Layer-ROUTING_KEEPOUT_LAYER)) {
                if ((mode & 0x10)==0) {
                ...
                sprintf(LayerText,SC(118,"RoutingKeepoutInner_%d"),NewLayer);
              }
            }
          }
          break;
        case 3:
          if (Layer<32) {
            ...
            if (Layer==0) {
              strcpy(LayerText,SC(119,"Traces bottom layer"));
            } else {
              if (Layer==Design.NrBoardLayers-1) {
                strcpy(LayerText,SC(120,"Traces top layer"));
              } else {
                sprintf(LayerText,SC(121,"Traces inner layer %d"),Layer);
              }
            }
          } else {
            if ((Layer>=ROUTING_KEEPOUT_LAYER)
        ...
        case 4:
          if (Layer<32) {
            ...
            if ((Layer>=ROUTING_KEEPOUT_LAYER)
              ...
              if (CheckIfInnerLayer(Layer-ROUTING_KEEPOUT_LAYER)) {
                sprintf(LayerText,SC(124,"Routing keepout inner layer %d"),NewLayer);
              }
            }
          }
          break;
...
int32 CheckLineCrossesCircle(double x1,double y1,
                             double x2,double y2,
                             double cx,double cy,
                             double ThickNess)

{
  ...
  if (x1!=x2) {
    a1=(y2-y1)/(x2-x1);
    if ((a1<-100000) || (a1>100000)) Vert12=1;
  } else {
    Vert12=1;
  }
  if (Vert12) {
    if (divx1>R2) return 0;
    if ((cy>min(y1,y2))
       &&
       (cy<max(y1,y2))) return 1;
    return 0;
  } else {
    if (y1==y2) {
      if (divy1>R2) return 0;
      if ((cx>min(x1,x2))
         &&
         (cx<max(x1,x2))) return 1;
      return 0;
    } else {
      if (fabs(a1)<0.001) {
        dx=cx;
...
int32 CheckFillingLine(double x1,double y1,
                       double x2,double y2,
                       double x3,double y3,
                       double x4,double y4,
...
//  NrCalcs2++;
  ...
  if (CheckLineCrossesCircle(x3,y3,x4,y4,x1,y1,ThickNess)==1) return -1;
...
//  NrTries--;
  ...
  while ((cnt<NrTries+1)
    ...
    if (0) {
      ...
      ellips2(Mult(xx-Xoffset)+DrawWindowMinX,DrawWindowMaxY-Mult(yy-Yoffset)-1,Mult(ThickNess),Mult(ThickNess),255);
      ExitDrawing();
...
int32 CheckLineCrossesWithAreaFill(double x1,double y1,
                                   double x2,double y2,
                                   AreaFillRecord *AreaFill,
                                   ...
                                   double Thickness,int32 mode)
{
  ...
  double *x3,*y3,*x4,*y4,StartX,StartY,FoundX,FoundY,Length,NewLength,NewX,NewY,R,
         minx12,maxx12,miny12,maxy12,*FirstX3,*FirstY3;
  ...
  *NewY2=1000000000;
  ...
  if ((Polygon->maxx<minx12)
     ||
     (Polygon->maxy<miny12)
     ||
     (Polygon->minx>maxx12)
     ||
     (Polygon->miny>maxy12)) LineStartError=1;

  PolygonPos=(uint8 *)Polygon;
  cnt=0;
  while ((!LineStartError)
        &&
        (cnt<AreaFill->NrPolygons)) {
...
#endif
  //      sprintf(str,"%i",cnt2);
  //      TextOut(OutputDisplay,Mult(x3-Xoffset)+DrawWindowMinX,DrawWindowMaxY-Mult(y3-Yoffset)-1,
  //              str,strlen(str));
        ...
        if ((max(*x3,*x4)>minx12)
          ...
          res=CheckFillingLine(x1,y1,x2,y2,*x3,*y3,*x4,*y4,
                               &NewX,&NewY,Thickness);
          if (res==-1) LineStartError=1;
          if (res==1) {
...
/*
int32 InfoObjectInAreaFill(ObjectRecord *Object,int32 Layer)

{
  AreaFillRecord *AreaFill;
  int32  cnt;

//  0 -> Object not in a AreaFill
//  1 -> Object in a AreaFill with different net -> Anti powerpad
//  2 -> Object in a AreaFill with the same net -> Thermal relief
//  3 -> Object in a AreaFill with the same net -> No thermal relief
  ...
  for (cnt=0;cnt<Design.NrAreaFills;cnt++) {
    ...
    if (((AreaFill->Info & (OBJECT_NOT_VISIBLE)) == 0)
      ...
      if (CheckPointInAreaFill(Object->x1,Object->y1,AreaFill)) {
        if (AreaFill->NetNr!=Object->NetNr) return 1;
        if (Object->ObjectType==VIA_PUT_THROUGH_ROUND) return 3;
        return 2;
      }
    }
  }
...
int32 CopyThermalReliefInPolygon(ObjectRecord *Object,PolygonRecord *Polygon,double ThickNess,
                                 double Pos,int32 MaxPos,int32 mode)
...
// ****************************************************************************
  switch (Object->ObjectType) {
    ...
    case PIN_LINE_DIAG2:
      ...
      if (x2>(200000)) NrSegments=8;
      if (x2>(600000)) NrSegments=16;
...
// ****************************************************************************
  ...
  NrSegments=4;
  if (Object->x2>(200000)) NrSegments=8;
  if (Object->x2>(600000)) NrSegments=16;
...

{
  ...
  switch (Object->ObjectType) {
    ...
    case PIN_LINE_VER:
      ...
      return ((RectTestRect2(x1,y1+(x2/2),y2,x2))
             ||
             (RectTestCircle(x1,y1,y2,255))
             ||
    ...
    case PIN_LINE_HOR:
      ...
      return ((RectTestRect2(x1+x2/2,y1,x2,y2))
             ||
             (RectTestCircle(x1,y1,y2,255))
             ||
    ...
    case PIN_LINE_DIAG1:
      ...
      return ((RectTestDiag1(x1,y1,x2,y2))
             ||
             (RectTestCircle(x1,y1,y2,255))
             ||
    ...
    case PIN_LINE_DIAG2:
      ...
      return ((RectTestDiag2(x1,y1,x2,y2))
             ||
             (RectTestCircle(x1,y1,y2,255))
             ||
             (RectTestCircle(x1+x2,y1+x2,y2,255)));
    ...
    case OBJECT_ARC:
      ...
      if ((SearchMaxX>PolygonObject->minx)
         &&
         (SearchMinX<PolygonObject->maxx)
         &&
         (SearchMaxY>PolygonObject->miny)
         &&
         (SearchMinY<PolygonObject->maxy)) {
        return 1;
      }
      break;
    case OBJECT_POLYGON:
    ...
    case PIN_PUT_THROUGH_POLYGON:
      if (CheckObjectIsBigPolygon(Object)) {
        ...
        if ((SearchMaxX>AreaFill->minx)
           &&
           (SearchMinX<AreaFill->maxx)
           &&
           (SearchMaxY>AreaFill->miny)
           &&
...
{
  int32 SegmentCount,LineSegments,cnt2,TextAlignment,Mirror,ok,FontNr,
        MaxCountX,NrLines;
...
void MakePolygonFromSpecialLine(double x1,double y1,double x2,double y2,
                                ObjectPolygonRecord *ObjectPolygon,double Thickness1,
                                double Thickness2,int32 mode)
...

{
  ...
  if ((Comp->TextVisibility & 0x100) != 0) return -1;
...

{
  ...
  if ((Comp->TextVisibility & 0x200) != 0) return -1;
...
int32 AdjustOffsetForSnapOnGerberObject(double CursorX,double CursorY,double GridX,double GridY,
    ...
    if ((Object->Info & SelectionMask1) == SelectionMask2) {
      switch (Object->ObjectType) {
        ...
        case PIN_LINE_DIAG2:
          ...
          if ((PointInPolygon(PolygonObject,CursorX,CursorY) & 1) == 1) { // Point within object snap it
...
int32 CreateTraceObjectFromSpecialLine(ObjectRecord *TraceObject,double StartX,double StartY,
                                       double Length,double Angle,double TraceWidth,int32 mode)
  ...
  if ((InRange(x1,x2))
    ...
    if (InRange3(x1,x2)) {                // Ver trace
      ...
      if (InRange3(y1,y2)) {              // Hor trace
        ...
        if (InRange3(x1-x2,y1-y2)) {      // diag2 trace
          ...
          if (InRange3(x1-x2,y2-y1)) {    // diag1 trace
...
void CreateTraceObjectFromTrace(TraceRecord *Trace,ObjectRecord *TraceObject,int32 ObjectType,
                                int32 Layer,int32 TraceNr,int32 mode)
...
int32 AdjustOffsetForSnapOnObject(ObjectRecord *Object,
                                  double CursorX,double CursorY,double GridX,double GridY,
                                  double divx,double divy,
                                  double *ShiftOffsetX,double *ShiftOffsetY,int32 mode)

{
  ...
  switch (Object->ObjectType) {
    ...
    case PIN_PUT_THROUGH_ROUND:
#ifdef _DEBUG
      if ((InRange9(Object->x1,-7e5))
         &&
         (InRange9(Object->y1,-5.7e5))) {
        ok=1;
      }
#endif
...

{
  ...
  if (CurrentDrawingLayer==-1) CurrentDrawingLayer=0;
  if (CurrentDrawingLayer>=Design.NrBoardLayers) CurrentDrawingLayer=0;
  ...
  if ((mode & 2)==2) {
    ...
    if ((Design.NrBoardLayers>2)
      ...
      cnt++;
      if (cnt==Design.NrBoardLayers) cnt=0;
    }
  } else {
  ...
  while (cnt2<Design.NrBoardLayers) {
    if (DrawLayerCode[cnt]<0x10) {
      if (cnt==0) {
        if ((mode & 1)==0) return cnt;
        if (OkToDrawBottomPads) return cnt;
      } else {
        if (cnt==Design.NrBoardLayers-1) {
          if ((mode & 1)==0) return cnt;
          if (OkToDrawTopPads) return cnt;
        } else {
          if ((mode & 1)==0) return cnt;
          if (OkToDrawInnerPads) return cnt;
        }
      }
    }
    if ((Design.NrBoardLayers>2)
...

{
  ...
  pos=0;
  while ((pos<sizeof(Comp->Properties)-2)
        &&
        (CompPropertiesBuf[pos]!=0)) {
    LengthIdent=strlen((LPSTR)&CompPropertiesBuf[pos]);
    if ((LengthIdent>0)
       &&
       (LengthIdent<64)
       &&
       (pos+LengthIdent<sizeof(Comp->Properties)-1)) {
      ...
      if ((LengthValue>0)
         ...
         (LengthValue<64)
         &&
         (pos2+LengthValue<sizeof(Comp->Properties)-1)) {
        switch (mode & 0x60) {
          case 0:
...

{
  ...
  pos=0;
  while ((pos<sizeof(Net->Properties)-2)
        &&
        (NetPropertiesBuf[pos]!=0)) {
    LengthIdent=strlen((LPSTR)&NetPropertiesBuf[pos]);
    if ((LengthIdent>0)
       &&
       (LengthIdent<64)
       &&
       (pos+LengthIdent<sizeof(Net->Properties)-1)) {
      pos2=pos+LengthIdent+1;
      ...
      if ((LengthValue>0)
         ...
         (LengthValue<64)
         &&
         (pos2+LengthValue<sizeof(Net->Properties)-1)) {
        switch (mode & 0x60) {
          case 0:
