...
int32 LineCrossesNew(double LineX1,double LineY1,double LineX2,double LineY2,
...
//  if (InRange7(LineX2,LineX1)) {
    if (InRange7(LineY3,LineY4)) {
      ...
      if (InRange7(LineX4,LineX3)) return -1;
      ...
      if ((y>maxy12)
         ||
         (y<miny12)) return -1;
      if (fabs(b)>1) {  // almost vertical line check y
        if ((y>maxy34)
           ||
           (y<miny34)) return -1;
      } else {          // almost vertical line check x
        if ((x>maxx34)
           ||
           (x<minx34)) return -1;
      }
    ...
    if (InRange7(LineX4,LineX3)) {
      if (InRange7(LineY1,LineY2)) {
        ...
        if ((y>maxy34)
           ||
           (y<miny34)) return -1;
        if (fabs(a)>1) {    // almost vertical line check y
          if ((y>maxy12)
             ||
             (y<miny12)) return -1;
        } else {            // almost vertical line check x
...
int32 LineCrosses2(double LineX1,double LineY1,double LineX2,double LineY2,
                   double LineX3,double LineY3,double LineX4,double LineY4,
                   double *CenterX,double *CenterY,int32 mode)
...

{
  ...
  *CenterY=0.0;
  if ((InRangeSpecial(Angle1,Angle2,0.001))
     ||
     (InRangeSpecial(Angle1,Angle2+ANGLE_180,0.001))
     ||
     (InRangeSpecial(Angle1+ANGLE_180,Angle2,0.001))) {
    return 0;
  }

  if (InRangeSpecial(LineX1,LineX2,0.00001)) { // Vertical line
    a2=(LineY4-LineY3)/(LineX4-LineX3);
    b2=LineY4-a2*LineX4;
    *CenterX=LineX1;
    *CenterY=LineX1*a2+b2;
  } else {
    if (InRangeSpecial(LineX3,LineX4,0.00001)) { // Vertical line
...

{
  ...
  if (CheckPolygonCompleetlyInsidePolygon(Polygon,&SearchPolygon)==1) return 1;
  return 0;
}
...

{
  ...
  if (CheckPolygonCompleetlyInsidePolygon(&NewPolygon,&SearchPolygon)==1) return 1;
  return 0;
}
...

{
  ...
  while (cnt<NrVertices) {
    ...
    if ((xmin<rxmax)
       ...
       &&
       (ymax>rymin)) {
      if ((res=LineCrosses(rxmin,rymax,rxmax,rymax,x1,y1,x2,y2))!=-1) crosses++;
      if ((res=LineCrosses(rxmin,rymin,rxmax,rymin,x1,y1,x2,y2))!=-1) crosses++;
      if ((res=LineCrosses(rxmin,rymin,rxmin,rymax,x1,y1,x2,y2))!=-1) crosses++;
      if ((res=LineCrosses(rxmax,rymin,rxmax,rymax,x1,y1,x2,y2))!=-1) crosses++;
      if (crosses>0) return 0;
    }
    cnt++;
  }
  if ((rxmin<Polygon->minx)
     &&
     (rymin<Polygon->miny)
     &&
     (rxmax>Polygon->maxx)
     &&
     (rymax>Polygon->maxy)) return 2;
  else return 1;
}
...
void MakePolygonFromObject(ObjectRecord *Object,PolygonRecord *PolygonObject,
                           double Clearance,double Pos,int32 MaxPos,int32 mode)
/*
...
{
  int32  count,res,cnt,cnt2,Step,CircleRoundings,NrSegments,Rotation,
         StartCnt,CircleRoundings2,MemSizePolygonObject2,Mirror;
  ...
  double  x1,y1,x2,y2,x3,y3,x4,y4,x5,y5,x6,y6,dikte,dikte2,diktex,diktey,
          NewClearance,Angle,StartAngle,AngleInc,AngleInc2,Thickness,Angle1,Angle2,
          Length1,Length2,Length,Width,Height,Width2,Height2,TempValue,
          AngleMiddle,
          Angle4,Angle5,cx1,cy1,cx2,cy2,ExtraAngleFactor,
          Thickness2,sinx,sinx2,CircleRoundingCompensation=1.0;
...
//      CircleRoundings++;
      ...
      if (NotInRange(x5,0.0)) {
        ...
        if (StartAngle==0.0) {
          if (x5<0.0) StartAngle+=ANGLE_180;
        } else {
          if (StartAngle<0.0) StartAngle+=ANGLE_180;
        }
        if (y2<y1) StartAngle+=ANGLE_180;
      } else {
        if (y2>y1) {
          StartAngle=ANGLE_90;
        } else {
...
// **************************************************************************************
    ...
    case PIN_ARC:
      ...
      if ((x2>(100*2540))
         ||
         (y2>(100*2540))) NrSegments=128;
      if ((x2>(200*2540))
         ||
         (y2>(200*2540))) NrSegments=256;
#ifdef _DEBUG
      if (x2>10e5) {
        ok=1;
      }
      ...
      x4=Object->x4;
      y4=Object->y4;
      if ((InRangeSpecial(x3,x4,100.0))
         &&
         (InRangeSpecial(y3,y4,100.0))) {
        count=NrSegments;
        Angle1=PI/2;
        Angle2=0.0;
...
/*
#ifdef _DEBUG
          sprintf(str,"real %.1f,%.1f   %.1f,%.1f\n",cx1,cy1,cx2,cy2);
          OutputDebugStr(str);
          sprintf(str,"real angles  %.4f -  %.4f\n",Angle1,Angle2);
          OutputDebugStr(str);
#endif
*/

          ExtraAngleFactor=Width/Height;
          if (ExtraAngleFactor<1.0) ExtraAngleFactor=1/ExtraAngleFactor;
          if (NrSegments==64) {
...
*/
      ...
      if ((InRange(x3,x4))
        ...
        ConvNormalCoorToPolar(x1,y1,x1+x3,y1+y3,&Angle1,&Length1);
        ConvNormalCoorToPolar(x1,y1,x1+x4,y1+y4,&Angle2,&Length2);
        if (Angle2<Angle1) Angle2+=ANGLE_360;
...

{
  ...
  if ((SrcPolygon->NrVertices<3)
     ||
     (SrcPolygon->NrVertices>100000)) {
    ok=1;
  }
#endif
...

{
  ...
  for (cnt2=0;cnt2<AreaFill->NrPolygons;cnt2++) {
    ...
    sprintf(str2,"Polygon %3i  %4i vertices  mempos %5i   %8.3f,%8.3f,%8.3f,%8.3f",
                 cnt2,Polygon->NrVertices,PolygonPos-PolygonPos2,
                 Polygon->minx/100000,
                 Polygon->miny/100000,
                 Polygon->maxx/100000,
                 Polygon->maxy/100000);
...

{
  ...
  for (cnt2=0;cnt2<AreaFill->NrPolygons-1;cnt2++) {
    ...
    if ((mode==0)
       ||
       ((DrawPolygon->PolygonType & 1) == 0)) {
      res=CheckPolygonCrossesOtherPolygon(PolygonObject,DrawPolygon);
      if (res==-1) return 1;  // Polygon lines crosses
    }
  }
...

{
  ...
  if (InRange(divy,0)) {
    if (X1SmallerThenX2(divx,0)) return 2;
    else return 6;
  }
  if ((InRange(divx,divy))
     &&
     (X1SmallerThenX2(divx,0))) return 1;
  if ((InRange(divx,divy))
     &&
     (X1GreaterThenX2(divx,0))) return 5;
  if ((InRange(divx,-divy))
     &&
     (X1SmallerThenX2(divx,0))) return 3;
  if ((InRange(divx,-divy))
     &&
     (X1GreaterThenX2(divx,0))) return 7;
  return -1;
}
...

{
  ...
  for (cnt=0;cnt<Polygon->NrVertices;cnt++) {
    if (((NotInRange8(*x1,*x2))
       ...
       ||
       (NotInRange8(*y2,*y3)))) {
      if ((cnt+1)==Polygon->NrVertices) return 0;
      return cnt+1;
    }
    x1+=2;
    y1+=2;
...
int32 ReorganizePolygon(PolygonRecord *Polygon,PolygonRecord *ResultingPolygon,
                        double TrimPointsDistance)

{
  int32  count,cnt,cnt2,cnt3,ok,AddPoint,MemSize;
  double *x1,*y1,*x3,*y3,LastX,LastY,*FirstX,*FirstY,
         px1,py1,px2,py2,xx1,yy1,xx2,yy2,xx3,yy3,LinesAngle,Length12,Length23,
         Angle1,Angle2,Angle,TrimLength;
  ...
  if ((cnt=GetStartPoint(Polygon))==-1) return 0;
...
// ******************************************************************************
  while (cnt3<count) {
    if ((NotInRange8(*x1,LastX))
       ||
       (NotInRange8(*y1,LastY))) {
  ...
  for (cnt=0;cnt<TempPolygon->NrVertices;cnt++) {
...
#ifdef _DEBUG
    if ((InRangeSpecial(xx2,75.234e5,0.02e5))
       &&
       (InRangeSpecial(yy2,86.497e5,0.02e5))) {
      ok=1;
    }
    ...
    if (LinesAngle>ANGLE_CONVERT(180.0)) LinesAngle=ANGLE_CONVERT(360.0)-LinesAngle;
...
// **************************************************************************************
  ...
  if ((mode & 2) == 2) {
    ...
    sprintf(str,"1");
    TextOut(OutputDisplay,MultX(x1),MultY(y1),str,strlen(str));
    sprintf(str,"2");
    TextOut(OutputDisplay,MultX(x2),MultY(y2),str,strlen(str));
    sprintf(str,"3");
    TextOut(OutputDisplay,MultX(x3),MultY(y3),str,strlen(str));
    sprintf(str,"1a");
    TextOut(OutputDisplay,MultX(*NewX1),MultY(*NewY1),str,strlen(str));
    sprintf(str,"2a");
    TextOut(OutputDisplay,MultX(*NewX2),MultY(*NewY2),str,strlen(str));
#endif
  }
  return c1*2+c2;
}
...
int32 MakeBiggerSmallerPolygon2(PolygonRecord *Polygon,
                                PolygonRecord *BiggerPolygon,
                                double Thickness,int32 mode)
...
//  }
  ...
  for (cnt=0;cnt<count;cnt++) {
    ...
    while ((res<0)
          ...
          (TryCount<10)) {
      if ((mode & 1) == 0) { // Bigger polygon
        MakePolygonFromObject(&Object,CutPolygon,0.0,
                             (-cnt-1.0-RandValue)-(TryCount*133.3),1,0);
      } else {               // Smaller polygon
        MakePolygonFromObject(&Object,CutPolygon,0.0,
                             (cnt+1.0+RandValue)+(TryCount*133.3),1,0);
      }
...
int32 MakeBiggerSmallerPolygon(PolygonRecord *Polygon,
                               PolygonRecord *BiggerPolygon,
                               double Thickness,int32 mode)
...
#define    InRange15(x1,x2) ( (((x1>x2-10000.0) && (x1<x2+10000.0))) ? (1) : (0) )
  ...
  res=GetBiggerSmallerPoints(x1,y1,x2,y2,x3,y3,OldX,OldY,
                             &NewX1,&NewY1,&NewX2,&NewY2,&OldX2,&OldY2,0.01e5,0);
//  res=GetBiggerSmallerPoints(x1,y1,x2,y2,x3,y3,OldX,OldY,
...

{
  ...
  count=Polygon->NrVertices;
  for (cnt3=0;cnt3<count;cnt3++) {
    RotatePointFromOtherPoint2(&(*Polygon).Points[cnt3].x,&(*Polygon).Points[cnt3].y,
...

{
  ...
  if (res==1) return 0; // polygon outside
  if (res==-1) return 1; // polygon lines crossing
  if (res==-2) return 1; // polygon lines crossing
...

{
  ...
  while (cnt<NrLinePoints) {
    if ((InRangeSpecial(x,(*LinePoints)[cnt].x,5000.0))
       &&
       (InRangeSpecial(y,(*LinePoints)[cnt].y,5000.0))) {
      return cnt;
    }
    cnt++;
  }
...

{
  ...
  for (cnt=0;cnt<NrObjects2;cnt++) {
    ...
    if ((Object->Info & Mask)==0) {
      if ((InRangeSpecial(x,x1,5000.0))
         &&
         (InRangeSpecial(y,y1,5000.0))) {
        *NewX=x2;
        *NewY=y2;
        return cnt;
      }
      if ((InRangeSpecial(x,x2,5000.0))
         &&
         (InRangeSpecial(y,y2,5000.0))) {
        *NewX=x1;
        *NewY=y1;
        return cnt;
      }
...

{
  ...
  int32  cnt,cnt2,cnt3,NrLinePointsNumbers,NrVertices,LineSegments,
         SegmentCount,NrPolygons,MainPolygonStartObjectNr;
...
//      NrVertices++;
      ...
      while ((NotInRange4(x1,x2))
        ...
        while ((cnt2<NrObjects2)
          ...
          if ((Object->Info & 1)==0) {
            if ((InRange4(x2,Object->x1))
               &&
               (InRange4(y2,Object->y1))) {
              Found2=1;
              x2=Object->x2;
              y2=Object->y2;
              Polygon->Points[NrVertices].x=x2;
              ...
              if ((InRange4(x2,Object->x2))
                 &&
                 (InRange4(y2,Object->y2))) {
                Found2=1;
                x2=Object->x1;
                y2=Object->y1;
                Polygon->Points[NrVertices].x=x2;
...
{
  double  x1,y1,x2,y2,x3,y3,x4,y4,Width,Height,Angle1,Angle2,Length1,Length2,
          sinx,sinx2,Length;
...
{
  double  x1,y1,x2,y2,x3,y3,x4,y4,Width,Height,Angle1,Angle2,
          Length,sinx,sinx2,Length1,Length2;
...

{
  ...
  switch (Object->ObjectType) {
    ...
    case PIN_SMD_POLYGON:
    case PIN_PUT_THROUGH_POLYGON:
      if (Object->ObjectType2!=0) return 0;
      GeomPolygon=(GeomPolygonRecord *)Object->Address;
      if (!GeomPolygon) return 0;
      count=GeomPolygon->NrVertices;
      if (count<500) {
        if (GeomPolygon->NrSubPolygons==0) return 0;
        if (GeomPolygon->NrVerticesMainPolygon==0) return 0;
      }
      return 1;
    default:
      return 0;
...
int32 GetAreaFillFromBigPolygonObject(ObjectRecord *Object,AreaFillRecord **NewAreaFill,double Clearance,int32 mode)
...
*/
  ...
  res=1;
  for (cnt3=0;cnt3<NrCachePolygonObjects;cnt3++) {
    if ((InRange(CachePolygonObjects[cnt3].X,Object->x1))
       &&
       (InRange(CachePolygonObjects[cnt3].Y,Object->y1))
       &&
...
#ifdef _DEBUG
  if ((InRangeSpecial(x2,176.377e5,0.005e5))
     &&
     (InRangeSpecial(y2,54.039e5,0.005e5))) {
    ok=1;
  }
#endif
  cnt3=cnt2;
