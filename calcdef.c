...

{
  ...
  if (max(Width,Height)>(double)(4.0)) NrSegments=64;
  if (max(Width,Height)>(double)(10.0)) NrSegments=128;
...

{
  ...
  dy=*y-OY;
  length=sqrt(SQR(dx)+SQR(dy));
  if (InRangeSpecial(dx,0.0,0.001)) { // Vertical line
    if (dy>0.0) { // Vertical line to top
      hoek=ANGLE_90;
    } else {      // Vertical line to bottom
      hoek=ANGLE_270;
...

{
  ...
  hulp=x/Grid;
  if (hulp>=(double)0.0) hulp+=(double)0.5;
  else hulp-=(double)0.5;
  niks=modf(hulp,&f);
