...

{
  switch (Units) {
    case UNITS_MILS:
      ...
      break;
    case UNITS_MM:
      strcpy(str,SC(131,"mm"));
      break;
    case UNITS_INCH:
      strcpy(str,SC(132,"inch"));
      break;
    case UNITS_HPGL:
      strcpy(str,SC(133,"HPGL"));
      break;
    case UNITS_0_01MM:
      strcpy(str,SC(134,"0.01 mm"));
      break;
    case UNITS_0_1MILS:
      strcpy(str,SC(135,"0.1 mils"));
      break;
    case UNITS_0_1MM:
      strcpy(str,SC(136,"0.1 mm"));
      break;
    case UNITS_MICRON:
      strcpy(str,SC(137,"Âµm"));
      break;
  }
...
*/
  switch (mode & 15) {
    case 0: // mils/mm/inch
      if (Units>=UNITS_0_01MM)  Units=UNITS_MILS;
      break;
    case 1: // mils/mm
      if (Units>=UNITS_INCH)    Units=UNITS_MILS;
      break;
    case 2: // mils/mm/inch/0.01mm/0.1mils
      if (Units==UNITS_HPGL)    Units=UNITS_0_1MILS;
      if (Units==UNITS_0_1MM)   Units=UNITS_MILS;
      if (Units==UNITS_MICRON)  Units=UNITS_MILS;
      break;
    case 3: // mils/mm/µm
      if (Units==UNITS_INCH)    Units=UNITS_MICRON;
      if (Units==UNITS_0_01MM)  Units=UNITS_MICRON;
      if (Units==UNITS_HPGL)    Units=UNITS_MICRON;
      if (Units==UNITS_0_1MILS) Units=UNITS_MICRON;
      if (Units==UNITS_0_1MM)   Units=UNITS_MICRON;
      break;
    case 4: // mils/mm/inch/0.01mm
      if (Units==UNITS_HPGL)    Units=UNITS_MILS;
      if (Units==UNITS_0_1MILS) Units=UNITS_MILS;
      if (Units==UNITS_0_1MM)   Units=UNITS_MILS;
      break;
    case 5: // mils/mm/inch/0.01mm/0.1mm
      if (Units==UNITS_HPGL)    Units=UNITS_0_1MM;
      if (Units==UNITS_0_1MILS) Units=UNITS_0_1MM;
      if (Units==UNITS_MICRON)  Units=UNITS_MILS;
      break;
    case 6: // mm/inch
      if (Units>UNITS_INCH) {
  ...
  if (Units==8) Units=UNITS_MILS;
  if (Units<0)  Units=UNITS_MILS;
  return Units;
}
...

{
  ...
  for (cnt=0;cnt<le;cnt++) {
    if (Str[cnt]=='.') PointCount++;
    if (Str[cnt]=='.') CommaCount++;
    if (Str[cnt]==';') PuntCommaCount++;
  }
  if (Str[0]=='@') {
...
//      res=sscanf((LPSTR)&SubStr,"%i",&ParamsInt[cnt]);
//      if (res==0) return -3;
    }
  }
  if (mode==0) {
...

{
  ...
  *Length=sqrt(SQR(dx)+SQR(dy));
  if (InRangeSpecial(dx,0.0,0.00001)) { // Vertical line
    if (dy>0.0) { // Vertical line to top
      hoek=ANGLE_90;
    } else {      // Vertical line to bottom
      hoek=ANGLE_270;
...

{
  ...
  ya=((SearchMinX-LineX1)*rico) + LineY1;
  yb=((SearchMaxX-LineX1)*rico) + LineY1;
  if (((ya>SearchMinY-Thickness)
     &&
     (ya<SearchMaxY+Thickness))
     ||
     ((yb>SearchMinY-Thickness)
     &&
     (yb<SearchMaxY+Thickness))) return 3;

  rico2=LineX2;
  rico2=(rico2-LineX1)/(LineY2-LineY1);
  xa=((SearchMinY-LineY1)*rico2) + LineX1;
  xb=((SearchMaxY-LineY1)*rico2) + LineX1;
  if (((xa>SearchMinX-Thickness)
     &&
     (xa<SearchMaxX+Thickness))
     ||
     ((xb>SearchMinX-Thickness)
     &&
     (xb<SearchMaxX+Thickness))) return 3;
...

{
  ...
  MessageBoxOwn(NULL,MessageStr,SC(100,"Error"),MB_APPLMODAL|MB_OK);
#endif
}
...
*/
  ...
  double  Diag1X2,Diag1Y2,xx1,yy1,xx2,yy2,xx3,yy3,xx4,yy4,r,
  ...
  if ((CircleX+CircleY> xx2+yy2)
    ...
    hulpy=CircleY-yy1;
    hulp=sqrt(hulpx*hulpx+hulpy*hulpy);
    if (hulp>CircleThickness*0.5) return 0;
  }
 ...
 4   /              /
  ...
  x1  \   /
       \/
         3
*/
  double Diag2X2,Diag2Y2,
         xx1,yy1,xx2,yy2,xx3,yy3,xx4,yy4,r,hulp,hulpx,hulpy,
         CircleR,CircleMinXD,CircleMinYD,CircleMaxXD,CircleMaxYD;
...
*/
  ...
  double Diag1X2,Diag1Y2,xx2,yy2,xx4,yy4,r,
         RectMinX,RectMinY,RectMaxX,RectMaxY,
         Diag1MinX,Diag1MaxX,Diag1MinY,Diag1MaxY,hulp;
  ...
  if ((Diag1MaxX<RectMinX)
     ||
     (Diag1MinX>RectMaxX)
     ||
     (Diag1MaxY<RectMinY)
     ||
     (Diag1MinY>RectMaxY)) return 0;
  ...
  if ( RectMaxX+RectMaxY< xx4+yy4) return 0;
  if ( RectMinX+RectMinY> xx2+yy2) return 0;
  if (-RectMinX+RectMaxY<-xx2+yy2) return 0;
  if (-RectMaxX+RectMinY>-xx4+yy4) return 0;
 ...
 4   /              /
  ...
  x1  \   /
       \/
         3
*/
  double Diag2X2,Diag2Y2,xx2,yy2,xx4,yy4,r,hulp,
         RectMinX,RectMinY,RectMaxX,RectMaxY,
         Diag2MinX,Diag2MaxX,Diag2MinY,Diag2MaxY;
  ...
  if ((Diag2MaxX<RectMinX)
     ||
     (Diag2MinX>RectMaxX)
     ||
     (Diag2MaxY<RectMinY)
     ||
     (Diag2MinY>RectMaxY)) return 0;
...

{
  ...
  double Diag1X12,Diag1Y12,Diag1X22,Diag1Y22,
         xx12,yy12,xx14,yy14,r1,
         xx22,yy22,xx24,yy24,r2,hulp;
  ...
  if (xx12+yy12<xx24+yy24) return 0;

  if (xx14+yy14>xx22+yy22) return 0;
...
{
  double Diag1X12,Diag1Y12,Diag2X22,Diag2Y22,
         xx12,yy12,xx14,yy14,r1,
         xx22,yy22,xx24,yy24,r2,hulp;
...
{
  double Diag2X12,Diag2Y12,Diag2X22,Diag2Y22,
         xx12,yy12,xx14,yy14,r1,
         xx22,yy22,xx24,yy24,r2,hulp;
...

{
  ...
  while (!Stop) {
    if (cnt<l) {
      cnt2=0;
      while ((cnt2<DelimiterCount)
            &&
            (Str[cnt]!=Delimiters[cnt2])) cnt2++;
...

{
  ...
  for (cnt=0;cnt<NrGraphicsLayers;cnt++) {
    if (GraphicsLayers[cnt]==SrcLayer) l1=cnt;
  }
  l2=-1;
  for (cnt=0;cnt<NrGraphicsLayers;cnt++) {
    if (GraphicsLayers[cnt]==DestLayer) l2=cnt;
  }
...

{
  switch (Units) {
    case 0:
      ...
      break;
    case 1:
      SendDlgItemMessageOwn(Dialog,Control,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(131,"mm"));
      break;
    case 2:
...
// *******************************************************************************************************
  if (Design.NrBoardLayers==2) {
    if ((*Layer>0)
       &&
       (*Layer<NrGeomLayers-1)) { // Objects on inner geometry layers will not be used
      return 0;
    }
    if (Mirror==0) {
      if (*Layer>0) {
...
// Nr board layers > 2
  ...
  if (NrGeomLayers<Design.NrBoardLayers) {
    if (*Layer==NrGeomLayers-1) {      // Top layer in the geometrie
      *Layer=Design.NrBoardLayers-1;
    }
  }
  if (NrGeomLayers>Design.NrBoardLayers) {
...

{
  ...
  if (NrLayerPlotObjects[Layer]+1>MaxLayerPlotObjects[Layer]) {
    if (AllocateSpecialMem(MEM_LAYER_OBJECTS+Layer,MaxLayerPlotObjects[Layer]+2048,
                           &LayerPlotObjects[Layer])==-1) {
      return NULL;
    }
...
{
  if ((DrawPolygon->maxx>=ViewMinX)
     &&
     (DrawPolygon->minx<=ViewMaxX)
     &&
     (DrawPolygon->maxy>=ViewMinY)
     &&
...
{
  if ((AreaFill->maxx>=ViewMinX)
     &&
     (AreaFill->minx<=ViewMaxX)
     &&
     (AreaFill->maxy>=ViewMinY)
     &&
  ...
  switch (mode) {
    case 0:
      sprintf(TimeString,"%02d:%02d:%02d.%03d",
              ...
              CurrentDate.wSecond,CurrentDate.wMilliseconds);
      break;
    case 1:
      sprintf(TimeString,"%02d-%02d-%4d %02d:%02d:%02d.%03d",
              CurrentDate.wDay,CurrentDate.wMonth,CurrentDate.wYear,
              CurrentDate.wHour,CurrentDate.wMinute,
              CurrentDate.wSecond,CurrentDate.wMilliseconds);
      break;
  }
