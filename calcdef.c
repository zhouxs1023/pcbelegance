...

{
  ...
  if (InRange2(dx,0.0)) { // Vertical line
    if (dy>0.0) { // Vertical line to top
    ...
    hoek=atan(r1);
    if (r1>0.0) {
      if (dx<0.0) hoek+=ANGLE_CONVERT(180.0);
    } else {
      if (dx>0.0) hoek+=ANGLE_CONVERT(360.0);
      else hoek+=ANGLE_CONVERT(180.0);
    }
  }
  hoek+=ANGLE_CONVERT(Rotation);
  if (hoek>ANGLE_CONVERT(360.0)) hoek-=ANGLE_CONVERT(360.0);
...

{
  ...
  if (InRange2(dx,0.0)) { // Vertical line
    ...
    hoek=atan(r1);
    if (r1>0.0) {
      if (dx<0.0) hoek+=ANGLE_180;
    } else {
      if (dx>0.0) hoek+=ANGLE_360;
      else hoek+=ANGLE_180;
    }
  }
  hoek+=ANGLE_CONVERT(Rotation);
  if (hoek>ANGLE_360) hoek-=ANGLE_360;
...

{
  ...
  if (rico1==rico2) return 1;
  if ((rico1==0)
     &&
     (fabs(rico2)<0.0001)) return 1;
  if ((rico2==0)
     &&
     (fabs(rico1)<0.0001)) return 1;
  if ((rico1<0)
     &&
     (rico1<rico2*0.99)
     &&
     (rico1>rico2*1.01)) return 1;
  if ((rico1>0)
     &&
     (rico1>rico2*0.99)
     &&
     (rico1<rico2*1.01)) return 1;
  if ((fabs(rico1)<0.0001)
     &&
     (fabs(rico2)<0.0001)) return 1;
  return 0;
}
...

{
  ...
  if (Angle>ANGLE_CONVERT(180.0)) Angle-=ANGLE_CONVERT(180.0);
  return Angle;
}
...

{
  ...
  *Distance=length;
  if (InRange2(x,0.0)) { // Vertical line
    ...
    hoek=atan(r1);
    if (r1>0.0) {
      if (x<0.0) hoek+=ANGLE_CONVERT(180.0);
    } else {
      if (x>0.0) hoek+=ANGLE_CONVERT(360.0);
      else hoek+=ANGLE_CONVERT(180.0);
    }
  }
  if (hoek>ANGLE_CONVERT(360.0)-0.001) hoek-=ANGLE_CONVERT(360.0);
  *Angle=(hoek*180.0/PI);
}
...

{
  ...
  *Length=sqrt(SQR(dx)+SQR(dy));
  if (InRangeSpecial(dx,0.0,0.000001)) { // Vertical line
    if (dy>0.0) { // Vertical line to top
      hoek=PI*0.5;
    } else {      // Vertical line to bottom
      hoek=PI*1.5;
...
*/
  ...
  if (InRange(divx,2.0)) { // Vertical line
    *cx=x1;
    *cy=py;
    return 0;
  }
  if (InRange(divy,2.0)) { // Horizontal line
    *cx=px;
    *cy=y1;
    return 0;
  }
...

{
  ...
  if (fabs(divx)>fabs(divy)) { // Most horizontal
    if ((cx>min(x1,x2))
       &&
       (cx<max(x1,x2))) {
      ...
      if (Length1<Length2) { // Distance from point to x1,y1
        dist=CalcLengthLine(x1,y1,px,py);
      } else {               // Distance from point to x2,y2
...

{
  ...
  dist=1e9;
  if (LineCrosses(x11,y11,x12,y12,x21,y21,x22,y22)!=1) { // No crosses
...

{
  ...
  switch (Layer) {
    case SOLD_MASK_BOTTOM_LAYER:
      strcpy(TextStr,SC(5,"Solder mask bottom"));
      break;
    case SOLD_MASK_TOP_LAYER:
      strcpy(TextStr,SC(6,"Solder mask top"));
      break;
    case PASTE_MASK_BOTTOM_LAYER:
      strcpy(TextStr,SC(7,"Paste mask bottom"));
      break;
    case PASTE_MASK_TOP_LAYER:
      strcpy(TextStr,SC(8,"Paste mask top"));
      break;
    ...
    case DRILL_UNPLATED_LAYER:
      strcpy(TextStr,SC(19,"Drill unplated"));
      break;
    case POWER_PAD_LAYER:
      strcpy(TextStr,SC(20,"Power pad"));
      break;
    case INNER_PAD_LAYER:
      strcpy(TextStr,SC(68,"Inner pad"));
      break;
    default:
      if (Layer<32) {
        sprintf(TextStr,SC(21,"Layer %i"),Layer2);
        if (Layer==0) {
          strcat(TextStr,SC(22," (Bottom)"));
        }
        if (NrPadLayers>1) {
          if (Layer==NrPadLayers-1) {
            strcat(TextStr,SC(23," (Top)"));
          }
        }
      } else {
        if ((Layer>=ROUTING_KEEPOUT_LAYER)
          ...
          sprintf(TextStr,SC(24,"Routing keepout layer %i"),Layer2-ROUTING_KEEPOUT_LAYER);
...

{
  ...
  if (max(Width,Height)>(400*2540)) NrSegments=64;
  if (max(Width,Height)>(1000*2540)) NrSegments=128;
