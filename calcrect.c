...

{
  ...
  if ((RectXmax<SearchMinX)
     ||
     (RectXmin>SearchMaxX)
     ||
     (RectYmax<SearchMinY)
     ||
     (RectYmin>SearchMaxY)) return 0;


  if ((RectXmax<SearchMaxX)
     &&
     (RectXmin>SearchMinX)
     &&
     (RectYmax<SearchMaxY)
     &&
     (RectYmin>SearchMinY)) return 1;

  if (RectXmin==RectXmax) {
    if ((RectXmin>SearchMinX)
       &&
       (RectXmin<SearchMaxX)) {
      if ((RectYmax>SearchMinY)
         ||
         (RectYmin<SearchMaxY)) return 1;
    }
    return 0;
  }
  if (RectYmin==RectYmax) {
    if ((RectYmin>SearchMinY)
       &&
       (RectYmin<SearchMaxY)) {
      if ((RectXmax>SearchMinX)
         ||
         (RectXmin<SearchMaxX)) return 1;
    }
    return 0;
...

{
  ...
  CircleYmin=CircleY;
  CircleYmax=CircleY;
  if ((CircleMode & 0xf0) > 0) CircleXmin=CircleX-CircleThickness;
  if ((CircleMode & 0x3c) > 0) CircleYmin=CircleY-CircleThickness;
  if ((CircleMode & 0x0f) > 0) CircleXmax=CircleX+CircleThickness;
  if ((CircleMode & 0xc3) > 0) CircleYmax=CircleY+CircleThickness;
  ...
  if ((CircleXmax<SearchMaxX)
     &&
     (CircleXmin>SearchMinX)
     &&
     (CircleYmax<SearchMaxY)
     &&
     (CircleYmin>SearchMinY)) return 1;
  ...
  if (rxmin>=0) {
    ya=sqrt(rxmin);
    if (((CircleY-ya>SearchMinY)
       &&
       (CircleY-ya<SearchMaxY))
       ||
       ((CircleY+ya>SearchMinY)
       &&
       (CircleY+ya<SearchMaxY))) return 1;
  }
  if (rxmax>=0) {
    ya=sqrt(rxmax);
    if (((CircleY-ya>SearchMinY)
  ...
  if (rymin>=0) {
    xa=sqrt(rymin);
    if (((CircleX-xa>SearchMinX)
       &&
       (CircleX-xa<SearchMaxX))
       ||
       ((CircleX+xa>SearchMinX)
       &&
       (CircleX+xa<SearchMaxX))) return 1;
  }
  if (rymax>=0) {
    xa=sqrt(rymax);
    if (((CircleX-xa>SearchMinX)
...

{
  ...
  CircleXmax=CircleX;
  CircleYmax=CircleY;
  if ((CircleMode & 0xf0) > 0) CircleXmin=CircleX-CircleThickness2;
  if ((CircleMode & 0x3c) > 0) CircleYmin=CircleY-CircleThickness2;
  if ((CircleMode & 0x0f) > 0) CircleXmax=CircleX+CircleThickness2;
  if ((CircleMode & 0xc3) > 0) CircleYmax=CircleY+CircleThickness2;
  ...
  if (rxmin+rymin<r) return 1;
  if (rxmin+rymax<r) return 1;
  if (rxmax+rymin<r) return 1;
  if (rxmax+rymax<r) return 1;

  return RectTestCircleOutline(CircleX,CircleY,CircleThickness,(uint8)CircleMode);

}

void    GetMinMaxArc(double ArcX,double ArcY,double ArcWidth,double ArcHeight,
                     double StartDiffX,double StartDiffY,
                     double EndDiffX,double EndDiffY,
                     double *ArcXmin,double *ArcYmin,double *ArcXmax,double *ArcYmax)

{
  double  ArcThickness2;
  double StartAngle,StartX,StartY,EndX,EndY,
         EndAngle,DiffAngle,rico;
  int32  StartPos,ok;
  ...
  if (DiffAngle<0) DiffAngle+=360.0;
...
int32    RectTestArc(double ArcX,double ArcY,double ArcWidth,double ArcHeight,
                    double StartDiffX,double StartDiffY,
                    double EndDiffX,double EndDiffY)

{
  double ArcXmin,ArcXmax,ArcYmin,ArcYmax,ArcThickness2;
  double StartAngle,StartX,StartY,EndX,EndY,Diam,
         EndAngle,rico,SearchCentreX,SearchCentreY,SearchCircleDiam;
  ...
  if (Diam-SearchCircleDiam>max(ArcWidth,ArcHeight)*0.5) return 0;
  ...
  if (StartDiffX==0.0) {
    ...
    if (rico>0.0) {
      if (StartDiffX<0.0) StartAngle+=180.0;
    } else {
      if (StartDiffX>0.0) {
        StartAngle+=360.0;
      } else {
  ...
  if (EndDiffX==0.0) {
    ...
    if (rico>0.0) {
      if (EndDiffX<0.0) EndAngle+=180.0;
    } else {
      if (EndDiffX>0.0) {
        EndAngle+=360.0;
      } else {
...
*/
  ...
  double  Diag1Xmin,Diag1Xmax,Diag1Ymin,Diag1Ymax,Diag1X2,Diag1Y2,
         xx2,yy2,xx4,yy4,r;
  double hulp;
  ...
  if ((Diag1Xmax<SearchMinX)
     ||
     (Diag1Xmin>SearchMaxX)
     ||
     (Diag1Ymax<SearchMinY)
     ||
     (Diag1Ymin>SearchMaxY)) return 0;

  if ((Diag1Xmax<SearchMaxX)
     &&
     (Diag1Xmin>SearchMinX)
     &&
     (Diag1Ymax<SearchMaxY)
     &&
     (Diag1Ymin>SearchMinY)) return 1;
  ...
  if ( SearchMaxX+SearchMaxY< xx4+yy4) return 0;
  if ( SearchMinX+SearchMinY> xx2+yy2) return 0;
  if (-SearchMinX+SearchMaxY<-xx2+yy2) return 0;
  if (-SearchMaxX+SearchMinY>-xx4+yy4) return 0;
...
*/
  ...
  double  Diag2Xmin,Diag2Xmax,Diag2Ymin,Diag2Ymax,Diag2X2,Diag2Y2,
         xx2,yy2,xx4,yy4,r;
  double hulp;
  ...
  if ((Diag2Xmax<SearchMinX)
     ||
     (Diag2Xmin>SearchMaxX)
     ||
     (Diag2Ymax<SearchMinY)
     ||
     (Diag2Ymin>SearchMaxY)) return 0;

  if ((Diag2Xmax<SearchMaxX)
     &&
     (Diag2Xmin>SearchMinX)
     &&
     (Diag2Ymax<SearchMaxY)
     &&
     (Diag2Ymin>SearchMinY)) return 1;
