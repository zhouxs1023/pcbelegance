...
char   StringToChange[MAX_LENGTH_STRING],
       NewSymbolName[MAX_LENGTH_STRING],NewPartNr[MAX_LENGTH_STRING],
       NewValue[MAX_LENGTH_STRING],NewGeometry[MAX_LENGTH_STRING],
       NewDescription[MAX_LENGTH_STRING],
...

{
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],
        str4[MAX_LENGTH_STRING],str5[MAX_LENGTH_STRING],NewGeometry[MAX_LENGTH_STRING],
        str6[MAX_LENGTH_STRING],str7[MAX_LENGTH_STRING],str8[MAX_LENGTH_STRING],
        str9[MAX_LENGTH_STRING],LineBuf[MAX_LENGTH_STRING],Ref1[MAX_LENGTH_STRING],
  ...
  int32 Found1,Found2,FoundCnt,MaxMem,res,result,Designfp,
        cnt,cnt2,cnt3,cnt4,cnt5,cnt6,MemPos,MemPos2,
        fp,Length,PartNr1,PartNr2,SwapMode,
        NrGatePinSwaps,NrPinsPinBus,MemSize,SymbolNr,InstMemSize,
        Pos1,Pos2,LineNr;
...
//      sprintf(str,"CHANGE GEOM U100 WITH %s"
      ...
      if ((SwapMode!=-1)
         &&
         (NrGatePinSwaps<MaxGatesPinSwapInfos-1)) {
    ...
    for (cnt5=0;cnt5<NrGatePinSwaps;cnt5++) {
      ...
      switch (GateSwapInfo->SwapMode) {
        case 0:
          ...
          for (cnt=0;cnt<Design.NrInstances;cnt++) {
            Instance=(InstanceRecord *)&((*Instances)[cnt]);
            if ((stricmpUTF8(GateSwapInfo->Ref2,Instance->Reference)==0)
               &&
               (GateSwapInfo->Swapped2==0)) {
                      ...
                      case 3:
                        if ((GateSwapInfo->Swapped1==0)
                          ...
                          for (cnt6=0;cnt6<GateSwapInfo->NrPinsPinBus;cnt6++) {
                            res=GetPinNameFromPinBus(Object->Text1,str2,
                          ...
                          if ((Found1)
                            ...
                            for (cnt6=0;cnt6<Design.NrRedefinedPinBusses;cnt6++) {
                              ...
                              if ((stricmpUTF8(GateSwapInfo->Ref1,RedefinedPinBus->Reference)==0)
                                 &&
                                 (stricmpUTF8(GateSwapInfo->PinBusLabel1,RedefinedPinBus->Name)==0)) {
                                FoundCnt=cnt6;
                              }
                            }
                            FileChanged=1;
                            if (FoundCnt!=-1) {
                              ...
                              for (cnt6=0;cnt6<Object->Info3;cnt6++) {
                                ...
                                if (cnt6>0) strcat(NewPinBusStr,",");
                                strcat(NewPinBusStr,str2);
                                LineNr=res;
                              }
                              for (cnt6=0;cnt6<Object->Info3;cnt6++) {
                                res=GetPinNameFromPinBus(NewPinBusStr,str2,
                                                         Object->Info3,cnt6);
                                if (res>=0) {
                                  if (stricmpUTF8(str2,GateSwapInfo->PinStr1)==0) {
                                    Found1=1;
                              ...
                              RedefinedPinBus=&((*RedefinedPinBusses)[Design.NrRedefinedPinBusses]);
        ...
        case 2:
          if ((Found1)
            ...
            for (cnt3=0;cnt3<Design.NrWires;cnt3++) {
              Wire=&((*Wires)[cnt3]);
              if ((InRange(FoundX1,Wire->X1))
                 &&
                 (InRange(FoundY1,Wire->Y1))) {
                Pos1=cnt3;
              }
              if ((InRange(FoundX1,Wire->X2))
                 &&
                 (InRange(FoundY1,Wire->Y2))) {
                Pos1=cnt3|0x10000;
              }
              if ((InRange(FoundX2,Wire->X1))
                 &&
                 (InRange(FoundY2,Wire->Y1))) {
                Pos2=cnt3;
              }
              if ((InRange(FoundX2,Wire->X2))
                 &&
                 (InRange(FoundY2,Wire->Y2))) {
                Pos2=cnt3|0x10000;
              }
            }
            if (Pos1!=-1) {
            ...
            for (cnt3=0;cnt3<Design.NrBusses;cnt3++) {
              Bus=&((*Busses)[cnt3]);
              if ((InRange(FoundX1,Bus->X1))
                 &&
                 (InRange(FoundY1,Bus->Y1))) {
                Pos1=cnt3;
              }
              if ((InRange(FoundX1,Bus->X2))
                 &&
                 (InRange(FoundY1,Bus->Y2))) {
                Pos1=cnt3|0x10000;
              }
              if ((InRange(FoundX2,Bus->X1))
                 &&
                 (InRange(FoundY2,Bus->Y1))) {
                Pos2=cnt3;
              }
              if ((InRange(FoundX2,Bus->X2))
                 &&
                 (InRange(FoundY2,Bus->Y2))) {
                Pos2=cnt3|0x10000;
              }
            }
            if (Pos1!=-1) {
            ...
            for (cnt3=0;cnt3<Design.NrGlobalConnections;cnt3++) {
              ...
              if ((InRange(FoundX1,GlobalConnection->X))
                 &&
                 (InRange(FoundY1,GlobalConnection->Y))) {
                Pos1=cnt3;
              }
              if ((InRange(FoundX2,GlobalConnection->X))
                 &&
                 (InRange(FoundY2,GlobalConnection->Y))) {
                Pos2=cnt3;
              }
            }
            if (Pos1!=-1) {
            ...
            for (cnt3=0;cnt3<Design.NrBusConnections;cnt3++) {
              ...
              if ((InRange(FoundX1,BusConnection->X))
                 &&
                 (InRange(FoundY1,BusConnection->Y))) {
                Pos1=cnt3;
              }
              if ((InRange(FoundX2,BusConnection->X))
                 &&
                 (InRange(FoundY2,BusConnection->Y))) {
                Pos2=cnt3;
              }
            }
            if (Pos1!=-1) {
            ...
            for (cnt3=0;cnt3<Design.NrJunctions;cnt3++) {
              ...
              if ((InRange(FoundX1,Junction->X))
                 &&
                 (InRange(FoundY1,Junction->Y))) {
                Pos1=cnt3;
                FileChanged=1;
              }
              if ((InRange(FoundX2,Junction->X))
                 &&
                 (InRange(FoundY2,Junction->Y))) {
                Pos2=cnt3;
                FileChanged=1;
              }
            }
            ...
            for (cnt3=0;cnt3<Design.NrNetLabels;cnt3++) {
              ...
              if ((InRange(FoundX1,NetLabel->ConnectX))
                 &&
                 (InRange(FoundY1,NetLabel->ConnectY))) {
                Pos1=cnt3;
              }
              if ((InRange(FoundX2,NetLabel->ConnectX))
                 &&
                 (InRange(FoundY2,NetLabel->ConnectY))) {
                Pos2=cnt3;
              }
            }
            if (Pos1!=-1) {
      ...
      if ((strcmp(Design.Identification,SheetCode3)==0)
        ...
        if ((strcmp(Design.Identification,SheetCode3)==0)
          ...
          for (cnt=0;cnt<Design.NrInstances;cnt++) {
            ...
            FileWrite(Designfp,&DesignBuf[MemPos2],sizeof(OldInstanceRecord)-offsetof(OldInstanceRecord,Geometry),&result);
        ...
        for (cnt=0;cnt<Design.NrInstances;cnt++) {
          ...
          FileWrite(Designfp,&DesignBuf[MemPos2],sizeof(OldOldInstanceRecord)-offsetof(OldOldInstanceRecord,Geometry),&result);
          MemPos+=sizeof(InstanceRecord);
        }
      }
...
int32 GetComponentFromPartNr(LPSTR PartNr,LPSTR SymbolName,LPSTR Geometry,
                             LPSTR Value,LPSTR Description)
...
*/
  char FileStr[MAX_LENGTH_STRING],LineBuf[MAX_LENGTH_STRING],
       str1[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemTextUTF8(Dialog,IDC_STATIC3                ,SC(254,"Part number"));
      ...
      SendDlgItemMessage(Dialog,IDC_COMBO1,CB_ADDSTRING,0,(LPARAM)SC(253,"Symbol name"));
      SendDlgItemMessage(Dialog,IDC_COMBO1,CB_ADDSTRING,0,(LPARAM)SC(31,"Value"));
      SendDlgItemMessage(Dialog,IDC_COMBO1,CB_ADDSTRING,0,(LPARAM)SC(32,"Geometry"));
      SendDlgItemMessage(Dialog,IDC_COMBO1,CB_ADDSTRING,0,(LPARAM)SC(254,"Part number"));
//      SendDlgItemMessage(Dialog,IDC_COMBO1,CB_ADDSTRING,0,(LPARAM)SC(100,"Part description"));
      SendDlgItemMessage(Dialog,IDC_COMBO1,CB_ADDSTRING,0,(LPARAM)SC(265,"Add property"));
      SendDlgItemMessage(Dialog,IDC_COMBO1,CB_ADDSTRING,0,(LPARAM)SC(266,"Change property"));
      ...
      SetWindowText(Dialog,SC(263,"Change components"));
      return about;
    case WM_MOVE:
      break;
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDOK:
          ...
          if (((NewSymbolName[0]==0) || (stricmpUTF8(NewSymbolName,SC(252,"<Do not change>"))==0))
            ...
            MessageBoxUTF8(DESIGNWindow,SC(268,"At least one item should be filled in"),SC(19,"Error"),MB_APPLMODAL|MB_OK);
            break;
          }
          EndDialog(Dialog,1);
          return about;
...

{
  char  str[MAX_LENGTH_STRING],
        FileName[MAX_LENGTH_STRING],StringToCompare[MAX_LENGTH_STRING],
        PropertyID[MAX_LENGTH_STRING],PropertyValue[MAX_LENGTH_STRING],
        NewProperties[1024];
  SymbolsPosRecord NewSymbolsPos[256];
  int32 Found,res,result,Designfp,cnt,cnt2,cnt3,MemPos,Changed,
        count,MaxMem,NrProperties,MemSize,MemPos2,
        NrNewSymbols,pos;
...
// **********************************************************************************
  ...
  res=DialogBox(DESIGNClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_CHANGE_SYMBOLS),
                DESIGNWindow, (DLGPROC)ChangeSymbolDialogBody);
  if (res!=1) return -1;
  ...
  if ((MaxMem>DesignBufMemSize)
     &&
     (AllocateMemDesignBuf(MaxMem)!=0)) return -1;
...
*/
      ...
      if ((ChangedSymbolMode==4)
        ...
        if ((NewSymbolName[0])
           &&
           (stricmpUTF8(NewSymbolName,SC(252,"<Do not change>")))
           &&
          ...
          cnt3=0;
          while (stricmpUTF8(NewSymbolsPos[cnt3].SymbolName,NewSymbolName)!=0) cnt3++;
          if (cnt3==NrNewSymbols) {
            strncpy(NewSymbolsPos[cnt3].SymbolName,NewSymbolName,sizeof(NewSymbolsPos[cnt3].SymbolName)-1);
            NrNewSymbols++;
            sprintf(str,SC(264,"!!!! Symbol name %s will be changed in %s\r\n"),Instance->SymbolName,NewSymbolName);
            AddMessage(str);
//            SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
          }
          Changed=1;
        ...
        if ((NewValue[0])
           &&
           (stricmpUTF8(NewValue,SC(252,"<Do not change>")))
           &&
        ...
        if ((NewPartNr[0])
           &&
           (stricmpUTF8(NewPartNr,SC(252,"<Do not change>")))
           &&
        ...
        if ((NewGeometry[0])
           &&
           (stricmpUTF8(NewGeometry,SC(252,"<Do not change>")))
           &&
        ...
        if ((NewDescription[0])
           &&
           (stricmpUTF8(NewDescription,SC(252,"<Do not change>")))
           &&
        ...
        if ((NewPropertyID[0])
           &&
           (stricmpUTF8(NewPropertyID,SC(252,"<Do not change>")))
           &&
          ...
          for (cnt3=0;cnt3<NrProperties;cnt3++) {
            if (GetProperties(Instance->Properties,(LPSTR)&PropertyID,
              ...
              if (strcmpUTF8(PropertyID,NewPropertyID)==0) {
                if ((NewPropertyValue[0]!=0)
                   &&
                   (pos+strlen(PropertyID)+1+strlen(NewPropertyValue)+2<sizeof(Instance->Properties))) {
                ...
                if (pos+strlen(PropertyID)+1+strlen(PropertyValue)+2<sizeof(Instance->Properties)) {
                  strcpy((LPSTR)&NewProperties[pos],PropertyID);
                  pos+=strlen(PropertyID)+1;
          ...
          if ((!Found)
             ...
             &&
             (pos+strlen(NewPropertyID)+1+strlen(NewPropertyValue)+2<sizeof(Instance->Properties))) {
...
// **********************************************************************************
    ...
    if ((count>0)
       ...
       ((NrNewSymbols==Design.NrSymbols)
       ||
       (MessageBoxUTF8(DESIGNWindow,SC(345,"Symbol names will be changed\r\nDo you want to continue ?"),
                       SC(8,"Message"),MB_APPLMODAL|MB_OKCANCEL)==IDOK))) {
      MakeBackup(FileName);
