...
{
  int32  cnt,cnt2,cnt3,FoundWire,FoundPin,FoundNetLabel,FoundJunction,
         TestMode,FoundGlobalConnection,FoundBusConnection,count;
  double  x1,y1,x3,y3;
        ...
        for (cnt=0;cnt<Design.NrWires;cnt++) {
          ...
          if ((cnt2!=cnt)
            ...
            if (((InRange(x1,Wire2->X1))
              ...
              if (TestLineConnectedToCircle(Wire2->X1,Wire2->Y1,Wire2->X2,Wire2->Y2,
                                            x1,y1,(float)0.3)) {
                FoundJunction=-1;
                for (cnt3=0;cnt3<Design.NrJunctions;cnt3++) {
                  Junction=&((*Junctions)[cnt3]);
                  if ((Junction->Info & (OBJECT_NOT_VISIBLE)) == 0) {
                    if ((InRange(x1,Junction->X))
                       &&
                       (InRange(y1,Junction->Y))) {
                      FoundJunction=cnt3;
                      break;
                    }
                  }
...
#endif
            if ((x1>Instance->BoardPosMinX-0.5)
               &&
               (x1<Instance->BoardPosMaxX+0.5)
               &&
               (y1>Instance->BoardPosMinY-0.5)
               &&
               (y1<Instance->BoardPosMaxY+0.5)) {
              NrObjects=0;
              InstanceToObject(Instance,0.0,0.0,1);
              for (cnt=0;cnt<NrObjects;cnt++) {
                Object=&((*Objects)[cnt]);
                if ((InRange(x1,Object->x1))
                   &&
                   (InRange(y1,Object->y1))) {
                  FoundPin=cnt3;
                }
              }
            }
      ...
      if (TestWire) {
        ...
        for (cnt=0;cnt<Design.NrNetLabels;cnt++) {
          ...
          if ((NetLabel->Info & (OBJECT_NOT_VISIBLE)) == 0) {
            ...
            if (((InRange(TestWire->X1,x1))
               &&
               (InRange(TestWire->Y1,y1)))
               ||
               ((InRange(TestWire->X2,x1))
               &&
               (InRange(TestWire->Y2,y1)))) {
              FoundNetLabel=cnt;
            }
          }
        }
    ...
    if ((BusConnection->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      for (cnt2=cnt+1;cnt2<Design.NrBusConnections;cnt2++) {
        ...
        if ((BusConnection2->Info & (OBJECT_NOT_VISIBLE)) == 0) {
          if ((InRange(BusConnection->X,BusConnection2->X))
             &&
             (InRange(BusConnection->Y,BusConnection2->Y))
             &&
...
//             (TestLineConnectedToCircle(Wire->X1,Wire->Y1,Wire->X2,Wire->Y2,x1,y1,(double)0.3))) {
      ...
      for (cnt3=0;cnt3<Design.NrInstances;cnt3++) {
        Instance=(InstanceRecord *)&((*Instances)[cnt3]);
        if ((Instance->Info & OBJECT_NOT_VISIBLE) == 0) {
          if ((x1>Instance->BoardPosMinX-0.1)
             &&
             (x1<Instance->BoardPosMaxX+0.1)
             &&
             (y1>Instance->BoardPosMinY-0.1)
             &&
             (y1<Instance->BoardPosMaxY+0.1)) {
            NrObjects=0;
            InstanceToObject(Instance,0.0,0.0,1);
...

{
  ...
  if (mode==0) ClearMask=OBJECT_ERROR|OBJECT_DONE;
  else ClearMask=OBJECT_ERROR|OBJECT_DONE|OBJECT_SELECTED|0x000f;
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      if (DialogMode==0) {
        SetWindowText(Dialog,SC(4,"Error/message"));
      } else {
        SetWindowText(Dialog,SC(471,"Warning/message"));
      }
...

{
  ...
  if ((result=CheckInstances(0,&x1,&y1))) {
    ...
    if ((cnt!=-1)
       &&
       (SetBoardPosInstance((int16)cnt)==1)) {
/*
      Instance=(InstanceRecord *)&((*Instances)[cnt]);
...
//    RePaint();
    sprintf(str,SC(470,"There are %d wire(s) with one end of the wire unconnected"),res);
...

{
  ...
  for (cnt=0;cnt<DesignSymbol.NrPins;cnt++) {
    ...
    if ((Pin->Info & OBJECT_NOT_VISIBLE) == 0) {
      if (count<1000) memmove(&PinStrings[count],Pin->Name,sizeof(Pin->Name)-1);
      count++;
    }
  }
  for (cnt=0;cnt<DesignSymbol.NrPowerPins;cnt++) {
    ...
    if ((PowerPin->Info & OBJECT_NOT_VISIBLE) == 0) {
      ...
      while (str2[0]!=0) {
        ...
        if (count<1000) memmove(&PinStrings[count],&str3,11);
        count++;
      }
    }
  }
  ...
  for (cnt=0;cnt<count-1;cnt++) {
    ...
    for (cnt2=cnt+1;cnt2<count;cnt2++) {
      if (stricmpUTF8(str2,PinStrings[cnt2])==0) {
        if (NrErrors<200) Errors[NrErrors]=cnt;
        NrErrors++;
      }
    }
  }
