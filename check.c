...

{
  int32  cnt,cnt2,cnt3,FoundWire,FoundPin,FoundNetLabel,FoundJunction,
         TestMode,FoundGlobalConnection,FoundBusConnection;
  double  x1,y1,x3,y3;
  ...
  for (cnt2=0;cnt2<Design.NrWires;cnt2++) {
    ...
    if ((Wire->Info & (OBJECT_NOT_VISIBLE)) == 0) {
          ...
          if ((cnt2!=cnt)
            ...
            if (((InRange(x1,Wire2->X1))
              ...
              if (TestLineConnectedToCircle(Wire2->X1,Wire2->Y1,Wire2->X2,Wire2->Y2,
                ...
                if ((FoundJunction!=-1)
                   ||
                   (stricmp(Design.Identification,SheetCode1)==0)) {
                  FoundWire=cnt;
                }
              }
            }
      ...
      if (TestWire) {
        ...
        if (FoundNetLabel==-1) {
          sprintf(str,SC(238,"Warning: One end of the wire (%.1f,%.1f - %.1f,%.1f) is unconnected in sheet %s\r\n"),
                  Wire->X1,Wire->Y1,Wire->X2,Wire->Y2,Filename);
          AddMessage(str);
...

{
  ...
  else ClearMask=OBJECT_ERROR|OBJECT_DONE|OBJECT_CHANGED_TO_SELECTED|0x000f;
