...
{
  int32  cnt,cnt2,cnt3,cnt4,res,NrCopies,Comp1,Comp2,fp,lengte,
         InstNr,LoadResult,Pos,Length;
  char   str[MAX_LENGTH_STRING*5],FileName[MAX_LENGTH_STRING],LibName[MAX_LENGTH_STRING],
         EditFile2[MAX_LENGTH_STRING],*windir,
...
// ********************************************************************************************************
  ...
  if ((WParam>=ID_FILE_USED_FILES)
     &&
     (WParam<ID_FILE_USED_FILES+100)) {
    if (SaveFile2(0)==1) {
...
// ********************************************************************************************************
  ...
  if ((WParam>=ID_EDIT_PINBUSREORDER)
     &&
     (WParam<ID_EDIT_PINBUSREORDER+100)) {
...
// ********************************************************************************************************
  ...
  if ((WParam>=ID_ADD_PINBUSREORDER)
     &&
     (WParam<ID_ADD_PINBUSREORDER+100)) {
    for (cnt=0;cnt<Design.NrInstances;cnt++) {
      Instance=(InstanceRecord *)&((*Instances)[cnt]);
      if ((Instance->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
        if ((Instance->Info & 4)==4) {
...
// ********************************************************************************************************
    ...
    case ID_FILE_NEW_SHEET2:
      if (SaveFile2(0)==1) {
        ...
        sprintf(ExeParams,"\"%s\" /a /t0 /e \"%s\" /p \"%s\" /u \"%s\"",
    ...
    case ID_FILE_NEW_SYMBOL2:
      if (SaveFile2(0)==1) {
        sprintf(ExeFile,"%s\\sch.exe",ExePath);
        if (FileExistsUTF8(ExeFile)!=0) return;
        sprintf(ExeParams,"\"%s\" /a /t1 /e \"%s\" /p \"%s\" /u \"%s\"",
                ExeFile,ExePath,DesignPath,ProjectPath);
        if (ProjectActive) {
          strcat(str," /o");
        }
    ...
    case ID_FILE_NEW_SHEETSYMBOL2:
      if (SaveFile2(0)==1) {
        sprintf(ExeFile,"%s\\sch.exe",ExePath);
        if (FileExistsUTF8(ExeFile)!=0) return;
        sprintf(ExeParams,"\"%s\" /a /t2 /e \"%s\" /p \"%s\" /u \"%s\"",
                ExeFile,ExePath,DesignPath,ProjectPath);
        if (ProjectActive) {
          strcat(str," /o");
        }
    ...
    case ID_FILE_SAVEAS:
      ...
      if (SaveFile(1)!=0) {
        MessageBoxUTF8(SCHWindow,SC(18,"Error in saving file"),EditFile,MB_APPLMODAL|MB_OK);
        if (str[0]!=0) strcpy(EditFile,str);
      }
      break;
      ...
      if ((InstancesSelected>0)
        ...
        if (SaveFile2(0)==1) {
          ...
          if (InstNr>=0) {
            ...
            strcpy(EditFile2,str);
            if (FileExistsUTF8(EditFile2)!=0) { // Schematic does not exist
...
//                strcpy(NewDesign2.Identification,SheetCode5);
            ...
            if ((strlen(FileName)<sizeof(NestingFiles[0])-1)
               &&
               (NestingLevel<8)) {
    ...
    case ID_FILE_EDIT_SYMBOL:
      LoadResult=SearchSymbol(SelectedSymbolNameToEdit,LibName,&Pos,&Length,0);
      if (LoadResult==1) {
        if ((Pos!=-1)
           &&
           (Pos!=-2)) {
      ...
      sprintf(ExeFile,"%s\\sch.exe",ExePath);
      if (FileExistsUTF8(ExeFile)!=0) return;
      if (Pos==-2) { // editing symbol file from main sym directory
        sprintf(ExeParams,"\"%s\" \"%s\" /a /g /w %x /p \"%s\" /e \"%s\" /u \"%s\"",
                ExeFile,SelectedSymbolNameToEdit,(int32)SCHWindow,DesignPath,ExePath,ProjectPath);
      } else {
        sprintf(ExeParams,"\"%s\" \"%s\" /a /w %x /p \"%s\\sym\" /e \"%s\" /u \"%s\"",
                ExeFile,SelectedSymbolNameToEdit,(int32)SCHWindow,DesignPath,ExePath,ProjectPath);
      }
      if (ProjectActive) {
        strcat(str," /o");
...
//          PostMessage(SCHWindow,WM_PAINT,(WPARAM)NULL,(LPARAM)NULL);
    ...
    case ID_EDIT_PARTS_NR:
      if (EditingSymbol) PartNrDialog(0);
      break;
