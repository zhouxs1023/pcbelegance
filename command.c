...
// ********************************************************************************************************
	if ((WParam >= ID_TRACE_WIDTH) && (WParam < ID_TRACE_WIDTH + 80))
	{
		...
		{
			sprintf(InfoStr, SC(142, "%s     (%.4f,%.4f mm)"), Net->Name, CurrentTraceWidth / 100000.0,
...
//      SelectTracesFromRectWindow();
	...
	case ID_ACTION_MOVE_REFS:
		...
		{
			MessageBoxOwn(PCBWindow, SC(143, "Component references are not visible"), SC(100, "Error"),
			              MB_APPLMODAL | MB_OK);
...
//      RepeatMode=1;
	...
	case ID_ACTION_MOVE_COMPVALUES:
		...
		{
			MessageBoxOwn(PCBWindow, SC(144, "Component values are not visible"), SC(100, "Error"),
			              MB_APPLMODAL | MB_OK);
