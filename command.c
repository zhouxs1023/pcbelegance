...
//  MessageBoxUTF8(GEOMWindow,str,"Params",MB_APPLMODAL|MB_OK);
  ...
  if ((WParam>=ID_SETTINGS_GRID)
     &&
     (WParam<ID_SETTINGS_GRID+100)) {
...
//        MessageBox(NULL,DirPath,"Dir path",MB_APPLMODAL+MB_OK);
//        MessageBox(NULL,EditFile,"EditFile",MB_APPLMODAL+MB_OK);
        if (GetNewFileUTF8(GEOMWindow,NULL,EditFile,DirPath,SC(245,"Geometry files"),
                           NULL,SC(245,"Geometry files"),"shp",0)==0) {
          if (ProjectActive) {
            cnt=0;
            while ((cnt<32)
                  &&
                  ...
                  ||
                  (stricmpUTF8(ProjectInfo->FileNames[cnt],EditFile)!=0))) cnt++;
            if (cnt<32) {
    ...
    case ID_FILE_SAVEAS:
      if (SaveFile(4)==-1) {
        MessageBoxUTF8(GEOMWindow,EditFile,SC(30,"Error in saving file as"),MB_APPLMODAL|MB_OK);
      }
      break;
    ...
    case ID_CREATE_SIL:
      ...
      if (Units==0) SILGeom.Units=1;
      else SILGeom.Units=0;
...
*/
    case ID_LINE_WIDTH:
      ...
      for (cnt=0;cnt<NrObjects;cnt++) {
        ...
        if ((value<-9.0)
           &&
           ((Object->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)) {
