...

{
  ...
  char   ExeFile[MAX_LENGTH_STRING],ExeParams[MAX_LENGTH_STRING*5],
         str[MAX_LENGTH_STRING*5],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],
         str4[MAX_LENGTH_STRING],
         *windir,WorkFile[MAX_LENGTH_STRING],NewDesignFile[MAX_LENGTH_STRING];
  double x,y;
  float  x2,y2;
  HWND   WorkWindow;
  HKEY    Key;

  if ((WParam>=ID_SHEET_OPEN_TOP_SHEET)
     &&
     (WParam<ID_SHEET_OPEN_TOP_SHEET+128)) {
    if (!DesignActive) {
      MessageBoxUTF8(DESIGNWindow,"",SC(6,"No design active"),MB_APPLMODAL|MB_OK);
      return;
    }
    ...
    if (FileExistsUTF8(ExeFile)!=0) return;
    sprintf(ExeParams,"\"%s\" \"%s\" /a /o /e \"%s\" /u \"%s\" /x \"%s\\%s.dsn\"",
            ExeFile,WorkFile,ExePath,ProjectPath,DesignPath,DesignName);
    StartupInfo.cb=sizeof(StartupInfo);
    StartupInfo.wShowWindow=SW_SHOW;
    CreateProcess(ExeFile,ExeParams,NULL,NULL,1,0,NULL,NULL,&StartupInfo,&ProcessInfo);
    return;
  }
  if ((WParam>=ID_DESIGNS)
     &&
     (WParam<ID_DESIGNS+TotalNrDesigns)) {
  ...
  switch (WParam) {
    case ID_SYMBOL_OPEN:
      sprintf(ExeFile,"%s\\sch.exe",ExePath);
      if (FileExistsUTF8(ExeFile)!=0) break;
      if (DesignActive) {
        if (CheckExeDir()) {
          sprintf(ExeParams,"\"%s\" /a /t1 /o /e \"%s\" /p \"%s\" /u \"%s\" /x \"%s\\%s.dsn\"",
                  ExeFile,ExePath,DesignPath,ProjectPath,DesignPath,DesignName);
          StartupInfo.cb=sizeof(StartupInfo);
          StartupInfo.wShowWindow=SW_SHOW;
          CreateProcess(ExeFile,ExeParams,NULL,NULL,1,0,NULL,NULL,&StartupInfo,&ProcessInfo);
        }
      } else {
        sprintf(ExeParams,"\"%s\" /a /t1 /o /e \"%s\" /p \"%s\" /u \"%s\"",
...
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)SeparatorString);
      ...
      if (LoadDesign()==0) {
        ...
        DesignActive=1;
        sprintf(str,SC(11,"Design %s opened\r\n"),DesignFile);
        AddMessage(str);
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
    ...
    case ID_FILE_NEWDESIGN:
      ...
      if (NewProjectDialog(0)==1) MakeNewProject(0);
      break;
...
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
    ...
    case ID_SHEET_OPEN_REF:
      ...
      sprintf(ExeFile,"%s\\sch.exe",ExePath);
      if (FileExistsUTF8(ExeFile)!=0) break;
      sprintf(ExeParams,"\"%s\" \"%s\" /r \"%s\" /a /o /e \"%s\" /u \"%s\" /x \"%s\\%s.dsn\"",
              ExeFile,str2,str3,ExePath,ProjectPath,DesignPath,DesignName);
      StartupInfo.cb=sizeof(StartupInfo);
      StartupInfo.wShowWindow=SW_SHOW;
      CreateProcess(ExeFile,ExeParams,NULL,NULL,1,0,NULL,NULL,&StartupInfo,&ProcessInfo);
      break;
    case ID_SHEET_OPEN_PARTNR:
      ...
      if (SheetNr==-1) {
        sprintf(str2,SC(274,"Could not find partnr %s"),str3);
        MessageBoxUTF8(DESIGNWindow,str2,SC(19,"Error"),MB_APPLMODAL|MB_OK);
        break;
      }
      if (!DesignActive) {
      ...
      sprintf(ExeFile,"%s\\sch.exe",ExePath);
      if (FileExistsUTF8(ExeFile)!=0) break;
      sprintf(ExeParams,"\"%s\" \"%s\" /r2 \"%s\" /a /o /e \"%s\" /u \"%s\" /x \"%s\\%s.dsn\"",
              ExeFile,str2,str3,ExePath,ProjectPath,DesignPath,DesignName);
      StartupInfo.cb=sizeof(StartupInfo);
      StartupInfo.wShowWindow=SW_SHOW;
      CreateProcess(ExeFile,ExeParams,NULL,NULL,1,0,NULL,NULL,&StartupInfo,&ProcessInfo);
    ...
    case ID_LAYOUT_OPEN_REF:
      ...
      if (FileExistsUTF8(ExeFile)!=0) break;
      sprintf(ExeParams,"\"%s\" \"%s\" /a /o /e \"%s\" /u \"%s\" /x \"%s\\%s.dsn\" /r \"%s\"",
              ExeFile,WorkFile,ExePath,ProjectPath,DesignPath,DesignName,str3);
      StartupInfo.cb=sizeof(StartupInfo);
      StartupInfo.wShowWindow=SW_SHOW;
    ...
    case ID_LAYOUT_OPEN_PARTNR:
      ...
      sprintf(ExeFile,"%s\\pcb.exe",ExePath);
      if (FileExistsUTF8(ExeFile)!=0) break;
      sprintf(ExeParams,"\"%s\" \"%s\" /a /o /e \"%s\" /u \"%s\" /x \"%s\\%s.dsn\" /r2 \"%s\"",
              ExeFile,WorkFile,ExePath,ProjectPath,DesignPath,DesignName,str3);
      StartupInfo.cb=sizeof(StartupInfo);
      StartupInfo.wShowWindow=SW_SHOW;
      if (CreateProcess(ExeFile,ExeParams,NULL,NULL,1,0,NULL,NULL,&StartupInfo,&ProcessInfo)) {
...
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)"\r\n");
        ...
        if (MessageBoxUTF8(DESIGNWindow,str2,SC(8,"Message"),MB_APPLMODAL|MB_OKCANCEL)==IDOK) {
          if ((WorkWindow=GetProjectWindow(WorkFile,1,0))) {
            ...
            sprintf(ExeFile,"%s\\sch.exe",ExePath);
            if (FileExistsUTF8(ExeFile)!=0) break;
            sprintf(ExeParams,"\"%s\\sch.exe\" \"%s\" /a /o /e \"%s\" /u \"%s\" /x \"%s\\%s.dsn\"",
                    ExePath,WorkFile,ExePath,ProjectPath,DesignPath,DesignName);
            if ((x>-1000.0) && (x<1000.0) && (y>-1000.0) && (y<1000.0)) {
      ...
      if (CheckRefInstances(0)==0) {
        sprintf(str,SC(450,"No errors found in check schematics\r\n"));
        AddMessage(str);
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
      }
      SetNormalCursor();
    ...
    case ID_BUILD_NETLIST:
      ...
      SetWaitCursor();
      SendMessageUTF8(EditWindow,WM_SETTEXT,0,(LPARAM)(LPSTR)"");
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)(LPSTR)"\r\n");
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)(LPSTR)"\r\n");
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)(LPSTR)"\r\n");
      SaveOpenFiles(2);
      x=-100000.0;
...
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
        ...
        if (MessageBoxUTF8(DESIGNWindow,str2,SC(8,"Message"),MB_APPLMODAL|MB_OKCANCEL)==IDOK) {
          ...
          while ((cnt<32)
                ...
                ||
                (stricmpUTF8(ProjectInfo->FileNames[cnt],WorkFile)!=0))) cnt++;
          if (cnt<32) {
...
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
    ...
    case ID_OPEN_GEOM_EDITOR:
      ...
      if (FileExistsUTF8(ExeFile)!=0) break;
      if (DesignActive) {
        sprintf(ExeParams,"\"%s\" /d \"%s\" /a /o /e \"%s\" /u \"%s\"",
                ExeFile,DesignPath,ExePath,ProjectPath);
      } else {
        sprintf(ExeParams,"\"%s\" /a /o /e \"%s\" /u \"%s\"",
                ExeFile,ExePath,ProjectPath);
      }
      StartupInfo.cb=sizeof(StartupInfo);
      StartupInfo.wShowWindow=SW_SHOW;
      CreateProcess(ExeFile,ExeParams,NULL,NULL,1,0,NULL,NULL,&StartupInfo,&ProcessInfo);
...
//      WinExec(FileName,SW_SHOW);
    ...
    case ID_LIBRARYMANAGER_GEOMETRIES:
      sprintf(ExeFile,"%s\\libman.exe",ExePath);
      if (FileExistsUTF8(ExeFile)!=0) break;
      sprintf(ExeParams,"\"%s\" /d \"%s\" /g /e \"%s\" /u \"%s\"",
              ExeFile,DesignPath,ExePath,ProjectPath);
      StartupInfo.cb=sizeof(StartupInfo);
      StartupInfo.wShowWindow=SW_SHOW;
      CreateProcess(ExeFile,ExeParams,NULL,NULL,1,0,NULL,NULL,&StartupInfo,&ProcessInfo);
