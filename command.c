...
extern int32 FinishPolygon,TraceCheck,
             TraceChanged,TraceBackWardsKeyPressed,InsertViaKeyPressed,
             FinishedTraceKeyPressed,ChangeTraceWidthKeyPressed,
             ChangeTraceDrawingModeKeyPressed,EditDesignRulesNetKeyPressed,
             DefaultCrcError,ProjectIndexNr,ProjectActive,
             CrossHairType,CurrentButtonNr,TraceDrawingMode;

extern char  GatePinSwapReference[64],GatePinSwapPinName[64],MessageStr[MAX_LENGTH_STRING],
             SearchForReferenceString[MAX_LENGTH_STRING],
             SearchForPartNrString[MAX_LENGTH_STRING];
...
{
  int32  Layer,cnt,Found,res,CompNr,NetNr,count,NrCompsSelected,
         NrReferencesSelected,NrValuesSelected,lengte,
         SrcLayer,DestLayer;
  ...
  CompRecord   *Comp;
  uint8 *SelectedNets;
  char   str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],*windir,
         ExeFile[MAX_LENGTH_STRING*2],ExeParams[MAX_LENGTH_STRING*5];
  PROCESS_INFORMATION ProcessInfo;


//  sprintf(str,SC(138,"Command: %d = 0x%08x"),WParam,WParam);
//  MessageBoxOwn(PCBWindow,str,"",MB_APPLMODAL|MB_OK);

/*
  if (WParam==ID_ACTION_ROUTE_TRACES) {
    CloseApplication(__FILE__,__LINE__);
  }
*/

  if ((WParam>=ID_EXT_RESOURCE)
     &&
     (WParam<ID_EXT_RESOURCE+4096)) {
      ...
      if (SystemBusyMode==0) {
        ...
        switch (SystemBusyMode) {
          case 1: // Routing traces
            ...
            if ((WParam>=ID_CLEARANCE_WIDTH)
               &&
               (WParam<ID_CLEARANCE_WIDTH+0x100)) {
              break;
            }
            if ((WParam>=ID_TRACE_WIDTH)
               &&
               (WParam<ID_TRACE_WIDTH+0x100)) {
              break;
            }
            switch (WParam) {
...
// ********************************************************************************************************
// ********************************************************************************************************
  if ((WParam>=ID_SELECT_LAYER)
     &&
     (WParam<ID_SELECT_LAYER+0x1000)) {
    Layer=WParam & 0x00ff;
          ...
          if ((ViewSingleLayer)
            ...
            if ((SelectionMode==ROUTING_MODE)
              ...
              switch (CurrentDrawingLayer) {
                case -1:
                  sprintf(InfoStr,SC(139,"Routing traces"));
                  break;
                default:
                  GetLayerText(CurrentDrawingLayer,str,3);
                  sprintf(InfoStr,SC(140,"Routing traces %s"),str);
                  break;
              }
              RedrawInfoStr(1);
            }
...
// ********************************************************************************************************
  ...
  if ((WParam>=ID_ADD_LINE_OBJECT)
     &&
     (WParam<ID_ADD_LINE_OBJECT+0x400000)) {
// Layer range
//
...
// 101  .. 12701  ( increments 100)
    ...
    switch (Layer) {
      ...
      case INFO_LAYER4:
        ...
        if (LineWidth==0.0) LineWidth=(8*2540);
        break;
      ...
      case PASTE_MASK_TOP:
        ...
        if (LineWidth==0.0) LineWidth=(12*2540);
        break;
      case DRILL_LAYER:
...
//        PressButton(0,0);
      ...
      if ((GridVisible)
         &&
         (NotInRange(TempGridSize,GridSize))) {
    ...
    case ID_ACTION_MOVE_COMPS:
    case ID_ACTION_MOVE_COMPS2:
      if ((WParam==ID_ACTION_MOVE_COMPS2)
         &&
...
//      SelectTracesFromRectWindow();
    ...
    case ID_ACTION_MOVE_REFS:
      if (!OkToDrawCompReference) {
        MessageBoxOwn(PCBWindow,SC(143,"Component references are not visible"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
...
//      RepeatMode=1;
    ...
    case ID_ACTION_MOVE_COMPVALUES:
      if (!OkToDrawCompValue) {
        MessageBoxOwn(PCBWindow,SC(144,"Component values are not visible"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
...
// ********************************************************************************************************
    ...
    case ID_FILE_NEW:
      res=0;
      if (FileChanged) {
        res=MessageBoxOwn(PCBWindow,SC(146,"File has changed, save ?"),"",MB_APPLMODAL|MB_YESNOCANCEL);
        if (res==IDYES) {
          if ((res=SaveFile(0))==-1) {
...
//      DisableConnectionsNetsDialog();
    ...
    case ID_VIEW_UNSELECTTRACESVIASNET:
      PcbSettingsDialog(IDD_DIALOG_NETINFO,1);
//      UnselectTracesViasNetDialog();
    ...
    case ID_VIEW_CENTER_ON_COMPONENT:
      ...
      if ((CompNr=FindCompNr(ReferenceText.Text,0))!=-1) {
        ...
        if ((SelectionMode==MOVE_COMPONENTS_MODE)
           &&
           ((Comp->Info & (COMPONENT_PROTECTED)) == 0)) {
          switch (LParam) {
            case 2:
            case 3:
    ...
    case ID_VIEW_CENTER_ON_COMP_PARTNR:
      switch (LParam) {
        ...
        case 1:
          memset(&ReferenceText,0,sizeof(ObjectTextRecord));
          if (CompRefDialog(&ReferenceText,SC(149,"Center view on component with partnr"))!=1) {
            break;
          }
          break;
        case 2:
...
//      NetTypesDialog(-1);
    ...
    case ID_EDIT_PROTECT_COMP:
      PcbSettingsDialog(IDD_DIALOG_NETS2,0);
      break;
...
//        NetTypesDialog(NetNr);
    ...
    case ID_DELETE_TRACESVIASNET_FUNC:
      PcbSettingsDialog(IDD_DIALOG_NETINFO,2);
...
//          Beep(2000,500);
    ...
    case ID_COMPREF_HIDE:
      if (OkToDrawCompReference) ChangeVisibilityCompRefsValues(0);
      break;
    case ID_COMPREF_VISIBLE:
      if (OkToDrawCompReference) ChangeVisibilityCompRefsValues(1);
      break;
    case ID_COMPREF_ONTOP:
      if (OkToDrawCompReference) ChangeMirrorCompRefsValues(0);
      break;
    case ID_COMPREF_ONBOTTOM:
      if (OkToDrawCompReference) ChangeMirrorCompRefsValues(1);
      break;
    case ID_COMPREF_HEIGHT:
      if (OkToDrawCompReference) ChangeTextHeightCompRefsValues(0);
      break;
    case ID_COMPREF_LINEWIDTH:
      if (OkToDrawCompReference) ChangeLineWidthCompRefsValues(0);
      break;
    case ID_COMPVALUE_HIDE:
      if (OkToDrawCompValue) ChangeVisibilityCompRefsValues(2);
      break;
    case ID_COMPVALUE_VISIBLE:
      if (OkToDrawCompValue) ChangeVisibilityCompRefsValues(3);
      break;
    case ID_COMPVALUE_ONTOP:
      if (OkToDrawCompValue) ChangeMirrorCompRefsValues(2);
      break;
    case ID_COMPVALUE_ONBOTTOM:
      if (OkToDrawCompValue) ChangeMirrorCompRefsValues(3);
      break;
    case ID_COMPVALUE_HEIGHT:
      if (OkToDrawCompValue) ChangeTextHeightCompRefsValues(2);
      break;
    case ID_COMPVALUE_LINEWIDTH:
      if (OkToDrawCompValue) ChangeLineWidthCompRefsValues(2);
      break;
      ...
      switch (SelectionMode) {
        case MOVE_COMPONENT_VALUES_MODE:
          ...
          if (NrCompsSelected==1) {
            if (OkToDrawCompValue) ChangeCompValue();
          }
          break;
        case OBJECTS_MODE:
          if (OkToDrawObjects) ChangeText(0);
          break;
        case ROUTING_MODE:
          EditDesignRulesNetKeyPressed=1;
          break;
