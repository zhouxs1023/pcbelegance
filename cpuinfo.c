...
int SetUpRDMSR()
{
  if( FlagBrand == INTEL && ( Family == 0xF || (Family == 6 && Model >= 7)) ){ /* Pentium III & P4 */
  ...
  if( FlagBrand == INTEL && Family == 6 ){
    ...
    if( Model == 9 || Model >= 0xC ){// Banias & Dothan for Multiplier
...
*/
  ...
  }else if( FlagBrand == AMD && Family == 5 && Model >= 8 ){// K6-2 & K6-III
...
// Extended CPUID
  ...
  if( MaxCPUIDEx > 0x80000000 ){
    ...
    FeatureEx = EDX;
    FeatureExEcx = ECX;
    if( FlagBrand == AMD ){// for Hammer or later
...
// Family / Model / Step
  ...
  if(FlagHT && FlagBrand == INTEL){ // EnableHyperThreading
...
// FSBMode //
  if(FlagBrand == INTEL && Family == 0xF && FamilyEx == 0x0){
    FSBMultiplier = 4;
    strcpy(FSBMode,"QDR");
  }else if(FlagBrand == INTEL && Family == 0x6 && (Model == 0x9 || Model >= 0xC)){//Pentium M & Intel Core
    FSBMultiplier = 4;
    strcpy(FSBMode,"QDR");
