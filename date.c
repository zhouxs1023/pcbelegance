...
int32      Years[28]={5,7,8,9,10,12,13,14,15,17,18,19,20,22,23,24,25,27,28,29,30,32,33,34,35,37,38,39};
...
//  6  = Saterday
  ...
  hulp=0;
  if (Month>1) {
    for (cnt=1;cnt<Month;cnt++) hulp+=MonthDays[cnt-1];
    if (((Year % 4) == 0)
       &&
       (Month>2)) hulp++;
  }
...

{
  ...
  *NextYear=Year;
  ...
  if (NrDays<0) {
    while ((count<100)
          &&
          ((res=GetDateOfDayOfYear(DayOfYear,*NextYear,NextDay,NextMonth))==-1)) {
      *NextYear=*NextYear-1;
      if ((*NextYear % 4) == 0) {
        DayOfYear+=366;
      } else {
...
*/
  ...
  if ((res=_findfirst(FileName,&fileinfo))==-1) return -1;
...
//  year-=1900;
//  mon--;
  if (0 >= (int32) (mon -= 2)) {    /* 1..12 -> 11,12,1..10 */
  mon += 12;      /* Puts Feb last since it has leap day */
  year -= 1;
  }
...
#define _BASE_DOW         4                    /* 01-01-1970 was a Thursday */
  ...
  if ( SecondsAfter1970 < 0L ) return 0;
