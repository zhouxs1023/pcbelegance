...
static int32       SharedMemoryLength,*SharedMemoryPos,MaxDebugStringLength,
            *StringLengthP,MaxDebugStringLength,*OptionsP;
...

{
  ...
  switch (mode) {
    ...
    case 1:
      ...
      sec=temp_seconds;
      micro_seconds=(int32)(hulp % 1000000);
      sprintf(TimeString,"%02d-%02d-%4d %02d:%02d:%02d.%06d",
              CurrentDateProgramStart.wDay,
              CurrentDateProgramStart.wMonth,
              CurrentDateProgramStart.wYear,
              hour,min,sec,micro_seconds);
      break;
    case 2:
...

{
  ...
  int64 SpecialCounter,hulp;
  int32 micro_seconds,sec,cnt,cnt2,cnt3,length,
        length_string,length_string2;
  int32 *int32p1,*int32p2;

  if ((DebugSharedMemory==NULL)
     ||
     (!Str)
     ||
     ((length_string=(int32)strlen(Str))==0)) {
    return -1;
  }
