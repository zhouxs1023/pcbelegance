...
int32                       DrawWindowMinX,DrawWindowMinY,DrawWindowMaxX,DrawWindowMaxY,
                            MessageHeight,TimerValue,WindowStartX,WindowStartY,ScreenStartX,ScreenStartY,
...
HBITMAP                     BackgroundBitmap,
                            BackAnnotateButtonBitmap,
                            SymbolButtonBitmap,
                            SchematicButtonBitmap,
                            AnnotateButtonBitmap,
                            NetlistButtonBitmap,
                            LayoutButtonBitmap,
                            GeometryButtonBitmap,
                            GerberButtonBitmap,
                            BOMButtonBitmap,
                            BitmapDown,
                            BitmapUp;
...
#else
  if (DesignName) {
    sprintf(str,"Design %s",DesignName);
  } else {
    sprintf(str,"Design manager");
  }
...
{
  int32  cnt,cnt2;
  char   OpenSchematics[32][MAX_LENGTH_STRING],
...
// 4 -> layout
  ...
  for (cnt=0;cnt<cnt2;cnt++) {
    ...
    sprintf(ExeParams,"\"%s\" \"%s\" /a /o /e \"%s\"",
            ExeFile,OpenSchematics[cnt],ExePath);
...

{
  ...
  while ((cnt<32)
        ...
        ||
        (stricmpUTF8(ProjectInfo->FileNames[cnt],ActiveFile)!=0))) cnt++;
...
//    SelectObject(WindowsDisplay, SaveFont);
    if ( Painting ) EndPaint(DESIGNWindow, &PS);
    else ReleaseDC(DESIGNWindow, OutputDisplay);
    OutputDisplay=(HDC)-1;
    DCInUse=0;
  }
}
...

{
  ...
  if (mode==1) {
    ...
    if ((WindowStartX>x)
       ...
       (WindowStartY>y)
       ||
       (x+IntroBitmapSizeX>WindowStartX+WindowWidth)
       ||
...
//  CrtWindow = Window;
  ...
  switch ( Message ) {
    ...
    case WM_SIZE:
      ...
      if (EditWindow) {
        MoveWindow (EditWindow,1,MessageStartY,
                    WindowWidth-11,MessageHeight+2, 0);
...

{
  ...
  if ((ID<0)
     ||
     (ID>=MaxNrDesignNames)) return -1;
  if (DesignNamesPos+Length+2>=DesignNamesBufSize) return -1;
    ...
    if (Parameters[cnt].Option[0]!=0) {
      ...
      if ((mode==0)
        ...
        if ((Utf8ToUnicode(Parameters[cnt].Parameter,EditFileW,MAX_LENGTH_STRING-50)==0)
          ...
          MessageBoxUTF8(DESIGNWindow,Parameters[cnt].Parameter,
...
/*
  ...
  MessageBoxUTF8(0,GetCommandLine(),SC(8,"Message"),MB_APPLMODAL|MB_OK);
  return 0;
*/
...
//  res=TestLineConnectedToCircle(152.0,78.0,152.0,95.0,151.0,94.0,0.04);
  ...
  if (AddDesignLanguageStrings(str)==-2) return -1;
...
//  GetNameWithOutExtensionFromFileName("dgdfg\\1234.ewfqef",str);
  ...
  strcpy(WindowStr,SC(212,"Design manager"));
...
//    DESIGNClass.hbrBackground = COLOR_APPWORKSPACE+1;
    ...
    if ( ! RegisterClass(&DESIGNClass) ) exit(255);
  }
  ClipCursor(NULL);

  OkToUseSharedMemory=0;
  if ((SharedMemoryHandle = OpenFileMapping(FILE_MAP_WRITE,0,MEMORYMAPPEDSTRING))) {
    // The memory mapped file has already been created by another application
    ok=1;
#ifndef _DEBUG
    Beep(1000,200);
...
// The memory mapped file should be created
    if (!(SharedMemoryHandle = CreateFileMapping((HANDLE)0xFFFFFFFF,NULL,PAGE_READWRITE,
                                                 0,DefSharedMemoryLength,MEMORYMAPPEDSTRING))) {
...
//    SaveSymbolsLocally=1;
  ...
  NewFont=CreateFont(16,0,0,0,FW_BOLD,0,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,
                     CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,
                     "MS Sans Serif");
  MessagesFont=CreateFont(-1,0,0,0,0,0,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,
                          CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,
                          "MS Sans Serif");
...
//  WindowStyle|=WS_THICKFRAME;
  DESIGNWindow = CreateWindow(DesignClassName,
                             PWindowStr,
                             WindowStyle,
                             WindowStartX,
                             WindowStartY,
  ...
  EditWindow = CreateWindow("EDIT",
                             NULL,
                             WS_CHILD|ES_READONLY|ES_MULTILINE|WS_SIZEBOX|
