...
int32   NetDialogMode,NetsSelected,DialogResult,DialogMode,CurrentFontNr,
        ErrorNr,TempUnits,NetSelected,SelectedColorNr,OldValue4,OldValue5,
        NrTempGerberLayers,LayerCodes[64],LayerInfo[64],
        PlotLayers[512],NrPlotLayers,NrBitmapExportResolutions=11,
        *NetInfo,NrPageOutputIds=11,ok,
        NrGeometryLibFiles,CurrentGeometryLibraryNr,CurrentGeometryNrInLibrary;
...
double  TempTraceWidth,TempClearance,RotationCentreX,RotationCentreY,
        RotationValue,OldValue1,OldValue2,OldValue3;

char    DialogTextLine[2048],GerberLayerStr[30][128],
        GeometryLibNames[64][160],SearchString2[MAX_LENGTH_STRING],
        TempGeometrieName[MAX_LENGTH_STRING],GeometrieEditor[MAX_LENGTH_STRING];

uint8   *SelectedNets,*SelectedComponents;

LPSTR   InitDialogTextLine,DialogWindowText,LineInputDialogText,WorkingTextLong,
        MultiLineText,ViewText;
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDOK:
          ...
          if (Found!=LB_ERR) {
            SendDlgItemMessageOwn(Dialog,IDD_LISTBOX_NETS,LB_GETTEXT,Found,
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      switch (NetDialogMode) {
        ...
        case 1:
          ...
          break;
        case 2:
          sprintf(str,SC(164,"Disable connections net(s)      [%i nets]"),Design.NrNets);
          SetWindowTextOwn(Dialog,str);
          break;
        case 3:
          ...
          switch (NetDialogMode) {
            case 0:
              if ((Net->Info & OBJECT_HIGHLITED) == OBJECT_HIGHLITED) {
                cnt2=SendDlgItemMessageOwn(Dialog,IDD_LISTBOX_NETS,LB_FINDSTRING,(WPARAM)-1,(LPARAM)(LPCTSTR)&str);
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDOK:
          NetsSelected=(int32)SendDlgItemMessage(Dialog,IDD_LISTBOX_NETS,LB_GETSELITEMS,
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetWindowTextOwn(Dialog,(LPSTR)SC(167,"Unconnected nets"));
      for (cnt=0;cnt<Design.NrNets;cnt++) {
        Net=&((*Nets)[cnt]);
        if (Net->Name[0]!=0) {
          if (Net->Dummy==1) {
...

{
  ...
  NetDialogMode=0;
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_NETS), PCBWindow, (DLGPROC)UnconnectedNetsDialogBody);
}
...

{
  ...
  if (res==1) UnselectTracesViasNet(NetsSelected,NetInfo,SelectedNets);
  DeallocateSpecialMem(MEM_NETINFO);
  DeallocateSpecialMem(MEM_NET_SELECTED);
}
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      switch (DialogMode & 0x0f) {
        case 2:
          SetWindowTextOwn(Dialog,SC(184,"Edit component value"));
          break;
        case 3:
          SetWindowTextOwn(Dialog,SC(185,"Scale factor"));
          break;
        case 4:
          SetWindowTextOwn(Dialog,SC(186,"Rotation (degrees)"));
          break;
        case 5:
          SetWindowTextOwn(Dialog,SC(187,"Change start/end angle (degrees counter clockwise)"));
          break;
        case 6:
          SetWindowTextOwn(Dialog,SC(1204,"Goto x,y"));
          break;
        case 7:
          SetWindowTextOwn(Dialog,SC(1298,"Rotate each at any angle CCW"));
          break;
      }
        ...
        case IDD_UNITS:
          if ((DialogMode & 0x10) == 0) {
            ...
            if (TempUnits==0) {
              ...
              SendDlgItemMessageOwn(Dialog,IDC_EDIT4,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(131,"mm"));
            }
          }
          break;
        case IDOK:
          res=SendDlgItemMessageOwn(Dialog,IDD_TEXTINPUT_EDIT1,WM_GETTEXT,MAX_LENGTH_STRING-1,(LPARAM)DialogTextLine);
          if (res>0) {
          ...
          if ((DialogMode & 0x10) == 0) {
            ...
            if (WorkingObjectText->FontHeight<1000.0) {
              MessageBoxOwn(PCBWindow,SC(188,"Wrong value for text height"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
            WorkingObjectText->LineThickNess=(float)GetDialogValue(Dialog,IDC_EDIT3);
            if (WorkingObjectText->LineThickNess<100.0) {
              MessageBoxOwn(PCBWindow,SC(189,"Wrong value for line thickness"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      switch (DialogMode) {
        case 0:
          SetWindowText(Dialog,SC(190,"Add text"));
          break;
        case 2:
          SetWindowTextOwn(Dialog,SC(190,"Add text"));
          break;
        case 1:
          SetWindowText(Dialog,SC(191,"Change text"));
          break;
        case 3:
          SetWindowTextOwn(Dialog,SC(191,"Change text"));
          break;
      }
      TempUnits=Units;
      SetDialogValue(Dialog,IDC_EDIT1,WorkingObjectText->FontHeight);
      ...
      switch (DialogMode) {
        ...
        case 3:
          ...
          if (Design.UsedFontStr[0][0]==0) {
            strcpy(Design.UsedFontStr[0],SC(1162,"Arial"));
            strcpy(Design.UsedFontStr[1],SC(1163,"Courier New"));
            strcpy(Design.UsedFontStr[2],SC(1164,"Times New Roman"));
#ifdef _DEBUG
        ...
        case IDOK:
          ...
          for (cnt=255;cnt>=0;cnt--) {
            ...
            *Lengte=MAX_LENGTH_STRING-10;
            if ((res=SendDlgItemMessageOwn(Dialog,IDD_TEXTINPUT_EDIT1,EM_GETLINE,cnt,(LPARAM)DialogTextLine))>0) {
              if (NrLines==-1) NrLines=cnt+1;
            }
          }
          ok=1;
          ...
          switch (DialogMode) {
            case 1:
              if ((OldValue1!=WorkingObjectText->FontHeight)
                 ||
                 (OldValue3!=WorkingObjectText->Rotation)
                 ||
                 (OldValue2!=WorkingObjectText->LineThickNess)
...

{
  ...
  if ((DialogMode & 0x10) == 0) {
    switch (DialogMode & 0x0f) {
      case 0:
        res=OwnDialogBox(PCBClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT4), PCBWindow, (DLGPROC)TextInputDialog4);
        break;
      case 1:
        res=OwnDialogBox(PCBClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT4), PCBWindow, (DLGPROC)TextInputDialog4);
        break;
...
*/
        ...
        FileClose(fp);
#endif
        res=DialogBoxW(PCBClass.hInstance,(LPWSTR)MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT5), PCBWindow, (DLGPROC)TextInputDialog4);
        break;
      case 3:
      case 4:
        res=DialogBoxW(PCBClass.hInstance,(LPWSTR)MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT5), PCBWindow, (DLGPROC)TextInputDialog4);
    ...
    res=OwnDialogBox(PCBClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT2), PCBWindow, (DLGPROC)TextInputDialog1);
  }
  return res;
}
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,"");
      SetWindowTextOwn(Dialog,LineInputDialogText);
...

{
  ...
  DialogMode=Mode;
  MultiLineText=Text;
  res=OwnDialogBox(PCBClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT3), PCBWindow, (DLGPROC)TextInputDialog3);
  return res;
}
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      switch (DialogMode) {
        case 0:
          SetWindowTextOwn(Dialog,SC(193,"Change trace width"));
          break;
        case 1:
          SetWindowTextOwn(Dialog,SC(194,"Change clearance width"));
          break;
        case 2:
          SetWindowTextOwn(Dialog,SC(195,"Change text height"));
          break;
        case 3:
          SetWindowTextOwn(Dialog,SC(196,"Change line width"));
          break;
        case 4:
          SetWindowTextOwn(Dialog,SC(197,"Change diameter"));
          break;
        case 5:
          SetWindowTextOwn(Dialog,SC(198,"Change width/height"));
          break;
        case 6:
          SetWindowTextOwn(Dialog,SC(187,"Change start/end angle (degrees counter clockwise)"));
          break;
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        ...
        case IDOK:
          ...
          if ((value1<NewValue.MinValue)
             ||
             (value1>NewValue.MaxValue)) {
            MessageBoxOwn(PCBWindow,SC(199,"Wrong value"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          value2=0.0;
          if ((DialogMode==5)
            ...
            if ((value2<NewValue.MinValue2)
               ||
               (value2>NewValue.MaxValue2)) {
              MessageBoxOwn(PCBWindow,SC(199,"Wrong value"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
          }
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(200,"Rotation angle (degrees counter clockwise)"));
      SetWindowTextOwn(Dialog,SC(183,"Rotation"));
      SetDialogItemText(Dialog,IDC_RADIO1                 ,SC(201,"Rotate around selection centre (On grid)"));
        ...
        case IDOK:
          ...
          if (res>0) {
            ...
            if (RotationValue==0.0) {
              MessageBoxOwn(PCBWindow,SC(205,"Rotation is zero"),SC(102,"Warning"),MB_APPLMODAL|MB_OK);
            }
          ...
          } else MessageBoxOwn(PCBWindow,SC(206,"Wrong rotation value"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
          return about;
        case IDCANCEL:
          memset(DialogTextLine,0,200);
          EndDialog(Dialog, 10);
...
{
  int32   about,TabStops[10],BaseUnits,cnt,cnt2,cnt3,TabPos[10],MaxCharWidth,
          MaxStringLength[10],MaxStringDialogUnits[10],MaxStringPixels[10],MaxTabPos[10],NrTabs;
  char    str[MAX_LENGTH_STRING];
  LPSTR   BufP,LineP;
  HFONT   FontHandle;
  HDC     DesktopHdc;
...
*/
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDOK                       ,SC(155,"OK"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_BUTTON1                ,SC(1092,"Copy text to clipboard"));
      SetWindowTextOwn(Dialog,SC(207,"Messages"));
...
*/
          ...
          SetDialogItemText(Dialog,IDC_EDIT1,str);
          break;
        case 6: // objects
          TabStops[0]=( 75 * 13) / 20;
...

{
  ...
  res=OwnDialogBoxParam(PCBClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_MESSAGE), PCBWindow,
                        (DLGPROC)MessageDialog2,ObjectsSelected);
  ok=1;
  return res;
}
...
//  Static_SetText sscanf
          memset(&(WorkingObjectText->Text),0,sizeof(WorkingObjectText->Text));
          if ((res=SendDlgItemMessageOwn(Dialog,IDD_LINEINPUT_EDIT1,WM_GETTEXT,MAX_LENGTH_STRING-1,(LPARAM)DialogTextLine))>0) {
            strncpy(WorkingObjectText->Text,DialogTextLine,sizeof(WorkingObjectText->Text)-1);
            EndDialog(Dialog, 1);
          } else EndDialog(Dialog, 2);
          return about;
...

{
  ...
  WorkingObjectText=ObjectText;
  DialogMode=mode;
  res=OwnDialogBox(PCBClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_LINEINPUT), PCBWindow, (DLGPROC)LineInputDialog2);
  ok=1;
  return res;
}
  ...
  switch ( Message ) {
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDOK:
          memset(&(WorkingObjectText->Text),0,sizeof(WorkingObjectText->Text));
          if ((res=SendDlgItemMessageOwn(Dialog,IDC_EDIT1,WM_GETTEXT,MAX_LENGTH_STRING-1,(LPARAM)DialogTextLine))>0) {
            strncpy(WorkingObjectText->Text,DialogTextLine,sizeof(WorkingObjectText->Text)-1);
            EndDialog(Dialog, 1);
          } else EndDialog(Dialog, 2);
          return about;
...

{
  ...
  res=OwnDialogBox(PCBClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_LINEINPUT), PCBWindow, (DLGPROC)LineInputDialogLong2);
  ok=1;
  return res;
}
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(209,"Net name"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(210,"Thermal relief"));
      SetDialogItemText(Dialog,IDC_STATIC3                ,SC(211,"Clearance"));
      SetDialogItemText(Dialog,IDC_STATIC10               ,SC(212,"Areafill inside powerplane"));
      SetDialogItemText(Dialog,IDC_STATIC5                ,SC(213,"Clearance areafill inside powerplane"));
      SetDialogItemText(Dialog,IDC_STATIC6                ,SC(214,"Powerplane size"));
      SetDialogItemText(Dialog,IDC_STATIC7                ,SC(215,"Powerplane"));
        ...
        case IDOK:
          ...
          if (DialogMode==0) {
            if ((value1=GetDialogValue(Dialog,IDC_EDIT1))<100.0) {
              MessageBoxOwn(PCBWindow,SC(219,"Wrong value for clearance width in powerplane"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
            DialogAreaFill->SurroundThickNess=(float)value1;
          }
          if ((value2=GetDialogValue(Dialog,IDC_EDIT3))<100.0) {
            MessageBoxOwn(PCBWindow,SC(220,"Wrong value for clearance width"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          DialogAreaFill->Clearance=(float)value2;
          if (DialogMode==2) {
            if ((value2=GetDialogValue(Dialog,IDC_EDIT9))<100.0) {
              MessageBoxOwn(PCBWindow,SC(221,"Wrong value for surround thickness"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
          ...
          if (SendDlgItemMessageOwn(Dialog,IDC_CHECK2,BM_GETCHECK,0,0)) {
            ...
            if ((value1=GetDialogValue(Dialog,IDC_EDIT7))<100.0) {
              MessageBoxOwn(PCBWindow,SC(222,"Wrong value for thermal relief"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
          ...
          if (NetSelected!=-1) EndDialog(Dialog,1);
          else EndDialog(Dialog,2);
          return about;
        case IDD_UNITS:
          if (DialogMode==0) {
...

{
  ...
  switch (DialogMode) {
    case 0:
      res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_AREAFILL_INPOWER35), PCBWindow, (DLGPROC)AreaFillDialogBody);
      break;
    case 1:
      res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_AREAFILL35), PCBWindow, (DLGPROC)AreaFillDialogBody);
      break;
    case 2:
      res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_AREAFILL_POWER35), PCBWindow, (DLGPROC)AreaFillDialogBody);
      break;
  }
...
//          DialogAreaFill->Clearance=value2;
          if (SendDlgItemMessageOwn(Dialog,IDC_CHECK2,BM_GETCHECK,0,0)) {
            DialogAreaFill->Info|=AREAFILL_WITH_THERMAL_RELIEF;
            if ((value1=GetDialogValue(Dialog,IDC_EDIT7))<100.0) {
              MessageBoxOwn(PCBWindow,SC(223,"Wrong value for width"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
            if ((value2=GetDialogValue(Dialog,IDC_EDIT8))<100.0) {
              MessageBoxOwn(PCBWindow,SC(223,"Wrong value for width"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
...

{
  ...
  DialogAreaFill=AreaFill;
  NetSelected=-1;
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_AREAFILL_EDIT35), PCBWindow, (DLGPROC)AreaFillDialogBody2);
  return res;
}
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(227,"Origin"));
      SetDialogItemText(Dialog,IDC_STATIC5                ,SC(228,"Board outline keep out"));
      SetDialogItemText(Dialog,IDC_STATIC6                ,SC(229,"Design rules"));
      SetDialogItemText(Dialog,IDC_STATIC7                ,SC(170,"Trace width"));
      SetDialogItemText(Dialog,IDC_STATIC8                ,SC(211,"Clearance"));
      SetDialogItemText(Dialog,IDC_STATIC9                ,SC(230,"Silkscreen width"));
      SetDialogItemText(Dialog,IDC_STATIC10               ,SC(231,"Nr Layers"));
      SetDialogItemText(Dialog,IDC_STATIC11               ,SC(232,"Via definition"));
      SetDialogItemText(Dialog,IDC_STATIC12               ,SC(233,"Pad size"));
      SetDialogItemText(Dialog,IDC_STATIC13               ,SC(234,"Drill diameter"));
      SetDialogItemText(Dialog,IDC_STATIC14               ,SC(211,"Clearance"));
      SetDialogItemText(Dialog,IDC_STATIC15               ,SC(235,"Anti power pad"));
      SetDialogItemText(Dialog,IDC_STATIC16               ,SC(236,"Solder mask"));
      SetDialogItemText(Dialog,IDC_STATIC17               ,SC(237,"Board outline width"));
      ...
      SetWindowTextOwn(Dialog,SC(241,"New layout"));
      TempUnits=Units;
/*
      memset(&NewDesign,0,sizeof(DesignRecord));
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        ...
        case IDOK:
          ...
          if (Found!=LB_ERR) {
            SendDlgItemMessageOwn(Dialog,IDD_NEW_COMBO1,CB_GETLBTEXT,Found,
                               (LPARAM)DialogTextLine);
          ...
          if ((value=GetDialogValue(Dialog,IDD_NEW_EDIT3))<10000.0) {
            MessageBoxOwn(PCBWindow,SC(242,"Wrong value for height"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          NewDesign.BoardHeight=(float)value;
...
*/
  ...
  if (Set==1) { // Add this layer to the plottable layers
    PlotLayers[Layer2]=1;
  } else {      // Remove this layer from the plottable layers
    PlotLayers[Layer2]=0;
  }
  return 0;
}
...
*/
  ...
  if (Design.NrBoardLayers>1) {
    sprintf(GerberLayerStr[count],SC(258,"Routing keepout\tTop"));
  ...
  if (Design.NrBoardLayers>2) {
    for (cnt=1;cnt<Design.NrBoardLayers-1;cnt++) {
      ...
      sprintf(GerberLayerStr[count],SC(259,"Routing keepout\t%s"),str);
      LayerCodes[count++]=ROUTING_KEEPOUT_LAYER+Design.NrBoardLayers-1-cnt;
    }
  }
  sprintf(GerberLayerStr[count],SC(260,"Routing keepout\tBottom"));
...

{
 ...
 if ((GerberInfo.GerberNumberMode & 8) == 0) { // Mils
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156 ,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157 ,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(261 ,"Gerber output format (RS274X)"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(262 ,"Number format"));
      SetDialogItemText(Dialog,IDC_STATIC3                ,SC(263 ,"PCB testing"));
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(264 ,"Mirror"));
      SetDialogItemText(Dialog,IDC_STATIC5              ,SC(1155,"Material"));
      SetDialogItemText(Dialog,IDC_STATIC6              ,SC(216 ,"Thickness"));
      SetDialogItemText(Dialog,IDC_STATIC7              ,SC(1156,"Copper thickness"));
      SetDialogItemText(Dialog,IDC_STATIC11             ,SC(1086,"PCB"));
      SetDialogItemText(Dialog,IDC_CHECK5               ,SC(1159 ,"Reverse layer numbering"));
      SetDialogItemText(Dialog,IDC_CHECK6               ,SC(1160 ,"Output drill data as gerber file"));
      SetDialogItemText(Dialog,IDC_STATIC8                ,SC(265 ,"Drill option"));
      SetDialogItemText(Dialog,IDC_STATIC9                ,SC(266 ,"Layers"));
      SetDialogItemText(Dialog,IDC_STATIC10               ,SC(267 ,"Options"));
      SetDialogItemText(Dialog,IDC_STATIC12               ,SC(1227 ,"Extra"));
      SetDialogItemText(Dialog,IDC_RADIO4                 ,SC(268 ,"Component references on silkscreen"));
      SetDialogItemText(Dialog,IDC_RADIO5                 ,SC(269 ,"Component values on silkscreen "));
...
//      SetDialogItemText(Dialog,IDC_STATIC13               ,SC(1288,"Areafill option for hatch fill"));
      ...
      sprintf(str2,"%s\r\n%s\r\n%s\r\n%s",
                           GerberInfo.TextLine[0],
                           GerberInfo.TextLine[1],
                           GerberInfo.TextLine[2],
                           GerberInfo.TextLine[3]);
      SendDlgItemMessageOwn(Dialog,IDC_EDIT1,WM_SETTEXT,0,(LPARAM)str2);
      sprintf(str2,"%s\r\n%s\r\n%s\r\n%s",
                           GerberInfo.TextLine[4],
                           GerberInfo.TextLine[5],
                           GerberInfo.TextLine[6],
...
//      SendDlgItemMessageOwn(Dialog,IDC_EDIT2,WM_SETTEXT,0,(LPARAM)AperTureFile);
      ...
      for (cnt=0;cnt<NrTempGerberLayers;cnt++) {
        ...
        if ((Layer!=10000)
           &&
           (IsLayerPlottable(Layer))) {
          if ((LayerString=GetLayerString(Layer))!=NULL) {
            res=SendDlgItemMessageOwn(Dialog,IDC_LIST1,LB_FINDSTRINGEXACT,(UINT)-1,(LPARAM)LayerString);
            SendDlgItemMessageOwn(Dialog,IDC_LIST1,LB_SETSEL,1,(LPARAM)res);
          }
        }
      }
      if (GerberInfo.PcbMaterial[0]==0) {
        strcpy(GerberInfo.PcbMaterial,SC(1157,"FR4"));
      }
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(279,"Plot options"));
      SetDialogItemText(Dialog,IDC_STATIC5                ,SC(185,"Scale factor"));
      SetDialogItemText(Dialog,IDC_STATIC6                ,SC(280,"Pen speed (cm/sec)"));
      SetDialogItemText(Dialog,IDC_STATIC7                ,SC(281,"Offset"));
      SetDialogItemText(Dialog,IDC_STATIC8                ,SC(266,"Layers"));
      SetDialogItemText(Dialog,IDC_STATIC9                ,SC(185,"Scale factor"));
      SetDialogItemText(Dialog,IDC_STATIC10               ,SC(1086,"PCB"));
      SetDialogItemText(Dialog,IDD_CLEAR                  ,SC(273,"Clear all"));
      SetDialogItemText(Dialog,IDD_SETALL                 ,SC(274,"Set all"));
      ...
      SetDialogItemText(Dialog,IDC_RADIO5                 ,SC(269,"Component values on silkscreen "));
      if (DialogMode==0) {
        SetWindowTextOwn(Dialog,SC(282,"Plot output"));
      } else {
        SetWindowTextOwn(Dialog,SC(283,"Penplot output"));
      }
      TempUnits=1;
      if (GerberInfo.Invert) {
        SendDlgItemMessageOwn(Dialog,IDC_CHECK1,BM_SETCHECK,1,0);
      ...
      if (DialogMode==1) {
        ...
        SendDlgItemMessageOwn(Dialog,IDC_EDIT7,WM_SETTEXT,0,(LPARAM)SC(131,"mm"));
        sprintf(str,"%.4f",GerberInfo.PenSizes[1]/100000);
        SendDlgItemMessageOwn(Dialog,IDC_EDIT6,WM_SETTEXT,0,(LPARAM)str);
        SendDlgItemMessageOwn(Dialog,IDC_EDIT8,WM_SETTEXT,0,(LPARAM)SC(131,"mm"));
        ...
        case IDOK:
          ...
          if (DialogMode==1) {
            ...
            if ((value1<-10000e5)
               ||
               (value1>10000e5)) {
              MessageBoxOwn(PCBWindow,SC(284,"Wrong X offset"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
            ...
            if ((value1<-10000e5)
               ||
               (value1>10000e5)) {
              MessageBoxOwn(PCBWindow,SC(285,"Wrong Y offset"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
            ...
            ok=1;
            if ((sscanf(DialogTextLine,"%f",&value1)!=1)
               ||
               (value1<0.01)
               ||
               (value1>1000.0)) {
              MessageBoxOwn(PCBWindow,SC(286,"Wrong pen speed"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
            ...
            if ((sscanf(DialogTextLine,"%f",&value1)!=1)
               ...
               ||
               (value1>10.0)) {
              MessageBoxOwn(PCBWindow,SC(287,"Wrong pen size"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
          ...
          if ((sscanf(DialogTextLine,"%f",&value1)!=1)
             ...
             ||
             (value1>1000.0)) {
            MessageBoxOwn(PCBWindow,SC(288,"Wrong scaling factor"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
...
*/
  ...
  if (mode==0) {
    ...
    res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_PENPLOT20), PCBWindow, (DLGPROC)PlotDialogBody);
  }
  return res;
}
...
#define    InRange20(x1,x2) ( (((x1>x2-1.0) && (x1<x2+1.0))) ? (1) : (0) )
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(1086,"PCB"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(279,"Plot options"));
      SetDialogItemText(Dialog,IDC_STATIC3                ,SC(289,"Bitmap resolution"));
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(266,"Layers"));
      SetDialogItemText(Dialog,IDC_STATIC5                ,SC(290,"User"));
      SetDialogItemText(Dialog,IDC_STATIC6                ,SC(291,"Compression"));
      SetDialogItemText(Dialog,IDC_RADIO4                 ,SC(268,"Component references on silkscreen"));
      SetDialogItemText(Dialog,IDC_RADIO5                 ,SC(292,"Component values on silkscreen"));
      SetDialogItemText(Dialog,IDC_RADIO6                 ,SC(1087,"Not compressed"));
      ...
      for (cnt=0;cnt<NrBitmapExportResolutions;cnt++) {
        ...
        sprintf(str,SC(296,"%d dpi"),BitmapExportResolutions[cnt]);
        ...
        case IDOK:
          ...
          if (SendDlgItemMessageOwn(Dialog,IDC_RADIO3,BM_GETCHECK,0,0)==1) {
            ...
            if ((value1<1.0)
               ||
               (value1>100000.0)) {
              MessageBoxOwn(PCBWindow,SC(297,"Wrong dot size"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
            ...
            res=SendDlgItemMessageOwn(Dialog,IDC_COMBO1,CB_GETCURSEL,0,0);
            if (res==-1) {
              MessageBoxOwn(PCBWindow,SC(298,"A value should be selected"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
...
*/
  ...
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_BITMAP_OUTPUT35), PCBWindow, (DLGPROC)ExportBitmapDialogBody);
  return res;
}
...
#define    InRange20(x1,x2) ( (((x1>x2-1.0) && (x1<x2+1.0))) ? (1) : (0) )
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDC_STATIC3                ,SC(289,"Bitmap resolution"));
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(266,"Layers"));
      SetDialogItemText(Dialog,IDC_STATIC5                ,SC(299,"Paper options"));
      SetDialogItemText(Dialog,IDC_STATIC6                ,SC(300,"Orientation"));
      SetDialogItemText(Dialog,IDC_STATIC7                ,SC(301,"Scale"));
      SetDialogItemText(Dialog,IDC_STATIC8                ,SC(302,"Paper size"));

      SetDialogItemText(Dialog,IDC_RADIO4                 ,SC(268,"Component references on silkscreen"));
      SetDialogItemText(Dialog,IDC_RADIO5                 ,SC(292,"Component values on silkscreen"));
      SetDialogItemText(Dialog,IDC_CHECK2                 ,SC(293,"Plot board outline on each layer"));
      SetDialogItemText(Dialog,IDC_RADIO6                 ,SC(303,"Auto"));
      SetDialogItemText(Dialog,IDC_RADIO7                 ,SC(304,"Portrait"));
      SetDialogItemText(Dialog,IDC_RADIO8                 ,SC(305,"Landscape"));
      SetDialogItemText(Dialog,IDC_RADIO9                 ,SC(306,"Scale 1X"));
      SetDialogItemText(Dialog,IDC_RADIO10                ,SC(307,"Fit to page"));
      SetDialogItemText(Dialog,IDD_CLEAR                  ,SC(273,"Clear all"));
      SetDialogItemText(Dialog,IDD_SETALL                 ,SC(274,"Set all"));
      ...
      SetWindowTextOwn(Dialog,SC(308,"PDF output"));
      if (GerberInfo.PlotBoardOutline) {
        SendDlgItemMessageOwn(Dialog,IDC_CHECK2,BM_SETCHECK,1,0);
      }
...
*/
  ...
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_PDF_OUTPUT35), PCBWindow, (DLGPROC)ExportPDFDialogBody);
  return res;
}
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDOK         ,SC(155,"OK"));
      sprintf(str,SC(309,"Build version %i.%i.%i  [ %s ]"),
                  PROGRAM_VERSION/100,PROGRAM_VERSION % 100,BUILD_VERSION,TIME_STRING);
#ifdef GCC_COMP
...
#endif
      SendDlgItemMessageOwn(Dialog,IDC_EDIT2,WM_SETTEXT,0,(LPARAM)(LPSTR)str);
      return about;
    case WM_MOVE:
      break;
    case WM_COMMAND:
...

{
  ...
  char    sel[MAX_LENGTH_STRING],RefName[MAX_LENGTH_STRING],PositionX[MAX_LENGTH_STRING],
          PositionY[MAX_LENGTH_STRING],Rotation[MAX_LENGTH_STRING],LineCopy[MAX_LENGTH_STRING],
          UnitsStr[MAX_LENGTH_STRING];
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetWindowTextOwn(Dialog,SC(310,"Component placement"));
      return about;
    case WM_MOVE:
      break;
    case WM_COMMAND:
...
// Component  U100  Top DIP24S   <partnr> GAL20V8     1000.0,10020.0 mil  0
          ...
          for (cnt=0;cnt<5000;cnt++) {
            ...
            *Lengte=MAX_LENGTH_STRING-1;
            ...
            if (res>MAX_LENGTH_STRING-10) res=MAX_LENGTH_STRING-10;
            DialogTextLine[res]=0;
...
//                if (stricmpOwn(sel,SC(311,"top"))==0) Layer=0;
                if (stricmpOwn(sel,SC(312,"bottom"))==0) Layer=1;
                strcpy(LineCopy,DialogTextLine);
                ...
                y=0.0;
                MultFactor=100000.0;
                if ((sscanf(PositionX,"%f",&x)==1)
                   &&
                   (sscanf(PositionY,"%f",&y)==1)
                   &&
                  ...
                  if ((stricmpOwn(UnitsStr,"mils")==0)
                     ||
                     (stricmpOwn(UnitsStr,"mil")==0)) {
                    MultFactor=2540.0;
                  }
                  x*=MultFactor;
                  y*=MultFactor;
...
//  DialogWindowText=DialogText;
  DialogMode=Mode;
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_SETCOMP), PCBWindow, (DLGPROC)ComponentPlacement2);
  ok=1;
  return res;
}
...

{
  ...
  if ((Libfp=FileOpenReadOnlyUTF8(str))<=0) return 0;
...

{
  ...
  char    str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING*5],
          InitStr[MAX_LENGTH_STRING],ExeFile[MAX_LENGTH_STRING],ExeParams[MAX_LENGTH_STRING*5];
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(314,"Geometry source"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(315,"Geometries"));
      SetWindowTextOwn(Dialog,SC(316,"Select geometry"));
...
//              CurrentGeometryNrInLibrary=Found2;
            ...
            if (FileExistsUTF8(GeometrieEditor)!=0) break;

            if (LibMode==0) {
              if (FileExistsUTF8(str)==0) {
                sprintf(ExeParams,"\"%s\" \"%s\" /S %i,%i,%i,%i %s /a /e \"%s\" /w %x",
                        GeometrieEditor,str,GeomScreenWidth,GeomScreenHeight,
                        GeomStartX,GeomStartY,InitStr,ExePath,(int32)PCBWindow);
                if (ClosingWindowMessage!=0) {
                  SendNotifyMessage(HWND_BROADCAST,ClosingWindowMessage,0,1);
...
//                  WinExec(str3,SW_SHOW);
                ...
                MessageBoxOwn(PCBWindow,str3,SC(100,"Error"),MB_APPLMODAL|MB_OK);
                return about;
              }
            } else {
...
//  sprintf(GeometrieEditor,"E:\\new\\geom\\Debug\\geom.exe");
  ...
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_EDITGEOMETRY), PCBWindow, (DLGPROC)SelectGeomDialog2);
  if (ActiveViewGeometry) {
    ActiveViewGeometry=0;
    ViewFull();
  }
  if (res==1) {
    if (TempGeometrieName[0]!=0) strcpy(GeometrieName,TempGeometrieName);
  }
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SelectionEsc=0;
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(323,"Current back annotations"));
      SetDialogItemText(Dialog,IDAPPEND                   ,SC(324,"Append to current back annotations"));
      SetDialogItemText(Dialog,IDDELETE                   ,SC(325,"Remove current back annotations"));
      SetWindowTextOwn(Dialog,SC(326,"Back annotation"));
...

{
  ...
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_BACK_ANNOTATE), PCBWindow, (DLGPROC)BackAnnotationDialogBody);
  return res;
}
...

{
  ...
  char    str[MAX_LENGTH_STRING],LayerString[MAX_LENGTH_STRING],
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDOK                       ,SC(155 ,"OK"));
      SetDialogItemText(Dialog,IDHELP                     ,SC(156 ,"Help"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(1257,"Sort by"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157 ,"Cancel"));
      SetDialogItemText(Dialog,IDC_BUTTON1                ,SC(1268,"Move components"));
      SetDialogItemText(Dialog,IDC_BUTTON2                ,SC(1269,"Center screen on component"));
      SetDialogItemText(Dialog,IDD_SELECTALL              ,SC(160 ,"Select all"));
      SetDialogItemText(Dialog,IDD_DESELECTALL            ,SC(161 ,"Deselect all"));
      ...
      SetWindowTextOwn(Dialog,SC(1246,"Component selections"));
      return about;
    case WM_MOVE:
      break;
    case WM_COMMAND:
...

{
  ...
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_COMP_SEL), PCBWindow,
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(1085,"Units"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(1277,"Component value/part number option"));
      SetDialogItemText(Dialog,IDC_STATIC3                ,SC(1278,"SMD/Through hole components"));
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(1279,"Special options"));
      SetDialogItemText(Dialog,IDC_RADIO1                 ,SC(954,"SMD"));
      SetDialogItemText(Dialog,IDC_RADIO2                 ,SC(1281,"SMD/Through hole"));
      ...
      if (CompPosOutput.NotPlaced) {
        SendDlgItemMessage(Dialog,IDC_RADIO5,BM_SETCHECK,BST_CHECKED,0);
      } else {
        SendDlgItemMessage(Dialog,IDC_RADIO5,BM_SETCHECK,BST_UNCHECKED,0);
      }
      SetWindowTextOwn(Dialog,SC(1283,"Component positions (Pick and Place)"));
      return about;
    case WM_MOVE:
      break;
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDOK:
          ...
          if (SendDlgItemMessage(Dialog,IDC_RADIO5,BM_GETCHECK,0,0)) {
