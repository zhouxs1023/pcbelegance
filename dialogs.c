    ...
    case WM_INITDIALOG:
      ...
      switch (DialogMode & 0xf) {
        ...
        case 5:
          ...
          break;
        case 6:
          SetWindowTextUTF8(Dialog,SC(445,"Pin text"));
          break;
        case 7:
          SetWindowTextUTF8(Dialog,SC(453,"Geometry height"));
          break;
        case 8:
          SetWindowTextUTF8(Dialog,SC(457,"Goto x,y"));
          break;
      }
      if ((DialogMode & 0x10) == 0) {
        switch (Units) {
          ...
          case 1:
            ...
            SendDlgItemMessage(Dialog,IDD_GEOM_UN1,LB_DELETESTRING,0,0);
            SendDlgItemMessageUTF8(Dialog,IDD_GEOM_UN1,LB_ADDSTRING,0,(LPARAM)(LPSTR)SC(42,"mm"));
            break;
        }
...
//  Static_SetText sscanf
          ...
          if (SendDlgItemMessageUTF8(Dialog,IDD_TEXTINPUT_EDIT1,WM_GETTEXT,MAX_LENGTH_STRING-1,(LPARAM)DialogTextLine)>0) {
          ...
          if ((DialogMode & 0x10) == 0) {
            if (SendDlgItemMessageUTF8(Dialog,IDD_TEXTINPUT_EDIT2,WM_GETTEXT,MAX_LENGTH_STRING-1,(LPARAM)DialogTextLine)) {
              ...
              if (WorkingObject->x2<1000.0) WorkingObject->x2=(100*2540.0);
            }
          }
          EndDialog(Dialog, 1);
          return about;
...
//  DialogWindowText=DialogText;
  ...
  if ((Mode & 0x10) == 0) {
    res=DialogBox(GEOMClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT), GEOMWindow, (DLGPROC)TextInputDialogBody);
  } else {
    if ((Mode & 0x100) == 0) {
      res=DialogBox(GEOMClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT2), GEOMWindow, (DLGPROC)TextInputDialogBody);
    } else {
      res=DialogBox(GEOMClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT4), GEOMWindow, (DLGPROC)TextInputDialogBody);
    }
  }
  ok=1;
  return res;
...

{
  ...
  DialogMode=Mode;
  MultiLineText=Text;
  res=DialogBox(GEOMClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT3), GEOMWindow, (DLGPROC)TextInputDialog2Body);
  return res;
}
...

{
  ...
  DialogMode=Mode;
  res=DialogBox(GEOMClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_ADD_TEXT_OBJECTS), GEOMWindow, (DLGPROC)AddTextObjectsDialog2);
  ok=1;
  return res;
}
...
//  Static_SetText sscanf SelectionEsc
          if (SendDlgItemMessageUTF8(Dialog,IDD_LINEINPUT_EDIT1,WM_GETTEXT,MAX_LENGTH_STRING-1,(LPARAM)DialogTextLine)>0) {
  ...
  switch (DialogMode) {
    case IDD_DIALOG_BGA:
      for (cnt=0;cnt<20;cnt++)
        if (DialogControl==BGAGeomSet[cnt]) return 1;
      break;
    case IDD_DIALOG_DIL:
      for (cnt=0;cnt<20;cnt++)
        if (DialogControl==DILGeomSet[cnt]) return 1;
      break;
    case IDD_DIALOG_PGA:
      for (cnt=0;cnt<20;cnt++)
        if (DialogControl==PGAGeomSet[cnt]) return 1;
      break;
    case IDD_DIALOG_QUAD:
      for (cnt=0;cnt<20;cnt++)
        if (DialogControl==QUADGeomSet[cnt]) return 1;
      break;
    case IDD_DIALOG_SIL:
    case IDD_DIALOG_SIL2:
      for (cnt=0;cnt<20;cnt++)
        if (DialogControl==SILGeomSet[cnt]) return 1;
      break;
    case IDD_DIALOG_SIL_SMD_RECT:
      for (cnt=0;cnt<20;cnt++)
        if (DialogControl==SIL_SMD_RECTGeomSet[cnt]) return 1;
      break;
    case IDD_DIALOG_SIL_SMD_CIRCLE:
      for (cnt=0;cnt<20;cnt++)
        if (DialogControl==SIL_SMD_CIRCLEGeomSet[cnt]) return 1;
      break;
    case IDD_DIALOG_SOIC:
      for (cnt=0;cnt<20;cnt++)
        if (DialogControl==SOICGeomSet[cnt]) return 1;
      break;
  }
  return 0;
...
{
  if (!CheckDialogControl(DialogMode,DialogControl)) return 0;
...

{
  ...
  switch (DialogControl) {
    case IDD_GEOM_NRPINS:
      if (mode==0) return TempGeom->NrPins;
      else TempGeom->NrPins=value;
      break;
    case IDD_GEOM_NRPINS_X:
      if (mode==0) return TempGeom->NrPinsX;
      else TempGeom->NrPinsX=value;
      break;
    case IDD_GEOM_NRPINS_Y:
      if (mode==0) return TempGeom->NrPinsY;
      else TempGeom->NrPinsY=value;
      break;
    case IDD_GEOM_START2:
      if (mode==0) return TempGeom->Start2;
      else TempGeom->Start2=value;
      break;
    case IDD_GEOM_INC:
      if (mode==0) return TempGeom->PinInc;
      else TempGeom->PinInc=value;
      break;
...
*/
    case IDD_GEOM_PIN1_SQUARE:
      if (mode==0) return TempGeom->Pin1Square;
      else TempGeom->Pin1Square=value;
      break;
    ...
    case IDD_GEOM_TOP_LAYER:
      if (mode==0) return TempGeom->Layer;
      else TempGeom->Layer=value;
...

{
  ...
  switch (DialogControl) {
    case IDD_GEOM_DIST_X:
      if (mode==0) return TempGeom->DistX;
      else TempGeom->DistX=value;
      break;
    case IDD_GEOM_DIST_Y:
      if (mode==0) return TempGeom->DistY;
      else TempGeom->DistY=value;
      break;
    case IDD_GEOM_DRILL:
      if (mode==0) return TempGeom->Drill;
      else TempGeom->Drill=value;
      break;
    case IDD_GEOM_PAD:
      if (mode==0) return TempGeom->Pad;
      else TempGeom->Pad=value;
      break;
    case IDD_GEOM_PAD_X:
      if (mode==0) return TempGeom->PadX;
      else TempGeom->PadX=value;
      break;
    case IDD_GEOM_PAD_Y:
      if (mode==0) return TempGeom->PadY;
      else TempGeom->PadY=value;
      break;
    case IDD_GEOM_PAST:
      if (mode==0) return TempGeom->Paste;
      else TempGeom->Paste=value;
      break;
    case IDD_GEOM_PAST_X:
      if (mode==0) return TempGeom->PasteX;
      else TempGeom->PasteX=value;
      break;
    case IDD_GEOM_PAST_Y:
      if (mode==0) return TempGeom->PasteY;
      else TempGeom->PasteY=value;
      break;
    case IDD_GEOM_MASK:
      if (mode==0) return TempGeom->Mask;
      else TempGeom->Mask=value;
      break;
    case IDD_GEOM_MASK_X:
      if (mode==0) return TempGeom->MaskX;
      else TempGeom->MaskX=value;
      break;
    case IDD_GEOM_MASK_Y:
      if (mode==0) return TempGeom->MaskY;
      else TempGeom->MaskY=value;
      break;
    case IDD_GEOM_POW_PAD:
      if (mode==0) return TempGeom->PowerPad;
      else TempGeom->PowerPad=value;
      break;
    case IDD_GEOM_INNER_PADSIZE:
      if (mode==0) return TempGeom->InnerPad;
      else TempGeom->InnerPad=value;
      break;
    case IDD_GEOM_PITCH:
      if (mode==0) return TempGeom->Pitch;
      else TempGeom->Pitch=value;
      break;
    case IDD_GEOM_PITCH_X:
      if (mode==0) return TempGeom->PitchX;
      else TempGeom->PitchX=value;
      break;
    case IDD_GEOM_PITCH_Y:
      if (mode==0) return TempGeom->PitchY;
      else TempGeom->PitchY=value;
      break;
    case IDD_GEOM_CLEARANCE:
      if (mode==0) return TempGeom->Clearance;
      else TempGeom->Clearance=value;
      break;
  }
  return 0.0;
...
void ChangeDialogUnits(HWND Dialog,int32 DialogMode,int32 DialogControl,
                       int32 OldUnits,int32 NewUnits)

{
  ...
  if ((res=GetDialogText(Dialog,DialogMode,DialogControl))!=0) {
    if (res==1) strcpy(DialogTextLine,"0");
...

{
  ...
  for (cnt=0;cnt<NrGeomInts;cnt++) {
    ...
    if (CheckDialogControl(DialogMode,DialogControl)) {
      SetDialogIntValue(Dialog,DialogMode,DialogControl,
                        SetGetGeomIntValue(DialogMode,DialogControl,0,0));
    }
  }
    ...
    if (CheckDialogControl(DialogMode,DialogControl)) {
      if ((res=GetDialogText(Dialog,DialogMode,DialogControl))!=0) {
        ...
        if ((res=sscanf(DialogTextLine,"%f",&FloatValue))==1) {
          ...
          OwnUnits=-1;
          if (le>2) {
            if ((DialogTextLine[le-1]=='m')
               &&
               (DialogTextLine[le-2]=='m')) {
              OwnUnits=1;
            }
            if ((DialogTextLine[le-1]=='h')
               &&
               (DialogTextLine[le-2]=='t')) {
              OwnUnits=0;
            }
          }
          if (le>4) {
            if ((DialogTextLine[le-1]=='s')
               &&
               (DialogTextLine[le-2]=='l')
               &&
               (DialogTextLine[le-3]=='i')
               &&
               (DialogTextLine[le-4]=='m')) {
    //          ParamsFloat[cnt]*=2540.0;
              OwnUnits=0;
            }
            if ((DialogTextLine[le-1]=='h')
               &&
               (DialogTextLine[le-2]=='c')
               &&
               (DialogTextLine[le-3]=='n')
               &&
               (DialogTextLine[le-4]=='i')) {
    //          ParamsFloat[cnt]*=2540.0;
              OwnUnits=2;
            }
          }
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      switch (DialogMode) {
        case IDD_DIALOG_BGA:
          SetWindowTextUTF8(Dialog,SC(50,"Create BGA"));
          SetDialogItemText(Dialog,IDC_STATIC1                ,SC(51,"Pad"));
          SetDialogItemText(Dialog,IDC_STATIC2                ,SC(52,"Nr pins X"));
          SetDialogItemText(Dialog,IDC_STATIC3                ,SC(53,"Pitch"));
          SetDialogItemText(Dialog,IDC_STATIC4                ,SC(54,"Nr pins Y"));
          ...
          SetDialogItemText(Dialog,IDD_GEOM_PIN1_SQUARE       ,SC(59,"Pin A1 square"));
          SetDialogItemText(Dialog,IDD_GEOM_PIN1_CIRCLE       ,SC(60,"Pin A1 circle"));
          SetDialogItemText(Dialog,IDOK                       ,SC(45,"OK"));
          SetDialogItemText(Dialog,IDHELP                     ,SC(47,"Help"));
          SetDialogItemText(Dialog,IDCANCEL                   ,SC(46,"Cancel"));
          SetDialogItemText(Dialog,ID_UNITS                   ,SC(43,"mils/mm"));
          SetDialogItemText(Dialog,IDD_GEOM_USE_DEFAULT_RULES ,SC(450,"Use default rules for solder paste/mask and clearance"));
          break;
        case IDD_DIALOG_DIL:
          SetWindowTextUTF8(Dialog,SC(61,"Create DIP"));
          ...
          SetDialogItemText(Dialog,IDC_STATIC9                ,SC(65,"Drill hole"));
          SetDialogItemText(Dialog,IDC_STATIC10               ,SC(58,"Clearance"));
          SetDialogItemText(Dialog,IDC_STATIC11               ,SC(66,"Anti power pad"));
          SetDialogItemText(Dialog,IDC_STATIC12               ,SC(67,"Pad size"));
          SetDialogItemText(Dialog,IDC_STATIC13               ,SC(68,"Inner pad"));
          SetDialogItemText(Dialog,IDC_STATIC14               ,SC(67,"Pad size"));
          SetDialogItemText(Dialog,IDD_GEOM_PIN1_SQUARE       ,SC(69,"Pin 1 square"));
          SetDialogItemText(Dialog,IDD_GEOM_PIN1_CIRCLE       ,SC(70,"Pin 1 circle"));
          SetDialogItemText(Dialog,IDOK                       ,SC(45,"OK"));
          SetDialogItemText(Dialog,IDHELP                     ,SC(47,"Help"));
          SetDialogItemText(Dialog,IDCANCEL                   ,SC(46,"Cancel"));
          SetDialogItemText(Dialog,ID_UNITS                   ,SC(43,"mils/mm"));
          SetDialogItemText(Dialog,IDD_GEOM_USE_DEFAULT_RULES ,SC(451,"Use default rules for solder paste/mask, inner/anti power pad and clearance"));
          break;
        case IDD_DIALOG_PGA:
          SetWindowTextUTF8(Dialog,SC(71,"Create PGA"));
          SetDialogItemText(Dialog,IDC_STATIC1                ,SC(51,"Pad"));
          SetDialogItemText(Dialog,IDC_STATIC2                ,SC(52,"Nr pins X"));
          SetDialogItemText(Dialog,IDC_STATIC3                ,SC(53,"Pitch"));
          SetDialogItemText(Dialog,IDC_STATIC4                ,SC(54,"Nr pins Y"));
          ...
          SetDialogItemText(Dialog,IDC_STATIC7                ,SC(65,"Drill hole"));
          SetDialogItemText(Dialog,IDC_STATIC8                ,SC(58,"Clearance"));
          SetDialogItemText(Dialog,IDC_STATIC9                ,SC(66,"Anti power pad"));
          SetDialogItemText(Dialog,IDC_STATIC10               ,SC(67,"Pad size"));
          SetDialogItemText(Dialog,IDC_STATIC11               ,SC(68,"Inner pad"));
          SetDialogItemText(Dialog,IDC_STATIC12               ,SC(67,"Pad size"));
          ...
          break;
        case IDD_DIALOG_QUAD:
          SetWindowTextUTF8(Dialog,SC(72,"Create QUAD"));
          SetDialogItemText(Dialog,IDC_STATIC1                ,SC(73,"Starting pin nr"));
          SetDialogItemText(Dialog,IDC_STATIC2                ,SC(74,"Distance Y"));
          SetDialogItemText(Dialog,IDC_STATIC3                ,SC(75,"Pad Y"));
          SetDialogItemText(Dialog,IDC_STATIC4                ,SC(76,"Distance X"));
          SetDialogItemText(Dialog,IDC_STATIC5                ,SC(52,"Nr pins X"));
          SetDialogItemText(Dialog,IDC_STATIC6                ,SC(77,"Pad X"));
          SetDialogItemText(Dialog,IDC_STATIC7                ,SC(54,"Nr pins Y"));
          SetDialogItemText(Dialog,IDC_STATIC8                ,SC(53,"Pitch"));
          SetDialogItemText(Dialog,IDC_STATIC9                ,SC(51,"Pad"));
          SetDialogItemText(Dialog,IDC_STATIC10               ,SC(78,"Solder paste X"));
          SetDialogItemText(Dialog,IDC_STATIC11               ,SC(79,"Solder paste Y"));
          SetDialogItemText(Dialog,IDC_STATIC12               ,SC(80,"Solder mask Y"));
          SetDialogItemText(Dialog,IDC_STATIC13               ,SC(81,"Solder mask X"));
        ...
        case IDD_DIALOG_SIL:
        case IDD_DIALOG_SIL2:
          SetWindowTextUTF8(Dialog,SC(82,"Create SIL"));
          SetDialogItemText(Dialog,IDC_STATIC1                ,SC(83,"Start pin"));
          SetDialogItemText(Dialog,IDC_STATIC2                ,SC(84,"Pin type"));
          SetDialogItemText(Dialog,IDC_STATIC3                ,SC(85,"Pin counting"));
          SetDialogItemText(Dialog,IDC_STATIC4                ,SC(73,"Starting pin nr"));
          SetDialogItemText(Dialog,IDC_STATIC5                ,SC(86,"Increment"));
          ...
          SetDialogItemText(Dialog,IDC_STATIC11               ,SC(65,"Drill hole"));
          SetDialogItemText(Dialog,IDC_STATIC12               ,SC(58,"Clearance"));
          SetDialogItemText(Dialog,IDC_STATIC13               ,SC(66,"Anti power pad"));
          SetDialogItemText(Dialog,IDC_STATIC14               ,SC(67,"Pad size"));
          SetDialogItemText(Dialog,IDC_STATIC15               ,SC(68,"Inner pad"));
          SetDialogItemText(Dialog,IDC_STATIC16               ,SC(67,"Pad size"));
          SetDialogItemText(Dialog,IDOK                       ,SC(45,"OK"));
          SetDialogItemText(Dialog,IDHELP                     ,SC(47,"Help"));
          SetDialogItemText(Dialog,IDCANCEL                   ,SC(46,"Cancel"));
          SetDialogItemText(Dialog,ID_UNITS                   ,SC(43,"mils/mm"));
          ...
          break;
        case IDD_DIALOG_SIL_SMD_RECT:
          SetWindowTextUTF8(Dialog,SC(87,"Create SIL SMD Rectangle"));
          SetDialogItemText(Dialog,IDC_STATIC1                ,SC(83,"Start pin"));
          SetDialogItemText(Dialog,IDC_STATIC2                ,SC(88,"Nr pads"));
          SetDialogItemText(Dialog,IDC_STATIC3                ,SC(53,"Pitch"));
          SetDialogItemText(Dialog,IDC_STATIC4                ,SC(51,"Pad"));
          SetDialogItemText(Dialog,IDC_STATIC5                ,SC(51,"Pad"));
          SetDialogItemText(Dialog,IDC_STATIC6                ,SC(83,"Start pin"));
          SetDialogItemText(Dialog,IDC_STATIC7                ,SC(86,"Increment"));
          SetDialogItemText(Dialog,IDC_STATIC8                ,SC(51,"Pad"));
          ...
          SetDialogItemText(Dialog,IDC_STATIC14               ,SC(89,"Layer"));
          SetDialogItemText(Dialog,IDOK                       ,SC(45,"OK"));
          SetDialogItemText(Dialog,IDHELP                     ,SC(47,"Help"));
          SetDialogItemText(Dialog,IDCANCEL                   ,SC(46,"Cancel"));
          SetDialogItemText(Dialog,ID_UNITS                   ,SC(43,"mils/mm"));
          ...
          break;
        case IDD_DIALOG_SIL_SMD_CIRCLE:
          SetWindowTextUTF8(Dialog,SC(90,"Create SIL SMD Circle"));
          SetDialogItemText(Dialog,IDC_STATIC1                ,SC(83,"Start pin"));
          SetDialogItemText(Dialog,IDC_STATIC2                ,SC(51,"Pad"));
          SetDialogItemText(Dialog,IDC_STATIC3                ,SC(53,"Pitch"));
          SetDialogItemText(Dialog,IDC_STATIC4                ,SC(64,"Nr pins"));
          SetDialogItemText(Dialog,IDC_STATIC5                ,SC(83,"Start pin"));
          SetDialogItemText(Dialog,IDC_STATIC6                ,SC(86,"Increment"));
          SetDialogItemText(Dialog,IDC_STATIC7                ,SC(51,"Pad"));
          SetDialogItemText(Dialog,IDC_STATIC8                ,SC(55,"Solder mask"));
          SetDialogItemText(Dialog,IDC_STATIC9                ,SC(56,"Solder paste"));
          SetDialogItemText(Dialog,IDC_STATIC10               ,SC(58,"Clearance"));
          SetDialogItemText(Dialog,IDC_STATIC11               ,SC(89,"Layer"));
          SetDialogItemText(Dialog,IDOK                       ,SC(45,"OK"));
          SetDialogItemText(Dialog,IDHELP                     ,SC(47,"Help"));
          SetDialogItemText(Dialog,IDCANCEL                   ,SC(46,"Cancel"));
          SetDialogItemText(Dialog,ID_UNITS                   ,SC(43,"mils/mm"));
          ...
          break;
        case IDD_DIALOG_SOIC:
          SetWindowTextUTF8(Dialog,SC(91,"Create SOIC"));
          SetDialogItemText(Dialog,IDC_STATIC1                ,SC(62,"Pin 1"));
          SetDialogItemText(Dialog,IDC_STATIC2                ,SC(63,"Distance"));
          SetDialogItemText(Dialog,IDC_STATIC3                ,SC(75,"Pad Y"));
          SetDialogItemText(Dialog,IDC_STATIC4                ,SC(77,"Pad X"));
          SetDialogItemText(Dialog,IDC_STATIC5                ,SC(53,"Pitch"));
          SetDialogItemText(Dialog,IDC_STATIC6                ,SC(64,"Nr pins"));
          SetDialogItemText(Dialog,IDC_STATIC7                ,SC(51,"Pad"));
...

{
  ...
  switch (Mode) {
    ...
    case IDD_DIALOG_SIL2:
      ...
      break;
    case IDD_DIALOG_SIL_SMD_RECT:
      res=DialogBox(GEOMClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_SIL_SMD_RECT), GEOMWindow, (DLGPROC)CreateGeomDialog2);
      break;
    case IDD_DIALOG_SIL_SMD_CIRCLE:
      res=DialogBox(GEOMClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_SIL_SMD_CIRCLE), GEOMWindow, (DLGPROC)CreateGeomDialog2);
      break;
    case IDD_DIALOG_SOIC:
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SelectionEsc=0;
//      SetWindowTextUTF8(Dialog,"");
      for (Layer=NrPadLayers-1;Layer>=0;Layer--) {
        GetLayerText(Layer,TextStr,0);
        res=SendDlgItemMessage(Dialog,IDC_LIST1,LB_ADDSTRING,0,(LPARAM)TextStr);
        if (PadsVisible[Layer]!=0) {
          res=SendDlgItemMessage(Dialog,IDC_LIST1,LB_SETSEL,1,res);
        }
        res=SendDlgItemMessage(Dialog,IDC_LIST2,LB_ADDSTRING,0,(LPARAM)TextStr);
      ...
      SetWindowTextUTF8(Dialog,SC(92,"Viewable objects"));
      SetDialogItemText(Dialog,IDD_VIEW_DRILLS                  ,SC(93,"Drill plated"));
      SetDialogItemText(Dialog,IDD_VIEW_DRILL_UNPLATED          ,SC(19,"Drill unplated"));
      ...
      SetDialogItemText(Dialog,IDD_VIEW_PASTE_BOTTOM            ,SC(94,"Paste mask"));
      SetDialogItemText(Dialog,IDD_VIEW_PASTE_TOP               ,SC(94,"Paste mask"));
      SetDialogItemText(Dialog,IDD_VIEW_PLACEM_OUTLINE          ,SC(17,"Placement outline"));
      ...
      SetDialogItemText(Dialog,IDD_VIEW_POWERPADS               ,SC(96,"Anti power pads"));
      SetDialogItemText(Dialog,IDD_VIEW_PINNAMES                ,SC(448,"Pin names"));
      if (NrPadLayers==2) {
        SetDialogItemText(Dialog,IDD_VIEW_INNERPADS             ,SC(139,"Inner pads"));
      } else {
        SetDialogItemText(Dialog,IDD_VIEW_INNERPADS             ,"");
      }
      ...
      SetDialogItemText(Dialog,IDD_VIEW_INFO1                   ,SC(98,"Info layer 1"));
      SetDialogItemText(Dialog,IDD_VIEW_INFO2                   ,SC(99,"Info layer 2"));
      SetDialogItemText(Dialog,IDD_VIEW_INFO3                   ,SC(100,"Info layer 3"));
      SetDialogItemText(Dialog,IDD_VIEW_INFO4                   ,SC(101,"Info layer 4"));
      SetDialogItemText(Dialog,ID_VIEW_INSERTPOINT              ,SC(102,"Insertion point"));
      SetDialogItemText(Dialog,IDD_VIEW_GRID                    ,SC(103,"Grid"));
      SetDialogItemText(Dialog,IDD_VIEW_CROSSHAIR               ,SC(452,"Crosshair"));
      SetDialogItemText(Dialog,IDC_STATIC1                      ,SC(104,"Top"));
      SetDialogItemText(Dialog,IDC_STATIC2                      ,SC(105,"Bottom"));
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDOK:
          NrLayersSelected=(int32)SendDlgItemMessage(Dialog,IDC_LIST1,LB_GETSELITEMS,
                                                     NrPadLayers,
          ...
          NrLayersSelected=(int32)SendDlgItemMessage(Dialog,IDC_LIST2,LB_GETSELITEMS,
                                                     NrPadLayers,
                                                     (LPARAM)((LPINT)&LayersSelected));
          for (cnt=0;cnt<NrPadLayers;cnt++) {
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetWindowTextUTF8(Dialog,SC(112,"Assign objects to pin(s)"));
      SetDialogItemText(Dialog,IDOK                       ,SC(45,"OK"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(46,"Cancel"));
      SetDialogItemText(Dialog,IDHELP                     ,SC(47,"Help"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(113,"Used pins"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(114,"Pin"));
//      SendDlgItemMessage(Dialog,IDD_VIEW_CLEARANCE     ,BM_SETCHECK,ClearanceVisible        ,0);
// CB_ADDSTRING
      return about;
    case WM_MOVE:
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDOK:
          ...
          if (res==-1) {
            res=SendDlgItemMessageUTF8(Dialog,IDD_PINASSIGN_NEWPIN,WM_GETTEXT,MAX_LENGTH_STRING-1,(LPARAM)DialogTextLine);
            if (res==0) memset(DialogTextLine,0,MAX_LENGTH_STRING);
          } else {
...
//  WorkingObject=ObjectText;
  if ((DialogMode==0)
    ...
    res=DialogBox(GEOMClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_PINASSIGN), GEOMWindow, (DLGPROC)AssignPinDialog2);
    if (res==1) {
...

{
  ...
  if (Units==0) {
    ...
    SendDlgItemMessage(Dialog,IDC_EDIT2,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
    SendDlgItemMessage(Dialog,IDC_EDIT4,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
    SendDlgItemMessage(Dialog,IDC_EDIT6,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
    SendDlgItemMessage(Dialog,IDC_EDIT8,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
    SendDlgItemMessage(Dialog,IDC_EDIT10,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
    SendDlgItemMessage(Dialog,IDC_EDIT12,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
  }
...
// *******************************************************************************
          ...
          if (res==-1) {
            res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT1,WM_GETTEXT,MAX_LENGTH_STRING-1,
...
// *******************************************************************************
          ...
          if (res==-1) {
            res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT3,WM_GETTEXT,MAX_LENGTH_STRING-1,
                                   (LPARAM)DialogTextLine);
            if (res==0) memset(DialogTextLine,0,MAX_LENGTH_STRING);
            if (sscanf(DialogTextLine,"%f",&Value)==1) {
              if (Units==0) {
...
// *******************************************************************************
          ...
          if (res==-1) {
            res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT5,WM_GETTEXT,MAX_LENGTH_STRING-1,
                                   (LPARAM)DialogTextLine);
            if (res==0) memset(DialogTextLine,0,MAX_LENGTH_STRING);
            if (sscanf(DialogTextLine,"%f",&Value)==1) {
              if (Units==0) {
...
// *******************************************************************************
          ...
          if (res==-1) {
            res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT7,WM_GETTEXT,MAX_LENGTH_STRING-1,
                                   (LPARAM)DialogTextLine);
            if (res==0) memset(DialogTextLine,0,MAX_LENGTH_STRING);
            if (sscanf(DialogTextLine,"%f",&Value)==1) {
              if (Units==0) {
...
// *******************************************************************************
          ...
          if (res==-1) {
            res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT9,WM_GETTEXT,MAX_LENGTH_STRING-1,
                                   (LPARAM)DialogTextLine);
            if (res==0) memset(DialogTextLine,0,MAX_LENGTH_STRING);
            if (sscanf(DialogTextLine,"%f",&Value)==1) {
              if (Units==0) {
...
// *******************************************************************************
          ...
          if (res==-1) {
            res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT11,WM_GETTEXT,MAX_LENGTH_STRING-1,
                                   (LPARAM)DialogTextLine);
            if (res==0) memset(DialogTextLine,0,MAX_LENGTH_STRING);
            if (sscanf(DialogTextLine,"%f",&Value)==1) {
              if (Units==0) {
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SelectionEsc=0;
      SetWindowTextUTF8(Dialog,SC(393,"About geometrie editor"));
      SetDialogItemText(Dialog,IDC_STATIC1  ,SC(460,"Geometrie editor PCB Elegance"));
      SetDialogItemText(Dialog,IDOK         ,SC(45,"OK"));
      SetDialogItemText(Dialog,IDCANCEL     ,SC(46,"Cancel"));
      sprintf(str,SC(133,"Build version %i.%i.%i  [ %s ]"),PROGRAM_VERSION/100,PROGRAM_VERSION % 100,BUILD_VERSION,TIME_STRING);
#ifdef GCC_COMP
...
*/
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDC_STATIC7                ,SC(436,"SMD"));
      SetDialogItemText(Dialog,IDC_STATIC8                ,SC(437,"Through hole"));
...
//        SendDlgItemMessage(Dialog,IDD_GEOM_UN6,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(41,"mils"));
        ...
        SendDlgItemMessageUTF8(Dialog,IDD_GEOM_UN1,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
        SendDlgItemMessageUTF8(Dialog,IDD_GEOM_UN2,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
        SendDlgItemMessageUTF8(Dialog,IDD_GEOM_UN3,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
        SendDlgItemMessageUTF8(Dialog,IDD_GEOM_UN4,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
        SendDlgItemMessageUTF8(Dialog,IDD_GEOM_UN5,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
        SendDlgItemMessageUTF8(Dialog,IDD_GEOM_UN6,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
//        SendDlgItemMessage(Dialog,IDD_GEOM_UN6,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(42,"mm"));
      }
      return about;
    case WM_MOVE:
      break;
...

{
  ...
  res=DialogBox(GEOMClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_PAD_RULES), GEOMWindow, (DLGPROC)DeafultPadRulesDialogBody);
