...

{
  ...
  if (!OkToDrawConnections) return;

  if ((Connection->Info & (CONNECTIONS_NOT_VISIBLE|OBJECT_HIGHLITED)) == CONNECTIONS_NOT_VISIBLE) return;
  x1=Connection->x1;
  y1=Connection->y1;
  x2=Connection->x2;
  y2=Connection->y2;
...
//        SetROP2(OutputDisplay,R2_COPYPEN);
        if (!Printing) SetROP2(OutputDisplay,R2_XORPEN);
      } else {
...
*/
  ...
  if ((Mode & 32) == 32) {
    if ((Design.BoardOutlineKeepOut!=0.0)
       &&
       (OkToDrawObjectsLayer(BOARD_OUTLINE_LAYER)!=-1)) {
    ...
    for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
      ...
      if (((ObjectLine->Info & OBJECT_NOT_VISIBLE) == 0)
         &&
         (OkToDrawObjectsLayer(ObjectLine->Layer)==0)) {
        DrawObjectLine(ObjectLine,0.0,0.0,0);
      }
    }
    return;
  }
  if ((Mode & 8) == 8) { // With clearances
    InitDrawingObject(0,CLEARANCE_LAYER,0,DRAW_WITH_PEN_AND_NOT_FILLED);
    if ((Mode & 16) == 0) {
      for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
        ObjectLine=&((*ObjectLines)[cnt]);
  ...
  if ((Mode & 16) == 0) {
    for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
      ...
      if (((ObjectLine->Info & OBJECT_NOT_VISIBLE) == 0)
         ...
         (OkToDrawObjectsLayer(ObjectLine->Layer)==0)
         &&
         (CheckIfCopperLayer(ObjectLine->Layer))
         &&
         (ObjectLine->Layer==CurrentDrawingLayer)) {
#ifdef _DEBUG
        if (cnt==106) {
...

{
  ...
  if (OkToDrawObjectsLayer(ObjectLine->Layer)==-1) return;
  ...
  if ((Mode & 8)==8) {
    ...
    if ((ObjectLine->Layer>=0)
       &&
       (ObjectLine->Layer<Design.NrBoardLayers)) {
  ...
  if ((Xmax>=ViewMinX)
    ...
    if (((Mode & 1)==0)
       &&
       ((Info & OBJECT_SELECTED) == OBJECT_SELECTED)) {
      if (!Printing) SetROP2(OutputDisplay,SelectColorMode);
    }
/*
    if ((x1==x2)
    ...
    if ((Mode & 16) == 0) {
      for (cnt=0;cnt<Design.NrObjectRects;cnt++) {
        ...
        if (((ObjectRect->Info & OBJECT_NOT_VISIBLE) == 0)
           ...
           (OkToDrawObjectsLayer(ObjectRect->Layer)==0)
           &&
           (ObjectRect->Layer==CurrentDrawingLayer)
           &&
           (CheckIfCopperLayer(ObjectRect->Layer))) {
...

{
  ...
  if (OkToDrawObjectsLayer(ObjectRect->Layer)==-1) return;

  Clearance=0.0;
  if ((ObjectRect->Layer>=0)
     &&
     (ObjectRect->Layer<Design.NrBoardLayers)) {
    ...
    if ((Mode & 8) == 8) { // With clearances
      ...
      for (cnt=0;cnt<Design.NrObjectArcs;cnt++) {
        ...
        if (((ObjectArc->Info & OBJECT_NOT_VISIBLE) == 0)
           &&
           (OkToDrawObjectsLayer(ObjectArc->Layer)==0)
           &&
  ...
  if ((Mode & 16) == 0) {
    for (cnt=0;cnt<Design.NrObjectArcs;cnt++) {
      ...
      if (((ObjectArc->Info & OBJECT_NOT_VISIBLE) == 0)
         ...
         (CheckIfCopperLayer(ObjectArc->Layer))
         &&
         (ObjectArc->Layer==CurrentDrawingLayer)) {
        DrawObjectArc(ObjectArc,0.0,0.0,0);
      }
    }
    return;
...

{
  ...
  if (OkToDrawObjectsLayer(Layer)==-1) return;
  ...
  if ((Mode & 8)==8) {
    if ((Layer==DRILL_LAYER)
       ...
       ((Layer>=0)
       &&
       (ObjectArc->Layer<Design.NrBoardLayers))) {
...

{
  ...
  int32  Info,TextAlignment,Mirror,FontNr,cnt,MaxCountX,
         Thickness,Layer,NrLines,Length,ok;

  if (OkToDrawObjectsLayer(ObjectText2->Layer)==-1) return;

  if ((Length=strlen(ObjectText2->Text))==0) return;
  x1=ObjectText2->X;
  y1=ObjectText2->Y;
  x2=x1+OX;
  y2=y1+OY;
  ...
  if ((NrLines=ConvertObjectTextToStrings(ObjectText2->Text,FontNr,&MaxCountX,ObjectText2->Layer))==0) return;
  x2=x1+OX;
  y2=y1+OY;
  for (cnt=0;cnt<NrLines;cnt++) {
    if (FontNr==0) {
      GetMinMaxText2(x2,y2,x3,FontNr,Rotation+NewRotation,TextAlignment,
                     Mirror,TextStrings2[cnt]);
    } else {
      GetMinMaxText2b(x2,y2,x3,FontNr,Rotation+NewRotation,TextAlignment,
                      Mirror,TextStrings[cnt]);
    }
...
//    DrawStr2(x2,y2,x3,4*2540.0,TextRotation,TextAlignment,ObjectText2->Text);
    ...
    for (cnt=0;cnt<NrLines;cnt++) {
      if (FontNr==0) {
        ...
        DrawTrueTypeStrWithRotation(x2,y2,x3,FontNr,Rotation+NewRotation,0,Mirror,
                                    TextStrings[cnt],Layer,Mode);
      }
      if (FontNr==0) {
        if (Mirror==0) {
          x2+=sin(ANGLE_CONVERT(Rotation+NewRotation))*x3*1.1;
...

{
  ...
  if (OkToDrawObjectsLayer(ObjectPolygon->Layer)==-1) return;
  count=ObjectPolygon->NrVertices;
  AllocateSpecialMem(MEM_POINTS,count*sizeof(POINT)+16384,(void **)&Points);

  if ((ObjectPolygon->maxx+OX>=ViewMinX)
     &&
     (ObjectPolygon->minx+OX<=ViewMaxX)
     &&
     (ObjectPolygon->maxy+OY>=ViewMinY)
     &&
     (ObjectPolygon->miny+OY<=ViewMaxY)) {
...

{
  ...
  if ((ObjectPolygon->maxx>=ViewMinX)
     &&
     (ObjectPolygon->minx<=ViewMaxX)
     &&
     (ObjectPolygon->maxy>=ViewMinY)
     &&
...
{
  int32  cnt,cnt2,cnt4,NrPolyLines,lengte,count,oldx,oldy,hulp,code,
         res,xx1,yy1,xx2,yy2,xx3,yy3,xx4,yy4,hx,hy;
  ...
  if ((hulp<8)
    ...
    if ((InRange2(Rotation,0.0))
       ||
       (InRange2(Rotation,90.0))
       ||
       (InRange2(Rotation,180.0))
       ||
       (InRange2(Rotation,270.0))) {
      xx1=Mult((TextMaxX+TextMinX)*0.5-Xoffset)+DrawWindowMinX;
      if (ReverseY) yy1=DrawWindowMaxY-Mult((TextMaxY+TextMinY)*0.5-Yoffset)-1;
      else yy1=Mult(TextMinY-Yoffset);
      ...
      xx1=MultX(tx1);
      if (ReverseY) yy1=MultY(ty1);
      else yy1=Mult(ty1-Yoffset);

      xx2=MultX(tx2);
      if (ReverseY) yy2=MultY(ty2);
      else yy2=Mult(ty2-Yoffset);

      xx3=MultX(tx3);
      if (ReverseY) yy3=MultY(ty3);
      else yy3=Mult(ty3-Yoffset);

      xx4=MultX(tx4);
      if (ReverseY) yy4=MultY(ty4);
      else yy4=Mult(ty4-Yoffset);
...
//            RotateFlipPoint(&x1,&y1,0.0,0.0,Rotation);
          ...
          if (ReverseY) hy=(DrawWindowMaxY-Mult(y1+y-Yoffset)-1);
          else hy=(Mult(y1+y-Yoffset));
...
{
  int32  cnt,cnt2,cnt4,lengte,count,oldx,oldy,hulp,code,
         DrawChar,xx1,yy1,xx2,yy2,xx3,yy3,xx4,yy4;
    ...
    if ((Mode & 16) == 16) {
      for (cnt=0;cnt<Design.NrAreaFills;cnt++) {
        ...
        if (((AreaFill->Info & (OBJECT_NOT_VISIBLE)) == 0)
           &&
           (AreaFill->Layer!=CurrentDrawingLayer)) {
...
// DrawPolygon->PolygonType  =  4   ->  polygon is cut out
  ...
  if ((AreaFill->Info & (OBJECT_NOT_VISIBLE)) != 0) return;
  ...
  if (((AreaFill->Info & (OBJECT_SELECTED|POWERPLANE)) == 0)
     &&
     ((Mode & 2)==0)
     &&
     (AreafillDrawMode==0)) {
  ...
  //  InitDrawingObject(TRACE_HOR,DrawCode,0,NORMAL_FILLED_AND_NO_PEN);
    ...
    for (cnt2=1;cnt2<AreaFill->NrPolygons;cnt2++) {
      ...
      if (((DrawPolygon->PolygonType & 8) == 0)
         &&
         (IsPolygonVisible(DrawPolygon,0))) {
        DrawOwnPolygon((double *)&((*DrawPolygon).Points),count,0);
      }
      PolygonPos+=MemSizePolygon(DrawPolygon);
      DrawPolygon=(PolygonRecord *)PolygonPos;
    ...
    if (IsPolygonVisible(DrawPolygon,0)) {
      ...
      if (cnt2==0) {
        ...
        if (((DrawPolygon->PolygonType & 5) == 0)
           ||
           ((DrawPolygon->PolygonType & 5) == 5)) {
          MakeBiggerSmallerPolygon(DrawPolygon,BiggerPolygon,Clearance*2.0,1);
          SetMinMaxPolygon(BiggerPolygon,0);
          VerticeCount=BiggerPolygon->NrVertices;
        }
      }
      if ((BiggerPolygon->NrVertices>0)
         &&
         (IsPolygonVisible(BiggerPolygon,0))) {
