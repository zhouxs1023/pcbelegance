...
//  Drill=Junction->DrillThickNess;
  ...
  if ((Xmax>=ViewMinX)
    ...
    if (ReverseY) y1=DrawWindowMaxY-Mult(yy1-Yoffset)-1;
    else y1=Mult(yy1-Yoffset);
    ThickNess=Mult(0.5);
    InitDrawingJunctions();
    if (((Info & OBJECT_SELECTED) == OBJECT_SELECTED)
       &&
       (Mode==0)) {
      if (!Printing) SetROP2(OutputDisplay,SelectColorMode);
    }
    if (!ReverseY) ellips2(x1,y1,ThickNess,ThickNess,255);
    else ellips2(x1,y1,ThickNess,ThickNess,255);
    if (((Info & OBJECT_SELECTED) == OBJECT_SELECTED)
       &&
       (Mode==0)) if (!Printing) SetROP2(OutputDisplay,R2_COPYPEN);
  }
}
...
//  InitDrawingJunctions();
  if (!Printing) if (!Printing) SetROP2(OutputDisplay,R2_COPYPEN);
...
//  Drill=OnePinNet->DrillThickNess;
  ...
  if ((Xmax>=ViewMinX)
    ...
    if (ReverseY) y1=MultY(yy1-ONE_PIN_NET_SIZE);
    else y1=Mult(yy1-Yoffset-ONE_PIN_NET_SIZE);
    x2=MultX(xx1+ONE_PIN_NET_SIZE);
    if (ReverseY) y2=MultY(yy1+ONE_PIN_NET_SIZE);
    else y2=Mult(yy1-Yoffset+ONE_PIN_NET_SIZE);
...
//  }
  ...
  if ((Xmax>=ViewMinX)
    ...
    if (((Info & OBJECT_SELECTED) == OBJECT_SELECTED)
       &&
       (Mode==0)) if (!Printing) SetROP2(OutputDisplay,SelectColorMode);
    xx=MultX(x);
    if (ReverseY) yy=MultY(y);
    else yy=Mult(y-Yoffset);
    switch (BusConnectionType) {
      case 0:                            /*  \    */
                                         /*  /    */
...

{
  ...
  if ((Mode!=2)
     ||
     ((Info & (OBJECT_SELECTED|1)) == (OBJECT_SELECTED|1))) {
    ...
    if ((Xmax>=ViewMinX)
      ...
      if (((Info & (OBJECT_SELECTED|1)) == (OBJECT_SELECTED|1))
         &&
         (Mode==0)) if (!Printing) SetROP2(OutputDisplay,SelectColorMode);
      if (ReverseY) {
        switch (ConnectionType >> 1) {
  ...
  if ((Mode!=2)
     ||
     ((Info & (OBJECT_SELECTED|2)) == (OBJECT_SELECTED|2))) {
...

{
  ...
  if ((Info & 1) == 1) { // ConnectX and TextX
    xx1+=OX;
    yy1+=OY;
  ...
  if ((TextMaxX>=ViewMinX)
    ...
    if (ReverseY) y2a=DrawWindowMaxY-Mult(yy2-Yoffset)-1;
  ...
  for (cnt=0;cnt<min(MaxNrNetLabels,Design.NrNetLabels);cnt++) {
    ...
    if ((NetLabel->Info & OBJECT_NOT_VISIBLE) == 0) {
      if ((NetLabel->Info & OBJECT_SELECTED) != 0) {
        if (!Printing) if (!Printing) SetROP2(OutputDisplay,SelectColorMode);
      }
...
//  yy2=y2/2;
  ...
  if ((Xmax>=ViewMinX)
    ...
    if (ReverseY) {
      SpecialArc(x1a,y1a,Mult(x2),Mult(x2),
                 Mult(x1+x3-Xoffset)+DrawWindowMinX,DrawWindowMaxY-Mult(y1+y3-Yoffset)-1,
                 Mult(x1+x4-Xoffset)+DrawWindowMinX,DrawWindowMaxY-Mult(y1+y4-Yoffset)-1);
      if (Mode==3) {
        DrawLine(x1a-3,y1a-3,
                 x1a+3,y1a+3);
        DrawLine(x1a-3,y1a+3,
    ...
    } else SpecialArc(x1a,y1a,Mult(x2),Mult(x2),
                      Mult(x1+x3-Xoffset)+DrawWindowMinX,Mult(y1+y3-Yoffset)-1,
                      Mult(x1+x4-Xoffset)+DrawWindowMinX,Mult(y1+y4-Yoffset)-1);
    if ((Mode==0)
       &&
       ((Info & OBJECT_SELECTED) == OBJECT_SELECTED)) {
      if (!Printing) SetROP2(OutputDisplay,R2_COPYPEN);
...

{
  ...
  if ((Length=strlen(ObjectText->Text))==0) return;
#ifdef _DEBUG
  if (Length>120) {
    ok=1;
  }
...

{
  ...
  if ((Mode!=2)
    ...
    if ((Xmax>=ViewMinX)
      ...
      hulp=Mult(0.25);
      hulpx=Mult(xx1-Xoffset)+DrawWindowMinX;
      if (ReverseY) hulpy=DrawWindowMaxY-Mult(yy1-Yoffset)-1;
      else hulpy=Mult(yy1-Yoffset);
...

{
  ...
  int32  Info,FontNr,NrLines,lengte,TextAlignment,TextRotation,
         cnt,hulp,hulpx,hulpy,AddMode,LinePos[16];
...
{
  char   code,*str2;
  int32  ok,cnt,cnt2,cnt4,NrPolyLines,lengte,count,oldx,oldy,
         Found,hulp,xx1,yy1,xx2,yy2,hx,hy;
  ...
  if ((hulp<4)
    ...
    if (!Printing) yy1=DrawWindowMaxY-Mult((TextMaxY+TextMinY)*0.5-Yoffset)-1;
...
//          Mult(TextMaxX-TextMinX),Mult(TextMinY-TextMaxY));
    ...
    case 0:
      ...
      switch (Alignment) {
        ...
        case ALIGN_LEFT_CENTRE:
          if (ReverseY) x+=Size*0.1*DefFontSize;
  ...
  if (Printing) {
    if ((InRangeSpecial(Size,1.0,0.2))
       ||
       (InRangeSpecial(Size,2.0,0.2))
       ||
       (InRangeSpecial(Size,3.0,0.2))) {
#ifdef _DEBUG
      ...
      xx1=Mult(x-Xoffset);
  //    yy1=Mult(y-Yoffset);
      if (ReverseY) yy1=DrawWindowMaxY-Mult(y-Yoffset)-1;
      else yy1=Mult(y-Yoffset);
      switch (Alignment) {
        case ALIGN_LEFT_BOTTOM:
        case ALIGN_LEFT_CENTRE:
        case ALIGN_LEFT_TOP:
      ...
      TextOut(OutputDisplay,xx1,yy1,str,strlen(str));
      return;
    }
    ok=1;
  }
...
//        NrLines=0;
        ...
        for (cnt=0;cnt<NrPolyLines;cnt++) {
          ...
          do {
            ...
            switch (Rotation) {
              ...
              case 1:
                ...
                if (ReverseY) hy=(DrawWindowMaxY-Mult(x1+y-Yoffset)-1);
                else hy=(Mult(x1+y-Yoffset));
                LinePoints[count].x=(int32)hx;
                LinePoints[count].y=(int32)hy;
                break;
            }
          ...
          if (ReverseY) {
            ...
            SetPixel(OutputDisplay,(int32)LinePoints[count-1].x,(int32)LinePoints[count-1].y,LineColor);
          } else {
...
{
  char   code,*str2;
  int32  cnt,cnt2,cnt4,NrPolyLines,lengte,count,oldx,oldy,hulp,
  ...
  if ((hulp<4)
    ...
    if ((InRangeSpecial(Rotation,0.0,0.001))
       ||
       (InRangeSpecial(Rotation,90.0,0.001))
       ||
       (InRangeSpecial(Rotation,180.0,0.001))
       ||
       (InRangeSpecial(Rotation,270.0,0.001))) {
      xx1=Mult((TextMaxX+TextMinX)*0.5-Xoffset)+DrawWindowMinX;
