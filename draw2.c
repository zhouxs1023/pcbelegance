...

{
  ...
  for (cnt=0;cnt<NrObjects;cnt++) {
    ...
    if ((Object->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      SpecialObject=0;
      if ((ObjectLayer!=DRILL_LAYER)
         &&
         (ObjectLayer!=DRILL_UNPLATED_LAYER)
         &&
         (ObjectLayer!=COMP_OUTLINE_LAYER)) {
        if (((Object->Info & OBJECT_SELECTED) == OBJECT_SELECTED)
           &&
           ((Mode & 4)==0)) {
          switch (ObjectLayer) {
            ...
            case INNER_PAD_LAYER:
              ...
              break;
            default:
              if ((ObjectLayer>=ROUTING_KEEPOUT_LAYER)
                 &&
                 (ObjectLayer<ROUTING_KEEPOUT_LAYER+32)) {
                SpecialObject=1;
              }
              break;
          }
...
//            InitDrawingObject(OBJECT_CIRCLE,Layer,0,2);
          ...
          for (cnt2=0;cnt2<ObjectPolygon->NrSubPolygons;cnt2++) {
            if ((PBuf-(uint8 *)ObjectPolygon)>=(int32)(ObjectPolygonLength-sizeof(ObjectSubPolygonInitRecord))) {
              break;
            }
            ...
            if ((ObjectSubPolygon->maxx+OX>=ViewMinX)
               &&
               (ObjectSubPolygon->minx+OX<=ViewMaxX)
               &&
               (ObjectSubPolygon->maxy+OY>=ViewMinY)
               &&
               (ObjectSubPolygon->miny+OY<=ViewMaxY)) {
              for (cnt3=0;cnt3<count;cnt3++) {
...
//     (Layer!=PAD_TOP_LAYER)) return;
  ...
  if ((ObjectPolygon->maxx+ObjectPolygon->Clearance+OX>=ViewMinX)
     ...
     &&
     (ObjectPolygon->miny-ObjectPolygon->Clearance+OY<=ViewMaxY)
     &&
     (ObjectPolygon->Clearance!=0.0)) {
    if (MakeBiggerSmallerObjectPolygon(ObjectPolygon,NewObjectPolygon2,
                                       ObjectPolygon->Clearance*2.0,0)==-1) {
      return;
    }
    Layer=ObjectPolygon->Layer;
    if (CurrentObjectCode!=ClearanceObjectCode) InitDrawingClearance();
    Info=ObjectPolygon->Info;
    if (((Mode & 4)==0)
       &&
       ((Info & OBJECT_SELECTED) == OBJECT_SELECTED)) {
    ...
    if (ObjectPolygon->NrSubPolygons>0) {
      ...
      if (MakeBiggerSmallerObjectPolygon(NewObjectPolygon2,NewObjectPolygon3,
                                         ObjectPolygon->Clearance*2.0,1)==-1) {
        return;
      }
      for (cnt3=0;cnt3<count;cnt3++) {
...

{
  ...
  int32 dikte,Layer,DrawMode,ObjectSelected,SpecialMode,
        LineSegments,SegmentCount,cnt2,x1a,y1a,x2a,y2a,ObjectType,CircleMode;
...
*/
        ...
        if ((Mode & 1)==1) rect3(x1a,y1a,Mult(xx2)-2,Mult(yy2)-2);
      }
      break;
    case OBJECT_CIRCLE:
#ifdef _DEBUG
...
*/
        ...
        if ((Layer==DRILL_LAYER)
          ...
          if ((Mode & 1)==1) ellips2(x1a,y1a,Mult(xx2)-2,Mult(xx2)-2,CircleConv[CircleMode]);
        }
      }
      break;
    case OBJECT_ARC:
      ...
      if ((Xmax>=ViewMinX)
        ...
        if ((Object->Thickness==0.0)
          ...
          if (ReverseY) {
            SpecialArc(x1a,y1a,
                       Mult(xx2),Mult(yy2),
                       Mult(xx1+xx3-Xoffset)+DrawWindowMinX,DrawWindowMaxY-Mult(yy1+yy3-Yoffset)-1,
                       Mult(xx1+xx4-Xoffset)+DrawWindowMinX,DrawWindowMaxY-Mult(yy1+yy4-Yoffset)-1);
            if ((Mode & 1)==1)
              SpecialArc(x1a,y1a,
                         Mult(xx2)+dikte-2,Mult(yy2)+dikte-2,
                         Mult(xx1+xx3-Xoffset)+DrawWindowMinX,DrawWindowMaxY-Mult(yy1+yy3-Yoffset)-1,
                         Mult(xx1+xx4-Xoffset)+DrawWindowMinX,DrawWindowMaxY-Mult(yy1+yy4-Yoffset)-1);
          } else {
            SpecialArc(x1a,y1a,
                       Mult(xx2)+dikte,Mult(yy2)+dikte,
...

{
  ...
  int32 Layer,DrawMode,ObjectSelected,x1a,y1a,
    ...
    default:
      if (Layer<32) {
        ...
        if ((Layer>=ROUTING_KEEPOUT_LAYER)
           &&
           (Layer<ROUTING_KEEPOUT_LAYER+32)) {
          if (!RoutingKeepoutVisible[Layer-ROUTING_KEEPOUT_LAYER]) return;
        } else {
          return;
        }
      }
...
//            SaveFont=SelectObject(DrawItem->hDC,GetStockObject(SYSTEM_FONT));
            ...
            Rectangle(DrawItem->hDC,DrawItem->rcItem.left,DrawItem->rcItem.top,
                                    DrawItem->rcItem.left+180,DrawItem->rcItem.bottom+1);
            SelectObject(DrawItem->hDC,GetStockObject(WHITE_BRUSH));
            SelectObject(DrawItem->hDC,WhitePen);
            Rectangle(DrawItem->hDC,DrawItem->rcItem.left+180,DrawItem->rcItem.top,
...
//            SetBkColor(DrawItem->hDC,RGB(0,0,0));
            ...
            switch (ItemNr) {
              ...
              case ShapePlacementOutLineColorNr:
                strcpy(TextStr,SC(17,"Placement outline"));
                break;
              case ShapePinsDrillColorNr:
                strcpy(TextStr,SC(18,"Drill"));
                break;
              case ShapePinsDrillUnplatedColorNr:
                strcpy(TextStr,SC(136,"Drill (unplated)"));
                break;
              case ShapePinsTopColorNr:
                strcpy(TextStr,SC(137,"Pads top"));
                break;
              case ShapePinsBottomColorNr:
                strcpy(TextStr,SC(138,"Pads bottom"));
                break;
              case ShapePinsInnerColorNr:
                strcpy(TextStr,SC(139,"Inner pads"));
                break;
              case ShapePasteMaskTopColorNr:
                strcpy(TextStr,SC(140,"Paste mask pads top"));
                break;
              case ShapePasteMaskBottomColorNr:
                strcpy(TextStr,SC(141,"Paste mask pads bottom"));
                break;
              case ShapeSoldMaskTopColorNr:
                strcpy(TextStr,SC(142,"Solder mask pads top"));
                break;
              case ShapeSoldMaskBottomColorNr:
                strcpy(TextStr,SC(143,"Solder mask pads bottom"));
                break;
              case ShapeGeomNameColorNr:
                strcpy(TextStr,SC(97,"Geometry name"));
                break;
              case ShapePowerPadColorNr:
                strcpy(TextStr,SC(96,"Anti power pads"));
                break;
              case ShapeBoardOutlineColorNr:
                strcpy(TextStr,SC(12,"Board outline"));
                break;
              case ShapeInfo1ColorNr:
                strcpy(TextStr,SC(13,"Info1"));
                break;
              case ShapeInfo2ColorNr:
                strcpy(TextStr,SC(14,"Info2"));
                break;
              case ShapeInfo3ColorNr:
                strcpy(TextStr,SC(15,"Info3"));
                break;
              case ShapeInfo4ColorNr:
                strcpy(TextStr,SC(16,"Info4"));
                break;
