...
#endif
{
  if (ObjectLayer<0) return -1;


  if (!ViewSingleLayer) {
    if (ObjectLayer==0) {  // Draw on bottom layer
      if (OkToDrawBottomPads) {
        ...
        if ((mode & 1)==0) {
          if (OkToDrawTopPads) {
            ...
            if (OkToDrawInnerPads) { // Draw on inner layer
...

{
  ...
  double dikte;
  divx=PixelToReal(1);
  if (!Repaint) StartDrawingEditingWindow();
    ...
    case PIN_SMD_POLYGON:
      ...
      if ((Mode & 1)==0) {
        ...
        if ((Mode & 2)==2) {
          ...
          le=strlen(str);
          TextOut(OutputDisplay,x-4-((le/2)*8),y-8,str,le);
        }
      } else {
...

{
  ...
  for (cnt=0;cnt<NrObjects;cnt++) {
    Object=&((*Objects)[cnt]);
    if (TempBackGroundActive) SetBackGroundActive(0);
    DrawObject(Object,Mode);
  }
}
...

{
  ...
  if ((Mode & 0x40)==0) {          // Initdrawing
    if ((Mode & 2)==0) {           // Normal drawing
      if ((Mode & 0x80)==0) {      // Normal drawing
        if (!Hilited) {
          switch (Mode & 0xC00) {
...
*/
  ...
  if ((Mode & 0xC00)==0xC00) { // Draw inner pads
...
*/
  ...
  double xx1,yy1,xx2,yy2,xx3,Xmax,Xmin,Ymax,Ymin,
         yy2a,xxx2,yyy2,Clearance,Thickness;
  int32 x1a,y1a;
...
*/
  ...
  double xx1,yy1,xx2,yy2,Xmax,Xmin,Ymax,Ymin,xx3,
         xx2a,yy2a,xxx2,yyy2,Clearance,Thickness;
...
*/
  ...
  double xx1,yy1,xx2,yy2,yy2a,yy2b,xx3,yy3,xx4,yy4,Xmax,Xmin,Ymax,Ymin,
        xxx2,yyy2,Clearance,Thickness;
...
//              DrawFilledPolygon(PolygonObject,0);
    ...
    case PIN_ARC:
      if ((Mode & 0x80)==0) {
        if ((Mode & 8)==0) {
          InitDrawingSmdPinLayer(NewDrawLayer,Mult(Thickness),Mode);
          SpecialArc(x1a,y1a,Mult(xx2)+1,Mult(yy2)+1,MultX(xx1+xx3),MultY(yy1+yy3),MultX(xx1+xx4),MultY(yy1+yy4));
//                MakePolygonFromObject(Object,PolygonObject,0.0,0.0,1,1);
//                DrawFilledPolygon(PolygonObject,0);
        } else {
          MakePolygonFromObject(Object,PolygonObject,Clearance,0.0,1,1);
...
{
  double xx1,yy1,xx2,yy2,xx3,yy3,xx4,yy4,xx2a,yy2a,yy2b,Xmax,Xmin,Ymax,Ymin,
         Angle1,Length1,Angle2,Length2,xxx2,yyy2,Clearance,Thickness,RotationAngle;
  int32  yyy2a,Layer,x1a,y1a,x2a,y2a,x3,y3,x4,y4,
         ArcMode,tx1,ty1,tx2,ty2,tdikte,res,NewDrawLayer,Mirror;
  uint8  PolygonBuf[10240];
  PolygonRecord *PolygonObject;
#ifdef _DEBUG
  int32 ok;
...
*/
  ...
  xx3=Object->x3;
  x1a=MultX(xx1);
  if (ReverseY) y1a=MultY(yy1);
  else y1a=Mult(yy1-Yoffset);
  Clearance=Object->Clearance;
  Thickness=Object->Thickness;
  xxx2=xx2;
  yy2=Object->y2;
...
// ***********************************************************************************
    case TRACE_HOR:
      if ((Object->Layer>=0)
         ...
         (Object->Layer<32)
         &&
         ((DrawCode=DrawLayerCode[Object->Layer])>=0)
         &&
    ...
    case TRACE_ALL_ANGLE:
      if ((Object->Layer>=0)
        ...
        if ((Xmax>=ViewMinX)
           ...
           (Ymin<=ViewMaxY)) {
          x2a=MultX(xx2);
          if (ReverseY) y2a=MultY(yy2);
          else y2a=Mult(yy2-Yoffset);

          if ((Mode & 0x40)==0) {         // No initdrawing
            if ((Mode & 0x20)==0x20) {    // Trace hatch
              if ((Mode & 0x100)==0) {    // Draw filled
                if ((Object->Info & OBJECT_HIGHLITED) == 0) {
                  InitDrawingObject(Object->ObjectType,DrawCode,0,IN_NET_FILLED_AND_PEN1);
                } else {
                  InitDrawingObject(Object->ObjectType,DrawCode,0,HILITED_IN_NET_FILLED_AND_PEN1);
                }
              } else {                    // Draw with a thick pen
                if ((Object->Info & OBJECT_HIGHLITED) == 0) {
                  InitDrawingObject(Object->ObjectType,DrawCode,Mult(Thickness),DRAW_WITH_PEN_AND_NOT_FILLED);
                } else {
                  InitDrawingObject(Object->ObjectType,DrawCode,Mult(Thickness),DRAW_WITH_HILITED_PEN_AND_NOT_FILLED);
                }
              }
            }
            if ((Mode & 0x20)==0) {
              if ((Mode & 0x100)==0x100) {    // Draw filled
                if ((Object->Info & OBJECT_HIGHLITED) == 0) {
                  InitDrawingObject(Object->ObjectType,DrawCode,0,NORMAL_FILLED_AND_PEN1);
                } else {
                  InitDrawingObject(Object->ObjectType,DrawCode,0,HILITED_NORMAL_FILLED_AND_PEN1);
...
// ***********************************************************************************
    case OBJECT_LINE:
      ...
      if ((Xmax>=ViewMinX)
        ...
        DrawLine(x1a,y1a,x2a,y2a);
        if ((Mode & 1)==1) {
          if (fabs(xx2-xx1)>fabs(yy2-yy1)) DrawLine(x1a,y1a-1,x2a,y2a-1);
          else DrawLine(x1a+1,y1a,x2a+1,y2a);
        }
      }
      break;
    case OBJECT_RECT:
...
//      Thickness=Object->Clearance;
      if ((PolygonObject->maxx>=ViewMinX)
         &&
         (PolygonObject->minx<=ViewMaxX)
         &&
         (PolygonObject->maxy>=ViewMinY)
         &&
...
*/
  ...
  if (ReverseY) y1a=DrawWindowMaxY-Mult(yy1-Yoffset)-1;
  else y1a=Mult(yy1-Yoffset);
  Clearance=Object->Clearance;
  xxx2=xx2;
  yy2=Object->y2;
    ...
    if ((Comp->Info & OBJECT_SELECTED) == OBJECT_SELECTED) {
      ...
      for (cnt2=0;cnt2<NrObjects;cnt2++) {
        ...
        if (((Object->Layer==SILKSCREEN_TOP)
           ...
           ||
           ((Object->Layer==SILKSCREEN_BOTTOM)
           &&
           (OkToDrawSilkScreenBottom))) {
          if ((Mode & 0x200)==0) {
            if (Mode & 0x100) {
...
*/
  ...
  if (!ViewSingleLayer) {
    TempBackGroundActive=BackGroundActive;
    if (((OkToDrawTopPads) && (Design.NrBoardLayers>1))
       ||
       (OkToDrawBottomPads)
       ||
    ...
    if (((OkToDrawTopPads) && (Design.NrBoardLayers>1) && ((DrawLayerCode[Design.NrBoardLayers-1] & 0x10) == 0))
       ||
       ((OkToDrawBottomPads) && ((DrawLayerCode[0] & 0x10) == 0))
       ||
       ((OkToDrawInnerPads) && (Design.NrBoardLayers>2) && ((DrawCode & 0x10) == 0))) {
      for (cnt=0;cnt<NrObjects;cnt++) {
        Object=&((*Objects)[cnt]);
        if (TempBackGroundActive) SetBackGroundActive(0);
        DrawDrillObject(Object,Mode);
...
//     0x400   rotated (45)
  ...
  if ((Comp->TextVisibility & 0x100) != 0) return;
//  Mirror=(Comp->CompMode & 8) >> 3;
  ...
  if (!ViewSingleLayer) {
    ...
    if (Mirror == 0 ) {
      if ((!OkToDrawSilkScreenTop)
         ||
         ((DrawLayerCode[Design.NrBoardLayers-1] & 0x10) == 0x10)) {
        return;
      }
    } else {
...
//     0x800   rotated (45)
  ...
  if ((Comp->TextVisibility & 0x200) != 0) return;
...

{
  ...
  for (cnt2=0;cnt2<NrObjects;cnt2++) {
    ...
    switch (Object->Layer) {
      case INFO_LAYER:
        if (OkToDrawInfoObjects) OkToDraw=1;
        break;
      case INFO_LAYER2:
        if (OkToDrawInfo2Objects) OkToDraw=1;
        break;
      case INFO_LAYER3:
        if (OkToDrawInfo3Objects) OkToDraw=1;
        break;
      case INFO_LAYER4:
        if (OkToDrawInfo4Objects) OkToDraw=1;
        break;
    }
    if (OkToDraw) {
      if (TempBackGroundActive) SetBackGroundActive(0);
...

{
  ...
  if (!OkToDrawBoardOutline) return;
  ...
  for (cnt2=0;cnt2<NrObjects;cnt2++) {
    ...
    if ((Layer>=ROUTING_KEEPOUT_LAYER)
      ...
      if (Layer!=CurrentDrawingLayer) {
        if (Layer==0) {
          if ((OkToDrawRoutingKeepoutBottom)
             &&
             ((DrawLayerCode[0] & 0x10) == 0)) {
            if (TempBackGroundActive) SetBackGroundActive(0);
            DrawObject(Object,Mode);
          }
        } else {
          if (Layer==Design.NrBoardLayers-1) {
            if ((OkToDrawRoutingKeepoutTop)
               &&
               ((DrawLayerCode[Design.NrBoardLayers-1] & 0x10) == 0)) {
              if (TempBackGroundActive) SetBackGroundActive(0);
              DrawObject(Object,Mode);
            }
          } else {
            if ((OkToDrawRoutingKeepoutInner)
               &&
               ((DrawLayerCode[Layer] & 0x10) == 0)) {
              if (TempBackGroundActive) SetBackGroundActive(0);
              DrawObject(Object,Mode);
              Object->Info|=OBJECT_DONE;
            }
...
*/
  ...
  if (DrawWindowMaxX/Xpixels>3) {
    ...
    for (cntx=Xmin2;cntx<Xmax2;cntx++) {
      ...
      if ((x>=DrawWindowMinX)
         &&
         (x<DrawWindowMaxX)) GridBits[x >> 3]&=255-(128 >> (x & 7));
    }
...
void DrawTryingObjectCross(double CurrentX,double CurrentY,double MinX,double MinY,double MaxX,double MaxY,int32 Mode)
