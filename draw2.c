...
{
  double xx1,yy1,xx2,yy2,xx3,yy3,xx4,yy4,xx2a,yy2a,Xmax,Xmin,Ymax,Ymin,
         Rotation,Angle,Length;
  int32 ok,NrLines,cnt2,lengte,x1a,y1a,x2a,y2a,x3a,y3a,AddMode,
        LinePos[16],cnt3,LineMode,TextAlignment,TextRotation,ObjectType;
  uint8  CircleMode;
  ...
  switch (ObjectType) {
    case SYMBOL_LINE:
      ...
      if ((Xmax>=ViewMinX)
         ...
         (Ymin<=ViewMaxY)) {
        x1a=MultX(xx1+OX);
        if (ReverseY) y1a=MultY(yy1+OY);
        else y1a=Mult(yy1-Yoffset+OY);
        x2a=MultX(xx2+OX);
        if (ReverseY) y2a=MultY(yy2+OY);
        else y2a=Mult(yy2-Yoffset+OY);
    ...
    case SYMBOL_ARC:
      ...
      if ((Xmax>=ViewMinX)
        ...
        if (ReverseY) SpecialArc(x1a,y1a,Mult(xx2),Mult(xx2),
                                  MultX(xx1+xx3+OX),MultY(yy1+yy3+OY),
                                  MultX(xx1+xx4+OX),MultY(yy1+yy4+OY));
        else SpecialArc(x1a,y1a,Mult(xx2),Mult(xx2),
                        Mult(xx1+xx3-Xoffset+OX)+DrawWindowMinX,Mult(yy1+yy3-Yoffset+OY),
                        Mult(xx1+xx4-Xoffset+OX)+DrawWindowMinX,Mult(yy1+yy4-Yoffset+OY));
      }
      break;
    case SYMBOL_TEXT:
...
#endif
      if (InRange(xx2,(double)1.0)) xx2=(double)0.9;
          ...
          if (NrLines<64) {
            if (cnt3-cnt2>0) {
              ...
              if ((Rotation==0.0)
                ...
                GetMinMaxText2(xx1,yy1,xx2,0,Rotation,TextAlignment,0,(LPSTR)&TextStrings[NrLines]);
              }
...

{
  ...
  int32  InstanceInfo,TextRotation,Alignment,ObjectMirrorX,ObjectMirrorY,
         Rotation,cnt,MemPos,SymbolNr;
  ...
  if ((!EditingSymbol)
     ...
     ||
     ((InstanceInfo & SHEET_SYMBOL) == SHEET_SYMBOL))) return;
...
//  Instance=(InstanceRecord *)&((*Instances)[InstanceNr]);
  ...
  if ((!EditingSymbol)
     &&
     ((Instance->ValueInfo & TEXT_NOT_VISIBLE) == TEXT_NOT_VISIBLE)) return;
  Rotation=(Instance->SymbolInfo) & OBJECT_ROTATE90;
  ObjectMirrorX=((Instance->SymbolInfo & OBJECT_MIRRORX) >> 4);
  ObjectMirrorY=((Instance->SymbolInfo & OBJECT_MIRRORY) >> 5);
...

{
  ...
  if ((Mode!=2)
     ||
     ((Info & (OBJECT_SELECTED|4)) == (OBJECT_SELECTED|4))) {
    if ((Mode==0)
       &&
       ((Info & (OBJECT_SELECTED|4)) == (OBJECT_SELECTED|4))) {
  ...
  if ((Mode!=2)
    ...
    if ((Mode==2)
       &&
       ((Info & (OBJECT_SELECTED|5)) == (OBJECT_SELECTED|1))) {
  ...
  if ((Mode!=2)
    ...
    if ((Mode==2)
       &&
       ((Info & (OBJECT_SELECTED|6)) == (OBJECT_SELECTED|2))) {
...
//            SetBkColor(DrawItem->hDC,RGB(0,0,0));
            ...
            switch (ItemNr) {
              ...
              case NetLabelColorNr:
                strcpy(TextStr,SC(211,"Netlabel"));
                break;
              case InstanceRefTextColorNr:
                strcpy(TextStr,SC(212,"Component reference"));
                break;
