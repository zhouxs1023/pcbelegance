...
{
  double   ZoomMinX,ZoomMinY,ZoomMaxX,ZoomMaxY,hulp,cx,cy,PixelsX,PixelsY,
          CentreX,CentreY,cx2,cy2;
  int32   mode;
  int32   OldX2,OldY2;
  RECT    Rect;
...
//  if (CurrentDrawMode==0) CurrentDrawMode=1;
  ...
  if (!SelectionEsc) {
    if ((SelectRectX1!=SelectRectX2)
       &&
       (SelectRectY1!=SelectRectY2)) {
      if ((SelectRectX1<=SelectRectX2)
         ||
         (SelectRectY1<=SelectRectY2)) {
...
//    hulp=(MousePosX-DrawWindowMinX-(cx-(DisplayX/2)));
    Xoffset=((hulp*BoardWidth/DisplayX)-(DisplX*0.5)+
            BoardCentreX-BoardWidth/2);
    hulp=(cy+(DisplayY/2)-MousePosY);
    Yoffset=((hulp*BoardHeight/DisplayY)-(DisplY*0.5)+
            BoardCentreY-BoardHeight/2);
//    DisplX=PixelToRealOffX(DrawWindowMaxX)-PixelToRealOffX(DrawWindowMinX);
//    DisplY=PixelToRealOffY(DrawWindowMaxY)-PixelToRealOffY(DrawWindowMinY);
    Rect.left=(int32)DrawWindowMinX;
    Rect.right=(int32)DrawWindowMaxX;
...
*/
    ...
    if ((MouseDivX>2)
       ...
       (MouseDivY>2)
       &&
       (MousePosX>DrawWindowMaxX-ScrollEndOfWindow)) {
    ...
    if ((MouseDivX>2)
       ...
       (MouseDivY>2)
       &&
       (MousePosY>DrawWindowMaxY-ScrollEndOfWindow)) {
    ...
    if ((MouseDivX>2)
       ...
       (MouseDivY>2)
       &&
       (MousePosX<DrawWindowMinX+ScrollEndOfWindow)) {
      SetCursorPos(MousePosX+ScrollSizeDrawing+ClientStartX,MousePosY+ClientStartY);
      DrawMeasurementLine(CurrentX2,CurrentY2,OldX,OldY,mode|0x10);
    ...
    if ((MouseDivX>2)
       ...
       (MouseDivY>2)
       &&
       (MousePosY<DrawWindowMinY+ScrollEndOfWindow)) {
      SetCursorPos(MousePosX+ClientStartX,MousePosY+ScrollSizeDrawing+ClientStartY);
      DrawMeasurementLine(CurrentX2,CurrentY2,OldX,OldY,mode|0x10);
      ScrollUp(ScrollSize);
//        NrGraphicsObjects
...
// String InfoStr
  ...
  if ((val>=0)
    ...
    if (NewButtonNr!=NrButtonInfo) {
      ...
      if (!DisplayButtonNr) {
        if ((OldButtonNr!=NewButtonNr)
           &&
           (TimerValue>TimeOut+5)) {
          TimeOut=TimerValue;
          DrawMouseInfoOff(0);
          DisplayButtonNr=1;
          ...
          switch (val) {
            case 0:
              strcpy(str,SC(241,"New layout"));
              if (GetKeyString(str2,ID_FILE_NEW,0x21)==1) strcat(str,str2);
              break;
            case 1:
              strcpy(str,SC(327,"Open"));
              if (GetKeyString(str2,ID_FILE_OPEN,0x21)==1) strcat(str,str2);
              break;
            case 2:
              strcpy(str,SC(328,"Save"));
              if (GetKeyString(str2,ID_FILE_SAVE,0x21)==1) strcat(str,str2);
              break;
            case 3:
              strcpy(str,SC(329,"Print"));
              if (GetKeyString(str2,ID_FILE_PRINT,0x21)==1) strcat(str,str2);
              break;
            case 4:
              strcpy(str,SC(330,"Move/rotate/change components"));
              if (GetKeyString(str2,ID_ACTION_MOVE_COMPS2,0x21)==1) strcat(str,str2);
              break;
            case 5:
              strcpy(str,SC(331,"Drag one trace"));
              if (GetKeyString(str2,ID_ACTION_DRAG_TRACE,0x21)==1) strcat(str,str2);
              break;
            case 6:
              strcpy(str,SC(332,"Change traces/vias"));
              if (GetKeyString(str2,ID_ACTION_DRAG_TRACES_VIAS,0x21)==1) strcat(str,str2);
              break;
            case 7:
              strcpy(str,SC(333,"Route traces"));
              if (GetKeyString(str2,ID_ACTION_ROUTE_TRACES,0x21)==1) strcat(str,str2);
              break;
            case 8:
              strcpy(str,SC(334,"Areafills/powerplanes"));
              if (GetKeyString(str2,ID_ACTION_AREAFILLS,0x21)==1) strcat(str,str2);
              break;
            case 9:
              strcpy(str,SC(335,"Draw/change objects other layers"));
              if (GetKeyString(str2,ID_ACTION_OBJECTS,0x21)==1) strcat(str,str2);
              break;
            case 10:
              strcpy(str,SC(336,"Modify component references"));
              if (GetKeyString(str2,ID_ACTION_MOVE_REFS,0x21)==1) strcat(str,str2);
              break;
            case 11:
              strcpy(str,SC(337,"Modify component values"));
              if (GetKeyString(str2,ID_ACTION_MOVE_COMPVALUES,0x21)==1) strcat(str,str2);
              break;
            case 12:
              strcpy(str,SC(338,"Drag traces/vias/component"));
              if (GetKeyString(str2,ID_ACTION_DRAG_COMP_TRACE_VIAS,0x21)==1) strcat(str,str2);
              break;
            case 13:
              strcpy(str,SC(339,"Gate/pin swap"));
              if (GetKeyString(str2,ID_ACTION_GATEPINSWAP,0x21)==1) strcat(str,str2);
              break;
            case 14:
              strcpy(str,SC(340,"Select layers"));
              if (GetKeyString(str2,ID_VIEW_LAYERS,0x21)==1) strcat(str,str2);
              break;
            case 15:
              strcpy(str,SC(341,"Undo"));
              if (GetKeyString(str2,ID_EDIT_UNDO,0x21)==1) strcat(str,str2);
              break;
            case 16:
              strcpy(str,SC(342,"Redo"));
              if (GetKeyString(str2,ID_EDIT_REDO,0x21)==1) strcat(str,str2);
              break;
            case 17:
              strcpy(str,SC(343,"Copy"));
              if (GetKeyString(str2,ID_COPY_OBJECTS,0x21)==1) strcat(str,str2);
              break;
            case 18:
              strcpy(str,SC(344,"Delete"));
              if (GetKeyString(str2,ID_EDIT_DELETE,0x21)==1) strcat(str,str2);
              break;
            case 19:
              strcpy(str,SC(345,"Move"));
              if (GetKeyString(str2,ID_MOVE_OBJECTS,0x21)==1) strcat(str,str2);
              break;
            case 20:
              strcpy(str,SC(346,"Unselect all"));
              if (GetKeyString(str2,ID_UNSELECT_ALL,0x21)==1) strcat(str,str2);
              break;
            case 21:
              strcpy(str,SC(347,"Info selected objects"));
              if (GetKeyString(str2,ID_VIEW_INFOSELECTEDOBJECTS,0x21)==1) strcat(str,str2);
              break;
          }
          ...
          RoundRect(OutputDisplay,ButtonRect.left,ButtonRect.top,
                                  ButtonRect.right,ButtonRect.bottom,5,5);
...

{
  ...
  SelectClipRgn(OutputDisplay,NULL);
  res=BitBlt(OutputDisplay,5,ButtonSizeY*val+5,ButtonSizeX-4,ButtonSizeY-3,ButtonMemoryDC,3,ButtonSizeY*val+3,SRCCOPY);
...

{
  ...
  old=SelectObject(ButtonMemoryDC,ButtonBitmap);
  SelectClipRgn(OutputDisplay,NULL);
  res=BitBlt(OutputDisplay,1,ButtonSizeY*val+1,ButtonSizeX+1,ButtonSizeY+2,ButtonMemoryDC,0,ButtonSizeY*val,SRCCOPY);
  SelectObject(ButtonMemoryDC,old);
  DeleteDC(ButtonMemoryDC);
  DeleteObject(ButtonBitmap);
  SelectClipRgn(OutputDisplay, EditingRegion);
...

{
  ...
  if ((val>=0)
    ...
    if (val!=val2) {
      ...
      if (mode==0) {
        if (FoundButton) DepressButton(val2,1);
        PressButton(val,2);
        return;
      }
      if (mode==1) {
...

{
  ...
  if (Layer==-1) Layer=Design.NrBoardLayers-1;
  ...
  if ((MousePosX!=OldMouseX)
    ...
    if (!DisplayMouseInfo) {
          ...
          if ((CompInfo & OBJECT_NOT_VISIBLE) == 0) {
            ...
            if ((PosX>x1a)
              ...
              for (cnt2=0;cnt2<NrObjects;cnt2++) {
                ...
                if ((!Found)
                   &&
                   (ObjectsConnected(&NewObject,Object))) {
      ...
      if (TimerValue>TimeOut+50) { // 5 seconds
        DrawMouseInfoOff(0);
      }
    }
  }
...
{
  if (DisplayInfoStr[0]==0) return 0;
  if (TimeOutInfo==-1) return 0;
...
*/
  ...
  RoundRect(OutputDisplay,InfoRect.left,InfoRect.top,
                          InfoRect.right,InfoRect.bottom,5,5);
//  SelectObject(OutputDisplay,BackGroundPen);
  ...
  for (cnt=0;cnt<NrLines;cnt++) {
    if (cnt==0) {
      ...
      TextOut(OutputDisplay,InfoRect.left+4+ox,InfoRect.top+4+MaxY*cnt,strp[cnt],strlen(strp[cnt]));
    } else {
      if (strcmp(strp[cnt],"\f")==0) {
        oy=InfoRect.top+8+MaxY*cnt;
        DrawLine(InfoRect.left+1,oy,InfoRect.right-1,oy);
      } else {
        TextOut(OutputDisplay,InfoRect.left+4,InfoRect.top+4+MaxY*cnt,strp[cnt],strlen(strp[cnt]));
      }
    }
  }
  SelectObject(OutputDisplay,SaveFont);
...

{
  ...
  if (mode==0) {
    if (!PopupDisplayVisible) return 0;
  }
  if (TimeOutInfo!=-1) {
    ok=1;
  }
  ...
  if ((TimeOutInfo!=-1)
     &&
     (TimerValue>TimeOutInfo+ButtonInfoTimeout)) {
    DisplayInfoOff(0);
    GetInfoStr(NULL,1);
    return 1;
  }
