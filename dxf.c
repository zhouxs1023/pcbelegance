...
char  BlockNames[MAX_BLOCKS][MAX_NR_LAYERS],BlockNestingNames[5][MAX_NR_LAYERS],
      BlockLayerName[5][MAX_NR_LAYERS];
char  ImportDXFFile[MAX_LENGTH_STRING];
...
double BlockInsertScaleX[5],BlockInsertScaleY[5],
...
double PointsX[1000],PointsY[1000],MinX,MinY,MaxX,MaxY,
...
// ************************************************************************************************
      ...
      SendDlgItemMessage(Dialog,IDOK                        ,WM_SETTEXT,0,(LPARAM)SC(45,"OK"));
      SendDlgItemMessage(Dialog,IDCANCEL                    ,WM_SETTEXT,0,(LPARAM)SC(46,"Cancel"));
      SendDlgItemMessage(Dialog,ID_UNSELECTALL              ,WM_SETTEXT,0,(LPARAM)SC(109,"Unselect all"));
      SendDlgItemMessage(Dialog,ID_SELECTALL                ,WM_SETTEXT,0,(LPARAM)SC(110,"Select all"));
      SendDlgItemMessage(Dialog,IDC_CHECK1                  ,WM_SETTEXT,0,(LPARAM)SC(168,"Filled objects"));
      SendDlgItemMessage(Dialog,IDC_CHECK2                  ,WM_SETTEXT,0,(LPARAM)SC(439,"Mirror X"));
...
// ************************************************************************************************
      ...
      SetWindowTextUTF8(Dialog,SC(169,"DXF export"));
      cnt3=0;
...
{
  int32 cnt,cnt2,cnt3,cnt4,res,NrLayers,Layer,SegmentCount,ObjectPolygonLength,
        SelectMask1,SelectMask2,Mirror,LineSegmentCount,
        count,TextRotation,lengte,CircleMode;
  double x1,y1,x2,y2,x3,y3,x4,y4,h2,Rotation,DXFOutputResolution,
        x3a,Thickness;
  int   fp;
          ...
          switch (Object->ObjectType) {
            case OBJECT_LINE:
              if (Object->Info2!=0) {
                ...
                x2=Object->x2;
                y2=Object->y2;
                LineSegmentCount=DimensionToLineSegments(x1,y1,x2,y2,(double *)&LineSegments,Object->Info2);
                SegmentCount=0;
...

{
  ...
  int32 fp,Found,res,result,ReadMode,FilePos,FilePos2,LineNr,
        Nesting,CommandCode,ValueInt,NrPolylinePoints,
        LineLength,ok,cnt;
  double ValueDouble;
  char  NumberString[MAX_LENGTH_STRING],ValueStr[MAX_LENGTH_STRING],
        CurrentCommandString[MAX_LENGTH_STRING],
        str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],
        LayerName[MAX_LENGTH_STRING],RegelBuf[4096],
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SelectionEsc=0;
      SendDlgItemMessage(Dialog,IDC_STATIC1                 ,WM_SETTEXT,0,(LPARAM)"");
      SendDlgItemMessage(Dialog,IDOK                        ,WM_SETTEXT,0,(LPARAM)SC(45,"OK"));
      SendDlgItemMessage(Dialog,IDCANCEL                    ,WM_SETTEXT,0,(LPARAM)SC(46,"Cancel"));
      SendDlgItemMessage(Dialog,IDHELP                      ,WM_SETTEXT,0,(LPARAM)SC(47,"Help"));
...

{
  ...
  DialogMode=0;
  res=DialogBox(GEOMClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_DXFLAYER), GEOMWindow, (DLGPROC)DXFLayerDialogBody);
  if (res<=0) return -1;
  return res;
}
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      if (DialogMode==0) {
        ...
        SetWindowTextUTF8(Dialog,SC(173,"Select layer"));
      ...
      SendDlgItemMessage(Dialog,IDOK                   ,WM_SETTEXT,0,(LPARAM)SC(45,"OK"));
      SendDlgItemMessage(Dialog,IDCANCEL               ,WM_SETTEXT,0,(LPARAM)SC(46,"Cancel"));
      SendDlgItemMessage(Dialog,IDC_STATIC1            ,WM_SETTEXT,0,(LPARAM)SC(174,"Layers"));
      return about;
    case WM_MOVE:
      break;
    case WM_COMMAND:
...
/*
  ...
  strcpy(LayerNames[NrGeomLayers++],SC(137,"Pads top"));
  strcpy(LayerNames[NrGeomLayers++],SC(138,"Pads bottom"));
  ...
  strcpy(LayerNames[NrGeomLayers++],SC(17,"Placement outline"));
  strcpy(LayerNames[NrGeomLayers++],SC(95,"Silkscreen"));
  strcpy(LayerNames[NrGeomLayers++],"Routing keepout");
  strcpy(LayerNames[NrGeomLayers++],SC(18,"Drill"));
  strcpy(LayerNames[NrGeomLayers++],SC(19,"Drill unplated"));
*/
  strcpy(LayerNames[NrGeomLayers++],SC(175,"Info 1"));
  strcpy(LayerNames[NrGeomLayers++],SC(176,"Info 2"));
  strcpy(LayerNames[NrGeomLayers++],SC(177,"Info 3"));
  strcpy(LayerNames[NrGeomLayers++],SC(178,"Info 4"));
...
*/
  ...
  PolyLineFlags=0;
  ClearObject(BlockAngles);
  if (GetNewFileUTF8(GEOMWindow,NULL,ImportDXFFile,DXFFilePath,SC(179,"DXF file"),
                     NULL,SC(180,"DXF files"),"dxf",0)!=0) return -1;
                    ...
                    if (BulgeFlags==1) {
                      if (InRangeSpecial(LengthLine,VertexStartWidthTemp[cnt],
                        ...
                        ConvNormalCoorToPolar(PointsX[cnt],PointsY[cnt],PointsX[cnt+1],PointsY[cnt+1],
                                              &Angle,&Length2);
                        NewObject.x1=(float)(PointsX[cnt]+VertexStartWidthTemp[cnt]*0.5*cos(Angle));
                        NewObject.y1=(float)(PointsY[cnt]+VertexStartWidthTemp[cnt]*0.5*sin(Angle));
                        NewObject.x2=(float)(PointsX[cnt+1]-VertexStartWidthTemp[cnt]*0.5*cos(Angle));
                        NewObject.y2=(float)(PointsY[cnt+1]-VertexStartWidthTemp[cnt]*0.5*sin(Angle));
                        LengthLine=CalcLengthLine(NewObject.x1,NewObject.y1,NewObject.x2,NewObject.y2);
...
//  GetMaxRectSelectedObjects(0,&MinX,&MinY,&MaxX,&MaxY);
  GetMinMaxSelectedObjects();
  ViewFull();
  sprintf(str,SC(181,"minx,miny = %.4f,%.4f : maxx,maxy = %.4f,%.4f\n\nFor scaling use the right mouse button menu"),
              SelectedMinX/100000.0,SelectedMinY/100000.0,
              SelectedMaxX/100000.0,SelectedMaxY/100000.0);
