...
double  CentreSelectedX,CentreSelectedY,
        SelectedMinX,SelectedMinY,SelectedMaxX,SelectedMaxY,
        DXFminx,DXFminy,DXFmaxx,DXFmaxy;



int32  NrBlocks,NrDXFLayers,SelectedLayers[32],DialogMode,ExportLayers[32],
       BlockStartP[MAX_BLOCKS],BlockEndP[MAX_BLOCKS],NrBlocks,BlockNr[5],BlockP[5],BlockSize[5],
       BlockLineNr[MAX_BLOCKS],FilePointerDXF,FileSizeDXF,ok,
       BlockNestingLevel,FilePosition[5];
char   ExportFileName[MAX_LENGTH_STRING],ImportDXFFile[MAX_LENGTH_STRING],
       BlockNames[MAX_BLOCKS][64],BlockNestingNames[5][64],BlockLayerName[5][64],
       DXFLayers[128][64],ExportDXFFileName[MAX_LENGTH_STRING]="",*DXFBuffer;

double BlockInsertX[5],BlockInsertY[5],BlockInsertZ[5],BlockRotation[5],
       BlockBaseX[5],BlockBaseY[5],BlockBaseZ[5],
       BlockInsertScaleX[5],BlockInsertScaleY[5],
       StartAngle,EndAngle,PolyLineX[1000],PolyLineY[1000],PolyLineZ[1000],
       StartWidth,EndWidth,PolyLineStartWidth,PolyLineEndWidth,
       VertexStartWidth[1000],VertexEndWidth[1000],VertexBulge[1000],
       AngleBetweenLines[1000],
       PointsX[1000],PointsY[1000],MinX,MinY,MaxX,MaxY,
...

{
  ...
  while ((cnt<NrBlocks)
        &&
        (stricmpOwn(BlockNames[cnt],BlockName)!=0)) cnt++;
  if (cnt<NrBlocks) return cnt;
...
{
  int32  FoundStr,BlockStart,fp,Found,res,result,ReadMode,FilePos,FilePos2,LineNr,
         Nesting,CommandCode,ValueInt,NrPolylinePoints,LineLength,cnt;
  double ValueDouble;
  char  NumberString[MAX_LENGTH_STRING],ValueStr[MAX_LENGTH_STRING],CurrentCommandString[MAX_LENGTH_STRING],
        str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],LayerName[MAX_LENGTH_STRING],RegelBuf[4096],CopyStr[MAX_LENGTH_STRING];

  if ((FileSizeDXF=FileSizeUTF8(FileName))<0) return -1;
  ...
  // TextLineNr
        ...
        switch (CommandCode) {
          case 8:
            ...
            cnt=0;
            while ((cnt<NrDXFLayers)
                  &&
                  (stricmpOwn(DXFLayers[cnt],ValueStr)!=0)) cnt++;
            if (cnt==NrDXFLayers) {
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      if (DialogMode==0) {
        SetWindowTextOwn(Dialog,SC(362,"Select DXF layer"));
        res=SendDlgItemMessageOwn(Dialog,IDC_LIST1,LB_SELITEMRANGE,(WPARAM)1,(LPARAM)MAKELPARAM(0,NrDXFLayers));
      } else {
        SetWindowTextOwn(Dialog,SC(340,"Select layers"));
      }
      return about;
    case WM_MOVE:
      break;
...
{
  int32  FoundStr,StartPoint,BlockStart,HeaderStart,FoundRectangle,
        BlocksStart,TableStart,ArcCounterClockWise,AngleInDegrees;
  int32  Attrib40;
  int32  Attrib41;
  int32  Attrib42;
  int32  Attrib50;
  ...
  int32 Found,res,TextAlignment,
        PolyLineFlags,cnt,cnt2,cnt3,LineLength,Units,LineNr,
        LastEntityCommand,Attributes,ReadMode,FilePos,FilePos2,PreviousFilePos,
        Nesting,CommandCode,ValueInt,NrPolylinePoints,ok,
        LineNrP[5],BulgeFlags,LayerCnt,NrSelectedDXFLayers,TextMirror;
  double ValueDouble,X[10],Y[10],Z[10],Width,Height,Thickness,Diameter,Angle,
         Length2,LengthLine,dx,dy,BlockAngles[5],StartAngle,EndAngle,
         BlockInsertScaleValueX,BlockInsertScaleValueY,
         MinX,MinY,MaxX,MaxY,AnglesAreafill[32],x1,y1,x2,y2,x3,y3;
  char  NumberString[MAX_LENGTH_STRING],ValueStr[MAX_LENGTH_STRING],CurrentCommandString[MAX_LENGTH_STRING],AttributeName[MAX_LENGTH_STRING],
        LayerName[MAX_LENGTH_STRING],TextString[MAX_LENGTH_STRING],BlockName[MAX_LENGTH_STRING],FileName[MAX_LENGTH_STRING],
        str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],RegelBuf[4096],CopyStr[MAX_LENGTH_STRING],
        VarName[MAX_LENGTH_STRING],PolyLineLayerName[MAX_LENGTH_STRING],
        LayerNameToCheck[MAX_LENGTH_STRING],EenString[10]=" ";
...
//  Layer=INFO_LAYER2;
  ...
  if (GetNewFileUTF8(PCBWindow,NULL,ImportDXFFile,ExportDir,SC(363,"DXF file"),
                     NULL,SC(364,"DXF files"),"dxf",0)) {
    return -1;
  }

//  if (LoadNewFile3(ImportDXFFile,SC(363,"DXF file"),SC(364,"DXF files"),"dxf",0)!=0) return -1;
...
// NrDXFLayers DXFLayers
  ...
  if (NrSelectedDXFLayers<=0) return -1;
/*
  ReWriteDXF(FileName);
  return 0;
*/
...
// ***************************************************************************************************
          ...
          if ((StartPoint)
             &&
             (stricmpOwn(ImportLayerName,LayerName)==0)) {
            res=1;
          }
#endif
...
*/
              ...
              if ((StartPoint)
                ...
                for (cnt=0;cnt<NrPolylinePoints-1;cnt++) {
                  if ((cnt==0)
                     ||
                     (cnt==NrPolylinePoints-1)) {
                    AngleBetweenLines[cnt]=10000.0;
                  } else {
                    AngleBetweenLines[cnt]=GetAngleBetweenLines(PointsX[cnt-1],PointsY[cnt-1],
                                                                PointsX[cnt  ],PointsY[cnt  ],
                ...
                for (cnt=0;cnt<NrPolylinePoints-1;cnt++) {
                  ...
                  LengthLine=CalcLengthLine(PointsX[cnt],PointsY[cnt],PointsX[cnt+1],PointsY[cnt+1]);
                  if (VertexStartWidthTemp[cnt]!=0.0) {
                    if (VertexBulge[cnt]!=0.0) {
                      if (InRangeSpecial(VertexBulge[cnt],1.0,0.001)) {
                        if (InRangeSpecial(LengthLine,VertexStartWidthTemp[cnt],
                                           max(0.0000001,(fabs(VertexStartWidthTemp[cnt])/1000000)))) { // Round filled circle
                          NewObjectArc.CentreX=(float)((PointsX[cnt]+PointsX[cnt+1])*0.5);
                          NewObjectArc.CentreY=(float)((PointsY[cnt]+PointsY[cnt+1])*0.5);
                        ...
                        if (dx==0.0) {   // Vertical trace
                          NewObjectLine.X1=(float)PointsX[cnt];
                          NewObjectLine.X2=(float)PointsX[cnt];
                          if (PointsY[cnt]<PointsY[cnt+1]) {
                            NewObjectLine.Y1=(float)(PointsY[cnt]+VertexStartWidthTemp[cnt]*0.5);
                            NewObjectLine.Y2=(float)(PointsY[cnt+1]-VertexStartWidthTemp[cnt]*0.5);
                          } else {
                            NewObjectLine.Y1=(float)(PointsY[cnt]-VertexStartWidthTemp[cnt]*0.5);
                            NewObjectLine.Y2=(float)(PointsY[cnt+1]+VertexStartWidthTemp[cnt]*0.5);
                          }
                        } else {
                          if (dy==0.0) { // Horizontal trace
                            NewObjectLine.Y1=(float)PointsY[cnt];
                            NewObjectLine.Y2=(float)PointsY[cnt];
                            if (PointsX[cnt]<PointsX[cnt+1]) {
                              NewObjectLine.X1=(float)(PointsX[cnt]+VertexStartWidthTemp[cnt]*0.5);
                              NewObjectLine.X2=(float)(PointsX[cnt+1]-VertexStartWidthTemp[cnt]*0.5);
                            } else {
                              NewObjectLine.X1=(float)(PointsX[cnt]-VertexStartWidthTemp[cnt]*0.5);
                              NewObjectLine.X2=(float)(PointsX[cnt+1]+VertexStartWidthTemp[cnt]*0.5);
                            }
                          } else {       // Trace at any angle
                            ConvNormalCoorToPolar(PointsX[cnt],PointsY[cnt],PointsX[cnt+1],PointsY[cnt+1],
                                                  &Angle,&Length2);
                            NewObjectLine.X1=(float)(PointsX[cnt]+VertexStartWidthTemp[cnt]*0.5*cos(Angle));
                            NewObjectLine.Y1=(float)(PointsY[cnt]+VertexStartWidthTemp[cnt]*0.5*sin(Angle));
                            NewObjectLine.X2=(float)(PointsX[cnt+1]-VertexStartWidthTemp[cnt]*0.5*cos(Angle));
                            NewObjectLine.Y2=(float)(PointsY[cnt+1]-VertexStartWidthTemp[cnt]*0.5*sin(Angle));
                          }
                        }
                        ...
                        LengthLine=CalcLengthLine(NewObjectLine.X1,NewObjectLine.Y1,
                                                  NewObjectLine.X2,NewObjectLine.Y2);
                        if (LengthLine<0.01e6) { // Round pad
                      ...
                      if (InRangeSpecial(VertexStartWidthTemp[cnt],VertexEndWidthTemp[cnt],
                                         max(0.00000001,(fabs(VertexStartWidthTemp[cnt])/1000000)))) {
                        if (dx==0.0) {   // Vertical block
                          NewObjectRect.CentreX=(float)((PointsX[cnt]+PointsX[cnt+1])*0.5);
                          NewObjectRect.CentreY=(float)((PointsY[cnt]+PointsY[cnt+1])*0.5);
                          ...
                          if (dy==0.0) { // Horizontal block
                            ...
                            NewObjectRect.Info|=OBJECT_FILLED;
                            AddObjectRect(&NewObjectRect);
                          } else {       // Diagonal polygon
                            ok=1;
                            MakePolygonFromSpecialLine(PointsX[cnt],PointsY[cnt],PointsX[cnt+1],PointsY[cnt+1],
                                                       &NewObjectPolygon,VertexStartWidthTemp[cnt],
                      ...
                      } else { // Different start and end width -> Make polygon
                        ok=1;
                        NewObjectPolygon.NrVertices=4;
                        if (dx==0.0) {   // Vertical aligned polygon
                          NewObjectPolygon.Points[0].x=PointsX[cnt]-VertexStartWidthTemp[cnt]*0.5;
                          NewObjectPolygon.Points[0].y=PointsY[cnt];
                          NewObjectPolygon.Points[1].x=PointsX[cnt]+VertexStartWidthTemp[cnt]*0.5;
                          NewObjectPolygon.Points[1].y=PointsY[cnt];
                          NewObjectPolygon.Points[2].x=PointsX[cnt+1]+VertexEndWidthTemp[cnt]*0.5;
                          NewObjectPolygon.Points[2].y=PointsY[cnt+1];
                          NewObjectPolygon.Points[3].x=PointsX[cnt+1]-VertexEndWidthTemp[cnt]*0.5;
                          NewObjectPolygon.Points[3].y=PointsY[cnt+1];
                        } else {
                          if (dy==0.0) { // Horizontal aligned polygon
                            NewObjectPolygon.Points[0].x=PointsX[cnt];
                            NewObjectPolygon.Points[0].y=PointsY[cnt]-VertexStartWidthTemp[cnt]*0.5;
                            NewObjectPolygon.Points[1].x=PointsX[cnt];
                            NewObjectPolygon.Points[1].y=PointsY[cnt]+VertexStartWidthTemp[cnt]*0.5;
                            NewObjectPolygon.Points[2].x=PointsX[cnt+1];
                            NewObjectPolygon.Points[2].y=PointsY[cnt+1]+VertexEndWidthTemp[cnt]*0.5;
                            NewObjectPolygon.Points[3].x=PointsX[cnt+1];
                            NewObjectPolygon.Points[3].y=PointsY[cnt+1]-VertexEndWidthTemp[cnt]*0.5;
                          } else {       // Diagonal polygon
                            ok=1;
                            MakePolygonFromSpecialLine(PointsX[cnt],PointsY[cnt],PointsX[cnt+1],PointsY[cnt+1],
                                                       &NewObjectPolygon,VertexStartWidthTemp[cnt],
...
#ifdef _DEBUG
                    if (CalcLengthLine(NewObjectLine.X1,NewObjectLine.Y1,
                                       NewObjectLine.X2,NewObjectLine.Y2)==0.0) {
                      ok=1;
                    }
#endif
                    AddObjectLine(&NewObjectLine);
...
//    LayerName
              ...
              if ((StartPoint)
                 &&
                 ((LayerCnt=CheckDXFLayer(LayerName,NrSelectedDXFLayers))!=-1)) {
                for (cnt=0;cnt<4;cnt++) {
                ...
                if (FoundRectangle) {
                  NewObjectRect.CentreX=(float)((NewObjectPolygon.maxx+NewObjectPolygon.minx)*0.5);
                  NewObjectRect.CentreY=(float)((NewObjectPolygon.maxy+NewObjectPolygon.miny)*0.5);
...
//    LayerName   atan2
              ...
              if ((StartPoint)
                ...
                NewObjectArc.LineThickNess=(float)Design.SilkScreenWidth;
                if (StartAngle-0.01>360.0) StartAngle-=360.0;
...
//      4 = Centre
              ...
              if ((StartPoint)
                ...
                memmove(&NewObjectText2.Text,TextString,max(strlen(TextString),sizeof(NewObjectText2.Text)-1));
...
// ***************************************************************************************************
  ...
  // TextLineNr
          ...
          if ((stricmpOwn(ValueStr,"INSERT")==0)
             ||
             (stricmpOwn(ValueStr,"TEXT")==0)) {
  // TextLineNr
            BlockInsertScaleValueX=1.0;
            BlockInsertScaleValueY=1.0;
          }
...
// Check %% for characters
          ...
          case 8:
            ...
            break;
          case 9:
            if (VarName[0]!=0) { // Previous variable name
  ...
  ViewFull();
  sprintf(str,SC(365,"minx,miny = %.4f,%.4f : maxx,maxy = %.4f,%.4f\n\nFor scaling use the right mouse menu"),
              MinX/100000.0,MinY/100000.0,MaxX/100000.0,MaxY/100000.0);
  MessageBoxOwn(PCBWindow,str,SC(366,"Info message"),MB_APPLMODAL|MB_OK);
  return 0;
}
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(266,"Layers"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(267,"Options"));
      SetDialogItemText(Dialog,IDC_STATIC3                ,SC(367,"Filename"));
      SetDialogItemText(Dialog,IDC_RADIO1                 ,SC(268,"Component references on silkscreen"));
      SetDialogItemText(Dialog,IDC_RADIO2                 ,SC(269,"Component values on silkscreen "));
      SetDialogItemText(Dialog,IDC_RADIO3                 ,SC(368,"Component refs/values separate"));
      SetDialogItemText(Dialog,IDC_CHECK1                 ,SC(293,"Plot board outline on each layer"));
      SetDialogItemText(Dialog,IDC_CHECK2                 ,SC(272,"Mirror X"));
      SetDialogItemText(Dialog,IDC_CHECK3                 ,SC(1106,"Objects filled"));
      SetWindowTextOwn(Dialog,SC(369,"DXF export"));
      switch (DXFOptions & 3) {
        case 0:
      ...
      if ((DXFOptions & 16) == 0) {  // Objects filled
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        ...
        case IDC_CHECK5:
          ...
          break;
        case IDC_BUTTON1:
          if (!GetNewFileUTF8(PCBWindow,NULL,ExportFileName,ExportDir,SC(363,"DXF file"),
                              NULL,SC(364,"DXF files"),"dxf",1)) {
            SendDlgItemMessageOwn(Dialog,IDC_EDIT1,WM_SETTEXT,0,(LPARAM)ExportFileName);
          }
          ok=1;
          break;
...

{
  ...
  if ((DXFOptions & 8) == 8) {  // Mirror X
    x1=-x1+DXFminx+DXFmaxx;
    x2=-x2+DXFminx+DXFmaxx;
  }
  if ((DXFOptions & 16) == 0) { // Do not fill object
    Thickness=0.0;
    sprintf(str,"  0\r\nPOLYLINE\r\n  8\r\n%s\r\n",DXFLayer);
    WriteToFile(fp,str);
    sprintf(str," 66\r\n      1\r\n 40\r\n%.4f\r\n 41\r\n%.4f\r\n",Thickness,Thickness);
...
*/
  ...
  if (((mode & 2)==0)            // Open circle
     ||
     ((DXFOptions & 16) == 0)) { // Do not fill object
    sprintf(str,"  0\r\nCIRCLE\r\n  8\r\n%s\r\n",DXFLayer);
    WriteToFile(fp,str);
    sprintf(str," 10\r\n%.4f\r\n 20\r\n%.4f\r\n 30\r\n0.0\r\n 40\r\n%.4f\r\n",x1,y1,x2*0.5);
    WriteToFile(fp,str);
...
void DXFWriteRectangle(double x1,double y1,double x2,double y2,double Thickness,
                       LPSTR DXFLayer,int32 fp,int32 mode)
...
void DXFWriteText(double x1,double y1,double x2,double Rotation,double Thickness,
                  int32 FontNr,int32 Alignment,int32 Mirror,LPSTR Text,
                  LPSTR DXFLayer,int32 fp,int32 mode)

{
  int32 Length,cnt3,cnt5,cnt6,code,DrawChar,lengte,MaxCountX,
        count,NrLines,cnt4;
  char  str[1024];
...
{
  int32 cnt,cnt2,cnt3,cnt4,cnt5,cnt6,NrLayers,Layer,
        SelectMask1,SelectMask2,Mirror,ok,CompOutlineOnBottom,CompOutlineLayers,
        count,TextRotation,fp,ViaType,
        SegmentCount,LineSegments;
  ...
  NrLayers=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_DXF_EXPORT35), PCBWindow, (DLGPROC)DXFExportLayerDialogBody);
  if (NrLayers<=0) return -1;
// SelectedLayers
...
*/
  if (FileExistsUTF8(ExportFileName)==0) {
    sprintf(str2,SC(370,"File %s exist overwrite ?"),ExportFileName);
    if (MessageBoxOwn(PCBWindow,str2,SC(1,"Message"),MB_OKCANCEL|MB_APPLMODAL)!=IDOK) {
      return -1;
    }
  }

  if ((fp=FileOpenWriteUTF8(ExportFileName))<=0) return -1;
    ...
    for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
      ...
      if ((ObjectLine->Info & (OBJECT_NOT_VISIBLE|SelectMask1)) == SelectMask2) {
        if ((ObjectLine->Layer==Layer)
           ||
           ((ObjectLine->Layer==BOARD_OUTLINE_LAYER)
           &&
           ((DXFOptions & 4) == 0))) {
          ...
          if (ObjectLine->LineMode!=0) {
            ...
            for (cnt3=0;cnt3<LineSegments;cnt3++) {
              ...
              if ((DXFOptions & 16) == 16) { // Filled objects
    ...
    for (cnt=0;cnt<Design.NrObjectRects;cnt++) {
      ...
      if ((ObjectRect->Info & (OBJECT_NOT_VISIBLE|SelectMask1)) == SelectMask2) {
        if ((ObjectRect->Layer==Layer)
           ...
           &&
           ((ObjectRect->Info & OBJECT_FILLED) == 0)
           &&
           ((DXFOptions & 4) == 0))) {
          ...
          if (((ObjectArc->Info & OBJECT_FILLED) != 0)
            ...
            if ((ObjectArc->Info & OBJECT_FILLED) != 0) {
              DXFWriteCircle(ObjectArc->CentreX/100000,ObjectArc->CentreY/100000,
                             ObjectArc->Width/100000,ObjectArc->LineThickNess/100000,DXFLayer,fp,2);
            } else {
              DXFWriteCircle(ObjectArc->CentreX/100000,ObjectArc->CentreY/100000,
                             ObjectArc->Width/100000,ObjectArc->LineThickNess/100000,DXFLayer,fp,0);
            }
          } else {
            if (InRange2(ObjectArc->Width,ObjectArc->Height)) {
              ...
              LineSegments=ArcToLineSegments(ObjectArc->CentreX,ObjectArc->CentreY,ObjectArc->Width,ObjectArc->Height,
                                             ObjectArc->StartDiffX,ObjectArc->StartDiffY,
      ...
      if ((ObjectText2->Info & (OBJECT_NOT_VISIBLE|SelectMask1)) == SelectMask2) {
        if (ObjectText2->Layer==Layer) {
          ...
          DXFWriteText(ObjectText2->X,ObjectText2->Y,ObjectText2->FontHeight,ObjectText2->Rotation,
                       ObjectText2->LineThickNess,ObjectText2->FontNr,0,Mirror,ObjectText2->Text,DXFLayer,fp,0);
        }
      }
    }
            ...
            switch (Object->ObjectType) {
              ...
              case OBJECT_ARC:
                if (Object->Layer==Layer) {
                  ...
                  if ((InRange(Object->x3,Object->x4))
                     &&
                     (InRange(Object->y3,Object->y4))
                     &&
                    ...
                    if (InRange(Object->x2,Object->y2)) {
                      ...
                      LineSegments=ArcToLineSegments(Object->x1,Object->y1,Object->x2,Object->y2,
                                                     Object->x3,Object->y3,Object->x4,Object->y4,(double *)&LineBuf,1);
                      SegmentCount=0;
                      for (cnt5=0;cnt5<LineSegments;cnt5++) {
                        x1=LineBuf[SegmentCount]/100000.0;
                        SegmentCount++;
            ...
            if (LayerOk) {
              ...
              switch (Object->ObjectType) {
                ...
                case OBJECT_TEXT:
                  ...
                  DXFWriteText(Object->x1,Object->y1,Object->x2,Object->RotationAngle,
                               0.0,Object->Test >> 16,0,Mirror,(LPSTR)Object->TraceNr,DXFLayer,fp,0);
                  break;
              }
            }
          }
// *******************************************************************************************************
          ...
          switch (Layer) {
            case SILKSCREEN_BOTTOM:
              if ((DXFOptions & 3) == 0) { // Reference on silkscreen
                if (Mirror==1) {
                  if (MakeObjectFromCompRef(Comp,&NewObject,0)==0) {
                    LayerOk=1;
                  }
                }
              } else {                     // Value on silkscreen
                if (Mirror==1) {
          ...
          if (LayerOk) {
            ...
            DXFWriteText(NewObject.x1,NewObject.y1,NewObject.x2,NewObject.RotationAngle,
                         0.0,0,0,Mirror,(LPSTR)NewObject.TraceNr,DXFLayer,fp,0);
          }
        }
      }
    }
