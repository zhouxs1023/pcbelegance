...
int32 NrBlocks,NrDXFLayers,SelectedLayers[128],DialogMode,
      ...
      NrGeomLayers,BlockNestingLevel,FilePosition[5],LayerIDs[32],NrSelectedDXFLayers;

char  BlockNames[MAX_BLOCKS][MAX_NR_LAYERS],BlockNestingNames[5][MAX_NR_LAYERS],
      BlockLayerName[5][MAX_NR_LAYERS],ImportDXFFile[MAX_LENGTH_STRING],
      DXFLayers[128][64],DXFLayer[MAX_LENGTH_STRING],LayerNames[32][128],
...
double BlockInsertX[5],BlockInsertY[5],BlockInsertZ[5],BlockRotation[5],
       BlockBaseX[5],BlockBaseY[5],BlockBaseZ[5],BlockRotation[5],
       ...
       VertexStartWidth[1000],VertexEndWidth[1000],
...
// ************************************************************************************************
      ...
      SetDialogItemTextUTF8(Dialog,IDCANCEL      ,"Cancel");

// ************************************************************************************************
      SelectionEsc=0;

      SetWindowTextUTF8(Dialog,"DXF export");
    ...
    case WM_COMMAND:
      switch (WParam) {
        ...
        case ID_UNSELECT_ALL:
          ...
          break;
        case IDOK:
          NrSelectedLayers=SendDlgItemMessage(Dialog,IDC_LIST1,LB_GETSELITEMS,NrOutputLayers,(LPARAM)&SelectedLayers);
          for (cnt=0;cnt<NrOutputLayers;cnt++) {
            LayerInfo[cnt]&=~1;
          }
...
void DXFWriteText(double x1,double y1,double Height,double Rotation,int32 TextAlignment,
                  LPSTR DXFLayer,int32 fp,LPSTR TextString)
...
{
  double x1,y1,x1a,y1a,x2,y2,x3,y3,x4,y4,hulp,Angle1,Angle2,Length2,
         Thickness,Rotation,x11,y11,x22,y22;
  int32  cnt,cnt2,cnt3,cnt4,cnt5,TextAlignment,BusConnectionType,
         AddMode,ok,NrLines,lengte,ConnectionType,CircleMode,
         TextMode,Alignment,fp,Layer,InstanceRotation,TextRotation,
         MemPos,res,SymbolNr,NrProperties,
  ...
  char   PinBusStrings[16][64],TextStr[1024],TextStr2[1024],ExportFileName[MAX_LENGTH_STRING],
         NewPinBusStr[512],str2[MAX_LENGTH_STRING];
  int32  LinePos[16];
  LPSTR  LabelText;
  ...
  if (!EditingSymbol) {
    ...
    strcpy(DXFLayers[NrOutputLayers],"Wires");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=BUS_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"Busses");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=JUNCTION_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"Junctions");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=BUSCONNECTIONS_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"Busconnections");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=GLOBALCONNECTIONS_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"Globalconnections");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=ONEPINNET_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"OnePinNets");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=NETLABEL_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"Netlabels");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=SYMBOLS_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"Symbols");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=REFERENCE_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"SymbolReferences");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=VALUE_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"SymbolValues");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=OBJECTS_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"OtherObjects");
    NrOutputLayers++;
  } else {
    ExportLayers[NrOutputLayers]=PIN_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"Pins");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=POWERPIN_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"Powerpins");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=PINBUS_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"Pinbusses");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=REFERENCE_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"SymbolReference");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=VALUE_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"SymbolValue");
    NrOutputLayers++;
    ExportLayers[NrOutputLayers]=OBJECTS_LAYER;
    strcpy(DXFLayers[NrOutputLayers],"OtherObjects");
    NrOutputLayers++;
  ...
  if (FileExistsUTF8(ExportFileName)==0) {
    sprintf(str2,"File %s exist overwrite ?",ExportFileName);
    if (MessageBoxUTF8(SCHWindow,str2,"Message",MB_OKCANCEL|MB_APPLMODAL)!=IDOK) {
      return -1;
    }
  }
    ...
    if (Layer==GLOBALCONNECTIONS_LAYER) {
      for (cnt=0;cnt<Design.NrGlobalConnections;cnt++) {
        ...
        if ((GlobalConnection->Info & (OBJECT_NOT_VISIBLE|OBJECT_PROTECTED)) == 0) {
          ...
          DXFWriteText(x1a,y1a,1.0,0,(GlobalConnection->NameInfo & 15),
                       DXFLayer,fp,GlobalConnection->Text);
        }
      }
    }
// ****************************************************************************************************
      ...
      for (cnt=0;cnt<DesignSymbol.NrPins;cnt++) {
        ...
        if ((Pin->Info & OBJECT_NOT_VISIBLE) == 0) {
          ...
          if (!EditingSheetSymbol) {
            DXFWriteText(x1a,y1a,1.0,TextRotation*90.0,(Pin->NameInfo & 15),
                         DXFLayer,fp,Pin->Name);
          } else {
            DXFWriteText(x1a,y1a,1.0,TextRotation*90.0,(Pin->NameInfo & 15),
                         DXFLayer,fp,Pin->Label);
          }
        }
      }
    }
    ...
    if (Layer==PINBUS_LAYER) {
      for (cnt=0;cnt<DesignSymbol.NrPinBusses;cnt++) {
        ...
        if ((PinBus->Info & OBJECT_NOT_VISIBLE) == 0) {
          ...
          AddMode=0;
          if (TextRotation) AddMode+=4;
          if ((TextAlignment==6)
             ||
             (TextAlignment==8)) AddMode+=1; // Mirror X
          if ((TextAlignment==2)
             ||
             (TextAlignment==8)) AddMode+=2; // Mirror Y
...
// ****************************************************************************************************
        ...
        if (Layer==VALUE_LAYER) {
          if ((!EditingSymbol)
             &&
             ((Instance->ValueInfo & TEXT_NOT_VISIBLE) == TEXT_NOT_VISIBLE)) {
          } else {
            InstanceRotation=(Instance->SymbolInfo) & OBJECT_ROTATE90;
            ObjectMirrorX=((Instance->SymbolInfo & OBJECT_MIRRORX) >> 4);
            ObjectMirrorY=((Instance->SymbolInfo & OBJECT_MIRRORY) >> 5);
            ...
            switch (Object->ObjectType) {
              ...
              case SYMBOL_CIRCLE:
                if (Object->Thickness==0.0) {
                  ...
                  CircleMode=((Object->Info2 & 0x80) >> 4)+
                             ((Object->Info2 & 0x20) >> 3)+
                             ((Object->Info2 & 0x08) >> 2)+
                             ((Object->Info2 & 0x02) >> 1);
                  switch (CircleMode) {
              ...
              case SYMBOL_PINBUS_TEXT:
                ...
                for (cnt3=0;cnt3<NrLines;cnt3++) {
                  if (InRange(x2,1.0)) x2=0.9;
                  DXFWriteText(x3,y3,x2,TextRotation*90.0,TextAlignment,DXFLayer,fp,PinBusStrings[cnt3]);
                  switch (AddMode) {
                    case 0: // Rotation = 0 , MirrorY = 0 , MirrorX = 0
                    case 1: // Rotation = 0 , MirrorY = 0 , MirrorX = 1
                    case 2: // Rotation = 0 , MirrorY = 1 , MirrorX = 0
...

{
  ...
  while ((cnt<NrBlocks)
        &&
        (stricmp(BlockNames[cnt],BlockName)!=0)) cnt++;
...
{
  int32  FoundStr,BlockStart,fp,Found,res,result,ReadMode,FilePos,FilePos2,
         LineNr,Nesting,CommandCode,ValueInt,NrPolylinePoints,LineLength,cnt;
  double ValueDouble;
  char  NumberString[MAX_LENGTH_STRING],ValueStr[MAX_LENGTH_STRING],CurrentCommandString[MAX_LENGTH_STRING],
        ...
        LayerName[MAX_LENGTH_STRING],RegelBuf[4096],CopyStr[MAX_LENGTH_STRING];
        ...
        switch (CommandCode) {
          case 8:
            ...
            while ((cnt<NrDXFLayers)
                  &&
                  (stricmp(DXFLayers[cnt],ValueStr)!=0)) cnt++;
            if (cnt==NrDXFLayers) {
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SelectionEsc=0;
      SetDialogItemTextUTF8(Dialog,IDC_STATIC1     ,"Layers");
      SetDialogItemTextUTF8(Dialog,IDOK            ,"OK");
      SetDialogItemTextUTF8(Dialog,IDCANCEL        ,"Cancel");
      SetDialogItemTextUTF8(Dialog,IDHELP          ,"Help");
      SetWindowTextUTF8(Dialog,"Select DXF layers");
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDOK:
          NrSelectedLayers=SendDlgItemMessage(Dialog,IDC_LIST1,LB_GETSELITEMS,MAX_NR_LAYERS,(LPARAM)&SelectedLayers);
...
{
  int32  FoundStr,StartPoint,BlockStart,HeaderStart,
  ...
  int32 Found,res,TextAlignment,fp,
        PolyLineFlags,cnt,cnt2,LineLength,Units,LineNr,
  ...
  double ValueDouble,X[10],Y[10],Z[10],Width,Height,Thickness,Diameter,Angle,LengthLine,
         Length2,BlockAngles[5],BlockInsertScaleValueX,BlockInsertScaleValueY,dx,dy;
  char  NumberString[MAX_LENGTH_STRING],ValueStr[MAX_LENGTH_STRING],CurrentCommandString[MAX_LENGTH_STRING],AttributeName[MAX_LENGTH_STRING],
        LayerName[MAX_LENGTH_STRING],TextString[MAX_LENGTH_STRING],BlockName[MAX_LENGTH_STRING],
        LayerNameToCheck[MAX_LENGTH_STRING],FileName[MAX_LENGTH_STRING],
        str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],RegelBuf[4096],CopyStr[MAX_LENGTH_STRING],
...
//  Layer=INFO_LAYER2;
  ...
  if (ExportDir[0]==0) {
    strcpy(ExportDir,ExePath);
  }
  if (GetNewFileUTF8(SCHWindow,NULL,ImportDXFFile,ExportDir,"DXF file",NULL,"DXF files","dxf",0)!=0) return -1;
                  ...
                  if (VertexStartWidthTemp[cnt]!=0.0) {
                    if (BulgeFlags==1) {
                      if (InRangeSpecial(LengthLine,VertexStartWidthTemp[cnt],
                        ...
                        NewObjectCircle.CentreX=(float)((PointsX[cnt]+PointsX[cnt+1])*0.5);
                        NewObjectCircle.CentreY=(float)((PointsY[cnt]+PointsY[cnt+1])*0.5);
                        ...
                        if (LengthLine<0.01e6) { // Round pad
                          NewObjectCircle.CentreX=(float)(PointsX[cnt]+VertexStartWidthTemp[cnt]*0.5*cos(Angle));
                          NewObjectCircle.CentreY=(float)(PointsY[cnt]+VertexStartWidthTemp[cnt]*0.5*sin(Angle));
                          NewObjectCircle.Diam=(float)(VertexStartWidthTemp[cnt]*2.0);
                          NewObjectCircle.CircleMode=15;
                          AddObjectCircle(&NewObjectCircle);
                        } else {
...
*/
                    ...
                    NewObjectLine.X2=(float)PointsX[cnt+1];
                    NewObjectLine.Y2=(float)PointsY[cnt+1];
//                    NewObject.Thickness=VertexStartWidthTemp[cnt];
                    AddObjectLine(&NewObjectLine);
                  }
                }
              }
...
// ***************************************************************************************************
  ...
  // TextLineNr
          ...
          if ((stricmp(ValueStr,"INSERT")==0)
             ||
             (stricmp(ValueStr,"TEXT")==0)) {
  // TextLineNr
            BlockInsertScaleValueX=1.0;
            BlockInsertScaleValueY=1.0;
          }
