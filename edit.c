...

{
  ...
  if (OldObjectType==OBJECT_TEXT) OkToDraw=1;
  SpecialMode=Mode & 0x40;
  Mode&=0x3f;

  if (Mode>0) OkToDraw=1;
  ...
  switch (NewObject.ObjectType) {
    case OBJECT_LINE:
      if (OkToDraw) {
        if (GetDimensionTextFromLine(CurrentX2,CurrentY2,CurrentX,CurrentY,
                                     &NewObjectText2,NewObject.Info2)==0) {
    ...
    case OBJECT_RECT:
      if (Mode>0) {
        ...
        DrawLine(MultX(NewObject.x1)-3,MultY(NewObject.y1)-3,MultX(NewObject.x1)+3,MultY(NewObject.y1)+3);
        DrawLine(MultX(NewObject.x1)-3,MultY(NewObject.y1)+3,MultX(NewObject.x1)+3,MultY(NewObject.y1)-3);
      }
      break;
    case OBJECT_CIRCLE:
...

{
  ...
  if ((NewObject.ObjectType==OBJECT_RECT)
     &&
     (NewObject.Info2==1)) {
    ...
    if (AddObject(&NewObjectLine)) DrawObject(&NewObjectLine,0.0,0.0,0);
    NewObjectLine.x1=NewObject.x1-(float)(NewObject.x2*0.5);
    NewObjectLine.y1=NewObject.y1+(float)(NewObject.y2*0.5);
    NewObjectLine.x2=NewObject.x1+(float)(NewObject.x2*0.5);
    NewObjectLine.y2=NewObject.y1+(float)(NewObject.y2*0.5);
      ...
      case OBJECT_POLYGON:
        if (NewObject.Info2==0) {
          ...
          if (AddObjectPolygon(NewObjectPolygon)) {
            ObjectPolygon2=(ObjectPolygonRecord *)&(ObjectPolygonMem[(*ObjectPolygons)[NrObjectPolygons-1]]);
          ...
          if ((NewObjectPolygon->NrSubPolygons>0)
             &&
             (NewObjectPolygon->NrVerticesMainPolygon>0))  {
        ...
        break;
      case OBJECT_LINE:
        if (AddObject(&NewObject)) DrawObject(&NewObject,0.0,0.0,0);
        if ((NewObject.Info2!=0)
           &&
           (NewObjectText2.Text[0]!=0)) {
          if (AddObject(&NewObjectText2)) DrawObject(&NewObjectText2,0.0,0.0,0);
        }
        break;
      default:
        if ((NewObject.Layer==GEOM_NAME_LAYER)
           &&
...
{
  int32   cnt,cnt2,cnt3,Mode,NrParams,ok,InsertMode,Mode2,
          Found,count,result;
  double  OldX,OldY,CurrentX,CurrentY,NewX,NewY,FirstX,FirstY,
          CursorX,CursorY,ShiftOffsetX,ShiftOffsetY;
  float   Scale;
...
//  AppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_ESCAPE,SC(394,"Escape"));
...
*/
    ...
    case OBJECT_RECT:
      ...
      break;
    case OBJECT_TEXT:
      if (NewObject.x2==0.0) NewObject.x2=CurrentTextHeight;
      ...
      if (TextInputDialog(&NewObject,0)==2) return;
      if (strlen(NewObject.Text)==0) return;
      ...
      switch (NewObject.ObjectType) {
        ...
        case OBJECT_LINE:
          if (Mode>=1) {
            RelX=CurrentX2;
            RelY=CurrentY2;
            if ((NotInRange(NewObject.x1,NewObject.x2))
               ||
               (NotInRange(NewObject.y1,NewObject.y2))) {
        ...
        case OBJECT_ARC:
          ...
          if ((Mode==1)
             &&
             (((InRange(NewObject.x2,0.0))
             ||
             (InRange(NewObject.y2,0.0))))) {
            MessageBoxUTF8(GEOMWindow,SC(441,"Arc width or height is zero"),"Error",MB_APPLMODAL|MB_OK);
          } else {
            if (Mode!=5) Mode++;
            else Mode=2;
          }
...
//                     MousePosX+5,MousePosY+40,0,SCHWindow,NULL);
      ...
      if ((NewObject.ObjectType==OBJECT_LINE)
        ...
        PopUpMenu=CreatePopupMenu();
        if (NewObject.ObjectType==OBJECT_POLYGON) {
          AppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_FINISH_POLYGON,SC(183,"Finish"));
        }
        if ((SnapMode & 1) == 0) {
          AppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_CHANGE_SNAP_MODE_ON,SC(184,"Snap on"));
        } else {
          AppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_CHANGE_SNAP_MODE_OFF,SC(185,"Snap off"));
        }
        if (LinesAllDirection) {
          AppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_LINES_45_DIR,SC(186,"Draw with 45/90 degrees directions"));
        } else {
          AppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_LINES_ALL_DIR,SC(187,"Draw in all directions"));
        }
        if (NewObjectPolygon->NrVertices>0) {
          AppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_TRACE_BACKWARDS,SC(190,"Goto previous point"));
        }
      ...
      if (SpacePressed) {
        ...
        switch (NewObject.ObjectType) {
          ...
          case OBJECT_RECT:
            ...
            switch (Mode) {
              case 0:
                if (LineInputDialog(&TypeObject,SC(193,"Add rectangle (width,height,[CenterX,CenterY])"))==1) {
                  if ((NrParams=ScanParameters(-1,(char *)TypeObject.Text,0))>=0) {
                    if (NrParams==2) {
                      NewObject.x2=(float)ParamsFloat[0];
                      NewObject.y2=(float)ParamsFloat[1];
              ...
              case 0:
                switch (Layer) {
                  ...
                  case DRILL_UNPLATED_LAYER:
                    result=LineInputDialog(&TypeObject,SC(196,"Add drill hole (diameter,[CenterX,CenterY])"));
                    break;
                  default:
          ...
          case OBJECT_ARC:
            ...
            switch (Mode) {
              case 0:
                if (LineInputDialog(&TypeObject,SC(202,"Add arc (width,height), [CenterX,CenterY], [StartAngle,EndAngle])"))==1) {
                  if ((NrParams=ScanParameters(-1,(char *)TypeObject.Text,0))>=0) {
                    if (NrParams==2) {
                      NewObject.x2=(float)ParamsFloat[0];
                      NewObject.y2=(float)ParamsFloat[1];
          ...
          case OBJECT_POLYGON:
            ...
            TextP=(LPSTR)&Buf[0];
            *TextP=0;
            if (TextInputDialog2((LPSTR)&Buf[0],SC(206,"Add polygon points x1,x2,y2,x3,y3,x4,y4, .... )"),0)==1) {
              cnt=0;
              NewObjectPolygon->NrVertices=0;
              while ((*TextP!=0)
                    &&
                    (NewObjectPolygon->NrVertices<512)) {
                TextP2=TextP;
                ...
                if (*TextP!=0) {
                  ...
                  if (*TextP==',') TextP++;
                  if (*TextP=='\r') TextP++;
                  if (((NrParams=ScanParameters(-1,(char *)str,0))>=2)
                     &&
                     ((NrParams & 1) == 0)) {
...
//            DrawSelectedObjects(CurrentX,CurrentY,1);
    ...
    if (!Focused) {
      ...
      CheckInputMessages(0);
      ClipMouseCursor();
      if (!SelectionEsc) DrawObjects2(CurrentX-CentreSelectedX,CurrentY-CentreSelectedY,1);
    }
    if ((ZoomActive())
       &&
       (!SelectionEsc)) {
...
//      DisplayCursorPosition();
    ...
    if (!Focused) {
      ...
      CheckInputMessages(0);
      ClipMouseCursor();
      if (!SelectionEsc) DrawTryingObjectCross(CurrentX,CurrentY,Mode);
    }
    if ((ZoomActive())
       &&
       (!SelectionEsc)) {
...

{
  ...
  if (!OpenClipboard(GEOMWindow)) return -1;
  ...
  if (count+NrPolygonObjects==0) return -1;

  if ((GlobalClipBoardMem=GlobalAlloc(GHND|GMEM_DDESHARE,TotalMemSize+4096))==NULL) return -1;
...

{
  ...
  if (!OpenClipboard(GEOMWindow)) return -2;
...

{
  ...
  if (NrPadLayers>OldNrPadLayers) { // Increase nr layers
    for (cnt=0;cnt<TempNrObjects;cnt++) {
      Object=&((*Objects)[cnt]);
      if (Object->Layer==OldNrPadLayers-1) { // Old top layer
        Object->Layer=NrPadLayers-1;
      }
      if (Object->Layer==ROUTING_KEEPOUT_LAYER+OldNrPadLayers-1) { // Old routing keepout top layer
    ...
    for (cnt=0;cnt<TempNrObjectPolygons;cnt++) {
      ObjectPolygon=(ObjectPolygonRecord *)&(ObjectPolygonMem[(*ObjectPolygons)[cnt]]);
      if (ObjectPolygon->Layer==OldNrPadLayers-1) { // Old top layer
        ObjectPolygon->Layer=NrPadLayers-1;
      }
      if (ObjectPolygon->Layer==ROUTING_KEEPOUT_LAYER+OldNrPadLayers-1) { // Old routing keepout top layer
        ObjectPolygon->Layer=ROUTING_KEEPOUT_LAYER+NrPadLayers-1;
      }
    }
  } else {
    if (NrPadLayers<OldNrPadLayers) { // Decrease nr layers
      for (cnt=0;cnt<TempNrObjects;cnt++) {
        Object=&((*Objects)[cnt]);
        if ((Object->Layer>0)
           &&
           (Object->Layer<OldNrPadLayers-1)) {
          Object->Info=OBJECT_NOT_VISIBLE;
          Object->AddNr=0;
          Object->DeleteNr=0;
          if ((Object->Layer==1)
        ...
        if (Object->Layer==ROUTING_KEEPOUT_LAYER+OldNrPadLayers-1) { // Old top layer
          Object->Layer=ROUTING_KEEPOUT_LAYER+NrPadLayers-1;
        }
      }
      for (cnt=0;cnt<TempNrObjectPolygons;cnt++) {
        ObjectPolygon=(ObjectPolygonRecord *)&(ObjectPolygonMem[(*ObjectPolygons)[cnt]]);
        if ((ObjectPolygon->Layer>0)
           &&
           (ObjectPolygon->Layer<OldNrPadLayers-1)) {
        ...
        if (ObjectPolygon->Layer==ROUTING_KEEPOUT_LAYER+OldNrPadLayers-1) { // Old top layer
          ObjectPolygon->Layer=ROUTING_KEEPOUT_LAYER+NrPadLayers-1;
        }
      }
    }
...
{
  int32 cnt,cnt2,cnt3,cnt4,ok,fp,result,BytesPerLine,NrBitmapColors,
        MaxNrBitmapColors,ColorIndex,ColorPixelCount,NrCodedBytes,NrBytesBitmapData;
  ...
  int32  OldDrawWindowMinX,OldDrawWindowMinY,OldDrawWindowMaxX,OldDrawWindowMaxY,
         BitmapMemsize,BitPerPixel,NrPlanes,PixelsX,PixelsY;
...
//  Rectangle(OutputDisplay,0,0,PixelsX+1,PixelsY+1);
  ...
  DrawLine(-1,DrawWindowMaxY-Mult(-Yoffset)-1,
           4000,DrawWindowMaxY-Mult(-Yoffset)-1);
  DrawLine(Mult(-Xoffset)+DrawWindowMinX,-10,
           Mult(-Xoffset)+DrawWindowMinX,4000);
  if (ViewInsertionPoint) DrawInsertionPoint(0);
...
//  res=GetDIBits(OutputDisplay,ViewBitmap,0,1,TempMem,(BITMAPINFO *)BitmapInfo,DIB_RGB_COLORS);
  ...
  for (cnt=0;cnt<PixelsY;cnt++) {
    ...
    for (cnt2=0;cnt2<PixelsX;cnt2++) {
      cnt3=0;
      while ((cnt3<NrBitmapColors)
            &&
            (LinePixels[cnt2]!=BitmapColors[cnt3])) cnt3++;
      if (cnt3==NrBitmapColors) { // Add color
...

{
  ...
  if (TextInputDialog(&ChangedTextObject,6)!=1) return -1;
