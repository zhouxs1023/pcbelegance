...
typedef struct {
          int32 MemSize,ShapeNr,NrPins,CompMode,Info,AddNr,DeleteNr,
                Info2,Info3,Info4,Info5;
          double CompOriginX,CompOriginY,CompHeight,
          ...
          double CompNameOriginX,CompNameOriginY,CompNameHeight,CompNamePenThickNess,
...
//  if ((fp=FileOpenReadOnlyUTF8("c:\\viewplot\\blad1.bmp"))>0) {
  ...
  if (GetNewFileUTF8(PCBWindow,NULL,BitmapFileName,ExportDir,SC(374,"Bitmap file"),
                     NULL,SC(375,"Bitmap files"),"bmp",0)) {
    return -1;
  }

//  if (LoadNewFile3(BitmapFileName,SC(374,"Bitmap file"),SC(375,"Bitmap files"),"bmp",0)!=0) return -1;
...

{
  ...
  if (!OpenClipboard(PCBWindow)) return -2;
      ...
      if ((DrawPolygon->PolygonType & 2) == 2) {
        ...
        for (cnt2=0;cnt2<AreaFill->NrPolygons;cnt2++) {
          ...
          if (cnt2<AreaFill->NrPolygons-1) {
            sprintf(str,"    SubPolygon=(PolygonRecord *)((uint8 *)SubPolygon+MemSizePolygon(SubPolygon));\r\n");
            OutputDebugString(str);
          }
