...
//  NewObjectText.FontHeight=1.0;
  ...
  if (!EditingSheetSymbol) {
    if (AddPinsDialog(&AddedPins[0],0)==2) return;
  } else {
    if (AddPinsDialog(&AddedPins[0],2)==2) return;
  }
  if (NrAddPins==0) return;
...
//  while (LeftButtonPressed) CheckInputMessages(0) ;
  ...
  while (!SelectionEsc) {
    ...
    if (!Focused) {
      ...
      CheckInputMessages(0);
      ClipMouseCursor();
      if (!SelectionEsc) DrawTryingAddPins(CurrentX,CurrentY,Mode);
    }
    if ((ZoomActive())
       &&
       (!SelectionEsc)) {
...
//      RightButtonPressed=0;
    ...
    if (NrFunctionsInBuf>0) {
      ...
      else DrawTryingAddPins(CurrentX,CurrentY,Mode);
    }
  }
  UnClipMouseCursor();
...

{
  ...
  if (Mode==1) NewPowerPin.NameInfo=(int16)((NewPowerPin.NameInfo & 0x0f)+(1 << 8));
...

{
  ...
  if (AddPowerPinsDialog(&AddedPowerPin,0)==2) return;
...
//  NewObjectText.Text[31]=0;
  ...
  while (!SelectionEsc) {
    ...
    if (!Focused) {
      ...
      CheckInputMessages(0);
      ClipMouseCursor();
      if (!SelectionEsc) DrawTryingAddPowerPins(CurrentX,CurrentY,Mode);
    }
...
//      RightButtonPressed=0;
    ...
    if (NrFunctionsInBuf>0) {
      ...
      ClipMouseCursor();
      if (SelectionEsc) DisplayObjectOnEscape=0;
      else DrawTryingAddPowerPins(CurrentX,CurrentY,Mode);
    }
  }
  UnClipMouseCursor();
  if (DisplayObjectOnEscape) DrawTryingAddPowerPins(CurrentX,CurrentY,Mode);
  SystemBusyMode=0;
}
...
//  sprintf(&AddedPinBus.Label,"hallo");
  ...
  if (AddPinBusDialog(&AddedPinBus,0)==2) return;
//  TextInputDialog("Add Text",&ObjectTextLine);
//  if (strlen(&ObjectTextLine)==0) return;
//  memmove(&NewObjectText.Text,&ObjectTextLine,31);
//  NewObjectText.Text[31]=0;
  ...
  while (!SelectionEsc) {
    ...
    if (!Focused) {
      ...
      CheckInputMessages(0);
      ClipMouseCursor();
      if (!SelectionEsc) DrawTryingAddPinBus(CurrentX,CurrentY,Mode);
    }
    if ((ZoomActive())
       &&
       (!SelectionEsc)) {
...
//      RightButtonPressed=0;
    ...
    if (NrFunctionsInBuf>0) {
      ...
      ClipMouseCursor();
      if (SelectionEsc) DisplayObjectOnEscape=0;
      else DrawTryingAddPinBus(CurrentX,CurrentY,Mode);
    }
  }
  UnClipMouseCursor();
  if (DisplayObjectOnEscape) DrawTryingAddPinBus(CurrentX,CurrentY,Mode);
  SystemBusyMode=0;
}
...
//          Beep(1000,500);
    ...
    if ((ZoomActive())
      ...
      OldX=CurrentX;
      OldY=CurrentY;
      if (!SelectionEsc) DrawTryingInstance(CurrentX,CurrentY,Mode);
    }
    if ((PanActive())
       &&
       (!SelectionEsc)) {
...
//      RightButtonPressed=0;
    ...
    if (NrFunctionsInBuf>0) {
      ...
      ClipMouseCursor();
      if (SelectionEsc) DisplayObjectOnEscape=0;
      else DrawTryingInstance(CurrentX,CurrentY,Mode);
    }
  }
  UnClipMouseCursor();
...
{
  int32   res,result,Libfp,Symbolfp,cnt,NrLibEntries,SymbolPosition,SymbolLength,pos,SymbolNr,MemPos;
  LibRecord Lib;
  ...
  for (cnt=0;cnt<Design.NrSymbols;cnt++) {
    ...
    if (stricmp(SymbolPos->SymbolName,TestSymbolName)==0) SymbolNr=cnt;
  }
  if (SymbolNr!=-1) {
...
// *******************************************************************************
      ...
      while ((cnt<NrLibEntries)
            &&
            ((res=FileRead(Libfp,&LibEntry,sizeof(LibNameRecord),&result))==0)) {
...
//      strcat(SymbolFile,".sym");
      ...
      if ((strcmp(NewSymbol.SymbolIdent,SymbolCode1)!=0)
         &&
         (strcmp(NewSymbol.SymbolIdent,SymbolCode2)!=0)
         &&
...
// *******************************************************************************
  ...
  NewInstance.Info=0;
  if ((LoadSymbol->Info & 1) == 1) NewInstance.ValueInfo|=TEXT_NOT_VISIBLE;
  if ((LoadSymbol->Info & 2) == 2) NewInstance.RefInfo|=TEXT_NOT_VISIBLE;
  if (LoadSymbol->NrPartsPerPackage>1) NewInstance.PackagePartNr=1;
...
{
  int32  cnt,cnt2,res,InstanceInfo,TempLastActionNr,NrInstances=0,
         NrCompProperties,NrOldCompProperties,Changed=0,Changed2,pos,
         ChangedCompNetProperties,ChangedCompProperties;
...
//      memmove(&ChangedInstance,FirstInstance,sizeof(InstanceRecord));
      ...
      for (cnt=0;cnt<NrCompProperties;cnt++) {
        ...
        if (strcmp(PropertyID,"MULTI_ASSY")!=0) {
          if (PropertyID[0]!='~') {
            if ((strcmpUTF8(str2,PropertyID)==0)
               &&
               (strcmpUTF8(str2,PropertyValue)==0)) {
              ChangedCompProperties=0;
            }
          } else {
            if ((strcmpUTF8(str2,&PropertyID[1])==0)
               &&
               (strcmpUTF8(str2,PropertyValue)==0)) {
      ...
      if (res==1) {
        for (cnt=0;cnt<Design.NrInstances;cnt++) {
          Instance=(InstanceRecord *)&((*Instances)[cnt]);
          if (((Instance->Info & (OBJECT_NOT_VISIBLE|OBJECT_PROTECTED|SHEET_SYMBOL|OBJECT_SELECTED)) == OBJECT_SELECTED)
             &&
             (Instance->AddNr<=TempLastActionNr)) {
            Changed2=0;
            ...
            if (strcmpUTF8(ChangedInstance.PartDescription,str2)!=0) {
              memcpy(NewInstance.PartDescription,ChangedInstance.PartDescription,sizeof(ChangedInstance.PartDescription));
              Changed2=1;
            }
            pos=0;
            if ((ChangedCompProperties)
               ||
               (ChangedCompNetProperties)) {
              ...
              if ((ChangedCompProperties)
                ...
                for (cnt2=0;cnt2<NrOldCompProperties;cnt2++) {
                  ...
                  if (PropertyID[0]=='~') { // Copy old comp net properties
                    if (pos+strlen(PropertyID)+strlen(PropertyValue)+3<sizeof(ChangedInstance.Properties)) {
                ...
                for (cnt2=0;cnt2<NrCompProperties;cnt2++) {
                  ...
                  if (PropertyID[0]!='~') { // Copy new comp properties
                    if (pos+strlen(PropertyID)+strlen(PropertyValue)+3<sizeof(ChangedInstance.Properties)) {
                      memcpy(&NewInstance.Properties[pos],PropertyID,strlen(PropertyID));
                      pos+=strlen(PropertyID)+1;
                      memcpy(&NewInstance.Properties[pos],PropertyValue,strlen(PropertyValue));
              ...
              if ((!ChangedCompProperties)
                ...
                for (cnt2=0;cnt2<NrCompProperties;cnt2++) {
                  GetCompProperties(&ChangedInstance,PropertyID,PropertyValue,0x20+cnt2);
                  if (PropertyID[0]=='~') { // Copy new comp net properties
                    if (pos+strlen(PropertyID)+strlen(PropertyValue)+3<sizeof(ChangedInstance.Properties)) {
                      memcpy(&NewInstance.Properties[pos],PropertyID,strlen(PropertyID));
                      pos+=strlen(PropertyID)+1;
                      memcpy(&NewInstance.Properties[pos],PropertyValue,strlen(PropertyValue));
              ...
              if ((ChangedCompProperties)
                 &&
                 (ChangedCompNetProperties)) {
                memcpy(&NewInstance.Properties,&ChangedInstance.Properties,sizeof(ChangedInstance.Properties));
              }
              Changed2=1;
            }
...

{
  ...
  if (TextInputDialog(&NewObjectText,4)==2) return;
  ...
  while (!SelectionEsc) {
    ...
    if (!Focused) {
      ...
      CheckInputMessages(0);
      ClipMouseCursor();
      if (!SelectionEsc) DrawTryingGlobalConnection(CurrentX,CurrentY,Mode);
    }
    if ((ZoomActive())
       &&
       (!SelectionEsc)) {
...
//      RightButtonPressed=0;
    ...
    if (NrFunctionsInBuf>0) {
      ...
      ClipMouseCursor();
      if (SelectionEsc) DisplayObjectOnEscape=0;
      else DrawTryingGlobalConnection(CurrentX,CurrentY,Mode);
    }
  }
  UnClipMouseCursor();
  if (DisplayObjectOnEscape) DrawTryingGlobalConnection(CurrentX,CurrentY,Mode);
  SystemBusyMode=0;
}
...

{
  ...
  if (AddWireLabelsDialog()==2) return;
//  TextInputDialog("Add Text",&ObjectTextLine);
//  if (strlen(&ObjectTextLine)==0) return;
//  memmove(&NewObjectText.Text,&ObjectTextLine,31);
//  NewObjectText.Text[31]=0;
  ...
  while (!SelectionEsc) {
    ...
    if (!Focused) {
      ...
      CheckInputMessages(0);
      ClipMouseCursor();
      if (!SelectionEsc) DrawTryingWireLabels(CurrentX,CurrentY,Mode);
    }
    if ((ZoomActive())
       &&
       (!SelectionEsc)) {
...
//      CheckInputMessages(0);
    ...
    if (NrFunctionsInBuf>0) {
      ...
      ClipMouseCursor();
      if (SelectionEsc) DisplayObjectOnEscape=0;
      else DrawTryingWireLabels(CurrentX,CurrentY,Mode);
    }
  }
  UnClipMouseCursor();
  if (DisplayObjectOnEscape) DrawTryingWireLabels(CurrentX,CurrentY,Mode);
  SystemBusyMode=0;
}
...

{
  ...
  if (GetNewFileUTF8(SCHWindow,NULL,TextObjectsFile,DesignPath,"text file",NULL,"All files","*",0)!=0) return -1;
...

{
  ...
  char  SearchString2[512]="",str2[512],*strp,*strp2,
        *SearchStringP,*SrcP;
