...
{
  int32 res,cnt,cntx,cnty,Layer,cntx2,cnty2,ok,
        NrSectionsX,NrSectionsY,Found,NrSectionDots;
  double  MinX,MaxX,MinY,MaxY,StartX,StartY,DotSize,
          DotSection,DotDistance,MaxScreenX,MaxScreenY;
      ...
      for (cntx=0;cntx<NrSectionsX;cntx++) {
        ...
        for (cnty2=0;cnty2<NrSectionDots;cnty2++) {
          for (cntx2=0;cntx2<NrSectionDots;cntx2++) {
            ...
            for (cnt=0;cnt<NrObjects4;cnt++) {
              Object=&((*Objects4)[cnt]);
              if ((Object->maxx>=NewObject.minx)
                 &&
                 (Object->minx<=NewObject.maxx)
                 &&
                 (Object->maxy>=NewObject.miny)
                 &&
                 (Object->miny<=NewObject.maxy)) {
                continue;
              }
