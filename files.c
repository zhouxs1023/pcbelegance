...
#ifdef TEST
  sprintf(CopyStr,"TEST TEST TEST TEST TEST TEST TEST TEST TEST     %s     TEST TEST TEST TEST TEST TEST TEST TEST TEST",str);
  strcpy(str,CopyStr);
#endif
  if (FileChanged) strcat(str," *");
  if (SCHWindow!=NULL) {
...

{
  ...
  cnt=0;
  while ((cnt<NrUsedFiles)
        &&
        (stricmpUTF8(EditFile,UsedFiles[cnt])!=0)) cnt++;
  if (cnt==NrUsedFiles) {
    if (NrUsedFiles<16) {
  ...
  for (cnt=0;cnt<Design.NrInstances;cnt++) {
    ...
    if ((Instance->Info & OBJECT_NOT_VISIBLE) == 0) {
      ...
      for (cnt2=0;cnt2<Design.NrSymbols;cnt2++) {
        ...
        if (stricmpUTF8(SymbolPos->SymbolName,Instance->SymbolName)==0) SymbolNr=cnt;
      }
      if (SymbolNr==-1) {
          ...
          if (Length<1024*1024) {
            ...
            if ((Libfp=FileOpenReadOnlyUTF8(LibName))>0) {
              if ((Pos==-1)
                 ...
                 (FileSeek(Libfp,Pos)!=-1)) {
                LoadResult=1;
                if (FileRead(Libfp,Symbol,(int)Length,&result)==-1) LoadResult=0;
                if (result!=Length) LoadResult=0;
              } else {
                LoadResult=0;
              }
            }
            ...
            OutputDebugString(str);
#endif
            if (LoadResult==0) memset(Symbol,0,Length);
...

{
  ...
  NrAttributes=0;
  if ((strcmp(Symbol->SymbolIdent,SymbolCode2)!=0)
     &&
     (strcmp(Symbol->SymbolIdent,SymbolCode3)!=0)) return -1;
...

{
  ...
  if (AttributeIdent[0]==0) return -1;
  if (AttributeValue[0]==0) return -1;
...
{
  int32   BytesToRead,SymbolNr,MemPos,result,Designfp,cnt,cnt2,
          Version25,Version30,Version35,pos,DesignFileSize;
#ifdef _DEBUG
  int32   Time1,Time2;
  char  str[MAX_LENGTH_STRING];
#endif
  ...
  if (AllocateMemWires             (Design.NrWires*2               ,0)<0) return -2;
  if (AllocateMemBusses            (Design.NrBusses*2              ,0)<0) return -2;
  if (AllocateMemJunctions         (Design.NrJunctions*2           ,0)<0) return -2;
  if (AllocateMemOnePinNets        (Design.NrOnePinNets*2          ,0)<0) return -2;
  if (AllocateMemNetLabels         (Design.NrNetLabels*2           ,0)<0) return -2;
  if (AllocateMemBusConnections    (Design.NrBusConnections*2      ,0)<0) return -2;
  if (AllocateMemGlobalConnections (Design.NrGlobalConnections*2   ,0)<0) return -2;
  if (AllocateMemObjectLines       (Design.NrObjectLines*2         ,0)<0) return -2;
  if (AllocateMemObjectRects       (Design.NrObjectRects*2         ,0)<0) return -2;
  if (AllocateMemObjectCircles     (Design.NrObjectCircles*2       ,0)<0) return -2;
  if (AllocateMemObjectArcs        (Design.NrObjectArcs*2          ,0)<0) return -2;
  if (AllocateMemObjectTexts       (Design.NrObjectTexts*2         ,0)<0) return -2;
  if (AllocateMemInstances         (Design.NrInstances*2           ,0)<0) return -2;
  if (AllocateMemSheetSymbolsPos   (Design.NrSymbols*2             ,0)<0) return -2;
  if (AllocateMemSheetSymbols      (DefMaxSymbolsMemory            ,0)<0) return -2;
  if (AllocateMemRedefinedPinBusses(Design.NrRedefinedPinBusses    ,0)<0) return -2;
...
// ********************************************************************************************************
  ...
  if (pos==DesignFileSize) {
    if (Design.NrInstances>0) {
      MessageBoxUTF8(SCHWindow,EditFile,SC(515,"File is corrupt"),MB_APPLMODAL|MB_OK);
      memset(&Design,0,sizeof(DesignRecord));
      return -1;
    }
  }
...
// ********************************************************************************************************
  ...
  for (cnt=0;cnt<Design.NrInstances;cnt++) {
    ...
    for (cnt2=0;cnt2<Design.NrSymbols;cnt2++) {
      SymbolPos=&((*SymbolsPos)[cnt2]);
      if (stricmpUTF8(SymbolPos->SymbolName,Instance->SymbolName)==0) SymbolNr=cnt2;
    }
    if (SymbolNr!=-1) {
      ...
      if ((Instance->Geometry[0]==0)
         &&
         (stricmp(Symbol->SymbolIdent,SymbolCode3)==0)) {
  ...
  sprintf(str,"Load %s time1 = %d msec, time2 = %d msec, time3 = %d msec\n",
          EditFile,Time1,Time2,GetDifferenceTimer1inMilliSeconds());
  OutputDebugString(str);
#endif
  if (LoadObjects5()!=0) {
    ChangeViewMode=0;
...
// findfirst
  ...
  while ((!Found)
    ...
    if ((Libfp=FileOpenReadOnlyUTF8(FileStr))==-1) return -1;
...

{
  ...
  if (LengthSearchString==0) return 0;
  if (strcmp(SearchString,"*")==0) return 1;
  ...
  cnt=0;
  while ((cnt<LengthSearchString)
        &&
        (SearchString2[cnt]=='?')) cnt++;
  cnt2=LengthSearchString-1;
  while ((cnt2>0)
        &&
        (SearchString2[cnt2]=='?')) cnt2--;
...

{
  ...
  if ((Designfp=FileOpenReadOnlyUTF8(EditFile))==-1) return -1;
  ...
  if (AllocateMemPins         (DesignSymbol.NrPins            ,0)<0) return -2;
  if (AllocateMemPowerPins    (DesignSymbol.NrPowerPins       ,0)<0) return -2;
  if (AllocateMemPinBusses    (DesignSymbol.NrPinBusses       ,0)<0) return -2;
  if (AllocateMemSubPinDefs   (DesignSymbol.NrSubPinDefs      ,0)<0) return -2;
  if (AllocateMemObjectLines  (DesignSymbol.NrObjectLines*2   ,0)<0) return -2;
  if (AllocateMemObjectRects  (DesignSymbol.NrObjectRects*2   ,0)<0) return -2;
  if (AllocateMemObjectCircles(DesignSymbol.NrObjectCircles*2 ,0)<0) return -2;
  if (AllocateMemObjectArcs   (DesignSymbol.NrObjectArcs*2    ,0)<0) return -2;
  if (AllocateMemObjectTexts  (DesignSymbol.NrObjectTexts*2   ,0)<0) return -2;
  if (AllocateMemInstances(16,0)<0) return -2;
  ...
  if ((DesignSymbol.Info & 1) == 1) Instance->ValueInfo|=TEXT_NOT_VISIBLE;
  if ((DesignSymbol.Info & 2) == 2) Instance->RefInfo|=TEXT_NOT_VISIBLE;
...

{
  ...
  for (cnt=0;cnt<Design.NrInstances;cnt++) {
    Instance=(InstanceRecord *)&((*Instances)[cnt]);
    if ((Instance->Info & OBJECT_NOT_VISIBLE) == 0) {
      if (stricmpUTF8(Instance->SymbolName,SymbolName)==0) return 1;
    }
  }
  return 0;
}
...
{
  int32 result,cnt,cnt2,res,MemSize,SymbolCount,ObjectMirrorX,ObjectMirrorY,
        cnt3,cnt4,ok,TextRotation,Alignment,Rotation,NrPins,
        Zero=0,OnlyPowerPins=1,LoadResult,NrSubPinDefsLines,
        Libfp=0,fp2,Pos,Length,res2;
  ...
  struct  tm                   *today;
  time_t                       ltime;
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],LibName[MAX_LENGTH_STRING],
        SymbolFile[MAX_LENGTH_STRING],buf[1024];
  uint8 ZeroByte=0;
  Instance=NULL;
  ...
  strcpy(str,EditFile);
  if ((mode & 1)==0) res=MakeBackup(EditFile);
    ...
    for (cnt=0;cnt<Design.NrInstances;cnt++) {
      ...
      if ((Instance->Info & OBJECT_NOT_VISIBLE) == 0) {
        for (cnt2=cnt;cnt2<Design.NrInstances;cnt2++) {
          ...
          if ((cnt!=cnt2)
             &&
             ((Instance2->Info & OBJECT_NOT_VISIBLE) == 0)) {
            if ((stricmpUTF8(Instance2->SymbolName,Instance->SymbolName)==0)
              ...
              sprintf(str,SC(286,"Two or more symbols %s overlaps ( %.1f,%.1f )"),
                          Instance->SymbolName,Instance->OriginX,Instance->OriginY);
...
// ******************************************************************************************
    ...
    for (cnt=0;cnt<Design.NrSymbols;cnt++) {
      ...
      if ((CheckSymbolNameUsed(SymbolPos->SymbolName,0))
         &&
         (SymbolPos->Length>0)) {
...
// ******************************************************************************************
    ...
    for (cnt=0;cnt<Design.NrBusConnections;cnt++) {
      ...
      } else NewDesign.NrBusConnections--;
    }
    ...
    for (cnt=0;cnt<Design.NrGlobalConnections;cnt++) {
      ...
      } else NewDesign.NrGlobalConnections--;
    }
  } else {
...
// ******************************************************************************************
    ...
    if (EditingSheetSymbol) NewDesignSymbol.Info|=SHEET_SYMBOL;
...
// ******************************************************************************************
    ...
    for (cnt=0;cnt<DesignSymbol.NrPowerPins;cnt++) {
      ...
      } else NewDesignSymbol.NrPowerPins--;
    }
    for (cnt=0;cnt<DesignSymbol.NrPinBusses;cnt++) {
      ...
      } else NewDesignSymbol.NrPinBusses--;
    }
  }
...
// ******************************************************************************************
  ...
  if (!EditingSymbol) {
    for (cnt=0;cnt<Design.NrRedefinedPinBusses;cnt++) {
      ...
      } else NewDesign.NrRedefinedPinBusses--;
    }
    ...
    if ((Instance->Reference[0]==0)
       ...
       &&
       (!EditingSheetSymbol)) {
      if (MessageBoxUTF8(SCHWindow,SC(287,"Symbol does not have a reference indication\n\nDo you want to add this reference ?"),SC(288,"Warning"),MB_YESNO)==IDYES) {
    ...
    for (cnt=0;cnt<MaxNrSymbolAttributes-cnt2;cnt++) {
      if ((SymbolAttributesIdent[cnt][0]!=0)
        ...
        if ((strcmp(SymbolAttributesIdent[cnt],"GEOMETRY")!=0)
          ...
          FileWrite(Designfp,&SymbolAttributesIdent[cnt],
                    strlen((LPSTR)&SymbolAttributesIdent[cnt])+1,&result);
          FileWrite(Designfp,&SymbolAttributesValue[cnt],
                    strlen((LPSTR)&SymbolAttributesValue[cnt])+1,&result);
        }
      }
    }
...

{
  ...
  if (FileChanged) {
    res=MessageBoxUTF8(SCHWindow,SC(289,"Schematic/symbol has changed, save ?"),"",MB_YESNOCANCEL);
    if (res==IDYES) {
      res=SaveFile(16);
      if (res==-1) {
...

{
  ...
  for (cnt=0;cnt<MaxNrInstances;cnt++) {
    ...
    if (!EditingSymbol) {
      ...
      if ((fabs(Instance->OriginX)>90000.0)
         ||
         (fabs(Instance->OriginY)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrWires;cnt++) {
    ...
    Wire->AddNr=0;
    Wire->DeleteNr=0;
    if ((fabs(Wire->X1)>90000.0)
       ||
       (fabs(Wire->X2)>90000.0)
       ||
       (fabs(Wire->Y1)>90000.0)
       ||
  ...
  for (cnt=0;cnt<MaxNrJunctions;cnt++) {
    ...
    Junction->AddNr=0;
    Junction->DeleteNr=0;
    if ((fabs(Junction->X)>90000.0)
       ||
       (fabs(Junction->Y)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrOnePinNets;cnt++) {
    ...
    if ((fabs(OnePinNet->X)>90000.0)
       ||
       (fabs(OnePinNet->Y)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrBusConnections;cnt++) {
    ...
    if ((fabs(BusConnection->X)>90000.0)
       ||
       (fabs(BusConnection->Y)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrGlobalConnections;cnt++) {
    ...
    if ((fabs(GlobalConnection->X)>90000.0)
       ||
       (fabs(GlobalConnection->Y)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrNetLabels;cnt++) {
    ...
    NetLabel->AddNr=0;
    NetLabel->DeleteNr=0;
    if ((fabs(NetLabel->ConnectX)>90000.0)
       ||
       (fabs(NetLabel->ConnectY)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrObjectLines;cnt++) {
    ...
    if ((fabs(ObjectLine->X1)>90000.0)
       ||
       (fabs(ObjectLine->X2)>90000.0)
       ||
       (fabs(ObjectLine->Y1)>90000.0)
       ||
  ...
  for (cnt=0;cnt<MaxNrObjectRects;cnt++) {
    ...
    if ((fabs(ObjectRect->CentreX)>90000.0)
       ||
       (fabs(ObjectRect->CentreY)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrObjectCircles;cnt++) {
    ...
    if ((fabs(ObjectCircle->CentreX)>90000.0)
       ||
       (fabs(ObjectCircle->CentreY)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrObjectArcs;cnt++) {
    ...
    if ((fabs(ObjectArc->CentreX)>90000.0)
       ||
       (fabs(ObjectArc->CentreY)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrObjectTexts;cnt++) {
    ...
    if ((fabs(ObjectText->X)>90000.0)
       ||
       (fabs(ObjectText->Y)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrPowerPins;cnt++) {
    ...
    PowerPin->AddNr=0;
    PowerPin->DeleteNr=0;
    if ((fabs(PowerPin->NameX)>90000.0)
       ||
       (fabs(PowerPin->NameY)>90000.0)) {
  ...
  for (cnt=0;cnt<MaxNrPinBusses;cnt++) {
    ...
    PinBus->AddNr=0;
    PinBus->DeleteNr=0;
    if ((fabs(PinBus->X)>90000.0)
       ||
       (fabs(PinBus->Y)>90000.0)) {
...

{
  ...
  if (EditFile[0]==0) return -1;
  lengte=strlen(EditFile);
  if (lengte<5) return -1;
  mode=-1;
  if (stricmp((LPSTR)&EditFile[lengte-3],"sym")==0) mode=0;
  if (stricmp((LPSTR)&EditFile[lengte-3],"sch")==0) mode=1;
  if (mode==-1) return -1;
  ...
  lengte2=strlen(str);
  if (mode==0) {
    if (stricmp((LPSTR)&str[lengte2-4],"\\sym")!=0) return -1;
  } else {
    if (stricmp((LPSTR)&str[lengte2-4],"\\sch")!=0) return -1;
  }
...

{
  ...
  if (FileExistsUTF8(Object5File)!=0) return -1;
  if ((lengte2=FileSizeUTF8(Object5File))<1) return -2;
  ...
  if (AllocateMemObjects5(count)!=0) return -4;

  if ((fp=FileOpenReadOnlyUTF8(Object5File))==-1) return -5;
...
*/
  ...
  if (lengte>4) {
    if (stricmp((LPSTR)&FileName2[lengte-3],"sym")==0) EditingMode=1;
    if (stricmp((LPSTR)&FileName2[lengte-3],"sch")==0) EditingMode=0;
  }
  CheckResult=-1;
  if (EditingMode!=-1) {
  ...
  if (EditFile[0]!=0) {
    EditingSheetSymbol=0;
    if (EditingMode==0) EditingSymbol=0;
    else EditingSymbol=1;
    IniFile[0]=0;
...
//    MessageBox(NULL,DesignPath,"DesignPath",MB_APPLMODAL+MB_OK);
  ...
  if (((OldIniFile[0]==0)
     ||
     (stricmpUTF8(OldIniFile,IniFile)!=0))
     &&
     (IniFile!=NULL)
     &&
     (IniFile[0]!=0)
...

{
  ...
  char  str[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],str5[MAX_LENGTH_STRING],NewEditFile[MAX_LENGTH_STRING],
        EditFile2[MAX_LENGTH_STRING],FileName[MAX_LENGTH_STRING],
  ...
  strcpy(str5,SC(296,"Schematic/symbol files"));
  strcat(str5," (*.sch;*.sym)");
  length=strlen(str5);
    ...
    if (mode==0) {
      ...
      if (ProjectActive) {
        ...
        while ((cnt<32)
              ...
              ||
              (stricmp(ProjectInfo->FileNames[cnt],NewEditFile)!=0))) cnt++;
        if (cnt<32) {
      ...
      sprintf(ExeParams,"\"%s\" \"%s\" /a /e \"%s\" /u \"%s\"",
              ExeFile,NewEditFile,ExePath,ProjectPath);
      if (ProjectActive) {
        strcat(FileName," /o");
      }
...

{
  ...
  char  str[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],
        str5[MAX_LENGTH_STRING],FileName[MAX_LENGTH_STRING],DefExt[10];
  LPSTR Niks;
  ...
  str[0]=0;
  strcat(str,EditFile);
  if (EditingSymbol) strcpy(DefExt,"sym");
  else strcpy(DefExt,"sch");
  ...
  if (!EditingSymbol) {
    ...
    sprintf(str4,SC(298,"Save as [%s]"),str3);
    FileInputInfo.lpstrTitle=(LPSTR)&str4;
    strcpy(str,"*.sch");
    FileInputInfo.lpstrFile=str;
  } else {
    memset(str5,0,sizeof(str5));
    strcpy(str5,SC(299,"Symbol files"));
    strcat(str5," (*.sym)");
    length=strlen(str5);
  ...
  if (GetSaveFileName(&FileInputInfo)) {
    if (GetFullPathName(str,150,EditFile,&Niks)>0) {
      if (EditingSymbol) {
        ...
        memmove(Instance->Value,&FileName,
                min(sizeof(Instance->Value)-1,strlen(FileName)));
