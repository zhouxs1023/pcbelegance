...
char      PDFCreatorOrganisation[MAX_LENGTH_STRING],PDFCreatorName[MAX_LENGTH_STRING],
          PDFSubject[MAX_LENGTH_STRING],PDFTitle[MAX_LENGTH_STRING];
...

{
  ...
  if ((Designfp=FileOpenReadOnlyUTF8(FileName))==-1) return -1;
  if (FileRead(Designfp,&Design,sizeof(DesignRecord),&result)==-1) {
    FileClose(Designfp);
    return -1;
  }
  FileClose(Designfp);
  if (strcmp((LPSTR)&Design,SheetCode1)!=0) return -2;
  return 0;
}
...
{
  int32     result,Designfp,cnt,cnt2,cnt3,cnt4,Libfp,MemPos,cnt5,
            LoadResult,ok,Pos,Length,TotalSymbolsMem,FilePos,MemPos2,
            NrErrors,MemPosTemp,MaxMem,SymbolNr,NewSymbolNr,
            pos,pos2,LengthIdent,LengthValue,
            Version25,Version30,Version35;
  char      SymbolName[MAX_LENGTH_STRING],LibName[MAX_LENGTH_STRING],
            str[MAX_LENGTH_STRING],FileName[MAX_LENGTH_STRING];
...
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
  ...
  if ((stricmp(Design.Identification,SheetCode1)!=0)
     &&
     (stricmp(Design.Identification,SheetCode2)!=0)
     &&
     (stricmp(Design.Identification,SheetCode3)!=0)
     &&
...
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
  ...
  if (MaxMem>DesignBufMemSize) {
    if (AllocateMemDesignBuf(MaxMem)==-1) return -1;
  }
...
//      FileRead(Designfp,&DesignBuf[MemPos],sizeof(OldInstanceRecord),&result);
//      MemPos+=sizeof(OldInstanceRecord);
//      memset(&DesignBuf[MemPos],0,sizeof(Instance->Properties));
//      MemPos+=sizeof(Instance->Properties);
      MemPos2=MemPos;
      ...
      FileRead(Designfp,&DesignBuf[MemPos2],sizeof(OldOldInstanceRecord)-offsetof(OldOldInstanceRecord,Geometry),&result);
...
//        MemPos+=sizeof(Instance->Properties);
        ...
        MemPos2+=offsetof(InstanceRecord,Geometry);
        FileRead(Designfp,&DesignBuf[MemPos2],sizeof(OldInstanceRecord)-offsetof(OldInstanceRecord,Geometry),&result);
        MemPos+=sizeof(InstanceRecord);
      }
    } else {
...
// 3 = Local sheet symbol
      if (LoadResult>=0) {
        ...
        if ((Libfp=FileOpenReadOnlyUTF8(LibName))>=0) {
          if ((Pos==-1)
             ||
             (FileSeek(Libfp,Pos)!=-1)) {
            Symbol=(SymbolRecord *) &((*SymbolsMem)[TotalSymbolsMem]);
            if (FileRead(Libfp,Symbol,(int32)Length,&result)==-1) LoadResult=-1;
            if (result!=Length) LoadResult=-1;
          }
          FileClose(Libfp);
...
// *******************************************************************************************************
    ...
    if (((mode & 4) == 4)
       &&
       ((Instance->Info & (OBJECT_NOT_VISIBLE)) == 0)
       &&
...
//         (stricmp(Instance->SymbolName,"POWER_CONNECT")!=0)
        ...
        if ((NrObjects4>=MaxNrObjects4)
           &&
           (AllocateMemObjects4(MaxNrObjects4+1024)==-1)) MemError=1;
        if (!MemError) {
          ...
          pos=0;
          while ((pos<sizeof(Instance->Properties)-2)
                &&
                (InstanceAttrBuf[pos]!=0)) {
      ...
      for (cnt2=0;cnt2<NrObjects;cnt2++) {
        ...
        if (Unique) {
          if ((NrObjects3>=MaxNrObjects3)
             &&
             (AllocateMemObjects3(MaxNrObjects3+128)==-1)) MemError=1;
          if (!MemError) {
...
// *******************************************************************************************************
    ...
    if (((mode & 8) == 8)
       ...
       (SymbolNr>=0)
       &&
       (Instance->Info & (SHEET_SYMBOL|OBJECT_PROTECTED)) == 0) {
      NewSymbolNr=-1;
      for (cnt3=0;cnt3<NrNewSymbols;cnt3++) {
        NewSymbolPos2=&((*NewSymbolsPos2)[cnt3]);
        if (stricmpUTF8(NewSymbolPos2->SymbolName,Instance->SymbolName)==0) NewSymbolNr=cnt3;
      }
      if (NewSymbolNr==-1) {
        ...
        if (NewSymbolsMemSize+Length>MaxNewSymbolsMemSize) {
          if (AllocateMemNewSymbols(NewSymbolsMemSize+Length+65536)==-1) return -1;
        }
...

{
  ...
  SymbolRecord Symbol;
  char   FileStr[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],
  ...
  if (FileExistsUTF8(str)==0) {
    if ((Symbolfp=FileOpenReadOnlyUTF8(str))==-1) return -1;
    if (FileRead(Symbolfp,&Symbol,sizeof(SymbolRecord),&result)==-1) return -1;
    if (FileClose(Symbolfp)==-1) return -1;
    strcpy(FileName,str);
    *Pos=-1;
    *Length=FileSizeUTF8(str);
    if ((mode & 1)==0) {
      if ((Symbol.Info & SHEET_SYMBOL) == SHEET_SYMBOL ) return 3;
      else return 2;
    } else {
      if ((Symbol.Info & SHEET_SYMBOL) == 0) return 2;
    }
  }
...
// Search own libraries
  ...
  while ((!Found)
        &&
        (cnt<NrSchematicSymbolLibraries)) {
    ...
    if (strcmp(Lib.Identification,LibraryCode1)==0) {
      ...
      AllocateSpecialMem(MEM_LIBNAMES,min(NrLibEntries,4096)*sizeof(LibNameRecord),
                         (void *)&LibNames);
...

{
  ...
  if (stricmp(&FileName[lengte-3],"sym")!=0) return;
...

{
  ...
  if (DesignFile[0]==0) {
    ...
    result=GetNewFileUTF8(DESIGNWindow,NULL,DesignFile,DesignPath,SC(27,"Design files"),
                          NULL,SC(27,"Design files"),"dsn",0);
    if (result!=0) {
      return -1;
    }
...
{
  char  DirPart[MAX_LENGTH_STRING],FilePart[MAX_LENGTH_STRING],Extension[20],
        BackupFile1[MAX_LENGTH_STRING],BackupFile2[MAX_LENGTH_STRING];

  if ((GetDirFromFileName(DirPart,CurrentFile)==0)
     &&
     (GetFilePartFromFileName(FilePart,CurrentFile)==0)) {
...

{
  ...
  char   LineBuf[MAX_LENGTH_STRING],str1[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
  ...
  if ((fp=TextFileOpenUTF8(str1))<0) return;
  ParamMode=0;
  while ((Length=ReadLnWithMaxLength(fp,LineBuf,MAX_LENGTH_STRING-50))>=0) {
    ...
    if ((Length>1)
      ...
      if (str1[0]=='[') {
        ParamMode=0;
        if (stricmp(str1,"[SymbolDirs]")==0) ParamMode=1;
        if (stricmp(str1,"[Printer]")==0) ParamMode=2;
      } else {
        switch (ParamMode) {
          case 1:
            if (mode==0) {
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      if (DialogMode==0) {
        SetWindowTextUTF8(Dialog,SC(35,"Component reference"));
      } else {
        SetWindowTextUTF8(Dialog,SC(272,"Component partnr"));
      }
    ...
    case WM_COMMAND:
      switch (WParam) {
        case IDOK:
          TextChanged=0;
          if ((res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT1,WM_GETTEXT,MAX_LENGTH_STRING-1,(LPARAM)DialogTextLine))==0) {
...

{
  ...
  DialogMode=Mode;
  res=DialogBox(DESIGNClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_TEXTINPUT), DESIGNWindow, (DLGPROC)TextInputDialog2);
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemTextUTF8(Dialog,IDC_STATIC1   ,SC(39,"Design directory"));
      SetDialogItemTextUTF8(Dialog,IDC_STATIC2   ,SC(40,"Design name"));
      SetDialogItemTextUTF8(Dialog,IDC_STATIC3   ,SC(41,"Top sheet name"));
      SetDialogItemTextUTF8(Dialog,IDC_STATIC4   ,SC(42,"User symbol libraries"));
      SetDialogItemTextUTF8(Dialog,IDC_STATIC5   ,SC(43,"User geometry libraries"));
...
*/
    ...
    case WM_COMMAND:
      switch (WParam) {
        case IDOK:
          if ((res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT2,WM_GETTEXT,MAX_LENGTH_STRING-50,(LPARAM)DialogTextLine))!=0) {
            ...
            if (SetCurrentDirectoryUTF8(DesignPath)) {
              if (MessageBoxUTF8(DESIGNWindow,DesignPath,SC(46,"Directory exists, Ok to overwrite"),MB_APPLMODAL|MB_OKCANCEL)==IDCANCEL) {
                return about;
              }
            }
          }
          if ((res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT5,WM_GETTEXT,MAX_LENGTH_STRING-50,(LPARAM)DialogTextLine))!=0) {
            ...
            *Lengte=MAX_LENGTH_STRING-50;
            if ((res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT6,EM_GETLINE,cnt,(LPARAM)DialogTextLine))!=0) {
              if (NrSchematicSymbolLibraries<16) {
                ...
                strncpy(SchematicSymbolLibraries[NrSchematicSymbolLibraries++],DialogTextLine,min(res,MAX_LENGTH_STRING-1));
              }
            }
          }
            ...
            *Lengte=MAX_LENGTH_STRING-50;
            if ((res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT7,EM_GETLINE,cnt,(LPARAM)DialogTextLine))!=0) {
              if (NrGeometryLibraries<16) {
                ...
                strncpy(GeometryLibraries[NrGeometryLibraries++],DialogTextLine,min(res,MAX_LENGTH_STRING-1));
              }
            }
          }
          EndDialog(Dialog, 1);
...

{
  ...
  DialogMode=Mode;
  res=DialogBox(DESIGNClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_NEW_PROJECT), DESIGNWindow, (DLGPROC)NewProjectDialog2);
  return res;
}
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SetWindowTextUTF8(Dialog,SC(38,"Project settings"));
      SetDialogItemTextUTF8(Dialog,IDC_STATIC1  ,SC(250,"Layout name"));
      ...
      SetDialogItemTextUTF8(Dialog,IDC_STATIC8  ,SC(43,"User geometry libraries"));
      SetDialogItemTextUTF8(Dialog,IDC_CHECK1   ,SC(44,"Use part numbers"));
      SetDialogItemTextUTF8(Dialog,IDC_CHECK2   ,SC(217,"Disable one pin net check"));
...
*/
    ...
    case WM_COMMAND:
      switch (WParam) {
        case IDOK:
          LayoutFile[0]=0;
          if ((res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT1,WM_GETTEXT,MAX_LENGTH_STRING-50,(LPARAM)DialogTextLine))!=0) {
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SetWindowTextUTF8(Dialog,SC(52,"Bill of materials"));
      SetDialogItemTextUTF8(Dialog,IDC_BUTTON1   ,SC(53,"List of components"));
      SetDialogItemTextUTF8(Dialog,IDCANCEL      ,SC(37,"Cancel"));
      SetDialogItemTextUTF8(Dialog,IDHELP        ,SC(45,"Help"));
      SetDialogItemTextUTF8(Dialog,IDC_STATIC1   ,SC(221,"Columns"));
      SetDialogItemTextUTF8(Dialog,IDC_STATIC2   ,SC(222,"Columns exported file component.tdl"));
      ...
      while (cnt<BOMInfo.NrColumnsUsed) {
        ...
        if (BOMInfo.ColumnStr[cnt2][0]==0) {
          if (BOMInfo.NrColumnsUsed-cnt>1) {
            memmove(&BOMInfo.ColumnsUsed[cnt],&BOMInfo.ColumnsUsed[cnt+1],(BOMInfo.NrColumnsUsed-cnt-1)*4);
          }
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SetWindowTextUTF8(Dialog,SC(56,"Annotate"));
...
//  DialogMode=Mode;
  ...
  res=DialogBox(DESIGNClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_ANNOTATION), DESIGNWindow, (DLGPROC)AnnotationDialog2);
  if (res!=-1) {
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SetWindowTextUTF8(Dialog,SC(62,"About design manager"));
      SetDialogItemTextUTF8(Dialog,IDC_STATIC1  ,SC(63,"Design manager PCB Elegance"));
      SetDialogItemTextUTF8(Dialog,IDOK         ,SC(36,"OK"));

      SelectionEsc=0;
      sprintf(str,SC(65,"Build version %i.%i.%i  [ %s ]"),PROGRAM_VERSION/100,PROGRAM_VERSION % 100,BUILD_VERSION,TIME_STRING);
#ifdef GCC_COMP
...

{
  ...
  if (mode==0) {
    return GetNewFileUTF8(DESIGNWindow,NULL,OrcadFile,OrcadDesignPath,SC(67,"ORCAD schematic files"),
                          SC(211,"All files"),SC(67,"ORCAD schematic files"),"sch",0);
  } else {
    return GetNewFileUTF8(DESIGNWindow,NULL,OrcadFile,OrcadDesignPath,SC(68,"ORCAD library files"),
                          SC(211,"All files"),SC(68,"ORCAD library files"),"lib",0);
  }
...
/*
  ...
  FileInputInfo.hwndOwner=DESIGNWindow;
  str[0]=0;
  FileInputInfo.lpstrFile=str;
  FileInputInfo.lpstrFileTitle=NULL;
  FileInputInfo.lpstrTitle=NULL;
  memset(str3,0,sizeof(str3));
  if (mode==0) {
    FileInputInfo.lpstrDefExt="sch";
    strcpy(str3,SC(67,"ORCAD schematic files"));
    strcat(str3," (*.sch)");
    length=strlen(str3);
    strcpy(&str3[length+1],"*.sch");
    length+=strlen("*.sch")+1;
    strcpy(&str3[length+1],SC(211,"All files (*.*)"));
    length+=strlen(SC(211,"All files (*.*)"))+1;
    strcpy(&str3[length+1],"*.*");

//    FileInputInfo.lpstrFilter=SC(67,"ORCAD schematic files (*.sch)\0*.sch\0All files (*.*)\0*.*\0\0");
  } else {
    FileInputInfo.lpstrDefExt="lib";
    strcpy(str3,SC(68,"ORCAD library files"));
    strcat(str3," (*.lib)");
    length=strlen(str3);
    strcpy(&str3[length+1],"*.lib");

//    FileInputInfo.lpstrFilter=SC(68,"ORCAD library files (*.lib)\0*.lib\0\0");
  }
  FileInputInfo.lpstrFilter=str3;
  FileInputInfo.lpstrInitialDir=OrcadDesignPath;
  FileInputInfo.Flags=OFN_FILEMUSTEXIST|OFN_LONGNAMES|OFN_HIDEREADONLY;
  FileInputInfo.nMaxFile=200;
  FileInputInfo.nMaxFileTitle=32;

  if ( GetOpenFileName(&FileInputInfo) ) {
    strcpy(OrcadFile,FileInputInfo.lpstrFile);
    GetDirFromFileName(OrcadDesignPath,OrcadFile);
    res=1;
  } else {
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      if (DialogMode==0) {
        SetWindowTextUTF8(Dialog,SC(69,"Conversion ORCAD schematic file"));
        ...
        SetWindowTextUTF8(Dialog,SC(74,"Conversion ORCAD library file"));
      ...
      SendDlgItemMessageUTF8(Dialog,IDC_EDIT3,WM_SETTEXT,0,(LPARAM)OrcadConversionInfo.GeometryConversionFileName);
      for (cnt=0;cnt<9;cnt++) {
        if (cnt==0) {
    ...
    case WM_COMMAND:
      switch (WParam) {
        case IDOK:
          if ((res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT1,WM_GETTEXT,(WPARAM)180,
                                          (LPARAM)OrcadConversionInfo.OrcadFile))==0) {
            MessageBoxUTF8(DESIGNWindow,"",SC(78,"The ORCAD conversion file must be specified"),MB_APPLMODAL|MB_OK);
            return about;
          }
          if ((res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT2,WM_GETTEXT,(WPARAM)180,
                                          (LPARAM)OrcadConversionInfo.OutputDirectory))==0) {
            MessageBoxUTF8(DESIGNWindow,"",SC(79,"The output directory must be specified"),MB_APPLMODAL|MB_OK);
            return about;
          }
          if (DialogMode==1) {
            if ((res=SendDlgItemMessageUTF8(Dialog,IDC_EDIT3,WM_GETTEXT,(WPARAM)180,
        ...
        case ID_READ_FILE:
          res=0;
          result=GetNewFileUTF8(DESIGNWindow,NULL,OrcadConversionInfo.GeometryConversionFileName,DesignPath,
                                SC(80,"Geometry conversion schematic files"),
                                SC(211,"All files"),SC(80,"Geometry conversion schematic files"),"txt",0);
/*
          memset(&FileInputInfo,0,sizeof(OPENFILENAME));
          FileInputInfo.lStructSize= sizeof(OPENFILENAME);
          FileInputInfo.hwndOwner=DESIGNWindow;
          ...
          FileInputInfo.lpstrFileTitle=NULL;
          FileInputInfo.lpstrTitle=NULL;
          FileInputInfo.lpstrDefExt="txt";
          FileInputInfo.lpstrFilter=SC(80,"Geometry conversion schematic files (*.txt)\0*.txt\0All files (*.*)\0*.*\0\0");
          FileInputInfo.lpstrInitialDir=OrcadDesignPath;
          FileInputInfo.Flags=OFN_FILEMUSTEXIST|OFN_LONGNAMES|OFN_HIDEREADONLY;
          FileInputInfo.nMaxFile=200;
          FileInputInfo.nMaxFileTitle=32;
          ...
          if ( GetOpenFileName(&FileInputInfo) ) {
            strcpy(OrcadConversionInfo.GeometryConversionFileName,FileInputInfo.lpstrFile);
          }
*/
          break;
        case IDHELP:
...

{
  ...
  if (mode==0) {
    if (LoadOrcadFileName(OrcadConversionInfo.OrcadFile,0)<0) return -1;
    DialogMode=0;
    res=DialogBox(DESIGNClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_ORCAD_CONV), DESIGNWindow, (DLGPROC)OrcadConvertDialog2);
    if (res==2) return -1;
  } else {
    if (LoadOrcadFileName(OrcadConversionInfo.OrcadFile,1)<0) return -1;
    DialogMode=1;
    res=DialogBox(DESIGNClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_ORCAD_CONV2), DESIGNWindow, (DLGPROC)OrcadConvertDialog2);
    if (res==2) return -1;
  }
...
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)SeparatorString);
      SaveDesignIniFile();
      DesignActive=0;
      UpdateLayout=0;
    ...
    if (NewProjectDialog(1)==1) MakeNewProject(0);
    else return -1;
  }
  if (mode==0) {
    sprintf(ExeParams,"\"%s\" /s%s /d%s",ExeFile,OrcadConversionInfo.OrcadFile,OrcadConversionInfo.OutputDirectory);
    ...
    sprintf(ExeParams,"\"%s\" /l%s /d%s  /w%x /m%d",
            ExeFile,OrcadConversionInfo.OrcadFile,OrcadConversionInfo.OutputDirectory,
            (int32)DESIGNWindow,ID_ORCAD_EXIT_MESSAGE);
    memset(&StartupInfo,0,sizeof(StartupInfo));
    StartupInfo.cb=sizeof(StartupInfo);
    StartupInfo.wShowWindow=SW_SHOW;
    CreateProcess(ExeFile,ExeParams,NULL,NULL,1,0,NULL,NULL,&StartupInfo,&ProcessInfo);
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SelectionEsc=0;
      SetDialogItemTextUTF8(Dialog,IDOK                       ,SC(1,"OK"));
      SetDialogItemTextUTF8(Dialog,IDHELP                     ,SC(45,"Help"));
      ...
      SetDialogItemTextUTF8(Dialog,IDC_STATIC7                ,SC(334,"Scale"));
      SetDialogItemTextUTF8(Dialog,IDC_STATIC8                ,SC(425,"Paper size"));

      SetDialogItemTextUTF8(Dialog,IDC_RADIO6                 ,SC(426,"Auto"));
      ...
      SetWindowText(Dialog,SC(431,"PDF output"));
...
{
  char  SchematicFile[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING*5],
        FileStr[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],buf[4096],
        PageTitles[128][MAX_LENGTH_STRING],
        ExeFile[MAX_LENGTH_STRING],ExeParams[MAX_LENGTH_STRING*5];
  int32 cnt,cnt2,cnt3,cnt4,pos,fp=0,fp2,count,res,res2,
        test,test2,PageSizeUnitsX,PageSizeUnitsY,
        NrPDFObjectsRootDir,NrPDFObjects,len,SheetPdfObjectStart[256];
  ...
  if (mode==1) {
    res=DialogBox(DESIGNClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_PDF_OUTPUT), DESIGNWindow, (DLGPROC)ExportPDFDialogBody);
    if (res==2) return;

    sprintf(str,"%s\\sch\\%s.pdf",DesignPath,DesignName);
    if (FileExistsUTF8(str)==0) {
      sprintf(str2,SC(370,"File %s exist overwrite ?"),str);
      if (MessageBoxOwn(DESIGNWindow,str2,SC(1,"Message"),MB_OKCANCEL|MB_APPLMODAL)!=IDOK) {
        return;
      }
    }
    ...
    if (fp<=0) {
      ...
      MessageBoxUTF8(DESIGNWindow,str2,SC(100,"Error"),MB_APPLMODAL|MB_OK);
      return;
    }
...
//    SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
    ...
    if (mode==0) {
      sprintf(ExeParams,"\"%s\" /o /e \"%s\" /p \"%s\" /u \"%s\" /n \"%s,%d\" /x \"%s\\%s.dsn\" \"%s\"",
              ExeFile,ExePath,DesignPath,ProjectPath,PrinterName,PaperSize,DesignPath,DesignName,SchematicFile);
    } else {
      sprintf(ExeParams,"\"%s\" /o /e \"%s\" /p \"%s\" /u \"%s\" /n \"$PDF$,%d,%d,%d,%d\" /x \"%s\\%s.dsn\" \"%s\"",
              ExeFile,ExePath,DesignPath,ProjectPath,
              PDFInfo.PaperSize,PDFInfo.PaperOrientation,PDFInfo.PaperFitToPage,
              SheetPdfObjectStart[cnt2],
              DesignPath,DesignName,SchematicFile);
...
//    Sleep(1000);
    ...
    count=0;
    while ((!ProjectInfo->PrintingBusy)
          &&
          (count<400)) {
//      CheckInputMessages();
      Sleep(10);
    ...
    count=0;
    while ((ProjectInfo->PrintingBusy!=0)
          &&
          (count<1000)) {
//      CheckInputMessages();
      Sleep(10);
...

{
  ...
  WriteLn(fp,Line);
  if ((ExcludeInBOMID[0]!=0)
     &&
     (ExcludeInBOMValue[0]!=0)) {
...

{
  ...
  if ((fp=TextFileOpenUTF8(DesignFile))<0) return;
  ...
  while ((Length=ReadLnWithMaxLength(fp,LineBuf,MAX_LENGTH_STRING-50))>=0) {
    ...
    if ((Length>1)
      ...
      if (str1[0]=='[') {
        ParamMode=0;
        if (stricmp(str1,"[TopSheet]")==0) ParamMode=1;
        if (stricmp(str1,"[LayoutFile]")==0) ParamMode=2;
        if (stricmp(str1,"[Settings]")==0) ParamMode=3;
        if (stricmp(str1,"[GeometryLibraries]")==0) ParamMode=4;
        if (stricmp(str1,"[SchematicSymbolLibraries]")==0) ParamMode=5;
      } else {
        switch (ParamMode) {
          case 1:
...

{
  ...
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
        FileName[MAX_LENGTH_STRING],Line[MAX_LENGTH_STRING];
...

{
  ...
  char   LineBuf[MAX_LENGTH_STRING],str1[MAX_LENGTH_STRING],
         str2[MAX_LENGTH_STRING],
         CurrentDir[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING];

  if ((fp=TextFileOpenUTF8(UserIniFile))<0) return;
  ParamMode=0;
  while ((Length=ReadLnWithMaxLength(fp,LineBuf,MAX_LENGTH_STRING-50))>=0) {
    strcpy(str4,LineBuf);
    if ((Length>1)
...
//        if (stricmp(str1,"[GeometryLibraryPath]")==0) ParamMode=4;
  //      if (stricmp(str1,"[SchematicSymbolLibraryPath]")==0) ParamMode=5;
        if (stricmp(str1,"[LastDesigns]")==0) ParamMode=1;
        if (stricmp(str1,"[Settings]")==0) ParamMode=2;
      } else {
        switch (ParamMode) {
          case 1:
            if (NrDesigns<16) {
