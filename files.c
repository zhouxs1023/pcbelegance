...
extern   int32  ProjectIndexNr,WidthScrollBar,PaintCount,
                HeightInfoBar,HeightScrollBar,CheckFilenameForSaving,
...

{
  ...
  if (stricmp(&FileName[lengte-3],"shp")!=0) return;
...
{
  int32           PinOffset,MemPos,cnt,NrPins,PinNr,NrLines,
                  PolygonVertices,rot,*ShapeInfoP,
                  ShapeInfo,NrPinShapes,*PolygonPointer;
...
#ifdef _DEBUG
      if ((InRange5(NewObject.x1,39.3e5))
         &&
         (InRange5(NewObject.y1,54.6e5))) {
        ok=1;
      }
#endif
  ...
  while (NrLines>0) {
    ...
    switch (ShapeInfo) {
      ...
      case OBJECT_CIRCLE:     // circle   (X2 = thickness,Y2 circle type)
        ...
        break;
      case OBJECT_ARC:        // arc
        if (NewObject.y2==0) NewObject.y2=NewObject.x2;
        if (NewObject.x2==0) NewObject.x2=NewObject.y2;
    ...
    switch (Pad->ShapeType) {
      ...
      case OBJECT_ARC:
        ...
        if ((NewObject.Layer>=ROUTING_KEEPOUT_LAYER)
           &&
           (NewObject.Layer<ROUTING_KEEPOUT_LAYER+32)) {
            ok=1;
          }
#endif
        x1=(*ShapeLines)[2];
...
// *******************************************************************************************************
  ...
  for (cnt=0;cnt<NrObjectPolygons;cnt++) { // ObjectPolygonMemorySize
    ObjectPolygon=(ObjectPolygonRecord *)&(ObjectPolygonMem[(*ObjectPolygons)[cnt]]);
...
{
  char  DirPart[MAX_LENGTH_STRING],FilePart[MAX_LENGTH_STRING],
        BackupFile1[MAX_LENGTH_STRING];
...

{
  ...
  MessageBoxUTF8(NULL,MessageStr,"Message",MB_APPLMODAL|MB_OK);
}
...

{
  ...
  if (LibraryFile[0]!=0) {
    MessageBoxUTF8(GEOMWindow,SC(211,"Can not save a file in a library"),SC(1,"Error"),MB_APPLMODAL|MB_OK);
    return -2;
  }
  ...
  if ((mode & 4)==0) MakeBackup(EditFile);
...
// *******************************************************************************************************
  ...
  Shape.Info=0;
  if (SMDdevice==1) Shape.Info|=SMD_DEVICE;
...
*/
    if ((Object->Info & (OBJECT_NOT_VISIBLE)) == 0) Object->Info=0;
  }
  DeAllocateMemShapes();
    ...
    if (res==IDYES) {
      ...
      switch (res) {
        case -2:
          return 0;
        case -1:
          MessageBoxUTF8(GEOMWindow,EditFile,SC(29,"Error in saving file"),MB_APPLMODAL|MB_OK);
          return 1;
        case 0:
          return 1;
      }
...

{
  ...
  if (EditFile[0]==0) {
    strcpy(str,SC(125,"New geometry"));
  } else {
    if (LibraryFile[0]!=0) {
...
{
  int32 cnt,Layer,cnt2,cnt3,Layer2,PinNr,count,NrDrills,SaveError,
        DrillPinNr,ThrouHoleInfo,res,NrInnerPadObjects,
        AntiPowerPadsLayerInfo,NrPadsInnerSelected,
        GeomNameObjectCnt,InnerPadMask,ObjectInfo,ObjectType,
        Found,First,ObjectsAdded,Warning;
  uint32 ThrouHoleLayerInfo;
  ObjectRecord    *Object,*Object2,*ObjectTopLayer,*ObjectBottomLayer,NewObject,
                  *DrillObject,*FirstInnerPadObject,*InnerPadObjects[32],
                  *InnerPadObject,*PowerPadObject,
                  *GeomNameObject,*ObjectPasteBottomLayer,*ObjectPasteTopLayer,
                  *DrillObject2,*ObjectSoldBottomLayer,*ObjectSoldTopLayer;
  double DrillX1,DrillY1,x1,y1,PadX1,PadY1;
  ObjectPolygonRecord *ObjectPolygon;

  PinInfoRecord   *PinInfo;
  char            str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
                  str3[MAX_LENGTH_STRING],ValueStr[MAX_LENGTH_STRING],TextStr[MAX_LENGTH_STRING];
  ...
  for (cnt=0;cnt<NrObjects;cnt++) {
    ...
    if ((Object->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      for (cnt2=cnt+1;cnt2<NrObjects;cnt2++) {
        ...
        if ((cnt2!=cnt)
           &&
           ((Object2->Info & (OBJECT_NOT_VISIBLE)) == 0)) {
          if ((InRange(Object->x1,Object2->x1))
             &&
             (InRange(Object->y1,Object2->y1))
             &&
            ...
            switch (Object->ObjectType) {
              ...
              case OBJECT_LINE:
                if ((Object->Layer==Object2->Layer)
                   ...
                   ||
                   (Object->Layer==PASTE_MASK_TOP_LAYER)
                   ||
                   (Object->Layer==DRILL_LAYER)
                   ||
                   (Object->Layer==DRILL_UNPLATED_LAYER)
                   ||
                   (Object->Layer==INNER_PAD_LAYER)
                   ||
...
// *******************************************************************************************************
  ...
  if (NrCompOutlinesSelected==0) {
    if (AddToMessageBuf(SC(216,"There were no component outlines defined"))!=0) return -1;
    if ((mode & 2)==0) {
      SaveError=1;
    }
  }
  if (NrPlacemOutlinesSelected==0) {
    if (AddToMessageBuf(SC(217,"There were no placement outlines defined"))!=0) return -1;
    if ((mode & 2)==0) {
      SaveError=1;
    }
  }
/*
  if (MessageBufPos!=0) {
    MessageDialog(SC(218,"Warning"),1);
...
*/
  ...
  for (cnt=0;cnt<NrDrills;cnt++) {
    ...
    for (cnt2=0;cnt2<NrObjects;cnt2++) {
      ...
      if ((InRange5(Object->x1,-22.2e5))
         &&
         (InRange5(Object->y1,-14.6e5))) {
        ok=1;
      }
      if ((InRange5(DrillX1,150.5e5))
         &&
         (InRange5(DrillY1,108.1e5))) {
        ok=1;
        if ((InRange5(Object->x1,150.5e5))
           &&
           (InRange5(Object->y1,108.1e5))) {
          ok=1;
      ...
      if (((Object->Info & (OBJECT_NOT_VISIBLE)) == 0)
         &&
         (InRange(DrillX1,Object->x1))
         &&
         (InRange(DrillY1,Object->y1))
         &&
        ...
        switch (Layer2) {
          ...
          case SOLD_MASK_TOP_LAYER:      // Solder mask top layer
            ...
            break;
          default:
            if ((Object->PinNr==DrillPinNr)
               &&
               (Layer2<32)) {
              ThrouHoleInfo|=64;           // Does have a pinname
              if (Layer2==0) {             // Bottom layer
                ObjectBottomLayer=Object;
                ThrouHoleInfo|=2;
                ThrouHoleLayerInfo|=1 << Layer2;
              }
              if (Layer2==NrPadLayers-1) { // Top layer
                ObjectTopLayer=Object;
                ThrouHoleInfo|=1;
                ThrouHoleLayerInfo|=1 << Layer2;
              }
              if ((CheckIfInnerLayer(Layer2))
                 &&
...
#ifdef _DEBUG
    if ((InRange5(DrillX1,-8.0e5))
       &&
       (InRange5(DrillY1,-2.4e5))) {
      ok=1;
    }
#endif
    ...
    if (((ThrouHoleInfo & (64+2+1)) == (64+2+1))
       &&
       (ThrouHoleLayerInfo==(uint32)-1)) {
      ...
      Found=0;
      if ((ObjectBottomLayer->ObjectType==OBJECT_CIRCLE)
         &&
         (ObjectTopLayer->ObjectType==OBJECT_CIRCLE)
         &&
         (InRange(ObjectBottomLayer->x2,ObjectTopLayer->x2))) {
        Found=1;
        NewObject.ObjectType=PIN_PUT_THROUGH_ROUND;
      }
      if ((ObjectBottomLayer->ObjectType==OBJECT_RECT)
         &&
         (ObjectTopLayer->ObjectType==OBJECT_RECT)
         &&
      ...
      if (Found) {
        ...
        if ((ThrouHoleInfo & 8) == 8) {  // Inner pad
          ...
          sprintf(str,SC(446,"No inner pad\t\t%s,%s"),str2,str3);
      ...
      if (ThrouHoleLayerInfo!=-1) {
        ...
        for (cnt2=1;cnt2<NrPadLayers-1;cnt2++) {
          if ((ThrouHoleLayerInfo & (1 << cnt2))==0) {
            if (ThrouHoleInfo & 256) { // Different inner pads sizes
              sprintf(str,SC(455,"Different inner padsize\t%d\t%s,%s"),cnt2,str2,str3);
              if (AddToMessageBuf(str)!=0) return -1;
            } else {
        ...
        if ((ThrouHoleLayerInfo & (1 << (NrPadLayers-1)))==0) {
          sprintf(str,SC(442,"Missing pad\tTOP\t%s,%s"),str2,str3);
          if (AddToMessageBuf(str)!=0) return -1;
          if ((mode & 2)==0) {
            SaveError=1;
          }
        ...
        if ((ThrouHoleLayerInfo & (1 << 0))==0) {
          sprintf(str,SC(443,"Missing pad\tBOTTOM\t%s,%s"),str2,str3);
          if (AddToMessageBuf(str)!=0) return -1;
          if ((mode & 2)==0) {
            SaveError=1;
          }
            ...
            for (cnt2=0;cnt2<NrObjects;cnt2++) {
              if (cnt2!=cnt) {
                ...
                if (((Object2->Info & (OBJECT_NOT_VISIBLE|OBJECT_SAVED)) == 0)
                   &&
                   (InRange(PadX1,Object2->x1))
                   &&
                   (InRange(PadY1,Object2->y1))) {
...
// *****************************************************************************************
    ...
    if ((ObjectInfo & (OBJECT_NOT_VISIBLE|OBJECT_SAVED)) == 0) {
      ...
      switch (Object->Layer) {
        case POWER_PAD_LAYER:
          for (cnt2=0;cnt2<NrDrills;cnt2++) {
            DrillObject=&((*Objects)[DrillObjects[cnt2]]);
            if ((InRange(DrillObject->x1,x1))
               &&
               (InRange(DrillObject->y1,y1))) {
              break;
            }
          }
          if (cnt2==NrDrills) {
  ...
  if (NrSilkTopObjectsSelected+NrSilkBottomObjectsSelected==0) {
    if (AddToMessageBuf(SC(233,"No silkscreen outlines"))!=0) return -1;
  }
  if (GeomNameObjectCnt!=-1) {
    Object=&((*Objects)[GeomNameObjectCnt]);
    if (strcmp(Object->Text,"noname")==0) {
      if (AddToMessageBuf(SC(234,"Default geometry name"))!=0) return -1;
    }
  }
...

{
  if (BufPos+MemSize>MaxShapesMem) {
    if (AllocateMemShapes(max(BufPos+MemSize+16384,MaxShapesMem*2))!=0) return -1;
  }
...
{
  int32           ok,cnt,StartCnt,cnt2,cnt3,Layer2,NrPins,
                  PolygonUsage,PolygonPos,LineSegments,SegmentCount,ObjectType,
                  Layer,NrPinShapesSaved,NrPinShapesBufPos,
                  ObjectSize,NewBufPos,ObjectInfo,Found,LayerOk,
        ...
        *ShapeInfoP=(int32)ObjectType;
        ...
        switch (ObjectType) {
          ...
          case OBJECT_CIRCLE:
            ...
            break;
          case OBJECT_ARC:
            if (Object->y2==0) Object->y2=Object->x2;
            if (Object->x2==0) Object->x2=Object->y2;
            ShapeLines[3]=(float)Object->x2;
            ShapeLines[4]=(float)Object->y2;
...
*/
  ...
  while ((cnt<NrPinObjects)
    ...
    for (cnt2=0;cnt2<NrObjects;cnt2++) {
      ...
      if ((InRange5(Object->x1,-8.0e5))
         &&
         (InRange5(Object->y1,-2.4e5))) {
        if (stricmp(ShapePad.Name,"5")==0) {
          ok=1;
        }
        ok=1;
      ...
      if (((ObjectInfo & (OBJECT_NOT_VISIBLE|OBJECT_SAVED)) == 0)
         &&
         (Object->Test==1) // Copper object
         &&
         (Object->PinNr==cnt)
         &&
         (ShapePad.NrPinShapes<MaxObjectsPerPin)) {
        if ((Object->ObjectType==OBJECT_CIRCLE)
           &&
           (Object->Layer==POWER_PAD_LAYER)) {
            if (!PowerPadObject) {
    ...
    for (cnt2=0;cnt2<NrObjectPolygons-NrMappableObjectPolygons;cnt2++) {
      ...
      if (((ObjectPolygon->Info & (OBJECT_NOT_VISIBLE|OBJECT_SAVED)) == 0)
         &&
         (ObjectPolygon->BlockNr==1) // Copper object
         &&
         (ObjectPolygon->PinNr==cnt)
         &&
         (ShapePad.NrPinShapes<MaxObjectsPerPin)) {
        ...
        if ((PinObjectMode[cnt2] & 32)==0) { // Non polygons
          switch (ObjectType) {
            ...
            case PIN_PUT_THROUGH_ROUND:
              ...
              if ((NewBufPos=WriteToBuf((uint8 *)&Pad,BufPos,ObjectSize))==-1) return -1;
              BufPos=NewBufPos;
              NrPinShapesSaved++;
...
// **********************************************************************************
        ...
        for (cnt2=0;cnt2<ShapePad.NrPinShapes;cnt2++) {
...
#endif
          if ((PinObjectMode[cnt2] & 32)==0) { // Non polygon object
            Object=(ObjectRecord *)PinObjects[cnt2];
            ObjectType=Object->ObjectType;
            Layer2=Object->Layer;
          ...
          if ((PinObjectMode[cnt2] & 32)==0) { // Non polygons
            ...
            if (ObjectType!=OBJECT_ARC) {
              ...
              BufPos=NewBufPos;
              ObjectSize=40;
              if ((NewBufPos=WriteToBuf((uint8 *)&ShapeLines[2],BufPos,ObjectSize))==-1) return -1;
              BufPos=NewBufPos;
          }
        } else {  // Polygons
          ObjectPolygon=(ObjectPolygonRecord *)PinObjects[cnt2];
  ...
  // **********************************************************************************
      ...
      if (NrPinShapesSaved!=ShapePad.NrPinShapes) {
        ...
        ok=1;
#ifdef _DEBUG
        MessageBoxUTF8(GEOMWindow,"Severe error",SC(1,"Error"),MB_APPLMODAL|MB_OK);
#endif
      }
    }
    cnt++;
...
// **********************************************************************************
  ...
  for (cnt=0;cnt<NrObjects;cnt++) {
    ...
    ObjectInfo=Object->Info;
    if ((ObjectInfo & (OBJECT_NOT_VISIBLE|OBJECT_SAVED)) == 0) {
      if ((Object->Layer==DRILL_UNPLATED_LAYER)
         ||
         (Object->Layer==POWER_PAD_LAYER)) {
        ShapePad.NrPinShapes++;
      }
    ...
    if (((ObjectInfo & (OBJECT_NOT_VISIBLE)) == 0)) {
      ...
      if (LayerOk) {
        switch (ObjectType) {
          ...
          case OBJECT_CIRCLE:
#ifdef _DEBUG
            if ((InRange5(Object->x1,108.0e5))
               &&
               (InRange5(Object->y1,336.1e5))) {
              ok=1;
            }
#endif
            memset(&Pad,0,sizeof(PadRecord));
          ...
          case OBJECT_TEXT:
            switch (Layer) {
              ...
              case SILKSCREEN_BOTTOM_LAYER:
                ...
                BufPos=NewBufPos;
                ObjectSize=64;
                if ((NewBufPos=WriteToBuf((uint8 *)&ShapeLines[7],BufPos,ObjectSize))==-1) return -1;
                BufPos=NewBufPos;
                Shape.NrOtherObjects++;
                Object->Info|=OBJECT_SAVED;
                break;
...
// **********************************************************************************
  ...
  for (cnt=MappableObjectPolygonStart;cnt<MappableObjectPolygonStart+NrMappableObjectPolygons;cnt++) {
    ...
    cnt2++;
    if ((NewBufPos=WriteToBuf((uint8 *)ObjectPolygon,BufPos,ObjectSize))==-1) return -1;
    BufPos=NewBufPos;
  }
  ...
  if (LibraryFile[0]==0) {
    if (FileExistsUTF8(FileName2)==0) {
      ...
      if (FileRead(Designfp,&Shape,sizeof(ShapeRecord),&result)==-1) {
        ...
        MessageBoxUTF8(GEOMWindow,str,SC(1,"Error"),MB_OK);
        FileClose(Designfp);
        if ((mode & 2)==2) EditFile[0]=0;
        return -1;
      }
...
//  MessageBox(NULL,"LoadDesign","Message",MB_APPLMODAL|MB_OK);

  Pos=0;
  SizeFile=0;

  if (EditFile[0]==0) return 0;

  if (LibraryFile[0]!=0) {
    GetFilePartFromFileName(str,EditFile);
    strcpy(EditFile,str);

    if ((Libfp=FileOpenReadOnlyUTF8(LibraryFile))==-1) return 0;
    ...
    FileSeek(Libfp,Pos);
    if (FileRead(Libfp,ShapesMem,SizeFile,&result)!=0) return 0;
    FileClose(Libfp);
  } else {
    LengteFile=FileSizeUTF8(EditFile);
    if ((Designfp=FileOpenReadOnlyUTF8(EditFile))==0) return 0;
    AllocateMemShapes(LengteFile+16384);
    if (FileRead(Designfp,ShapesMem,MaxShapesMem,&result)!=0) return 0;
    FileClose(Designfp);
  }
...

{
  ...
  result=GetNewFileUTF8(GEOMWindow,NULL,EditFile,EditPath,SC(246,"Bitmap files"),
                        NULL,SC(246,"Bitmap files"),"bmp",0);
  if ((result==-1)
     ||
     (result==0)) return -1;
  return 0;
...

{
  ...
  result=GetNewFileUTF8(GEOMWindow,NULL,EditFile,EditPath,SC(245,"Geometry files (*.shp)"),
                        NULL,SC(245,"Geometry files (*.shp)"),"shp",1);
...

{
  ...
  int32             BitActive,MemSize,fp,result,cnt,cnt2,cnt3,cnt4,
                    res,Count,Width,Height,BitPos2;
...
//  if ((fp=FileOpenReadOnlyUTF8("c:\\viewplot\\blad1.bmp"))>0) {
  ...
  CheckInputMessages(0);
  FileName[0]=0;
  if (LoadBitmapFile(FileName)!=0) return -1;
  memset(&NewObject,0,sizeof(ObjectRecord));
  NewObject.ObjectType=OBJECT_LINE;
  NewObject.Layer=0;
  ...
  if ((fp=FileOpenReadOnlyUTF8(FileName))>0) {
    ...
    if ((BmpHeader.Identifier!=0x4d42)
       ||
      ...
      MessageBoxUTF8(GEOMWindow,FileName,SC(247,"Wrong bitmap file"),MB_APPLMODAL|MB_OK);
      FileClose(fp);
      return -1;
    }
...
//      return -2;
    ...
    if (LineInputDialog(&TypeObject,SC(248,"Pixel size bitmap"))==1) {
