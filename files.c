...
int32 TempMaxNrHorTraces[32],
      TempMaxNrVerTraces[32],
      TempMaxNrDiag1Traces[32],
      TempMaxNrDiag2Traces[32],
      TempMaxNrNetsPos[32],
      TempMaxNrConnections,
      TempMaxNrConnectionsPos,
      TempMaxNrVias,
      TempMaxNrViasPos,
      TempMaxNrNets,
      TempMaxNrComps,
      TempMaxNrShapes,
      TempMaxNrAreaFills,
      TempMaxNrObjectLines,
      TempMaxNrObjectRects,
      TempMaxNrObjectCircles,
      TempMaxNrObjectArcs,
      TempMaxNrObjectTexts,
      TempMaxNrObjectTexts2,
      TempMaxCompsMemory,
      TempMaxShapesMemory,
      TempMaxAreaFillMemory,
      TempMaxNrObjectPolygons,
      TempMaxObjectPolygonMemory;
...
{
  int32  MemPos,cnt,cnt2,SizeFile,Pos,NrLibEntries,NrGeometryLibFiles,
         *ShapeTypeP,ShapeType,NrLines,Shapefp,result,res,Libfp;
  int32   Found;
  ShapeLinesArray *ShapeLines;
  char   str[MAX_LENGTH_STRING],GeometryLibNames[64][MAX_LENGTH_STRING],
         SearchFileName[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING];
...
// Check shape libraries
  ...
  if (Design.NrShapes+2>=MaxNrShapes) {
    if (AllocateMemShapes(Design.NrShapes+32)!=0) return -1;
  }
  if (Design.ShapesMem+SizeFile>=MaxShapesMemory) {
    if (AllocateMemShapesMemory(Design.ShapesMem+SizeFile+32768)!=0) return -1;
  }
  if (Pos==-1) {
    SizeFile=FileSizeUTF8(str);
    if ((Shapefp=FileOpenReadOnlyUTF8(str))==-1) return -2;
    if (FileRead(Shapefp,&ShapesMem[Design.ShapesMem],SizeFile,&result)==-1) return -2;
    FileClose(Shapefp);
  } else {
    if ((Shapefp=FileOpenReadOnlyUTF8(str))==-1) return -2;
  ...
  if (stricmp(Shape->Identification,ShapeCode)==0) { // Old shape definition
    Shape->NrLayers=2;
    Shape->NrPolygons=0;
...
{
  int32 hulp,result,Designfp,cnt,RotationInt,TextRotationRef,TextRotationValue,
        TextRotation,*CompPos,CompsMemLength,PinsLength;
  NetRecord *Net;
  ...
  for (cnt=0;cnt<Design.NrBoardLayers;cnt++) {
    if (AllocateMemHorTraces(cnt,TempMaxNrHorTraces[cnt])!=0) return -1;
    if (AllocateMemVerTraces(cnt,TempMaxNrVerTraces[cnt])!=0) return -1;
    if (AllocateMemDiag1Traces(cnt,TempMaxNrDiag1Traces[cnt])!=0) return -1;
    if (AllocateMemDiag2Traces(cnt,TempMaxNrDiag2Traces[cnt])!=0) return -1;
  }

  if (AllocateMemComps(TempMaxNrComps)!=0) return -1;
  if (AllocateMemShapes(TempMaxNrShapes)!=0) return -1;
  if (AllocateMemVias(TempMaxNrVias)!=0) return -1;
  if (AllocateMemObjectLines(TempMaxNrObjectLines)!=0) return -1;
  if (AllocateMemObjectRects(TempMaxNrObjectRects)!=0) return -1;
  if (AllocateMemObjectCircles(TempMaxNrObjectCircles)!=0) return -1;
  if (AllocateMemObjectArcs(TempMaxNrObjectArcs)!=0) return -1;
  if (AllocateMemObjectTexts(max(TempMaxNrObjectTexts,TempMaxNrObjectTexts2))!=0) return -1;
  if (AllocateMemObjectTexts2(max(TempMaxNrObjectTexts,TempMaxNrObjectTexts2))!=0) return -1;

  if (AllocateMemObjectPolygons(TempMaxNrObjectPolygons)!=0) return -1;
  if (AllocateMemObjectPolygonMemory(TempMaxObjectPolygonMemory)!=0) return -1;

  if (AllocateMemAreaFills(TempMaxNrAreaFills)!=0) return -1;
  if (AllocateMemAreaFillMemory(TempMaxAreaFillMemory)!=0) return -1;

  if (AllocateMemCompsMemory(TempMaxCompsMemory)!=0) return -1;
  if (AllocateMemNets(TempMaxNrNets)!=0) return -1;
  if (AllocateMemNetConnections(1024)!=0) return -1;
  if (AllocateMemConnections(TempMaxNrConnections)!=0) return -1;

  if (FileRead(Designfp,Comps,sizeof(int32)*(int32)Design.NrComps,&result)==-1) return -3;
  ok=1;
  if ((strcmp(Design.Identification,PCBCode)==0)
     ||
     (strcmp(Design.Identification,PCBCode2)==0)) {
...
//  OldCompRecord          OldComp;
    ...
    if (FileRead(Designfp,CompsMem,
                           (int32)Design.CompsMem,&result)==-1) return -3;
    for (cnt=0;cnt<Design.NrComps;cnt++) {
      Comp=(CompRecord *)&(CompsMem[(*Comps)[cnt]]);
#ifdef _DEBUG
...
//        Comp->TextVisibility&=~0x88;
  ...
  if (LoadShapes(0)!=0) return -3;
  hulp=FileCurrentPointer(Designfp);
  if ((strcmp(Design.Identification,PCBCode)==0)
     ||
     (strcmp(Design.Identification,PCBCode2)==0)) {
    for (cnt=0;cnt<Design.NrNets;cnt++) {
      if (FileRead(Designfp,&OldNet,sizeof(OldNetRecord),&result)==-1) return -3;
      Net=&((*Nets)[cnt]);
...
/*
          int32 Info,AddNr,DeleteNr,NetNr;
          int32 Layer;
          double X1,Y1,X2,Y2,LineThickNess;
*/
      if (FileRead(Designfp,&OldObjectLine,sizeof(OldObjectLineRecord),&result)==-1) return -3;
...
//      ObjectLine->NetNr=OldObjectLine.NetNr;
    ...
    for (cnt=0;cnt<Design.NrObjectRects;cnt++) {
      if (FileRead(Designfp,&OldObjectRect,sizeof(OldObjectRectRecord),&result)==-1) return -3;
    ...
    for (cnt=0;cnt<Design.NrObjectCircles;cnt++) {
      if (FileRead(Designfp,&OldObjectCircle,sizeof(OldObjectCircleRecord),&result)==-1) return -3;
/*
          int32 Info,AddNr,DeleteNr;
          int32 Layer;
          int32 NetNr,LineMode,CircleMode;
          double CentreX,CentreY,Diam,LineThickNess;
*/
...
//      ObjectCircle->NetNr=OldObjectCircle.NetNr;
      ObjectCircle->Clearance=Design.StandardClearance;
      ObjectCircle->Dummy=0.0;
    }
    for (cnt=0;cnt<Design.NrObjectArcs;cnt++) {
      if (FileRead(Designfp,&OldObjectArc,sizeof(OldObjectArcRecord),&result)==-1) return -3;
...
//      ObjectArc->NetNr=OldObjectArc.NetNr;
  ...
  if ((strcmp(Design.Identification,PCBCode)==0)
    ...
    for (cnt=0;cnt<Design.NrObjectTexts2;cnt++) {
      if (FileRead(Designfp,&OldObjectText2,sizeof(OldObjectTextRecord2),&result)==-1) return -3;
...

{
  ...
  if (MaxDesignSaveMem==0) {
    ...
    if ((DesignSaveMemGlobal=GlobalAlloc(GHND,DesignSaveMemSize))==NULL) return -1;
    if ((DesignSaveMem=(uint8 *)GlobalLock(DesignSaveMemGlobal))==NULL) return -1;
    MaxDesignSaveMem=DesignSaveMemSize;
  } else {
    if ((NewMem=GlobalReAlloc(DesignSaveMemGlobal,DesignSaveMemSize,GHND))==NULL) return -1;
...

{
  if (BufPos+MemSize>MaxDesignSaveMem) {
    if (AllocateMemDesignSave(max(BufPos+MemSize+16384,MaxDesignSaveMem*2))!=0) return -1;
  }
...
{
  char  DirPart[MAX_LENGTH_STRING],FilePart[MAX_LENGTH_STRING],BackupFile1[MAX_LENGTH_STRING],BackupFile2[MAX_LENGTH_STRING];
...
{
  int32  cnt,cnt2,Layer,NewBufPos,TempPos,res,NrAreaFills,
         AreaFillMemSize,NewLayer,
         StartOfTracesPos,Libfp=0,fp2,Pos,Length,res2,
         HorTracesCount[32],VerTracesCount[32],Diag1TracesCount[32],Diag2TracesCount[32],
         ObjectLineCount,ObjectRectCount,ObjectCircleCount,ObjectArcCount,
         ObjectTextCount,ObjectText2Count,ObjectPolygonCount,PolygonMemSize,
         ViasCount,ConnectionsCount,CompsCount,CompsMemSize,CompLength,
         BufPos=0,fp,result,SpecialCompRotation;
  double value;
  ShapeInfoRecord *ShapeInfo;
  TraceRecord  *Trace;
  ViaRecord    *Via;
...
//  sprintf(PerformanceStrings[PerformanceCount++],"%s   [  %.3f  µs  ]\r\n",String,value);
  ...
  if (SaveSymbolsLocally) {
    for (cnt2=0;cnt2<Design.NrShapes;cnt2++) {
      ...
      if ((LoadShapeDirectory(ShapeInfo->ShapeName,LibName,&Pos,&Length)>=0)
         &&
         (stricmpUTF8(LibName,str))
         &&
         (Length<256*1024)
         &&
...
//  WriteToBuf(&Design,int32 BufPos,int32 MemSize)
  ok=1;
  for (cnt=0;cnt<Design.NrComps;cnt++) {
    Comp=(CompRecord *)&(CompsMem[(*Comps)[cnt]]);
...
//        sprintf(str,"Error in saving file (%d)",__LINE__);MessageBoxOwn(PCBWindow,str,"Message",MB_APPLMODAL|MB_OK);
        return -1;
      }
...
/*
  if (SpecialCompRotation) {
    MessageBoxOwn(PCBWindow,"Components will not be saved under 45 degree rotation",SC(102,"Warning"),MB_APPLMODAL|MB_OK);
  }
*/
...
//  strcpy(NewDesign.Identification,PCBCode);
//  strcpy(NewDesign.Identification,PCBCode2);
...
//    sprintf(str,"Error in saving file (%d) fp=%d",__LINE__,fp);MessageBoxOwn(PCBWindow,str,"Message",MB_APPLMODAL|MB_OK);
    return -1;
  }
...
//  MessageBoxOwn(NULL,str,"Message",MB_APPLMODAL|MB_OK);
...

{
  ...
  if (FileChanged) {
    sprintf(str,SC(382,"File %s has changed, save ?"),EditFile);
...

{
  ...
  ViaRecord    *Via;
  ConnectionsRecord *Connection;
  if ((fp=FileOpenUTF8("c:\\pcb\\tr.!!!"))==-1) return;
...

{
  ...
  if (EditFile[0]!=0) {
    ...
    sprintf(str,SC(241,"New layout"));
  }
#ifdef TEST
    sprintf(str2,"TEST TEST TEST TEST TEST TEST TEST TEST TEST     %s     TEST TEST TEST TEST TEST TEST TEST TEST TEST",str);
    strcpy(str,str2);
#endif
  if (ProjectIndexNr!=-1) {
  ...
  if (mode==1) strcat(str," *");
/*
  sprintf(str2,"  %d",RedrawAbsPosStrCnt);
  strcat(str,str2);
*/
  if (PCBWindow!=NULL) {
...

{
  ...
  if (FileExistsUTF8(FileName2)==0) {
    ...
    if ((stricmpOwn(Design.Identification,PCBCode)!=0)
       &&
       (stricmpOwn(Design.Identification,PCBCode2)!=0)
       &&
       (stricmpOwn(Design.Identification,PCBCode3)!=0)
       &&
       (stricmpOwn(Design.Identification,PCBCode4)!=0)) {
...

{
  ...
  if (EditFile[0]!=0) {
    ...
    if (FoundDesignPath) {
      ...
      if ((!SetCurrentDirectoryUTF8(FileName2))
         &&
         (!CreateDirectoryUTF8(FileName2))) {
...
*/
  if ((mode & 2)==0) {
    ...
    CheckInputMessages(0);
    if (MessageBufPos!=0) {
      MessageDialog(SC(100,"Error"),0,0);
...

{
  ...
  if (GetNewFileUTF8(PCBWindow,NULL,EditFile2,ProjectPath,SC(389,"Layout files"),
                     NULL,SC(389,"Layout files"),"pcb",0)) {
    return -1;
  }
  if (ProjectActive) {
    cnt=0;
    while ((cnt<32)
          ...
          ||
          (stricmpOwn(ProjectInfo->FileNames[cnt],EditFile2)!=0))) cnt++;
    if (cnt<32) {
...

{
  ...
  if (GetNewFileUTF8(PCBWindow,NULL,EditFile,str,SC(389,"Layout files"),
                     NULL,SC(389,"Layout files"),"pcb",1)) {
    return -1;
  }
  return 0;
}
...
{
  if (GetNewFileUTF8(PCBWindow,NULL,NewFile,ExportDir,SC(363,"DXF file"),
...

{
  ...
  while ((Length=ReadLn(fp,LineBuf))>=0) {
    ...
    if ((Length>1)
      ...
      if (str1[0]=='[') {
        ParamMode=0;
        if (stricmpOwn(str1,"[TopSheet]")==0) ParamMode=1;
        if (stricmpOwn(str1,"[LayoutFile]")==0) ParamMode=2;
        if (stricmpOwn(str1,"[Settings]")==0) ParamMode=3;
        if (stricmpOwn(str1,"[GeometryLibraries]")==0) ParamMode=4;
        if (stricmpOwn(str1,"[SchematicSymbolLibraries]")==0) ParamMode=5;
      } else {
        switch (ParamMode) {
          case 1:
...
//            strcpy(LayoutFile,str1);
          ...
          case 4: //  GeometryLibraryPath
            ...
            break;
/*
          case 5: //  SchematicSymbolLibraryPath
            if (NrSchematicSymbolLibraryPath<16) {
              strcpy(SchematicSymbolLibraryPath[NrSchematicSymbolLibraryPath++],str1);
            }
            break;
*/
        }
