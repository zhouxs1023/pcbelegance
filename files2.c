...
typedef int32 (WINAPI *PGETDISKFREESPACEEX)(LPCSTR,
...

{
  ...
  WriteLnError=0;
  if (Utf8ToUnicode(FileName,FileStringUC,MAX_LENGTH_UC_STRING-1)==-1) return -2;
...

{
  ...
  if (res==0xffffffff) return -2;

  if ((res & FILE_ATTRIBUTE_READONLY) == 0) return 0;
  return -1;
}
...

{
  ...
  EndOfLineMode=0;
  WriteTextLine[0]=0;
  if (Utf8ToUnicode(FileName,FileStringUC,MAX_LENGTH_STRING-1)==0) return -2;
...

{
  int32 lengte,niks;
  if ((lengte=strlen(LineBuf))>0) {
    if (FileWrite(fp,LineBuf,lengte,&niks)==-1) return -1;
  }
  return 0;
}
...

{
  ...
  while (!Stop) {
    if (TextBufLength==0) {
      BytesRead=0;
      if ((FileRead(fp,TextBuf,TextBufSize,&BytesRead)==-1)
         ||
         (BytesRead==0)) {
        if (LineLength>0) {
          TextLineNr++;
...

{
  ...
  if ((lengte<3)
    ...
    if (pGetDiskFreeSpaceEx) {
      fResult = pGetDiskFreeSpaceEx (Dir,(PULARGE_INTEGER)&i64FreeBytesToCaller,(PULARGE_INTEGER)&i64TotalBytes,(PULARGE_INTEGER)&i64FreeBytes);
...

{
  ...
  if (fp==-1) return -1;
  if (lseek(fp,0,SEEK_END)==-1) return -2;
  pos2=tell(fp);
  if (pos2==-1) return -3;
...

{
  ...
  if (Utf8ToUnicode(Dir,FileStringUC,MAX_LENGTH_UC_STRING-1)==-1) return -2;
...

{
  ...
  Dir[0]=0;
  if (UnicodeToUtf8(FileStringUC,Dir,MAX_LENGTH_UC_STRING-1)==-1) return -2;
...

{
  ...
  lengte=strlen(Dir);
  if (lengte>3) {
    if (Dir[lengte-1]=='\\') Dir[lengte-1]=0;
  }
}
...

{
  ...
  while ((cnt<l)
        ...
        ||
        (Str[cnt-1]=='\\'))) cnt++;
  if (cnt>pos) memmove(Result,&Str[pos],cnt-pos+1);
  if (cnt==l) {
    Result[cnt-pos]='\"';
  }
  Result[cnt-pos+1]=0;
...

{
  ...
  l=strlen(Str);
  if (l==0) return;
  if ((mode & 0x100) == 0) { // Check for maximum length of MAX_LENGTH_STRING characters
    if (l>=MAX_LENGTH_STRING) return;
  }
  cnt=0;
  while ((cnt<l)
        &&
...

{
  ...
  if (lengte>=MAX_LENGTH_STRING) return 0;
...
int32 GetNewFile(HWND Window,HINSTANCE hInstance,LPSTR OwnFile,LPSTR DirPath,LPSTR FileInfoStr,
...

{
  WCHAR  str[8192],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],*StrP,*TempFileName,
         FileInfoStrUC[MAX_LENGTH_STRING],FileInfoStrUC2[MAX_LENGTH_STRING],*NiksW,
         ExtStr[40],DialogInfoStrUC[MAX_LENGTH_STRING],SearchStr[40],
         DirPathTempW[MAX_LENGTH_STRING];
  ...
  memset(&str,0,sizeof(str));
  memset(&str2,0,sizeof(str2));
  if (Utf8ToUnicode(Extension,ExtStr,sizeof(ExtStr)/2-1)==0) return -1;
#ifdef GCC_COMP
  ...
  if (strlen(OwnFile)>0) {
    if (OwnFile[strlen(OwnFile)-1]!='\\') {
      ...
      if (Utf8ToUnicode(DirPathTemp,DirPathTempW,MAX_LENGTH_STRING-1)==0) return -1;
    } else {
      if (Utf8ToUnicode(OwnFile,str3,MAX_LENGTH_STRING-1)==0) return -1;
      wcscpy(DirPathTempW,str3);
    }
  } else {
    if (Utf8ToUnicode(DirPath,DirPathTempW,MAX_LENGTH_STRING-1)==0) return -1;
  }
  if (FileInfoStr) {
    if (Utf8ToUnicode(FileInfoStr,FileInfoStrUC,MAX_LENGTH_STRING-1)==0) return -1;
#ifdef GCC_COMP
    ...
    if ((mode & 1) == 0) {
      ...
      if (FileInfoStr2) {
        if (Utf8ToUnicode(FileInfoStr2,FileInfoStrUC2,MAX_LENGTH_STRING-1)==0) return -1;
  ...
  if (Utf8ToUnicode(DialogInfo,DialogInfoStrUC,MAX_LENGTH_STRING-1)==0) return -1;
  ...
  if (result) {
    if ((mode & 1) == 0) {
      if ((mode & 2) == 0) {
        if (UnicodeToUtf8(FileInputInfo.lpstrFile,OwnFile,MAX_LENGTH_STRING-50)==0) return -1;
        GetDirFromFileName(DirPath,OwnFile);
      } else {
        ...
        if (TempFileName[0]==0) {
          if (UnicodeToUtf8(FileInputInfo.lpstrFile,OwnFile,MaxSize)==0) return -1;
          GetDirFromFileName(DirPath,OwnFile);
        } else {
          ...
          for (cnt=0;cnt<lengte;cnt++)
            if (TempFileName2[cnt]=='\t') TempFileName2[cnt]=0;
      ...
      if (GetFullPathNameW(FileInputInfo.lpstrFile,MAX_LENGTH_STRING-50,str3,&NiksW)>0) {
        if (UnicodeToUtf8(str3,OwnFile,MAX_LENGTH_STRING-1)==0) return -1;
        return result;
      }
    }
    return 0;
...

{
  ...
  if (Utf8ToUnicode(Src,SrcW,MAX_LENGTH_STRING-1)==0) return 0;
...

{
  ...
  if (Utf8ToUnicode(Src,SrcW,MAX_LENGTH_STRING-1)==0) return 0;
  if (Utf8ToUnicode(Dest,DestW,MAX_LENGTH_STRING-1)==0) return 0;
...

{
  ...
  if (Utf8ToUnicode(Dir,DirW,MAX_LENGTH_STRING-1)==0) return 0;
