...

{
  ...
  if (cnt==NrSpecialFonts) {
    ...
    UsedFont=CreateFont(1200,0,0,0,0,0,0,0,ANSI_CHARSET,
...
//  TextChar=32332;
  ...
  res=GetGlyphOutlineW(OutputDisplay,(WCHAR)TextChar,GGO_NATIVE,&GlyphMetrics,count,&CharInfoMem,&TransFormation);
  if (res==GDI_ERROR) {
    ReleaseDC(PCBWindow,OutputDisplay);
    return 0;
  }
