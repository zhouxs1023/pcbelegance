...

{
  ...
  if (ObjectText[0]==0) lengte=0;
  else lengte=strlen(ObjectText);
...
{
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[1024],str4[MAX_LENGTH_STRING],str5[MAX_LENGTH_STRING],LineBuf[1024];
  int32 fp,Length,hulp,GateNr,ok;
  GatePinSwapRecord *GatePinSwap;

  GateNr=0;
  sprintf(str,"%s\\pcb\\gatepin.swp",DesignPath);
  if (FileExistsUTF8(str)!=0) return -1;
...
/*
GATE    U203         0    0   SRAM32KX8_H
GATE    RP602        7    3   1k
GATE    reference    partnr  GateCode  value
*/
        GateNr=0;
...
/*
PINBUS  16 '37,36,35,34,33,32,100,99\82,81,44,45,46,47,48,49'  0  A[0:15]
*/
            if (NrGatePinSwaps==MaxNrGatePinSwaps) {
              AllocateMemGatePinSwaps(MaxNrGatePinSwaps+16);
            }
...
{
  int32  cnt,cnt2,cnt3,cnt4,res,GateNr,CurrentGateNr,CurrentGateNr2,GateNrCount,
         PartNr,FirstCntGateNr,LastCntGateNr,FirstCntGateNr2,LastCntGateNr2,
         GateNr2,PartNr2,GateSpecialNr,DisplayMode,
         CurrentCntGateNr,CurrentCntGateNr2,PinNr,CompNr,GroupIndex[16],
         SwapInfo,PinGroupNr,CurrentPinGroupNr,CurrentPinGroupPos,PinGroupPos,
         PinGroupLine,CurrentPinGroupLine,
         SwapInfo2,CurrentPinGroupNr2,CurrentPinGroupPos2,
...
// *******************************************************************************************
  ...
  if ((mode & 4) == 0) {
    ...
    if (GatePinSwapReference[0]==0) return -1;
  }
  GateSpecialNr=0;
  PinStrPos1=SrcPins;
  PinStrPos2=DestPins;
  ...
  for (cnt=0;cnt<NrGatePinSwaps;cnt++) {
    ...
    if (GatePinSwap->ObjectType==1) {
      ...
      if ((!Found)
         &&
         (stricmpOwn(RefName,GatePinSwapReference)==0)) {
        ...
        if (GatePinSwap->ObjectType==3) {
          ...
          for (cnt4=0;cnt4<GatePinSwap->Info3;cnt4++) {
            if ((res=GetPinNameFromPinBus(GetObjectText(GatePinSwap->Info2),
                                          str,GatePinSwap->Info3,cnt4))>=0) {
  ...
  if ((mode & 2) == 2) {
    ...
    for (cnt=0;cnt<NrGatePinSwaps;cnt++) {
      ...
      if (GatePinSwap->ObjectType==1) {
        ...
        if ((!Found2)
           &&
           (stricmpOwn(RefName,Reference2)==0)) {
          if (GatePinSwap->ObjectType==2) {
...
// Swap pins inside a component
  if (CurrentGateNr==0) {
    ...
    if ((mode & 2) == 2) {
      if ((CurrentGateNr!=CurrentGateNr2)
         ||
         (CurrentPinGroupNr!=CurrentPinGroupNr2)
         ||
         (CurrentPinGroupLine==CurrentPinGroupLine2)
         ||
         (CurrentPinGroupPos!=CurrentPinGroupPos2)) return -3;
    } else {
...
// *******************************************************************************************
  ...
  if ((CurrentGateNr!=CurrentGateNr2)
     ||
     (FirstCntGateNr==FirstCntGateNr2)) return -3;
...
{
  int32  cnt,cnt2,cnt3,cnt4,res,GateNr,PartNr,
         FirstCntGateNr,LastCntGateNr,
         CurrentCntGateNr,CurrentCntGateNr2,PinNr,CompNr;
  GatePinSwapRecord *GatePinSwap;
  int32  SwapInfo,PinGroupNr,CurrentPinGroupNr,CurrentPinGroupNr2,
         CurrentPinGroupPos,PinGroupPos,PinGroupLine,CurrentPinGroupLine,
         CurrentPinGroupPos2,CurrentPinGroupLine2;
  ...
  if ((mode & 4) == 0) {
    if ((mode & 2) == 2) {
      if (stricmpOwn(Reference,Reference2)!=0) return -1;
      if (stricmpOwn(GatePinSwapPinName,PinName2)==0) return -1;
    }
    strcpy(GatePinSwapReference,Reference);
    strcpy(GatePinSwapPinName,PinName);
  } else {
  ...
  GatePinSwap=&((*GatePinSwaps)[CurrentCntGateNr]);
  if (GatePinSwap->SwapInfo==0) return -2;
  if ((GatePinSwap->SwapInfo & 1)==0) return -3;
...
// *******************************************************************************************
  ...
  if ((mode & 2) == 2) {
    ...
    if (GatePinSwap->SwapInfo==0) return -2;
    if ((GatePinSwap->SwapInfo & 1)==0) return -3;
    if ((CurrentPinGroupNr!=CurrentPinGroupNr2)
       ||
       (CurrentPinGroupLine!=CurrentPinGroupLine2)
       ||
       (CurrentPinGroupPos!=CurrentPinGroupPos2)) return -4;
  ...
  if ((mode & 2) == 0) {
    ...
    for (cnt=FirstCntGateNr;cnt<LastCntGateNr;cnt++) {
      ...
      if ((GatePinSwap->ObjectType>1)
         ...
         (PinGroupLine==CurrentPinGroupLine)
         &&
         (PinGroupPos==CurrentPinGroupPos)
         &&
         (PinSwappable)) {
        if (GatePinSwap->ObjectType==2) {
          PinStr=GetObjectText(GatePinSwap->Info2);
...

{
  ...
  PadRecord *Pad;
  int32  cnt2,MemPos,NrPins,PinsMemPos,PinOffset,PolygonVertices,ShapeType,ShapePos,
         Mirror,Layer,MemPosComp,NrPinShapes;
...
int32 SwapPinFunction(LPSTR Reference1,LPSTR PinName1,
                      LPSTR Reference2,LPSTR PinName2,
                      LPSTR PinStrPos1,LPSTR PinStrPos2)

{
  char  str[500],str1[500],str2[500],str3a[500],str4a[500],str4b[500],str5[500],
        str6a[500],str6b[500],PinBusLabel1[MAX_LENGTH_STRING],PinBusLabel2[MAX_LENGTH_STRING];
  int32 cnt,le1,le2,NrPins,res,ok,CompNr1,CompNr2,NetNr1,NetNr2,PinBusMode,SwapMode,
        PartNr1,PartNr2,NrPins2;
...
*/
    ...
    if (stricmpOwn(str3a,"PINBUS")==0) {
      ...
      GetString2(str1,str5,500);
      if ((res=sscanf(str5,"%i",&NrPins))!=1) return -5;
      NrPins2=NrPins;
      GetString2(str1,str5,500);
      ...
      for (cnt=0;cnt<NrPins;cnt++) {
        ...
        if (((NetNr1>=0)
          ...
          if ((PinBusMode==0)
             &&
             (SwapMode==0)) {
            sprintf(str,"SWAP PINBUS0 %s %i %s PIN %s WITH %s",Reference1,NrPins2,PinBusLabel1,str6a,str6b);
            WriteLn(fp,str);
#ifdef _DEBUG
            strcat(str,"\n");
            OutputDebugString(str);
...

{
  ...
  if (res>=0) {
    if (GatePinSwapMode) {
      res=-1;
      if (GateSwapResult>=0) {
        res=GetGateSwapInfo(GatePinSwapReference,GatePinSwapPinName,
                            ReferenceStr,PinStr,(LPSTR)GatePinSwapBuf,(LPSTR)&GatePinSwapBuf[131072],2);
        if (res==0) {
          OkToSwap=1;
          GatePinSwapMode=0;
        }
      ...
      if ((PinSwapResult>=0)
         &&
         (res!=0)) {
        res2=GetPinSwapInfo(GatePinSwapReference,GatePinSwapPinName,
                            ReferenceStr,PinStr,(LPSTR)GatePinSwapBuf,(LPSTR)&GatePinSwapBuf[131072],2);
        if (res2==0) {
          OkToSwap=1;
          GatePinSwapMode=0;
      ...
      ok=1;
      if (OkToSwap) {
        SwapPinFunction(GatePinSwapReference,GatePinSwapPinName,
                        ReferenceStr,PinStr,
                        (LPSTR)GatePinSwapBuf,(LPSTR)&GatePinSwapBuf[131072]);
      }
      if (!GatePinSwapMode) {
        OkToSwap=0;
      ...
      if (res==1) {
        ...
        if ((GateSwapResult>=0)
           ...
           (PinSwapResult>=0)) {
          GatePinSwapMode=1;
          sprintf(str,SC(394,"Gate/pin swap  [ Component %s-%s ]"),GatePinSwapReference,GatePinSwapPinName);
          if (CurrentPartNr>0) {
