...
int32                       DrawWindowMinX,DrawWindowMinY,DrawWindowMaxX,DrawWindowMaxY,
                            TimerInfoStr,ScreenSizeX,ScreenSizeY,
                            ScrollBarSizeX,ScrollBarSizeY,ScreenStartX,ScreenStartY,
                            HeightScrollBar,WidthScrollBar,
                            OldWindowWidth,StartTimerValue,
                            ScreenPosAbsCursor,ScreenPosAbsGridCursor,
                            MousePosOldX,MousePosOldY,
...
char                        IniFile[MAX_LENGTH_STRING],CurrentDir[MAX_LENGTH_STRING],
                            StartDir[MAX_LENGTH_STRING],ExecutableDir[MAX_LENGTH_STRING],
                            GeomExecutable[MAX_LENGTH_STRING],
                            ProjectPath[MAX_LENGTH_STRING],LibraryFile[MAX_LENGTH_STRING];
...
{
  InitDeviceContext();
  EditingRegion = CreateRectRgn(DrawWindowMinX,DrawWindowMinY,
...
{
  if ( DCInUse ) {
    if ( Painting ) EndPaint(GEOMWindow, &PS);
    else ReleaseDC(GEOMWindow, OutputDisplay);
    DCInUse=0;
    OutputDisplay=NULL;
  }
}
...
//    res=GetDIBits(OutputDisplay,ViewBitmap,0,ViewPixelsY,TempMem,BitmapInfo,DIB_RGB_COLORS);
    ...
    if ( Painting ) EndPaint(GEOMWindow, &PS);
    else ReleaseDC(GEOMWindow, OutputDisplay2);
    DCInUse=0;
    ViewBitmap=NULL;
    OutputDisplay=NULL;
    OutputDisplay2=NULL;
...
//      SelectClipRgn(PCBDisplay, NULL);
  ...
  FillRect(OutputDisplay,&Rect,GetStockObject(LTGRAY_BRUSH));
  TextOut(OutputDisplay,ScreenPosAbsCursor,ClientRect.bottom-(HeightInfoBar-4),(LPCSTR)&AbsPosStr,strlen(AbsPosStr));
...
//      SelectClipRgn(PCBDisplay, NULL);
  ...
  Rect.bottom=RightBottomY;
  FillRect(OutputDisplay,&Rect,GetStockObject(LTGRAY_BRUSH));
  TextOut(OutputDisplay,ScreenPosInfoStr+2,ClientRect.bottom-(HeightInfoBar-4),(LPCSTR)&InfoStr,strlen(InfoStr));
  SelectObject(OutputDisplay,SaveFont);
...

{
  ...
  if ( GetUpdateRect(GEOMWindow,&UpdateRect,0) ) {
    ...
    if (!PaintBecauseOfResize) {
      ...
      if (UpdateRect.bottom>=DrawWindowMaxY+HeightScrollBar) RedrawInfoBar();
      if (UpdateRect.left<DrawWindowMinX) RedrawButtons();
      if ((UpdateRect.bottom>=DrawWindowMinY)
         &&
         (UpdateRect.top<DrawWindowMaxY)) {
        RedrawMainWindow();
      }
      if (FastPaint) {
        DoneDeviceContext2();
...
{
  int32 x,y,res,IntroBitmapSizeX,IntroBitmapSizeY,IntroBitmapSizeX2,IntroBitmapSizeY2,
        ScreenSizeX,ScreenSizeY,x2,y2;
  HBITMAP IntroBitmap;
...
//              MessageBoxUTF8(NULL,"Send size",SC(1,"Error"),MB_APPLMODAL|MB_OK);
...
//      Focused=0;
    ...
    case WM_CREATE:
      ...
      break;
    case WM_COMMAND:
      if (HIWORD(LParam)==0) GEOMCommand((int32)WParam,(int32)LParam);
      break;
    case WM_PAINT:
      if (FirstPaint) {
        FirstPaint=0;
...
//      MenuItemSize->itemHeight=40;
    ...
    case WM_MOUSEMOVE:
      ...
      if (GetCursor()!=SystemCursor) SetCursor(SystemCursor);
      break;
...
//          LeftPressedOk=1;
    ...
    case WM_PALETTECHANGED:
      if ((HWND) WParam == GEOMWindow) return 0;
    ...
    case WM_QUERYNEWPALETTE:
      ...
      if (i > 0) InvalidateRect(GEOMWindow, NULL, 1);
      return i;
    case WM_SETFOCUS:
      Focused=1;
...
//              MessageBoxUTF8(NULL,str,SC(1,"Error"),MB_APPLMODAL|MB_OK);
...
{
  int32  fp,cnt,Length,ParamMode,Value,ok,Key,
         OldWindowStartX,OldWindowStartY,OldWindowWidth,OldWindowHeight;

  char   LineBuf[512],str2[MAX_LENGTH_STRING],str1[MAX_LENGTH_STRING],
        ...
        switch (ParamMode) {
          case 3:
            if (GetStringValue(str4,str1,str2)) {
              ...
              if (stricmp(str1,SC(250,"DefaultRulePad"))==0) {
                if (sscanf(str2,"%f",&Value1)==1) {
...

{
  ...
  if (WindowInitialSized) return;
//  if (MasterWindow!=NULL) return;
  if (LibraryFile[0]!=0) return;
  if (FoundDesignPath) {
  ...
  if (IniFile[0]==0) return;
  if ((fp=FileOpenWriteUTF8(IniFile))<0) return;
...
*/
  ...
  WriteLn(fp,str1);
  sprintf(str1,SC(251,"DefaultRulePad=%.1f"),DefaultRulePad);
  WriteLn(fp,str1);
...

{
  ...
  if ((ID<0)
     ||
     (ID>=MaxNrGeomNames)) return -1;
  if (GeomNamesPos+Length+2>=GeomNamesBufSize) return -1;
...

{
  ...
  char   LineBuf[512],str2[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],
         LanguageFileName[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],
         str6[MAX_LENGTH_STRING];
...
/*
    ...
    MessageBoxUTF8(NULL,str,SC(218,"Warning"),MB_OK);
*/
    return -1;
  }
  ...
  while ((Length=ReadLn(fp,LineBuf))>=0) {
    ...
    if ((Length>1)
      ...
      if (LanguageFileName[0]==0) {
        ...
        sprintf(LanguageFileName,"%s\\%s",ProjectPath,str);
      }
    }
  }
  TextFileClose(fp);
    ...
    if (Parameters[cnt].Option[0]!=0) {
      ...
      if ((mode==0)
        ...
        if (ConvertFileNameFromCommandLineUTF8(Parameters[cnt].Parameter,EditFile)==-1) {
          MessageBoxUTF8(GEOMWindow,Parameters[cnt].Parameter,
...
//  MessageBox(NULL,DesignPath,"Design path",MB_APPLMODAL+MB_OK);
  ...
  if (AddGeomLanguageStrings(str)==-2) return -1;
...
//      GEOMClass.lpszMenuName= "IDR_MENU1";
    if ( ! RegisterClass(&GEOMClass) ) exit(255);
  }
...
// Creatoolbarex
// CreateStatusWindow
  GEOMWindow = CreateWindow(GEOMClass.lpszClassName,
                            NULL,
                            WS_OVERLAPPEDWINDOW,
//                            WS_OVERLAPPEDWINDOW | WS_HSCROLL | WS_VSCROLL,
  ...
  hwndHBar = CreateWindow ("scrollbar", NULL, WS_CHILD | WS_VISIBLE | SBS_HORZ,
                           0,0,0,0, GEOMWindow, (HMENU) 1, hInstance, NULL);
  hwndVBar = CreateWindow ("scrollbar", NULL, WS_CHILD | WS_VISIBLE | SBS_VERT,
                           0,0,0,0, GEOMWindow, (HMENU) 2, hInstance, NULL);
...
//      if ( exit22 ) PostQuitMessage(0);
  ...
  if ( Created ) DestroyWindow(GEOMWindow);
  ClipCursor(NULL);
