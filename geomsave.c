...

{
  if (BufPos+MemSize+64*1024>MaxTempMemory) {
    if (AllocateMemTemp(BufPos+MemSize+64*1024)!=0) return -1;
  }
...

{
  ...
  if (NrGeomPolygons>=MaxGeomPolygons) {
    if (AllocateMemTemp3((MaxGeomPolygons+4096)*sizeof(int32))!=0) return NULL;
  ...
  if ((AreaFill->NrPolygons>1)
     ||
     (DrawPolygon->NrVertices>500)) {
    SpecialPolygon=1;
  }
  MemSize=AreaFill->MemSize+16384;
  if (MaxGeomPolygonsMem+MemSize>MaxTempMemory2) {
    if (AllocateMemTemp2(MaxGeomPolygonsMem+MemSize)!=0) return NULL;
...
GeomPolygonRecord *AddObjectPolygonAsGeomPolygon(ObjectPolygonRecord *ObjectPolygon,
...
GeomPolygonRecord *AddGeomPolygonAsGeomPolygon(GeomPolygonRecord *GeomPolygon,
                                               double x1,double y1,double Rotation,int32 Mirror,
                                               double *OffsetX,double *OffsetY)
...

{
  ...
  if ((InRange9(Object->x1,39.3e5))
     &&
     (InRange9(Object->y1,54.6e5))) {
    ok=1;
  }
#endif
  ...
  switch (ObjectType) {
    case AREAFILL:
      ...
      if ((NewBufPos=WriteToShapeBuf((uint8 *)&Pad,BufPos,ObjectSize))==-1) return -1;
      BufPos=NewBufPos;
      NrPinShapesSaved++;
    ...
    case PIN_SMD_POLYGON:
      ...
      if (Object->ObjectType2==0) {
...
#endif
        NewGeomPolygon=AddGeomPolygonAsGeomPolygon(GeomPolygon,Object->x1,Object->y1,
                                                   Object->RotationAngle,Object->Mirror,
                                                   &OffsetX,&OffsetY);
      } else {
#ifdef _DEBUG
        if (NrPins==231) {
        ...
        if ((InRange9(Object->x1,360.7e5))
           &&
           (InRange9(Object->y1,259.6e5))) {
          ok=1;
        }
#endif
        ...
        NewGeomPolygon=AddGeomPolygonAsGeomPolygon(&SpecialGeomPolygon,Object->x1,Object->y1,
                                                   Object->RotationAngle,Object->Mirror,
                                                   &OffsetX,&OffsetY);
      }
      Pad.X=(float)(-OffsetX);
    ...
    case PIN_PUT_THROUGH_POLYGON:
      ...
      if (Object->ObjectType2==0) {
        ...
        NewGeomPolygon=AddGeomPolygonAsGeomPolygon(&SpecialGeomPolygon,0.0,0.0,
                                                   Object->RotationAngle,Object->Mirror,
                                                   &OffsetX,&OffsetY);
    ...
    case TRACE_ARC:
      ...
      BufPos=NewBufPos;
      ObjectSize=40;
      if ((NewBufPos=WriteToShapeBuf((uint8 *)&ShapeLines[2],BufPos,ObjectSize))==-1) return -1;
      BufPos=NewBufPos;
      NrPinShapesSaved++;
      Object->Info|=OBJECT_DONE;
      break;
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDC_CHECK1                 ,SC(1126,"Copy component references on silkscreen"));
      SetDialogItemText(Dialog,IDC_CHECK2                 ,SC(1127,"Copy component values on silkscreen"));
      SetDialogItemText(Dialog,IDC_CHECK3                 ,SC(1128,"Convert board outline to component outline"));
      SetDialogItemText(Dialog,IDC_CHECK4                 ,SC(1129,"Soldermask on vias"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(1130,"Start text geometry pins"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetWindowTextOwn(Dialog,SC(1131,"Save layout as a geoemtry"));
      if (StartPinText[0]==0) {
      ...
      SetDialogItemText(Dialog,IDC_EDIT1                  ,StartPinText);
      return about;
    case WM_MOVE:
      break;
    case WM_COMMAND:
...
{
  int32 NetNr,cnt,cnt2,cnt3,cnt4,cnt5,Layer,ObjectType,LineSegments,PolygonPos,
        res,SegmentCount,ObjectSize,ObjectInfo,fp,Written,MaxCountX,NrLines,
        *Int32P,Mirror,Length,PinNr,FoundAreaFill,
        ViaType,ShapePadBufPos,*MemSizeObjectPolygons,*ShapeInfoP,ok;
  ...
  for (cnt=0;cnt<Design.NrAreaFills;cnt++) {
    ...
    if ((AreaFill->Info & (OBJECT_NOT_VISIBLE|POWERPLANE)) == POWERPLANE) {
      MessageBoxOwn(PCBWindow,SC(1025,"Powerplanes are not supported"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
    }
  }
  ...
  if (AllocateMemTemp2(256*1024)!=0) return -1;
  MaxGeomPolygonsMem=MaxTempMemory2;
  GeomPolygonsMem=TempMem2;

  if (AllocateMemTemp3(4096*sizeof(int32))!=0) return -1;
          ...
          case OBJECT_LINE:
            if (Object->Info2!=0) {
              ...
              for (cnt2=0;cnt2<LineSegments;cnt2++) {
                ...
                ObjectSize=24;
                if ((NewBufPos=WriteToShapeBuf((uint8 *)&ShapeLines,BufPos,ObjectSize))==-1) return -1;
                BufPos=NewBufPos;
  ...
  for (cnt=0;cnt<Design.NrComps;cnt++) {
    ...
    if ((Comp->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      for (cnt2=0;cnt2<NrObjects;cnt2++) {
        ...
        Layer=Object->Layer;
        if ((Layer==COMP_OUTLINE_LAYER_TOP)
           ||
           (Layer==COMP_OUTLINE_LAYER_BOTTOM)
           ||
           ((GeomSaveOptions & 4)
           &&
...
#ifdef _DEBUG
      if ((InRange9(ObjectArc->CentreX,167.6e5))
         &&
         (InRange9(ObjectArc->CentreY,-4.9e5))) {
        ok=1;
      }
#endif
      LayerOk=0;
    ...
    for (NetNr=0;NetNr<Design.NrNets;NetNr++) {
      ...
      for (cnt=0;cnt<NrObjects2;cnt++) {
        ...
        if ((Object->ObjectType==AREAFILL)
           ||
           (CheckObjectIsBigPolygon(Object))) {
      ...
      if (((cnt5==0)
        ...
        if ((NewBufPos=WriteToShapeBuf((uint8 *)&ShapePad,BufPos,ObjectSize))==-1) return -1;
        BufPos=NewBufPos;
        for (cnt=0;cnt<NrObjects2;cnt++) {
          Object=&((*Objects2)[cnt]);
...
//        case DRILL_LAYER:
      ...
      if (LayerOk) {
        ...
        NrPins++;
#ifdef _DEBUG
        if ((InRange9(ObjectArc->CentreX,165.7e5))
           &&
           (InRange9(ObjectArc->CentreY,-0.3e5))) {
          ok=1;
        }
#endif
        memset(&Pad,0,sizeof(PadRecord));
      ...
      if ((Layer>=0)
        ...
        if ((NrLines=ConvertObjectTextToStrings(ObjectText2->Text,0,&MaxCountX,ObjectText2->Layer))!=0) {
          ...
          for (cnt3=0;cnt3<NrLines;cnt3++) {
            LineSegments=TextStringToLineSegments2(x2,y2,x3,ObjectText2->Rotation,0,Mirror,
                                                   TextStrings2[cnt3],(double *)&LineBuf);
  ...
  for (cnt=0;cnt<Design.NrComps;cnt++) {
    ...
    if ((Comp->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      for (cnt2=0;cnt2<NrObjects;cnt2++) {
        Object=&((*Objects)[cnt2]);
#ifdef _DEBUG
        if ((InRange9(Object->x1,150.5e5))
           &&
           (InRange9(Object->y1,108.1e5))) {
          ok=1;
            ...
            case OBJECT_TEXT:
              switch (Layer) {
                ...
                case SILKSCREEN_BOTTOM:
                  ...
                  BufPos=NewBufPos;
                  ObjectSize=64;
                  if ((NewBufPos=WriteToShapeBuf((uint8 *)&ShapeLines[7],BufPos,ObjectSize))==-1) return -1;
                  BufPos=NewBufPos;
                  Shape.NrOtherObjects++;
                  break;
                default:
...
// **********************************************************************************
  ...
  cnt2=MaxTempMemory3;
  if (AllocateMemTemp3(MaxTempMemory3+(NrGeomPolygons+16)*sizeof(int32))!=0) return -1;
...
// MaxGeomPolygons; NrGeomPolygons,MaxGeomPolygonsMem; GeomPolygonsMemSize;
  ...
  for (cnt=0;cnt<NrGeomPolygons;cnt++) {
    ...
    cnt2++;
    if ((NewBufPos=WriteToShapeBuf((uint8 *)GeomPolygon,BufPos,ObjectSize))==-1) return -1;
    BufPos=NewBufPos;
  }
