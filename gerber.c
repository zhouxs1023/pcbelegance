...
AperTureRecord LastRoundPadAperTure,LastRectPadAperTure,
...
int32          MaxNrAperTures,NrAperTures,NrDrillAperTures,
               GerberLineairInterpolation,
...
extern double  LastGerberX,LastGerberY,CurrentPlotPenSize,
...

{
  ...
  if (NrGerberLayers>0) {
    ...
    sprintf(str,SC(398,"Penplot output ready\n\nHPGL files are located in directory %s\\pcb\\hpgl"),DesignPath);
...

{
  ...
  if (GerberDialog(0)==1) {
    ...
    sprintf(str,SC(403,"Gerber/drill output ready\n\nOutput files are located in directory %s\\pcb\\gerber"),
            DesignPath);
    MessageBoxOwn(PCBWindow,str,SC(1,"Message"),MB_APPLMODAL|MB_OK);
    SetNormalCursor();
  }
...
{
  int32  Layer,ok,cnt,cnt2,cnt3,MemSizeAreaFill,Mirror,
         ApertureMacroPolygons,AperturePolygonUsageCount,NrTraceObjects,
         count,DCode,CompOutlineLayers,Layer2,
         NrCheckObjects,ButterFlyMacroWritten,TempUnits,
         res,NrTempGerberLayers,TempGerberLayers[64],ApertureIndex[2048],
         PowerPlaneLayer,UseMacros,WriteThermalReliefs;
  ...
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
        str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],FileStr[MAX_LENGTH_STRING];
...
// ****************************************************************************
    if (!PenPlotMode) {
      ...
      sprintf(str,"G04 *");
      WriteGerberString(str,1);
      if ((mode & 2)==0) {  // RS274D gerber mode
        sprintf(str,"G90*");
        WriteGerberString(str,1);
        sprintf(str,"G70*");
        WriteGerberString(str,1);
    ...
    if ((mode & 2)==2) {  // RS274X gerber mode
      DeleteAperTures();
      NrLoadedAperTures=0;
      DcodeRoundPad=GERBER_PAD_ROUND;
      DcodeRectPad=GERBER_PAD_RECT;
...
*/
              if (AperTure->SpecialType==0) {
                ...
                while ((cnt2<AperturePolygonUsageCount)
                      ...
                      ||
                      (AperturePolygonUsage[cnt2].Address!=AperTure->Address))) cnt2++;
                if ((cnt2==AperturePolygonUsageCount)
                   &&
                   (cnt2<MAX_APERTURE_POLYGON_DEFS)) {
              ...
              switch (TempUnits) {
                case UNITS_INCH:
                  sprintf(str2,"%.6fX%.6fX%.6f",ConvertToUnits(UNITS_INCH,AperTure->y),
                                                ConvertToUnits(UNITS_INCH,AperTure->x),
                                                ConvertToUnits(UNITS_INCH,AperTure->x2));
                  break;
                case UNITS_MM:
                  sprintf(str2,"%.4fX%.4fX%.4f",ConvertToUnits(UNITS_MM,AperTure->y),
                                                ConvertToUnits(UNITS_MM,AperTure->x),
                                                ConvertToUnits(UNITS_MM,AperTure->x2));
                  break;
              }
...
// Plot addtional polygons
    ...
    for (cnt=0;cnt<Design.NrObjectPolygons;cnt++) {
      ...
      if (((ObjectPolygon->Info & (OBJECT_NOT_VISIBLE)) == 0)
        ...
        memmove(&(*Polygon).Points,&(*ObjectPolygon).Points,
