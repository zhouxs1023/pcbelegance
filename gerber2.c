...
extern  double    LastGerberX,LastGerberY,AreafillPenSize1,AreafillPenSize2,
                  PowerPlanePenSize1,PowerPlanePenSize2;

extern  int32     Gerberfp,Drillfp,NrCalcs1,NrCalcs2,NrCalcs3,Plotfp,
                  CurrentAperTureNr,FillAperTureNr,GerberLineairInterpolation,
                  DcodeTrace,DcodeRoundPad,DcodeRectPad,DcodeDrill,DcodeThermalRelief;


extern  AperTureArray  *AperTures,*AperTures2,*DrillAperTures;

double   DesignBoardOriginX,DesignBoardOriginY,DesignBoardWidth,DesignBoardHeight,
         CurrentPlotPenSize;
...
// X038803Y076503D02*X039797Y076503D01*
  ...
  char  str[MAX_LENGTH_STRING],Prefix[MAX_LENGTH_STRING],StrX[MAX_LENGTH_STRING],StrY[MAX_LENGTH_STRING],StrX2[MAX_LENGTH_STRING],StrY2[MAX_LENGTH_STRING];
...
//          res=PlotAreaFillToGerber(AreaFill,AreafillPenSize1,AreafillPenSize2,1);
          ...
          if ((Object->ObjectType2!=0)
            ...
            GerberWriteLine(PolygonObject->Points[0].x,PolygonObject->Points[0].y,
                            PolygonObject->Points[1].x,PolygonObject->Points[1].y);
            GerberWriteLine(PolygonObject->Points[1].x,PolygonObject->Points[1].y,
                            PolygonObject->Points[2].x,PolygonObject->Points[2].y);
            GerberWriteLine(PolygonObject->Points[2].x,PolygonObject->Points[2].y,
                            PolygonObject->Points[3].x,PolygonObject->Points[3].y);
            GerberWriteLine(PolygonObject->Points[3].x,PolygonObject->Points[3].y,
                            PolygonObject->Points[0].x,PolygonObject->Points[0].y);
          } else {
    ...
    switch (Object->ObjectType) {
      case DRILL:
      case DRILL_UNPLATED:
        res=DrawGerberCircleOpenPad(x1,y1,min(Object->x2,120000.0),
                                    50000.0,CurrentPlotPenSize);
...
int32 DrawGerberCirclePad(double x1,double y1,
...

{
  ...
  x4=0.0;
  y4=0.0;
  if (PenSize>ThickNess+10.0) return -1;
  ...
  while (!Stop) {
    ...
    if (CircleDiam<0.0) CircleDiam=0.0;
...

{
  ...
  if (PenPlotMode) {
    if (PenSize>PadSizeX+10.0) return -1;
    if (PenSize>PadSizeY+10.0) return -1;
  }
  ...
  while (!Stop) {
    ...
    RectX-=PenSize*(0.8);
    RectY-=PenSize*(0.8);
    if (RectX<0.0) RectX=0.0;
    if (RectY<0.0) RectY=0.0;
    cnt++;
  }
  return 0;
}
...

{
  ...
  if (PenPlotMode) {
    if (CurrentPlotPenSize>Object->x2+10.0) return -1;
    if (CurrentPlotPenSize>Object->Thickness+10.0) return -1;
  }

  NrSegments=32;
  if (Object->x2>(400*2540)) NrSegments=64;
  if (Object->x2>(1000*2540)) NrSegments=128;

  CircleMode=(int32)(Object->y2+0.1);

  LineSegments=CircleToLineSegments(Object->x1,Object->y1,Object->x2,
                                    CircleMode,(double *)&LineBuf);
  SegmentCount=0;
...
// Object->y2   Drawing thickness
  ...
  if (PenPlotMode) {
    if (CurrentPlotPenSize>Object->y2+10.0) return -1;
...

{
  ...
  if (ThickNess<PenSize*1.91) NrRounds=2;

  if (PenPlotMode) {
    if ((ThickNess>PenSize*0.9)
       &&
       (ThickNess<PenSize*1.1)) {
      GerberWriteLine(x1,y1,x2,y2);
      return 0;
    }
  } else {
  ...
  Step=4;
  if (NrRounds>4) Step=2;
  if (NrRounds>8) Step=1;

// *************************************************************************
// *************************************************************************
  if (InRange3(x1,x2)) {  // Vertical
    x1a=x1;
    y1a=min(y1,y2);
    y2a=max(y1,y2);
    CircleDiam=(ThickNess-PenSize)*0.5;
    ...
    while (cnt>0) {
      ...
      if (cnt>1) {
        ...
        if (cnt-2>0) {
          ...
          if (CircleDiam<0) CircleDiam=0.0;
          x4=x1a+CircleDiam;
          y4=y2a;
          GerberWriteLine(x3,y3,x4,y4);
        }
...

{
  ...
  if ((InRange4(x1,133.07e5))
     &&
     (InRange4(y1,75.83e5))
     &&
     (InRange4(x2,133.27e5))
     &&
     (InRange4(y2,75.99e5))) {
    ok=1;
  }
...
int32 CheckLineCrossesCircle2(double x1,double y1,
                              double x2,double y2,
                              double cx,double cy,
                              double ThickNess)
...
typedef uint8 ByteArray[1000];
  ...
  int32  cnt,cnt2,cnt3,cnt4,res,res2,count,count2,ok,NrLines,NrSegmentLines,
         FirstCutOutNr,MaxRoundings,NrPoints,AperTureNr,NrShortenLines,MemSize,
         Found,LineOn,FoundFirstLine,AreaFillChanged,
         DrawWithTwoPens,LineShorten;
  double *x1,*y1,*x2,*y2,*FirstX,*FirstY,*LinesBuf,*LinesBufCopy,LineMinX,LineMaxX,
         PointX,PointY,x3,y3,x4,y4,minx,miny,maxx,maxy,LineX1,LineX2,Thickness_1,
         NewThickNess,x33,y33,R,
         xx1,yy1,xx2,yy2,SearchMinY,SearchMaxY,IncY,PaintY,
         x11,y11,x22,y22;
  ...
  if ((PenPlotMode)
    ...
    for (cnt=0;cnt<Design.NrVias;cnt++) {
      ...
      if (((Via->Info & (OBJECT_NOT_VISIBLE)) == 0)
        ...
        if ((AreaFill->maxx>=Object->minx)
           &&
           (AreaFill->minx<=Object->maxx)
           &&
           (AreaFill->maxy>=Object->miny)
           &&
           (AreaFill->miny<=Object->maxy)) {
...
//  AreaFillToGerber->NrPolygons=1;
  cnt2=0;
  SelectionEsc=0;
  while ((cnt2<AreaFillToGerber->NrPolygons)
        &&
        (!SelectionEsc)) {
...
//  for (cnt2=0;cnt2<AreaFillToGerber->NrPolygons;cnt2++) {
  ...
  */
  ...
  #endif
        sprintf(InfoStr,"%s  %i (%i)   %d (%i)",str2,cnt2,AreaFillToGerber->NrPolygons-1,cnt4,MaxRoundings);
        RedrawInfoStr(1);
...
*/
            LineShorten=0;
            if ((res=CheckLineCrossesWithAreaFill(x11,y11,x22,y22,AreaFillToGerber,&xx1,&yy1,&xx2,&yy2,
                                                  Thickness1*0.99,0))!=-1) {
              if (res==0) {
                xx1=x11;
                yy1=y11;
...
/*
  ...
  */
            ...
            if (res!=0) {
              if ((res2=CheckLineCrossesWithAreaFill(x22,y22,x11,y11,AreaFillToGerber,&xx1,&yy1,&xx2,&yy2,
                                                    Thickness1*0.99,0))!=-1) {
                if (res2==0) {
                  xx1=x11;
                  yy1=y11;
...
//    RedrawInfoStr(1);
  ...
  if (SelectionEsc) {
    MessageBoxOwn(PCBWindow,SC(423,"Plotting areafill has been stopped"),SC(102,"Warning"),MB_APPLMODAL|MB_OK);
  }
...
//  cnt4=NrLines;
  ...
  while ((cnt4<NrLines)
    ...
    for (cnt=0;cnt<AreaFillToGerber->NrPolygons;cnt++) {
      ...
      if ((Polygon->maxy+100>SearchMinY)
         &&
         (Polygon->miny-100<SearchMaxY)) {
        ...
        for (cnt2=0;cnt2<count;cnt2++) {
          ...
          if ((min(*y1,*y2)<SearchMaxY)
             &&
             (max(*y1,*y2)>SearchMinY)
             &&
             (NrSegmentLines<8192)) {
            *LinesBuf++=*x1;
            *LinesBuf++=*y1;
            ...
            if (FoundFirstLine) {
              ...
              if (CheckPointInAreaFill(LineX1,PointY,AreaFillToGerber)==1) {
                ...
                if (!PenPlotMode) {
                  ...
                  if (!DrawWithTwoPens) {
                    DrawGerberLine(LineX1,PointY,LineX2,PointY,
...
{
  int32  cnt,cnt2,cnt3,count,ok,NrLayerPolygons,
