...
HBRUSH                      GraphicsObjectBrush[192],
                            EmptyBrush;
HPEN                        GraphicsObjectPen[192],
                            EmptyPen;
...
{
  HPEN   NewPen;
  int32  CurrentThickness,CurrentGraphicsObjectNr,
         NewColor2,NewPenMode,NewBrushMode,GraphicsPenObjectNr,GraphicsBrushObjectNr,
         NewGraphicsObjectCode,CurrentGraphicsObjectCode,res,
         ChangePen,ChangeBrush,ModifyPen,ModifyBrush;
#ifdef _DEBUG
  LOGPEN  *CurrentPenObject;
  LOGBRUSH *CurrentBrushObject;
  HGDIOBJ CurrentBrush,CurrentPen;
...
//  CurrentThickness=CurrentGraphicsObjectCode & 0xFFF;
  ...
  if ((GraphicsObjectNr==CurrentGraphicsObjectNr)
     &&
     (CurrentObjectCode==NewGraphicsObjectCode)
     &&
...
// ****************************************************************************************************
  ...
  if ((((CurrentGraphicsObjectCode >> 20) & 0x0F) != PenMode)
     ||
     (CurrentThickness!=ThickNess)
     ||
  ...
  if (ModifyPen) {
    ...
    switch (PenMode) {
      ...
      case 2: // Dotted pen
        ...
        if (GraphicsObjectPen[GraphicsObjectNr]==NULL) { // Create pen for the first time
          CreateNewPen(&GraphicsObjectPen[GraphicsObjectNr],ThickNess,
                       GraphicsObjectColor[GraphicsObjectNr],NewPenMode);
        } else { // Change pen
      ...
      case 1: // No pen
        ThickNess=1;
        NewPenMode=7;
        if (GraphicsObjectPen[EmptyPenObjectNr]==NULL) { // Create pen for the first time
          CreateNewPen(&GraphicsObjectPen[EmptyPenObjectNr],ThickNess,
                       GraphicsObjectColor[EmptyPenObjectNr],NewPenMode);
        }
        GraphicsObjectPenThickness[GraphicsObjectNr]=ThickNess;
        CurrentGraphicsObjectCode|=(1 << 20);
        break;
      case 3: // White pen
        ThickNess=1;
        NewPenMode=0;
        if (GraphicsObjectPen[WhitePenObjectNr]==NULL) { // Create pen for the first time
          CreateNewPen(&GraphicsObjectPen[WhitePenObjectNr],ThickNess,
                       RGB_White,NewPenMode);
        }
        GraphicsObjectPenThickness[GraphicsObjectNr]=ThickNess;
...
// ****************************************************************************************************
  ...
  if ((GraphicsObjectNr!=CurrentGraphicsObjectNr)
     ||
     (GraphicsObjectNr==ViewLayer2InNetObjectNr)) {
    ChangeBrush=1;
  }
  switch (BrushMode) {
    case 0: // Normal brush
    ...
    switch (BrushMode) {
      ...
      case 2: // Background brush
        ...
        if (GraphicsObjectBrush[GraphicsBrushObjectNr]==(HGDIOBJ)NULL) {
          CreateNewBrush(&GraphicsObjectBrush[GraphicsBrushObjectNr],
                         GraphicsBrushObjectNr,NewBrushMode);
        }
        switch (BrushMode) {
          case 0: // Normal brush
            break;
...
//      case DRAW_WITH_PEN_AND_BACKGROUND_BRUSH:
  ...
  switch (Layer) {
    case SHAPE_PINS_BOTTOM:
      if (((mode & 0x07) != HILITED_NORMAL_FILLED_AND_PEN1)
         &&
         ((mode & 0x07) != HILITED_NORMAL_FILLED_AND_NO_PEN)) {
    ...
    default:
      ...
      if ((Layer>=ROUTING_KEEPOUT_LAYER)
        ...
        if (Layer==ROUTING_KEEPOUT_LAYER) { // Bottom
          ...
          if (Layer==ROUTING_KEEPOUT_LAYER+Design.NrBoardLayers-1) { // top
...
//  DrawLine(Mult(x1-Xoffset),DrawWindowMaxY-Mult(y1-Yoffset)-1,Mult(x2-Xoffset),DrawWindowMaxY-Mult(y2-Yoffset)-1);
  ellips2(Mult(x-Xoffset)+DrawWindowMinX,DrawWindowMaxY-Mult(y-Yoffset)-1,
          Mult(dikte),Mult(dikte),255);
  ExitDrawing();
  EndDrawingEditingWindow();
}
