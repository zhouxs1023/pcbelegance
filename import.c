...
int32  *NrNetItemsP,MaxNrNets2,NrNets2,
       NrCurrentNetItems,NetPinsConnected,NetPinsNotConnected;
...

{
  ...
  for (cnt=0;cnt<Design.NrComps;cnt++) {
    ...
    if ((Comp->Info & OBJECT_NOT_VISIBLE) == 0) {
      switch (mode) {
        case 0:
          if (stricmpOwn(CompName,Comp->Name)==0) return cnt;
          break;
        case 1:
          if (stricmpOwn(CompName,Comp->PartNr)==0) return cnt;
          break;
...

{
  char  LineBuf[512],LineBufCopy[512],NetName[MAX_LENGTH_STRING],OldNetName[MAX_LENGTH_STRING],
        CompName[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
        str4[MAX_LENGTH_STRING],NetItemStr[MAX_LENGTH_STRING];
  int32 fp,InfoPos[MAX_LENGTH_STRING],cnt,cnt2,cnt3,Length,lengte,count,Found,MemPos,CompNr,ShapeNr,
        MemPosComp,PinOffset,NrPins,PinsMemPos,NrPinShapes,Lines,Layer,
        PolygonVertices,ShapeType,ShapePos,PropertiesPos,NrProperties,
        Mirror,CompInfoIndex,pos,NrNets,ok,weg;
  ...
  PropertiesPos=0;
  NrProperties=0;
  if ((fp=TextFileOpenUTF8(FileName))<0) return -1;

  while ((Length=ReadLn(fp,LineBuf))>=0) {
    Lines++;
//    sprintf(InfoStr,"Line %i",Lines);
//    RedrawInfoStr(1);
    LineBuf[Length]=0;
    if ((Length>1)
       &&
      ...
      if (stricmpOwn(str,"net")==0) {
        ...
        if (stricmpOwn(NetName,OldNetName)!=0) {
          if (Design.NrNets+1>=MaxNrNets) {
            if (AllocateMemNets(Design.NrNets+128)!=0) return -1;
          }
          NrProperties=0;
          PropertiesPos=0;
            ...
            if (cnt2<lengte) {
              ...
              if ((CompNr=FindCompNr(CompName,0))!=-1) {
                if (NrNetItems>=MaxNrNetItems) {
                  if (AllocateMemNetItems(NrNetItems+128)!=0) return -1;
                }
...
//        RedrawInfoStr(1);
  ...
  for (cnt=0;cnt<NrNetItems;cnt++) {
    ...
    if (NetItem->Info!=1) {
      ...
      if (AddToMessageBuf(str2)!=0) return -1;
      ok=1;
    } else {
...

{
  char  FileName[MAX_LENGTH_STRING],LineBuf[512],Ref[MAX_LENGTH_STRING],Geometrie[MAX_LENGTH_STRING],
        str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],
        Properties[32][128];
  int32 ok,fp,ShapeCompNrs[512],cnt,cnt2,Length,count,Found,MemPos,
        NrProperties,NrTries,
        NrShapeAreas,MinAreaResistorShapeNr,MinAreaCapacitorShapeNr;
  uint8  ShapeAreaPresence[512],*CompBuf;
  double ShapeArea[512],Area,MinY,MaxY,MaxX,
         PlaceX,PlaceY,PlaceX2,PlaceY2,
         MinAreaResistorShape,MinAreaCapacitorShape,
         HeightResistorShape,WidthResistorShape,
         HeightShape,WidthShape,
         HeightCapacitorShape,WidthCapacitorShape;
  CompRecord   *Comp;
  ShapeRecord  *Shape;
  CompPinRecord *CompPin;
  int32  Error=0;
  ...
  if (Design.NrBoardLayers==0) {
    MessageBoxOwn(PCBWindow,SC(432,"Number of layers is 0"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
    return -1;
  }
  ...
  if (count>0) {
    if (MessageBoxOwn(PCBWindow,SC(433,"The whole design will be deleted, Ok to go on ?"),SC(1,"Message"),MB_APPLMODAL|MB_OKCANCEL)!=IDOK) return -1;
  }
  TimerUpdateNetlist=0;
  FileName[0]=0;
  sprintf(str,"%s\\pcb",DesignPath);
  if (GetNewFileUTF8(PCBWindow,NULL,FileName,str,SC(389,"Netlist files"),
                     NULL,SC(389,"Netlist files"),"net",0)) {
//    MessageBoxOwn(PCBWindow,"ImportNetList",SC(100,"Error"),MB_APPLMODAL|MB_OK);
    return -1;
  }

//  strcpy(FileName,"c:\\testpcb\\mb\\pcb\\mb.net");
...
*/
          ...
          for (cnt=0;cnt<NrProperties;cnt++) {
            ...
            if (stricmp(str2,"PARTNR")==0) {
              ...
              if (stricmp(str2,"PARTDESCR")==0) {
                memmove(Comp->PartDescription,str3,min(sizeof(Comp->PartDescription)-1,strlen(str3)));
              } else {
                if (cnt2+strlen(str2)+strlen(str3)+3<256) {
                  ...
                  sprintf(str2,"For component %s some properties will not be used because of lack of space\r\n",Comp->Name);
                  if (AddToMessageBuf(str2)!=0) return 0;
                  break;
                }
              }
          ...
          if ((Design.NrBoardLayers==1)
             &&
             ((Shape->Info & SMD_DEVICE) == SMD_DEVICE)) {
            Comp->CompMode|=8;
...
// ********************************************************************************
  if (!Error) {
    if ((InRange(Design.BoardWidth,0.0))
       ||
       (InRange(Design.BoardHeight,0.0))) {
...
// ********************************************************************************
    ...
    if (MinAreaResistorShapeNr!=-1) {
      ...
      for (cnt=0;cnt<Design.NrComps;cnt++) {
        ...
        if ((Comp->Info2==1)
           &&
           (Comp->ShapeNr==MinAreaResistorShapeNr)) {
          if (First) {
            First=0;
            PlaceX=Design.BoardOriginX;
...
// ********************************************************************************
    ...
    for (cnt=0;cnt<Design.NrComps;cnt++) {
      ...
      if ((Comp->Info2==1)   // Resistor
         ...
         (Comp->Info3==0)
         &&
         (ShapeAreaPresence[Comp->ShapeNr]==0)) {
        Area=Comp->PlacementWidth*Comp->PlacementHeight;
        cnt2=0;
        ...
        if (cnt2==NrShapeAreas) {
          ...
          memmove(&ShapeArea[cnt2+1],&ShapeArea[cnt2],
                  (NrShapeAreas-cnt2)*4);
          memmove(&ShapeCompNrs[cnt2+1],&ShapeCompNrs[cnt2],
                  (NrShapeAreas-cnt2)*4);
          ShapeArea[cnt2]=Area;
          ShapeCompNrs[cnt2]=Comp->ShapeNr;
          ShapeAreaPresence[Comp->ShapeNr]=1;
...
// ********************************************************************************
    ...
    if (MinAreaCapacitorShapeNr!=-1) {
      ...
      for (cnt=0;cnt<Design.NrComps;cnt++) {
        ...
        if ((Comp->Info2==2)
           &&
           (Comp->ShapeNr==MinAreaCapacitorShapeNr)) {
          if (First) {
            First=0;
...
{
  char  FileName[MAX_LENGTH_STRING],LineBuf[512],Ref[MAX_LENGTH_STRING],
        Geometrie[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],
        str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],
        Properties[32][128];
  int32 fp,ShapeCompNrs[512],NrProperties,NrTries,
        cnt,cnt2,cnt3,Length,count,Found,MemPos,ok,TempNrComps,NrShapeAreas,FoundCompNr,
        res,MinAreaResistorShapeNr,MinAreaCapacitorShapeNr,ChangedCompNrByShape;
  uint8  ShapeAreaPresence[512],*CompBuf;
  double ShapeArea[512],Area,MinY,MaxY,MaxX,
        PlaceX,PlaceY,PlaceX2,PlaceY2,
        MinAreaResistorShape,MinAreaCapacitorShape,
  ...
  res=0;
  if (FileChanged) {
    if (MessageBoxOwn(PCBWindow,SC(146,"File has changed, save ?"),"",MB_APPLMODAL|MB_OKCANCEL)==IDOK) {
      if ((res=SaveFile(0))==-1) {
        MessageBoxOwn(PCBWindow,SC(147,"Error in saving file"),EditFile,MB_APPLMODAL|MB_OK);
      }
    }
  }
  if (res!=0) return -1;
  ...
  // TextBuf
        if (Found==-1) {
          ...
          for (cnt=0;cnt<TempNrComps;cnt++) {
            ...
            if ((FoundCompNr==-1)
               &&
               ((SearchComp->Info & (OBJECT_NOT_VISIBLE)) == 0)
               &&
              ...
              for (cnt3=0;cnt3<NrProperties;cnt3++) {
                ...
                if (stricmp(str2,"PARTNR")==0) {
                  ...
                  if (stricmp(str2,"PARTDESCR")==0) {
                    memmove(SearchComp->PartDescription,str3,min(sizeof(Comp->PartDescription)-1,strlen(str3)));
                  } else {
                    if (cnt2+strlen(str2)+strlen(str3)+3<256) {
...
{
  int32           Count,cnt2,pos,LengthLine,count2,PinOffset,MemPos,cnt,ShapePos,
                  NrPins,PinNr,PinsMemPos,MemPosComp,NetNr,ShapeInfo,NrPinShapes,ShapeType,ShapeNr,fp;
  int32            Found;
  CompRecord      *Comp;
  ShapePadRecord  *ShapePad;
  PadRecord       *Pad;
  ...
  NetRecord       *Net;
  NetItemsRecord  *NetItem;
  char            Line2[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],FileStr[MAX_LENGTH_STRING];
...
//      }
  ...
  if ((mode & 1)==1) {
    sprintf(FileStr,"%s\\pcb\\design.net",DesignPath);
...

{
  ...
  ShapePadRecord *ShapePad;
  TraceRecord  *Trace;
  int32  cnt,cnt2,MemPos,Length,NrPins,PinsMemPos,PinOffset,ShapePos,
         count,PolygonVertices,ShapeType,Found,FoundPinNr,PinNr,MemPosComp,NrPinShapes;
  LPSTR  PinText;
  PadRecord       *Pad;
  ...
  while ((cnt2<Length)
        &&
        (NetItem->PinStr[cnt2]!='-')) cnt2++;
  if (cnt2==Length) return -1;
...

{
  ...
  ShapePadRecord *ShapePad;
  PadRecord *Pad;
  int32  cnt2,MemPos,Length,NrPins,PinsMemPos,PinOffset,
         PolygonVertices,ShapeType,ShapePos,Mirror,Layer,MemPosComp,NrPinShapes;
  LPSTR  PinText;
...
/*
  if (mode==1) {
    sprintf(str2,"Component ");
//    sprintf(str2,"Component  %s  Shape %s PinText %s ",Comp->Name,Shape->ShapeName,PinText);
//    MessageBoxOwn(PCBWindow,SC(1,"Message"),str2,MB_APPLMODAL|MB_OK);
//    if (AddToMessageBuf(str2)!=0) return -1;
  }
*/
  NrPins=Shape->NrPins;
/*
  if (stricmpOwn(NetItem->PinStr,"C24-1")==0) {
    sprintf(str2,"Component  %s  Shape %s NrPins %d PinText %s ",Comp->Name,Shape->ShapeName,NrPins,PinText);
    MessageBoxOwn(PCBWindow,str2,SC(1,"Message"),MB_APPLMODAL|MB_OK);
//    if (AddToMessageBuf(str2)!=0) return -1;
    ok=1;
  }
*/
...
/*
    if (stricmpOwn(NetItem->PinStr,"C24-1")==0) {
      sprintf(str2,"Component  %s  Shape %s Shape Nr Layers %d Mirror %d NrPinShapes %d  ShapePad name %s",
                   Comp->Name,Shape->ShapeName,Shape->NrLayers,NrPinShapes,Mirror,ShapePad->Name);
      MessageBoxOwn(PCBWindow,str2,SC(1,"Message"),MB_APPLMODAL|MB_OK);
  //    if (AddToMessageBuf(str2)!=0) return -1;
      ok=1;
    }
...

{
  ...
  for (cnt=0;cnt<Design.NrNets;cnt++) {
    Net=&((*Nets)[cnt]);
    if (stricmpOwn(NetName,Net->Name)==0) return cnt;
  }
  return -1;
}
...
{
  char  LineBuf[512],LineBufCopy[512],NetName[MAX_LENGTH_STRING],
        OldNetName[MAX_LENGTH_STRING],
        CompName[10],str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
        str4[MAX_LENGTH_STRING],Properties[256],
        NetItemStr[MAX_LENGTH_STRING];
  int32 fp,cnt,cnt2,cnt3,cnt4,Length,lengte,count,CompNr,res,lengte2,BufPos2,
        NrNets2,Count,NetItemEqualNr,OldNrPins,Test,result,NrProperties,
        PropertiesPos,
        NrEqualNetItems,LineCount,NrEqualNets,MaxEqualNetItems,NetNrMaxEqualNetItems,
        pos,pos1a,pos2,pos3,ok,NetListBufSize;
  uint8 *NetListBuf;
  CompRecord   *Comp;
...
/*
  for (cnt=0;cnt<Design.NrNets;cnt++) {
    Net=&((*Nets)[cnt]);
    if (stricmpOwn(Net->Name,"IDE0D1")==0) {
      ok=1;
    }
  }
*/
...
*/
    ...
    //            strncpy(NetItem->PinStr,CompName[cnt2+1],lengte-cnt2-1);
              } else {
                result=1;
                pos3=0;
                while ((pos3<BufPos2)
                      &&
                      (stricmpOwn((LPSTR)&NetListBuf[pos3],CompName)!=0)) {
...
//      sprintf(InfoStr,"%i",LineCount);
    ...
    for (cnt2=0;cnt2<NrNets2;cnt2++) {
      ...
      if ((NrEqualNetItems==Net->NrPins)
         &&
         (NrEqualNetItems==Net2->Count)) {
        Net2->NetInfo|=3;
        Net->NetInfo|=3;
#ifdef _DEBUG
...
#endif
      if ((MaxEqualNetItems>0)
         &&
         (MaxEqualNetItems>(Net->NrPins/4))) {
        ...
        for (cnt4=0;cnt4<Net2->Count;cnt4++) {
          ...
          if ((res=AssignNetToCompPin(NetNrMaxEqualNetItems,NetItem2,0))!=1) {
            if (res==0) {
              sprintf(str2,SC(439,"Netitem  %s  Net  %s  does not exist"),NetItem2->PinStr,Net2->Name);
            } else {
        ...
        Net->NetInfo|=1;
//        Net->Info=0;
//        sprintf(str2,"Net '%s' (%i) replaced by '%s' (%i)",
//                Net->Name,OldNrPins,Net2->Name,Net->NrPins);
//        if (AddToMessageBuf(str2)!=0) return -1;
#ifdef _DEBUG
        if (stricmpOwn(Net->Name,"vin")==0) {
          ok=1;
...
*/
      ...
      if (!Found) {
        if (Design.NrNets+1>=MaxNrNets) {
          if (AllocateMemNets(Design.NrNets+32)!=0) return -1;
        }
        Net=&((*Nets)[Design.NrNets]);
        memset(Net,0,sizeof(NetRecord));
//        sprintf(str2,"Net '%s' added",Net2->Name);
//        if (AddToMessageBuf(str2)!=0) return -1;
        strcpy(Net->Name,Net2->Name);
        memcpy(Net->Properties,Net2->Properties,sizeof(Net2->Properties));
/*
...
// Delete the nets not used anymore
  ...
  for (cnt=0;cnt<Design.NrNets;cnt++) {
    ...
    if ((Net->NetInfo & 1)==0) {
      DeleteNetFromComponents(cnt);
      DeleteNet((int32)cnt);
//      sprintf(str2,"Net '%s' deleted",Net->Name);
//      if (AddToMessageBuf(str2)!=0) return -1;
      memset(Net,0,sizeof(NetRecord));
    }
  }
...
/*
  for (cnt=0;cnt<NrNets2;cnt++) {
    Net2=&((*Nets2)[cnt]);
    if (stricmpOwn(Net2->Name,"3V3")==0) {
      ok=1;
    }
    if ((cnt3=GetOldNetNr(Net2->Name))!=-1) {
      OldNet=&((*Nets)[cnt3]);
      Net2->Info=OldNet->Info & (OBJECT_HIGHLITED|CONNECTIONS_DISABLED|CONNECTIONS_NOT_VISIBLE);
    }
  }
*/
  SetWaitCursor();
...
//      CheckNet((int32)cnt,0);
      CheckForEscape();
//      InsertConnections((int32)cnt,0); ID_VIEW_UNSELECTTRACESVIASNET
    }
  }
...
#define SetBitMapLinePointX(x)   (LineBytes[(x) >> 3]|=(0x80 >> ((x) & 7)))
  ...
  int32             MemSize,fp,result,cnt,cnt2,NrLineBytes,
                    x,res,DataOffset,count;

  SpecialBitmapRecord SpecialBitmap;

  if ((fp=FileOpenReadOnlyUTF8(FileName))<=0) return -1;
...
//      FileRead(fp,&BitmapMem[cnt*NrLineBytes],cnt2,&result);
