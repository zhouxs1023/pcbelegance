...
ObjectRecord             NewObject,*ApertureObject,DefaultApertureObject,
                         DefaultApertureObject2;
  ...
  if (mode==0) {
    ...
    while (cnt<LengthBuffer-LengthString+1) {
      if (TextBuf[cnt]==FirstChar) {
        cnt2=0;
        while ((cnt2<LengthString)
              &&
              (TextBuf[cnt+cnt2]==SearchString[cnt2])) cnt2++;
    ...
    while (cnt<LengthBuffer-LengthString+1) {
      if (toupper(TextBuf[cnt])==FirstChar) {
        ...
        while ((cnt2<LengthString)
              &&
              (toupper(TextBuf[cnt+cnt2])==SearchString2[cnt2])) cnt2++;
        if (cnt2==LengthString) {
          count++;
        }
      }
...

{
  ...
  if (lengte>2) {
    ...
    if (stricmpOwn(str2,SC(131,"mm"))==0) {
      *Value*=100000.0;
      return 1;
    }
    if (lengte>4) {
...
{
  int32  fp,CheckLength,CountD01,CountD02,CountD03,result,
         Check1,Check2,Check3,Check4,Check5;

  AllocateSpecialMem(MEM_POINTS,GerberBufSize*2,(void **)&GerberBuf);

  if ((fp=FileOpenReadOnlyUTF8(FileName))<0) return 0;
  ...
  if (CountD01+CountD02+CountD03>3) return 1;
  ...
  if (Check1+Check2+Check3+Check4+Check5>2) return 1;
...
{
  int32  fp,cnt,result,CheckLength,
         CountRound,
         CountRounded,
         CountRectangle,
         CountRectangular,
         ...
         CountCircle,
         CountG04,
         CountHeader,CountSection,CountEntities,CountEndSections,
         CountTables;
  ...
  CountRound       =CountNrStrings(GerberBuf,CheckLength,SC(443,"rund"),1);
  CountRounded     =CountNrStrings(GerberBuf,CheckLength,SC(444,"rounded"),1);
  CountRectangle   =CountNrStrings(GerberBuf,CheckLength,SC(445,"rechteck"),1);
  ...
  CountSquare      =CountNrStrings(GerberBuf,CheckLength,SC(449,"quadrat"),1);
  ...
  CountCircle      =CountNrStrings(GerberBuf,CheckLength,SC(452,"kreis"),1);
  ...
  CountAperture    =CountNrStrings(GerberBuf,CheckLength,SC(458,"blende"),1);
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
// ************************************************************************************************
      SetWindowTextOwn(Dialog,SC(1,"Message"));
      SetDialogItemText(Dialog,IDOK           ,SC(155,"OK"));
      SetDialogItemText(Dialog,IDCANCEL       ,SC(459,"Retry"));
// ************************************************************************************************
            ...
            switch (ApertureType) {
              ...
              case 96:  // Oblong
                ...
                if (TempGerberInfo.SizeYColumn==-1) {
                  if (ValueY<0.0) {
                    if ((ApertureLines[cnt3].Column[TempGerberInfo.SizeXColumn+ExtraAdd+1] & 12) == 8) {
                      res=GetSpecialValue(Strings[TempGerberInfo.SizeXColumn+ExtraAdd+1],&ValueY,0);
                      if (res==1) OkToUseUnits=0;
//                      if (sscanf(Strings[TempGerberInfo.SizeXColumn+ExtraAdd+1],"%f",&ValueY)==0) ValueY=-1.0;
                    } else {
                      if ((ApertureLines[cnt3].Column[TempGerberInfo.SizeXColumn+ExtraAdd+2] & 12) == 8) {
                        res=GetSpecialValue(Strings[TempGerberInfo.SizeXColumn+ExtraAdd+2],&ValueY,0);
                        if (res==1) OkToUseUnits=0;
...
//                  if (sscanf(Strings[TempGerberInfo.SizeYColumn],"%f",&ValueY)==0) ValueY=-1.0;
                ...
                switch (ApertureType) {
                  ...
                  case 96:  // Oblong
                    ...
                    if (ValueX<0.0) Error=1;
                    if (ValueY<0.0) Error=1;
                    break;
                }
                break;
              case 128: // square
...
//  TextFileClose(fp);
  if (MessageBufPos>0) {
    MessageDialog(SC(460,"The following apertures could not be decoded"),0,0);
    DeAllocateMemMessageBuf();
  }
// NrAperTures
  sprintf(RetryStr,SC(461,"%i apertures have been found"),TempGerberInfo.NrApertureDefs);
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
// ************************************************************************************************
      SetWindowTextOwn(Dialog,SC(467,"Load apertures"));
      SetDialogItemText(Dialog,IDC_STATIC1     ,SC(468,"Aperture file"));
      SetDialogItemText(Dialog,IDC_STATIC2     ,SC(469,"Info lines"));
      SetDialogItemText(Dialog,IDC_STATIC3     ,SC(470,"Columns"));
      SetDialogItemText(Dialog,IDC_STATIC4     ,SC(471,"Apertures"));
      ...
      SetDialogItemText(Dialog,IDC_STATIC9     ,SC(474,"DCode"));
      SetDialogItemText(Dialog,IDC_STATIC10    ,SC(475,"Shape"));
      SetDialogItemText(Dialog,IDC_STATIC11    ,SC(476,"X size"));
      SetDialogItemText(Dialog,IDC_STATIC12    ,SC(477,"Y size"));
      SetDialogItemText(Dialog,IDC_STATIC13    ,SC(1085,"Units"));
      SetDialogItemText(Dialog,IDC_STATIC14    ,SC(1085,"Units"));

      SetDialogItemText(Dialog,IDC_CHECK1      ,SC(478,"Reverse X and Y when rectangle or oblong"));
...

{
  ...
  if ((fp=TextFileOpenUTF8(AperTureFile))<0) return -1;
...
// ************************************************************************************************
  ...
  for (cnt=0;cnt<NrAperTextLines;cnt++) {
    for (cnt2=0;cnt2<ApertureLines[cnt].NrStrings;cnt2++) {
      if ((Dcode==-1)
         &&
         ((ApertureLines[cnt].Column[cnt2] & 1) == 1)) {
        Dcode=cnt2;
      }
    }
  }
  if (Dcode==-1) {
    ...
    while ((!Stop)
          ...
          (cnt<NrAperTextLines)) {
      cnt2=0;
      while ((cnt2<ApertureLines[cnt].NrStrings)
            &&
            ((ApertureLines[cnt].Column[cnt2] & (128+64+32)) == 0)) cnt2++;
  ...
  cnt=0;
  while ((cnt<NrAperTextLines)
        &&
        (ApertureLines[cnt].Column[0] != 1)
        &&
        (ApertureLines[cnt].Column[1] != 1)
        &&
        (ApertureLines[cnt].Column[2] != 1)
        &&
        (ApertureLines[cnt].Column[3] != 1)
        &&
        (ApertureLines[cnt].Column[4] != 1)
        &&
        (ApertureLines[cnt].Column[5] != 1)
        &&
        (ApertureLines[cnt].Column[6] != 1)
        &&
        (ApertureLines[cnt].Column[7] != 1)
        &&
        (ApertureLines[cnt].Column[8] != 1)
        &&
        (ApertureLines[cnt].Column[9] != 1)
        &&
        (ApertureLines[cnt].Column[10] != 1)) cnt++;
...
//  return -2;
  ...
  TempGerberInfo.TotalLines=NrAperTextLines;
  while ((cnt>TempGerberInfo.SkipLines)
        &&
        (ApertureLines[cnt].Column[0]  != 1)
        &&
        (ApertureLines[cnt].Column[1]  != 1)
...
// ************************************************************************************************
  ...
  if (TempGerberInfo.DcodeColumn==-1) {
    if ((ApertureLines[TempGerberInfo.SkipLines].Column[0]==10)
      ...
      Stop2=0;
      while ((cnt2<TempGerberInfo.MaxLineNr)
            &&
            (!Stop2)) {
...
// ************************************************************************************************
  ...
  while ((cnt<ApertureLines[TempGerberInfo.SkipLines].NrStrings)
        &&
        ((ApertureLines[TempGerberInfo.SkipLines].Column[cnt] & (16+8)) == 0)) cnt++;
  if (cnt<ApertureLines[TempGerberInfo.SkipLines].NrStrings) {
    if ((TempGerberInfo.DcodeColumn!=cnt)
       &&
       (TempGerberInfo.ApertureTypeColumn!=cnt)) {
  ...
  if (TempGerberInfo.SizeXColumn!=-1) TempGerberInfo.SizeYColumn=TempGerberInfo.SizeXColumn+1;
  if (TempGerberInfo.SizeYColumn!=-1) {
    ...
    while ((!Stop)
      ...
      while ((cnt2<ApertureLines[cnt].NrStrings)
            &&
            ((ApertureLines[cnt].Column[cnt2] & (64+32)) < 64)) cnt2++;
      if (cnt2<ApertureLines[cnt].NrStrings) {
  ...
  if (TempGerberInfo.ApertureTypeColumn==-1) TempGerberInfo.SizeYColumn=-1;
// ************************************************************************************************
  ...
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_APERTURE_IMPORT), PCBWindow, (DLGPROC)ApertureDialog2);
  return res;
}
...

{
  if (GerberBufPos==GerberBufLength) {
    if ((FileRead(fp,GerberBuf,GerberBufSize,&GerberBufLength)==-1)
       ||
       (GerberBufLength==0)) {
...

{
  ...
  if (InRangeSpecial(x1,x2,2540.0)) {  // Vert
    ...
    if (InRangeSpecial(y1,y2,2540.0)) {  // hor
      if (AperTureObject->ObjectType==PIN_SMD_ROUND) {
      ...
      if (InRangeSpecial(x1-x2,y1-y2,2540.0)) {  // diag2
        if (AperTureObject->ObjectType==PIN_SMD_ROUND) {
          if (x1>x2) {
            NewObject.x1=x2;
            NewObject.y1=y2;
        ...
        if (InRangeSpecial(x1-x2,y2-y1,2540.0)) {  // diag1
          if (AperTureObject->ObjectType==PIN_SMD_ROUND) {
            if (x1>x2) {
              NewObject.x1=x2;
              NewObject.y1=y2;
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
// ************************************************************************************************
      SetWindowTextOwn(Dialog,SC(482,"Load gerber file"));
      SetDialogItemText(Dialog,IDC_STATIC1  ,SC(483,"First gerber data"));
      SetDialogItemText(Dialog,IDC_STATIC2  ,SC(484,"Gerber format"));
      SetDialogItemText(Dialog,IDC_STATIC3  ,SC(262,"Number format"));
      SetDialogItemText(Dialog,IDC_STATIC4  ,SC(1085,"Units"));

      SetDialogItemText(Dialog,IDC_RADIO1   ,SC(485,"Leading zero suppression"));
      SetDialogItemText(Dialog,IDC_RADIO2   ,SC(486,"Trailing zero suppression"));
      SetDialogItemText(Dialog,IDC_RADIO3   ,SC(487,"Floating point"));
...
{
  int32  fp,cnt,cnt2,cnt3,FoundCnt,Length,AperNr,
         res,ok,GerberAction,Temp,NewAperNr,DigitCountX[10],DigitCountY[10],
         NrMacroNames,NrGerberLayers,
         ValueXint,ValueYint,ValueSides,
         GerberObjectCount,
         NrPolygonPoints,NrPolygonPoints2,CntX,CntY,
         StepAndRepeatMode,StepCountX,StepCountY,
         Length2,FilePosK,NrGerberCommands;
  char   str1[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],DCodeStr[MAX_LENGTH_STRING],StrX[MAX_LENGTH_STRING],StrY[MAX_LENGTH_STRING],
         StrX2[MAX_LENGTH_STRING],StrY2[MAX_LENGTH_STRING],CopyInfoStr[MAX_LENGTH_STRING],ObjectChar;
  double LastX,LastY,MinX,MinY,MaxX,MaxY,x1,y1,x2,y2,x3,y3,x4,y4,
         AperTureParameters[10],Angle1,Angle2,LengthArc1,
         LengthArc2,DiffAngle,DiffAngles[4];
  ...
  int32   LightActive,Stop,AutoAperTures,SingleQuadrantInterpolation,
         CircularInterpolation,FoundFirstLayer,StepAndRepeat,
         G7XCode,CircularInterpolationCW,LoadExtraPoint,PolygonMode;

  if (GerberFile[0]==0) return -1;
  if ((fp=TextFileOpenUTF8(GerberFile))<0) {
    MessageBoxOwn(PCBWindow,GerberFile,SC(100,"Error"),MB_APPLMODAL|MB_OK);
    return -1;
  }
...
//  memmove(&TempGerberInfo,&Design.DefaultGerberInfo,sizeof(GerberInfoRecord));
  ...
  Stop=0;
  while (((Length=ReadLnWithMaxLength(fp,LineBuf,1024))>=0)
        &&
        (!Stop)) {
    LineBuf[Length+1]=0;
    LineBuf[Length+2]=0;
    ...
    if (TempGerberInfo.TotalLines>800) Stop=1;
  }
  TextFileClose(fp);
...
// ************************************************************************************************
  ...
  SetWaitCursor();
  if ((fp=FileOpenReadOnlyUTF8(GerberFile))<0) return -1;
  Stop=0;
  NrGerberCommands=0;
  ...
  while (((Length=ReadGerberCommand(fp,LineBuf))>=0)
    ...
    if (Length>0) {
      ...
      if ((Length>3)
         ...
         (LineBuf[0]=='G')
         &&
         ((LineBuf[1]=='1') || (LineBuf[1]=='2') || (LineBuf[1]=='3'))
         &&
         ((LineBuf[2]=='X') || (LineBuf[2]=='Y'))) {
      ...
      if ((Length>4)
         ...
         (LineBuf[1]=='0')
         &&
         ((LineBuf[2]=='1') || (LineBuf[2]=='2') || (LineBuf[2]=='3'))
         &&
...
//            FoundFirstLayer=0;
    ...
    NrGerberCommands++;
    if (NrGerberCommands>10000) Stop=1;
  }
  FileClose(fp);
  SetNormalCursor();
      ...
      if ((DigitCountX[cnt]>0)
        ...
        switch (cnt) {
          case 5:
            switch (TempGerberInfo.Units) {
              case -1:
                ...
                break;
              case 1: // mm
                if ((MaxX-MinX>40000.0)
                   ||
                   (MaxY-MinY>40000.0)) { // 400 mm
                  TempGerberInfo.XDigits1=3;
                  TempGerberInfo.YDigits1=3;
                  TempGerberInfo.XDigits2=2;
                  TempGerberInfo.YDigits2=2;
                ...
                break;
              case 2: // inch
                if ((MaxX-MinX>25000.0)
                   ||
                   (MaxY-MinY>25000.0)) { // 25 inch
                  TempGerberInfo.XDigits1=2;
                  TempGerberInfo.YDigits1=2;
                  TempGerberInfo.XDigits2=4;
          ...
          case 6:
            switch (TempGerberInfo.Units) {
              case -1:
                ...
                break;
              case 1: // mm
                if ((MaxX-MinX>400000.0)
                   ||
                   (MaxY-MinY>400000.0)) { // 400 mm
                  TempGerberInfo.XDigits1=4;
          ...
          case 8:
            switch (TempGerberInfo.Units) {
              case -1:
                if ((MaxX-MinX>10000000.0)
                   ||
                   (MaxY-MinY>10000000.0)) {
                  TempGerberInfo.Units=2;    // inch
                  TempGerberInfo.XDigits1=3;
                  TempGerberInfo.YDigits1=3;
                  TempGerberInfo.XDigits2=5;
              ...
              case 1:
                ...
                break;
              case 2:
                if ((MaxX-MinX>00200000.0)
                   ||
                   (MaxY-MinY>00200000.0)) { // 200 mm
                  TempGerberInfo.XDigits1=4;
                  TempGerberInfo.YDigits1=4;
                  TempGerberInfo.XDigits2=4;
                  TempGerberInfo.YDigits2=4;
...
*/
    res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_GERBER_INPUT), PCBWindow, (DLGPROC)GerberDialog2);
    if (res==2) return -3;
    if (LocalAperTureFile[0]==0) {
      strcpy(LocalAperTureFile,GerberFile);
    }
    if (GetNewFileUTF8(PCBWindow,NULL,LocalAperTureFile,ExportDir,SC(489,"Load aperture file"),
                       NULL,SC(489,"Load aperture file"),"*",0)) {
      return -1;
    }
...
//              SingleQuadrantInterpolation=1;
          ...
          if ((Length>4)
             ...
             ((LineBuf[2]=='1') || (LineBuf[2]=='2') || (LineBuf[2]=='3'))
             &&
             ((LineBuf[3]=='X') || (LineBuf[3]=='Y') || (LineBuf[3]=='I') || (LineBuf[3]=='J'))) {
            switch (LineBuf[2]) {
              case '1':
  //              SingleQuadrantInterpolation=0;
//                SingleQuadrantInterpolation=1;
...
// ************************************************************************************************
          ...
          if (TempGerberInfo.NumberFormat==0) { // Normal gerber
            Length2=strlen(StrX);
            if (Length2>0) {
              ValueXint=atol(StrX);
//              sscanf(StrX,"%d",&ValueXint);
              ...
              if ((StrX[0]=='-')
                 ||
                 (StrX[0]=='+')) Length2--;
              if (TempGerberInfo.ZeroMode==1) {  // Trailing zeros omitted
...
//              sscanf(StrY,"%d",&ValueYint);
            ...
            if (LoadExtraPoint) {
              ...
              if (Length2>0) {
                ...
                if (TempGerberInfo.ZeroMode==1) {  // Trailing zeros omitted
                  if (TempGerberInfo.XDigits1+TempGerberInfo.XDigits2>Length2) {
                    ValueX2*=(float)Mult10[TempGerberInfo.XDigits1+TempGerberInfo.XDigits2-Length2];
                  }
                }
              ...
              if (Length2>0) {
                ...
                if (TempGerberInfo.ZeroMode==1) {  // Trailing zeros omitted
                  if (TempGerberInfo.XDigits1+TempGerberInfo.XDigits2>Length2) {
                    ValueY2*=(float)Mult10[TempGerberInfo.XDigits1+TempGerberInfo.XDigits2-Length2];
                  }
                }
                ValueY2/=(float)Mult10[TempGerberInfo.YDigits2];
              }
            }
          } else {   // Floating point gerber
            if (StrX[0]!=0) {
              ValueX=0.0;
              if (strcmp(StrX,"0")!=0) {
                sscanf(StrX,"%f",&ValueX);
...
*/
                  ...
                  FoundCnt=-1;
                  if (SingleQuadrantInterpolation) { // Single quadrant
// ************************************************************************************************

                    if ((NotInRange(LastX,ValueX))
                       ||
                       (NotInRange(LastY,ValueY))) {
...

{
  ...
  double  OldX,OldY,CurrentX,CurrentY,divx,divy,ShiftX,ShiftY,x1,y1,
          CursorX,CursorY,DiagValue;
...
//        divy=CurrentY-CurrentY2;
        ...
        AdjustOffsetForSnapOnGerberObject(CursorX,CursorY,CurrentX,CurrentY,divx,divy,&ShiftOffsetX,&ShiftOffsetY,0);
        DrawGerberObjects(CurrentX,CurrentY,Layer,1);
        DisplayCursorPosition();
      }
    } else {
    ...
    if ((ZoomActive())
      ...
      OldX=CurrentX;
      OldY=CurrentY;
      if (!SelectionEsc) DrawGerberObjects(CurrentX,CurrentY,Layer,1);
      else CompPlaced=1;
    }
    if ((PanActive())
       &&
    ...
    if (LeftButtonPressed) {
      ...
      AdjustOffsetForSnapOnGerberObject(CursorX,CursorY,CurrentX,CurrentY,0.0,0.0,&ShiftOffsetX,&ShiftOffsetY,4+2);
      divx=CurrentX-CurrentX2-ShiftOffsetX;
      divy=CurrentY-CurrentY2-ShiftOffsetY;
...

{
  ...
  for (cnt=0;cnt<NrObjects6;cnt++) {
    ...
    if (Object6->ObjectType==PIN_SMD_RECT) {
      ok=1;
      if ((InRange9(Object6->x1,35.57e5))
         &&
         (InRange9(Object6->y1,29.46e5))) {
        ok=1;
      }
    }
#endif
  ...
  while (SearchUnconnectedGerberObject(&ObjectNr)) {
    ...
    for (cnt5=0;cnt5<NrObjects5;cnt5++) {
      ...
      if (Object5->ObjectType==PIN_SMD_RECT) {
        ok=1;
        if ((InRange9(Object5->x1,35.57e5))
           &&
           (InRange9(Object5->y1,29.46e5))) {
          ok=1;
        }
      }
    }
            ...
            for (cnt3=0;cnt3<NrObjects3;cnt3++) {
              ...
              switch (Object3->ObjectType) {
                ...
                case PIN_SMD_POLYGON:
                  if ((Object3->Layer==Layer)
                     &&
                     (ObjectsConnected(Object3,Object5))) {
...

{
  ...
  if ((mode & 1)==0) { // Import traces/vias from gerber file
    Layer=SelectLayer(0);
    if (Layer==-1) return -1;
  ...
  */
    if (GetNewFileUTF8(PCBWindow,NULL,GerberFileName,ExportDir,SC(495,"Import gerber file"),
                       NULL,SC(496,"Gerber files"),"*",0)) {
      return -1;
    }
//    if (LoadNewFile3(GerberFileName,SC(495,"Import gerber file"),SC(496,"Gerber files"),0,0)!=0)
