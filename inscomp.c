...
{
  int32    about;
  int32   res,cnt,Comp1,Comp2,Length,Code1,Code2,Selected,
          lengte,pos,fp,dialogunitX,TabStops[5];
  char    LineBuf[512],Name[MAX_LENGTH_STRING],PartNr[MAX_LENGTH_STRING],Value[MAX_LENGTH_STRING],Geometry[MAX_LENGTH_STRING],CompInfo[MAX_LENGTH_STRING],
          Code1Str[MAX_LENGTH_STRING],Code2Str[MAX_LENGTH_STRING],FileName[MAX_LENGTH_STRING],FileStr[100];
...
// LB_SETCOLUMNWIDTH
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemTextUTF8(Dialog,IDC_STATIC3  , SC(122,"Part number"));
      ...
      SetDialogItemTextUTF8(Dialog,IDC_STATIC9  , SC(122,"Part number"));
...
//            strcpy(PartNr,"321313");
            sprintf(DialogTextLine,"%s\t%s\t%s\t%s\t%s",
                    Name,
                    PartNr,
                    Value,
                    Geometry,
...
//  DialogWindowText=DialogText;
  ...
  if (!EditingSymbol) {
    res=DialogBox(SCHClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_SELECTCOMP), SCHWindow, (DLGPROC)SelectCompDialog2);
    if (OkToAddSymbol) {
