...

{
  ...
  for (Layer=0;Layer<32;Layer++) {
    for (cnt=0;cnt<Design.NrVerTraces[Layer];cnt++) {
      ...
      if ((TraceInfo & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
        if (Trace->NetNr<Design.NrNets) SelectedNets[Trace->NetNr]=1;
        NrTracesSelected++;
      }
    }
  ...
  for (cnt=0;cnt<Design.NrVias;cnt++) {
    ...
    if ((ViaInfo & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
      if (Via->NetNr<Design.NrNets) SelectedNets[Via->NetNr]=1;
      NrViasSelected++;
    }
  }
      ...
      if (AreaFillChanged) {
        if ((FirstPolygon->PolygonType & 2) == 2) {
          if ((AreaFill->Info & (POWERPLANE)) == 0) {
            ...
            if ((AreaFill->NetNr>=0)
               &&
               (AreaFill->NetNr<Design.NrNets)) SelectedNets[AreaFill->NetNr]=1;
            DataBaseChanged=1;
            AreaFillCount++;
          }
        } else {
...
//          StartDrawingEditingWindow();
  ...
  if (AreaFillCount>1) {
    MessageBoxOwn(PCBWindow,SC(498,"More then one areafill deleted"),SC(102,"Warning"),MB_APPLMODAL|MB_OK);
  }
...

{
  ...
  for (cnt=0;cnt<AreaFill->NrPolygons;cnt++) {
    if (cnt>0) {
      if (((PointInPolygon(Polygon,cx,cy) & 1 ) == 1)
         &&
         (Found<1000)) {
        PolygonMap[Found++]=1;
      }
...
// ***************************************************************************
  ...
  if (Design.NrAreaFills>=MaxNrAreaFills) {
    if (AllocateMemAreaFills(MaxNrAreaFills+16)!=0) return -1;
  }
...
//        NrObjects3++;
        ...
        break;
      case TRACE_VER:
        if (!MakeRoomVerTraces(Layer)) return 0;
...
//        NrObjects3++;
        ...
        break;
      case TRACE_DIAG1:
        if (!MakeRoomDiag1Traces(Layer)) return 0;
...
//        NrObjects3++;
        ...
        break;
      case TRACE_DIAG2:
        if (!MakeRoomDiag2Traces(Layer)) return 0;
...

{
  ...
  if ((InRange(Connection->x1,Connection->x2))
     &&
     (InRange(Connection->y1,Connection->y2))) {
    ok=1;
    return 1;
  }
...
//  NewConnection->Info&=~0x003f;
  ...
  if ((Connection->NetNr>=0)
     &&
     (Connection->NetNr<Design.NrNets)) {
    Net=&((*Nets)[Connection->NetNr]);
    NewConnection->Info=(int16)((Net->Info) & (CONNECTIONS_DISABLED|CONNECTIONS_NOT_VISIBLE|OBJECT_HIGHLITED));
  }
  ZeroUnusedObjects(0);
...

{
  ...
  if (NewestVia==NULL) NewestVia=NewVia;
  LastVia=NewVia;
...

{
  ...
  for (cnt2=1;cnt2<NewAreaFill->NrPolygons;cnt2++) {
    Polygon=(PolygonRecord *)PolygonPos;
    if ((Polygon->NrVertices<0)
       ||
       (Polygon->NrVertices>100000)) {
      ok=1;
      return;
    }
    Length2+=MemSizePolygon(Polygon);
...

{
  ...
  uint8 *AreaFillP,*PolygonPos,*NewAreaPos,*NewAreaFillPolygonPos,
        *CheckPolygonPos,PolygonInclude[4000];
...

{
  ...
  for (cnt2=0;cnt2<AreaFill->NrPolygons;cnt2++) {
    ...
    cx=(minx+maxx)*0.5;
    cy=(miny+maxy)*0.5;
    sprintf(str,"Polygon (%3d %3d %2d) minx,miny maxx,maxy  (%8.3f,%8.3f) %8.3f,%8.3f %8.3f,%8.3f",
                cnt2,Polygon->NrVertices,Polygon->PolygonType,cx,cy,minx,miny,maxx,maxy);
    WriteLn(fp,str);
    if (cnt2==0) {
      WriteLn(fp,"------------------------------------------------------------------------------------------");
    }
...

{
  ...
  if (strlen(ObjectText2->Text)==0) return 0;
...

{
  ...
  if (Design.NrObjectPolygons>=MaxNrObjectPolygons) {
    if (AllocateMemObjectPolygons(MaxNrObjectPolygons+128)!=0) return -1;
  }
...

{
  ...
  cnt=0;
  Stop=0;
  while ((cnt<Design.NrConnections)
        &&
        (!Stop)) {
    if (((MinConnectionX1==(*Connections)[cnt].x1)
       &&
       (MinConnectionY1==(*Connections)[cnt].y1)
       &&
       (MinConnectionX2==(*Connections)[cnt].x2)
       &&
       (MinConnectionY2==(*Connections)[cnt].y2))
       ||
       ((MinConnectionX1==(*Connections)[cnt].x2)
       &&
       (MinConnectionY1==(*Connections)[cnt].y2)
       &&
       (MinConnectionX2==(*Connections)[cnt].x1)
       &&
       (MinConnectionY2==(*Connections)[cnt].y1))) Stop=1;
    cnt++;
  }
  return Stop;
}
...

{
  ...
  if (Object2->Test==0) return 0;

  Object1=&((*Objects2)[Object2Nr]);
  if (Object1->ObjectType!=Object2->ObjectType) return 0;
  if (Object1->ObjectType==CONNECTION) {
    if (!(((Object1->x1==Object2->x1)
         &&
         (Object1->y1==Object2->y1)
         &&
         (Object1->x2==Object2->x2)
         &&
         (Object1->y2==Object2->y2))
         ||
         ...
         &&
         (Object1->y2==Object2->y1)))) return 0;
  } else {
    if (Object1->Layer!=Object2->Layer) return 0;
    if (Object1->x1!=Object2->x1) return 0;
    if (Object1->y1!=Object2->y1) return 0;
    if (Object1->x2!=Object2->x2) return 0;
    if (Object1->y2!=Object2->y2) return 0;
    if (Object1->Clearance!=Object2->Clearance) return 0;
  }
  Object1->Test=0;
  Object2->Test=0;
  return 1;
...

{
  ...
  if (LastActionNr<2) return;
  if (MaxLastActionNr<LastActionNr) {
    MaxLastActionNr=LastActionNr;
  }
  if (LastActionNr>1) LastActionNr--;
...

{
  ...
  if ((LastActionNr>MaxLastActionNr)
     ||
     (LastActionNr==0)) return;
...
// ****************************************************************************************
  if (MaxLastActionNr>LastActionNr) LastActionNr++;
  if (Changed) {
    DataBaseChanged=1;
    UndoRedoActive=1;
  }
...
{
#ifdef _DEBUG
  int32  Layer,TraceInfo,ViaInfo,cnt,cnt2,TracesDeleted[32],
         ViasDeleted,AreaFillMemDeleted,CompsMemDeleted,CompsDeleted,
         ConnectionsDeleted,TotalMemDeleted,TotalMem,
         ObjectLinesDeleted,ObjectRectsDeleted,ObjectCirclesDeleted,
         ObjectArcsDeleted,ObjectTextsDeleted,AreaFillsDeleted,
         MemoryUse;
  TraceRecord  *Trace;
  ViaRecord *Via;
  ObjectLineRecord *ObjectLine;
  ...
  for (cnt=0;cnt<32;cnt++) TracesDeleted[cnt]=0;
  ViasDeleted=0;
  ...
  for (Layer=0;Layer<Design.NrBoardLayers;Layer++) {
    ...
    cnt2=Design.NrVerTraces[Layer]+
         Design.NrHorTraces[Layer]+
         Design.NrDiag1Traces[Layer]+
         Design.NrDiag2Traces[Layer];
    sprintf(str,"Traces layer %i \tTotal nr\t%i\t%i\t\tDeleted\t%i\t%i",
                Layer,cnt2,cnt2*sizeof(TraceRecord),
                TracesDeleted[Layer],
                TracesDeleted[Layer]*sizeof(TraceRecord));
  ...
  sprintf(str,"Vias             \tTotal nr\t%i\t%i\t\tDeleted\t%i\t%i",
              Design.NrVias,Design.NrVias*sizeof(ViaRecord),
              ViasDeleted,
              ViasDeleted*sizeof(ViaRecord));
  if (AddToMessageBuf(str)!=0) return 0;
  ...
  sprintf(str,"Components       \tTotal nr\t%i\t\t\tDeleted\t%i ",
              Design.NrComps,CompsDeleted);
  if (AddToMessageBuf(str)!=0) return 0;
  sprintf(str,"Comps memory \tTotal\t%i\t\t\tDeleted\t\t%i",
              Design.CompsMem,
              CompsMemDeleted);
  if (AddToMessageBuf(str)!=0) return 0;
  TotalMem+=Design.CompsMem;
  TotalMemDeleted+=CompsMemDeleted;
  sprintf(str,"Geometries       \tTotal nr\t%i",
              Design.NrShapes);
  if (AddToMessageBuf(str)!=0) return 0;
  sprintf(str,"Geom memory \tTotal\t%i",
              Design.ShapesMem);
  if (AddToMessageBuf(str)!=0) return 0;
  ...
  sprintf(str,"Connections      \tTotal nr\t%i\t%i\t\tDeleted\t%i\t%i",
              Design.NrConnections,Design.NrConnections*sizeof(ConnectionsRecord),
              ConnectionsDeleted,
  ...
  sprintf(str,"AreaFills        \tTotal nr\t%i\t\t\tDeleted\t%i",
              Design.NrAreaFills,AreaFillsDeleted);
  if (AddToMessageBuf(str)!=0) return 0;
  sprintf(str,"AreaFill mem \tTotal\t\t%i\t\tDeleted\t\t%i",
              Design.AreaFillMem,
              AreaFillMemDeleted);
  if (AddToMessageBuf(str)!=0) return 0;
  ...
  sprintf(str,"Lines            \tTotal nr\t%i\t%i\t\tDeleted\t%i\t%i",
              Design.NrObjectLines,Design.NrObjectLines*sizeof(ObjectLineRecord),
              ObjectLinesDeleted,
  ...
  sprintf(str,"Rectangle        \tTotal nr\t%i\t%i\t\tDeleted\t%i\t%i",
              Design.NrObjectRects,Design.NrObjectRects*sizeof(ObjectRectRecord),
              ObjectRectsDeleted,
...
*/
  ...
  sprintf(str,"Arcs             \tTotal nr\t%i\t%i\t\tDeleted\t%i\t%i",
              Design.NrObjectArcs,Design.NrObjectArcs*sizeof(ObjectArcRecord),
              ObjectArcsDeleted,
  ...
  sprintf(str,"Texts            \tTotal nr\t%i\t%i\t\tDeleted\t%i\t%i",
              Design.NrObjectTexts,Design.NrObjectTexts*sizeof(ObjectTextRecord),
              ObjectTextsDeleted,
  ...
  if (AddToMessageBuf("")!=0) return 0;

// *******************************************************************************************************
// *******************************************************************************************************

  MemoryUse=GetUsedMemSize(1,(LPSTR)&TextBuf);
  if (AddToMessageBuf(TextBuf)!=0) return 0;
  if (AddToMessageBuf("")!=0) return 0;
...

{
  ...
  if (Design.NrBoardLayers==1) return -1;
  if (Layer>=Design.NrBoardLayers) return -1;
...
// *******************************************************************************************************
  ...
  if ((Design.NrBoardLayers==2)
     &&
     (Layer==0)) { // Remove bottom layer, and move top layer objects to bottom
...
// *******************************************************************************************************
  ...
  if (Layer<OldDesign.NrBoardLayers-1) {
    for (cnt=Layer;cnt<OldDesign.NrBoardLayers-1;cnt++) {
      memmove(HorTraces[cnt],HorTraces[cnt+1],
              OldDesign.NrHorTraces[cnt+1]*sizeof(TraceRecord));
      memmove(VerTraces[cnt],VerTraces[cnt+1],
              OldDesign.NrVerTraces[cnt+1]*sizeof(TraceRecord));
      memmove(Diag1Traces[cnt],Diag1Traces[cnt+1],
              OldDesign.NrDiag1Traces[cnt+1]*sizeof(TraceRecord));
      memmove(Diag2Traces[cnt],Diag2Traces[cnt+1],
              OldDesign.NrDiag2Traces[cnt+1]*sizeof(TraceRecord));
    }
  }
...

{
  ...
  memmove(&OldDesign,&Design,sizeof(DesignRecord));
  if (Layer>Design.NrBoardLayers) return -1;
  if (Design.NrBoardLayers>16) return -1;
  ...
  if (Layer<Design.NrBoardLayers-1) {
    ...
    for (cnt=OldDesign.NrBoardLayers-1;cnt>=Layer;cnt--) {
      memmove(HorTraces[cnt+1],HorTraces[cnt],
              OldDesign.NrHorTraces[cnt]*sizeof(TraceRecord));
      memmove(VerTraces[cnt+1],VerTraces[cnt],
              OldDesign.NrVerTraces[cnt]*sizeof(TraceRecord));
      memmove(Diag1Traces[cnt+1],Diag1Traces[cnt],
              OldDesign.NrDiag1Traces[cnt]*sizeof(TraceRecord));
      memmove(Diag2Traces[cnt+1],Diag2Traces[cnt],
              OldDesign.NrDiag2Traces[cnt]*sizeof(TraceRecord));
    }
// *******************************************************************************************************
// *******************************************************************************************************
    for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
...
// *******************************************************************************************************
  ...
  if (((Layer1==0)
     &&
     (Layer2==Design.NrBoardLayers-1))
     ||
     ((Layer2==0)
     &&
     (Layer1==Design.NrBoardLayers-1))) {
...
*/
  ...
  CompMode=0;
  if (((SourceLayer==PLACEMENT_OUTLINE_TOP)
     ||
     (SourceLayer==PLACEMENT_OUTLINE_BOTTOM))
     &&
     ((mode & 1) == 1)) {
    CompMode=1;
    NrObjects=0;
  ...
  if (((SourceLayer==SILKSCREEN_TOP)
     ||
     (SourceLayer==SILKSCREEN_BOTTOM))
     &&
     ((mode & 1) == 1)) {
    CompMode=1;
    NrObjects=0;
  ...
  if ((IsRoutingKeepoutLayer(SourceLayer))
     &&
     ((mode & 1) == 1)) {
    CompMode=1;
    NrObjects=0;
  ...
  if ((SourceLayer==COMP_OUTLINE_LAYER)
     &&
     ((mode & 1) == 1)) {
    CompMode=1;
    NrObjects=0;
    ...
    for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
      ...
      if (((ObjectLine->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
        ...
        if ((OkToCopy)
           ...
           ((((mode & 2) == 0)
           &&
           (ObjectLine->Layer==SourceLayer))
           ||
           (((mode & 2) == 2)
           &&
           (ObjectLine->Layer!=DestinationLayer)))) {
          memmove(&NewObjectLine,ObjectLine,sizeof(ObjectLineRecord));
          switch (DestinationLayer) {
            case SILKSCREEN_TOP:
            case SILKSCREEN_BOTTOM:
...
// *******************************************************************************************************
    for (cnt=0;cnt<Design.NrObjectPolygons;cnt++) {
      ...
      if (((ObjectPolygon->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
        ...
        if ((((mode & 2) == 0)
           &&
           (ObjectPolygon->Layer==SourceLayer))
           ||
           (((mode & 2) == 2)
           &&
           (ObjectPolygon->Layer!=DestinationLayer))) {
    ...
    for (cnt=0;cnt<Design.NrObjectTexts2;cnt++) {
      ...
      if (((ObjectText2->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
        ...
        if ((OkToCopy)
           ...
           ((((mode & 2) == 0)
           &&
           (ObjectText2->Layer==SourceLayer))
           ||
           (((mode & 2) == 2)
           &&
           (ObjectText2->Layer!=DestinationLayer)))) {
          memmove(&NewObjectText2,ObjectText2,sizeof(ObjectTextRecord2));
          ObjectText2->Info&=~OBJECT_SELECTED;
          NewObjectText2.Layer=DestinationLayer;
          switch (DestinationLayer) {
