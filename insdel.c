    ...
    if ((Wire->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
      ...
      if (points2<3) {
        for (cnt2=0;cnt2<Design.NrJunctions;cnt2++) {
          Junction=&((*Junctions)[cnt2]);
          if ((Junction->Info & (OBJECT_NOT_VISIBLE)) == 0) {
            if ((InRange(Junction->X,x2))
               &&
               (InRange(Junction->Y,y2))) {
              Junction->Info|=OBJECT_NOT_VISIBLE;
              Junction->DeleteNr=(int16)LastActionNr;
              DataBaseChanged=1;
              Changed=1;
...

{
  ...
  if ((InRange(Wire->X1,Wire->X2))
     &&
     (InRange(Wire->Y1,Wire->Y2))) {
    return 0;
  }
...

{
  ...
  if (GlobalConnection->Text[0]==0) return 1;
...

{
  ...
  int32 cnt,NrWires,NrBusses,NrJunctions,NrOnePinNets,NrNetLabels,NrBusConnections,
        NrGlobalConnections,NrPins,NrPowerPins,NrPinBusses,
        NrObjectLines,NrObjectRects,NrObjectCircles,NrObjectArcs,NrObjectTexts,
        NrInstances;
...

{
  ...
  for (cnt=0;cnt<Design.NrJunctions;cnt++) {
    Junction=&((*Junctions)[cnt]);
    if ((Junction->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      if ((InRange(Junction->X,x))
         &&
         (InRange(Junction->Y,y))) {
        FoundJunction=cnt;
      }
    }
  }
  ...
  for (cnt=0;cnt<Design.NrWires;cnt++) {
    ...
    if ((Wire->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      x2=Wire->X2;
      y2=Wire->Y2;
      if (((InRange(x1,x)) && (InRange(y1,y)))
         ||
         ((InRange(x2,x)) && (InRange(y2,y)))) {
        if (Found1==-1) {
          Found1=cnt;
        } else {
          if (Found2==-1) {
  ...
  for (cnt=0;cnt<Design.NrInstances;cnt++) {
    Instance=(InstanceRecord *)&((*Instances)[cnt]);
    if ((Instance->Info & OBJECT_NOT_VISIBLE) == 0) {
      if ((x>Instance->BoardPosMinX-0.1)
         &&
         (x<Instance->BoardPosMaxX+0.1)
         &&
         (y>Instance->BoardPosMinY-0.1)
         &&
         (y<Instance->BoardPosMaxY+0.1)) {
        NrObjects=0;
        InstanceToObject(Instance,0.0,0.0,1);
        for (cnt2=0;cnt2<NrObjects;cnt2++) {
          Object=&((*Objects)[cnt2]);
...

{
  ...
  JunctionRecord *Junction;
  int32  cnt2,NrLineJunctions,cnt3,Vertical,
         FoundNetLabelNr1,FoundNetLabelNr2;
  ...
  if (((InRange(x1,x2))
     ...
     (InRange(y1,y2)))
     &&
     (CalcLengthLine(x1,y1,x2,y2)>0.0)) {
    Vertical=0;
    if (InRange(x1,x2)) {
      Vertical=1;
    }
      ...
      for (cnt2=0;cnt2<Design.NrJunctions;cnt2++) {
        if (cnt!=cnt2) {
          ...
          if ((Junction2->Info & (OBJECT_NOT_VISIBLE)) == 0) {
            if ((InRange(Junction->X,Junction2->X))
               &&
               (InRange(Junction->Y,Junction2->Y))) {
...
/*
            ...
            y4=10.0;
*/
            if ((max(x3,x4)>min(x1,x2)-0.01)
               &&
               (max(y3,y4)>min(y1,y2)-0.01)
               &&
               (min(x3,x4)<max(x1,x2)+0.01)
               &&
               (min(y3,y4)<max(y1,y2)+0.01)) {
              if (mode==1) {
                ...
                if (((NotInRange(x1,x3)) || (NotInRange(y1,y3)))
                   &&
                   ((NotInRange(x1,x4)) || (NotInRange(y1,y4)))
                   &&
                   ((NotInRange(x2,x3)) || (NotInRange(y2,y3)))
                   &&
...
*/
  if (mode==0) {
    ...
    if (cnt2>0) {
      sprintf(str,SC(518,"There are %d extra junctions added (selected)"),cnt2);
