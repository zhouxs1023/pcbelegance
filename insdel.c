...

{
  ...
  if (NrObjects>=MaxNrObjects) {
    if (AllocateMemObjects(MaxNrObjects+128)!=0) return 0;
  }
  DataBaseChanged=1;
...

{
  ...
  if (NrObjects2>=MaxNrObjects2) {
    if (AllocateMemObjects2(MaxNrObjects2+128)!=0) return 0;
  }
  Object->AddNr=(int16)LastActionNr;
  Object->DeleteNr=0;
...

{
  ...
  if (NrPinObjects>=MaxNrPinObjects) {
    if (AllocateMemPinObjects(MaxNrPinObjects*2)!=0) return 0;
  }
...
// Check parameters
  ...
  if ((NewGeomCreate.NrPinsX<=0)
     &&
     (NewGeomCreate.NrPinsX>100)) {
    MessageBoxUTF8(GEOMWindow,SC(254,"Nr pins x direction <= 0 or >100"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if ((NewGeomCreate.NrPinsY<=0)
     &&
     (NewGeomCreate.NrPinsY>100)) {
    MessageBoxUTF8(GEOMWindow,SC(255,"Nr pins y direction <= 0 or >100"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.Pitch<100.0) {
    MessageBoxUTF8(GEOMWindow,SC(256,"Pitch < 0.001 mm"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.Pad<100.0) {
    MessageBoxUTF8(GEOMWindow,SC(257,"Pad < 0.001 mm"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.Clearance<100.0) {
    MessageBoxUTF8(GEOMWindow,SC(258,"Clearance < 0.001 mm"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.Pitch<NewGeomCreate.Pad) {
    MessageBoxUTF8(GEOMWindow,SC(259,"Pitch < pad size"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.PasteX>NewGeomCreate.PadX) {
    MessageBoxUTF8(GEOMWindow,SC(260,"Pad size solder paste > pad size"),SC(218,"Warning"),MB_OK);
  }
  NrObjects=0;
  NrPinObjects=0;
  NrObjectPolygons=0;
...
// Check parameters
  ...
  if ((NewGeomCreate.NrPins<=0)
     &&
     (NewGeomCreate.NrPins>1000)) {
    MessageBoxUTF8(GEOMWindow,SC(261,"Nr pins y direction <= 0 or >1000"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.PitchX<100.0) {
    MessageBoxUTF8(GEOMWindow,SC(262,"Pitch x direction < 0.001 mm"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.PitchY<100.0) {
    MessageBoxUTF8(GEOMWindow,SC(263,"Pitch y direction < 0.001 mm"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.Pad<100.0) {
    MessageBoxUTF8(GEOMWindow,SC(257,"Pad < 0.001 mm"),SC(1,"Error"),MB_OK);
  ...
  if (NewGeomCreate.PitchX<NewGeomCreate.Pad) {
    MessageBoxUTF8(GEOMWindow,SC(264,"Pitch x direction < pad size"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.PitchY<NewGeomCreate.Pad) {
    MessageBoxUTF8(GEOMWindow,SC(265,"Pitch y direction < pad size"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.Drill>NewGeomCreate.Pad) {
    MessageBoxUTF8(GEOMWindow,SC(266,"Pad size < drill size"),SC(218,"Warning"),MB_OK);
  }
  NrObjects=0;
  NrPinObjects=0;
  NrObjectPolygons=0;
...
// Check parameters
  ...
  if (NewGeomCreate.PadX<100.0) {
    MessageBoxUTF8(GEOMWindow,SC(267,"Pad x < 0.001 mm"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.PadY<100.0) {
    MessageBoxUTF8(GEOMWindow,SC(268,"Pad y < 0.001 mm"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.Clearance<100.0) {
    MessageBoxUTF8(GEOMWindow,SC(258,"Clearance < 0.001 mm"),SC(1,"Error"),MB_OK);
  ...
  if (NewGeomCreate.PitchY<NewGeomCreate.Pad) {
    MessageBoxUTF8(GEOMWindow,SC(269,"Pitch y direction < pad y direction"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.PasteX>NewGeomCreate.PadX) {
    MessageBoxUTF8(GEOMWindow,SC(270,"Pad x solder paste > pad x"),SC(218,"Warning"),MB_OK);
  }
  if (NewGeomCreate.PasteY>NewGeomCreate.PadY) {
    MessageBoxUTF8(GEOMWindow,SC(271,"Pad y solder paste > pad y"),SC(218,"Warning"),MB_OK);
  }
  NrObjects=0;
  NrPinObjects=0;
  NrObjectPolygons=0;
...
// *******************************************************************************
    ...
    if ((NewGeomCreate.PasteX>0.0)
       &&
       (NewGeomCreate.PasteY>0.0)) {
...
// *******************************************************************************
    ...
    if ((NewGeomCreate.MaskX>0.0)
       &&
       (NewGeomCreate.MaskY>0.0)) {
...
// Check parameters
  ...
  for (cnt=0;cnt<NewGeomCreate.NrPins;cnt++) {
    ...
    Found=-1;
    cnt4=0;
    while ((cnt4<NrPinObjects)
          &&
          (Found==-1)) {
      PinInfo=&((*PinInfos)[cnt4]);
      if (stricmpUTF8((LPSTR)PinInfo->PinText,NewPinText)==0) Found=cnt4;
      cnt4++;
    }
    if (Found==-1) {
      memset(&NewPinInfo,0,sizeof(PinInfoRecord));
...
// *******************************************************************************
    ...
    if (NewGeomCreate.Layer==0) SIL_SMD_RECTObject.Layer=0;
...
// *******************************************************************************
    ...
    if ((NewGeomCreate.PasteX>0.0)
      ...
      if (NewGeomCreate.Layer==0) SIL_SMD_RECTObject.Layer=PASTE_MASK_BOTTOM_LAYER;
...
// *******************************************************************************
    ...
    if ((NewGeomCreate.MaskX>0.0)
      ...
      if (NewGeomCreate.Layer==0) SIL_SMD_RECTObject.Layer=SOLD_MASK_BOTTOM_LAYER;
...
// *******************************************************************************
    ...
    if (NewGeomCreate.Layer==0) SIL_SMD_CIRCLEObject.Layer=0;
...
// *******************************************************************************
    ...
    if (NewGeomCreate.Paste>0.0) {
      ...
      if (NewGeomCreate.Layer==0) SIL_SMD_CIRCLEObject.Layer=PASTE_MASK_BOTTOM_LAYER;
...
// *******************************************************************************
    ...
    if (NewGeomCreate.Mask>0.0) {
      ...
      if (NewGeomCreate.Layer==0) SIL_SMD_CIRCLEObject.Layer=SOLD_MASK_BOTTOM_LAYER;
...

{
  ...
  if ((NewGeomCreate.NrPinsX<=0)
     &&
     (NewGeomCreate.NrPinsX>1000)) {
    MessageBoxUTF8(GEOMWindow,SC(272,"Nr pins x direction <= 0 or >1000"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if ((NewGeomCreate.NrPinsY<=0)
     &&
     (NewGeomCreate.NrPinsY>1000)) {
    MessageBoxUTF8(GEOMWindow,SC(261,"Nr pins y direction <= 0 or >1000"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.Pitch<100.0) {
  ...
  if (NewGeomCreate.Pitch<NewGeomCreate.PadY) {
    MessageBoxUTF8(GEOMWindow,SC(273,"Pitch < pad x"),SC(1,"Error"),MB_OK);
    return -1;
  }
  if (NewGeomCreate.PasteX>NewGeomCreate.PadX) {
    MessageBoxUTF8(GEOMWindow,SC(270,"Pad x solder paste > pad x"),SC(218,"Warning"),MB_OK);
...

{
  ...
  if (mode==0) {
    ...
    for (cnt=0;cnt<NrObjects;cnt++) {
      Object=&((*Objects)[cnt]);
      if ((Object->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
        if (Object->ObjectType==OBJECT_RECT) mode=2;
      }
    }
    if (mode==2) {
