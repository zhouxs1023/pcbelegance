...
DCL DCLinfo;
char Buf[BUF_SIZE], DialogTextLine[MAX_LENGTH_STRING], EditFile[MAX_LENGTH_STRING], NewDir[MAX_LENGTH_STRING],
     CurrentDir[MAX_LENGTH_STRING], WindowStr[MAX_LENGTH_STRING], TempDir[MAX_LENGTH_STRING], CpuVendor[40],
     ExecutableName[MAX_LENGTH_STRING], ZipName[MAX_LENGTH_STRING], InstallDir[MAX_LENGTH_STRING],
     ProjectDir[MAX_LENGTH_STRING], MyDocumentsDir[MAX_LENGTH_STRING], NewArgv[40][MAX_LENGTH_STRING],
     ProgramFilesDir[MAX_LENGTH_STRING], ApplicationDataDir[MAX_LENGTH_STRING], VersionStr[MAX_LENGTH_STRING],
     InstallDirStr[MAX_LENGTH_STRING], ProjectDirStr[MAX_LENGTH_STRING], StartupDir[MAX_LENGTH_STRING];
...
HFONT TextFont;
int32 DrawWindowMinX, DrawWindowMinY, DrawWindowMaxX, DrawWindowMaxY, MessageHeight, TimerValue, ExitValue,
      StandardInstallDir, ClientWindowDivX, ClientWindowDivY, ClientStartX, ClientStartY, Created, Painting, DCInUse,
      MouseChanged, Focused, LeftButtonPressed, RightButtonPressed, ok, LeftButtonDoublePressed, ShortInstallation,
...
{
	char str[MAX_LENGTH_STRING], str2[MAX_LENGTH_STRING], str3[MAX_LENGTH_STRING], FileStr[MAX_LENGTH_STRING], *Path,
	     *FileName, *Extension;
...
int32 MessageDialog(LPSTR Message)
{
	...
	MessagePtr = Message;
	res =
	    DialogBox(INSTALLClass.hInstance, MAKEINTRESOURCE(IDD_MESSAGE_DIALOG), INSTALLWindow, (DLGPROC) MessageDialog2);
 ...
 */
void WINAPI ReceiveDllMessage(unsigned long ucsize, unsigned long csiz, unsigned cfactor, unsigned mo, unsigned dy,
                              unsigned yr, unsigned hh, unsigned mm, char c, LPSTR filename, LPSTR methbuf,
...
int32 CALLBACK TextInputDialog2(HWND Dialog, WORD Message, WORD WParam, int32 LParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_BUTTON1:
			...
			if ((GetDirectory(NewDir)) && (NewDir[0] != 0))
			{
...
//  DialogMode=Mode;
	...
	res =
	    DialogBox(INSTALLClass.hInstance, MAKEINTRESOURCE(IDD_TEXTINPUT_DIALOG), INSTALLWindow,
...
int32 CALLBACK InstallationDialog(HWND Dialog, WORD Message, WORD WParam, int32 LParam)
{
	...
	case WM_INITDIALOG:
		...
		*/
		...
		if (!FoundInstallation)
		{
			EnableWindow(GetDlgItem(Dialog, IDC_STATIC5), FALSE);
			EnableWindow(GetDlgItem(Dialog, IDC_RADIO3), FALSE);
			EnableWindow(GetDlgItem(Dialog, IDC_EDIT4), FALSE);
		}
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			if (SendDlgItemMessage(Dialog, IDC_RADIO1, BM_GETCHECK, 1, 0) == BST_CHECKED)
			{
				...
				res = SendDlgItemMessage(Dialog, IDC_EDIT3, WM_GETTEXT, sizeof(ProjectDir), (LPARAM)ProjectDir);
			...
			else if (SendDlgItemMessage(Dialog, IDC_RADIO2, BM_GETCHECK, 1, 0) == BST_CHECKED)
			{
				memset(InstallDir, 0, sizeof(InstallDir));
				res = SendDlgItemMessage(Dialog, IDC_EDIT1, WM_GETTEXT, sizeof(InstallDir), (LPARAM)InstallDir);
				memset(ProjectDir, 0, sizeof(ProjectDir));
				res = SendDlgItemMessage(Dialog, IDC_EDIT2, WM_GETTEXT, sizeof(ProjectDir), (LPARAM)ProjectDir);

				if ((InstallDir[0] == 0) || (ProjectDir[0] == 0))
				{
					sprintf(str, "Wrong directory names");
					MessageBox(NULL, str, "Error", MB_APPLMODAL | MB_OK);
					return about;
...
// *********************************************************************************************
	...
	if ((res =
	            RegCreateKeyEx(HKEY_LOCAL_MACHINE, str, 0, "", REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &NewKey,
	                           (PDWORD) & KeyInfo)) != ERROR_SUCCESS)
	{
		ok = 1;
		return -2;
	...
	sprintf(str, "PCB Elegance %i.%i", VER_VERSION / 100, VER_VERSION % 100);
...
int32 UnzipPcbElegance(int32 mode)
{
	...
	char str[MAX_LENGTH_STRING], *argv[40];
	...
	strcpy(NewArgv[argc++], "COPYING.txt");
	strcpy(NewArgv[argc++], "NEWS.txt");
	strcpy(NewArgv[argc++], "README.txt");

	for (cnt = 0; cnt < argc; cnt++)
		argv[cnt] = (char *) &NewArgv[cnt];

	DCLinfo.lpszExtractDir = InstallDir;
	retcode = Wiz_SingleEntryUnzip(argc, argv, 0, NULL, &DCLinfo, &UserFunctions);

	if ((retcode != 0) && (retcode != 11))
	{
	...
	DCLinfo.noflag = 0;			/* Over-write all files if 1 */
...
int32 CreateInstallDir(LPSTR Directory)
{
	...
	if ((Length > 2) && (Directory[0] == '\\') && (Directory[1] == '\\'))
	{
	}
	else
	{
		if ((Length < 4) || (Directory[1] != ':'))
			DirNameError = 1;
	}
...
void Start()
{
	int32 res, ok, cnt, cnt2, result, DirNameError, DirSlashCnt, KeyInfo, fp, KeySize, KeyType, LengthInstallDir, Stop;
	HKEY NewKey, Key;
	HANDLE *SharedMemoryHandle;
	PROCESS_INFORMATION ProcessInfo;
	char File1[MAX_LENGTH_STRING], str[MAX_LENGTH_STRING], str2[MAX_LENGTH_STRING], str3[MAX_LENGTH_STRING],
...
// *********************************************************************************************
	...
	if (FoundInstallation)
	{
		sprintf(str, "%s\\PCB Elegance", ProgramFilesDir);
		if (strnicmp(InstallDirStr, str, strlen(str)) == 0)
		{
			StandardInstallDir = 1;
		...
		{
			if (DirectoryExists(ProjectDirStr) == -1)
				strcpy(ProjectDir, "c:\\pcb_elegance");
			else
	...
	{
		result =
		    DialogBox(INSTALLClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_INSTALL), NULL, (DLGPROC) InstallationDialog);
...
void DoneDeviceContext()
{
	...
	{
		if (Painting)
			EndPaint(INSTALLWindow, &PS);
		else
			ReleaseDC(INSTALLWindow, OutputDisplay);
...
//    y=(WindowHeight)/2-IntroBitmapSizeY/2;
		...
		if (!FoundInstallation)
			sprintf(str, "Installing PCB Elegance %i.%i", VER_VERSION / 100, VER_VERSION % 100);
		else
			sprintf(str, "Updating to PCB Elegance %i.%i", VER_VERSION / 100, VER_VERSION % 100);
...
{
	char str[MAX_LENGTH_STRING], str2[MAX_LENGTH_STRING], str3[MAX_LENGTH_STRING], StartUpStr[MAX_LENGTH_STRING],
	     *FileName, *land;
	int32 retcode, fp, fp2, result, FileLength, cnt3, WriteResult, cnt2, cnt, Length, crccnt, cnt4, res, KeyInfo,
	      Feature, KeyType;
	...
	if (!FlagSSE2)
	{
		MessageBox(NULL, "PCB elegance requires a CPU with SSE2 extensions", "Error", MB_APPLMODAL | MB_OK);
		return 0;
	}
	...
	if ((res = RegOpenKeyEx(HKEY_USERS, str, 0, KEY_QUERY_VALUE, &Key)) == ERROR_SUCCESS)
	{
		...
		if ((res =
		            RegQueryValueEx(Key, "Version", 0, NULL, (LPBYTE) & VersionStr, (LPDWORD) & KeySize)) == ERROR_SUCCESS)
			ok = 1;

		KeySize = sizeof(InstallDirStr);

		if ((res =
		            RegQueryValueEx(Key, "InstallDir", 0, NULL, (LPBYTE) & InstallDirStr,
		                            (LPDWORD) & KeySize)) == ERROR_SUCCESS)
			ok = 1;

		if (InstallDirStr[0] != 0)
		{
			sprintf(str, "%s\\design.exe", InstallDirStr);

			if (FileExists(str) == 0)
				FoundInstallation = 1;
		}

		KeySize = sizeof(ProjectDirStr);

		if ((res =
		            RegQueryValueEx(Key, "ProjectDir", 0, NULL, (LPBYTE) & ProjectDirStr,
		                            (LPDWORD) & KeySize)) == ERROR_SUCCESS)
			ok = 1;
...
//  MessageBox(NULL,ExecutableName,"ExecutableName",MB_APPLMODAL|MB_OK);
	...
	if ((result != sizeof(ZipIdent)) || (strcmp(ZipIdent.Ident, ZipIdentString) != 0))
	{
		FileClose(fp);
		MessageBox(NULL, "Internal error", "Error", MB_APPLMODAL | MB_OK);
		return -2;
	...
	INSTALLWindow =
	    CreateWindow(INSTALLClassName, (LPSTR) & WindowStr, WindowStyle, 0, 0, WindowWidth, WindowHeight, HWND_DESKTOP,
	                 0, hInstance, NULL);
