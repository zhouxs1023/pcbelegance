...
int32 NrSpecialComponents,NrMultipleSymbolsComps,ok,
      NrUsableComps,NrPartNames,MaxMem;
...
int32  NrRefCodes,NrCodesPerRef[MAX_REF_TYPES],MaxCodesRef[MAX_REF_TYPES],
       ok,NrSymbNames,ComponentLineOutputCount,UsedLayoutCompRefPtr;

char   SheetRefCodes[MAX_REF_TYPES][8];
int32  NrSheetRefCodes,NrSheetCodesPerRef[MAX_REF_TYPES],RefCodeInfo[MAX_REF_TYPES],
       SheetMaxCodesRef[MAX_REF_TYPES],SheetMinCodesRef[MAX_REF_TYPES];
...

{
  ...
  RefAlpha[0]=0;
  lengte=strlen(Ref);
  if (lengte<2) return -1;
  if (!isalpha(Ref[0])) return -1;

  if ((!isdigit(Ref[lengte-1]))
     &&
     (Ref[lengte-1]!='?')) return -1;

  pos1=0;
  while ((pos1<lengte)
        &&
        (isalpha(Ref[pos1]))) pos1++;
  ...
  if ((Ref[pos1]!='?')
     &&
     (sscanf((LPSTR)&Ref[pos1],"%i",&hulp)!=1)) return -1;

  *Num=hulp;

  FoundNr=-1;
  for (cnt=0;cnt<NrRefCodes;cnt++) {
    if (stricmpUTF8(RefCodes[cnt],RefAlpha)==0) FoundNr=cnt;
  }
  if (FoundNr==-1) {
    ...
    if (NrRefCodes<MAX_REF_TYPES) {
      ...
      if (hulp!=-1) MaxCodesRef[NrRefCodes]=hulp;
      NrRefCodes++;
    }
    return NrRefCodes-1;
  } else {
...

{
  ...
  for (cnt=0;cnt<NrSheetRefCodes;cnt++) {
    if (stricmpUTF8(SheetRefCodes[cnt],Ref)==0) return cnt;
  }
  return -1;
}
...

{
  ...
  for (cnt=0;cnt<NrMultipleSymbolsComps;cnt++) {
    ...
    FoundNr=-1;
    cnt2=0;
    while ((cnt2<NrSymbNames)
          &&
          (stricmpUTF8(Component->InterfaceName,MultipleSymbols[cnt2].SymbName)!=0)) {
      cnt2++;
    }
    if (cnt2<NrSymbNames) {
...
//
  ...
  for (cnt=NrMultipleSymbolsComps;cnt<NrSpecialComponents;cnt++) {
    Component=&((*Components)[cnt]);
    if ((Component->MultiPartNr==CompIndex)
       &&
       (Component->RefNum!=-1)) {
...
{
  int32     cnt,cnt2,cnt3,cnt4,MemPos,res,MemPos2,
            FoundNr,RefNum,Index,NrUsedMultiPartRefs,
            CompCnt,SymbolNr,InstMemSize,
            NrPartsPerRef[4096];
  ...
  for (cnt2=0;cnt2<NrSheets;cnt2++) {
    ...
    if (LoadSheetInMemory(cnt2,3)<0) return -1;
    CompCnt+=Design.NrInstances;
  }
  InstMemSize=((MaxMem+7) & ~7)+CompCnt*sizeof(PartNameRecord)+CompPartMemSize;
  if ((InstMemSize>DesignBufMemSize)
     &&
     (AllocateMemDesignBuf(InstMemSize)!=0)) return -1;

  if (CompCnt+1>MaxNrComponents) AllocateMemComponents(CompCnt);
...
//      if (stricmp(Instance->SymbolName,"MOUNTINGHOLE")==0) {
      ...
      for (cnt3=0;cnt3<Design.NrSymbols;cnt3++) {
        ...
        if (stricmpUTF8(SymbolPos2->SymbolName,Instance->SymbolName)==0) SymbolNr=cnt3;
      }
      if (SymbolNr!=-1) {
...
*/
        ...
        cnt3=0;
        while ((cnt3<NrUsedMultiPartRefs)
              &&
              (stricmpUTF8(str2,UsedMultiPartRefs[cnt3])!=0)) cnt3++;
  ...
  for (cnt2=0;cnt2<NrSymbNames;cnt2++) {
    ...
    for (cnt=0;cnt<NrMultipleSymbolsComps;cnt++) {
      ...
      if (Component->MultiSymbolNr==cnt2) {
        ...
        while ((cnt3<MultipleSymbols[cnt2].NrSymbInterfaceNames)
              &&
              (stricmpUTF8(MultipleSymbols[cnt2].SymbInterfaceNames[cnt3],Component->SymbolName)!=0)) cnt3++;
        if (cnt3<MultipleSymbols[cnt2].NrSymbInterfaceNames) {
          cnt4=0;
          while ((cnt4<NrUsedMultiPartRefs)
                &&
                (stricmpUTF8(str2,UsedMultiPartRefs[cnt4])!=0)) cnt4++;
          if (NrUsedMultiPartRefs<4095) {
...

{
  ...
  if (GetComponentsSheets(0)==-1) return -10;
  switch (mode & 3) {
    case 0:
...

{
  ...
  if (LayoutDesign.NrComps==0) return -1;
...
//    if (OldLayoutComp->Info2==0) {
      if ((Designfp=FileOpenUTF8(PCBFile))==-1) return -1;
...

{
  ...
  LayoutMode=0;
  if ((mode & 2)==2) {
    if ((strcmp(LayoutDesign.Identification,PCBCode)==0)
       ||
       (strcmp(LayoutDesign.Identification,PCBCode2)==0)) {
      LayoutMode=1;
    }
  }
  FileSeek(fp,0);
      ...
      if ((mode & 2)==2) {
        for (cnt=0;cnt<LayoutDesign.NrComps;cnt++) {
          if (LayoutMode==0) {
            ...
            if ((LayoutComp->Info2==0)
               &&
               (stricmpUTF8(LayoutComp->Name,OldRef)==0)
               &&
            ...
            if ((OldLayoutComp->Info2==0)
               &&
               (stricmpUTF8(OldLayoutComp->Name,OldRef)==0)
               &&
...
{
  int32     res,result,Designfp,cnt,cnt2,cnt3,cnt4,cnt5,
            StartNumber,ok,SheetNr,Start,MaxRefs,MaxOccupation,MinDiv;
  ...
  if (MessageBoxUTF8(DESIGNWindow,SC(94,"All references will be renamed, Ok to go on"),"",MB_OKCANCEL)!=IDOK) return -2;
...
//    SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str3);
    ...
    if (Design.AnnotateStartNumber>=1000000) {
      ...
      if (StartNumber>=Start) {
        ...
        sprintf(str3,SC(225,"On sheet %s annotation can not start at %d\r\nIncrease the max number of refs per sheet"),
    ...
    if (Changed) {
      for (cnt=NrMultipleSymbolsComps;cnt<NrSpecialComponents;cnt++) {
        ...
        if ((Component->SheetNr==SheetNr)
          ...
          if (PartName->UsedParts==1) {  // Start new reference
            Component->RefNum=SheetMaxCodesRef[Component->RefCodeNr];
...

{
  ...
  if (cnt==0) cnt++;
//  if (cnt<100) cnt=100;
  if ((Component->RefCodeNr<0)
     ||
     (Component->RefCodeNr>=NrRefCodes)) {
    return -1;
  }
  while ((cnt<MAX_NR_CODES_PER_REF)
        &&
        ((*RefBuf)[Component->RefCodeNr][cnt]==1)) cnt++;
  if (cnt<MAX_NR_CODES_PER_REF) return cnt;
  return -1;
}
...

{
  ...
  int32     res,Designfp,cnt,cnt3,cnt4,SearchedComp,
            Occupation,MaxOccupation,SheetNr,
            RefNum,Start,RefCount,PartNr;
  char      FileName[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING];
      ...
      for (cnt=0;cnt<NrMultipleSymbolsComps;cnt++) {
        ...
        if ((Component->SheetNr==SheetNr)
          ...
          if (cnt3==32) {
            ...
            memmove(&MultipleSymbols[cnt4].RefName[cnt3+1],
                    &MultipleSymbols[cnt4].RefName[cnt3],(32-cnt3-1)*8);
            memmove(&MultipleSymbols[cnt4].ReferenceNr[cnt3+1],
                    &MultipleSymbols[cnt4].ReferenceNr[cnt3],(32-cnt3-1)*2);
    ...
    if (Changed) {
      for (cnt=NrMultipleSymbolsComps;cnt<NrSpecialComponents;cnt++) {
        ...
        if ((Component->SheetNr==SheetNr)
           ...
           (Component->RefNum==-1)) {
          MakeChange=1;
          SearchedComp=SearchMultiPartComp(Component->MultiPartNr,
                                           Component->NrPartsPerPackage,
                                           SheetMaxCodesRef[Component->RefCodeNr]);
          if (SearchedComp==-1) {
            if ((RefNum=SearchRefNum(Component,
                                     SheetMaxCodesRef[Component->RefCodeNr]))!=-1) {
              Component->RefNum=RefNum;
              PartNr=1;
...
/e f:\test4567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789 /p f:\test4567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789
/e e:\pcb_elegance /p e:\pcb_elegance
*/
...
//  return -1;
  if (Mode==1) return 0;
  if (GetComponentsSheets(1)==-1) return -1;
...

{
  ...
  for (cnt2=0;cnt2<NrSheets;cnt2++) {
    sprintf(FileName,"%s%s.sch",SheetDir,Sheets[cnt2].SheetName);
    if (LoadSheetInMemory(cnt2,1)<0) return -2;
    for (cnt=0;cnt<Design.NrInstances;cnt++) {
      Instance=(InstanceRecord *)&((*Instances)[cnt]);
...
*/
    ...
    for (cnt=0;cnt<NrColumns;cnt++) {
      ...
      if (stricmpUTF8(BOMInfo.ColumnStr[cnt2],SC(28,"Position"))==0) {
      ...
      if (stricmpUTF8(BOMInfo.ColumnStr[cnt2],SC(244,"Description"))==0) {
        FoundSystemColumn=1;
...

{
  ...
  if (cnt<length-1) {
    ...
    if (isdigit(CompValueStr[cnt+1])) {
      ...
      while ((cnt2<length)
            &&
            (isdigit(CompValueStr[cnt2]))) cnt2++;
...
{
  int32     cnt,cnt2,cnt3,cnt4,MemPos,fp,fp2,fp3,CompLineCount,ok,ColumnCount=0,
            FoundNr,RefNum,Index,NrRefComps,FoundCompNrR,FoundCompNrC,RefMode,
            CompCnt,SymbolNr,FoundSystemColumn,InstMemSize,CharDelimiterMode,length,
            RefError,ExcludeFromBOM,DoubleRefComponentLineOutputCount,mode2,
            ComponentTypeCount,NrValueComps,NrGeomComps,NrProperties,NrWriteModes,WriteMode,
            CompIndex[MAX_NR_CODES_PER_REF],DoubleRefs[MAX_DOUBLE_REFS],NrDoubleRefs,
            AllCompsPlaced,CompValueIndex[4096],CompGeomIndex[4096];
  double     LowestCompValue;
  char      SymbolName[MAX_LENGTH_STRING],*CompStr,
            FileName[MAX_LENGTH_STRING],FileName2[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],
            str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],RefCompare[80],
            str4[MAX_LENGTH_STRING],str5[MAX_LENGTH_STRING],
            PartStr[MAX_LENGTH_STRING],*InstanceAttrBuf,
            PropertyID[128],PropertyValue[128],
            ValueStr[MAX_LENGTH_STRING],GeomStr[MAX_LENGTH_STRING],
            WriteString[MAX_LENGTH_STRING];
#ifdef _DEBUG
  int32 res;
#endif
...
//  mode|=4;
  ...
  CompCnt=0;
  for (cnt2=0;cnt2<NrSheets;cnt2++) {
    if (LoadSheetInMemory(cnt2,3)<0) return -2;
    CompCnt+=Design.NrInstances;
  }
...
// **********************************************************************************
      ...
      if (SymbolNr!=-1) {
        ...
        if (((Symbol->Info & (SHEET_SYMBOL)) == 0)
          ...
          for (cnt3=0;cnt3<NrProperties;cnt3++) {
            if ((GetProperties(Instance->Properties,(LPSTR)&PropertyID,
                              (LPSTR)&PropertyValue,0x20+cnt3)==0)
               &&
               (PropertyID[0]!='~')) {
              ...
              if ((cnt4==BOMInfo.NrUniqueProperties)
                 &&
                 (BOMInfo.NrUniqueProperties<16)) {
  ...
  if (mode==-1) {
    ...
    cnt=6;
    strcpy(BOMInfo.ColumnStr[0],SC(28,"Position"));
    ...
    strcpy(BOMInfo.ColumnStr[5],SC(244,"Description"));
    for (cnt4=0;cnt4<BOMInfo.NrUniqueProperties;cnt4++) {
...
// ***************************************************************************************************************
  ...
  switch (mode & 3) {
    case 0:
      ...
      if ((fp=FileOpenWriteUTF8(FileName))<=0) return -1;
      if ((fp2=FileOpenWriteUTF8(FileName2))<=0) return -1;
      break;
    case 1:
    case 2:
    case 3:
      ...
      if ((fp=FileOpenWriteUTF8(FileName))<=0) return -1;
      if ((fp2=FileOpenWriteUTF8(FileName2))<=0) return -1;
      WriteLn(fp,SC(109,"Bill Of Materials"));
      WriteLn(fp,"");
  ...
  NrWriteModes=1;
  RefError=0;
  if (AllCompsPlaced==0) NrWriteModes++;
    ...
    for (cnt=0;cnt<NrRefCodes;cnt++) {
      ...
      for (cnt2=0;cnt2<NrRefCodes;cnt2++) {
        if (RefCodeInfo[cnt2]==0) {
          if (stricmp(RefCodes[cnt2],"R      ")==0) FoundCompNrR=cnt2;
          if (stricmp(RefCodes[cnt2],"C      ")==0) FoundCompNrC=cnt2;
  ...
  // **********************************************************************************
          case 0: // List per component
            for (cnt2=0;cnt2<MAX_NR_CODES_PER_REF;cnt2++) CompIndex[cnt2]=-1;
            cnt3=0;
            for (cnt2=0;cnt2<CompCnt;cnt2++) {
              ...
              if (Component->Error==0) {
                if ((((WriteMode==0)
                   &&
                   (Component->PlacingOption==-1))
                   ||
                   ((WriteMode==1)
                   &&
                   (Component->PlacingOption!=-1)))
                   &&
                   (Component->RefCodeNr==FoundNr)
                   &&
                   (Component->RefNum>0)
                   &&
                   (Component->RefNum<MAX_NR_CODES_PER_REF)) {
...
#endif
                  WriteCompLine((LPSTR)&TempMem[ComponentLineOutputCount*256],
                                CompIndex[cnt2],CompCnt,ComponentLineOutputCount+1,
            ...
            for (cnt2=0;cnt2<CompCnt;cnt2++) {
              ...
              if (Component->Error==0) {
                if ((((WriteMode==0)
                   ...
                   (Component->RefNum<0)) {
                  WriteCompLine((LPSTR)&TempMem[ComponentLineOutputCount*256],
                                cnt2,CompCnt,ComponentLineOutputCount+1,
                                BOMInfo.NrColumnsUsed,4);
                  ComponentLineOutputCount++;
                  RefError=1;
                }
              }
            ...
            if (cnt==NrRefCodes-1) {
              ...
              for (cnt2=0;cnt2<NrDoubleRefs;cnt2++) {
                WriteCompLine((LPSTR)&TempMem[ComponentLineOutputCount*256],
                              DoubleRefs[cnt2],CompCnt,ComponentLineOutputCount+1,
                              BOMInfo.NrColumnsUsed,8);
                ComponentLineOutputCount++;
              }
            }
            break;
  ...
  // **********************************************************************************
            for (cnt2=0;cnt2<NrRefComps;cnt2++) {
              Component=&((*Components)[CompIndex[cnt2]]);
              RefMode=2;
              if ((islower(Component->Reference[0]=='r'))
                 &&
                 (isdigit(Component->Reference[1]))) {
                RefMode=0; // Resistor
              }
              if ((islower(Component->Reference[0]=='c'))
                 &&
                 (isdigit(Component->Reference[1]))) {
                RefMode=1; // Capacitor
              }
  ...
  // **********************************************************************************
            ...
            while (!Stop) {
              ...
              for (cnt2=0;cnt2<NrRefComps;cnt2++) {
                ...
                if (((Component->Info & 1)==0)
                   &&
                   (Component->RealCompValue<LowestCompValue)) {
  ...
  */
              if (FoundNr!=-1) {
                ...
                for (cnt2=0;cnt2<NrRefComps;cnt2++) {
                  ...
                  if (((Component->Info & 1)==0)
                     &&
                     (stricmpUTF8(ValueStr,Component->Value)==0)
                     &&
                     (NrValueComps<4096)) {
  ...
  // **********************************************************************************
                ...
                while (!Stop2) {
                  FoundNr=-1;
                  cnt2=0;
                  while ((cnt2<NrValueComps)
                        &&
                        (FoundNr==-1)) {
                    Component=&((*Components)[CompValueIndex[cnt2]]);
                    if ((Component->Info & 1)==0) FoundNr=CompValueIndex[cnt2];
                    cnt2++;
                  }
                  if (FoundNr!=-1) {
                    Component=&((*Components)[FoundNr]);
                    ...
                    for (cnt2=0;cnt2<NrValueComps;cnt2++) {
                      ...
                      if (((Component->Info & 1)==0)
                         &&
                         (stricmpUTF8(GeomStr,Component->Geometry)==0)
                         &&
                         (NrGeomComps<4096)) {
...
// **********************************************************************************
                    ...
                    switch (mode & 3) {
                      case 1: // pos,value,#,geometry
                        ...
                        BOMInfo.ColumnWidth[2]=max(BOMInfo.ColumnWidth[2],(int32)strlen(Component->Geometry));
                        sprintf((LPSTR)&TempMem[ComponentLineOutputCount*256],
                                "%s\t%d\t%s\t",str2,NrGeomComps,Component->Geometry);
                        break;
                      case 2: // pos,value,#,geometry,part nr
                              // pos,part nr,#,geometry,value
                        if (BOMInfo.BOMSortNr==0) {
                          BOMInfo.ColumnWidth[0]=max(BOMInfo.ColumnWidth[0],(int32)strlen(str2));
                          BOMInfo.ColumnWidth[3]=max(BOMInfo.ColumnWidth[3],(int32)strlen(Component->PartNr));
                        } else {
                          BOMInfo.ColumnWidth[0]=max(BOMInfo.ColumnWidth[0],(int32)strlen(Component->PartNr));
                          BOMInfo.ColumnWidth[3]=max(BOMInfo.ColumnWidth[3],(int32)strlen(str2));
                        }
                        BOMInfo.ColumnWidth[2]=max(BOMInfo.ColumnWidth[2],(int32)strlen(Component->Geometry));
                        if (BOMInfo.BOMSortNr==0) {
                          sprintf((LPSTR)&TempMem[ComponentLineOutputCount*256],
                                  "%s\t%d\t%s\t%s\t",str2,NrGeomComps,
                                  Component->Geometry,Component->PartNr);
                        } else {
                          sprintf((LPSTR)&TempMem[ComponentLineOutputCount*256],
                      ...
                      case 3: // pos,value,#,geometry,part nr,description
                        ...
                        BOMInfo.ColumnWidth[2]=max(BOMInfo.ColumnWidth[2],(int32)strlen(Component->Geometry));
                        BOMInfo.ColumnWidth[4]=max(BOMInfo.ColumnWidth[4],(int32)strlen(Component->Description));
                        if (BOMInfo.BOMSortNr==0) {
                          sprintf((LPSTR)&TempMem[ComponentLineOutputCount*256],
                                  "%s\t%d\t%s\t%s\t%s\t",str2,NrGeomComps,
                                  Component->Geometry,Component->PartNr,Component->Description);
                        } else {
                          sprintf((LPSTR)&TempMem[ComponentLineOutputCount*256],
    ...
    switch (mode & 3) {
      case 0:
        for (cnt2=0;cnt2<BOMInfo.NrColumnsUsed;cnt2++) {
          FoundSystemColumn=0;
          if (stricmpUTF8(BOMInfo.ColumnStr[BOMInfo.ColumnsUsed[cnt2]],SC(28,"Position"))==0) {   // 4 characters
            BOMInfo.ColumnWidth[cnt2]=max(BOMInfo.ColumnWidth[cnt2],(int32)strlen(SC(114,"pos")));
            FoundSystemColumn=1;
          }
          if (stricmpUTF8(BOMInfo.ColumnStr[BOMInfo.ColumnsUsed[cnt2]],SC(30,"Reference"))==0) {  // 9 characters
          ...
          if (stricmpUTF8(BOMInfo.ColumnStr[BOMInfo.ColumnsUsed[cnt2]],SC(33,"PartNr"))==0) {
            BOMInfo.ColumnWidth[cnt2]=max(BOMInfo.ColumnWidth[cnt2],(int32)strlen(SC(111,"Part nr")));
            FoundSystemColumn=1;
          }
          if (stricmpUTF8(BOMInfo.ColumnStr[BOMInfo.ColumnsUsed[cnt2]],SC(244,"Description"))==0) {
            BOMInfo.ColumnWidth[cnt2]=max(BOMInfo.ColumnWidth[cnt2],(int32)strlen(SC(244,"Description")));
            FoundSystemColumn=1;
          }
          if (!FoundSystemColumn) {
            BOMInfo.ColumnWidth[cnt2]=max(BOMInfo.ColumnWidth[cnt2],(int32)strlen(BOMInfo.ColumnStr[BOMInfo.ColumnsUsed[cnt2]]));
          }
        }
        ...
        for (cnt2=0;cnt2<BOMInfo.NrColumnsUsed;cnt2++) {
          ...
          str3[0]=0;
          if (stricmpUTF8(BOMInfo.ColumnStr[BOMInfo.ColumnsUsed[cnt2]],SC(28,"Position"))==0) {
            strcpy(str3,SC(114,"pos"));
            FoundSystemColumn=1;
          }
          ...
          if (stricmpUTF8(BOMInfo.ColumnStr[BOMInfo.ColumnsUsed[cnt2]],SC(33,"PartNr"))==0) {
            strcpy(str3,SC(111,"Part nr"));
            FoundSystemColumn=1;
          }
          if (stricmpUTF8(BOMInfo.ColumnStr[BOMInfo.ColumnsUsed[cnt2]],SC(244,"Description"))==0) {
            strcpy(str3,SC(244,"Description"));
            FoundSystemColumn=1;
          }
          if (!FoundSystemColumn) {
      ...
      case 3: // pos,value,#,geometry,part nr,description
        ...
        if (BOMInfo.BOMSortNr==0) {
          BOMInfo.ColumnWidth[0]=max(BOMInfo.ColumnWidth[0],(int32)strlen(SC(110,"Component value")));
          BOMInfo.ColumnWidth[3]=max(BOMInfo.ColumnWidth[3],(int32)strlen(SC(111,"Part nr")));
        } else {
          BOMInfo.ColumnWidth[0]=max(BOMInfo.ColumnWidth[0],(int32)strlen(SC(111,"Part nr")));
          BOMInfo.ColumnWidth[3]=max(BOMInfo.ColumnWidth[3],(int32)strlen(SC(110,"Component value")));
        }
        BOMInfo.ColumnWidth[4]=max(BOMInfo.ColumnWidth[4],(int32)strlen(SC(244,"Description")));
        strcpy(WriteString,"+");
        strcpy(str,"|");
        str2[0]=0;
        str5[0]=0;
          ...
          switch (cnt2) {
            case 0:
              if ((BOMInfo.BOMSortNr==0)
                 ||
                 ((mode & 3)==1)) {
                strcpy(str3,SC(110,"Component value"));
              } else {
                strcpy(str3,SC(111,"Part nr"));
              }
              break;
...
*/
  ...
  DeallocateMem();
//  SendMessage(EditWindow,WM_SETTEXT,0,(LPARAM)(LPSTR)"");
  switch (mode & 3) {
    case 0:
      sprintf(str2,SC(115,"Bill of materials -> %s\\component.txt ready\r\n"),DesignPath);
      AddMessage(str2);
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)(LPSTR)str2);
      if ((mode & 8)==0) {
        sprintf(str2,SC(116,"Bill of materials -> %s\\component.tdl ready\r\n"),DesignPath);
      } else {
        sprintf(str2,SC(279,"Bill of materials -> %s\\component.csv ready\r\n"),DesignPath);
      }
      AddMessage(str2);
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)(LPSTR)str2);
      break;
    ...
    case 3:
      sprintf(FileName,"%s\\%s.bom",DesignPath,LayoutFile);
      sprintf(str,SC(117,"Bill of materials -> %s ready\r\n"),FileName);
      AddMessage(str);
      if ((mode & 8)==0) {
        sprintf(str,SC(118,"Bill of materials -> %s ready (.tdl file)\r\n"),FileName2);
      } else {
        sprintf(str,SC(280,"Bill of materials -> %s ready (.csv file)\r\n"),FileName2);
      }
      AddMessage(str);
      break;
  }
...
//  SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
  ...
  for (cnt2=0;cnt2<NrSheets;cnt2++) {
    ...
    if (LoadSheetInMemory(cnt2,0)<0) {
      MessageBoxUTF8(DESIGNWindow,SC(275,"Error in loading sheets"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
      return -1;
    }
...

{
  char  FileName[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
        SearchFileName[MAX_LENGTH_STRING],
        ShapeFileName[MAX_LENGTH_STRING],GeometryLibNames[64][MAX_LENGTH_STRING];
  int32 result,Designfp,cnt,Libfp,niks,CompsMemLength,PinsLength,cnt2,res,NrLibEntries,
        *CompPos,NrGeometryLibFiles,cnt3,TempMaxCompsMemory,Pos,SizeFile,fp,Found;
...
//  SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
  ...
  if ((Designfp=FileOpenReadOnlyUTF8(PCBFile))==-1) return -3;
  if (FileRead(Designfp,&LayoutDesign,
                         sizeof(PCBDesignRecord),&result)==-1) return -3;
  if ((stricmp(LayoutDesign.Identification,PCBCode)!=0)
     &&
     (stricmp(LayoutDesign.Identification,PCBCode2)!=0)
     &&
     (stricmp(LayoutDesign.Identification,PCBCode3)!=0)
     &&
     (stricmp(LayoutDesign.Identification,PCBCode4)!=0)) return -4;
//  if (FileSeek(Designfp,sizeof(PCBDesignRecord)+Design.BoardOutlineSize)==-1) return -3;
  ...
  if (AllocateMemTemp(TempMaxCompsMemory)==-1) return -1;
  LayoutComps=(CompsArray *)&TempMem[0];
  LayoutCompsMem=&TempMem[sizeof(int32)*(int)LayoutDesign.NrComps];
  if (FileRead(Designfp,LayoutComps,sizeof(int32)*(int)LayoutDesign.NrComps,&result)==-1) return -3;
  if ((strcmp(LayoutDesign.Identification,PCBCode)==0)
     ||
     (strcmp(LayoutDesign.Identification,PCBCode2)==0)) {
    CompsMemLength=0;
    for (cnt=0;cnt<LayoutDesign.NrComps;cnt++) {
      if (FileRead(Designfp,&OldComp,sizeof(OldCompRecord),&result)==-1) return -3;
      ...
      *CompPos=CompsMemLength;
      ...
      memmove((uint8 *)&NewComp->PartDescription ,(uint8 *)&OldComp.PartDescription      ,sizeof(OldComp.PartDescription));
      ...
      if (FileRead(Designfp,CompP,PinsLength,&result)==-1) return -3;
      CompsMemLength+=PinsLength;
    }
  } else {
    if (FileRead(Designfp,LayoutCompsMem,
                 (int)LayoutDesign.CompsMem,&result)==-1) return -3;
  }
//  Comp=CompsMem[0];
...
*/
        ...
        while ((!Found)
              &&
              (cnt3<NrGeometryLibFiles)) {
          if ((Libfp=FileOpenReadOnlyUTF8(GeometryLibNames[cnt3]))==-1) return -1;
          if (FileRead(Libfp,&Lib,sizeof(LibRecord),&result)==-1) return -1;
          if (strcmp(Lib.Identification,LibraryCode2)==0) {
            ...
            while ((!Found)
                  &&
                  (cnt2<NrLibEntries)) {
              if (FileRead(Libfp,&LibName,sizeof(LibNameRecord),&result)==-1) return -1;
...
//  if (FileSeek(Designfp,sizeof(PCBDesignRecord)+LayoutDesign.BoardOutlineSize)==-1) return -3;
  if (AllocateMemTemp(sizeof(int32)*(int)LayoutDesign.NrComps+LayoutDesign.CompsMem)==-1) return -1;
  LayoutComps=(CompsArray *)&TempMem[0];
  LayoutCompsMem=&TempMem[sizeof(int32)*(int)LayoutDesign.NrComps];
  if (FileRead(Designfp,LayoutComps,sizeof(int32)*(int)LayoutDesign.NrComps,&result)==-1) return -3;
  if (FileRead(Designfp,LayoutCompsMem,
  ...
  for (cnt4=0;cnt4<NrObjects;cnt4++) {
    ...
    switch (Object->ObjectType) {
      ...
      case SYMBOL_PINBUS:
          ...
          if ((RedefinedPinBus->Info & OBJECT_NOT_VISIBLE) == 0) {
            if ((stricmpUTF8(RedefinedPinBus->Reference,Reference)==0)
              ...
              for (cnt3=0;cnt3<Object->Info3;cnt3++) {
                res=GetPinNameFromPinBus(Object->Text1,str2,Object->Info3,
                                         RedefinedPinBus->Order[cnt3]);
                if (res>LineNr) strcat(NewPinBusStr,"\\");
        ...
        if (ChangedPinBus) {
          sprintf(str,"PINBUS  %i '%s'  %i  %s",Object->Info3,NewPinBusStr,
                                                Object->Info2,Object->Text2);
        } else {
          sprintf(str,"PINBUS  %i '%s'  %i  %s",Object->Info3,Object->Text1,
                                                Object->Info2,Object->Text2);
        }
        WriteLn(fp2,str);
        break;
...
// NrObjects
  ...
  //        WriteLn(fp2,";---------------------------------------------------------------");
          ...
          if (Symbol->NrPartsPerPackage>1) {
            ...
            strcpy(str2,"        ");
            memmove(&str2,&NewInstance->Reference,strlen(NewInstance->Reference));
            sprintf(str,"GATE    %s   %3i  %3i   %s  %i",str2,NewInstance->PackagePartNr,
                                                         GateNr,NewInstance->Value,NrGates);
  ...
  //          WriteLn(fp2,";---------------------------------------------------------------");
            ...
            for (cnt2=cnt+1;cnt2<NrNewInstances;cnt2++) {
              NewInstance2=&((*NewInstances)[cnt2]);
              if (((NewInstance2->Info & 1) == 0)
                 &&
                 (NewInstance2->AddNr==SymbolNr)
                 &&
...
// **********************************************************************************
  ...
  for (cnt=0;cnt<NrNewInstances;cnt++) {
    ...
    if (NewInstance->AddNr>=0) {
      Symbol=(SymbolRecord *) &((*NewSymbolsMem)[(*NewSymbolsPos2)[NewInstance->AddNr].Pos]);
      if (((NewInstance->Info & (MULTIPLE_SYMBOLS|2)) == 0)
         &&
         (Symbol->NrPartsPerPackage>1)) {
        ...
        for (cnt3=1;cnt3<=Symbol->NrPartsPerPackage;cnt3++) {
          ...
          for (cnt2=0;cnt2<NrNewInstances;cnt2++) {
            ...
            if (((NewInstance2->Info & (MULTIPLE_SYMBOLS|2)) == 0)
               &&
               (NewInstance2->PackagePartNr==cnt3)
               &&
               (NewInstance2->AddNr==SymbolNr)
               &&
          ...
          if (!Found) {
            ...
            sprintf(str,"GATE    %s   %3i  %3i   %s  %i",str2,NewInstance3.PackagePartNr,
                                                         NewInstance->Code1,NewInstance3.Value,
                                                         NrGates);
            WriteLn(fp2,str);
            NrGates++;
...

{
  ...
  if (MaxMem+65536>DesignBufMemSize) {
    if (AllocateMemDesignBuf(MaxMem+65536)==-1) return -1;
  }
  ok=1;
  for (cnt2=0;cnt2<NrSheets;cnt2++) {
    FileChanged=0;
    ...
    sprintf(FileName,"%s%s.sch",SheetDir,Sheets[cnt2].SheetName);
    if ((res=LoadSheetInMemory(cnt2,0))<0) return -2;


    CutExtensionFileName(FileName);
    strcat(FileName,"_new.sch");
    if ((Designfp=FileOpenWriteUTF8(FileName))<=0) return -4;
