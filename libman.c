...
RECT                        RectListWindow1,
                            RectListWindow2;
HRGN                        EditingRegion;
HWND                        LIBMANWindow;
HDC                         LIBMANDisplay,OutputDisplay;
HFONT                       NewFont;
int32                       TimerValue,WindowWidth,WindowHeigth,SymbolsSelected,
                            DialogMode,NrLibEntries,NrParams,MouseCount,ok,
                            ScreenSizeX,ScreenSizeY,WindowStartX,WindowStartY,
                            DCInUse,Painting,ProjectActive,
                            DrawWindowMinX,
                            DrawWindowMinY,
                            DrawWindowMaxX=10000,
                            DrawWindowMaxY=10000,
                            MousePosX,MousePosY,
                            SymbolToBeEditedPos[2],
                            SymbolCountToBeEditedPos[2],
                            OperatingSystem,
                            ReverseY=1,
                            LibIndex[MAX_NR_LIB_SYMBOLS+10],
                            OkToUseSharedMemory,
                            BackupMade[2],
                            ProjectIndexNr=-1,
                            TotalNrLibmanNames,
                            FoundInstallation,
                            LibraryMode,  // 0 = symbol   1 = geometry
                            MaxTempMemory,
                            TempMemoryPos,
                            TextBufPos,TextBufLength,TextLineNr,FileP,FileP2,
                            ListWindowWidth=420,
                            ListWindowHeight=350,
                            ButtonSizeX=120,
                            ButtonSizeY=25,
                            ButtonStepX=150,
                            ButtonStepY=35;
...
uint32                      TimerObject,
                            TimerIdentifier=0x12345678,
                            StartUpMessage;
HMENU                       MainMenu,SubMenu;
char                        *Buf,ExportDir[MAX_LENGTH_STRING],
                            ExecutableDir[MAX_LENGTH_STRING],
                            VersionStr[MAX_LENGTH_STRING],
                            InstallDirStr[MAX_LENGTH_STRING],
                            ProgramFilesDir[MAX_LENGTH_STRING],
                            ProjectDirStr[MAX_LENGTH_STRING],
                            ProjectPath[MAX_LENGTH_STRING],
                            DefaultLibraryPath[MAX_LENGTH_STRING],
                            DefaultSymbolPath[MAX_LENGTH_STRING],
                            *TextBuf,
                            LibraryText1[MAX_LENGTH_STRING],
                            LibraryText2[MAX_LENGTH_STRING],
                            EditPath[MAX_LENGTH_STRING],
                            LibraryFile[2][MAX_LENGTH_STRING],
                            DialogTextLine[MAX_LENGTH_STRING],
                            NewLibName[MAX_LENGTH_STRING],
                            Params[8][MAX_LENGTH_STRING],
                            ExePath[MAX_LENGTH_STRING],
                            SymbolToBeEdited[2][20][MAX_LENGTH_STRING],
                            TempPath[MAX_LENGTH_STRING],
                            SymbolImportDir[MAX_LENGTH_STRING];
...
/*
    sprintf(str3,SC(0,"Error in string number %d of the language file (Wrong parameters)."),Nr);
    strcat(str3,SC(1,"\n\nInstead the english string will be used"));
    MessageBox(NULL,str3,SC(2,"System error"),MB_APPLMODAL|MB_OK);
*/
    return str;
  }
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      sprintf(str,SC(57,"Build version %i.%i.%i  [ %s ]"),
...
#endif
      SendDlgItemMessage(Dialog,IDC_EDIT1,WM_SETTEXT,0,(LPARAM)(LPSTR)str);
      return about;
    case WM_MOVE:
      break;
    case WM_COMMAND:
...

{
  ...
  while ((cnt<32)
        &&
        (ProjectInfo->FileTypes[cnt]!=0)) cnt++;
  if (cnt==32) return;
  ProjectIndexNr=cnt;
  if (LibraryMode==0) {
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      SetDialogItemText(Dialog,IDOK                       ,SC(3,"OK"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(4,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(5,"Select directory"));
...
{
  if ( DCInUse ) {
    if ( Painting ) EndPaint(LIBMANWindow, &PS);
    else ReleaseDC(LIBMANWindow, LIBMANDisplay);
    DCInUse=0;
    LIBMANDisplay=NULL;
    OutputDisplay=NULL;
  }
...
{
  if (SavePen==(HGDIOBJ)-1) SavePen=SelectObject(OutputDisplay,WhitePen);
  else SelectObject(OutputDisplay,WhitePen);
  LineColor=RGB_White;
}
...
{
  if (SavePen==(HGDIOBJ)-1) SavePen=SelectObject(OutputDisplay,BlackPen);
  else SelectObject(OutputDisplay,BlackPen);
  LineColor=RGB_Black;
}
...
{
  if (SavePen==(HGDIOBJ)-1) SavePen=SelectObject(OutputDisplay,GrayPen);
  else SelectObject(OutputDisplay,GrayPen);
  LineColor=RGB_Gray;
}
...
{
  if (SavePen==(HGDIOBJ)-1) SavePen=SelectObject(OutputDisplay,LightGrayPen);
  else SelectObject(OutputDisplay,LightGrayPen);
...

{
  ...
  if (SaveFont==(HGDIOBJ)-1) SaveFont=SelectObject(OutputDisplay,NewFont);
  else SelectObject(OutputDisplay,NewFont);
...

{
  ...
  if ( GetUpdateRect(LIBMANWindow,&UpdateRect,0) ) {
    ...
    rect4a(RectListWindow1.left-7,RectListWindow1.top-30,
          RectListWindow1.right+32,RectListWindow1.bottom+113,0);
    rect4a(RectListWindow2.left-7,RectListWindow2.top-30,
          RectListWindow2.right+32,RectListWindow2.bottom+113,0);
    InitDrawingColorWhite();
    rect4a(RectListWindow1.left+1,RectListWindow1.top-23,
          RectListWindow1.right+24,RectListWindow1.top-6,0);
    rect4a(RectListWindow2.left+1,RectListWindow2.top-23,
          RectListWindow2.right+24,RectListWindow2.top-6,0);
...

{
  ...
  if (FileRead(Libfp,&LibNames[ListNr],Lib[ListNr].NrLibEntries*sizeof(LibNameRecord),&result)<0) return -1;
  FileClose(Libfp);
  if (ListNr==0) {
  ...
  for (cnt=0;cnt<MAX_NR_LIB_SYMBOLS;cnt++) LibIndex[cnt]=-1;
...

{
  ...
  switch ( Message ) {
    ...
    case WM_COMMAND:
      switch (WParam) {
        case IDOK:
          if (SendDlgItemMessageUTF8(Dialog,IDC_EDIT1,WM_GETTEXT,MAX_LENGTH_STRING-50,
                                     (LPARAM)(LPSTR)&DialogTextLine)>0) {
...

{
  ...
  LibraryFile[ListNr][0]=0;
  if (GetNewFileUTF8(LIBMANWindow,LIBMANClass.hInstance,(char *)&LibraryFile[ListNr],
                     DefaultLibraryPath,str5,NULL,str5,str2,0)!=0) {
    return 0;
  }
...
{
  int32 fp,fp2,fp3,result,LibraryFileSize,res2,LibSymbolDataPos,
        res,SymbolFileSize,SymbolIndexNr,FilePos,BytesToWrite,cnt;
  ...
  if (LibraryMode==0) {
    if (FileRead(fp,&Symbol,sizeof(SymbolRecord),&result)==0) {
      if ((strcmp((LPSTR)&Symbol,SymbolCode1)!=0)
         &&
         (strcmp((LPSTR)&Symbol,SymbolCode2)!=0)
         &&
    ...
    if (FileRead(fp,&Shape,sizeof(ShapeRecord),&result)==0) {
      if ((strcmp((LPSTR)&Shape,ShapeCode)!=0)
         &&
         (strcmp((LPSTR)&Shape,ShapeCode2)!=0)) {
        if ((mode & 1) == 1) {
  ...
  if ((mode & 1) == 1) {
    if (LibraryMode==0) {
      ...
      if (SymbolFileSize>4096*1024) {
        MessageBoxUTF8(LIBMANWindow,SC(61,"Geometry size >4096 kbyte"),SC(9,"Error"),MB_APPLMODAL|MB_OK);
        return -5;
      }
    }
  }
    ...
    if (stricmp(LibNames[ListNr][cnt].Text,SymbolName)==0) {
      if (SymbolIndexNr==-1) {
        ...
        if ((mode & 1) == 1) {
          ...
          res=MessageBoxUTF8(LIBMANWindow,str,SC(60,"Message"),MB_APPLMODAL|MB_YESNOCANCEL);
          switch (res) {
            case IDYES:
              mode|=2;
              break;
...
//
    if (Lib[ListNr].NrLibEntries < Lib[ListNr].MaxNrLibEntries) {
      ...
      while (result>0) {
        ...
        if (result>0) FileWrite(fp,Buf,result,&res2);
      }
      FileClose(fp3);
      FileClose(fp);
    } else {
...
//
      ...
      if ((fp2=FileOpenWriteUTF8(str))>0) {
        ...
        while (result>0) {
          ...
          if (result>0) FileWrite(fp2,Buf,result,&res2);
        }
        FileClose(fp);
...

{
  ...
  if (ListNr==0) {
    SymbolsSelected=SendMessage(ListWindow1,LB_GETSELITEMS,MAX_NR_LIB_SYMBOLS+1,
                                (LPARAM)(LPINT)&LibIndex);
  } else {
    SymbolsSelected=SendMessage(ListWindow2,LB_GETSELITEMS,MAX_NR_LIB_SYMBOLS+1,
                                (LPARAM)(LPINT)&LibIndex);
  }

  if (SymbolsSelected<1) return -1;
  MakeBackup(ListNr);
  ...
  for (cnt=SymbolsSelected-1;cnt>=0;cnt--) {
    ...
    cnt2=0;
    while ((cnt2<Lib[ListNr].NrLibEntries)
          &&
          (stricmp(LibNames[ListNr][cnt2].Text,str)!=0)) cnt2++;
  ...
  for (cnt=0;cnt<Lib[ListNr].NrLibEntries;cnt++) {
    ...
    if (LibNamesDeleted[cnt]==0) {
      if (count!=cnt) memmove(&LibNames[ListNr][count],&LibNames[ListNr][cnt],sizeof(LibNameRecord));
...

{
  ...
  char   str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],
         SymbolFile[MAX_LENGTH_STRING],ExportDir2[MAX_LENGTH_STRING];

  if (ListNr==0) {
    SymbolsSelected=SendMessage(ListWindow1,LB_GETSELITEMS,MAX_NR_LIB_SYMBOLS,
                                (LPARAM)(LPINT)&LibIndex);
  } else {
    SymbolsSelected=SendMessage(ListWindow2,LB_GETSELITEMS,MAX_NR_LIB_SYMBOLS,
                                (LPARAM)(LPINT)&LibIndex);
  }
  if (SymbolsSelected<1) return -1;
  ...
  for (cnt=0;cnt<SymbolsSelected;cnt++) {
    ...
    while ((cnt2<Lib[ListNr].NrLibEntries)
          &&
          (stricmp(LibNames[ListNr][cnt2].Text,str3)!=0)) cnt2++;
    if (cnt2<Lib[ListNr].NrLibEntries) {
...
//      MessageBoxUTF8(LIBMANWindow,FileName2,"Add symbol",MB_APPLMODAL|MB_OK);
  ...
  if (mode==0) {
    if (count>0) {
      if (LibraryMode==0) {
        sprintf(str2,SC(62,"%d symbols added/replaced for library %s"),count,LibraryFile[ListNr]);
      } else {
        sprintf(str2,SC(63,"%d geometries added/replaced for library %s"),count,LibraryFile[ListNr]);
      }
      MessageBoxUTF8(LIBMANWindow,str2,SC(60,"Message"),MB_OK);
    }
  }
  return 0;
}
...

{
  ...
  if (LibraryMode==0) {
    if (GetNewFileUTF8(LIBMANWindow,LIBMANClass.hInstance,(char *)TempMem,
                       SymbolImportDir,str5,NULL,str5,"sym",6)==0) {
      return 1;
    }
  } else {
    if (GetNewFileUTF8(LIBMANWindow,LIBMANClass.hInstance,(char *)TempMem,
                   SymbolImportDir,str5,NULL,str5,"shp",6)==0) {
      return 1;
    }
  }
  return 0;
...

{
  ...
  char   str[MAX_LENGTH_STRING*5],SymbolFile[600],
...
//            sprintf(str2,"%s\\sch.exe \"%s\" /e \"%s\" /a /w %x /z %d",ExePath,SymbolFile,ExePath,LIBMANWindow,pos);
            ...
            sprintf(ExeParams,"\"%s\\sch.exe\" \"%s\" /e \"%s\" /a /w %x /z %d",
                    ExePath,SymbolFile,ExePath,(int32)LIBMANWindow,pos);
...
//            MessageBoxUTF8(LIBMANWindow,SymbolFile,"edit geom file(2)",MB_APPLMODAL+MB_OK);
            strcpy(SymbolToBeEdited[ListNr][SymbolToBeEditedPos[ListNr]],SymbolFile);
            SymbolToBeEditedPos[ListNr]++;
            sprintf(ExeParams,"\"%s\\geom.exe\" \"%s\" /e \"%s\" /a /w %x /z %d",
                    ExePath,SymbolFile,ExePath,(int32)LIBMANWindow,pos);
            StartupInfo.cb=sizeof(StartupInfo);
            StartupInfo.wShowWindow=SW_SHOW;
            CreateProcess(ExeFile,ExeParams,NULL,NULL,1,0,NULL,NULL,&StartupInfo,&ProcessInfo);
...

{
  ...
  if (SymbolsSelected==1) {
    ...
    TrackPopupMenu(PopUpMenu,TPM_LEFTALIGN+TPM_RIGHTBUTTON,
                   RealWindow.left+MousePosX+5,RealWindow.top+MousePosY+40,0,LIBMANWindow,NULL);
...

{
  ...
  ListWindow1 = CreateWindowExW(WS_EX_ACCEPTFILES,
                               L"LISTBOX",
                               L"",
                               WS_CHILD|WS_BORDER|LBS_SORT|LBS_NOTIFY|WS_VSCROLL|
                               WS_THICKFRAME|
//                               LBS_MULTIPLESEL|
                               LBS_EXTENDEDSEL|
                               LBS_DISABLENOSCROLL,
// LBS_WANTKEYBOARDINPUT
                               RectListWindow1.left,
                               RectListWindow1.top,
                               ListWindowWidth,
                               ListWindowHeight,
                               LIBMANWindow,
...
//  RectListWindow2.left=460;
  ...
  ListWindow2 = CreateWindowExW(WS_EX_ACCEPTFILES,
                               L"LISTBOX",
                               L"LIST",
                               WS_CHILD|WS_BORDER|LBS_SORT|LBS_NOTIFY|WS_VSCROLL|
                               WS_THICKFRAME|
...
//  Utf8ToUnicode("New lib и",Wstr,200);
  ...
  CreateNewLibButton1 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                     RectListWindow1.left+ButtonStepX*0,RectListWindow1.bottom+14,
                                     ButtonSizeX,ButtonSizeY,
                                     LIBMANWindow,(HMENU)ID_CREATE_LIB1,hInstance,NULL);

  ShowWindow(CreateNewLibButton1,SW_SHOW);
  Utf8ToUnicode(SC(33,"Open lib"),Wstr,200);
  OpenLibButton1 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                RectListWindow1.left+ButtonStepX,RectListWindow1.bottom+14,
                                ButtonSizeX,ButtonSizeY,
                                LIBMANWindow,(HMENU)ID_OPEN_LIB1,hInstance,NULL);
...
*/
  ...
  CloseLibButton1 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                 RectListWindow1.left+ButtonStepX*2,RectListWindow1.bottom+14,
                                 ButtonSizeX,ButtonSizeY,
                                 LIBMANWindow,(HMENU)ID_CLOSE_LIB1,hInstance,NULL);
  ShowWindow(CloseLibButton1,SW_SHOW);
  if (LibraryMode==0) {
    Utf8ToUnicode(SC(35,"Import symbols"),Wstr,200);
    ImportSymbolsButton1 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                        RectListWindow1.left+ButtonStepX*1,RectListWindow1.bottom+ButtonStepY+14,
                                        ButtonSizeX,ButtonSizeY,
                                        LIBMANWindow,(HMENU)ID_IMPORT_SYMBOLS1,hInstance,NULL);
    ShowWindow(ImportSymbolsButton1,SW_SHOW);
    Utf8ToUnicode(SC(36,"Export symbols"),Wstr,200);
    ExportSymbolsButton1 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                        RectListWindow1.left+ButtonStepX*2,RectListWindow1.bottom+ButtonStepY+14,
                                        ButtonSizeX,ButtonSizeY,
                                        LIBMANWindow,(HMENU)ID_EXPORT_SYMBOLS1,hInstance,NULL);
  ...
  DeselectAllButton1 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                    RectListWindow1.left+ButtonStepX*0,RectListWindow1.bottom+ButtonStepY*2+14,
                                    ButtonSizeX,ButtonSizeY,
                                    LIBMANWindow,(HMENU)ID_DESELECT_ALL1,hInstance,NULL);
  ShowWindow(DeselectAllButton1,SW_SHOW);
  Utf8ToUnicode(SC(58,"Select all"),Wstr,200);
  SelectAllButton1 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                    RectListWindow1.left+ButtonStepX*1,RectListWindow1.bottom+ButtonStepY*2+14,
                                    ButtonSizeX,ButtonSizeY,
                                    LIBMANWindow,(HMENU)ID_SELECT_ALL1,hInstance,NULL);
  ShowWindow(SelectAllButton1,SW_SHOW);
  if (LibraryMode==0) {
    Utf8ToUnicode(SC(38,"Delete symbols"),Wstr,200);
    DeleteSymbolsButton1 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                        RectListWindow1.left+ButtonStepX*0,RectListWindow1.bottom+ButtonStepY*1+14,
                                        ButtonSizeX,ButtonSizeY,
                                        LIBMANWindow,(HMENU)ID_DELETE_SYMBOLS1,hInstance,NULL);
  } else {
...
//  Utf8ToUnicode(SC(39,"Copy ->"),Wstr,200);
  CopyButton1 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                             RectListWindow2.left-ButtonSizeX-15,120,
                             ButtonSizeX,ButtonSizeY,
                             LIBMANWindow,(HMENU)ID_COPY_SYMBOLS1,hInstance,NULL);
  ShowWindow(CopyButton1,SW_SHOW);
  Utf8ToUnicode(SC(40,"Move ►"),Wstr,200);
//  Utf8ToUnicode(SC(40,"Move ->"),Wstr,200);
  MoveButton1 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                             RectListWindow2.left-ButtonSizeX-15,160,
                             ButtonSizeX,ButtonSizeY,
  ...
  Utf8ToUnicode(SC(32,"New lib"),Wstr,200);
  CreateNewLibButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                     RectListWindow2.left+ButtonStepX*0,RectListWindow2.bottom+14,
                                     ButtonSizeX,ButtonSizeY,
                                     LIBMANWindow,(HMENU)ID_CREATE_LIB2,hInstance,NULL);
  ShowWindow(CreateNewLibButton2,SW_SHOW);
  Utf8ToUnicode(SC(33,"Open lib"),Wstr,200);
  OpenLibButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                RectListWindow2.left+ButtonStepX,RectListWindow2.bottom+14,
                                ButtonSizeX,ButtonSizeY,
                                LIBMANWindow,(HMENU)ID_OPEN_LIB2,hInstance,NULL);
...
*/
  Utf8ToUnicode(SC(34,"Close lib"),Wstr,200);
  CloseLibButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                 RectListWindow2.left+ButtonStepX*2,RectListWindow2.bottom+14,
                                 ButtonSizeX,ButtonSizeY,
                                 LIBMANWindow,(HMENU)ID_CLOSE_LIB2,hInstance,NULL);
  ShowWindow(CloseLibButton2,SW_SHOW);
  if (LibraryMode==0) {
    Utf8ToUnicode(SC(35,"Import symbols"),Wstr,200);
    ImportSymbolsButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                        RectListWindow2.left+ButtonStepX*1,RectListWindow2.bottom+ButtonStepY+14,
                                        ButtonSizeX,ButtonSizeY,
                                        LIBMANWindow,(HMENU)ID_IMPORT_SYMBOLS2,hInstance,NULL);
  } else {
    Utf8ToUnicode(SC(52,"Import geometries"),Wstr,200);
    ImportSymbolsButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                        RectListWindow2.left+ButtonStepX*1,RectListWindow2.bottom+ButtonStepY+14,
  ...
  if (LibraryMode==0) {
    Utf8ToUnicode(SC(36,"Export symbols"),Wstr,200);
    ExportSymbolsButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                        RectListWindow2.left+ButtonStepX*2,RectListWindow2.bottom+ButtonStepY+14,
                                        ButtonSizeX,ButtonSizeY,
                                        LIBMANWindow,(HMENU)ID_EXPORT_SYMBOLS2,hInstance,NULL);
  } else {
    Utf8ToUnicode(SC(53,"Export geometries"),Wstr,200);
    ExportSymbolsButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                        RectListWindow2.left+ButtonStepX*2,RectListWindow2.bottom+ButtonStepY+14,
  ...
  Utf8ToUnicode(SC(37,"Deselect all"),Wstr,200);
  DeselectAllButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                    RectListWindow2.left+ButtonStepX*0,RectListWindow2.bottom+ButtonStepY*2+14,
                                    ButtonSizeX,ButtonSizeY,
                                    LIBMANWindow,(HMENU)ID_DESELECT_ALL2,hInstance,NULL);
  ShowWindow(DeselectAllButton2,SW_SHOW);
  Utf8ToUnicode(SC(58,"Select all"),Wstr,200);
  SelectAllButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                    RectListWindow2.left+ButtonStepX*1,RectListWindow2.bottom+ButtonStepY*2+14,
                                    ButtonSizeX,ButtonSizeY,
                                    LIBMANWindow,(HMENU)ID_SELECT_ALL2,hInstance,NULL);
  ShowWindow(SelectAllButton2,SW_SHOW);
  if (LibraryMode==0) {
    Utf8ToUnicode(SC(38,"Delete symbols"),Wstr,200);
    DeleteSymbolsButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                        RectListWindow2.left+ButtonStepX*0,RectListWindow2.bottom+ButtonStepY*1+14,
                                        ButtonSizeX,ButtonSizeY,
                                        LIBMANWindow,(HMENU)ID_DELETE_SYMBOLS2,hInstance,NULL);
  } else {
    Utf8ToUnicode(SC(57,"Delete geometries"),Wstr,200);
    DeleteSymbolsButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                                        RectListWindow2.left+ButtonStepX*0,RectListWindow2.bottom+ButtonStepY*1+14,
...
//  Utf8ToUnicode(SC(41,"<- Copy"),Wstr,200);
  CopyButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                             RectListWindow2.left-ButtonSizeX-15,220,ButtonSizeX,ButtonSizeY,
                             LIBMANWindow,(HMENU)ID_COPY_SYMBOLS2,hInstance,NULL);
  ShowWindow(CopyButton2,SW_SHOW);
  Utf8ToUnicode(SC(42,"◄ Move"),Wstr,200);
//  Utf8ToUnicode(SC(42,"<- Move"),Wstr,200);
  MoveButton2 = CreateWindowW(L"BUTTON",Wstr,WS_CHILD,
                             RectListWindow2.left-ButtonSizeX-15,260,ButtonSizeX,ButtonSizeY,
...
// *****************************************************************************************
  ...
  AppendMenuUTF8(LIBMANMenu ,MF_ENABLED|MF_POPUP,(UINT)LIBMANMenu1,SC(43,"File"));
  AppendMenuUTF8(LIBMANMenu1,MENU_ID,ID_FILE_EXIT                 ,SC(44,"Exit"));
//  AppendMenu(LIBMANMenu ,MENU_ID,ID_HELP                      ,"Help");
  AppendMenuUTF8(LIBMANMenu ,MENU_ID,ID_ABOUT                     ,SC(45,"About"));
...
{
  int32  cnt,cnt2,length,res,NrFiles;
  char   str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
         Dir[MAX_LENGTH_STRING],FileName[MAX_LENGTH_STRING];
  WCHAR  Wstr[MAX_LENGTH_STRING];
  ...
  switch (WParam) {
    ...
    case ID_EXPORT_SYMBOLS1:
      ...
      if (res>0) {
        if (LibraryMode==0) {
          sprintf(str2,SC(46,"%d symbols added to directory %s"),res,ExportDir);
        } else {
          sprintf(str2,SC(59,"%d geometries added to directory %s"),res,ExportDir);
        }
        MessageBoxUTF8(LIBMANWindow,str2,SC(60,"Message"),MB_OK);
      }
      DeselectList1();
...
#endif
    case ID_ABOUT:
      res=DialogBox(LIBMANClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_ABOUT), LIBMANWindow, (DLGPROC)AboutDialogBody);
      break;
  }
}
...

{
  ...
  if ((ID<0)
     ||
     (ID>=MaxNrLibmanNames)) return -1;
  if (LibmanNamesPos+Length+2>=LibmanNamesBufSize) return -1;
...

{
  ...
  char   LineBuf[512],str2[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],
         LanguageFileName[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],
         str4[MAX_LENGTH_STRING],str6[MAX_LENGTH_STRING];

  count=0;
  if (FileName[0]==0) return -1;
  if ((fp=TextFileOpenUTF8(FileName))<0) {
/*
    sprintf(str,SC(47,"Can not read from language file %s, default language used"),FileName);
    MessageBox(NULL,str,SC(48,"Warning"),MB_APPLMODAL|MB_OK);
*/
    return -1;
  }
...
// ********************************************************************************************************
  ...
  if ((LanguageFileName[0]==0)
     ||
     ((fp=TextFileOpenUTF8(LanguageFileName))<0)) {
/*
    sprintf(str,SC(49,"Can not read from language file %s"),LanguageFileName);
    MessageBox(NULL,str,SC(2,"System error"),MB_APPLMODAL|MB_OK);
    return -2;
*/
    return -1;
  }
                    ...
                    LPSTR lpszCmd, int nCmdShow)
{
  ...
  if (!FlagSSE2) {
    MessageBox(NULL,"This version of the library manager requires a CPU with SSE2 extensions",
  ...
  if (TempPath[length-1]=='\\') TempPath[length-1]=0;
...
//    LIBMANClass.hbrBackground = COLOR_APPWORKSPACE+1;
    ...
    if ( ! RegisterClass(&LIBMANClass) ) exit(255);
  }
...
//  ToetsMain();
  ...
  if (AddLibmanLanguageStrings(str)==-2) return -1;
  if (ExePath[0]==0) {
...
//  Utf8ToUnicode("Mibrary manager geometries_ä_Диап_поломкиend_śč",Wstr,200);
...
#if 1
  NewFont=CreateFont(16,0,0,0,0,0,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,
                     CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,"Arial");
#else
  NewFont=CreateFont(-1,0,0,0,0,0,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,
                     CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,"MS Sans Serif");
#endif
  LIBMANWindow = CreateWindowExW(0,L"LIBMAN",
                                 L"",
                                 WS_OVERLAPPEDWINDOW,
                                 WindowStartX,
                                 WindowStartY,
  ...
  if (LibraryMode==0) {
    res=SetWindowTextUTF8(LIBMANWindow,SC(50,"Library manager symbols"));
  } else {
    res=SetWindowTextUTF8(LIBMANWindow,SC(51,"Library manager geometries"));
  }
