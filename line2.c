...
extern int32                Printing,ReverseY,OperatingSystem,
...

{
  ...
  a2_2=a2*a2;
  pos2=pos;
  if (pos2<0) pos2+=NrEllipssPoints;
  if (pos2>=NrEllipssPoints) pos2-=NrEllipssPoints;
...

{
  ...
  NrPoints=256;
  if (a<256) NrPoints=128;
  if (a<128) NrPoints=64;
  if (a<64) NrPoints=32;
  if (a<32) NrPoints=16;
...

{
  ...
  if ((maxx2<DrawWindowMinX-1000)
     ||
     (minx2>=DrawWindowMaxX+1000)
     ||
     (maxy2<DrawWindowMinY-1000)
     ||
     (miny2>=DrawWindowMaxY+1000)) return;
...

{
  ...
  Height=(Height-1) >> 1;
  if (ReverseY) y1=DrawWindowMaxY-y1-1;
  if (x1>x2) {
    hulp=x2;
    x2=x1;
    x1=hulp;
  }
  if ((x1-Height>=DrawWindowMaxX)
     ||
     (x2+Height<DrawWindowMinX)
     ||
     (y1-Height>=DrawWindowMaxY)
     ||
     (y1+Height<DrawWindowMinY)) return;

  if (x1+Height<DrawWindowMinX-1) x1=DrawWindowMinX-Height-1;
  if (x2-Height>=DrawWindowMaxX+1) x2=DrawWindowMaxX+Height+1;
  switch (Height) {
    case 0:
      MoveToEx(OutputDisplay,x1,y1,NULL);
    ...
    case 3:
      ...
      if (mode==0) Rectangle(OutputDisplay,x1,y1-3,x2+1,y1+4);
      else RectangleXor(OutputDisplay,x1,y1-3,x2+1,y1+4);
    ...
    case 4:
      ...
      if (mode==0) Rectangle(OutputDisplay,x1-2,y1-4,x2+3,y1+5);
      else RectangleXor(OutputDisplay,x1-2,y1-4,x2+3,y1+5);
    ...
    default:
      if (Height<10000) {
        if ((mode & 1)==0) {
          if (x2>x1+1)
            Rectangle(OutputDisplay,(x1),(y1-Height),
                                 (x2+1),(y1+Height+1));
          Ellipse(OutputDisplay,(x1-Height),(y1-Height),
                             (x1+Height+1),(y1+Height+1));
          Ellipse(OutputDisplay,(x2-Height),(y1-Height),
                             (x2+Height+1),(y1+Height+1));
        } else {
...

{
  ...
  if ((x1-Height>=DrawWindowMaxX)
     ||
     (x1+Height<DrawWindowMinX)
     ||
     (y1-Height>=DrawWindowMaxY)
     ||
     (y2+Height<DrawWindowMinY)) return;

  if (y1+Height<DrawWindowMinY-1) y1=DrawWindowMinY-Height-1;
  if (y2-Height>=DrawWindowMaxY+1) y2=DrawWindowMaxY+Height+1;
  switch (Height) {
    case 0:
      MoveToEx(OutputDisplay,x1,y1,NULL);
    ...
    case 3:
      ...
      if (mode==0) Rectangle(OutputDisplay,x1-3,y1,x1+4,y2+1);
      else RectangleXor(OutputDisplay,x1-3,y1,x1+4,y2+1);
    ...
    case 4:
      ...
      MoveToEx(OutputDisplay,x1-3,y1-3,NULL);
      LineTo(OutputDisplay,x1+4,y1-3);
      if (mode==0) Rectangle(OutputDisplay,x1-4,y1-2,x1+5,y2+3);
      else RectangleXor(OutputDisplay,x1-4,y1-2,x1+5,y2+3);
...
{
  int32  hulp,Height2,Height2a,y2,x1a,y1a,x2a,y2a,x3a,y3a,x4a,y4a,x10,x20,
         x1b,y1b,x2b,y2b,x3b,y3b,x4b,y4b,OldHeight,ok;
  double h2;
  POINT  LinePoints[10];
  HGDIOBJ SavePen=NULL;
...
{
  int32  hulp,Height2,Height2a,y2,x1a,y1a,x2a,y2a,x3a,y3a,x4a,y4a,x20,x10,
         x1b,y1b,x2b,y2b,x3b,y3b,x4b,y4b,OldHeight,ok;
  double h2;
  POINT  LinePoints[10];
  HGDIOBJ SavePen=NULL;
