...
int32  MaxCollections,px,py,ro,OldMousePosX,OldMousePosY,
       OldX2,OldY2,NetNrCheck,MousePosXR,MousePosYR,
...
//        NrGraphicsObjects
    ...
    if (!Focused) {
      ...
      CheckInputMessages(0);
      ClipMouseCursor();
      if (!LeftButtonPressed) SelectionEsc=1;
      if (!SelectionEsc) {
        DrawXorWindow(MultX(CurrentX2),MultY(CurrentY2),MultX(OldX),MultY(OldY),0);
      }
    }
