...
int32  MaxCollections,px,py,ro,OldMousePosX,OldMousePosY,MousePosXR,MousePosYR,
       direction,OldX2,OldY2,NetNrCheck,DrawingObjects,DrawingCommand,
       FirstRightButtonDepressed,FirstRightButtonPressed,CrossHairType,
...
//      y3=(y-RelY)/254000.0;
      if (GridSize>=(double)0.0999) {
        sprintf(str,"%.1f,%.1f",x2,y2);
        sprintf(str2,"%.1f,%.1f",x3,y3);
      } else {
        sprintf(str,"%.3f,%.3f",x2,y2);
        sprintf(str2,"%.3f,%.3f",x3,y3);
      }
//      sprintf(str2,"%.3f,%.3f",x4,y4);
//      sprintf(str2,"%i,%i",MousePosX,MousePosY);
//      sprintf(str2,"%i,%i",MousePosX,DrawWindowMaxY-MousePosY);
//      sprintf(str2,"%i,%i",MousePosX,DrawWindowMaxY-MousePosY);
      break;
    case 2:
      x2=x/100000.0;
  ...
  RedrawRelPosStr(1);
//  TextOut(WindowsDisplay,100,UserWindowCoor.bottom,&str,strlen(str));
//  SetWindowText(SCHWindow,str);
//  TextOut(WindowsDisplay,100,100,&str,strlen(str));
}
...
//    Yoffset=cy-PixelToReal(DrawWindowMaxY-MouseY);
    ...
    UpdateWindow(SCHWindow);
    if ((mode & 4)==0) {
      SetCursorPos(DrawWindowMinX+ClientStartX+ClientWindowDivX/2,ClientStartY+(ClientWindowDivY/2)-2);
    }
  }
  if ((mode & 1)==0) {
    ZoomInOutProcessed=1;
...
//           ((Instance->RefInfo & TEXT_NOT_VISIBLE) == 0)
  ...
  if (((Instance->BoardPosMaxX-Instance->BoardPosMinX)/PixelsX)>((Instance->BoardPosMaxY-Instance->BoardPosMinY)/PixelsY)) {
