...
extern  HBITMAP                     BackgroundBitmap,
...

{
  ...
  res=BitBlt(OutputDisplay,ButtonBitmapStartX,ButtonBitmapStartY,
             ButtonBitmapSizeX,ButtonBitmapSizeY,ButtonMemoryDC,0,0,SRCAND);
  return old;
}
...

{
  ...
  res=BitBlt(OutputDisplay,x1,y1,
             ButtonBitmapSizeX,ButtonBitmapSizeY,ButtonMemoryDC,0,0,SRCCOPY);
  strcpy(str,SC(128,"Back"));
...

{
  ...
  res=BitBlt(OutputDisplay,x1,y1,
             ButtonBitmapSizeX,ButtonBitmapSizeY,ButtonMemoryDC,0,0,SRCCOPY);
//  strcpy(str,SC(128,"Back"));
//  TextOutUTF8(OutputDisplay,x1+23,y1+4,str,strlen(str));
  strcpy(str,SC(56,"Annotate"));
  GetTextExtentPoint32(OutputDisplay,str,strlen(str),&TextSize);
  TextOutUTF8(OutputDisplay,x1+(ButtonSizeX-TextSize.cx)/2,y1+20,str,strlen(str));
  SelectObject(OutputDisplay,old2);
...

{
  ...
  if (!ButtonBackGroundColor) {
    res=RegOpenKeyEx(HKEY_CURRENT_USER,"Control Panel\\Colors",
...
//            DrawWindowMaxX-1,DrawWindowMaxY-1);
  Rectangle(OutputDisplay,1,1,
            DrawWindowMaxX-1,MessageStartY-24);
...
//            DrawWindowMaxX,DrawWindowMaxY);
  Rectangle(OutputDisplay,0,0,
            DrawWindowMaxX,MessageStartY-23);
  SelectObject(OutputDisplay,TempPen2);
  SelectObject(OutputDisplay,TempBrush2);
  Rectangle(OutputDisplay,2,MessageStartY-21,
            DrawWindowMaxX-2,MessageStartY-1);
...
*/
  ...
  for (cnty=0;cnty<3;cnty++) {
    ...
    for (cntx=0;cntx<5;cntx++) {
      ...
      if ((MousePosX>CurrentButtonRect.left)
         &&
         (MousePosX<CurrentButtonRect.right)
         &&
         (MousePosY>CurrentButtonRect.top)
         &&
         (MousePosY<CurrentButtonRect.bottom)) {
        nr=cnty*5+cntx;
        switch (nr) {
          case 2:
          case 5:
        ...
        if (!DisplayButtonNr) {
          ...
          switch (val) {
            case 2:
              ...
              break;
            case 5:
              strcpy(str,SC(140,"Leftbutton   -> Start symbol editor"));
              break;
            case 6:
              ...
              break;
            case 7:
              strcpy(str,SC(143,"Start annotation sheets"));
              break;
            case 8:
              strcpy(str,SC(144,"Start building netlist/components from sheets"));
              break;
            case 9:
              strcpy(str,SC(145,"Start layout editor with current design"));
              break;
            case 10:
              strcpy(str,SC(109,"Bill Of Materials"));
              break;
            case 11:
          ...
          if (str2[0]!=0) GetTextExtentPoint32(OutputDisplay,str2,strlen(str2),&NewTextSize2);
          sx=270;
...

{
  ...
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],FileName[MAX_LENGTH_STRING],
        FileName2[MAX_LENGTH_STRING],FileName3[MAX_LENGTH_STRING],Line[MAX_LENGTH_STRING];

  if ((!SetCurrentDirectoryUTF8(DesignPath))
     &&
     (!CreateDirectoryUTF8(DesignPath))) {
  ...
  if ((!SetCurrentDirectoryUTF8(FileName))
     &&
     (!CreateDirectoryUTF8(FileName))) {
...
//  SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
  sprintf(str2,SC(150,"The initial settings of geom.ini,sch.ini,pcb.ini and viewplot.ini will be copied (if exists)"));
