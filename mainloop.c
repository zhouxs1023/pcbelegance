...
int32  MaxCollections,px,py,ro,OldMousePosX,OldMousePosY,CrossHairMode,CrossHairType,
       direction,ok,FirstRightButtonPressed,FirstRightButtonDepressed,OldX2,OldY2,NetNrCheck,ZoomActivated,
       PanActivated,MousePosXR,MousePosYR,RightButtonDivTime;
...
//  CurrentY=AdjustToDrawGrid(PixelToRealOffY(DrawWindowMaxY-MousePosY));
  ...
  ClipMouseCursor();
  SetNormalCursor();
  while ((((mode==1) && (LeftButtonPressed))
        ||
        ((mode==0) && (!LeftButtonPressed)))
        &&
        (!SelectionEsc)) {
    CurrentX=PixelToRealOffX(MousePosX);
    CurrentY=PixelToRealOffY(DrawWindowMaxY-MousePosY);
...
//    CurrentY=AdjustToDrawGrid(PixelToRealOffY(DrawWindowMaxY-MousePosY));
    if ((OldX!=CurrentX)
       ...
       (OldY!=CurrentY)) {
      DrawCrossHair(0);
      if (mode==1) DrawXorWindow(MultX(CurrentX2),MultY(CurrentY2),MultX(OldX),MultY(OldY),0);
      OldX=CurrentX;
      OldY=CurrentY;
      if (Units==0) {
        x1=fabs((CurrentX-CurrentX2)/2540.0);
...

{
  ...
  switch (SelectionMode) {
        ...
        case -1:
          switch (SelectionMode) {
            case ROUTING_MODE:
              ...
              break;
            case MOVE_ONE_TRACE_MODE:
              sprintf(InfoStr,SC(331,"Drag one trace"));
              break;
            case MOVING_TRACES_VIAS_MODE:
              sprintf(InfoStr,SC(332,"Change traces/vias"));
              break;
          }
          break;
        default:
          ...
          switch (SelectionMode) {
            case ROUTING_MODE:
              ...
              break;
            case MOVE_ONE_TRACE_MODE:
              sprintf(InfoStr,SC(529,"Drag one trace %s"),str);
              break;
            case MOVING_TRACES_VIAS_MODE:
              sprintf(InfoStr,SC(530,"Change traces/vias %s"),str);
              break;
          }
          break;
      }
    ...
    case MOVE_COMPONENT_REFERENCES_MODE:
      strcpy(InfoStr,SC(336,"Modify component references"));
      break;
    case MOVE_COMPONENT_VALUES_MODE:
      strcpy(InfoStr,SC(337,"Modify component values"));
      break;
    case OBJECTS_MODE:
      strcpy(InfoStr,SC(335,"Draw/change objects other layers"));
      break;
    case AREAFILLS_MODE:
      strcpy(InfoStr,SC(531,"Add/change areafills/powerplanes"));
      break;
    case GATE_PINSWAP_MODE:
      strcpy(InfoStr,SC(339,"Gate/pin swap"));
      break;
        ...
        if ((MousePosX>DrawWindowMinX)
          ...
          if (!CrossHairNewStart) {
            ...
            if (CrossHairType==0) {
              ...
              DrawLine(MultX(CrossHairX)+100000,MultY(CrossHairY)+100000,
                       MultX(CrossHairX)-100000,MultY(CrossHairY)-100000);
              DrawLine(MultX(CrossHairX)-100000,MultY(CrossHairY)+100000,
                       MultX(CrossHairX)+100000,MultY(CrossHairY)-100000);
            }
          }
  ...
  if (RightButtonPressed) {
    ...
    if (FirstRightButtonPressed==0) {
      ...
      if (MousePosX!=10000) {
        ...
        if ((abs(DivX)>MaxDisplayDiv)
           ||
           (abs(DivY)>MaxDisplayDiv)) {
...
*/
  if ((MousePosX==10000)
     ...
     (MousePosY==10000)
     ||
     (MousePosX>=DrawWindowMaxX)
     ||
     (MousePosY>=DrawWindowMaxY)) return;
...
#ifdef _DEBUG
      sprintf(str,"(x,y) %.4f , %.4f mm [ %d,%d ]",x4,y4,MousePosX,MousePosY);
#else
...
//      sprintf(str3,"(rel x,y) %.2f,%.2f mils",x3,y3);
    ...
    case 1:
      ...
      sprintf(str,SC(533,"(x,y) %.4f , %.4f mm"),x4,y4);
#endif
      sprintf(str2,SC(536,"(grid x,y) %.4f , %.4f mm"),x2,y2);
//      sprintf(str3,"(rel x,y) %.4f,%.4f mm",x3,y3);
      sprintf(str3,SC(537,"(rel x,y) %.4f , %.4f [%.4f %.1f] mm"),x3,y3,Length,Angle);
      break;
  }
...

{
  ...
  DrawCrossHair(7);
  if (MousePosY>=DrawWindowMaxY) mode&=~1;
  if (((mode & 1)==1)
     &&
     (MousePosX!=10000)
     &&
...

{
  ...
  DrawCrossHair(2);
  if (MousePosY>=DrawWindowMaxY) mode=0;
  if ((mode==1)
     &&
     (MousePosX!=10000)
     &&
