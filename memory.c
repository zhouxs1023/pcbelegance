...
int32                       ClientWindowDivX,ClientWindowDivY,
                            CurrentDrawMode,RepeatMode,
                            WireSelectMode,ViewMode,
                            MousePanMultiply,
                            PopupDisplayVisible,ButtonInfoTimeout,
                            ButtonInfoTimeoutStart,ZoomInOutProcessed,
                            SaveSymbolsLocally,ZoomMode,
...
int32                       WindowWidth,WindowHeight,MousePosX,MousePosY,
                            ...
                            ScrollEndOfWindow,MaxLastActionNr,
                            MaxScrollBarX,MaxScrollBarY,
                            MouseActionMode,EditingProtectedSymbol,
...
//                            MaxNrSubSheets,
                            MaxNrPins,
                            MaxNrPinBusses,
                            MaxNrRedefinedPinBusses,
                            MaxNrPowerPins,
                            MaxNrExtraPins,
                            MaxNrObjects,
                            MaxNrObjects2,
                            MaxNrObjects5,
                            MaxNrStandardObjects,
                            MessageBufMemSize,
                            MessageBufPos,
                            ClipBoardMemSize,
                            ClipBoardMemPos,
...
int32                       NetNrWires,NetNrBusses,NetNrJunctions,NetNrBusConnections,
                            NetNrGlobalConnections,NetNrInstances,NetNrNetLabels;
...
int32                       Created,Painting,DCInUse,MouseChanged,Focused,
                            ...
                            DataBaseChanged,ReplaceSelections,
                            GridVisible,BackGroundActive,
                            MemoryAllocated,HelpAsked,
                            SearchReplaceOptions,
                            EditingSheetSymbol,DisableOnePinNetCheck,
                            Printing,EditingSymbol,FileChanged,
                            UndoRedoActive,SelectionEsc,
                            FirstPaint,SearchReference,SearchPartnr,
                            AppendPropertiesToNetlabel,
                            AppendPropertiesToReferences,
                            OkToAddViewPos;

double                      Xoffset,Yoffset,Factor,RelX,RelY,
                            CentreSelectedX,CentreSelectedY,CurrentX2,CurrentY2,
                            GridSize,DrawGridSize;

int32                       FirstSize,NrObjects,NrObjects2,NrObjects3,NrObjects4,
                            NrStandardObjects,
                            NrObjects5,SystemBusyMode,
                            MouseOnGridPosX,MouseOnGridPosY,
                            CurrentObjectCode,Units,LastAction,
                            ViewPosPointer,NrWireLabels,
                            NrGeometryLibraries,
                            NrLibFiles,
                            NrSchematicSymbolLibraries,
                            OperatingSystem,
                            SCHSystemError,
                            CrossHairVisible,
                            ButtonInfoTimeout,
...
uint8                       *SymbolsMem;
HGLOBAL                     DesignsGlobal,
                            WiresGlobal,
                            BussesGlobal,
                            BusConnectionsGlobal,
                            JunctionsGlobal,
                            OnePinNetsGlobal,
                            InstancesGlobal,
                            NetLabelsGlobal,
                            ...
                            PowerPinsGlobal,
                            PinBussesGlobal,
                            RedefinedPinBussesGlobal,
                            GlobalConnectionsGlobal,
                            GlobalTempMem,
                            GlobalTempMem2,
...
//        &&
//        (Str[cnt2]==' ')) cnt2++;
  if (cnt>0) memmove(Result,Str,cnt);
  if (cnt>=l-1) {
    Str[0]=0;
    return;
  }
  if (l-cnt>0) memmove(Str,&Str[cnt+1],l-cnt);
  l=strlen(Result);
  cnt=l-1;
//  while ((cnt>=0)
//        &&
...

{
  ...
  for (cnt=0;cnt<lengte;cnt++) {
    ...
    switch (mode) {
      ...
      case 2:
        if (cnt==0) {
          if (isdigit(str[cnt])) return -1;
          if (str[cnt]=='_') return -1;
        }
        if ((!isalnum(str[cnt]))
           &&
           (str[cnt]!='_')) return -1;
      ...
      case 3:
        ...
        break;
      case 4:
        if (!isalpha(str[cnt])) return -1;
        break;
      case 5:
        if (!isalnum(str[cnt]))  return -1;
        if (isalpha(str[cnt])) Alphas=1;
        if (isdigit(str[cnt])) Digits=1;
        break;
      case 6:
        if ((ch=='\'')
           ||
...

{
  ...
  if ((length=strlen(str))==0) return -1;
  if (CheckString(str,2)==-1) return -1;
...

{
  ...
  while ((cnt<lengte)
    ...
    if ((PinBusName[cnt]==',')
      ...
      if ((cnt<lengte-1)
         &&
         (PinBusName[cnt+1]=='\\')) cnt++;
      cnt2++;
    } else {
      if (FoundKomma) {
        pos1=cnt;
        FoundKomma=0;
        if (PreviousChar=='\\') LineNr++;
      }
    }
...

{
  ...
  while (str2[0]!=0) {
    ...
    if (CheckPinName(str3,0)==-1) return -1;
  }
  return 0;
}
...

{
  ...
  if (WiresGlobal                !=0) MemSize+=GlobalSize(WiresGlobal                );
  if (BussesGlobal               !=0) MemSize+=GlobalSize(BussesGlobal               );
  if (BusConnectionsGlobal       !=0) MemSize+=GlobalSize(BusConnectionsGlobal       );
  if (JunctionsGlobal            !=0) MemSize+=GlobalSize(JunctionsGlobal            );
  if (InstancesGlobal            !=0) MemSize+=GlobalSize(InstancesGlobal            );
  if (NetLabelsGlobal            !=0) MemSize+=GlobalSize(NetLabelsGlobal            );
  if (ObjectLinesGlobal          !=0) MemSize+=GlobalSize(ObjectLinesGlobal          );
  if (ObjectRectsGlobal          !=0) MemSize+=GlobalSize(ObjectRectsGlobal          );
  if (ObjectCirclesGlobal        !=0) MemSize+=GlobalSize(ObjectCirclesGlobal        );
  if (ObjectArcsGlobal           !=0) MemSize+=GlobalSize(ObjectArcsGlobal           );
  if (ObjectTextsGlobal          !=0) MemSize+=GlobalSize(ObjectTextsGlobal          );
  if (SymbolsMemGlobal           !=0) MemSize+=GlobalSize(SymbolsMemGlobal           );
  if (SymbolsPosGlobal           !=0) MemSize+=GlobalSize(SymbolsPosGlobal           );
  if (MessageBufGlobal           !=0) MemSize+=GlobalSize(MessageBufGlobal           );
  if (SubPinDefsGlobal           !=0) MemSize+=GlobalSize(SubPinDefsGlobal           );
  if (SymbolsGlobal              !=0) MemSize+=GlobalSize(SymbolsGlobal              );
  if (PinsGlobal                 !=0) MemSize+=GlobalSize(PinsGlobal                 );
  if (PowerPinsGlobal            !=0) MemSize+=GlobalSize(PowerPinsGlobal            );
  if (PinBussesGlobal            !=0) MemSize+=GlobalSize(PinBussesGlobal            );
  if (GlobalConnectionsGlobal    !=0) MemSize+=GlobalSize(GlobalConnectionsGlobal    );
  if (ObjectsGlobal              !=0) MemSize+=GlobalSize(ObjectsGlobal              );
  if (Objects2Global             !=0) MemSize+=GlobalSize(Objects2Global             );
  if (Objects5Global             !=0) MemSize+=GlobalSize(Objects5Global             );
  if (CharsGlobal                !=0) MemSize+=GlobalSize(CharsGlobal                );
  return MemSize;
}
...

{
  ...
  if (MaxNrStandardObjects>=Count) return 1;
...

{
  ...
  if (MaxNrObjects>=Count) return 1;
...

{
  ...
  if (MaxNrObjects2>=Count) return 1;
  if (Count>LimitMaxNrObjects) {
    return -2;
  }
  if (MaxNrObjects2==0) {
...

{
  ...
  if ((!error)
     &&
     ((CharsGlobal=GlobalAlloc(GHND,(int32)sizeof(CharsArray)))==NULL)) error=1;
  count++;
  if (AllocateMemObjects(500,0)==-1) error=1;

  if ((!error)
     &&
     ((Chars=(CharsArray *)GlobalLock(CharsGlobal))==NULL)) error=1;
  count2++;
...

{
  ...
  if (MaxNrWires>NrWires) return 1;
...

{
  ...
  if (MaxNrBusses>NrBusses) return 1;
...

{
  ...
  if (MaxNrJunctions>NrJunctions) return 1;
...

{
  ...
  if (MaxNrOnePinNets>NrOnePinNets) return 1;
...

{
  ...
  if (MaxNrNetLabels>NrNetLabels) return 1;
...

{
  ...
  if (MaxNrBusConnections>NrBusConnections) return 1;
  ...
  if (MaxNrBusConnections==0) {
    ...
    if ((NewMem=GlobalReAlloc(BusConnectionsGlobal,NrBusConnections*sizeof(BusConnectionRecord),GHND))==NULL) {
      return -1;
    }
...

{
  ...
  if (MaxNrGlobalConnections>NrGlobalConnections) return 1;
  ...
  if (MaxNrGlobalConnections==0) {
    ...
    if ((NewMem=GlobalReAlloc(GlobalConnectionsGlobal,NrGlobalConnections*sizeof(GlobalConnectionRecord),GHND))==NULL) {
      return -1;
    }
...

{
  ...
  if (MaxNrObjectLines>NrObjectLines) return 1;
...

{
  ...
  if (MaxNrObjectRects>NrObjectRects) return 1;
...

{
  ...
  if (MaxNrObjectCircles>NrObjectCircles) return 1;
...

{
  ...
  if (MaxNrObjectArcs>NrObjectArcs) return 1;
...

{
  ...
  if (MaxNrObjectTexts>NrObjectTexts) return 1;
...

{
  ...
  if (MaxNrPowerPins>NrPowerPins) return 1;
...

{
  ...
  if (MaxNrPinBusses>NrPinBusses) return 1;
...

{
  ...
  if (MaxNrRedefinedPinBusses>NrRedefinedPinBusses) return 1;
  if (NrRedefinedPinBusses>LimitMaxNrPinBusses) {
    return -2;
  }
  NrRedefinedPinBusses=max(NrRedefinedPinBusses,10);
  if (MaxNrRedefinedPinBusses==0) {
    if ((RedefinedPinBussesGlobal=GlobalAlloc(GHND,NrRedefinedPinBusses*sizeof(RedefinedPinBusRecord)))==NULL) {
      return -1;
    }
    ...
    if ((NewMem=GlobalReAlloc(RedefinedPinBussesGlobal,NrRedefinedPinBusses*sizeof(PinBusRecord),GHND))==NULL) {
      return -1;
    }
...

{
  ...
  if (MaxNrInstances>NrInstances) return 1;
...

{
  ...
  if (MaxNrSubPinDefs>NrSubPinDefs) return 1;
...

{
  ...
  if (MaxNrSheetSymbols>NrSheetSymbols) return 1;
...

{
  ...
  if (MaxSymbolsMemSize>MemSheetSymbols) return 1;
...

{
  ...
  if ((mode & 2)==2) {
    if (AllocateMemWires(DefMaxNrWires,0)!=0) return 0;
    if (AllocateMemBusses(DefMaxNrBusses,0)!=0) return 0;
    if (AllocateMemJunctions(DefMaxNrJunctions,0)!=0) return 0;
    if (AllocateMemOnePinNets(DefMaxNrOnePinNets,0)!=0) return 0;
    if (AllocateMemNetLabels(DefMaxNrNetLabels,0)!=0) return 0;
    if (AllocateMemBusConnections(DefMaxNrBusConnections,0)!=0) return 0;
    if (AllocateMemGlobalConnections(DefMaxNrGlobalConnections,0)!=0) return 0;
    if (AllocateMemObjectLines(DefMaxNrObjectLines,0)!=0) return 0;
    if (AllocateMemObjectRects(DefMaxNrObjectRects,0)!=0) return 0;
    if (AllocateMemObjectCircles(DefMaxNrObjectCircles,0)!=0) return 0;
    if (AllocateMemObjectArcs(DefMaxNrObjectArcs,0)!=0) return 0;
    if (AllocateMemObjectTexts(DefMaxNrObjectTexts,0)!=0) return 0;
    if (AllocateMemInstances(DefMaxNrInstances,0)!=0) return 0;
    if (AllocateMemSheetSymbolsPos(DefMaxNrSheetSymbols,0)!=0) return 0;
    if (AllocateMemSheetSymbols(DefMaxSymbolsMemory,0)!=0) return 0;
  }
  return 1;
}
...

{
  ...
  if ((mode & 2)==2) {
    if (AllocateMemPins(DefMaxNrPins,0)!=0) return 0;
    if (AllocateMemPowerPins(DefMaxNrPowerPins,0)!=0) return 0;
    if (AllocateMemPinBusses(DefMaxNrPinBusses,0)!=0) return 0;
    if (AllocateMemSubPinDefs(DefMaxNrSubPinDefs,0)!=0) return 0;
    if (AllocateMemObjectLines(DefMaxNrObjectLines,0)!=0) return 0;
    if (AllocateMemObjectRects(DefMaxNrObjectRects,0)!=0) return 0;
    if (AllocateMemObjectCircles(DefMaxNrObjectCircles,0)!=0) return 0;
    if (AllocateMemObjectArcs(DefMaxNrObjectArcs,0)!=0) return 0;
  ...
  if (AllocateMemInstances(1,0)!=0) return 0;
