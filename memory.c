...
int32                       ClientWindowDivX,ClientWindowDivY,NrParams,
                            RepeatMode,RepeatModeActive=1,LastAction,CurrentDirection,
                            PreviousDirection,PopupDisplayVisible=1,
                            StartWithMaximumView,SaveSymbolsLocally,
                            MoveCompAutoZoom,ZoomMode,MousePanMultiply,
                            ButtonInfoTimeout=40,ButtonInfoTimeoutStart=10,
                            CurrentGuideNr,AreafillDrawMode,
                            SystemBusyMode,DrawDrillMode,
                            ZoomInOutProcessed,FastPaint,
                            DrawLayerCode[32],DrawCode;

double                      SearchMinX,SearchMinY,SearchMaxX,SearchMaxY,
                            CurrentDrawX1,CurrentDrawY1,
                            CurrentDrawX2,CurrentDrawY2,
                            ShiftOffsetX,ShiftOffsetY,
                            VisibleMaxX,VisibleMinX,VisibleMaxY,VisibleMinY,
                            CurrentX2,CurrentY2,ViewOffsetX,
                            ViewOffsetY,ViewScale,
                            SearchX1,SearchY1,SearchX2,SearchY2,
                            ConnectionX1,ConnectionY1,ConnectionX2,ConnectionY2,
                            StartFirstDrawingTraceX,StartFirstDrawingTraceY,
                            StartSecondDrawingTraceX,StartSecondDrawingTraceY,
                            EndSecondDrawingTraceX,EndSecondDrawingTraceY,
                            EndFirstDrawingTraceX,EndFirstDrawingTraceY,
                            TraceWidths[30],
                            ClearanceWidths[30],
                            MinDrillForThermalRelief,
                            StartPreviousTraceX,StartPreviousTraceY,
                            EndPreviousTraceX,EndPreviousTraceY,
                            DrawingConnectionX1,DrawingConnectionY1,
                            DrawingConnectionX2,DrawingConnectionY2;
...
double                      GridSizes[30],GridSize,UserGridSize,CompGridSize,
                            TraceGridSize,AreafillGridSize;

int32                       WindowWidth,WindowHeight,MousePosX,MousePosY,
                            WindowStartX,WindowStartY,
                            MaxScrollBarX,MaxScrollBarY,
                            ScrollSize,ScrollSizeDrawing,SelectionMode,LastActionNr,
                            ScrollEndOfWindow,
                            CurrentDrawingLayer,MaxLastActionNr,
                            CurrentDrawingNetNr,
                            ClientStartX,ClientStartY,
                            SelectRectX1,SelectRectY1,
                            SelectRectX2,SelectRectY2;

int32                       NrGraphicsLayers,
                            GraphicsLayers[256];
...
int32                       ViasSelected,
                            ...
                            AllTracesSelected,
                            TracesSelected[32],
                            HorTracesSelected[32],
                            VerTracesSelected[32],
                            Diag1TracesSelected[32],
                            Diag2TracesSelected[32],
                            TraceLayersSelectable[32],
                            ...
                            MaxNrObjectTexts2,
                            MaxNrObjectPolygons,
                            MaxObjectPolygonMemory,
                            MaxNrPolygon8Objects,
                            MaxNrVerticesPolygon,
                            MaxCompsMemory,
                            MaxTempMemory,
                            TempMemorySize,
                            MaxTempMemory2,
                            ...
                            TempMemorySize3,
                            MaxSpecialMemory[97],
                            MaxLayerPlotObjects[33],
                            NrLayerPlotObjects[33],
                            ClipBoardMemSize,
                            ClipBoardMemPos,
                            MaxShapesMemory,
                            MessageBufMemSize,
                            MessageBufPos,
                            MaxAreaFillMemoryTemp,
                            MaxAreaFillMemory;


int32                       GeometryDialogInitialX,
                            GeometryDialogInitialY,
                            GeomScreenWidth,
                            GeomScreenHeight,
                            GeomStartX,
                            GeomStartY;

int32                        Created,Painting,DCInUse,MouseChanged,Focused,
                            LeftButtonPressed,MiddleButtonPressed,RightButtonPressed,
                            UnselectAll,LeftButtonDoublePressed,
                            DataBaseChanged,
                            MovingComponents,
                            MouseCursorOnGrid,
                            GridVisible,
                            BackGroundActive,
                            DrawTwoTryingTraces,FileChanged,
                            SpacePressed,
                            HelpAsked,RecalcAreafillAfterInsert,
                            ReDrawing,DrawTraceUsingGuide,
                            SelectionEsc,
                            OkToSwitchDrawingLayers,OkToAddViewPos,
                            ParametersRelative,ActiveSchematicSelect,
                            ReplaceSelections,AddExtraTraceMode,
                            OkToUseSharedMemory,DrawClearanceForTryingTrace,
                            DrawViaInfoForTryingTrace,
                            UndoRedoActive,
                            FirstPaint,LinesAllDirection,
                            DrawTraceLimitInfo,

                            OkToDrawAreaFills,
                            OkToDrawAreaFillWithHatches,
                            OkToDrawClearances,
                            OkToDrawCompOutline,
                            OkToDrawConnections,
                            OkToDrawInnerPads,
                            OkToDrawTopPads,
                            OkToDrawBottomPads,
                            OkToDrawCompPlacement,
                            OkToDrawPowerPlanes,
                            OkToDrawSilkScreenBottom,
                            OkToDrawSilkScreenTop,
                            OkToDrawObjects,
                            OkToDrawInfoObjects,
                            OkToDrawInfo2Objects,
                            OkToDrawInfo3Objects,
                            OkToDrawInfo4Objects,
                            OkToDrawBoardOutline,
                            OkToDrawRoutingKeepoutTop,
                            OkToDrawRoutingKeepoutBottom,
                            OkToDrawRoutingKeepoutInner,
                            OkToDrawVias,
                            OkToDrawViaClearances,
                            OkToDrawWarnings,
                            OkToDrawErrors,
                            OkToRePaintAfterCompMove,
                            OkToRePaintAfterTraceDrawing,
                            DoNotShowAreafillThinLinesError,
                            DrawTopComponents=1,
                            DrawBottomComponents=1,
                            OkToDrawCrossHair,
                            OkToDrawCompReference,
                            OkToDrawCompValue;

int32                       DrawSoldMaskBottomMode,
                            DrawSoldMaskTopMode,
                            DrawPasteMaskBottomMode,
                            DrawPasteMaskTopMode,
                            ViewSingleLayer;

int64                       SpecialCounterProgramStart;
SYSTEMTIME                  CurrentDateProgramStart;

ObjectRecord                LimitObject,
                            TraceObject1,TraceObject2,DrawTrace1,DrawTrace2,
                            CurrentWorkingTrace,NullObject,
                            ConnectionObject1,ConnectionObject2,
                            OverlapObject,LastAddedObject,
                            LayerPlotObjects[33],
                            FoundObjects1[MaxFoundObjects],FoundObjects2[MaxFoundObjects];
ValueRecord                 NewValue;
ViaRecord                   CurrentVia,DefaultVia;

double                       Xoffset,Yoffset,Factor,RelX,RelY,
                            CurrentTraceWidth,CurrentClearance,
                            TextMinX,TextMinY,
                            TextMaxX,TextMaxY,
                            TraceWidthUser[10],
                            ClearanceWidthUser[10];

int32                       FirstSize,NrObjects,NrObjects2,NrObjects3,NrObjects4,
                            NrObjects5,NrObjects6,NrErrorObjects,
                            CurrentErrorNr,ComponentConnectionMode,
                            MaxNrSelectedObjectsIndex,NrGridSizes,NrTraceWidths,
                            NrClearanceWidths,MouseOnGridPosX,MouseOnGridPosY,
                            CurrentGuideNr,Units,SnapMode,CompSnapMode,
                            ViewPosPointer,LastTracePosPointer,
                            FoundProjectPath,FoundDesignPath,
                            CurrentNet,PCBSystemError;

int32                       CurrentObjectCode;

int32                       *IndexVerTraces[32],
                            *IndexHorTraces[32],
                            *IndexDiag1Traces[32],
                            *IndexDiag2Traces[32],
                            *IndexVias,
                            *IndexConnections,
                            NrNetItems,MaxNrNetItems,NrNetItems2,MaxNrNetItems2,
                            NrIndexesVerTraces[32],
                            NrIndexesHorTraces[32],
                            NrIndexesDiag1Traces[32],
                            NrIndexesDiag2Traces[32],
                            NrIndexesVias,
...
TracesArray                 *HorTraces[32],
                            *VerTraces[32],
                            *Diag1Traces[32],
                            *Diag2Traces[32];
int32                       DebugTest1;
ObjectArray                 *Objects,*Objects2,*Objects3,
                            *Objects4,*Objects5,*Objects6,*ErrorObjects;
ViaArray                    *Vias;
NetArray                    *Nets;
...
AreaFillsArray              *AreaFills;
PolygonRecord               *BufferPolygon,*ResultPolygon,*ExtraPolygon,*NewPolygon,
                            *WorkPolygon[MAX_WORK_POLGONS];

WCHAR                       StrW16[512],TextStrings[64][128];
char                        TextStrings2[64][128];
uint8                       *ShapesMem,*CompsMem,*AreaFillMem,*TempMem,*TempMem2,*TempMem3,
                            *AreaFillMemTemp,*AreaFillMemTemp2,*SpecialMem[MAX_SPECIAL_MEM];
HGLOBAL                     HorTracesGlobal[32],
                            VerTracesGlobal[32],
                            Diag1TracesGlobal[32],
                            Diag2TracesGlobal[32],
                            ObjectsGlobal,
                            Objects2Global,
                            Objects3Global,
                            Objects4Global,
                            ...
                            Objects6Global,
                            ErrorObjectsGlobal,
                            SelectedObjectsIndexGlobal,
                            ViasGlobal,
                            NetsGlobal,
                            NetsPosGlobal[32],
                            ViasPosGlobal,
                            ConnectionsPosGlobal,
                            ConnectionsGlobal,
                            NetConnectionsGlobal,
                            NetTestGlobal,
                            NetTest2Global,
                            CompsGlobal,
                            ShapesGlobal,
                            ...
                            CompsMemGlobal,
                            ObjectTextsGlobal2,
                            ObjectPolygonMemGlobal,
                            ObjectPolygonsGlobal,
                            AreaFillMemGlobal,
                            AreaFillMemoryTempGlobal,
                            AreaFillMemoryTemp2Global,
                            AreaFillMemoryTemp3Global,
                            AreaFillsGlobal,
                            BufferPolygonGlobal,
                            ResultPolygonGlobal,
                            ExtraPolygonGlobal,
                            ...
                            GlobalTempMem2,
                            GlobalTempMem3,
                            GlobalSpecialMem[MAX_SPECIAL_MEM],
                            WorkPolygonGlobal[MAX_WORK_POLGONS],
                            ...
                            ObjectArcsGlobal,
                            ObjectTextsGlobal,
                            NetItemsGlobal,NetItems2Global,
...

{
  ...
  if (ObjectsGlobal              !=0) ObjectsGlobalMemSize                      =GlobalSize(ObjectsGlobal              );
  if (Objects2Global             !=0) Objects2GlobalMemSize                     =GlobalSize(Objects2Global             );
  if (Objects3Global             !=0) Objects3GlobalMemSize                     =GlobalSize(Objects3Global             );
  if (Objects4Global             !=0) Objects4GlobalMemSize                     =GlobalSize(Objects4Global             );
  if (Objects5Global             !=0) Objects5GlobalMemSize                     =GlobalSize(Objects5Global             );
  if (Objects6Global             !=0) Objects6GlobalMemSize                     =GlobalSize(Objects6Global             );
  if (ErrorObjectsGlobal         !=0) ErrorObjectsGlobalMemSize                 =GlobalSize(ErrorObjectsGlobal         );
  if (SelectedObjectsIndexGlobal !=0) SelectedObjectsIndexGlobalMemSize         =GlobalSize(SelectedObjectsIndexGlobal );
  if (ViasGlobal                 !=0) ViasGlobalMemSize                         =GlobalSize(ViasGlobal                 );
  if (NetsGlobal                 !=0) NetsGlobalMemSize                         =GlobalSize(NetsGlobal                 );
  if (ViasPosGlobal              !=0) ViasPosGlobalMemSize                      =GlobalSize(ViasPosGlobal              );
  if (ConnectionsPosGlobal       !=0) ConnectionsPosGlobalMemSize               =GlobalSize(ConnectionsPosGlobal       );
  if (ConnectionsGlobal          !=0) ConnectionsGlobalMemSize                  =GlobalSize(ConnectionsGlobal          );
  if (NetConnectionsGlobal       !=0) NetConnectionsGlobalMemSize               =GlobalSize(NetConnectionsGlobal       );
  if (NetTestGlobal              !=0) NetTestGlobalMemSize                      =GlobalSize(NetTestGlobal              );
  if (NetTest2Global             !=0) NetTest2GlobalMemSize                     =GlobalSize(NetTest2Global             );
  if (CompsGlobal                !=0) CompsGlobalMemSize                        =GlobalSize(CompsGlobal                );
  if (ShapesGlobal               !=0) ShapesGlobalMemSize                       =GlobalSize(ShapesGlobal               );
  if (ShapesMemGlobal            !=0) ShapesMemGlobalMemSize                    =GlobalSize(ShapesMemGlobal            );
  if (CompsMemGlobal             !=0) CompsMemGlobalMemSize                     =GlobalSize(CompsMemGlobal             );
  if (AreaFillMemGlobal          !=0) AreaFillMemGlobalMemSize                  =GlobalSize(AreaFillMemGlobal          );
  if (AreaFillMemoryTempGlobal   !=0) AreaFillMemoryTempGlobalMemSize           =GlobalSize(AreaFillMemoryTempGlobal   );
  if (AreaFillMemoryTemp2Global  !=0) AreaFillMemoryTemp2GlobalMemSize          =GlobalSize(AreaFillMemoryTemp2Global  );
  if (AreaFillsGlobal            !=0) AreaFillsGlobalMemSize                    =GlobalSize(AreaFillsGlobal            );
  if (BufferPolygonGlobal        !=0) BufferPolygonGlobalMemSize                =GlobalSize(BufferPolygonGlobal        );
  if (ResultPolygonGlobal        !=0) ResultPolygonGlobalMemSize                =GlobalSize(ResultPolygonGlobal        );
  if (ExtraPolygonGlobal         !=0) ExtraPolygonGlobalMemSize                 =GlobalSize(ExtraPolygonGlobal         );
  if (NewPolygonGlobal           !=0) NewPolygonGlobalMemSize                   =GlobalSize(NewPolygonGlobal           );
  if (MessageBufGlobal           !=0) MessageBufGlobalMemSize                   =GlobalSize(MessageBufGlobal           );
  if (GlobalTempMem              !=0) GlobalTempMemMemSize                      =GlobalSize(GlobalTempMem              );
  if (GlobalTempMem2             !=0) GlobalTempMem2MemSize                     =GlobalSize(GlobalTempMem2             );
  if (ObjectLinesGlobal          !=0) ObjectLinesGlobalMemSize                  =GlobalSize(ObjectLinesGlobal          );
  if (ObjectRectsGlobal          !=0) ObjectRectsGlobalMemSize                  =GlobalSize(ObjectRectsGlobal          );
  if (ObjectCirclesGlobal        !=0) ObjectCirclesGlobalMemSize                =GlobalSize(ObjectCirclesGlobal        );
  if (ObjectArcsGlobal           !=0) ObjectArcsGlobalMemSize                   =GlobalSize(ObjectArcsGlobal           );
  if (ObjectTextsGlobal          !=0) ObjectTextsGlobalMemSize                  =GlobalSize(ObjectTextsGlobal          );
  if (ObjectTextsGlobal2         !=0) ObjectTextsGlobal2MemSize                 =GlobalSize(ObjectTextsGlobal2         );
  if (ObjectPolygonMemGlobal     !=0) ObjectPolygonMemGlobalMemSize             =GlobalSize(ObjectPolygonMemGlobal     );
  if (ObjectPolygonsGlobal       !=0) ObjectPolygonsGlobalMemSize               =GlobalSize(ObjectPolygonsGlobal       );
  if (CharsGlobal                !=0) CharsGlobalMemSize                        =GlobalSize(CharsGlobal                );
  if (NetItemsGlobal             !=0) NetItemsGlobalMemSize                     =GlobalSize(NetItemsGlobal             );
  if (NetItems2Global            !=0) NetItems2GlobalMemSize                    =GlobalSize(NetItems2Global            );

  for (cnt=0;cnt<32;cnt++) {
    if (HorTracesGlobal[cnt]     !=0) HorTracesGlobalMemSize[cnt]               =GlobalSize(HorTracesGlobal[cnt]       );
    if (VerTracesGlobal[cnt]     !=0) VerTracesGlobalMemSize[cnt]               =GlobalSize(VerTracesGlobal[cnt]       );
    if (Diag1TracesGlobal[cnt]   !=0) Diag1TracesGlobalMemSize[cnt]             =GlobalSize(Diag1TracesGlobal[cnt]     );
    if (Diag2TracesGlobal[cnt]   !=0) Diag2TracesGlobalMemSize[cnt]             =GlobalSize(Diag2TracesGlobal[cnt]     );
    if (NetsPosGlobal[cnt]       !=0) NetsPosGlobalMemSize[cnt]                 =GlobalSize(NetsPosGlobal[cnt]         );
  }
  for (cnt=0;cnt<8;cnt++) {
    if (WorkPolygonGlobal[cnt]   !=0) WorkPolygonGlobalMemSize[cnt]             =GlobalSize(WorkPolygonGlobal[cnt]     );
  }
  ...
  if (mode==1) {
    MemoryTextBuf[0]=0;
    sprintf(str,"Memory size Objects                   \t%i\t[ %i k ]\r\n",ObjectsGlobalMemSize                 ,ObjectsGlobalMemSize                /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Objects2                  \t%i\t[ %i k ]\r\n",Objects2GlobalMemSize                ,Objects2GlobalMemSize               /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Objects3                  \t%i\t[ %i k ]\r\n",Objects3GlobalMemSize                ,Objects3GlobalMemSize               /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Objects4                  \t%i\t[ %i k ]\r\n",Objects4GlobalMemSize                ,Objects4GlobalMemSize               /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Objects5                  \t%i\t[ %i k ]\r\n",Objects5GlobalMemSize                ,Objects5GlobalMemSize               /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Objects6                  \t%i\t[ %i k ]\r\n",Objects6GlobalMemSize                ,Objects6GlobalMemSize               /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ErrorObjects              \t%i\t[ %i k ]\r\n",ErrorObjectsGlobalMemSize            ,ErrorObjectsGlobalMemSize           /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size SelectedObjectsIndex      \t%i\t[ %i k ]\r\n",SelectedObjectsIndexGlobalMemSize    ,SelectedObjectsIndexGlobalMemSize   /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Vias                      \t%i\t[ %i k ]\r\n",ViasGlobalMemSize                    ,ViasGlobalMemSize                   /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Nets                      \t%i\t[ %i k ]\r\n",NetsGlobalMemSize                    ,NetsGlobalMemSize                   /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Connections               \t%i\t[ %i k ]\r\n",ConnectionsGlobalMemSize             ,ConnectionsGlobalMemSize            /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size NetConnections            \t%i\t[ %i k ]\r\n",NetConnectionsGlobalMemSize          ,NetConnectionsGlobalMemSize         /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Comps                     \t%i\t[ %i k ]\r\n",CompsGlobalMemSize                   ,CompsGlobalMemSize                  /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Shapes                    \t%i\t[ %i k ]\r\n",ShapesGlobalMemSize                  ,ShapesGlobalMemSize                 /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ShapesMem                 \t%i\t[ %i k ]\r\n",ShapesMemGlobalMemSize               ,ShapesMemGlobalMemSize              /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size CompsMem                  \t%i\t[ %i k ]\r\n",CompsMemGlobalMemSize                ,CompsMemGlobalMemSize               /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size AreaFillMem               \t%i\t[ %i k ]\r\n",AreaFillMemGlobalMemSize             ,AreaFillMemGlobalMemSize            /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size AreaFillMemoryTemp        \t%i\t[ %i k ]\r\n",AreaFillMemoryTempGlobalMemSize      ,AreaFillMemoryTempGlobalMemSize     /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size AreaFillMemoryTemp2       \t%i\t[ %i k ]\r\n",AreaFillMemoryTemp2GlobalMemSize     ,AreaFillMemoryTemp2GlobalMemSize    /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size AreaFills                 \t%i\t[ %i k ]\r\n",AreaFillsGlobalMemSize               ,AreaFillsGlobalMemSize              /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size BufferPolygon             \t%i\t[ %i k ]\r\n",BufferPolygonGlobalMemSize           ,BufferPolygonGlobalMemSize          /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ResultPolygon             \t%i\t[ %i k ]\r\n",ResultPolygonGlobalMemSize           ,ResultPolygonGlobalMemSize          /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ExtraPolygon              \t%i\t[ %i k ]\r\n",ExtraPolygonGlobalMemSize            ,ExtraPolygonGlobalMemSize           /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size NewPolygon                \t%i\t[ %i k ]\r\n",NewPolygonGlobalMemSize              ,NewPolygonGlobalMemSize             /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size MessageBuf                \t%i\t[ %i k ]\r\n",MessageBufGlobalMemSize              ,MessageBufGlobalMemSize             /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size TempMem                   \t%i\t[ %i k ]\r\n",GlobalTempMemMemSize                 ,GlobalTempMemMemSize                /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size TempMem2                  \t%i\t[ %i k ]\r\n",GlobalTempMem2MemSize                ,GlobalTempMem2MemSize               /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ObjectLines               \t%i\t[ %i k ]\r\n",ObjectLinesGlobalMemSize             ,ObjectLinesGlobalMemSize            /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ObjectRects               \t%i\t[ %i k ]\r\n",ObjectRectsGlobalMemSize             ,ObjectRectsGlobalMemSize            /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ObjectCircles             \t%i\t[ %i k ]\r\n",ObjectCirclesGlobalMemSize           ,ObjectCirclesGlobalMemSize          /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ObjectArcs                \t%i\t[ %i k ]\r\n",ObjectArcsGlobalMemSize              ,ObjectArcsGlobalMemSize             /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ObjectTexts               \t%i\t[ %i k ]\r\n",ObjectTextsGlobalMemSize             ,ObjectTextsGlobalMemSize            /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ObjectTexts2              \t%i\t[ %i k ]\r\n",ObjectTextsGlobal2MemSize            ,ObjectTextsGlobal2MemSize           /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ObjectPolygonMem          \t%i\t[ %i k ]\r\n",ObjectPolygonMemGlobalMemSize        ,ObjectPolygonMemGlobalMemSize       /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ObjectPolygons            \t%i\t[ %i k ]\r\n",ObjectPolygonsGlobalMemSize          ,ObjectPolygonsGlobalMemSize         /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Chars                     \t%i\t[ %i k ]\r\n",CharsGlobalMemSize                   ,CharsGlobalMemSize                  /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size NetItems                  \t%i\t[ %i k ]\r\n",NetItemsGlobalMemSize                ,NetItemsGlobalMemSize               /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size NetItems2                 \t%i\t[ %i k ]\r\n",NetItems2GlobalMemSize               ,NetItems2GlobalMemSize              /1024);
    strcat(MemoryTextBuf,str);

    for (cnt=0;cnt<Design.NrBoardLayers;cnt++) {
      sprintf(str,"Memory size HorTraces%02i         \t%i\t[ %i k ]\r\n",cnt,HorTracesGlobalMemSize[cnt]        ,HorTracesGlobalMemSize[cnt]         /1024);
      strcat(MemoryTextBuf,str);
      sprintf(str,"Memory size VerTraces%02i         \t%i\t[ %i k ]\r\n",cnt,VerTracesGlobalMemSize[cnt]        ,VerTracesGlobalMemSize[cnt]         /1024);
      strcat(MemoryTextBuf,str);
      sprintf(str,"Memory size Diag1Traces%02i       \t%i\t[ %i k ]\r\n",cnt,Diag1TracesGlobalMemSize[cnt]      ,Diag1TracesGlobalMemSize[cnt]       /1024);
      strcat(MemoryTextBuf,str);
      sprintf(str,"Memory size Diag2Traces%02i       \t%i\t[ %i k ]\r\n",cnt,Diag2TracesGlobalMemSize[cnt]      ,Diag2TracesGlobalMemSize[cnt]       /1024);
      strcat(MemoryTextBuf,str);
      sprintf(str,"Memory size NetsPos%02i           \t%i\t[ %i k ]\r\n",cnt,NetsPosGlobalMemSize[cnt]          ,NetsPosGlobalMemSize[cnt]           /1024);
      strcat(MemoryTextBuf,str);
    }
    for (cnt=0;cnt<8;cnt++) {
      sprintf(str,"Memory size WorkPolygon%i            \t%i\t[ %i k ]\r\n",cnt,WorkPolygonGlobalMemSize[cnt]      ,WorkPolygonGlobalMemSize[cnt]       /1024);
      strcat(MemoryTextBuf,str);
    }
    sprintf(str,"Memory size DesignSaveMem             \t%i\t[ %i k ]\r\n",DesignSaveMemGlobalMemSize           ,DesignSaveMemGlobalMemSize          /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ObjectTextBuf             \t%i\t[ %i k ]\r\n",ObjectTextBufGlobalMemSize           ,ObjectTextBufGlobalMemSize          /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size GatePinSwap               \t%i\t[ %i k ]\r\n",GatePinSwapGlobalMemSize             ,GatePinSwapGlobalMemSize            /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size AperTures                 \t%i\t[ %i k ]\r\n",AperTuresGlobalMemSize               ,AperTuresGlobalMemSize              /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size DrillAperTures            \t%i\t[ %i k ]\r\n",DrillAperTuresGlobalMemSize          ,DrillAperTuresGlobalMemSize         /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size LoadedAperTures           \t%i\t[ %i k ]\r\n",LoadedAperTuresGlobalMemSize         ,LoadedAperTuresGlobalMemSize        /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size Nets2                     \t%i\t[ %i k ]\r\n",Nets2GlobalMemSize                   ,Nets2GlobalMemSize                  /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size ActionNr                  \t%i\t[ %i k ]\r\n",ActionNrGlobalMemSize                ,ActionNrGlobalMemSize               /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size hClipMemory               \t%i\t[ %i k ]\r\n",hClipMemoryMemSize                   ,hClipMemoryMemSize                  /1024);
    strcat(MemoryTextBuf,str);
    sprintf(str,"Memory size NewClipBoardMem           \t%i\t[ %i k ]\r\n",NewGlobalClipBoardMemMemSize         ,NewGlobalClipBoardMemMemSize        /1024);
    strcat(MemoryTextBuf,str);
  }
...

{
  ...
  if (MaxNrVerticesPolygon==0) {
    ...
    if ((BufferPolygonGlobal=GlobalAlloc(GHND,MemSize))==NULL) return -1;
    if ((BufferPolygon=(PolygonRecord *)GlobalLock(BufferPolygonGlobal))==NULL) return -1;

    if ((ResultPolygonGlobal=GlobalAlloc(GHND,MemSize))==NULL) return -1;
    if ((ResultPolygon=(PolygonRecord *)GlobalLock(ResultPolygonGlobal))==NULL) return -1;

    if ((ExtraPolygonGlobal=GlobalAlloc(GHND,MemSize))==NULL) return -1;
    if ((ExtraPolygon=(PolygonRecord *)GlobalLock(ExtraPolygonGlobal))==NULL) return -1;

    if ((NewPolygonGlobal=GlobalAlloc(GHND,MemSize))==NULL) return -1;
    if ((NewPolygon=(PolygonRecord *)GlobalLock(NewPolygonGlobal))==NULL) return -1;
    for (cnt=0;cnt<MAX_WORK_POLGONS;cnt++) {
      if ((WorkPolygonGlobal[cnt]=GlobalAlloc(GHND,MemSize))==NULL) return -1;
      if ((WorkPolygon[cnt]=(PolygonRecord *)GlobalLock(WorkPolygonGlobal[cnt]))==NULL) return -1;
    }
    MaxNrVerticesPolygon=NrVerticesPolygon;
  } else {
    if (NrVerticesPolygon>MaxNrVerticesPolygon) {
      MemSize=NrVerticesPolygon*sizeof(PointRecord)+sizeof(PolygonInitRecord)+100;
      if ((NewMem=GlobalReAlloc(BufferPolygonGlobal,MemSize,GHND))==NULL) return -1;
      BufferPolygonGlobal=NewMem;
      if ((BufferPolygon=(PolygonRecord *)GlobalLock(BufferPolygonGlobal))==NULL) return -1;

      if ((NewMem=GlobalReAlloc(ResultPolygonGlobal,MemSize,GHND))==NULL) return -1;
      ResultPolygonGlobal=NewMem;
      if ((ResultPolygon=(PolygonRecord *)GlobalLock(ResultPolygonGlobal))==NULL) return -1;

      if ((NewMem=GlobalReAlloc(ExtraPolygonGlobal,MemSize,GHND))==NULL) return -1;
      ExtraPolygonGlobal=NewMem;
      if ((ExtraPolygon=(PolygonRecord *)GlobalLock(ExtraPolygonGlobal))==NULL) return -1;

      if ((NewMem=GlobalReAlloc(NewPolygonGlobal,MemSize,GHND))==NULL) return -1;
      NewPolygonGlobal=NewMem;
      if ((NewPolygon=(PolygonRecord *)GlobalLock(NewPolygonGlobal))==NULL) return -1;

      for (cnt=0;cnt<MAX_WORK_POLGONS;cnt++) {
        if ((NewMem=GlobalReAlloc(WorkPolygonGlobal[cnt],MemSize,GHND))==NULL) return -1;
...
{
  if ((CharsGlobal=GlobalAlloc(GHND,(int32)sizeof(CharsArray)))==NULL) return -1;
  if ((Chars=(CharsArray *)GlobalLock(CharsGlobal))==NULL) return -1;
  if (AllocateMemObjects(1024)!=0) return -1;
  if (AllocateMemObjects2(1024)!=0) return -1;
  if (AllocateMemObjects3(1024)!=0) return -1;
  return 0;
}
...

{
  ...
  if (lengte+2+MessageBufPos>=MessageBufMemSize) {
    if (AllocateMemMessageBuf(MessageBufMemSize+256*1024)!=0) return -1;
  }
