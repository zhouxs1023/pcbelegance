...
int32                       ClientWindowDivX,ClientWindowDivY,
                            CurrentDrawMode,RepeatMode,NrDefaultSymbolDirs,
                            CurrentDirection,Trace1Dir,Trace2Dir;
...
double                      SearchMinX,SearchMinY,SearchMaxX,SearchMaxY,
                            ...
                            DrawingTraceX1,DrawingTraceY1,
                            DrawingTraceX2,DrawingTraceY2,
...
int32                       WindowWidth,WindowHeight,MousePosX,MousePosY,
                            ...
                            ScrollEndOfWindow,MinActionNr,MaxLastActionNr,
                            CompRotationForMoving,
                            NrDesigns,SaveSymbolsLocally,
                            DeleteInfoCount,MouseActionMode,
                            ClientStartX,ClientStartY,
                            SelectRectX1,SelectRectY1,
                            SelectRectX2,SelectRectY2;


int32                       MaxNrComponents,
                            MessageMemSize,
                            ObjectTextBufMemSize,
                            ObjectTextBufPos,
                            NetInfoSize,
                            NrInt32Objects1,
                            MaxNrInt32Objects1,
...
int32                       Created,Painting,DCInUse,MouseChanged,Focused,
                            ...
                            CompsSelectionMode,DataBaseChanged,
                            MovingComponents,ReplaceSelections,
                            ForceDisableOnePinNetCheck,
                            MouseCursorOnGrid,FirstWrite,
                            CreateTopSheet,CreateLayoutFile,
                            DrawOnGrid,GridVisible,FirstPaint,
                            TraceCoorsSwitched,MemoryAllocated,
                            DrawTwoTryingTraces,HelpAsked,
                            InsertTwoTracesFromDrawing,EditingSheetSymbol,
                            ReDrawing,DrawSCH,DrawTraceUsingGuide,
                            Printing,EditingSymbol,FileChanged,
                            MouseTraceDrawing,UndoRedoActive,
                            SelectionActive,SelectionEsc,ExecuteZoom,TraceMode,
                            EmptyBrushActive,FillObjects,
                            DrawCompPlacement,DrawCompOutline,
                            DesignActive,UsingPartNumbers,
                            DisableOnePinNetCheck,
                            DrawCompReference,DrawCompValue,CompRotationChanged,
                            OkToAddViewPos,IncDeleteNr;

ObjectRecord                DrawWire1,DrawWire2,LimitObject,
                            WireObject1,WireObject2,
                            CurrentWorkingWire,NullObject;

uint8                       *ClipBoardMem;
HGLOBAL                     GlobalClipBoardMem;

double                       Xoffset,Yoffset,Factor,Factor2,RelX,RelY,CurrentFontSize,
                            GridSize,DrawGridSize;

int32                       FirstSize,NrObjects,NrObjects2,NrObjects3,NrObjects4,
                            NrObjects5,NrObjects6,MaxNrNewInstances,NrNewInstances,
                            MaxNrNewSymbols,NrNewSymbols,
                            MouseOnGridPosX,MouseOnGridPosY,NrSearchDirs,
                            CurrentObjectCode,CurrentColorCode,CurrentFontCode,
                            Units,Last,LastAction,MemoryAllocationMode,
                            ViewPosPointer,LastTracePosPointer,NrWireLabels,
                            LibNameNr,SymbolDirNameNr,NrLibFiles,NrSymbolDirs,
                            CurrentNet,SCHSystemError,NrSelectionsDialog,
                            AnnotateStart,AnnotateEnd,AnnotateMode,
                            NestingLevel,DesignBufMemSize,NrSheets,
                            NrComponents,AnnotateMode,PaperSize,
                            NrGeometryLibraries,NrSchematicSymbolLibraries,
                            NrNetInfos,NewSymbolsMemSize,MaxNewSymbolsMemSize,
                            CurrentFontNr,CurrentFontRotation;
...

{
  ...
  count=0;
  if ((length=strlen(str))==0) return -1;
  if (length>MAX_LENGTH_STRING-50) return -1;
...

{
  ...
  while ((cnt<lengte)
        ...
        &&
        (BusName[cnt]!=']')) cnt++;
  if (cnt==lengte) return 0;
  if (BusName[cnt]!='[') return -1;
  cnt=0;
  while ((cnt<lengte)
        &&
        (BusName[cnt]!='[')) cnt++;
  ...
  while ((cnt<lengte)
        &&
        (BusName[cnt]!=':')) {
    if (!isdigit(BusName[cnt])) return -1;
...

{
  ...
  if (GetNrLabelsBusName(BusName)<1) return -1;
...

{
  ...
  if ((PinBusCount<1)
     ...
     (SubPinBusCount>64)
     ||
     (SubPinBusCount>PinBusCount)) return -1;

  for (cnt=0;cnt<SubPinBusCount;cnt++) {
    if (GetLabelBusNameByIndex(SubPinBusName,str2,cnt)==-1) return -1;
    Found=0;
    cnt2=0;
    while ((cnt2<PinBusCount)
          &&
          (!Found)) {
      if (GetLabelBusNameByIndex(PinBusName,str1,cnt2)==-1) return -1;
      if (stricmpUTF8(str1,str2)==0) Found=1;
      cnt2++;
    }
    if (!Found) return -1;
  }
...

{
  ...
  if (MaxNrNewInstances>=NrInstances) return 1;
  ...
  if (NewInstancesGlobal==NULL) {
    ...
    if ((NewMem=GlobalReAlloc(NewInstancesGlobal,NrInstances*sizeof(InstanceRecord),GMEM_MOVEABLE|GMEM_ZEROINIT))==NULL) {
      return -1;
    }
