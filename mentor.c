...
typedef struct {
          int32 TopObjectType,BottomObjectType,
                UnplatedDrill,Info,Info2,
                Info3,Info4,Info5,NetNr,PinInfo,PinNr,
                Layer,CompNr,Mirror,PinCount,TraceNr;
          double TopPadWidth,TopPadHeight,
                 BottomPadWidth,BottomPadHeight,
                 InnerPadWidth,PowerPadWidth,
                 DrillWidth,
                 TopSolderMaskWidth,TopSolderMaskHeight,
                 BottomSolderMaskWidth,BottomSolderMaskHeight,
                 TopPasteMaskWidth,TopPasteMaskHeight,
                 BottomPasteMaskWidth,BottomPasteMaskHeight;
...

{
  ...
  while (cnt<NrTracePoints) {
    if ((InRangeSpecial(x,(*TracePoints)[cnt].x,100.0))
       &&
       (InRangeSpecial(y,(*TracePoints)[cnt].y,100.0))) {
      return cnt;
    }
    cnt++;
  }
...

{
  ...
  for (cnt=0;cnt<NrObjects6;cnt++) {
    ...
    if ((Object->Info & Mask)==0) {
      if ((InRangeSpecial(x,x1,100.0))
         &&
         (InRangeSpecial(y,y1,100.0))) {
        *NewX=x2;
        *NewY=y2;
        return cnt;
      }
      if ((InRangeSpecial(x,x2,100.0))
         &&
         (InRangeSpecial(y,y2,100.0))) {
        *NewX=x1;
        *NewY=y1;
        return cnt;
      }
...
*/
  ...
  sprintf(str2,"$$circle( \"%s\", %.4f, %.4f, %.4f, %.4f );",Layer,
          x/UNITS_MM,y/UNITS_MM,Width/UNITS_MM,LineWidth/UNITS_MM);
  WriteLn(fp,str2);
...
int32 MentorPrintArc(int32 fp,LPSTR Layer,double x1,double y1,double x3,double y3,
                     double x4,double y4,double LineWidth,int32 mode)
...
*/
  ...
  sprintf(str2,"$$arc( \"%s\", %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, \"\", @center, @radius );",
          Layer,(x1+x3)/UNITS_MM,(y1+y3)/UNITS_MM,(x1+x4)/UNITS_MM,(y1+y4)/UNITS_MM,
          x1/UNITS_MM,y1/UNITS_MM,LineWidth/UNITS_MM);
  WriteLn(fp,str2);
...
int32 MentorPrintPolygon(int32 fp,LPSTR Layer,PointRecord *Points,double ox,double oy,int32 NrVertices,int32 mode)
...
*/
  for (cnt=0;cnt<NrVertices;cnt++) {
    if (cnt==0) {
      sprintf(str2,"$$initial([%.4f,%.4f], , @nosnap );",
              (Points->x+ox)/UNITS_MM,(Points->y+oy)/UNITS_MM);
    } else {
      sprintf(str2,"$$terminal([%.4f,%.4f] );",
              (Points->x+ox)/UNITS_MM,(Points->y+oy)/UNITS_MM);
    }
    WriteLn(fp,str2);
    Points++;
...
*/
  ...
  x/=UNITS_MM;
  y/=UNITS_MM;
  sprintf(str2,"$$polygon( \"%s\", , [%.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f] );",
          Layer,x-x1,y-y1,x+x1,y-y1,x+x1,y+y1,x-x1,y+y1);
  WriteLn(fp,str2);
  return 0;
}
...
*/
  ...
  for (cnt=0;cnt<4;cnt++) {
    if (cnt==0) {
      sprintf(str2,"$$initial([%.4f,%.4f], , @nosnap );",
              Points[cnt].x/UNITS_MM,Points[cnt].y/UNITS_MM);
    } else {
      sprintf(str2,"$$terminal([%.4f,%.4f] );",
              Points[cnt].x/UNITS_MM,Points[cnt].y/UNITS_MM);
    }
...
*/
  if (mode==0) {
    ...
    if ((x==0.0)
       &&
       (y==0.0)) {
      sprintf(str2,"$$attribute( \"TERMINAL_DRILL_SIZE\", \"\", , @scale , , [%.4f, %.4f]);",
              Width/UNITS_MM,0.0);
    } else {
...
*/
  ...
  if (MentorUnits==1) {
    sprintf(str2,"$$path( \"%s\", %.1f, , [%.1f, %.1f, %.1f, %.1f]);",Layer,
            Width/2540.0,x1/2540.0,y1/2540.0,x2/2540.0,y2/2540.0);
  } else {
    sprintf(str2,"$$path( \"%s\", %.4f, , [%.4f, %.4f, %.4f, %.4f]);",Layer,
            Width/UNITS_MM,x1/UNITS_MM,y1/UNITS_MM,x2/UNITS_MM,y2/UNITS_MM);
  }
  WriteLn(fp,str2);
  return 0;
}
...

{
  ...
  if ((PinObject->RotationAngle<90.001)
     ||
     (PinObject->RotationAngle>270.0)) {
    strcpy(AlignmentStr,"@BL");
    strcpy(str4,"");
    if ((PinObject->RotationAngle<45.001)
       ||
       (PinObject->RotationAngle>314.999)) {
      JustX=1.0;
      JustY=0.0;
    } else {
      JustX=0.0;
...
//    strcpy(AlignmentStr,"@TR");
    ...
    if ((PinObject->RotationAngle>134.999)
       &&
       (PinObject->RotationAngle<225.0)) {
      JustX=-1.0;
      JustY=0.0;
    } else {
      JustX=0.0;
...
//    sprintf(str3,"%.1f",PinObject->RotationAngle-180.0);
  ...
  if ((PinObject->Layer==SILKSCREEN_TOP)
     ||
     (PinObject->Layer==SILKSCREEN_BOTTOM)) {
...
*/
  ...
  WriteLn(fp,str3);
  if (MentorUnits==1) {
    sprintf(str3,"$$page(0.0, 0.0, 0.0, @mils, 0.0, 0.0, [0.0,0.0,'PI$%s'] );",
...
*/
    ...
    if ((MentorPin->TopPadWidth)
       &&
       (MentorPin->BottomPadWidth)) {
      if ((MentorPin->TopObjectType==PIN_SMD_ROUND)
         &&
         (MentorPin->BottomObjectType==PIN_SMD_ROUND)) {
      ...
      if ((MentorPin->TopObjectType==PIN_SMD_RECT)
         &&
         (MentorPin->BottomObjectType==PIN_SMD_RECT)) {
        if ((InRange(MentorPin->TopPadWidth,MentorPin->BottomPadWidth))
           &&
    ...
    if ((MentorPin->TopSolderMaskWidth)
       &&
       (MentorPin->BottomSolderMaskWidth)) {
      if ((MentorPin->TopObjectType==PIN_SMD_ROUND)
         &&
         (MentorPin->BottomObjectType==PIN_SMD_ROUND)) {
      ...
      if ((MentorPin->TopObjectType==PIN_SMD_RECT)
        ...
        if ((InRange(MentorPin->TopSolderMaskWidth,MentorPin->BottomSolderMaskWidth))
          ...
          MentorPrintRectPad(fp,"SOLDER_MASK",0.0,0.0,MentorPin->TopSolderMaskWidth,MentorPin->TopSolderMaskHeight);
          Printed=1;
        } else {
          MentorPrintRectPad(fp,"SOLDER_MASK_1",0.0,0.0,MentorPin->TopSolderMaskWidth,MentorPin->TopSolderMaskHeight);
          MentorPrintRectPad(fp,"SOLDER_MASK_2",0.0,0.0,MentorPin->BottomSolderMaskWidth,MentorPin->BottomSolderMaskHeight);
          Printed=1;
        }
      }
    }
    ...
    if ((MentorPin->TopPasteMaskWidth)
       &&
       (MentorPin->BottomPasteMaskWidth)) {
      if ((MentorPin->TopObjectType==PIN_SMD_ROUND)
         &&
         (MentorPin->BottomObjectType==PIN_SMD_ROUND)) {
      ...
      if ((MentorPin->TopObjectType==PIN_SMD_RECT)
        ...
        if ((InRange(MentorPin->TopPasteMaskWidth,MentorPin->BottomPasteMaskWidth))
          ...
          MentorPrintRectPad(fp,"PASTE_MASK",0.0,0.0,MentorPin->TopPasteMaskWidth,MentorPin->TopPasteMaskHeight);
          Printed=1;
        } else {
          MentorPrintRectPad(fp,"PASTE_MASK_1",0.0,0.0,MentorPin->TopPasteMaskWidth,MentorPin->TopPasteMaskHeight);
          MentorPrintRectPad(fp,"PASTE_MASK_2",0.0,0.0,MentorPin->BottomPasteMaskWidth,MentorPin->BottomPasteMaskHeight);
          Printed=1;
        }
      }
    }
    ...
    if (MentorPin->Info & 2) {
      ...
      if (MentorPin->BottomPadWidth) {
        ...
        if (MentorPin->BottomPasteMaskWidth) {
          if (MentorPin->BottomObjectType==PIN_SMD_ROUND) {
            ...
            MentorPrintRectPad(fp,"PASTE_MASK",0.0,0.0,MentorPin->BottomPasteMaskWidth,MentorPin->BottomPasteMaskHeight);
          }
        }
        if (MentorPin->BottomSolderMaskWidth) {
          if (MentorPin->BottomObjectType==PIN_SMD_ROUND) {
            ...
            MentorPrintRectPad(fp,"SOLDER_MASK",0.0,0.0,MentorPin->BottomSolderMaskWidth,MentorPin->BottomSolderMaskHeight);
          }
        }
      }
    } else {
      ...
      if ((MentorPin->BottomObjectType)
        ...
        if (MentorPin->BottomObjectType==PIN_SMD_ROUND) {
          ...
          MentorPrintRectPad(fp,"PASTE_MASK_2",0.0,0.0,MentorPin->TopPasteMaskWidth,MentorPin->TopPasteMaskHeight);
        }
      }
      if ((MentorPin->TopObjectType)
         &&
         (MentorPin->TopSolderMaskWidth)) {
        if (MentorPin->TopObjectType==PIN_SMD_ROUND) {
          MentorPrintCircle(fp,"SOLDER_MASK_1",0.0,0.0,MentorPin->TopSolderMaskWidth,0.0);
        } else {
          MentorPrintRectPad(fp,"SOLDER_MASK_1",0.0,0.0,MentorPin->TopSolderMaskWidth,MentorPin->TopSolderMaskHeight);
...

{
  ...
  struct tm *today;
  time_t ltime;
  int32 fp,cnt,cnt2=0,cnt3,cnt4,res,ok,NrPinObjects,NrMainPinObjects,
        Found,ExtendedPin,NrExtendedPins,Layer,FoundThroughHolePin,
        ShapeSMD=0,MainPinObjectsPos[1024],ActiveLayers,FirstObject;
  ObjectRecord *PinObject=NULL,*PinObject2,
               TopPadObjectTemp,BottomPadObjectTemp,
               InnerPadObjectTemp,PowerPadObjectTemp,
               DrillObjectTemp,
               *TopPadObject,*BottomPadObject,
               *ThroughHoleObject,*DrillObject,
               *InnerPadObject=NULL,*PowerPadObject=NULL,
               *SoldMaskTopObject,*SoldMaskBottomObject,
               *PasteMaskTopObject,*PasteMaskBottomObject;
  ...
  char str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],
       *ShapeNameP,*PinNameP=NULL,*DefaultPadStackName,*GeomLayer;
...
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
    ...
    for (cnt2=(*Shapes)[cnt].ObjectPos;cnt2<NrObjects;cnt2++) {
      ...
      if ((PinObject->Text2)
         &&
         (strcmp(PinObject->Text2,"Layer2")==0)) {
        ok=1;
      }
      switch (PinObject->ObjectType) {
        case PIN_PUT_THROUGH_ROUND:
      ...
      for (cnt2=(*Shapes)[cnt].ObjectPos;cnt2<NrObjects;cnt2++) {
        ...
        if (PinObject->Info2==cnt3) {
          ...
          if ((NrShapeLayers>2)
             ...
             (PinObject->Layer>0)
             &&
             (PinObject->Layer<NrShapeLayers-1)) {
        ...
        if (stricmp(ShapeNameP,"con4a_usb_a_angle_f_lowprofile")==0) {
          ok=1;
          if ((InRange3(-8.0e5,PinObject->x1))
             &&
             (InRange3(-2.4e5,PinObject->y1))) {
            ok=1;
...
#endif
        if ((InRange2(x,PinObject->x1))
           &&
           (InRange2(y,PinObject->y1))) {
          switch (PinObject->Layer) {
            case SOLD_MASK_TOP:
              if ((Layer==-1)
                 ||
...
// *******************************************************************************************************
        ...
        for (cnt4=0;cnt4<NrMentorPins;cnt4++) {
          if (MentorPins[cnt4].TopObjectType) {
            if ((!TopPadObject)
               ||
               (TopPadObject->ObjectType!=MentorPins[cnt4].TopObjectType)
               ||
          ...
          if (MentorPins[cnt4].BottomObjectType) {
            if ((!BottomPadObject)
               ||
               (BottomPadObject->ObjectType!=MentorPins[cnt4].BottomObjectType)
               ||
          ...
          if (MentorPins[cnt4].DrillWidth>0.0) {
            if ((!DrillObject)
               ||
               (NotInRange2(DrillObject->x2,MentorPins[cnt4].DrillWidth))) {
              continue;
            }
          }
          if (MentorPins[cnt4].InnerPadWidth>0.0) {
            if ((!InnerPadObject)
               ||
               (NotInRange2(InnerPadObject->x2,MentorPins[cnt4].InnerPadWidth))) {
              continue;
            }
          }
          if (MentorPins[cnt4].PowerPadWidth>0.0) {
            if ((!PowerPadObject)
               ||
               (NotInRange2(PowerPadObject->x2,MentorPins[cnt4].PowerPadWidth))) {
              continue;
            }
          }
          if (MentorPins[cnt4].TopSolderMaskWidth>0.0) {
            if ((!SoldMaskTopObject)
               ||
               (SoldMaskTopObject->ObjectType!=MentorPins[cnt4].TopObjectType)
               ||
          ...
          if (MentorPins[cnt4].TopPasteMaskWidth>0.0) {
            if ((!PasteMaskTopObject)
               ||
               (PasteMaskTopObject->ObjectType!=MentorPins[cnt4].TopObjectType)
               ||
        ...
        if (!Found) {
          ...
          if (DrillObject) {
            ...
            if (DrillObject->x3) {
              MentorPins[NrMentorPins].InnerPadWidth=max(MentorPins[NrMentorPins].InnerPadWidth,DrillObject->x3);
            }
            if (DrillObject->y3) {
              MentorPins[NrMentorPins].PowerPadWidth=max(MentorPins[NrMentorPins].InnerPadWidth,DrillObject->y3);
            }
          }
          if (InnerPadObject) {
            MentorPins[NrMentorPins].InnerPadWidth=max(MentorPins[NrMentorPins].InnerPadWidth,InnerPadObject->x2);
            InnerPadObject->Info3=NrMentorPins;
          }
          if (PowerPadObject) {
            MentorPins[NrMentorPins].PowerPadWidth=max(MentorPins[NrMentorPins].PowerPadWidth,PowerPadObject->x2);
                ...
                for (cnt3=(*Shapes)[cnt].ObjectPos;cnt3<NrObjects;cnt3++) {
                  if (cnt2!=cnt3) {
                    ...
                    if ((PinObject->Info4==-1)
                       &&
                       (PinObject2->Info4>=0)) { // Extended pin
          ...
          if (PinObject->Info4==Found) {
            ...
            if (cnt3==0) {
              ...
              if (MentorUnits==1) {
                ...
                sprintf(str3,"$$page(0.0000, 0.0000, 0.0000, @mm, 0.0000, 0.0000, [0.0000,0.0000,'PI$%s_%s'] );",
                        ShapeNameP,PinObject->Text2);
                WriteLn(fp,str3);
              }
              WriteLn(fp,"$$point_mode(@vertex);");
...
//                  SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str2);
...
#endif
                MentorPrintRectPad(fp,GetMentorLayer(PinObject->Layer),OX,OY,PinObject->x2,PinObject->y2);
                break;
              case DRILL:
                MentorPrintDrill(fp,OX,OY,PinObject->y2,0);
                break;
              ...
              case PIN_LINE_ALL_ANGLE:
                MentorPrintPath(fp,GetMentorLayer(PinObject->Layer),
                                PinObject->x1-OX,PinObject->y1-OY,
                                PinObject->x2-OX,PinObject->y2-OY,PinObject->Thickness);
                break;
              case PIN_SMD_POLYGON:
                GeomPolygon=(GeomPolygonRecord *)PinObject->Address;
                PolygonPoints=(PointRecord *)GeomPolygon->Points;
                MentorPrintPolygon(fp,GetMentorLayer(PinObject->Layer),PolygonPoints,
                                   PinObject->x1-OX,PinObject->y1-OY,GeomPolygon->NrVertices,0);
                break;
            }
            cnt3++;
          }
...
*/
    ...
    for (cnt2=(*Shapes)[cnt].ObjectPos;cnt2<(*Shapes)[cnt+1].ObjectPos;cnt2++) {
      ...
      if (PinObject->Info3!=-1) {
        if ((PinObject->Text2)
           &&
           (stricmpUTF8(PinNameP,PinObject->Text2))) {
    ...
    for (cnt2=(*Shapes)[cnt].ObjectPos;cnt2<(*Shapes)[cnt+1].ObjectPos;cnt2++) {
            ...
            if (stricmpUTF8(MentorPins[PinObject->Info3].PadStackName,DefaultPadStackName)==0) {
              if (MentorUnits==1) {
                sprintf(str2,"$$attribute( \"COMPONENT_PIN_DEFINITION\", \"%s\", , @scale , , [%.1f,%.1f]);",
                        PinNameP,PinObject->x1/2540.0,
                                 PinObject->y1/2540.0);
              } else {
                sprintf(str2,"$$attribute( \"COMPONENT_PIN_DEFINITION\", \"%s\", , @scale , , [%.4f,%.4f]);",
                        PinNameP,PinObject->x1/UNITS_MM,
                                 PinObject->y1/UNITS_MM);
              }
              WriteLn(fp,str2);
            } else {
              sprintf(str2,"$$attribute( \"COMPONENT_PADSTACK_OVERRIDE\", \"%s,%s\");",
                      PinNameP,MentorPins[PinObject->Info3].PadStackName);
              WriteLn(fp,str2);
              if (MentorUnits==1) {
                sprintf(str2,"$$attribute( \"COMPONENT_PIN_DEFINITION\", \"%s\", , @scale , , [%.1f,%.1f]);",
                        PinNameP,PinObject->x1/2540.0,
      ...
      if (PinObject->Info4!=-1) {
        if ((PinObject->Text2)
          ...
          PinNameP=PinObject->Text2;
          sprintf(str2,"$$attribute( \"COMPONENT_PADSTACK_OVERRIDE\", \"%s,%s\");",
                  PinNameP,MentorSpecialPins[PinObject->Info4].PadStackName);
          WriteLn(fp,str2);
          if (MentorUnits==1) {
            sprintf(str2,"$$attribute( \"COMPONENT_PIN_DEFINITION\", \"%s\", , @scale , , [%.1f,%.1f]);",
...
*/
    ...
    if (y==0.0) y=UNITS_MM;
    if (MentorUnits==1) {
      sprintf(str2,"$$attribute( \"COMPONENT_HEIGHT\", \"\", , @scale , , [%.1f, 0.0]);",
              y/2540.0);
    } else {
      sprintf(str2,"$$attribute( \"COMPONENT_HEIGHT\", \"\", , @scale , , [%.4f, 0.0]);",
              y/UNITS_MM);
    }
    WriteLn(fp,str2);
/*
      ...
      if ((PinObject->Info3==-1)
        ...
        switch (PinObject->ObjectType) {
          ...
          case DRILL_UNPLATED:
            ...
            break;
          case OBJECT_LINE:
            MentorPrintPath(fp,GetMentorLayer(PinObject->Layer),x1,y1,
          ...
          case OBJECT_ARC:
            MentorPrintArc(fp,GetMentorLayer(PinObject->Layer),PinObject->x1,PinObject->y1,
                              PinObject->x3,PinObject->y3,PinObject->x4,
                              PinObject->y4,PinObject->Thickness,0);
            break;
          case OBJECT_TEXT:
          ...
          case OBJECT_POLYGON:
            ...
            PolygonPoints=(PointRecord *)GeomPolygon->Points;
            MentorPrintPolygon(fp,GetMentorLayer(PinObject->Layer),PolygonPoints,
                               -OX,-OY,GeomPolygon->NrVertices,0);
            break;
          default:
            ok=1;
            break;
...
*/
    sprintf(str2,"$$text( \"ASSEMBLY\", \"^$ref\", %.4f, %.4f, %.4f, @BL, 0, %s, %.4f, \"std\", \"None\", 0.0, 0.0 );",
            Shape->ShapeNameOriginX/UNITS_MM-PinObject->x2/UNITS_MM*MultTextOffset,
            Shape->ShapeNameOriginY/UNITS_MM,
            (Shape->ShapeNameHeight*MultTextY)/UNITS_MM,MentorAspectRatio,0.25);
    WriteLn(fp,str2);
    sprintf(str2,"$$text( \"SILKSCREEN\", \"^$ref\", %.4f, %.4f, %.4f, @BL, 0, %s, %.4f, \"std\", \"None\", 0.0, 0.0 );",
            Shape->ShapeNameOriginX/UNITS_MM-PinObject->x2/UNITS_MM*MultTextOffset,
            Shape->ShapeNameOriginY/UNITS_MM,
            (Shape->ShapeNameHeight*MultTextY)/UNITS_MM,MentorAspectRatio,SILKSCREEN_TEXT_THICKNESS);
    WriteLn(fp,str2);
      ...
      switch (PinObject->ObjectType) {
        case OBJECT_LINE:
          if ((FirstObject)
            ...
            sprintf(str2,"***************  Warning: There is more than one COMPONENT_PLACEMENT_OUTLINE object in shape %s\r\n",
                    ShapeNameP);
            AddMessage(str2);
//            SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str2);
            break;
...
*/
          ...
          x2=PinObject->x2*0.5;
          y2=PinObject->y2*0.5;
          sprintf(str2,"$$attribute( \"COMPONENT_PLACEMENT_OUTLINE\", \"\", , @scale , , [%.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f]);",
                  (x1-x2)/UNITS_MM,(y1-y2)/UNITS_MM,
                  (x1-x2)/UNITS_MM,(y1+y2)/UNITS_MM,
                  (x1+x2)/UNITS_MM,(y1+y2)/UNITS_MM,
                  (x1+x2)/UNITS_MM,(y1-y2)/UNITS_MM);
          WriteLn(fp,str2);
...
//            SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str2);
          ...
          if ((InRange(PinObject->x3,PinObject->x4))
             &&
             (InRange(PinObject->y3,PinObject->y4))) {
            x1=PinObject->x1;
            y1=PinObject->y1;
            x2=PinObject->x2*0.5;
            for (cnt3=0;cnt3<32;cnt3++) {
              ...
              if (cnt3==0) {
                if (MentorUnits==1) {
                  sprintf(str2,"$$initial([%.1f,%.1f], , @nosnap );",
                          x3/2540.0,y3/2540.0);
                } else {
                  sprintf(str2,"$$initial([%.4f,%.4f], , @nosnap );",
                          x3/UNITS_MM,y3/UNITS_MM);
                }
              } else {
                if (MentorUnits==1) {
                  sprintf(str2,"$$terminal([%.1f,%.1f] );",
                          x3/2540.0,y3/2540.0);
                } else {
                  sprintf(str2,"$$terminal([%.4f,%.4f] );",
                          x3/UNITS_MM,y3/UNITS_MM);
    ...
    for (cnt2=0;cnt2<NrObjects6;cnt2++) {
      ...
      switch (PinObject->ObjectType) {
        ...
        case OBJECT_POLYGON:
          ...
          PolygonPoints=(PointRecord *)GeomPolygon->Points;
          MentorPrintPolygon(fp,GetMentorLayer(PinObject->Layer),PolygonPoints,
                             PinObject->x1,PinObject->y1,GeomPolygon->NrVertices,0);
          break;
        default:
          ok=1;
          break;
    ...
    for (cnt2=0;cnt2<NrObjects6;cnt2++) {
      ...
      switch (PinObject->ObjectType) {
        ...
        case OBJECT_POLYGON:
          ...
          PolygonPoints=(PointRecord *)GeomPolygon->Points;
          MentorPrintPolygon(fp,GetMentorLayer(PinObject->Layer),PolygonPoints,
                             PinObject->x1,PinObject->y1,GeomPolygon->NrVertices,1);
          if (PinObject->Layer==ROUTING_KEEPOUT_LAYER+NrShapeLayers-1) {
            sprintf(str2,"$$attribute( \"%s\", \"PAD_1\", @mark, @scale );",GeomLayer);
          } else {
            if (PinObject->Layer==ROUTING_KEEPOUT_LAYER) {
...
// *******************************************************************************************************
    ...
    for (cnt2=0;cnt2<NrObjects6;cnt2++) {
      ...
      switch (PinObject->ObjectType) {
        ...
        case OBJECT_ARC:
          if (PinObject->Thickness==0.0) {
            MentorPrintCircle(fp,GeomLayer,PinObject->x1,PinObject->y1,
                              PinObject->x2,PinObject->Thickness);
          } else {
            if ((InRange(PinObject->x3,PinObject->x4))
               &&
              ...
              MentorPrintArc(fp,GeomLayer,PinObject->x1,PinObject->y1,
                                PinObject->x3,PinObject->y3,PinObject->x4,
                                PinObject->y4,PinObject->Thickness,0);
            }
          }
