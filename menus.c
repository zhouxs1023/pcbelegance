...
//      OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_SEPARATOR,0,0);
	...
	else
	{
		...
		{
			OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_NEXT_ERROR,
			              SC(540, "Cycle through design rule errors/warnings"));
		}
	}
}
...
//      OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_SEPARATOR,0,0);
	...
	{
		OwnAppendMenu(PopUpMenu11, MF_OWNERDRAW, ID_SELECT_LAYER + ID_SWITCH_TO_LAYER + Design.NrBoardLayers - 1 - cnt,
		              (LPSTR) (DrawLayerCode[Design.NrBoardLayers - 1 - cnt]));
	}

	OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_SEPARATOR, 0, 0);
	OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_FIND_UNROUTED_NET,
	              SC(545, "Start routing with the shortest net"));
	OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_RESET_FIRST_UNROUTED_NET, SC(546, "Reset find shortest net"));


	OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_VIEW_CENTER_ON_CONNECTION,
	              SC(547, "Center view on next connection"));
...
// ********************************************************************************************************
		...
		OwnAppendMenu(PopUpMenu30, MF_ENABLED | MF_STRING, ID_ROTATE_COMPONENTS_EACH_45,
		              SC(1296, "Rotate each at 45 degrees CCW"));
		OwnAppendMenu(PopUpMenu30, MF_ENABLED | MF_STRING, ID_ROTATE_COMPONENTS_EACH_90,
		              SC(1297, "Rotate each at 90 degrees CCW"));
		OwnAppendMenu(PopUpMenu30, MF_ENABLED | MF_STRING, ID_ROTATE_COMPONENTS_EACH_ANGLE,
		              SC(1298, "Rotate each at any angle CCW"));
		...
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_MOVE_COMP_TO_BOTTOM,
		              SC(554, "Move components to bottom layer"));
		...
		if (NrCompsSelected == 1)
		{
			...
			for (cnt = 0; cnt < Design.NrComps; cnt++)
			{
				...
				if ((Comp->Info & (OBJECT_NOT_VISIBLE | OBJECT_SELECTED)) == OBJECT_SELECTED)
				{
					...
					OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_POPUP, (UINT) PopUpMenu27,
					              SC(1239, "Component selections"));
					break;
				}
			}
		}
...
// ********************************************************************************************************
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_SEPARATOR, 0, 0);
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_POPUP, (UINT) PopUpMenu13Copy,
		              SC(564, "Copy objects component layer"));
...
// ********************************************************************************************************
		OwnAppendMenu(PopUpMenu13Copy, MF_ENABLED | MF_POPUP, (UINT) PopUpMenu21,
		              SC(568, "Placement outline bottom to"));
...
// ********************************************************************************************************
		...
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_SELECT_COMPS_BY_LIST,
		              SC(1256, "Select components by list"));
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_SEPARATOR, 0, 0);
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_UNSELECT_COMP_TOP, SC(1170, "Unselect top components"));
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_UNSELECT_COMP_BOTTOM,
		              SC(1171, "Unselect bottom components"));

// ********************************************************************************************************
		if (NrCompsSelected > 1)
		{
			OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_EDIT_ALIGN_COMP,
			              SC(580, "Component alignment/spacing"));
		}
	}
	else
	{
...
// ********************************************************************************************************
		...
		OwnAppendMenu(PopUpMenu1, MF_ENABLED | MF_STRING, ID_SELECT_BY_PARTNR2,
		              SC(1240, "Select all components with partnr"));
		OwnAppendMenu(PopUpMenu1, MF_ENABLED | MF_STRING, ID_SELECT_BY_GEOMETRY2,
		              SC(1241, "Select all components with geometry"));
		OwnAppendMenu(PopUpMenu1, MF_ENABLED | MF_STRING, ID_SELECT_BY_VALUE2,
		              SC(1242, "Select all components with value"));
		OwnAppendMenu(PopUpMenu1, MF_ENABLED | MF_STRING, ID_DESELECT_BY_PARTNR2,
		              SC(1243, "Deselect all components with partnr"));
		OwnAppendMenu(PopUpMenu1, MF_ENABLED | MF_STRING, ID_DESELECT_BY_GEOMETRY2,
		              SC(1244, "Deselect all components with geometry"));
		OwnAppendMenu(PopUpMenu1, MF_ENABLED | MF_STRING, ID_DESELECT_BY_VALUE2,
		              SC(1245, "Deselect all components with value"));
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_POPUP, (UINT) PopUpMenu1, SC(1246, "Component selections"));
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_SELECT_COMPS_BY_LIST,
		              SC(1256, "Select components by list"));
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_SEPARATOR, 0, 0);
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_MOVE_MULTIPLE_COMPS,
		              SC(582, "Move multiple components by list"));
...
// ********************************************************************************************************
		...
		OwnAppendMenu(PopUpMenu11, MF_ENABLED | MF_STRING, ID_COPY_ARRAY_OBJECTS,
		              SC(594, "Copy objects using a 2D array"));
...
// ********************************************************************************************************
		...
		{
			OwnAppendMenu(PopUpMenu12, MF_ENABLED | MF_STRING, ID_CHANGE_CIRCLE_OBJECT,
			              SC(603, "Change circle diameter"));
		}

		if (GetNrObjectSelections(0, 3) > 0)
		{
			OwnAppendMenu(PopUpMenu12, MF_ENABLED | MF_STRING, ID_CHANGE_ARC_OBJECT,
			              SC(604, "Change arc width/height"));
		}
		...
		{
			OwnAppendMenu(PopUpMenu12, MF_ENABLED | MF_STRING, ID_CHANGE_RECT_OBJECT,
			              SC(606, "Change rectangle width/height"));
		}
...
// ********************************************************************************************************
		...
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_ASSIGN_OBJECTS_TO_NET2,
		              SC(609, "Assign objects to a not used net"));
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_VIEW_INFOSELECTEDOBJECTS,
		              SC(610, "Info on selected objects"));
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_SEPARATOR, 0, 0);
// ********************************************************************************************************
		...
		*/
		...
		{
			res =
			    OwnAppendMenu(PopUpMenu30, MF_ENABLED | MF_STRING, ID_SEL_ONLY_SOLD_BOTTOM,
			                  SC(62, "Solder mask bottom"));
			ok = 1;
		}
		...
		{
			res =
			    OwnAppendMenu(PopUpMenu30, MF_ENABLED | MF_STRING, ID_SEL_ONLY_PASTE_BOTTOM,
			                  SC(70, "Paste mask bottom"));
			ok = 1;
		}
		...
		{
			res =
			    OwnAppendMenu(PopUpMenu30, MF_ENABLED | MF_STRING, ID_SEL_ONLY_DRILLS_UNPLATED,
			                  SC(619, "Drills (unplated)"));
			ok = 1;
		}
...
// ********************************************************************************************************
		...
		*/
		...
		{
			res =
			    OwnAppendMenu(PopUpMenu33, MF_ENABLED | MF_STRING, ID_UNSEL_SOLD_BOTTOM, SC(62, "Solder mask bottom"));
			ok = 1;
		}
		...
		{
			res =
			    OwnAppendMenu(PopUpMenu33, MF_ENABLED | MF_STRING, ID_UNSEL_PASTE_BOTTOM, SC(70, "Paste mask bottom"));
			ok = 1;
		}
		...
		{
			res =
			    OwnAppendMenu(PopUpMenu33, MF_ENABLED | MF_STRING, ID_UNSEL_DRILLS_UNPLATED,
			                  SC(619, "Drills (unplated)"));
			ok = 1;
		}
...
// ********************************************************************************************************
		...
		OwnAppendMenu(PopUpMenu, EnableMask | MF_STRING, ID_ADD_CIRCLE_OBJECT_F + NewLayer,
		              SC(638, "Add drill holes (plated)"));
		NewLayer = DRILL_UNPLATED_LAYER;
		OwnAppendMenu(PopUpMenu, EnableMask | MF_STRING, ID_ADD_CIRCLE_OBJECT_F + NewLayer,
		              SC(639, "Add drill holes (unplated)"));
// ********************************************************************************************************
	}
}
...
//      OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_SEPARATOR,0,0);
	...
	if (NrAreaFillsSelected > 0)
	{
		if (NrAreaFillsSelected == 1)
		{
			...
			OwnAppendMenu(PopUpMenu, EnableMask | MF_STRING, ID_CUT_AREAFILL_START_POLYGON,
			              SC(1306, "Cut from areafill start polygon"));
			OwnAppendMenu(PopUpMenu, EnableMask | MF_STRING, ID_CHANGE_AREAFILL, SC(649, "Change areafill"));
			OwnAppendMenu(PopUpMenu, EnableMask | MF_STRING, ID_SELECT_NET_AREAFILLS,
			              SC(1303, "Select areafills from the same net"));
		}

		OwnAppendMenu(PopUpMenu, EnableMask | MF_STRING, ID_VIEW_VERTICES, SC(650, "View vertices areafill polygon"));
		OwnAppendMenu(PopUpMenu, EnableMask | MF_STRING, ID_AREAFILL_COPY_TO_INFO4,
		              SC(1275, "Copy start polygon to info4 layer"));
	...
	else
	{
		...
		for (cnt = 0; cnt < Design.NrBoardLayers; cnt++)
		{
			OwnAppendMenu(PopUpMenu11, MF_OWNERDRAW, ID_SELECT_LAYER + ID_ADD_AREAFILL + Design.NrBoardLayers - 1 - cnt,
...
//      OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_SEPARATOR,0,0);
	...
	if ((NrTracesSelected > 0) || (NrConnectionsSelected > 0) || (NrViasSelected > 0))
	{
		...
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_COPY_TO_CLIPBOARD,
		              SC(661, "Copy traces/vias to clipboard"));
...
//    if (GetNrObjectSelections(0,0)>0) {
		...
		if ((NrTracesSelected > 0) || (NrViasSelected > 0))
		{
			...
			{
				OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_SELECT_NET_VIA_TRACES,
				              SC(1302, "Select vias/traces from the same net"));
			}
		}
		...
		if (NrTracesSelected == 1)
		{
			OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_DELETE_TRACESVIASNET,
			              SC(666, "Delete traces/vias net selected trace"));
			OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_GET_TRACELENGTH, SC(667, "Calculate length trace"));
			OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_EDIT_DESIGN_RULES_NET,
			              SC(1180, "Change design rules net"));
		}
	...
	else
	{
		...
		{
			OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_SWITCH_NEAREST_LAYER,
			              SC(543, "Switch to nearest layer"));
		}
		...
		{
			OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_NEXT_ERROR,
			              SC(668, "Cycle through design rule errors"));
		}

		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_INSERT_FROM_CLIPBOARD,
		              SC(669, "Load traces/vias from clipboard"));
	}
}
...
//  OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,0,SC(687,"Other menus"));
	...
	{
		OwnAppendMenu(PopUpMenu1000, MF_ENABLED | MF_STRING, ID_ACTION_MOVE_COMPVALUES,
		              SC(681, "Component values menu"));
	}
	...
	{
		OwnAppendMenu(PopUpMenu1000, MF_ENABLED | MF_STRING, ID_ACTION_AREAFILLS,
		              SC(683, "Areafills/powerplanes menu"));
	}

	if (SelectionMode != MOVING_TRACES_VIAS_MODE)
	{
		OwnAppendMenu(PopUpMenu1000, MF_ENABLED | MF_STRING, ID_ACTION_DRAG_TRACES_VIAS,
		              SC(684, "Change traces/vias menu"));
	}

	if (SelectionMode != DRAG_TRACES_VIAS_COMPS_MODE)
	{
		OwnAppendMenu(PopUpMenu1000, MF_ENABLED | MF_STRING, ID_ACTION_DRAG_COMP_TRACE_VIAS,
		              SC(685, "Drag traces/vias/components menu"));
	}
...
void TraceMenuPopUp(int32 mode)
{
	...
	{
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_TRACE_MODE_ARC90,
		              SC(697, "Arc trace drawing (90 degrees)"));
	}

	if (TraceDrawingMode != 3)
	{
		OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_TRACE_MODE_ARC45,
		              SC(698, "Arc trace drawing (45 degrees)"));
	}
	...
	OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_SWITCH_TWO_TRACES_ON_OFF,
	              SC(700, "Draw two trying traces on/off"));
	OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_SWITCH_TRYING_CLEAR_ON_OFF,
	              SC(701, "Draw clearance trying traces on/off"));
...
void MakeCheckMenu()
{
	...
	if (Design.NrBoardLayers > 0)
	{
		for (Layer = Design.NrBoardLayers - 1; Layer >= 0; Layer--)
		{
			...
			res =
			    OwnAppendMenu(CheckLayerMenu, MF_ENABLED | MF_STRING, ID_SELECT_LAYER + ID_CHECK_LAYERS + Layer,
			                  TextStr);
		}
	}
