...

{
  ...
  if (LineInputDialog(&TypeObject,SC(811,"Move entire PCB (This operation can not be undone)"),0)==1) {
    if ((NrParams=ScanParameters(-1,TypeObject.Text,0))!=2) return;
  }

  if ((InRange(ParamsFloat[0],0.0))
     &&
     (InRange(ParamsFloat[1],0.0))) return;
...

{
  ...
  ObjectLineRecord *ObjectLine,NewObjectLine,NewObjectLine2,
                   NewObjectLine3,NewObjectLine4;
...
//  Rotation=25.0;
  ...
  for (cnt=0;cnt<Design.NrConnections;cnt++) {
    ...
    if (((Connection->Info & (OBJECT_NOT_VISIBLE)) == 0)
       &&
       (Connection->AddNr<=TempLastActionNr)) {
  ...
  for (cnt=0;cnt<Design.NrAreaFills;cnt++) {
    ...
    if (((AreaFill->Info & (OBJECT_NOT_VISIBLE)) == 0)
       &&
       (AreaFill->AddNr<=TempLastActionNr)) {
    ...
    if (((ObjectRect->Info & (OBJECT_NOT_VISIBLE)) == 0)
      ...
      if (mode>0) {
        ...
        if ((ObjectRect->Info & OBJECT_FILLED) == 0) {
          ...
          if ((AddObjectLine(&NewObjectLine))
             &&
             (AddObjectLine(&NewObjectLine2))
             &&
             (AddObjectLine(&NewObjectLine3))
             &&
...
*/
  ...
  for (cnt=0;cnt<Design.NrObjectTexts2;cnt++) {
    ...
    if (((ObjectText2->Info & (OBJECT_NOT_VISIBLE)) == 0)
      ...
      if (NewObjectText2.TextMode & 0x10) { // Mirror
...
{
  int32  cnt,cnt2,CompPinNr,TempLastActionNr,MemSize,NewShapeNr,OldShapeNr,
         OkToRepaint,MemPos,res,CompInfo;
  CompRecord  *Comp,*NewComp;
  int    fp;
  ...
  if (GetNrCompsSelected()!=1) return;
  MessageBufPos=0;
  Geometrie[0]=0;
  SelectGeometrie(Geometrie);
  if (Geometrie[0]==0) return;
  NewShapeNr=LoadShape(Geometrie);
  OkToRepaint=0;

  if (NewShapeNr==-1) return;
...

{
  ...
  while ((cnt<l)
        &&
        (Str[cnt]!=',')) cnt++;
  if (cnt>pos) memmove(Result,&Str[pos],cnt-pos);
  Result[cnt-pos]=0;
  if (cnt==l) {
    Str[0]=0;
    return;
...

{
  ...
  double UnitMult;
  char  NewFile[MAX_LENGTH_STRING],LineBuf[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],
        str2[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],Params[10][MAX_LENGTH_STRING];

  if (GetNewFileUTF8(PCBWindow,NULL,NewFile,ExportDir,SC(390,"All files"),
  ...
  CurrentUnits=0;
  UnitMult=2540.0;
  if ((fp=TextFileOpenUTF8(NewFile))<0) return;
  while ((Length=ReadLn(fp,LineBuf))>=0) {
    ...
    if ((Length>1)
      ...
      if (stricmpOwn(str,SC(814,"UNITS"))==0) {
        if (stricmpOwn(str2,SC(815,"MM"))==0) {
          CurrentUnits=1;
          UnitMult=100000.0;
        }
        ...
        if (stricmpOwn(str2,SC(818,"CM"))==0) {
          CurrentUnits=3;
          UnitMult=1000000.0;
        }
      }
      if (stricmpOwn(str,"LINE")==0) {
        ...
        for (cnt=0;cnt<5;cnt++) {
          ...
          if ((Params[cnt][0]==0)
             ||
             (sscanf(Params[cnt],"%f",&ParamsFloat[cnt])!=1)) {
            OkToInclude=0;
          } else {
      ...
      if ((stricmpOwn(str,"RECT")==0)
         ||
         (stricmpOwn(str,"RECTANGLE")==0)) {
        OkToInclude=1;
        for (cnt=0;cnt<5;cnt++) {
          Params[cnt][0]=0;
          ParamsFloat[cnt]=0.0;
...

{
  ...
  Mask=0;
  if ((mode & 0x10) == 0x10) Mask=OBJECT_SELECTED;
  CurrentUnits=mode & 0x0f;

  if ((fp=FileOpenWriteUTF8(NewFile))<0) return;

  for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
    ObjectLine=&((*ObjectLines)[cnt]);
    if (((ObjectLine->Info & (OBJECT_NOT_VISIBLE|Mask)) == Mask)
       &&
  ...
  for (cnt=0;cnt<Design.NrObjectRects;cnt++) {
    ObjectRect=&((*ObjectRects)[cnt]);
    if (((ObjectRect->Info & (OBJECT_NOT_VISIBLE|Mask)) == Mask)
       &&
...
*/
  for (cnt=0;cnt<Design.NrObjectArcs;cnt++) {
    ObjectArc=&((*ObjectArcs)[cnt]);
    if (((ObjectArc->Info & (OBJECT_NOT_VISIBLE|Mask)) == Mask)
       &&
  ...
  for (cnt=0;cnt<Design.NrObjectTexts;cnt++) {
    ObjectText=&((*ObjectTexts)[cnt]);
    if (((ObjectText->Info & (OBJECT_NOT_VISIBLE|Mask)) == Mask)
       &&
       (ObjectText->Layer==Layer)) {
      sprintf(str,"TEXT   ");
...

{
  ...
  if (AreafillNrToMove==-1) return -1;
  AreaFill=(AreaFillRecord *)&(AreaFillMem[(*AreaFills)[AreafillNrToMove]]);
  FirstPolygon=(PolygonRecord *)((uint8 *)AreaFill+sizeof(AreaFillRecord));
...
*/
  ...
  ClipMouseCursor();
  Rotation=0.0;
  DrawSelectedAreafills(CurrentX,CurrentY,
                        CurrentX2,CurrentY2,
                        CentreSelectedX,CentreSelectedY,
                        Rotation,1);
  FirstShift=1;
  CompPlaced=0;
  SystemBusyMode=3;
  ...
  while (!SelectionEsc) {
    if (MouseChanged) {
      ...
      if ((OldX!=CurrentX)
         ...
         (OldY!=CurrentY)) {
        if (!ShiftPressed) {
          DrawSelectedAreafills(OldX,OldY,
                                CurrentX2,CurrentY2,
                                CentreSelectedX,CentreSelectedY,
                                Rotation,1);
        }
    ...
    if (LeftButtonPressed) {
      ...
      DrawSelectedAreafills(CurrentX,CurrentY,
                            ...
                            CentreSelectedX,CentreSelectedY,
                            Rotation,1);
      PlaceMovedAreafill(AreafillNrToMove,
                         CurrentX,CurrentY,
                         CurrentX2,CurrentY2,
                         CentreSelectedX,CentreSelectedY,
                         Rotation,mode*2+1);
...

{
  ...
  double  OldX,OldY,CurrentX,CurrentY,ShiftX,ShiftY,x1,y1,CurrentX2,CurrentY2,
          SpecialDivX1,SpecialDivY1,SpecialDivX2,SpecialDivY2;
  AreaFillRecord *AreaFill;
  char    TempInfoStr[MAX_LENGTH_STRING];
  PolygonRecord  *FirstPolygon;
  DrawXorFunctionRecord DrawXorFunction;
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(819,"Distance Y"));
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDD_UNITS:
          ...
          break;
        case IDOK:
          if ((GetDialogIntValue(Dialog,IDC_EDIT6,&CopyArrayCopyNrX)==0)
             ||
             (CopyArrayCopyNrX<1)
             ||
             (CopyArrayCopyNrX>100)) {
            MessageBoxOwn(PCBWindow,SC(824,"Nr X copies"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          if ((GetDialogIntValue(Dialog,IDC_EDIT5,&CopyArrayCopyNrY)==0)
             ||
             (CopyArrayCopyNrY<1)
             ||
             (CopyArrayCopyNrY>100)) {
            MessageBoxOwn(PCBWindow,SC(825,"Nr Y copies"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          if ((CopyArrayCopyNrX==1)
             &&
             (CopyArrayCopyNrY==1)) {
            MessageBoxOwn(PCBWindow,SC(826,"Nothing to copy"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
...

{
  ...
  res=OwnDialogBox(PCBClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_COPY_ARRAY1), PCBWindow, (DLGPROC)CopyArrayDialog2);
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(827,"Rotation angle"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(828,"degrees (counter clockwise)"));
      SetDialogItemText(Dialog,IDC_STATIC3                ,SC(829,"Nr copies"));
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(830,"Rotation centre"));
      SetDialogItemText(Dialog,IDC_STATIC5                ,SC(831,"Rotation individual objects"));
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        ...
        case IDD_UNITS:
          ...
          break;
        case IDOK:
          if ((GetDialogIntValue(Dialog,IDC_EDIT5,&CopyArrayRotationCopyNr)==0)
             ||
             (CopyArrayRotationCopyNr<1)
             ||
             (CopyArrayRotationCopyNr>100)) {
            MessageBoxOwn(PCBWindow,SC(829,"Nr copies"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          res=1;
          if ((GetDialogFloatValue(Dialog,IDC_EDIT4,&CopyArrayRotationValue)==0)
            ...
            MessageBoxOwn(PCBWindow,SC(205,"Rotation is zero"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
...
int32 CopyArrayRotationDialog(double *RotationX,double *RotationY,double *Rotation,
                              int32 *NrCopies,int32 *RotateMode)

{
  int   res;

  res=OwnDialogBox(PCBClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_COPY_ARRAY2), PCBWindow, (DLGPROC)CopyArrayRotationDialog2);
...
{
  int32  cntx,cnty,cnt,cnt2,cnt3,TempLastActionNr,PolygonSize,count,NrCopiesX,NrCopiesY,
         NrCopies,RotateMode,NrObjectsSelected;
  double  x1,y1,minx,miny,maxx,maxy,CentreSelectedX,CentreSelectedY,
          DistanceX,DistanceY,RotationCentreX,RotationCentreY,Rotation,RotationAngle;
  ...
  if (mode==0) {
    ...
    if ((RotateMode & 2) == 0) { // Do not rotate individual objects
      ...
      if ((NrObjectsSelected==1)
         &&
         (GetNrObjectSelections(0,4)>0)) {
        for (cnt=0;cnt<Design.NrObjectTexts2;cnt++) {
          ObjectText2=&((*ObjectTexts2)[cnt]);
          if ((ObjectText2->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
    ...
    if (((ObjectLine->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
      ...
      if (mode==0) {
        ...
        for (cnt2=0;cnt2<NrCopies;cnt2++) {
          ...
          if ((RotateMode & 2) == 0) { // Do not rotate individual objects
          ...
          } else { // Rotate individual objects
            ...
            RotatePointFromOtherPoint(&NewObjectLine.X1,&NewObjectLine.Y1,
                                      RotationCentreX,RotationCentreY,RotationAngle);
            RotatePointFromOtherPoint(&NewObjectLine.X2,&NewObjectLine.Y2,
                                      RotationCentreX,RotationCentreY,RotationAngle);
          }
          if (AddObjectLine(&NewObjectLine)) {
            Changed=1;
        ...
        if ((RotateMode & 2) == 0) { // Do not rotate individual objects
          ...
          if ((ObjectRect->Info & OBJECT_FILLED) == 0) {
            for (cnt2=0;cnt2<NrCopies;cnt2++) {
              ...
              RotatePointFromOtherPoint(&NewObjectLine.X1 ,&NewObjectLine.Y1,RotationCentreX,RotationCentreY,RotationAngle);
              RotatePointFromOtherPoint(&NewObjectLine.X2 ,&NewObjectLine.Y2,RotationCentreX,RotationCentreY,RotationAngle);
              RotatePointFromOtherPoint(&NewObjectLine2.X1,&NewObjectLine2.Y1,RotationCentreX,RotationCentreY,RotationAngle);
              RotatePointFromOtherPoint(&NewObjectLine2.X2,&NewObjectLine2.Y2,RotationCentreX,RotationCentreY,RotationAngle);
              RotatePointFromOtherPoint(&NewObjectLine3.X1,&NewObjectLine3.Y1,RotationCentreX,RotationCentreY,RotationAngle);
              RotatePointFromOtherPoint(&NewObjectLine3.X2,&NewObjectLine3.Y2,RotationCentreX,RotationCentreY,RotationAngle);
              RotatePointFromOtherPoint(&NewObjectLine4.X1,&NewObjectLine4.Y1,RotationCentreX,RotationCentreY,RotationAngle);
              RotatePointFromOtherPoint(&NewObjectLine4.X2,&NewObjectLine4.Y2,RotationCentreX,RotationCentreY,RotationAngle);
              if ((AddObjectLine(&NewObjectLine))
                 &&
                 (AddObjectLine(&NewObjectLine2))
                 &&
      ...
      if (mode==0) {
        ...
        for (cnt2=0;cnt2<NrCopies;cnt2++) {
          ...
          if ((RotateMode & 2) == 0) { // Do not rotate individual objects
            ...
            RotatePointFromOtherPoint(&NewObjectArc.CentreX,&NewObjectArc.CentreY,
                                      RotationCentreX,RotationCentreY,RotationAngle);
            RotatePointFromOtherPoint(&NewObjectArc.StartDiffX,&NewObjectArc.StartDiffY,
                                      0.0,0.0,RotationAngle);
            RotatePointFromOtherPoint(&NewObjectArc.EndDiffX,&NewObjectArc.EndDiffY,
                                      0.0,0.0,RotationAngle);
          }
          if (AddObjectArc(&NewObjectArc)) {
            Changed=1;
      ...
      if (mode==0) {
        ...
        for (cnt2=0;cnt2<NrCopies;cnt2++) {
          ...
          if ((RotateMode & 2) == 0) { // Do not rotate individual objects
            ...
            RotatePointFromOtherPoint(&NewObjectText2.X,&NewObjectText2.Y,
                                      RotationCentreX,RotationCentreY,RotationAngle);
          ...
          if ((RotateMode & 2) == 0) { // Do not rotate individual objects
            ...
            for (cnt3=0;cnt3<count;cnt3++) {
              RotatePointFromOtherPoint2(&NewObjectPolygon.Points[cnt3].x,
                                         &NewObjectPolygon.Points[cnt3].y,
                                         RotationCentreX,RotationCentreY,RotationAngle);
            }
          }
          if (AddObjectPolygon(&NewObjectPolygon)) {
...

{
  ...
  for (cnt3=0;cnt3<count;cnt3++) {
    sprintf(str,"[%d] %.12f,%.12f",cnt3,(*DrawPolygon).Points[cnt3].x/1e5,(*DrawPolygon).Points[cnt3].y/1e5);
    AddToMessageBuf(str);
  }
  if (DrawPolygon2) {
    ...
    for (cnt3=0;cnt3<count;cnt3++) {
      sprintf(str,"[%d] %.12f,%.12f",cnt3,(*DrawPolygon2).Points[cnt3].x/1e5,(*DrawPolygon2).Points[cnt3].y/1e5);
      AddToMessageBuf(str);
    }
  }
  if (MessageBufPos!=0) {
