...
int32 GetPolygonFromLines(int32 mode)
{
	...
	case GEOM_NAME_LAYER:
	case BOARD_OUTLINE_LAYER:
		MessageBoxUTF8(GEOMWindow, SC(403, "Can not create a polygon on this layer"), SC(404, "error"),
		               MB_APPLMODAL | MB_OK);
		return -1;
		break;
	}
...
//  LinePointsNumbers=
	...
	for (cnt = 0; cnt < NrLinePoints; cnt++)
	{
		...
		{
			ok = 1;
			MessageBoxUTF8(GEOMWindow, SC(405, "Did not found a closed polygon"), SC(404, "error"),
			               MB_APPLMODAL | MB_OK);
			return -1;
		}
	}
