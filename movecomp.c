  ...
  if (!EditingSymbol) {
    if (WiresSelected+
      ...
      if ((InstanceRefsSelected==0)
         ...
         (ObjectTextsSelected==0)
         &&
         (GlobalConnectionTextsSelected==1)) {
...

{
  ...
  double  OldX,OldY,CurrentX,CurrentY,hx,hy,divx,divy,ShiftX,ShiftY,
          CurrentX2,CurrentY2,x1,y1,Mode2=1;
  HMENU  PopUpMenu;
...
//  ObjectRecord *Object,*Object2;
  double  x1,y1,x2;
  int32  cnt,cnt2,lengte,NrLines,Count,LinePos[16],TextRotation,TextAlignment,
         NrProperties,BusConnectionType;
  ...
  for (cnt=0;cnt<Count;cnt++) {
    ...
    if (((Instance->Info & (OBJECT_SELECTED)) == OBJECT_SELECTED)
      ...
      if (((Instance->Info & 1) == 1)
         &&
         (GetMinMaxInstanceReferenceText(Instance)==0)) {
      ...
      if (((Instance->Info & 2) == 2)
         &&
         (GetMinMaxInstanceValueText(Instance))) {
        SelectedMinX=min(SelectedMinX,TextMinX);
        SelectedMaxX=max(SelectedMaxX,TextMaxX);
        SelectedMinY=min(SelectedMinY,TextMinY);
        SelectedMaxY=max(SelectedMaxY,TextMaxY);
  ...
  if (!EditingSymbol) {
    ...
    for (cnt=0;cnt<Design.NrNetLabels;cnt++) {
      ...
      if ((NetLabel->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
        ...
        GetMinMaxText(NetLabel->ConnectX+NetLabel->TextX,
                      NetLabel->ConnectY+NetLabel->TextY,
                      1.0,0,TextRotation,TextAlignment,str);
        SelectedMinX=min(SelectedMinX,TextMinX);
        SelectedMaxX=max(SelectedMaxX,TextMaxX);
        SelectedMinY=min(SelectedMinY,TextMinY);
        SelectedMaxY=max(SelectedMaxY,TextMaxY);
    ...
    for (cnt=0;cnt<DesignSymbol.NrPins;cnt++) {
      ...
      if ((Pin->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
        ...
        GetMinMaxText(Pin->NameX,Pin->NameY,1.0,0,TextRotation,
                      Pin->NameInfo & 15,(LPSTR)Pin->Name);
        SelectedMinX=min(SelectedMinX,TextMinX);
        SelectedMaxX=max(SelectedMaxX,TextMaxX);
        SelectedMinY=min(SelectedMinY,TextMinY);
        SelectedMaxY=max(SelectedMaxY,TextMaxY);
...
//      FontNr=ObjectText->FontNr;
      GetMinMaxText(ObjectText->X,ObjectText->Y,
                    ObjectText->FontHeight,0,TextRotation,TextAlignment,ObjectText->Text);
      SelectedMinX=min(SelectedMinX,TextMinX);
      SelectedMaxX=max(SelectedMaxX,TextMaxX);
      SelectedMinY=min(SelectedMinY,TextMinY);
      SelectedMaxY=max(SelectedMaxY,TextMaxY);
      ...
      if (((Instance->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
        ...
        if ((Instance->Info & 4) == 4) {
          ...
          if ((Instance->Info & 1) == 1) {
            ...
            if (Instance->SymbolInfo & OBJECT_MIRRORX) { // Mirror X
              RotateFlipPoint(&rx,&ry,0.0,0.0,4);
            }
            if (Instance->SymbolInfo & OBJECT_MIRRORY) { // Mirror Y
...
//  ObjectRecord *Object,*Object2;
  ...
  for (cnt=0;cnt<Design.NrInstances;cnt++) {
    ...
    if (((Instance->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
      ...
      PlacementMode=0;
      if (Instance->SymbolInfo & OBJECT_ROTATE90) PlacementMode|=4;
      if (Instance->SymbolInfo & OBJECT_MIRRORX)  PlacementMode|=1;
      if (Instance->SymbolInfo & OBJECT_MIRRORY)  PlacementMode|=2;
...

{
  ...
  for (cnt=0;cnt<Design.NrInstances;cnt++) {
    ...
    if (((Instance->Info & (Mask)) == Value)
      ...
      while ((cnt2>0)
            &&
            (isdigit(NewInstance.Reference[cnt2]))) {
