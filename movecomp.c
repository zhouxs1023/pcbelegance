...
double  CentreSelectedX,CentreSelectedY,CurrentX2,CurrentY2,
...

{
  ...
  for (cnt=0;cnt<NrObjects;cnt++) {
    Object=&((*Objects)[cnt]);
    OkToCopy=0;
    if (((Object->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == 0)
       &&
       (ObjectLayerVisible(Object->Layer))) {
...

{
  ...
  double  OldX,OldY,CurrentX,CurrentY,hx,hy,divx,divy,ShiftX,ShiftY,
          x1,y1,CursorX,CursorY,divx2,divy2;
  HMENU  PopUpMenu;
...
//  AppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_POPUP_ESCAPE,SC(394,"Escape"));
...
//    if (CheckRightButton(&DrawXorFunction)==1) {
    if (!Focused) {
      ...
      CheckInputMessages(0);
      ClipMouseCursor();
      if (!SelectionEsc) DrawSelectedObjects(CurrentX,CurrentY,1);
      else CompPlaced=1;
    }
    if ((ZoomActive())
       &&
...
//  MovingObjects=Mode;
  ...
  for (cnt=0;cnt<NrObjects;cnt++) {
    ...
    if ((Object->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
      ...
      if (ClearanceVisible) {
        DrawObjectWithClearance(Object,CurrentX-CurrentX2-ShiftOffsetX,CurrentY-CurrentY2-ShiftOffsetY,4);
      }
      switch (Object->ObjectType) {
        case OBJECT_LINE:
        case OBJECT_TEXT:
...
//  MovingObjects=0;
  for (cnt=0;cnt<NrObjectPolygons;cnt++) {
    ObjectPolygon=(ObjectPolygonRecord *)&(ObjectPolygonMem[(*ObjectPolygons)[cnt]]);
    if ((ObjectPolygon->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
      DrawObjectPolygon(ObjectPolygon,CurrentX-CurrentX2-ShiftOffsetX,CurrentY-CurrentY2-ShiftOffsetY,4);
    }
  }
  DrawCrossHair(16+8);
/*
...
*/
  ...
  for (cnt2=0;cnt2<2;cnt2++) {
    for (cnt=0;cnt<NrObjects;cnt++) {
      ...
      if (((Object->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
         ...
         &&
         ((Mode==0) || (Object->Layer!=GEOM_NAME_LAYER))) {
        if (((cnt2==0)
           &&
           ...
           ||
           (Object->Layer==DRILL_UNPLATED_LAYER)))) {
          if (Mode==0) Object->Info&=~OBJECT_SELECTED;
          ...
          if (Mode==1) NewObject.Info&=~OBJECT_SELECTED;
  ...
  for (cnt=0;cnt<NrObjectPolygons;cnt++) {
    ...
    if (((ObjectPolygon->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
      ...
      if (Mode==0) ObjectPolygon->Info&=~OBJECT_SELECTED;
      ...
      if (Mode==1) NewObjectPolygon->Info&=~OBJECT_SELECTED;
...
//  ObjectRecord *Object,*Object2;
  int32  cnt,cnt2,cnt3,TempLastActionNr,ModeCopy,count,
         CircleMode,Mirror,ObjectPolygonLength,Changed;
  ...
  if ((Mode & 32) == 32) {
    ...
    if (TextInputDialog(&ChangedTextObject,0x14)!=1) return;
      ...
      switch (Object->ObjectType) {
        ...
        case OBJECT_RECT:
          if ((ModeCopy & 32) == 0) {
            ...
            if (Object->Thickness==0.0) { // Filled object
              ...
              for (cnt2=0;cnt2<NewObjectPolygon->NrVertices;cnt2++) {
                RotatePointFromOtherPoint2(&NewObjectPolygon->Points[cnt2].x,
                                           &NewObjectPolygon->Points[cnt2].y,CentreX,CentreY,RotationAngle);
              }
              NewObjectPolygon->DeleteNr=0;
...
*/
        case OBJECT_ARC:
          if ((ModeCopy & 32) == 0) {
            ...
            if ((Object->Thickness!=0.0)
               ||
               (Object->Layer==PLACEMENT_OUTLINE_LAYER)) {
  ...
  for (cnt=0;cnt<NrObjectPolygons;cnt++) {
    ...
    if (((ObjectPolygon->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
      ...
      for (cnt2=0;cnt2<count;cnt2++) {
        if ((ModeCopy & 32) == 0) {
          RotateFlipPoint2(&NewObjectPolygon->Points[cnt2].x,
                           &NewObjectPolygon->Points[cnt2].y,CentreX,CentreY,Mode);
        } else {
          RotatePointFromOtherPoint2(&NewObjectPolygon->Points[cnt2].x,
                                     &NewObjectPolygon->Points[cnt2].y,CentreX,CentreY,RotationAngle);
        }
      ...
      if (NewObjectPolygon->NrSubPolygons>0) {
        ...
        for (cnt2=0;cnt2<NewObjectPolygon->NrSubPolygons;cnt2++) {
          if ((Buf-(uint8 *)NewObjectPolygon)>=(int32)(ObjectPolygonLength-sizeof(ObjectSubPolygonInitRecord))) {
            break;
          }
          ...
          for (cnt3=0;cnt3<count;cnt3++) {
            if ((ModeCopy & 32) == 0) {
              RotateFlipPoint2(&ObjectSubPolygon->Points[cnt3].x,
                               &ObjectSubPolygon->Points[cnt3].y,CentreX,CentreY,Mode);
            } else {
              RotatePointFromOtherPoint2(&ObjectSubPolygon->Points[cnt3].x,
                                         &ObjectSubPolygon->Points[cnt3].y,CentreX,CentreY,RotationAngle);
            }
          }
...

{
  ...
  while ((cnt<NrPinObjects)
    ...
    if (stricmpUTF8((LPSTR)PinInfo->PinText,(LPSTR)ObjectText->Text)==0) Found=cnt;
    cnt++;
  }
  if (Found==-1) {
...

{
  ...
  if (LineInputDialog(&TypeObject,SC(309,"Change text height"))!=1) return;
  if ((sscanf(TypeObject.Text,"%f",&value))!=1) return;
  switch (Units) {
    case 0:
...

{
  ...
  switch (mode) {
    case 0:
      ...
      if (LineInputDialog(&TypeObject,SC(397,"Change circle diameter"))!=1) return -1;
      if ((NrParams=ScanParameters(-1,TypeObject.Text,0))!=1) return -1;
      Width=ParamsFloat[0];
    ...
    case 1:
      ...
      if (LineInputDialog(&TypeObject,SC(398,"Change rectangle width,height"))!=1) return -1;
      if ((NrParams=ScanParameters(-1,TypeObject.Text,0))!=2) return -1;
      Width=ParamsFloat[0];
    ...
    case 2:
      ...
      if (LineInputDialog(&TypeObject,SC(399,"Change arc diameter"))!=1) return -1;
      if ((NrParams=ScanParameters(-1,TypeObject.Text,0))!=1) return -1;
      Width=ParamsFloat[0];
      if (Width<10) return -1;
      break;
    case 3:
      ...
      if (LineInputDialog(&TypeObject,SC(400,"Change angles arc (Degrees counter clock wise)"))!=1) return -1;
      if ((NrParams=ScanParameters(-1,TypeObject.Text,0))!=2) return -1;
  ...
  for (cnt=0;cnt<NrObjects;cnt++) {
    ...
    if ((Object->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
      if ((Object->Layer==Layer2)
        ...
        MessageBoxUTF8(GEOMWindow,str2,SC(1,"Error"),MB_APPLMODAL|MB_OK);
        return;
      }
    }
  }
...

{
  ...
  if (TextInputDialog(&ChangedTextObject,0x113)!=1) return;
  sscanf(ChangedTextObject.Text,"%f",&Scale);
  if ((Scale<0.0001)
     ||
     (Scale>10000.0)) return;
  TempLastActionNr=LastActionNr-1;
  Changed=0;
...

{
  ...
  double x1,y1,px1,py1,px2,py2,AddX,AddY,Length1,Length2,TempLength,
        FoundObjectsAddX[256],FoundObjectsAddY[256],Length,FoundObjectsLength[256];
...
//            return 1;
  ...
  //          return 1;
        ...
        case OBJECT_CIRCLE:
#ifdef _DEBUG
          if ((InRange5(Object->x1,-7e5))
             &&
             (InRange5(Object->y1,-5.7e5))) {
            ok=1;
          }
#endif
          if (((Object->Thickness==0.0)
...

{
  ...
  while (cnt<NrLinePoints) {
    if ((InRange4(x,(*LinePoints)[cnt].x))
       &&
       (InRange4(y,(*LinePoints)[cnt].y))) {
      return cnt;
    }
    cnt++;
  }
...

{
  ...
  int32  cnt,cnt2,NrLinePointsNumbers,NrVertices,Layer,PinNr,
         Found,Found2,PolygonError;
  ...
  switch (Layer) {
    ...
    case GEOM_NAME_LAYER:
    case BOARD_OUTLINE_LAYER:
      MessageBoxUTF8(GEOMWindow,SC(403,"Can not create a polygon on this layer"),SC(404,"error"),MB_APPLMODAL|MB_OK);
      return -1;
      break;
  }
  for (cnt=0;cnt<NrObjects;cnt++) {
...
//  LinePointsNumbers=
  ...
  for (cnt=0;cnt<NrLinePoints;cnt++) {
    ...
    if (LinePoint->NrLines!=2) {
      ok=1;
      MessageBoxUTF8(GEOMWindow,SC(405,"Did not found a closed polygon"),SC(404,"error"),MB_APPLMODAL|MB_OK);
      return -1;
    }
  }
...
//      NrVertices++;
    ...
    while ((NotInRange4(x1,x2))
      ...
      while ((cnt2<NrObjects2)
        ...
        if ((Object2->Info & 1)==0) {
          if ((InRange4(x2,Object2->x1))
             &&
             (InRange4(y2,Object2->y1))) {
            Found2=1;
            x2=Object2->x2;
            y2=Object2->y2;
            if (NrVertices<512) {
            ...
            if ((InRange4(x2,Object2->x2))
               &&
               (InRange4(y2,Object2->y2))) {
              Found2=1;
              x2=Object2->x1;
              y2=Object2->y1;
              if (NrVertices<512) {
  ...
  if (!CheckNoCrossesInObjectPolygon(NewObjectPolygon)) {
    MessageBoxUTF8(GEOMWindow,SC(406,"Polygon contains line crossings"),SC(404,"error"),MB_APPLMODAL|MB_OK);
  }
  for (cnt=0;cnt<NrObjects;cnt++) {
    Object=&((*Objects)[cnt]);
    if (((Object->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
