...
typedef struct {
          int32 GlobalPowerNetNr,NrPowerNames,
                PowerNetNrs[8];
        } PowerNetNrRecord;
...
int32  NrGlobalPowerNetNrs,NrPinsForLicenseCheck,NrSpecialPowerNetNames,
       UnNamedGlobalNetNr;
...
//  SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
  ...
  for (cnt2=0;cnt2<NrSheets;cnt2++) {
    ...
    if (LoadSheetInMemory(cnt2,0)<0) return -2;
...

{
  if ((NrInt32Objects1+1>=MaxNrInt32Objects1)
     &&
     (AllocateMemObjectsPosition1(MaxNrInt32Objects1+128)!=0)) return -1;
...
{
  if ((NrObjectsInfo+1>=MaxNrObjectsInfo)
     &&
     (AllocateMemObjectsInfo(MaxNrObjectsInfo+128)!=0)) return -1;
...
{
  int32  res,cnt,cnt2,start,stop,SheetNr,
         NrObjectPinBusses,TotalNrPins;
  char   FileName[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING];
  Object2Record *Object2;


//  SendMessage(EditWindow,EM_SETSEL,(WPARAM)-1,(LPARAM)10000000);
...
//  if (CheckERC()) return -1;
  ...
  AddMessage(str);
//  SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)(LPSTR)str);
  if (CreateGatePinSwapInfo(0)<0) {
    return -1;
  }
  strcpy(str,SC(177,"Output to netlist/component is ready\r\n"));
  AddMessage(str);
//  SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)(LPSTR)SC(177,"Output to netlist/component is ready\r\n"));
...

{
  ...
  char   FileName[MAX_LENGTH_STRING],*StrPos,Line[MAX_LENGTH_STRING],
         Line2[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],SheetStr[MAX_LENGTH_STRING];
  Object2Record *Object2;

  sprintf(str,"%s\\nets.net",DesignPath);
  if ((fp=FileOpenWriteUTF8(str))<=0) return -1;
  ShCnt=0;
  for (cnt=0;cnt<TotalNrNets;cnt++) {
    ...
    if (cnt<PowerPinNetPos) {
      ...
      sprintf(Line,"%sNET %i %s ITEMS %i    %08X",SheetStr,cnt,GetObjectText(Object2->Text1),
                                                  stop-start,(*NetInfos)[cnt].Info);
      WriteLn(fp,Line);
    }
    for (cnt2=start;cnt2<stop;cnt2++) {
      Object2=&((*Objects2)[cnt2]);
      switch (Object2->ObjectType) {
        case SYMBOL_PIN:
          ...
          if (Object2->Info3==0) {
            sprintf(Line,"%sCOMP     %s PIN %s %s %s",SheetStr,GetObjectText(Object2->RefNum),
                    GetObjectText(Object2->Text1),GetObjectText(Object2->Text2),
                    Line2);
          } else {
        ...
        case SYMBOL_PINBUS:
          ...
          sprintf(Line,"%sCOMP     %s PINBUS %s %i %s %s",SheetStr,GetObjectText(Object2->RefNum),
                  GetObjectText(Object2->Text1),Object2->Info3,
        ...
        case BUS_CONNECTION:
          if (Object2->Info3==0) {
            sprintf(Line,"%sBUSCON   %i,%i TO NET %d",SheetStr,
...

{
  ...
  for (cnt2=0;cnt2<NrObjects2;cnt2++) {
    Object2=&((*Objects2)[cnt2]);
    if ((Object2->ObjectType==GLOBAL_CONNECTION)
       &&
       ((SheetNr=Object2->SheetNr)!=-1)) {
      ...
      if (SheetAbove>=0) {
        ...
        for (cnt=start;cnt<stop;cnt++) {
          ...
          if ((Object2a->ObjectType==SYMBOL_PIN)
             &&
             (Object2a->Info3==1)) { // sheet symbol pin
        ...
        if (count==0) {
          sprintf(Line,SC(178,"External connection %s (%i,%i) sheet %s.sch is not connected\r\n"),
                               GetObjectText(Object2->Text1),(int32)(Object2->x1),(int32)(Object2->y1),
                               SheetName);
          AddMessage(Line);
...
//          SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)Line);
        ...
        sprintf(Line,SC(179,"External connection %s (%i,%i) sheet %s.sch "),
                             GetObjectText(Object2->Text1),(int32)(Object2->x1),(int32)(Object2->y1),
                             SheetName);
...

{
  ...
  for (cnt=Pos;cnt<Pos+Sheets[SheetNr].NrNets;cnt++) {
    ...
    if ((Info & 0x100000) == 0x100000) {  // Power pin
      if (((Info & 0x40000) == 0x00000)   // Sheetpins=0
         &&
         ((Info & 0x100) == 0x000)) {     // pins=0
        start=(*NetInfos)[cnt].Pos;
        stop=(*NetInfos)[cnt+1].Pos;
        for (cnt2=start;cnt2<stop;cnt2++) {
          Object2=&((*Objects2)[cnt2]);
...
//            SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)Line2);
    ...
    if (((Info & 0xD3300) == 0x10000)
       ||
       ((Info & 0xD3300) == 0x11000)
       ||
       ((Info & 0xDB300) == 0x50000)
//       ||
...
//          SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)Line2);
    ...
    if (((Info & 0xE7C00) == 0x20400)
       ||
       ((Info & 0xE7C00) == 0x21000)
       ||
       ((Info & 0xE7C00) == 0x20000)
       ||
...

{
  ...
  if (mode==0) {
    for (cnt=0;cnt<TotalNrNets;cnt++) {
      Info=(*NetInfos)[cnt].Info;
      Info&=~0xff;
      if ((((*NetInfos)[cnt].Progress & 5) == 0)
         &&
         ((Info & 0x1000) == 0x1000)) return cnt;  // External connection
    }
  }
  return -1;
}
...
{
  int32  cnt,cnt2,cnt3,cnt4,start,stop,NetLabelObjectNr,
         NetNr,NetNr2,Info,StartNet,EndNet,NetCount,SheetNr2,GlobalConnObjectNr;
  char   GlobalNetName[MAX_LENGTH_STRING],Line[MAX_LENGTH_STRING],
         Line2[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING];
...
// This sheet netnr will be the top of the tree
    ...
    while ((!Stop)
          &&
          ((GlobalConnObjectNr=(*NetInfos)[NetNr].GlobalConnObjectNr)!=-1)) {
    ...
    if (cnt2<NrGlobalConnectionNetsStrings) {
      sprintf(Line,SC(251,"Error: There are two or more global nets with the same name (%s) who are not connected\r\n"),GlobalConnectionStr);
      AddMessage(Line);
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)Line);
    }
#ifdef _DEBUG
...
// Search for sheet pins on net NetNr
      ...
      if ((Info & 0x40000) == 0x40000) {  // ObjectSheetPins
// Sheet pins found
        ...
        for (cnt2=start;cnt2<stop;cnt2++) {
          ...
          if ((Object2b->ObjectType==SYMBOL_PIN)
             &&
             (Object2b->Info3==1)) {
...
#endif
    if (((Info & 0x2C000) == 0x2C000)
       &&
       (((*NetInfos)[cnt].Progress & 1) == 0)) {
      ok=1;
      if ((Info & 0x400) == 0) {
...
#ifdef _DEBUG
  sprintf(str,"%s\\global.net",DesignPath);
  if ((fp=FileOpenWriteUTF8(str))<=0) return 0;
#endif
...

{
  ...
  if ((BusConnectionObjectNr=(*NetInfos)[NetInfoNr].BusConnectionObjectNr)==-1) return 0;
  ...
  for (cnt3=GlobalNetsPos[GlobalNetIndex];cnt3<GlobalNetsPos[GlobalNetIndex+1];cnt3++) {
    ...
    if (BusConnectionObject->Info4==GlobalNetNr) return 1;
  }
  return 0;
}
...

{
  ...
  if ((NetLabelObjectNr=(*NetInfos)[NetNr].NetLabelObjectNr)==-1) return 0;
  NetLabelObject=&((*Objects2)[NetLabelObjectNr]);
  NetLabelNr=NetLabelObject->Text1;
  NetLabelName=GetObjectText(NetLabelNr);
  ...
  if (FoundNr!=-1) {
    for (cnt6=GlobalNetsPos[FoundNr];cnt6<GlobalNetsPos[FoundNr+1];cnt6++) {
      if ((GlobalNetNr2=GlobalNets[cnt6])!=NetNr) {
        if (((*NetInfos)[GlobalNetNr2].Info & 0x2000) == 0) {  // NrBusConnections1=0
          if (AddObjectsPosition1(GlobalNetNr2 << 16)==-1) return;
...
{
  int32  Changed=0;
  int32 cnt3,cnt4,cnt5,GlobalNetNr,NetLabelObjectNr2,BusConnectionObjectNr2,
        BusConnectionObjectNr,PinBusNetNr,
...
// ********************************************************************************
        ...
        if ((BusConnectionObject2->Info4==GlobalNetNr)
           &&
           (NetLabelObjectNr2!=NetLabelObjectNr)) {
...
//              WriteLn(fp,str);
              ...
              if (((Info & 0x40000) == 0x40000) // SheetPin
                 ||
                 ((Info & 0x1000) == 0x1000)) { // External connection
                CheckCascadedNetObjects(cnt4,NetLabelName,fp);
              }
              if (AddObjectsPosition1(cnt4 << 16)==-1) return -1;
...
//                    WriteLn(fp,str);
#endif
                    if (AddObjectsPosition1((cnt4 << 16)+cnt5)==-1) return -1;
...
// Check in busses
      if (((Info & 0x20400) == 0x20400)   // NrPinBusses>0 , NrBusses>0
        ...
        if ((BusConnectionObjectNr2==-1)
           ||
           (BusConnectionObject2->Info4==GlobalNetNr)) {
          if (strnicmpUTF8(PinBusStr,NetLabelName,1)==0) {
            for (cnt5=0;cnt5<(*NetInfos)[cnt4].NrPinsPinBus;cnt5++) {
              if ((GetLabelBusNameByIndex(PinBusStr,str,cnt5)!=-1)
                 &&
...
// ********************************************************************************
        ...
        if (NetLabelObjectNr2!=NetLabelObjectNr) {
          if (((*NetInfos)[cnt4].Info & 0x400) == 0) {  // NrPinBusses=0
            NetLabelStr=GetObjectText(NetLabelObject2->Text1);
            if (stricmpUTF8(NetLabelName,NetLabelStr)==0) {
#ifdef _DEBUG
              AddNetNr(fp,cnt4,0,0,0);
...
#endif
              if (AddObjectsPosition1(cnt4 << 16)==-1) return -1;
              if ((Info & 0x1000) == 0x1000) {   // External connection
                CheckCascadedNetObjects(cnt4,NetLabelName,fp);
              }
              (*NetInfos)[cnt4].Progress|=2;
            }
...
// ********************************************************************************
  if ((BusConnectionObjectNr=(*NetInfos)[NetNr].BusConnectionObjectNr)!=-1) {
    ...
    if ((PinBusNetNr!=-1)
       &&
       (((*NetInfos)[PinBusNetNr].Info & 0x400) == 0x400)) { // NrPinBusses>0
...
//            WriteLn(fp,str);
#endif
            if (AddObjectsPosition1((PinBusNetNr << 16)+cnt5)==-1) return -1;
...
{
  int32  cnt4,SheetNr2,StartNet,EndNet,Info,BusConnectionObjectNr,
         BusConnectionObjectNr2,NetLabelObjectNr2;
  Object2Record *BusConnectionObject,*BusConnectionObject2,*NetLabelObject2;
  LPSTR  NetLabelStr;
#ifdef _DEBUG
  int32 ok;
...
// ********************************************************************************
      ...
      BusConnectionObjectNr2=(*NetInfos)[cnt4].BusConnectionObjectNr;
      BusConnectionObject2=&((*Objects2)[BusConnectionObjectNr2]);
      if ((BusConnectionObject2->Info4==BusConnectionObject->Info4)
         &&
         (NetLabelObjectNr2!=NetLabelObjectNr)) {
        NetLabelStr=GetObjectText(NetLabelObject2->Text1);
        if ((stricmpUTF8(NetLabelName,NetLabelStr)==0)
...
//          WriteLn(fp,str);
#endif
          if (AddObjectsPosition1(cnt4 << 16)==-1) return;
          (*NetInfos)[cnt4].Progress|=2;
        }
      }
    }
...
{
  int32  cnt2,cnt3,cnt4,cnt5,cnt6,cnt7,start,stop,FoundNr,NetLabelObjectNr,Changed,NetNr,Info,
         SearchNetNr,result,StartNet,EndNet,NetLabelObjectNr2,GlobalConnObjectNr,PowerNetName,
         NetLabelNr,UnNamedNetNr,OldNrObjectsInfo,FoundNr2,GlobalNetNr2,fp,
         GlobalNetNumbers[100],NrGlobalNetNumbers,PowerObjectNr,SheetNr2,cnt3a,
         Evaluate,TempPowerNetName[MAX_LENGTH_STRING],NrTempPowerNetNames;
  char   str2[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],*NetLabelName,
         *PinBusStr,*PinBusStr2=NULL,*NetLabelStr,*NetLabelName2=NULL;
...
// ********************************************************************************
  ...
  for (NetNr=0;NetNr<TotalNrNets;NetNr++) {
    ...
    if ((Info & 0x10A000) == 0x10A000) {   // NrPowerPins>0 , NrNetLabels>0  , NrBusConnections1>0
      NetLabelObjectNr=(*NetInfos)[NetNr].NetLabelObjectNr;
      if (NetLabelObjectNr!=-1) {
        NetLabelObject=&((*Objects2)[NetLabelObjectNr]);
        ...
        for (cnt2=start;cnt2<stop;cnt2++) {
          ...
          if ((Object2->ObjectType==SYMBOL_PIN)
             &&
             (Object2->Info2==POWER_CONNECTION)) {
    ...
    if ((GlobalNetsInfo[cnt2] & 1)==1) {
      for (NetNr=0;NetNr<TotalNrNets;NetNr++) {
        ...
        if ((((*NetInfos)[NetNr].Progress & 3) == 0)
           &&
           ((Info & 0xA000) == 0xA000)    // NrBusConnections1>0 , NrNetLabels>0
           &&
...
// Busconnection1
  ...
  //            WriteLn(fp,str);
#endif
              if (AddObjectsPosition1(NetNr << 16)==-1) return -1;
              ...
              if (AddObjectsInfo(NetLabelNr,NrInt32Objects1,-1)==-1) return -1;
#ifdef _DEBUG
              WriteLn(fp,"END");
#endif
            }
...
//              Presence=(__int64)0;
              if ((GetLabelBusNameByIndex(PinBusStr2,str2,cnt6)!=-1)
                ...
                if (CheckSpecialPowerNetNames(str2)==-1) {
                  ...
                  if (AddObjectsPosition1((NetNr << 16)+cnt6)==-1) return -1;
                  result=FindAndInsertNets(NetNr,NetLabelName,cnt2,fp);
                  if (AddObjectsInfo(NetLabelNr,NrInt32Objects1,cnt6)==-1) return -1;
#ifdef _DEBUG
                  WriteLn(fp,"END");
#endif
                  if ((result & 1) == 1) {
...
// ********************************************************************************
      ...
      for (NetNr=0;NetNr<TotalNrNets;NetNr++) {
        ...
        if ((((*NetInfos)[NetNr].Progress & 3) == 0)
           &&
           (((Info & 0x68000) == 0x48000)    // NrSheetPins>0 , NrNetLabels>0  , NrBusses=0
           ||
           ((Info & 0x29000) == 0x9000))) {  // NrNetLabels>0 , NrGlobalConnections>0 , NrBusses=0
// ********************************************************************************
// ********************************************************************************
// Netlabel
// Pin
          if ((NetLabelObjectNr=(*NetInfos)[NetNr].NetLabelObjectNr)!=-1) {
            ...
            NetLabelNr=NetLabelObject->Text1;
            NetLabelName=GetObjectText(NetLabelNr);
            if ((Info & 0x100000) == 0x000000) {  // NrPowerPins = 0
#ifdef _DEBUG
              AddNetNr(fp,NetNr,NetLabelName,0,2);
#endif
              if (AddObjectsPosition1(NetNr << 16)==-1) return -1;
            ...
            if (FoundNr!=-1) {
              for (cnt6=GlobalNetsPos[FoundNr];cnt6<GlobalNetsPos[FoundNr+1];cnt6++) {
                if ((GlobalNetNr2=GlobalNets[cnt6])!=NetNr) {
                  if (((*NetInfos)[GlobalNetNr2].Info & 0x2000) == 0) { // NrBusConnections1 = 0
                    if ((Info & 0x100000) == 0x000000) {                // NrPowerPins = 0
                      if (AddObjectsPosition1(GlobalNetNr2 << 16)==-1) return -1;
                      (*NetInfos)[GlobalNetNr2].Progress|=2;
#ifdef _DEBUG
...
// ********************************************************************************
  ...
  for (NetNr=0;NetNr<TotalNrNets;NetNr++) {
    ...
    if ((((*NetInfos)[NetNr].Progress & 3) == 0)
       &&
       ((Info & 0x22400) == 0x20400)) {  // NrPinBusses>0 , NrBusses>0 , NrBusConnections1=0
      NetLabelObjectNr=(*NetInfos)[NetNr].NetLabelObjectNr;
...
// pin bus label name is in other pin bus connected to a external connection
                if (((*NetInfos)[cnt4].Info & 0x21400) == 0x21400) { // NrPinBusses>0 , NrBusses>0 , NrWires>0
                  NetLabelObjectNr2=(*NetInfos)[cnt4].NetLabelObjectNr;
...
// pin bus label name is in other pin bus connected to the same sheet
                if (((*NetInfos)[cnt4].Info & 0x21400) == 0x20400) { // NrPinBusses>0 , NrBusses>0 , NrWires=0
                  Changed=1;
                }
              }
#ifdef _DEBUG
...
// ********************************************************************************
  ...
  for (NetNr=0;NetNr<TotalNrNets;NetNr++) {
    ...
    if ((((*NetInfos)[NetNr].Progress & 3) == 0)
       &&
       ((Info & 0x300000) == 0x300000)) { // NrPowerPins>1
#ifdef _DEBUG
      if (NetNr==372) {
        ok=1;
      }
      ...
      if (NrTempPowerNetNames>0) {
        ...
        if (FoundNr==-1) {
          if (NrGlobalPowerNetNrs<128) {
            ...
            memmove(&PowerNetNrs[NrGlobalPowerNetNrs].PowerNetNrs,
                    &TempPowerNetName,NrTempPowerNetNames*sizeof(int32));
...
// ********************************************************************************
      if (FoundNr!=-1) {
        if (Info == 0x100000) {  // NrPowerPins=1
          start=(*NetInfos)[NetNr].Pos;
          Object2=&((*Objects2)[start]);
...
// ********************************************************************************
  for (NetNr=0;NetNr<TotalNrNets;NetNr++) {
    ...
    if ((((*NetInfos)[NetNr].Progress & 3) == 0)
       &&
       ((Info & 0x300000) == 0x100000)) {   // NrPowerPins=1
#ifdef _DEBUG
      if (NetNr==528) {
        ok=1;
      }
...
//    AddNetNr(fp,NetNr,NetLabelName,0,2);
    ...
    for (NetNr=0;NetNr<TotalNrNets;NetNr++) {
      ...
      cnt3a=-1;
      if (((*NetInfos)[NetNr].Progress & 3) == 0) {
        if ((Info & 0x100000) == 0x100000) { // NrPowerPins>0
          Evaluate=1;
        }
        if (((*NetInfos)[NetNr].Info & 0x20400) == 0x20400) { // NrPinBusses>0 , NrBusses>0
          if ((NetLabelObjectNr=(*NetInfos)[NetNr].NetLabelObjectNr)!=-1) {
// Pinbus
#ifdef _DEBUG
            if (NetNr==223) {
...
//                          SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
              ...
              } else {  // Normal power pins
// ********************************************************************************
#ifdef _DEBUG
                AddNetNr(fp,Object2->NetNr,0,0,0);
#endif
                if (AddObjectsPosition1(Object2->NetNr << 16)==-1) return -1;
...
// All signal names inside a pinbus are evaluated, so pinbus is evaluated
    ...
    if (AddObjectsInfo(PowerNetNrs[cnt2].GlobalPowerNetNr,NrInt32Objects1,-1)==-1) return -1;
#ifdef _DEBUG
    WriteLn(fp,"END");
#endif
  }
...
// ********************************************************************************
        ...
        if (AddObjectsInfo(Object2a->Text1,NrInt32Objects1,-1)==-1) return -1;
  ...
  for (cnt2=0;cnt2<NrPowerNetNames;cnt2++) {
    for (cnt=0;cnt<NrObjects3;cnt++) {
      ...
      if (Object3->Info4==cnt2) {
        if ((NrObjects2+1>=MaxNrObjects2)
           &&
           (AllocateMemObjects2(MaxNrObjects2+128)!=0)) return 0;
    ...
    if (TotalNrNets+1>=NetInfoSize) {
      if (AllocateMemNetInfo(TotalNrNets+128)!=0) return -1;
    }
...
{
  int32  cnt,cnt2,NetCount,start,stop,Info,NetNr,NetNr1,PinBusNr,
         SheetNr,SheetNr1,NrPins,NrNetLabels;
  LPSTR  NetLabelName;
...
//            SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)Line);
  ...
  for (cnt=0;cnt<NrObjectsInfo;cnt++) {
    ...
    for (cnt2=start;cnt2<stop;cnt2++) {
      ...
      if ((Info & 0x100) == 0x100) NrPins++;
      if ((Info & 0x200) == 0x200) NrPins++;
      if ((Info & 0x400) == 0x400) NrPins++;
      if ((Info & 0x800) == 0x800) NrPins++;
      ok=1;
    }
/*
    if ((NetWithNetLabel)
...

{
  ...
  int32   TotalError;
  char   Line[MAX_LENGTH_STRING],DirName[MAX_LENGTH_STRING],
         str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING];
...
//        SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)Line);
        ...
        if (FoundNr==-1) {
          sprintf(Line,SC(191,"Geometry %s not found for component %s\r\n"),
                                GetObjectText(Object4->Text1),RefName);
          AddMessage(Line);
//          SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)Line);
        }
      }
    ...
    for (cnt3=start2;cnt3<stop2;cnt3++) {
      ...
      switch (Object2->ObjectType) {
        case SYMBOL_PIN:
          ...
          if ((Object2->Info3==0)
             &&
             ((RefName[0]!=0) || (Object2->Info2!=POWER_CONNECTION))) {
            PinName=GetObjectText(Object2->Text1);
            sprintf(NetItemStr,"%s-%s",RefName,PinName);
            cnt=0;
            while ((cnt<NrNetItems)
                  &&
                  (stricmpUTF8(NetItemStr,(*NetItemsPerNet)[cnt])!=0)) cnt++;
            if (cnt==NrNetItems) {
...
{
  int32  cnt,cnt2,cnt3,NetCount,start,stop,start2,stop2,NetNr,
         PinBusNr,res,NrLetsPos,LetPos[2048],LengthLine,cnt5,Length,
         PinCount,NrPins,fp2,FirstOpen,result,
         num1,num2;
  LPSTR  NetLabelName,RefName,PinName,PinBusName,Net1,Net2;
  int32   OkToWrite,NetError;
  char   Letter,Line[MAX_LENGTH_STRING],Line2[MAX_LENGTH_STRING],Line3[MAX_LENGTH_STRING],
         PinName2[20],PropertyFileName[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],
         str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],
         LineBuf[MAX_LENGTH_STRING];
  Object2Record *Object2;
#ifdef _DEBUG
  LPSTR StrPos;
#endif
    ...
    if (Object2->NewNetNr==-1) {
      switch (Object2->ObjectType) {
        case SYMBOL_PIN:
          PinTypeToText(Object2->Info2,Line2);
          if (Object2->Info3==0) {
            sprintf(Line,"COMP %s PIN %s %s %s",GetObjectText(Object2->RefNum),
                    GetObjectText(Object2->Text1),GetObjectText(Object2->Text2),
                    Line2);
          } else {
        ...
        case SYMBOL_PINBUS:
          ...
          break;
        case SYMBOL_POWERPIN:
          sprintf(Line,"COMP %s POWERPINS %s",GetObjectText(Object2->RefNum),
                  GetObjectText(Object2->Text2));
          WriteLn(fp2,Line);
          break;
        case WIRE:
          sprintf(Line,"WIRE %i,%i - %i,%i",(int32)Object2->x1,(int32)Object2->y1,
                                                    (int32)Object2->x2,(int32)Object2->y2);
          WriteLn(fp2,Line);
          break;
        case BUS:
          sprintf(Line,"BUS %i,%i - %i,%i",(int32)Object2->x1,(int32)Object2->y1,
                                                   (int32)Object2->x2,(int32)Object2->y2);
          WriteLn(fp2,Line);
          break;
        case BUS_CONNECTION:
          if (Object2->Info3==0) {
            sprintf(Line,"BUSCON %i,%i TO NET %d",
                                 (int32)Object2->x1,(int32)Object2->y1,
                                 Object2->Info4);
          } else {
        ...
        case GLOBAL_CONNECTION:
          ...
          sprintf(Line,"GLOBAL %s %s NET %i",Line2,GetObjectText(Object2->Text1),
                               Object2->Info4);
          WriteLn(fp2,Line);
          break;
        case NET_LABEL:
...
{
  int32  cnt,cnt2,cnt3,MinNr,pos,pos2,LengthIdent,LengthValue,
         NrRefNums,MinObjectNr,LastNr,LastObject4Nr,NrProperties,
         LengthStr,NrRefCodes,fp,TotalError;
  LPSTR  RefName,GeomName,PartValue,InstanceAttrBuf,AttributeValues[64],
         RefName2,PartNr,AttributeIdents[64],ErrorRef;
  int16  RefNums[16384];
  InstanceRecord *Instance;
  uint8   PartInfo[16];
  char   Line[MAX_LENGTH_STRING],FileName[MAX_LENGTH_STRING],
         str[2048],str2[MAX_LENGTH_STRING],
         RefCodes[100][6];
    ...
    if (Object4->Properties!=-1) {
      ...
      for (cnt2=0;cnt2<NrObjects4;cnt2++) {
        if (cnt2!=cnt) {
          ...
          if ((stricmpUTF8(RefName,RefName2)==0)
             &&
             (Object4a->Properties==-1)) {
  ...
  for (cnt=0;cnt<NrObjects4;cnt++) {
    ...
    if (LengthStr>0) {
      cnt2=0;
      while ((cnt2<LengthStr)
            &&
            (isalpha(RefName[cnt2]))) cnt2++;
      memset(&str,0,100);
      strncpy(str,RefName,cnt2);
      cnt3=0;
      while ((cnt3<NrRefCodes)
            &&
            (stricmpUTF8(RefCodes[cnt3],str)!=0)) cnt3++;
      if (cnt3>=NrRefCodes) {
  ...
  sprintf(FileName,"%s\\pcb\\%s___.net",DesignPath,LayoutFile);
  sprintf(str2,"%s\\pcb\\backup\\%s.net",DesignPath,LayoutFile);
...
//          if (stricmp(GetObjectText(Object4->Text2),"POWER_CONNECT")!=0) {          // symbolname
          WriteLn(fp,str);
//          }
          if (Object4->Info3>0) { // Multiple parts
...
//            RefName2=GetObjectText(Object4a->RefNum);
          if ((stricmpUTF8(GetObjectText(Object4->Text2),          // symbolname
                           GetObjectText(Object4a->Text2))!=0)
             &&
...
//                  SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)Line);
                  ...
                  if ((Object4->Info3>0)
                     &&
                     (PartInfo[Object4->Info3]==1)) {
                    sprintf(Line,SC(198,"Error in package part nr for the same reference %s%c\r\n"),
...
//    CreateMentorGeometries(0);
    if (!WriteNetlist2(fp)) TotalError=1;
  }
  FileClose(fp);
  if (WriteLnError!=0) {
  ...
  if (TotalError) {
    ...
    sprintf(FileName,"%s\\pcb\\%s.net",DesignPath,LayoutFile);
    sprintf(str2,"%s\\pcb\\backup\\%s.net",DesignPath,LayoutFile);
    CopyFileUTF8(FileName,str2,0);
    sprintf(str2,"%s\\pcb\\%s___.net",DesignPath,LayoutFile);
...
{
  int32  cnt,cnt2,cnt3,cnt4,cnt5,stop,start,
         start2,stop2,FoundNr,NetNr,NetNr3,NetNr4,Info,BusNetNr,
         Objects5Written,PreviousNetNr,Object5WireNr,
         StartNet,EndNet,BusConnectionObjectNr2,NetLabelObjectNr2,WireCount,BusCount,
         PinBusCount,WireNetNr,Object5BusPos,Object5BusNr,PreviousBusNr,
         SheetNr2;
...
#ifdef _DEBUG
      sprintf(Line,"*************************** BUS %i %s **********************",
              Object5BusNr,GlobalNetName);
      WriteLn(fp,Line);
#endif
      for (cnt3=GlobalNetsPos[cnt2];cnt3<GlobalNetsPos[cnt2+1];cnt3++) {
          ...
          switch (Object2->ObjectType) {
            case BUS:
              if ((NrObjects5+1>=MaxNrObjects5)
                 &&
                 (AllocateMemObjects5(MaxNrObjects5+128)!=0)) return 0;
      ...
      if ((Info & 0x20400) == 0x20400) {
...
#ifdef _DEBUG
          sprintf(Line,"***************** PINBUS %i %s ***********************",
                  Object5BusNr,NetLabelName);
#endif
        } else {
#ifdef _DEBUG
          sprintf(Line,"***************** PINBUS %i ***********************",
                  Object5BusNr);
          WriteLn(fp,Line);
#endif
        }
        ...
        for (cnt2=start;cnt2<stop;cnt2++) {
          ...
          if (Object2->ObjectType==BUS) {
            ...
            NrObjects5++;
#ifdef _DEBUG
            sprintf(Line,"PINBUS  %s %i,%i - %i,%i      %s",NetLabelName,
                                 (int32)Object2->x1,(int32)Object2->y1,
                                 (int32)Object2->x2,(int32)Object2->y2,
                                 Sheets[Object2->SheetNr].SheetName);
            WriteLn(fp,Line);
...
//    strcpy(FileName,"c:\\design\\");
    ...
    for (cnt3=0;cnt3<Object5BusNr;cnt3++) {
      ...
      for (cnt=0;cnt<Object5WireNr;cnt++) {
        ...
        if ((Object5->BusNr==cnt3)
           &&
           (((OkToWrite) && (Object5->SheetNr==-1))
           ||
           (Object5->SheetNr==cnt2))) {
          Object5->FirstObjectNr=Object5BusPos;
          FileWrite(fp,Object5,sizeof(Object5Record),&result);
#ifdef _DEBUG
          sprintf(str3,"BUS  %3.0f ,%3.0f  - %3.0f ,%3.0f    %i",
                  Object5->x1,
                  Object5->y1,
                  Object5->x2,
                  Object5->y2,
    ...
    for (cnt=Object5WireNr;cnt<NrObjects5;cnt++) {
      ...
      if (Object5->SheetNr==cnt2) {
        ...
        FileWrite(fp,Object5,sizeof(Object5Record),&result);
#ifdef _DEBUG
        sprintf(str3,"WIRE %3.0f ,%3.0f  - %3.0f ,%3.0f    %i",
                Object5->x1,
                Object5->y1,
                Object5->x2,
                Object5->y2,
...
{
  int32  cnt,cnt2,cnt3,NetCount,start,stop,start2,stop2,NetNr,PinBusNr,
         OutputPins[256],PowerPins[256],LengthLine,
         cnt5,PinCount,NrOutputPins,NrPowerPins,NrInputPins;
  LPSTR  NetLabelName,RefName,PinName,PinBusName;
  int32   OkToWrite;
  char   Line[MAX_LENGTH_STRING],Line2[MAX_LENGTH_STRING],Line3[MAX_LENGTH_STRING],
         PinName2[MAX_LENGTH_STRING];
  Object2Record *Object2;
