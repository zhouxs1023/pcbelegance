...

{
  ...
  switch (Object2->ObjectType) {
    ...
    case SYMBOL_PIN:
      ...
      if (PinObject->Info3==1) { // sheet symbol pin
        ok=1;
      }
      if (PinObject->Info3==1) { // sheet symbol pin
...
//        s2=GetObjectText(PinObject->Text2);
      ...
      if (PinObject->Info3==0) { // symbol pin
        ...
        if ((RedefinedPinBus->Info & OBJECT_NOT_VISIBLE) == 0) {
          if ((stricmpUTF8(RedefinedPinBus->Reference,Instance->Reference)==0)
            ...
            for (cnt4=0;cnt4<PinBusObject->Info3;cnt4++) {
              res=GetPinNameFromPinBus(PinBusObject->Text1,str2,PinBusObject->Info3,
                                       RedefinedPinBus->Order[cnt4]);
              if (res>LineNr) strcat(NewPinBusStr,"\\");
              else
                if (cnt4>0) strcat(NewPinBusStr,",");
...
#ifdef _DEBUG
  if (((InRange(152.0,Wire->X1))
     &&
     (InRange(95.0,Wire->Y1)))
     &&
     ((InRange(152.0,Wire->X2))
     &&
     (InRange(78.0,Wire->Y2)))) {
    ok=1;
  }
#endif
...

{
  ...
  if ((NrObjects2+1>=MaxNrObjects2)
     ...
     (AllocateMemObjects2(MaxNrObjects2+128)!=0)) return 0;
#ifdef _DEBUG
  if ((InRange(PinObject->x1,125.0))
     &&
     (InRange(PinObject->y1,55.0))) {
    ok=1;
  }
  if (NrObjects2==195) {
    ok=1;
...

{
  ...
  for (cnt3=0;cnt3<Design.NrWires;cnt3++) {
    ...
    if ((Wire->Info & (OBJECT_NOT_VISIBLE|OBJECT_DONE)) == 0) {
      if (((InRange(x1,Wire->X1))
         ...
         &&
         (InRange(y1,Wire->Y2)))) {
        if (!WireToObject(Wire,cnt3)) return -1;
      } else {
    ...
    for (cnt3=0;cnt3<Design.NrBusses;cnt3++) {
      ...
      if ((Bus->Info & (OBJECT_NOT_VISIBLE|OBJECT_DONE)) == 0) {
        if (((InRange(x1,Bus->X1))
           ...
           &&
           (InRange(y1,Bus->Y2)))) {
          if (!BusToObject(Bus,cnt3)) return -1;
        }
      }
    }
  }
  ...
  for (cnt=0;cnt<Design.NrInstances;cnt++) {
    ...
    for (cnt2=start;cnt2<stop;cnt2++) {
      ...
      if ((Object!=PinObject)
         ...
         &&
         ((Object->Info & OBJECT_DONE) == 0)
         &&
         (InRange(Object->x1,x1))
         &&
         (InRange(Object->y1,y1))) {
        if (!PinToObject(Object,cnt2)) return -1;
  ...
  for (cnt4=0;cnt4<Design.NrOnePinNets;cnt4++) {
    ...
    if (((OnePinNet->Info & (OBJECT_NOT_VISIBLE)) == 0)
       &&
       (InRange(OnePinNet->X,x1))
       &&
       (InRange(OnePinNet->Y,y1))) {
      OnePinNet->Info|=OBJECT_DONE;
      if (!OnePinNetToObject(OnePinNet,cnt4)) return -1;
    }
  }
  return 0;
}
...

{
  ...
  for (cnt3=0;cnt3<Design.NrWires;cnt3++) {
    ...
    if ((Wire->Info & (OBJECT_NOT_VISIBLE|OBJECT_DONE)) == 0) {
      if (((InRange(x1,x3)) && (InRange(y1,y3)))
         ||
         ((InRange(x1,x4)) && (InRange(y1,y4)))
         ||
         ((InRange(x2,x3)) && (InRange(y2,y3)))
         ||
         ((InRange(x2,x4)) && (InRange(y2,y4)))) {
        if (!WireToObject(Wire,cnt3)) return -1;
      } else {
        CheckJunction=0;
        x5=0.0;
  ...
  for (cnt3=0;cnt3<Design.NrGlobalConnections;cnt3++) {
    ...
    if ((GlobalConnection->Info & (OBJECT_NOT_VISIBLE|OBJECT_DONE)) == 0) {
      if (((InRange(x1,GlobalConnection->X))
         &&
         (InRange(y1,GlobalConnection->Y)))
         ||
         ((InRange(x2,GlobalConnection->X))
         &&
         (InRange(y2,GlobalConnection->Y)))) {
        if (!GlobalConnectionToObject(GlobalConnection,cnt3)) return -1;
  ...
  for (cnt3=0;cnt3<Design.NrBusConnections;cnt3++) {
    ...
    if ((BusConnection->Info & (OBJECT_NOT_VISIBLE|OBJECT_DONE)) == 0) {
      if (((InRange(x1,BusConnection->X))
         &&
         (InRange(y1,BusConnection->Y)))
         ||
         ((InRange(x2,BusConnection->X))
         &&
         (InRange(y2,BusConnection->Y)))) {
        if (!BusConnectionToObject(BusConnection,cnt3,0)) return -1;
  ...
  for (cnt3=0;cnt3<Design.NrNetLabels;cnt3++) {
    ...
    if ((NetLabel->Info & (OBJECT_NOT_VISIBLE|OBJECT_DONE)) == 0) {
      if (((InRange(x1,NetLabel->ConnectX))
         &&
         (InRange(y1,NetLabel->ConnectY)))
         ||
         ((InRange(x2,NetLabel->ConnectX))
         &&
         (InRange(y2,NetLabel->ConnectY)))) {
        if (!NetLabelToObject(NetLabel,cnt3)) return -1;
  ...
  for (cnt=0;cnt<Design.NrInstances;cnt++) {
    ...
    for (cnt2=start;cnt2<stop;cnt2++) {
      ...
      if ((Object->ObjectType==SYMBOL_PIN)
         &&
         ((Object->Info & OBJECT_DONE) == 0)) {
        if (((InRange(Object->x1,x1))
           &&
           (InRange(Object->y1,y1)))
           ||
           ((InRange(Object->x1,x2))
           &&
           (InRange(Object->y1,y2)))) {
          Connected=1;
        } else {
          if (TestLineConnectedToCircle(x1,y1,x2,y2,Object->x1,Object->y1,0.04)) {
            if (stricmp(Design.Identification,SheetCode1)!=0) {
              for (cnt4=0;cnt4<Design.NrJunctions;cnt4++) {
                ...
                if (((Junction->Info & (OBJECT_NOT_VISIBLE)) == 0)
                   &&
                   (InRange(Junction->X,Object->x1))
                   &&
                   (InRange(Junction->Y,Object->y1))) {
                  Connected=1;
                }
              }
            } else {
...

{
  ...
  for (cnt3=0;cnt3<Design.NrBusses;cnt3++) {
    ...
    if ((Bus->Info & (OBJECT_NOT_VISIBLE|OBJECT_DONE)) == 0) {
      if (((InRange(x1,Bus->X1)) && (InRange(y1,Bus->Y1)))
         ...
         ||
         ((InRange(x2,Bus->X1)) && (InRange(y2,Bus->Y1)))
         ||
  ...
  for (cnt3=0;cnt3<Design.NrBusConnections;cnt3++) {
    ...
    if ((BusConnection->Info & (OBJECT_NOT_VISIBLE|OBJECT_DONE2)) == 0) {
      ...
      if (TestLineConnectedToCircle(x1,y1,x2,y2,x4,y4,0.04)) {
        if (!BusConnectionToObject(BusConnection,cnt3,1)) return -1;
    ...
    for (cnt2=start;cnt2<stop;cnt2++) {
      ...
      if ((Instance->Info & SHEET_SYMBOL) == SHEET_SYMBOL) {
        ...
        if ((Object->ObjectType==SYMBOL_PINBUS)
           ...
           &&
           (InRange(Object->y1,y2))))) {
          if (!PinBusToObject(Object,cnt2)) return -1;
          Object->Info|=OBJECT_DONE;
        }
      }
    }
...
{
  int32  cnt,cnt2,ObjectNr,StartObjectNr,Object2Count,StartObjectsSheet,ok,
         NrObjectNetLabels,NrObjectSheetPins,NrObjectPins;
  char   str2[200];
...
{
  int32  cnt,cnt2,cnt3,cnt4,res,start,stop,
         NrObjectPins,NrObjectSheetPins,NrObjectPinBusses,NrObjectWires,
         NrObjectBusses,NrObjectBusConnections1,NrObjectBusConnections2,ok,
         NrPinsPinBus,NrNets,NrObjectGlobalConnections,NrPowerPins,
         NrObjectNetLabels,start2,stop2,ObjectNr,GlobalConnectionType,
         GlobalConnX1,GlobalConnY1,BusConnX1,BusConnY1,BusConnX2,BusConnY2,
         PinBusX1,PinBusY1,
         FirstPinX,FirstPinY,FirstSheetPinX,FirstSheetPinY,PinType,
         NrObjectOnePinNets,
         GlobalConnectionError,TotalError,PowerPinConn,
         NetLabelTextX,NetLabelTextY,PowerNameText,BusX1,BusY1,BusX2,BusY2;
    ...
    for (cnt2=start;cnt2<stop;cnt2++) {
      ...
      switch (Object2->ObjectType) {
        ...
        case NET_LABEL:
          ...
          if ((NrObjectNetLabels>1)
             &&
             (stricmpUTF8(NetLabelText,GetObjectText(Object2->Text1))!=0)) {
            sprintf(str,SC(200,"%s: For the same net different labels (%s,%s)\r\n"),
                                FileName,NetLabelText,GetObjectText(Object2->Text1));
            AddMessage(str);
          ...
          if (NrObjectPinBusses>1) {
            if (NrPinsPinBus!=Object2->Info3) {
              sprintf(str,SC(201,"%s: Nr pins pin busses are not equal (%i,%i)\r\n"),
                                  FileName,(int32)Object2->x1,(int32)Object2->y1);
              AddMessage(str);
//              SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
              *x=Object2->x1;
              *y=Object2->y1;
        ...
        case GLOBAL_CONNECTION:
          ...
          if (NrObjectGlobalConnections==2) {
            ...
            sprintf(str,SC(203,"%s: Only one external connection %s allowed\r\n"),
                                FileName,GetObjectText(Object2->Text1));
            AddMessage(str);
//            SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
            TotalError=1;
            *x=Object2->x1;
...
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
    ...
    if ((NrObjectBusses>0)
      ...
      sprintf(str,SC(205,"%s: No label connected to a bus (%i,%i - %i,%i)\r\n"),
                          FileName,BusX1,BusY1,BusX2,BusY2);
      *x=BusX1;
      *y=BusY1;
      AddMessage(str);
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
...
*/
    ...
    if ((NrObjectGlobalConnections>0)
      ...
      sprintf(str,SC(207,"%s: Bus and external connections can not be combined (%i,%i)\r\n"),
                          FileName,GlobalConnX1,GlobalConnY1);
      AddMessage(str);
//      SendMessageUTF8(EditWindow,EM_REPLACESEL,0,(LPARAM)str);
      *x=GlobalConnX1;
      *y=GlobalConnY1;
