...
int32     NetSegmentIndex[MaxNrNetSegments],SegmentObjects[MaxNrNetSegments],
          NrNetConnections,ConnectionIndex,ObjectCount,CheckCount,
          BigNetInfoCheck,DrawOn,ok;
  ...
  for (cnt=0;cnt<Net->Count;cnt++) {
    ...
    if (ObjectArrayNr==2) {
      ...
      if (NrObjects2>=MaxNrObjects2-1) {
        if (AllocateMemObjects2(MaxNrObjects2+1024)==0) NrObjects2++;
      } else NrObjects2++;
    } else {
      ObjectNet=&((*Objects3)[NrObjects3]);
      memcpy(ObjectNet,Object4,sizeof(ObjectRecord));
      if (NrObjects3>=MaxNrObjects3-1) {
        if (AllocateMemObjects3(MaxNrObjects3+1024)==0) NrObjects3++;
      } else NrObjects3++;
    }
  }
}
...
//        }
      ...
      if (0) {
        for (cnt2=0;cnt2<NrObjects2;cnt2++) {
          ...
          FillPositionObject(Object);
          if ((Object->minx<CurrentX)
             &&
             (Object->maxx>CurrentX)
             &&
             (Object->miny<CurrentY)
             &&
...

{
      ...
      if ((Object->ObjectType!=CONNECTION)
        ...
        switch (Object->ObjectType) {
          ...
          case TRACE_VER:
            ...
            if ((InRange(x,x1))
               &&
               (InRange(y,y1+x2))) return cnt;
            break;
          case PIN_LINE_VER:
            if ((InRange(x,x1))
               &&
               (InRange(y,y1+x2a))) return cnt;
            if ((InRange(x,x1))
               &&
               (InRange(y,y1))) return cnt;
            if ((InRange(x,x1))
  ...
  for (cnt=0;cnt<NrObjects3;cnt++) {
    ...
    x2a=x2*0.5;
    y2=Object->y2;
    if (Layer==-1) Object2.Layer=Object->Layer;
    if (Object->ObjectType!=CONNECTION) {
      switch (Object->ObjectType) {
        case PIN_PUT_THROUGH_ROUND:
        case VIA_PUT_THROUGH_ROUND:
        ...
        case PIN_PUT_THROUGH_POLYGON:
          if (((mode & 1)==1)
             ...
             &&
             (InRange(y1,y))) {
            if (PutThroughCnt==-1) PutThroughCnt=cnt;
          }
          break;
        case PIN_SMD_ROUND:
        case PIN_SMD_RECT:
  ...
  if (PutThroughCnt!=-1) return PutThroughCnt;
  if ((mode & 8)==0) {
    if (FirstCnt!=-1) return FirstCnt;
  }
  ...
  for (cnt=0;cnt<NrObjects3;cnt++) {
    ...
    if ((Object->ObjectType!=CONNECTION)
       &&
       (ObjectsConnected(Object,&Object2))) {
      switch (Object->ObjectType) {
        case PIN_PUT_THROUGH_ROUND:
        case VIA_PUT_THROUGH_ROUND:
        case PIN_PUT_THROUGH_SQUARE:
...
//        case VIA_PUT_THROUGH_ROUND:
        ...
        case PIN_SMD_POLYGON:
        case PIN_PUT_THROUGH_POLYGON:
          if ((InRange3(Object->x1,Object2.x1))
             &&
             (InRange3(Object->y1,Object2.y1))) {
            return cnt;
          }
      }
    }
...

{
  ...
  while ((cnt<NrNetConnections)
    ...
    if ((*NetConnections)[cnt].Layer==Layer) {
      if (((InRange(x1,ConnectionX1))
         &&
         (InRange(y1,ConnectionY1))
         &&
         (InRange(x2,ConnectionX2))
         &&
         (InRange(y2,ConnectionY2)))
         ||
         ((InRange(x1,ConnectionX2))
         &&
         (InRange(y1,ConnectionY2))
         &&
         (InRange(x2,ConnectionX1))
         &&
         (InRange(y2,ConnectionY1)))) Found=1;
    }
    cnt++;
  }
  return Found;
...

{
  ...
  for (cnt=0;cnt<NrObjects2;cnt++) {
    ...
    if ((cnt!=ObjectNr)
       &&
       ((Object->Info & (OBJECT_NOT_VISIBLE|OBJECT_DONE)) == 0)) {
      CheckCount++;
#ifdef _DEBUG
      if ((InRange9(Object->x1,61.9e5))
         &&
         (InRange9(Object->y1,16.66e5))) {
        ok=1;
      ...
      if ((Object->minx==0.0)
         ||
         (CheckObject->minx==0.0)) {
        ok=1;
      }
#endif
      if ((Object->minx<CheckObject->maxx)
         &&
         (Object->miny<CheckObject->maxy)
         &&
         (Object->maxx>CheckObject->minx)
         &&
         (Object->maxy>CheckObject->miny)) {
#ifdef _DEBUG
        if ((InRange9(Object->x1,61.9e5))
           &&
           (InRange9(Object->y1,16.66e5))) {
        ...
        res=ObjectsConnected(CheckObject,Object);
          ...
          if ((InRangeSpecial(Object->x1,2.54e5,0.02e5))
             &&
             (InRangeSpecial(Object->y1,11.05e5,0.02e5))) {
            ok=1;
          }
          if ((InRangeSpecial(Object->x2,2.54e5,0.02e5))
             &&
             (InRangeSpecial(Object->y2,11.05e5,0.02e5))) {
            ok=1;
          }
        ...
        if ((res)
           &&
           (ObjectCount<MaxNrNetSegments)) {
          SegmentObjects[ObjectCount++]=cnt;
          Object->Info|=OBJECT_DONE;
#ifdef _DEBUG
          if ((InRange9(Object->x1,5.1e5))
             &&
             (InRange9(Object->y1,108.2e5))) {
            ok=1;
          }
          if (DrawOn) {
...
{
  double  ObjectMinLength,ObjectMin2Length,ObjectLength,ShortestObjectLength,
          FoundObjectX,FoundObjectY,
          ShortestObjectLengthLayer,CentreX,CentreY;
  int32  ok,SegmentCount,hulp,LayerGuide,FoundObjectNr,OldConnectionCount,
         NrMinConnections,ConnectionObject1,ConnectionObject2,
         NrNetSegments,Collections,NrMinConnectionsLayer,cnt,cnt2,cnt3,cnt4;
  ...
  Net=&((*Nets)[NetNr]);
  if (Net->Name[0]==0) return -3;
  if (((mode & 1) == 0)
     &&
     ((Net->Info & CONNECTIONS_DISABLED) == CONNECTIONS_DISABLED)) {
    ok=1;
...
#endif
//  GetObjectsNet(NetNr,2,mode & 1);

  if ((mode & 4)==0) {
    GetObjectsNet(NetNr,2,1);
//    sprintf(InfoStr,"%s      %i",Net->Name,NrObjects2);
//    RedrawInfoStr(1);
//    cnt7=0;
//    for (cnt6=0;cnt6<10000000;cnt6++) {
//      cnt7++;
//    }
  } else {
...
#ifdef _DEBUG
                    if ((InRange9(ConnectionX1,150.6e5))
                       &&
                       (InRange9(ConnectionY1,66.6e5))) {
                      ok=1;
                    }
                    if ((InRange9(ConnectionX2,150.6e5))
                       &&
                       (InRange9(ConnectionY2,66.6e5))) {
                      ok=1;
                    }
                    if ((InRangeSpecial(ConnectionX1,2.54e5,0.02e5))
                       &&
...
*/
          ...
          InsertConnection=1;
          if (NrMinConnections==0) InsertConnection=0;
          if (InsertConnection) {
    //        SelectedMinConnection=0;
            if (NrNetConnections+1>=MaxNrNetConnections) {
              if (AllocateMemNetConnections(NrNetConnections+128)!=0) return -3;
            }
            ...
            LayerGuide=-1;
            if (Object->Layer!=-1) LayerGuide=Object->Layer;
            Object->Test++;
            ...
            if (Object->Layer!=-1) {
              if (LayerGuide==-1) LayerGuide=Object->Layer;
              else
                if (Object->Layer!=LayerGuide) LayerGuide=-1;
            } else {
              if (LayerGuide==-1) LayerGuide=Design.NrBoardLayers;
            }
#ifdef _DEBUG
...
#endif
            if (!CheckConnection(LayerGuide,MinConnections[0].x1,MinConnections[0].y1,
                                            MinConnections[0].x2,MinConnections[0].y2)) {
              NrNetConnections++;
            }
          }
        }
...
//        CentreY=CentreY/(NrObjects2)/2;
        ...
        for (cnt=0;cnt<NrNetSegments;cnt++) {
          ...
          for (cnt2=NetSegmentIndex[cnt];cnt2<NetSegmentIndex[cnt+1];cnt2++) {
            ...
            if ((CheckObject->ObjectType==AREAFILL)
               ||
               (CentreObject.ObjectType==AREAFILL)) {
              ObjectLength=-1000000000.0;
            }
            if (ObjectLength<ShortestObjectLength) {
              ShortestObjectLength=ObjectLength;
...
//  ObjectsPos
    ...
    if (NrNetConnections==0) ConnectedAll=1;
    Collections++;
...
//  sprintf(InfoStr,"%s               %i",Net->Name,NrNetConnections);
//  RedrawInfoStr(1);
#ifdef _DEBUG
  if (0) {
  ...
  if (NrNetConnections==0) {
    if (OldConnectionCount>0) return 1;
    return 0;
  }
  if (OldConnectionCount>0) return 3;
  return 2;
}
...

{
  ...
  if ((NetNr<0)
     ||
     (NetNr>=Design.NrNets)) return;
  Net=&((*Nets)[NetNr]);
  if (Net->Name[0]==0) return;

  if ((Net->Info & (CONNECTIONS_DISABLED)) != 0) {
    if (mode==1) {
      MessageBoxOwn(PCBWindow,SC(874,"Net connectivity check is disabled"),SC(1,"Message"),MB_APPLMODAL|MB_OK);
    }
    return ;
  }
/*
  if ((Net->Info & (CONNECTIONS_NOT_VISIBLE|OBJECT_HIGHLITED)) == CONNECTIONS_NOT_VISIBLE) {
    if (mode==1) {
      MessageBoxOwn(PCBWindow,"Net connections are hided",SC(1,"Message"),MB_APPLMODAL|MB_OK);
    }
    return ;
  }
*/
  ...
  if ((res==2)
    ...
    if (mode==2) {
      MessageBoxOwn(PCBWindow,SC(875,"Connectivity error(s)"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
    }
  } else {
    if (mode==2) {
...

{
  ...
  for (Layer=0;Layer<32;Layer++) {
    for (cnt=0;cnt<Design.NrVerTraces[Layer];cnt++) {
      ...
      if ((Trace->NetNr==NetNr)
         &&
         (Trace->Info & OBJECT_NOT_VISIBLE) == 0) {
...

{
  ...
  ///      (*NetInfo)[cnt]=0;
    ...
    if (mode==0) {
      ...
      if (!NetError) {
        MessageBox(PCBWindow,SC(876,"Connectivity ok"),SC(1,"Message"),MB_APPLMODAL|MB_OK);
      } else {
        ViewFull();
...

{
  ...
  y=FirstObject.y1;
  GuideMode=0;
  if ((x<Design.BoardOriginX)
     ||
     (x>Design.BoardOriginX+Design.BoardWidth)
     ||
...
/*
  ...
  InitDrawingSwappablePins(2);
  DrawSpecialObject(&FirstObject,0,1);
  InitDrawingSwappablePins(0);
  DrawSpecialObject(&FirstObject,0,0);
  ExitDrawing();
  EndDrawingEditingWindow();
  Net=&((*Nets)[NetNr2]);
  sprintf(InfoStr,SC(140,"Routing traces %s"),Net->Name);
*/
  return 0;
}
...
// *******************************************************************************************************
    if (NotInRange(Design.StandardClearance,NewDesign.StandardClearance)) {
      if (Units==0) {
        sprintf(str,SC(803,"Do you want to change the clearance of all the traces/vias/areafills and nets to %.1f mils\r\n\r\n"),NewDesign.StandardClearance/2540.0);
      } else {
        sprintf(str,SC(804,"Do you want to change the clearance of all the traces/vias/areafills and nets to %.4f mm \r\n\r\n"),NewDesign.StandardClearance/100000.0);
      }
...
// *******************************************************************************************************
    if (NotInRange(Design.SilkScreenWidth,NewDesign.SilkScreenWidth)) {
      if (Units==0) {
        ...
        sprintf(str,SC(806,"Do you want to change the width of all silkscreen lines to %.4f mm\r\n\r\n"),NewDesign.SilkScreenWidth/100000.0);
      }
      strcat(str,SC(179,"This operation can not be undone"));
...
// *******************************************************************************************************
    if (NotInRange(Design.BoardOutlineWidth,NewDesign.BoardOutlineWidth)) {
      if (Units==0) {
        ...
        sprintf(str,SC(808,"Do you want to change the width of all board outline lines to %.4f mm\r\n\r\n"),NewDesign.BoardOutlineWidth/100000.0);
      }
      strcat(str,SC(179,"This operation can not be undone"));
...
// *******************************************************************************************************
    if (NotInRange(Design.StandardTraceWidth,NewDesign.StandardTraceWidth)) {
      if (Units==0) {
        ...
        sprintf(str,SC(810,"Do you want to change the width of all the traces and nets to %.4f mm\r\n\r\n"),NewDesign.StandardTraceWidth/100000.0);
      }
      strcat(str,SC(179,"This operation can not be undone"));
      DataBaseChanged=1;
