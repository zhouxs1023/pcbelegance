...
int32  ok,NrTempOdbLayers,TempOdbLayers[64],NrOdbLayers,
       OdbExportUnits,TempUnits,TempUnits2,
       OdbLayersForExport[64],OdbLayersForExport2[64],
       NrOdbExportLayers2,OdbConvertLayer[64],
       PowerPlaneLayers[64],NrOdbExportLayers,
       OdbLayerIndex[64],OdbPrepareErrors;
char   OdbDir[MAX_LENGTH_STRING],OdbFile[MAX_LENGTH_STRING],
       MatrixFile[MAX_LENGTH_STRING];
...
#endif
      cnt2=SendDlgItemMessage(Dialog,IDC_LIST1,LB_ADDSTRING,0,(LPARAM)str);
      if (cnt2>=0) {
        res=SendDlgItemMessage(Dialog,IDC_LIST1,LB_SETITEMDATA,cnt2,(LPARAM)Layer);
      }
      ok=1;
    }
  }
...
#endif
      cnt2=SendDlgItemMessage(Dialog,IDC_LIST2,LB_ADDSTRING,0,(LPARAM)str);
      if (cnt2>=0) {
        SendDlgItemMessage(Dialog,IDC_LIST2,LB_SETITEMDATA,cnt2,(LPARAM)Layer);
      }
    }
  }
...
{
  int32   about,cnt,cnt2,res,Layer,AllLayers[64],TabStops[10],
          Mirror,NrTopComps,NrBottomComps;
...
// ************************************************************************************************
      ...
      SetDialogItemText(Dialog,IDD_UNITS             ,SC(1307,"Inch/mm"));
      SetDialogItemText(Dialog,IDC_STATIC1           ,SC(960,"Layer name"));
      SetDialogItemText(Dialog,IDC_STATIC2           ,SC(1308,"ODB++ layers for export"));
      SetDialogItemText(Dialog,IDC_STATIC3           ,SC(1309,"Available ODB++ layers"));
      SetDialogItemText(Dialog,IDC_STATIC4           ,SC(966,"Project name"));
      SetDialogItemText(Dialog,IDC_STATIC5           ,SC(1085,"Units"));
      SetDialogItemText(Dialog,IDC_STATIC6           ,SC(267,"Options"));
      SetDialogItemText(Dialog,IDC_STATIC7           ,SC(367,"Filename"));

      SendDlgItemMessage(Dialog,IDC_BUTTON1,BM_SETIMAGE,IMAGE_BITMAP,(LPARAM)BitMapRight);
      SendDlgItemMessage(Dialog,IDC_BUTTON2,BM_SETIMAGE,IMAGE_BITMAP,(LPARAM)BitMapLeft);
      TabStops[0]=150;
      SendDlgItemMessage(Dialog,IDC_LIST2,LB_SETTABSTOPS,1,(LPARAM)(LPINT)&TabStops);
      ...
      sprintf(OdbFile,"%s.tgz",str);
      SendDlgItemMessage(Dialog,IDC_EDIT1,WM_SETTEXT,0,(LPARAM)OdbFile);
...
// ************************************************************************************************
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDOK:
          res=SendDlgItemMessage(Dialog,IDC_EDIT1,WM_GETTEXT,200,(LPARAM)str);
          if (res==0) {
            MessageBoxOwn(PCBWindow,SC(962,"Missing odb export filename"),SC(199,"Error"),MB_APPLMODAL|MB_OK);
            break;
          }
        ...
        case IDC_BUTTON1: // Add layer
          ...
          if (res>=0) {
            Layer=SendDlgItemMessage(Dialog,IDC_LIST1,LB_GETITEMDATA,res,0);
            if (Layer) {
              for (cnt=0;cnt<NrOdbExportLayers;cnt++) {
        ...
        case IDC_BUTTON2: // remove layer
          ...
          if (res>=0) {
            Layer=SendDlgItemMessage(Dialog,IDC_LIST2,LB_GETITEMDATA,res,0);
            if (Layer) {
              for (cnt=0;cnt<NrOdbExportLayers2;cnt++) {
...
{
  int32 Layer,cnt,cnt2,cnt3,cnt4,cnt5,cnt6,cnt7,NrCheckObjects,NrTraceObjects,
        PowerPlaneLayer,count,count2,code,lengte,FontNr,NrLines,MaxCountX,
        Mirror,Length,TempUnits,ApertureNr,RotationNr,PadMode,
        OdbLayer,NrDocLayers,NrTopComps,NrBottomComps,PreviousCnt,MemSize,
        DrawChar,res,CompOutlineLayers,PlotErrors,
        PreviousLayer,OdbPowerPlaneLayer,
        ObjectNr,Layer2,PreviousObject,StartCnt,PowerPlaneNr,
        MountType,MemPos,ShapeNr,ShapesUsed[4096],PinsUsed[4096],
        NrCompProperties,SegmentCount,LineSegments;
  ...
  char  str[1024],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],
        str5[MAX_LENGTH_STRING],strx1[MAX_LENGTH_STRING],stry1[MAX_LENGTH_STRING],
        strx2[MAX_LENGTH_STRING],stry2[MAX_LENGTH_STRING],
        strx3[MAX_LENGTH_STRING],stry3[MAX_LENGTH_STRING],PolarityCh,
        Filename[MAX_LENGTH_STRING],JobName[MAX_LENGTH_STRING],PadPolarity,
        InfoStrCopy[MAX_LENGTH_STRING],PropertyID[MAX_LENGTH_STRING],
        PropertyValue[MAX_LENGTH_STRING];
  WCHAR OdbDirW[MAX_LENGTH_STRING];
  double LineBuf[2048],incX,incY,x1,y1,x2,y2,x3,y3,x4,y4,
         OriginX,OriginY,MinX,MaxX,MinY,MaxY,xx1,yy1,xx2,yy2,
         MinPitch,x1a,y1a,x2a,y2a,RotationAngle;
  ShapeRecord *Shape;
  ...
  NrObjects5=0;
  res=FindMinMaxBoard(&DesignBoardOriginX,&DesignBoardOriginY,
                      &DesignBoardWidth,&DesignBoardHeight,3);
  DesignBoardWidth-=DesignBoardOriginX;
  DesignBoardHeight-=DesignBoardOriginY;

  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_ODB_EXPORT),
                   PCBWindow, (DLGPROC)OdbExportDialogBody);
  if (res==2) {
    return -1;
  }
  NrTopComps=0;
...
// Collect all layer objects to Objects5 and fill in the aperture table
    ...
    if (Layer<32) {
      ...
      if (PowerPlaneLayer) {
        if ((Object5=GetNewObject5(Layer))==NULL) return -1;
    ...
    for (cnt=0;cnt<NrAperTures;cnt++) {
      ...
      switch (AperTure->Info) {
        ...
        case ODB_PAD_POLYGON:
          cnt2=AperTure->Info3;
          if ((NrOdbApertureMacroObjects>0)
             &&
             (cnt2<NrOdbApertureMacroObjects)) {
...
*/
    ...
    while (1) {
      ...
      y2=Object->y2;
#ifdef _DEBUG
      if ((InRange9(Object->x1,176.0e5))
         &&
         (InRange9(Object->y1,70.6e5))) {
        ok=1;
...
//            sprintf(str,"P %s %s %d P 0 0;0=%d,1=1",strx1,stry1,ApertureNr,Object->Info & 0xffff);
        ...
        case OBJECT_RECT:
          ...
          sprintf(str,"L %s %s %s %s %d %c 0 0;0=%d",strx1,stry1,strx2,stry2,
        ...
        case PIN_ARC:
          if ((Object->Info & OBJECT_FILLED) == 0) {
            ...
            sprintf(str,"A %s %s %s %s %s %s %d %c 0 N;4=1",strx2,stry2,strx3,stry3,
                    strx1,stry1,ApertureNr,PadPolarity);
            if ((Layer>=0)
               &&
               (Layer<32)
               &&
            ...
            GetUnitsValue(TempUnits,x1,strx1,0);
            GetUnitsValue(TempUnits,y1,stry1,0);
            sprintf(str,"P %s %s %d %c 0 0;4=1",strx1,stry1,
        ...
        case TRACE_ARC:
          if ((Object->Info & OBJECT_FILLED) == 0) {
            ...
            if (NotInRangeSpecial(x2,y2,100.0)) {
              ...
              GetUnitsValue(TempUnits,x2a,strx3,0);
              GetUnitsValue(TempUnits,y2a,stry3,0);
              sprintf(str,"A %s %s %s %s %s %s %d %c 0 N;0=%d",strx2,stry2,strx3,stry3,
                      strx1,stry1,ApertureNr,PadPolarity,PadMode);
              AddToMessageBuf(str);
            }
          } else {
            GetUnitsValue(TempUnits,x1,strx1,0);
            GetUnitsValue(TempUnits,y1,stry1,0);
            sprintf(str,"P %s %s %d %c 0 %d;0=%d",strx1,stry1,
                    ApertureNr,PadPolarity,RotationNr,PadMode);
            AddToMessageBuf(str);
          }
          cnt2++;
          break;
...
#
# NET 0
NET /BLOCK2/LMP_BI_DA+ ;0=2,1=1,2=0.019685
PRP NET_TYPE 'DIFF_PAIR2'
PRP DIFF_PAIR 'DP-0.1'
SNT TOP T 0 0
FID C 1 0
FID C 2 0
FID C 3 0
...
#ifdef _DEBUG
      if ((InRange9(Object6->x1,176.0e5))
         &&
         (InRange9(Object6->y1,70.6e5))) {
        ok=1;
        if (Layer==1) {
          ok=1;
        }
...
//            strcpy(str3,"NONE");
        ...
        sprintf(str,"CMP %d %s %s %.1f N %s %s ;0=%d",ShapeNr,strx1,stry1,
                360.0-Comp->Rotation,Comp->Name,str3,MountType);
        AddToMessageBuf(str);
/*
...
*/
        ...
        for (cnt4=0;cnt4<NrObjects;cnt4++) {
          ...
          if (CompPinText(Comp,Object->PinNr,0.0,0.0,str3)==0) {
            ...
            sprintf(str,"TOP %d %s %s 0.0 N %d %d %s-%s",Object->PinNr,strx1,stry1,
                    Object->NetNr,0,Comp->Name,str3);
            AddToMessageBuf(str);
          }
        }
      }
...
//    AddToMessageBuf("@1 .comp_height");
    ...
    for (cnt=0;cnt<Design.NrComps;cnt++) {
      ...
      if ((Comp->Info & (OBJECT_NOT_VISIBLE)) == 0) {
        ...
        sprintf(str,"CMP %d %s %s %.1f N %s %s ;0=%d",ShapeNr,strx1,stry1,
                Comp->Rotation,Comp->Name,str3,MountType);
        AddToMessageBuf(str);
/*
PRP .type_name '74LVC14A'
*/
...
// *******************************************************************************************************
  ...
  DeAllocateMemTemp();
  DeAllocateMemTemp2();
  sprintf(str,"ODB++ output ready\n\nOutput file is %s.tgz",OdbFile);
