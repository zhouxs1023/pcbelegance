...
AperTureRecord OdbLastRoundPadAperTure,OdbLastRectPadAperTure,
               OdbLastPolygonPadAperTure,
               OdbLastDrillAperTure,
               OdbLastThermalReliefAperTure,NewAperTure;

int32  ok,NrOdbApertureMacroObjects,MaxNrOdbApertureMacroObjects,
...

{
  ...
  if ((NrAperTures>0)
     &&
     (InRangeSpecial(OdbLastRoundPadAperTure.x,ThickNess,100.0))) {
  ...
  while ((cnt<NrAperTures)
        &&
        (((*AperTures)[cnt].Info!=ODB_PAD_ROUND)
        ||
        (NotInRangeSpecial((*AperTures)[cnt].x,ThickNess,100.0)))) cnt++;
  if (cnt<NrAperTures) {
...

{
  ...
  if ((NrAperTures>0)
     &&
     (InRangeSpecial(OdbLastThermalReliefAperTure.x,InsideDiameter,100.0))
     &&
  ...
  while ((cnt<NrAperTures)
        ...
        ||
        (NotInRangeSpecial((*AperTures)[cnt].x2,CrossHairThickness,100.0)))) cnt++;
  if (cnt<NrAperTures) {
    AperTure=&((*AperTures)[cnt]);
  } else {
    if (NrAperTures+1>=MaxNrAperTures) {
...

{
  ...
  if ((NrAperTures>0)
     &&
     (InRangeSpecial(OdbLastRectPadAperTure.x,x,100.0))
     &&
  ...
  while ((cnt<NrAperTures)
        &&
        (((*AperTures)[cnt].Info!=ODB_PAD_RECT)
        ||
        (NotInRangeSpecial((*AperTures)[cnt].x,x,100.0))
        ||
        (NotInRangeSpecial((*AperTures)[cnt].y,y,100.0)))) cnt++;
  if (cnt<NrAperTures) {
    AperTure=&((*AperTures)[cnt]);
  } else {
    if (NrAperTures+1>=MaxNrAperTures) {
...

{
  ...
  for (cnt=0;cnt<NrAperTures;cnt++) {
    if (ObjectType==0) {
      if ((InRangeSpecial((*AperTures)[cnt].Rotation,Rotation,0.01))
         &&
    ...
    for (cnt2=0;cnt2<NrOdbApertureMacroObjects;cnt2++) {
      if (ObjectType==0) {
        if ((InRangeSpecial(ApertureMacroObjects[cnt].Rotation,Rotation,0.01))
           ...
           &&
           (ApertureMacroObjects[cnt].Address==Address)
           &&
...
#endif
      sprintf(ApertureMacroObjects[NrOdbApertureMacroObjects].Name,
              "polygon_%s_%d",ActiveShape,NrOdbApertureMacroObjects+1);
          ...
          if (PowerPlaneLayer) {
            ...
            for (cnt2=0;cnt2<Design.NrAreaFills;cnt2++) {
              ...
              if (((AreaFill->Info & (OBJECT_NOT_VISIBLE)) == 0)
                 ...
                 &&
                 (AreaFill->maxx>=Object5->minx)
                 &&
                 (AreaFill->minx<=Object5->maxx)
                 &&
                 (AreaFill->maxy>=Object5->miny)
                 &&
                 (AreaFill->miny<=Object5->maxy)) {
...
// Add thermal relief if diameter greater MinDrillForThermalRelief
                Object5->Info2=OdbCheckThermalReliefAperTure(Via->DrillThickNess,
                                                             Via->DrillThickNess+AreaFill->ThermalReliefThickness*2.0,
...
// Do not add pad
                ...
                case PIN_SMD_POLYGON:
                  ...
                  if (Object->ObjectType2!=0) {
                    ...
                    Object5->Info2=OdbCheckPolygonAperTure(Object->ObjectType2,Object->x2,Object->y2,
                    ...
                    if (CheckObjectIsBigPolygon(Object5)) {
                      ...
                      Object5->Info2=OdbCheckPolygonAperTure(0,0.0,0.0,Object->RotationAngle,
                                                             Object->Mirror,Object->Address);
                    }
                  }
                  FillPositionObject(Object5);
                  cnt3++;
                ...
                case PIN_PUT_THROUGH_POLYGON:
                  ...
                  if (Object->ObjectType2!=0) {
                    Object5->Info2=OdbCheckPolygonAperTure(Object->ObjectType2,Object->x2,Object->x2,
                                                           Object->RotationAngle,
                                                           Object->Mirror,0);
                  } else {
                    Object5->Info2=OdbCheckPolygonAperTure(0,0.0,0.0,Object->RotationAngle,
...
//                        Object5->x3=AreaFill->ThermalReliefDistance;
                      Object5->Info2=OdbCheckThermalReliefAperTure(Object->y2,
                                                                   Object->y2+AreaFill->ThermalReliefThickness*2.0,
                                                                   AreaFill->ThermalReliefDistance);
//                        Object5->Info2=OdbCheckRoundPadAperTure(AreaFill->ThermalReliefThickness);
            ...
            case PIN_SMD_POLYGON:
              if (Object->ObjectType2!=0) {
                if ((Object->Info & OBJECT_FILLED) == OBJECT_FILLED) {
                  ...
                  Object4->Info2=OdbCheckPolygonAperTure(Object->ObjectType2,Object->x2,Object->y2,
                                                         Object->RotationAngle,
                                                         Object->Mirror,0);
                } else {
                ...
                Object4->ObjectType=PIN_SMD_POLYGON;
                Object4->Info2=OdbCheckPolygonAperTure(0,0.0,0.0,Object->RotationAngle,
                                                       Object->Mirror,Object->Address);
              }
              FillPositionObject(Object4);
              NrObjects4++;
...
#ifdef _DEBUG
                if ((InRange4(Object->x2,170*2540.0))
                   &&
                   (InRange4(Object->y2,75*2540.0))) {
                  ok=1;
                  if (Layer==INFO_LAYER3) {
                    ok=1;
                  }
  ...
  for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
    ...
    if ((ObjectLine->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      if (ObjectLine->Layer==Layer2) {
        if ((Layer2>=0)
           &&
           (Layer2<Design.NrBoardLayers)) {
          if ((Object4=GetNewObject5(Layer2))==NULL) return -1;
          CopperLayer=1;
        } else {
          CopperLayer=0;
          if ((Object4=GetNewObject4())==NULL) return -1;
    ...
    if ((ObjectRect->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      if (ObjectRect->Layer==Layer2) {
...
#ifdef _DEBUG
        if ((InRange9(Object4->x1,-88.7e5))
           &&
           (InRange9(Object4->y1,85.7e5))) {
          ok=1;
        }
#endif
        if ((ObjectRect->Info & OBJECT_FILLED) == 0) {
          ...
          Object4->ObjectType=PIN_SMD_RECT;
#ifdef _DEBUG
          if ((InRange4(ObjectRect->Width,170*2540.0))
             &&
             (InRange4(ObjectRect->Height,75*2540.0))) {
            ok=1;
            if (Layer==INFO_LAYER3) {
              ok=1;
            }
...
{
  int32  cnt,cnt2,cnt3,count,NrLayerPolygons,
...
int32 DrawOdbStr(double x,double y,double Size,int32 ApertureNr,
...

{
  ...
  for (cnt=0;cnt<NrOdbApertureMacroObjects;cnt++) {
    ...
    if (ApertureMacroObjects[cnt].SpecialType) {
      ...
      if ((GeomPolygon->NrVertices<500)
         &&
         (GeomPolygon->NrSubPolygons==0)
         &&
