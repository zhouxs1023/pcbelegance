...
//    WindowProperties[cnt].Status=WindowInfo.dwWindowStatus;
    ...
    res=GetClassInfo(NULL,(LPCTSTR)(uint32)WindowProperties[cnt].atomWindowType,
                     &WindowProperties[cnt].Class);
    if ((WindowProperties[cnt].ClassName)
       &&
       (WindowProperties[cnt].Name[0]==0)
       &&
...

{
  ...
  if (GetWindowProperties(NULL,mode)==0) return;
  ...
  for (cnt=0;cnt<WindowCount;cnt++) {
    if ((mode & 1)==0) {
      sprintf(str,"%-50s : %-50s : %3d : 0x%08x : 0x%08x : %d,%d %d,%d",
                  WindowProperties[cnt].Name,
                  WindowProperties[cnt].ClassName,
                  WindowProperties[cnt].Level,
                  (int32)WindowProperties[cnt].WindowHandle,
                  WindowProperties[cnt].Style,
                  WindowProperties[cnt].SizeX,
                  WindowProperties[cnt].SizeY,
                  WindowProperties[cnt].PosX,
                  WindowProperties[cnt].PosY);
      WriteLn(fp,str);
    } else {
      if (WindowProperties[cnt].TopWindow) {
        if ((mode & 2)==0) {
          sprintf(str,"%-50s : %-50s : %4d : 0x%08x : 0x%08x : %d,%d %d,%d",
                      WindowProperties[cnt].Name,
                      WindowProperties[cnt].ClassName,
                      WindowProperties[cnt].ProcessID,
                      (int32)WindowProperties[cnt].WindowHandle,
                      WindowProperties[cnt].Style,
                      WindowProperties[cnt].SizeX,
                      WindowProperties[cnt].SizeY,
  ...
  if ((mode & 3)==3) {
    for (cnt2=0;cnt2<16384;cnt2++) {
      ...
      for (cnt=0;cnt<WindowCount;cnt++) {
        if ((WindowProperties[cnt].TopWindow)
           &&
           (WindowProperties[cnt].ProcessID==cnt2)) {
          sprintf(str,"%-50s : %-50s : %4d : 0x%08x : 0x%08x : %d,%d %d,%d",
                      WindowProperties[cnt].Name,
                      WindowProperties[cnt].ClassName,
                      WindowProperties[cnt].ProcessID,
...
int32 GetProcessModule (DWORD dwPID, DWORD dwModuleID,
...
*/
            ...
            if (hProcess) {
              ...
              sprintf(str,"Process usertime %d msec",(int32)(KernelTime.dwLowDateTime+UserTime.dwLowDateTime)/10000);
              WriteLn(fp,str);
              CloseHandle (hProcess);
            }
  ...
  if (Process32First(ProcessSnap, &pe32)) {
    do {
      ...
      if (ProcessHandle) {
        ...
        ProcessProperties[ProcessesCount].UsedTime=(KernelTime.dwLowDateTime+UserTime.dwLowDateTime)/10000;
...

{
  ...
  if ((ProcessesCount=GetProcessProperties(NULL))==0) return;

  fp=FileOpenWrite(Filename);
  for (cnt=0;cnt<ProcessesCount;cnt++) {
    if (mode==0) {
      sprintf(str,"%-50s : %4d : %6d ms : %6d kb : %6d kb",
                  ProcessProperties[cnt].ExeName,
                  ProcessProperties[cnt].ProcessID,
                  ProcessProperties[cnt].UsedTime,
                  ProcessProperties[cnt].MinMemUsedInKb,
                  ProcessProperties[cnt].MaxMemUsedInKb);
      WriteLn(fp,str);
    } else {
    }
  }
