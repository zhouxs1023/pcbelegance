...
int32     DialogResult,DialogMode,NrSelectedLayers,SelectedLayers[32],
          ErrorNr,TempUnits,SelectedObjectNr,SelectedObjectCode;
...

{
  ...
  char    str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],FillStr[MAX_LENGTH_STRING];
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetWindowTextOwn(Dialog,SC(902,"View error"));
      TabStops[0]=15;
      TabStops[1]=165;
      TabStops[2]=195;
      TabStops[3]=225;
      ...
      for (cnt=0;cnt<NrErrorObjects/2;cnt++) {
        ...
        switch (Object1->ObjectType) {
          ...
          case PIN_PUT_THROUGH_ROUND:
            sprintf(str,SC(908,"Through hole pad %s\t\t"),str3);
            break;
          case PIN_PUT_THROUGH_SQUARE:
            sprintf(str,SC(909,"Through hole square pad %s\t\t"),str3);
            break;
          case PIN_PUT_THROUGH_POLYGON:
            sprintf(str,SC(910,"Through hole polygon pad %s\t\t"),str3);
            break;
          case DRILL:
        ...
        switch (Object2->ObjectType) {
          ...
          case PIN_LINE_DIAG2:
            ...
            break;
          case PIN_PUT_THROUGH_ROUND:
            sprintf(str2,SC(908,"Through hole pad %s\t\t"),str3);
            break;
          case PIN_PUT_THROUGH_SQUARE:
            sprintf(str2,SC(909,"Through hole square pad %s\t\t"),str3);
            break;
          case PIN_PUT_THROUGH_POLYGON:
            sprintf(str2,SC(910,"Through hole polygon pad %s\t\t"),str3);
            break;
          case DRILL:
          ...
          case PIN_SMD_RECT:
            if (Object2->ObjectType2!=OBJECT_TEXT2) {
              ...
              if (Units==0) {
                sprintf(str3,SC(904,"%.1f,%.1f mils"),
                        (Object2->x1-Object2->x2*0.5)/2540.0,
                        (Object2->y1-Object2->y2*0.5)/2540.0);
              } else {
                sprintf(str3,SC(905,"%.4f,%.4f mm"),
                        (Object2->x1-Object2->x2*0.5)/100000.0,
                        (Object2->y1-Object2->y2*0.5)/100000.0);
              }
              sprintf(str2,SC(1226,"Text %s\t%s"),str3,str4);
            }
            break;
          case PIN_SMD_POLYGON:
...
//        strcat(str," (Not completly surrounded with copper)");
        ...
        if ((Object1->ObjectType==AREAFILL)
           &&
           ((Object2->Info2 & 1)==1)) {
...

{
  ...
  ErrorNr=-2;
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_ERRORS), PCBWindow,
                (DLGPROC)ErrorDialogBody);
  return ErrorNr;
}
...
*/
  switch (MeasureItem->CtlType) {
    case ODT_MENU:
      ...
      if ((Command>=ID_EXT_RESOURCE)
         &&
         (Command<ID_EXT_RESOURCE+4096)) {
        Command=GetExtendedResource(Command-ID_EXT_RESOURCE);
      }
      if ((Command>=ID_SELECT_LAYER)
         &&
         (Command<ID_SELECT_LAYER+0x1000)) {
...
// **********************************************************************************
    case ODT_MENU:
      ...
      if ((Command>=ID_SELECT_LAYER)
        ...
        switch (DrawItem->itemAction) {
          case ODA_DRAWENTIRE:
            ...
            break;
          case ODA_SELECT:
            if ((DrawItem->itemState & ODS_SELECTED) == 0) Inverted=0;
            else Inverted=1;
            break;
        }
      //  if (Inverted) MenuColor^=0x00ffffff; PolyLine
        ...
        Rectangle(DrawItem->hDC,DrawItem->rcItem.left+20,DrawItem->rcItem.top,
...
//        }
        Rectangle(DrawItem->hDC,DrawItem->rcItem.left,DrawItem->rcItem.top,
                                DrawItem->rcItem.left+20,DrawItem->rcItem.bottom);
        if (Inverted) {
...
//        SetTextColor(DrawItem->hDC,RGB(0,0,0));
        ...
        TextOut(DrawItem->hDC,30,DrawItem->rcItem.top+9,TextStr,strlen(TextStr));
...
//            SetBkColor(DrawItem->hDC,RGB(255,255,255));
            switch (MenuHeadNr) {
              case 0:
                strcpy(TextStr,SC(685,"Drag traces/vias/components menu"));
                break;
              case 1:
                strcpy(TextStr,SC(677,"Routing menu"));
                break;
              case 2:
                strcpy(TextStr,SC(678,"Trace drag menu"));
                break;
              case 3:
                strcpy(TextStr,SC(679,"Component menu"));
                break;
              case 4:
                strcpy(TextStr,SC(680,"Component references menu"));
                break;
              case 5:
                strcpy(TextStr,SC(681,"Component values menu"));
                break;
              case 6:
                strcpy(TextStr,SC(682,"Specials menu"));
                break;
              case 7:
                strcpy(TextStr,SC(683,"Areafills/powerplanes menu"));
                break;
              case 8:
                strcpy(TextStr,SC(684,"Change traces/vias menu"));
                break;
              case 9:
                strcpy(TextStr,SC(686,"Gate/pin swap menu"));
                break;
            }
            TextOut(DrawItem->hDC,13,DrawItem->rcItem.top+6,TextStr,strlen(TextStr));
            SelectObject(DrawItem->hDC,SaveFont);
//            DeleteObject(MenuFont);
            break;
        }
...
//            SaveFont=SelectObject(DrawItem->hDC,GetStockObject(SYSTEM_FONT));
            ...
            switch (ItemNr) {
              ...
              case ViaPinsInNetObjectNr:
                Rectangle(DrawItem->hDC,DrawItem->rcItem.left,DrawItem->rcItem.top,
                                        DrawItem->rcItem.left+90,DrawItem->rcItem.bottom+1);
                ...
                TextOut(DrawItem->hDC,DrawItem->rcItem.left+100,DrawItem->rcItem.top+6,
            ...
            if (Inverted) {
              ...
              Rectangle(DrawItem->hDC,DrawItem->rcItem.left,DrawItem->rcItem.top+1,
                                      DrawItem->rcItem.right,DrawItem->rcItem.bottom+1);
            }
//            SaveFont=SelectObject(DrawItem->hDC,MenuFont);
            SetBkMode(DrawItem->hDC,TRANSPARENT);
//            SetBkColor(DrawItem->hDC,RGB(0,0,0));
            ...
            TextOut(DrawItem->hDC,COLOR_LIST_BOX_WIDTH+5,DrawItem->rcItem.top+6,TextStr,strlen(TextStr));
            SelectObject(DrawItem->hDC,SavePen);
            SelectObject(DrawItem->hDC,SaveBrush);
            SelectObject(DrawItem->hDC,SaveFont);
...
// DRAWITEMSTRUCT
    ...
    //        }
            Rectangle(DrawItem->hDC,DrawItem->rcItem.left,DrawItem->rcItem.top,
                                    DrawItem->rcItem.left+21,DrawItem->rcItem.bottom+1);
            if (Inverted) {
...
*/
  ...
  //        SetBkColor(DrawItem->hDC,MenuColor);
    ...
    //        SetTextColor(DrawItem->hDC,RGB(0,0,0));
            ...
            GetLayerText(Layer,(LPSTR)&TextStr,0);
            if (Layer!=100) {
              TextOut(DrawItem->hDC,30,DrawItem->rcItem.top+6,TextStr,strlen(TextStr));
            }
            SelectObject(DrawItem->hDC,SaveFont);
//            DeleteObject(MenuFont);
          }
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetWindowTextOwn(Dialog,SC(935,"Layer insert"));
...

{
  ...
  res=OwnDialogBox(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_INSERT_LAYER), PCBWindow, (DLGPROC)InsertLayerDialogBody);
  if (res==-1) return -1;
  AddDesignLayer(res);
  return 0;
}
...

{
  ...
  if (res==-1) return -1;
  if (res>1) {
    MessageBoxOwn(PCBWindow,SC(937,"Select only one layer"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
    return -1;
  }
  if (res==1) {
...

{
  ...
  if (res==-1) return -1;
  if (res!=2) {
    MessageBoxOwn(PCBWindow,SC(938,"Select two layers"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
    return -1;
  }
  SwitchDesignLayer(Design.NrBoardLayers-SelectedLayers[0]-1,
                    Design.NrBoardLayers-SelectedLayers[1]-1);
  return 0;
}
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(266,"Layers"));
      sprintf(str,SC(266,"Layers"));
      SetWindowTextOwn(Dialog,str);
      for (cnt=Design.NrBoardLayers-1;cnt>=0;cnt--) {
        GetLayerText(cnt,str,0);
        res=SendDlgItemMessageOwn(Dialog,IDC_LIST1,LB_ADDSTRING,0,(LPARAM)str);
      }
      res=SendDlgItemMessageOwn(Dialog,IDC_LIST1,LB_ADDSTRING,0,(LPARAM)SC(939,"Info layer1"));
      res=SendDlgItemMessageOwn(Dialog,IDC_LIST1,LB_ADDSTRING,0,(LPARAM)SC(940,"Info layer2"));
      res=SendDlgItemMessageOwn(Dialog,IDC_LIST1,LB_ADDSTRING,0,(LPARAM)SC(941,"Info layer3"));
      res=SendDlgItemMessageOwn(Dialog,IDC_LIST1,LB_ADDSTRING,0,(LPARAM)SC(942,"Info layer4"));
      return about;
    case WM_MOVE:
      break;
    case WM_COMMAND:
