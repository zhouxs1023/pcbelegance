...
*/
  ...
  if (!CommandLine) {
    mode=1;
    if (UnicodeToUtf8(GetCommandLineW(),OwnCommandLine,MAX_LENGTH_OWN_COMMAND_LINE)==0) return -2;
...
// ********************************************************************************************************
      if (CommandLine[cnt2]=='\"') {
        ...
        cnt=cnt3;
        if (LengthString>0) {
          if (LengthString>MAX_PARAM_STRING_LENGTH-1) LengthString=MAX_PARAM_STRING_LENGTH-1;
...
// ********************************************************************************************************
        if ((CommandLine[cnt2]=='/')
          ...
          if (OptionFound) {
            ...
            NrParams++;
            OptionFound=0;
            if (NrParams==MAX_PARAMETERS) return -1;
          }
          if (cnt2+1<lengte) {
            cnt3=cnt2+1;
            while ((CommandLine[cnt3]!=' ')
            ...
            LengthString=cnt3-cnt2-1;
            if (LengthString>0) {
              if (LengthString>MAX_OPTION_STRING_LENGTH-1) LengthString=MAX_OPTION_STRING_LENGTH-1;
...
// ********************************************************************************************************
  ...
  if (Parameters[0].Parameter[0]!=0) {
    if (mode==1) {
      if (Utf8ToUnicode(Parameters[0].Parameter,ExecutableW,MAX_PARAM_STRING_LENGTH)==0) return -2;
      ...
      if (UnicodeToUtf8(ExecutableW2,Parameters[0].Parameter,MAX_PARAM_STRING_LENGTH)==0) return -2;
    }
  }
  return result;
}
