...

{
  ...
  if (diff<0.00003) {
    ...
    if ((NewRotation & 1) == 0) { // 90 degrees increments
...

{
  int32  MemPos,cnt,cnt2,SizeFile,Pos,NrLibEntries,NrGeometryLibFiles,
         *ShapeTypeP,ShapeType,NrLines,Shapefp,result,res,Libfp,Found;
...
// Check shape libraries
      ...
      while ((!Found)
        ...
        if ((Libfp=FileOpenReadOnlyUTF8(GeometryLibNames[cnt2]))==-1) return -1;
  ...
  if ((PcbDesign.NrShapes+1)*(int32)sizeof(ShapeInfoRecord)>MaxSpecialMemory[MEM_SHAPES]) {
    AllocateSpecialMem(MEM_SHAPES,MaxSpecialMemory[MEM_SHAPES]+sizeof(ShapeInfoRecord)*128,
                       (void *)&Shapes);
  }
  ...
  PcbDesign.NrShapes++;
  if (Pos==-1) {
    if ((Shapefp=FileOpenReadOnlyUTF8(str))==-1) return -3;
    if (FileRead(Shapefp,&ShapesMem[ShapesMemSize],SizeFile,&result)==-1) return -3;
    FileClose(Shapefp);
  } else {
    if ((Shapefp=FileOpenReadOnlyUTF8(str))==-1) return -3;
  ...
  if (stricmpOwn(Shape->Identification,ShapeCode)==0) { // Old shape definition
    Shape->NrLayers=2;
    Shape->NrPolygons=0;
...
{
  int32           PinOffset,MemPos,ObjectInclude,
                  NrPins,PinNr,Rotation,NetPinCount,
                  PolygonVertices,NrLayers,MirroredLayer,CompObjectCount,
                  ShapePos,Layer,ShapeInfo,NrPinShapes,ShapeType,Mirror;
...
//      Object->CompNr=CompNr;
    ...
    if (NrObjects6>=MaxNrObjects6-1) {
      if (AllocateMemObjects6(MaxNrObjects6+1024)==0) NrObjects6++;
    } else NrObjects6++;
    NrLines--;
  }
}
...

{
  int32 MemPos,NrLines,Rotation,ShapeType,TextLength,ShapeMirror,NewMirror,
        *ShapeTypeP,PolygonVertices,ShapePos,*PolygonPointer,Mirror,
        ShapeInfo2,ObjectInclude;
  double hulp,x1,y1,x2,y2,x3,y3,x4,y4,OX,OY,OriginX,OriginY,RotationAngle,
...

{
  int32 MemPos,NrLines,Rotation,ShapeType,TextLength,PolygonVertices,
        ...
        NewMirror,Mirror,ShapeInfo2,ObjectInclude;
  double hulp,x1,y1,x2,y2,x3,y3,x4,y4,OX,OY,OriginX,OriginY,RotationAngle,
         RotationAngle2,TextRotation;
  ShapeLinesArray *ShapeLines;
  ObjectRecord *Object;
...
*/
  int32 MemPos,NrLayers,NrLines,Rotation,Layer,PolygonVertices,ShapePos,
        ShapeMirror,NewMirror,Mirror,ShapeType,
        OkToAddObject,ObjectInclude;
