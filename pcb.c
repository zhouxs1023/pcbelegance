...
PAINTSTRUCT                 PS;
RECT                        RealWindow,ClientRect,UpdateRect,OldClientRect,
                            OldRealWindow2,RealWindow2,OldRealWindow;
...
int32                       DrawWindowMinX,DrawWindowMinY,DrawWindowMaxX,DrawWindowMaxY,
                            ScreenSizeX,ScreenSizeY,ScreenStartX,ScreenStartY,
                            ScrollBarSizeX,ScrollBarSizeY,
                            HeightScrollBar,WidthScrollBar,
                            StartTimerValue,ok,
                            ScreenPosAbsCursor,ScreenPosAbsGridCursor,
                            ScreenPosRelGridCursor,ScreenPosInfoStr;
...
UINT                        TimerObject,ClipID2,ClosingWindowMessage,
                            ClipBoardLayoutTracesViasID,ClipBoardLayoutComponentsID;
...
static ProgramInfoRecord ProgramInfo={{0xe2,0x03,0x29,0x6e,0x74,0x5f,0x9a,0x3b,
...

{
  ...
  while ((cnt<32)
        &&
        (ProjectInfo->FileTypes[cnt]!=10)) cnt++;
  if ((cnt<32)
     &&
     (ProjectInfo->WindowHandles[cnt])) {
...
{
  int32 count,cnt,cnt2,Layer,MemPos,ok,NrBoardOutlineObjects,MemSize,
        AddedSpace,CompSize,CompMemSize,NrPins,PinsMemPos,MemPosComp,
        lengte2,StopCnt,TextRotation,Mirror,AreaFillCorrupt,ShapeNr,
        AreaFillError;
...
//  ViewWholeDesign(0);
  ...
  for (Layer=0;Layer<32;Layer++) {
    for (cnt=0;cnt<Design.NrVerTraces[Layer];cnt++) {
      ...
      if (Trace->ThickNess<100.0) Trace->ThickNess=2000.0;
      if (Trace->Length<0.0) {
  ...
  for (cnt=0;cnt<Design.NrVias;cnt++) {
    ...
    if ((Via->NetNr>=0)
      ...
      if (Via->ThickNess<100.0) Via->ThickNess=2000.0;
      if (Via->DrillThickNess<100.0) Via->DrillThickNess=1000.0;
    }
  }
...
/*
      if (Net->TraceWidth==0) {
        if (Design.StandardTraceWidth==0) {
          Net->TraceWidth=(8*2540);
        } else {
          Net->TraceWidth=Design.StandardTraceWidth;
        }
      }
      if (Net->TraceClearance==0) {
        if (Design.StandardClearance==0) {
...
// ********************************************************************************************************
  ...
  if (AreaFillError) {
    MessageBoxOwn(PCBWindow,SC(943,"Error in areafills"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
  }
...
// ********************************************************************************************************
  ...
  for (cnt=0;cnt<Design.NrObjectArcs;cnt++) {
    ...
    if ((ObjectArc->Info & OBJECT_FILLED) ==  0) {
      if ((ObjectArc->StartDiffX==0.0)
         &&
         (ObjectArc->StartDiffY==0.0)) {
        ObjectArc->StartDiffX=1e6;
        ObjectArc->StartDiffY=1e5;
      }
      if ((ObjectArc->EndDiffX==0.0)
         &&
         (ObjectArc->EndDiffY==0.0)) {
...
//  strcpy(InfoStr,SC(330,"Move/rotate/change components"));
//  RedrawInfoStr(1);
}
...
//    res=GetDIBits(OutputDisplay,ViewBitmap,0,ViewPixelsY,TempMem,BitmapInfo,DIB_RGB_COLORS);
    ...
    if ( Painting ) EndPaint(PCBWindow, &PS);
    else ReleaseDC(PCBWindow, OutputDisplay2);
    DCInUse=0;
    ViewBitmap=NULL;
    OutputDisplay=NULL;
    OutputDisplay2=NULL;
...

{
  if ( DCInUse ) {
    if ( Painting ) EndPaint(PCBWindow, &PS);
    else ReleaseDC(PCBWindow, OutputDisplay);
    DCInUse=0;
    OutputDisplay=NULL;
    OutputDisplay=NULL;
  }
...

{
  ...
  if (hulp2>MaxScrollBarY) hulp2=MaxScrollBarY;
  if (hulp2<0) hulp2=0.0;
  ...
  if (hulp2>MaxScrollBarX) hulp2=MaxScrollBarX;
  if (hulp2<0) hulp2=0.0;
...
//  DrawObjectArc(&ObjectArc,0.0,0.0,0);
  ...
  DrawLine(MultX(TestObject.x1),MultY(TestObject.y1),
           MultX(TestObject.x1+TestObject.x3*100),MultY(TestObject.y1+TestObject.y3*100));
  DrawLine(MultX(TestObject.x1),MultY(TestObject.y1),
           MultX(TestObject.x1+TestObject.x4*100),MultY(TestObject.y1+TestObject.y4*100));
}
...

{
  ...
  SetBkMode(OutputDisplay,OPAQUE);
  TextOut(OutputDisplay,20,ClientRect.bottom-(HeightInfoBar-4),(LPCSTR)&LayerStr,strlen(LayerStr));
  SetBkMode(OutputDisplay,TRANSPARENT);
...

{
  ...
  Rect.bottom=RightBottomY;
  FillRect(OutputDisplay,&Rect,GetStockObject(LTGRAY_BRUSH));
  TextOut(OutputDisplay,ScreenPosAbsCursor+2,ClientRect.bottom-(HeightInfoBar-4),(LPCSTR)&AbsPosStr,strlen(AbsPosStr));
  SelectObject(OutputDisplay,SaveFont);
  switch (Mode) {
    case 0:
      break;
...

{
  ...
  Rect.bottom=RightBottomY;
  FillRect(OutputDisplay,&Rect,GetStockObject(LTGRAY_BRUSH));
  TextOut(OutputDisplay,ScreenPosAbsGridCursor+2,ClientRect.bottom-(HeightInfoBar-4),(LPCSTR)&AbsGridPosStr,strlen(AbsGridPosStr));
  SelectObject(OutputDisplay,SaveFont);
  switch (Mode) {
    case 0:
      break;
...

{
  ...
  Rect.bottom=RightBottomY;
  FillRect(OutputDisplay,&Rect,GetStockObject(LTGRAY_BRUSH));
  TextOut(OutputDisplay,ScreenPosRelGridCursor+2,ClientRect.bottom-(HeightInfoBar-4),(LPCSTR)&RelPosStr,strlen(RelPosStr));
  SelectObject(OutputDisplay,SaveFont);
  switch (Mode) {
    case 0:
      break;
...

{
  ...
  if (Utf8ToUnicode(InfoStr,InfoStrUC,MAX_LENGTH_STRING-1)==0) return;
...

{
  ...
  if ( GetUpdateRect(PCBWindow,&UpdateRect,0) ) {
    ...
    if (!PaintBecauseOfResize) {
      ...
      if (UpdateRect.bottom>=(int32)DrawWindowMaxY+HeightScrollBar) RedrawInfoBar();
      if (UpdateRect.left<(int32)DrawWindowMinX) RedrawButtons();
      if ((UpdateRect.bottom>=(int32)DrawWindowMinY)
         &&
         (UpdateRect.top<(int32)DrawWindowMaxY)) {
        RedrawMainWindow();
      }
      if (FastPaint) {
        DoneDeviceContext2();
      ...
      if ((PaintBecauseOfResizeMode & (16)) != 0) {
        ...
        Rect2.right=ClientRect.right;
        Rect2.top=ClientRect.bottom-HeightInfoBar-HeightScrollBar-
                  abs(ClientRect.bottom-OldClientRect.bottom);
...

{
  ...
  if ((Message!=WM_TIMER)
    ...
    sprintf(str,"%s | Message 0x%04x, WPARAM = %d,%d LPARAM = %d,%d\n",
            str2,Message,LOWORD(WParam),HIWORD(WParam),
  ...
  switch ( Message ) {
    ...
    case WM_USER:
      ...
      break;
    case WM_COMMAND:
      if (HIWORD(LParam)==0) PCBCommand(WParam,(int32)LParam);
      break;
    case WM_PAINT:
      if (FirstPaint) {
        FirstPaint=0;
...
//      ScrollEndOfWindow=ScrollSize/2;
      ...
      MoveWindow (hwndHBar,DrawWindowMinX,DrawWindowMaxY,
                           DrawWindowMaxX-WidthButtonBar,HeightScrollBar, 0);
      MoveWindow (hwndVBar,DrawWindowMaxX,DrawWindowMinY,
                           WidthScrollBar,DrawWindowMaxY, 0);
      if (hwndHBar!=NULL) {
    ...
    case WM_MOUSEMOVE:
      ...
      if (GetCursor()!=CurrentCursor) SetCursor(CurrentCursor);
      break;
    ...
    case WM_TIMER:
      ...
      if ((TimerValue % ((1*1000)/NR_MILLI_SECONDS_TIMER))==0) {
        if (ProjectIndexNr!=-1) {
          if ((ProjectInfo->OtherInfos[0]!=0)
             &&
             ((Design.NrComps>0) || (Design.NrNets>0))
             &&
             (!TimerUpdateNetlist)) {
...
{
  int32  fp,cnt,Length,Value,res,ok,Key,ParamMode,
         OldWindowStartX,OldWindowStartY,OldWindowWidth,OldWindowHeight,
         ChangeGrid=0,ChangeTraces=0,ChangeClearances=0;
  char   LineBuf[512],str1[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],str5[MAX_LENGTH_STRING];
  float  Value1;

  if (IniFile[0]==0) return;
...
//  MessageBox(NULL,IniFile,"Load ini file from",MB_APPLMODAL+MB_OK);
  ...
  if ((fp=TextFileOpenUTF8(IniFile))<0) return;
  ParamMode=0;
  while ((Length=ReadLn(fp,LineBuf))>=0) {
    ...
    if ((Length>1)
      ...
      if (str1[0]=='[') {
        ...
        if (stricmp(str1,"[Keys]")==0) ParamMode=4;
      } else {
        switch (ParamMode) {
          case 1:
            break;
...
// ******************************************************************************
  ...
  for (cnt=0;cnt<NrLayerObjects;cnt++) {
    if (LayerObjectCodes[cnt].TextStr[0]!=0) {
      sprintf(str1,"%sColor=%i",LayerObjectCodes[cnt].TextStr,(int32)PCBColors[LayerObjectCodes[cnt].ColorNr]);
      WriteLn(fp,str1);
    }
  }
  for (cnt=0;cnt<NrLayerObjects;cnt++) {
    if (LayerObjectCodes[cnt].TextStr[0]!=0) {
      sprintf(str1,"%sColor2=%i",LayerObjectCodes[cnt].TextStr,(int32)PCBColors2[LayerObjectCodes[cnt].ColorNr]);
      WriteLn(fp,str1);
    }
  }
  for (cnt=0;cnt<NrLayerObjects;cnt++) {
  ...
  cnt=0;
  while (!Stop) {
    if ((GetKeyString(str2,cnt,3)==1)
       &&
       ((res=GetKeyString(str3,cnt,5))!=-1)) {
      if (res==0) str3[0]=0;
      if (str2[0]!=0) {
        strcat(str2,"                                              ");
        str2[36]=0;
...

{
  ...
  if ((ID<0)
     ||
     (ID>=MaxNrPcbNames)) return -1;
  if (PcbNamesPos+Length+2>=PcbNamesBufSize) return -1;

  if (!Utf8ToUnicode(Text,TextW,MAX_LENGTH_STRING)) return -1;
...
/*
    ...
    MessageBoxOwn(NULL,str,SC(102,"Warning"),MB_APPLMODAL|MB_OK);
*/
    return -1;
  }
  ...
  for (cnt=0;cnt<NrParams;cnt++) {
    if (Parameters[cnt].Option[0]!=0) {
      ...
      if ((mode==0)
         ...
         (cnt>0)
         &&
         (Parameters[cnt].Parameter[0]!=0)) {
        if (GetFullPathName(Parameters[cnt].Parameter,MAX_LENGTH_STRING-50,EditFile,&FileName)==0) {
          MessageBoxOwn(PCBWindow,Parameters[cnt].Parameter,
                         SC(253,"Error in opening file"),MB_APPLMODAL|MB_OK);
        }
      }
    }
  }
...
int32 PASCAL WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance,
                    LPSTR lpszCmd, int32 nCmdShow)

{
  ...
  if (!FlagSSE2) {
    MessageBox(NULL,"This version of the layout editor requires a CPU with SSE2 extensions",
  ...
  if (AddPcbLanguageStrings(str)==-2) return -1;
...
// The memory mapped file has already been created by another applicatoin (Design manager)
  ...
  if ( hPrevInstance == 0 ) {
    ...
    if ( ! RegisterClass(&PCBClass) ) exit(255);
  }
  ...
  HeightScrollBar = GetSystemMetrics (SM_CYHSCROLL),
  WidthScrollBar = GetSystemMetrics (SM_CXVSCROLL);

  if (CompMemoryError) {
    MessageBoxOwn(PCBWindow,SC(946,"Error in components\r\n\r\nUpdate the netlist"),SC(100,"Error"),MB_OK);
  }

  ok=1;
  PCBWindow = CreateWindow(PCBClass.lpszClassName,
                           0,
//                           WS_OVERLAPPEDWINDOW | WS_VSCROLL,
  ...
  hwndHBar = CreateWindow ("scrollbar", NULL, WS_CHILD | WS_VISIBLE | SBS_HORZ,
                           0,0,0,0, PCBWindow, (HMENU) 1, hInstance, NULL);
  hwndVBar = CreateWindow ("scrollbar", NULL, WS_CHILD | WS_VISIBLE | SBS_VERT,
                           0,0,0,0, PCBWindow, (HMENU) 2, hInstance, NULL);
  ...
  OwnAppendMenu(EditMenu,MF_ENABLED|MF_STRING,ID_MEMORY_USE,"Memory use");
#endif
  ...
  if ( Created ) DestroyWindow(PCBWindow);
  ClipCursor(NULL);
