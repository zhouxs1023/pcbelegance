		...
		if ((Comp->Info & (OBJECT_NOT_VISIBLE | OBJECT_SELECTED)) == OBJECT_SELECTED)
		{
			...
			if (FileExistsUTF8(str) != 0)
			{
				...
				if (FileExistsUTF8(str) != 0)
				{
					...
					sprintf(str,
					        SC(838,
					           "Do you want to copy geometry %s from library %s to the local dirctory %s\\pcb\\shapes"),
...
int32 ImportComponentPositions(int32 mode)
{
	...
	if (mode == 0)
	{
		...
		if (GetNewFileUTF8
		        (PCBWindow, NULL, ImportComponentFileName, ExportDir, SC(722, "Import component positions"), NULL,
		         SC(390, "All files"), "*", 0))
			return -1;
...
// ****************************************************************************************************
	...
	case 5:					// Evenly spaced horizontal
		if (count < 3)
		{
			MessageBoxOwn(PCBWindow, SC(871, "More than two components are needed"), SC(100, "Error"),
			              MB_APPLMODAL | MB_OK);
			return -1;
		}
