...
//  memset(&Buf[128*1024],0,128*1024);
  ...
  for (cnty=1;cnty<1023;cnty++) {
    ...
    CurrentPoint=0;
    NextPoint=0;
    if (BitMapXY(0,cnty) != 0) PreviousPoint=1;
    if (BitMapXY(1,cnty) != 0) CurrentPoint=1;
    if (BitMapXY(2,cnty) != 0) NextPoint=1;
    for (cntx=1;cntx<1023;cntx++) {
      if (CurrentPoint==0) {
        if (PreviousPoint==1) {
          SetBitMap2PointXY(mainx,mainy,cntx,cnty);
      ...
      NextPoint=0;
      if (cntx<1022) {
        if (BitMapXY(cntx+2,cnty) != 0) NextPoint=1;
      }
    }
  }
...

{
  if (((x)<0)
     ...
     ((y)<0)
     ||
     ((x)>=NrBytesBitmapLine*8)
     ||
     ((y)>=MaxNrBitmapLines)) return 0;
  if ((BitmapBuf2[(y)*NrBytesBitmapLine+((x) >> 3)] & (0x80 >> ((x) & 7)))!=0) return 1;
  return 0;
}
...
{
  return ( CheckBitMap2XYa(x  ,y+1) +
           CheckBitMap2XYa(x+1,y+1) +
           CheckBitMap2XYa(x+1,y  ) +
           CheckBitMap2XYa(x+1,y-1) +
           CheckBitMap2XYa(x  ,y-1) +
           CheckBitMap2XYa(x-1,y-1) +
           CheckBitMap2XYa(x-1,y  ) +
           CheckBitMap2XYa(x-1,y+1));
...
{
  int32  cnt,cnt2,cnt3,cntx,cnty,x,y,x1,y1,x2,y2,res,ok,StartPointX,StartPointY,
         NrPointsFound,NrTries,PosX,
         PreviousPointX,PreviousPointY,PPreviousPointX,PPreviousPointY,PixelNeighbours,
         NextDirection,FirstObjectNr,count,NotClosedLines,
         Direction,NextDirectionPoint[8],fp;
    ...
    for (cntx=0;cntx<NrBytesBitmapLine;cntx++) {
      if (BitmapBuf2[PosX+cntx]!=0) {
        ...
        for (x2=0;x2<8;x2++) {
          ...
          if ((x>0)
             &&
             (x<NrBytesBitmapLine*8-1)
             &&
             (CheckBitMap2XY(x,y))) {
            cnt++;
            PixelNeighbours=CheckBitMap2XY(x  ,y+1)+
                            CheckBitMap2XY(x+1,y+1)+
                            CheckBitMap2XY(x+1,y  )+
                            CheckBitMap2XY(x+1,y-1)+
                            CheckBitMap2XY(x  ,y-1)+
                            CheckBitMap2XY(x-1,y-1)+
                            CheckBitMap2XY(x-1,y  )+
                            CheckBitMap2XY(x-1,y+1);
...
//  DeAllocateMemBitmapPolygons();
  ...
  while (!Stop) {
    FoundBit=0;
    while ((cnty<MaxNrBitmapLines)
          &&
          (!FoundBit)) {
      cntx=0;
      while ((cntx<NrBytesBitmapLine)
            &&
            (!FoundBit)) {
...
*/
      ...
      while (!EndPointReached) {
        ...
        if (Direction==-1) {
          ...
          while ((cnt<8)
                &&
                (NextDirectionPoint[StartSequence[cnt]]==0)) cnt++;
          if (0) {
            while ((cnt<8)
                  &&
                  (NextDirectionPoint[StartSequence[cnt]]==0)
                  &&
                  (PixelNeighboursAroundPixel[StartSequence[cnt]]==0)) cnt++;
          }
          if (cnt<8) {
          ...
          if (NextDirectionPoint[Direction]==0) {
            ...
            if (cnt<7) {
              ...
              while ((cnt<7)
                    &&
                    (NextDirectionPoint[Sequences[Direction*8+cnt]]==0)) cnt++;
              if (cnt<7) {
                NextDirection=Sequences[Direction*8+cnt];
              } else {
                NextDirection=-1;
...

{
  ...
  for (cnty=1;cnty<MaxNrBitmapLines-2;cnty++) {
    ...
    CurrentPoint=0;
    NextPoint=0;
    if (BitMap3XY(0,cnty) != 0) PreviousPoint=1;
    if (BitMap3XY(1,cnty) != 0) CurrentPoint=1;
    if (BitMap3XY(2,cnty) != 0) NextPoint=1;
    for (cntx=1;cntx<NrBytesBitmapLine*8-2;cntx++) {
      ...
      CurrentPoint=NextPoint;
      NextPoint=0;
      if (BitMap3XY(cntx+2,cnty) != 0) NextPoint=1;
    }
  }
...

{
  ...
  PinTextFont=CreateFont(PixelsY,0,0,0,0,0,0,0,ANSI_CHARSET,
                         OUT_TT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,
                         FIXED_PITCH,"Courier New");
  SimpleTextFont=CreateFont(-12,0,0,0,FW_DONTCARE,0,0,0,OEM_CHARSET,OUT_DEFAULT_PRECIS,
                            CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,DEFAULT_PITCH,"SansSerif");
