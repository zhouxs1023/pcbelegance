...
extern AperTureRecord LastRoundPadAperTure,LastRectPadAperTure,
                      LastPlatedDrillAperTure,LastUnPlatedDrillAperTure,
                      LastThermalReliefAperTure,LastTraceAperTure,NewAperTure,
                      LastPolygonPadAperTure;

extern AperTureRecord LastButterflyPadAperTure;

extern int32          MaxNrAperTures,NrAperTures,NrDrillAperTures,
                      NrAperTures2,NrLoadedAperTures;

extern int32          GerberMode;

extern int32          DcodeTrace,DcodeRoundPad,DcodeRectPad,DcodeDrill,DcodeThermalRelief,
                      DcodePolygonPad,DcodeButterflyPad,
                      PlotErrors,
                      InfoLayerPlotWarnings,
                      BoardOutlinePlotWarnings;

extern int            Gerberfp,Drillfp,Plotfp;

extern  double   TextMinX,TextMinY,TextMaxX,TextMaxY,CurrentPlotPenSize,TextPen,
                DesignBoardOriginX,DesignBoardOriginY,DesignBoardWidth,DesignBoardHeight;
...
extern int32           ObjectsNetHorTraces,ObjectsNetVerTraces,
                       ObjectsNetDiag1Traces,ObjectsNetDiag2Traces,
                       ObjectsNetVias,ObjectsNetPins,
                       NrTracePoints,ObjectsNetConnecions;
...

{
  ...
  if (NrAperTures>0) {
    ...
    while ((cnt<NrAperTures)
          &&
          (((*AperTures)[cnt].Info!=GERBER_TRACE)
          ||
          (NotInRange8((*AperTures)[cnt].x,ThickNess)))) cnt++;
    if (cnt<NrAperTures) {
      AperTure=&((*AperTures)[cnt]);
      AperTure->Used++;
    } else {
      if (NrAperTures+1>=MaxNrAperTures) {
        if (AllocateMemAperTures(NrAperTures+64)!=0) return -1;
      }
      AperTure=&((*AperTures)[cnt]);
#ifdef _DEBUG
      if (DcodeTrace==402) {
...

{
  ...
  if (NrAperTures>0) {
    ...
    while ((cnt<NrAperTures)
          &&
          (((*AperTures)[cnt].Info!=GERBER_PAD_ROUND)
          ||
          (NotInRange8((*AperTures)[cnt].x,ThickNess)))) cnt++;
    if (cnt<NrAperTures) {
      AperTure=&((*AperTures)[cnt]);
...

{
  ...
  if (NrAperTures>0) {
    ...
    while ((cnt<NrAperTures)
          ...
          ||
          (NotInRange8((*AperTures)[cnt].x2,CrossHairThickness)))) cnt++;
    if (cnt<NrAperTures) {
      AperTure=&((*AperTures)[cnt]);
      AperTure->Used++;
    } else {
...

{
  ...
  if (NrAperTures>0) {
    if ((InRange8(LastRectPadAperTure.x,x))
       &&
       (InRange8(LastRectPadAperTure.y,y))) {
    ...
    while ((cnt<NrAperTures)
          &&
          (((*AperTures)[cnt].Info!=GERBER_PAD_RECT)
          ||
          (NotInRange8((*AperTures)[cnt].x,x))
          ||
          (NotInRange8((*AperTures)[cnt].y,y)))) cnt++;
    if (cnt<NrAperTures) {
      AperTure=&((*AperTures)[cnt]);
      AperTure->Used++;
    } else {
...
*/
  if (NrAperTures>0) {
    if (ObjectType==0) {
      if ((InRange2(LastPolygonPadAperTure.Rotation,Rotation))
         &&
      ...
      if ((InRange2(LastPolygonPadAperTure.Rotation,Rotation))
         ...
         (LastPolygonPadAperTure.Mirror==Mirror)
         &&
         (InRange(LastPolygonPadAperTure.x2,x2))
         &&
         (InRange(LastPolygonPadAperTure.y2,y2))
         &&
    ...
    for (cnt=0;cnt<NrAperTures;cnt++) {
      if ((*AperTures)[cnt].Info==GERBER_PAD_POLYGON) {
        if (ObjectType!=0) {
          if (((*AperTures)[cnt].SpecialType==ObjectType)
             &&
             (InRange((*AperTures)[cnt].x2,x2))
             &&
             (InRange((*AperTures)[cnt].y2,y2))
             &&
             (InRange2((*AperTures)[cnt].Rotation,Rotation))
             &&
          ...
          if ((InRange2((*AperTures)[cnt].Rotation,Rotation))
             &&
             ((*AperTures)[cnt].Mirror==Mirror)
             &&
...

{
  ...
  if (NrDrillAperTures>0) {
    ...
    while ((cnt<NrDrillAperTures)
          ...
          ||
          (NotInRange8((*DrillAperTures)[cnt].x,ThickNess)))) cnt++;
    if (cnt<NrDrillAperTures) {
      DrillAperTure=&((*DrillAperTures)[cnt]);
      DrillAperTure->Used++;
    } else {
      if (NrDrillAperTures+1>=MaxNrAperTures) {
        if (AllocateMemAperTures(NrDrillAperTures+64)!=0) return -1;
      }
      DrillAperTure=&((*DrillAperTures)[cnt]);
...

{
  ...
  int32 ok,cnt,cnt2,cnt3,Count,Start,End,NrObjects6,
        NrTracePointsNumbers,ObjectNr3,NrCopperObjects;
  ...
  for (cnt=0;cnt<NrCopperObjects;cnt++) {
    ...
    if ((Object4->NetNr>=0)
       &&
       (Object4->NetNr<Design.NrNets)
       &&
       (Object4->ObjectType!=0)) {
...
//  for (cnt=0;cnt<1;cnt++) {
  for (cnt=0;cnt<Design.NrNets;cnt++) {
    Net=&((*Nets)[cnt]);
    if ((cnt % 100)==0) CheckInputMessages(0);
//    if (stricmpOwn(Net->Name,"N$1003")==0) {
//      ok=1;
//    }
    Start=Net->Pos;
...
//              case PIN_SMD_ROUND:
              ...
              case PIN_PUT_THROUGH_SQUARE:
                LineChanged=0;
                d=(min(Object3a->y2,50000)+
                   max(CurrentPlotPenSize,Object3->y2))*0.5;
                if ((InRange4(Object3a->x1,x1))
                   &&
                   (InRange4(Object3a->y1,y1))) {
                  if ((NotInRange(TraceDivX,0.0))
                     &&
                     (NotInRange(TraceDivY,0.0))) {
                    d2=d*sqrt(0.5);
                    if ((fabs(TraceDivX)>d2)
                       &&
                       (fabs(TraceDivY)>d2)) {
                      if (TraceDivX>0) x1+=d2;
                      else x1-=d2;
                      if (TraceDivY>0) y1+=d2;
                      else y1-=d2;
                    } else AddLine=0;
                    LineChanged=1;
                  } else {
                ...
                if ((InRange4(Object3a->x1,x2))
                   &&
                   (InRange4(Object3a->y1,y2))) {
                  if ((NotInRange(TraceDivX,0.0))
                     &&
                     (NotInRange(TraceDivY,0.0))) {
                    d2=d*sqrt(0.5);
                    if ((fabs(TraceDivX)>d2)
                       &&
                       (fabs(TraceDivY)>d2)) {
                      if (TraceDivX>0) x2-=d2;
                      else x2+=d2;
                      if (TraceDivY>0) y2-=d2;
                      else y2+=d2;
                    } else AddLine=0;
                    LineChanged=1;
                  } else {
...

{
  ...
  int32 cnt,cnt2,cnt3,cntx,cnty,cnty2,Divisions,TempObjects4Pos,
        count,Start,End,Length,NrLines,FontNr,MaxCountX,code,lengte,
        cnt4,cnt5,cnt6,Mirror,DrawChar,
        SegmentCount,LineSegments,NrTempObjects4;
  double sx,sy,x1,y1,x2,y2,x3,y3,x4,y4,BoardDivX,BoardDivY,RotationAngle,
         incX,incY;
  ...
  for (cnt=0;cnt<512;cnt++) ObjectsApertureCount[cnt]=0;
  for (cnt=Start;cnt<End;cnt++) {
    Object4=&((*Objects4)[cnt]);
    if ((Object4->Info2!=-1)
       &&
       (Object4->Info2<NrAperTures)) {
      ...
      switch (Object4->ObjectType) {
        case OBJECT_POLYGON:
          ...
          if ((Object4->Info & OBJECT_FILLED) == 0) {
            ...
            if (GerberPlotObject(Object4,0)==-1) PlotErrors++;
            memcpy(Object5,Object4,sizeof(ObjectRecord));
            Object4->Info|=1;
          }
          break;
  ...
  for (cnt2=0;cnt2<NrAperTures;cnt2++) {
    if (ObjectsApertureCount[cnt2]>0) {
      if ((cnt2 % 100)==0) CheckInputMessages(0);
      ...
      for (cntx=0;cntx<Divisions;cntx++) {
        for (cnty=0;cnty<Divisions;cnty++) {
          cnty2=cnty;
          if ((cntx & 1) == 1) cnty2=Divisions-cnty-1;
          ...
          for (cnt=Start;cnt<End;cnt++) {
            Object5=&((*Objects5)[cnt]);
#ifdef _DEBUG
            if ((InRange9(Object5->x1,-88.7e5))
               &&
               (InRange9(Object5->y1,85.7e5))) {
              ok=1;
            }
            if ((Object5->minx<=SearchMaxX)
               &&
               (Object5->maxx>=SearchMinX)
               &&
               (Object5->miny<=SearchMaxY)
               &&
               (Object5->maxy>=SearchMinY)) {
            } else {
              ok=1;
            }
#endif
...
//                          PlotErrors++;
              ...
              switch (Object5->ObjectType) {
                case OBJECT_LINE:
                  if (Object5->Test!=0) {
                    LineSegments=DimensionToLineSegments(x1,y1,x2,y2,(double *)&LineBuf,Object5->Test);
                    SegmentCount=0;
                ...
                case DRILL:
                case DRILL_UNPLATED:
                  if (GerberPlotObject(Object5,0)==-1) PlotErrors++;
                  break;
                case OBJECT_CIRCLE:
                  if ((Object5->Info & OBJECT_FILLED) == 0) {
                ...
                case OBJECT_TEXT2:
                  ...
                  for (cnt3=0;cnt3<NrLines;cnt3++) {
                    if (FontNr==0) {
                      DrawGerberStr2(x1,y1,x2,Object5->Thickness,
                                     RotationAngle,0,Mirror,TextStrings2[cnt3]);
                    } else {
                      incX=0.0;
                      incY=0.0;
                      x4=x1;
                        ...
                        if (DrawChar) {
                          ...
                          for (cnt5=0;cnt5<AreaFill->NrPolygons;cnt5++) {
                            ...
                            for (cnt6=0;cnt6<count;cnt6++) {
                              ...
                              RotatePoint2(&DrawPolygon->Points[cnt6].x,&DrawPolygon->Points[cnt6].y,RotationAngle);
                              if (Mirror==1) {
...

{
  ...
  for (cnt=0;cnt<8;cnt++) {
    if (GerberInfo.TextLine[cnt]!=0) FoundText=1;
  }
  ...
  if ((CurrentAperTureNr<0)
     ||
     (CurrentAperTureNr>=NrAperTures)) {
    return -2;
  }
...

{
  ...
  if (AllocateMemAperTures(1024)!=0) return -1;
  NrDrillAperTures=0;

  sprintf(InfoStr,SC(949,"Drill output"));
  RedrawInfoStr(1);
  GetFilePartFromFileName(FilePart,EditFile);
  CutExtensionFileName(FilePart);
  if (GerberInfo.DrillOutputOption==0) {
    sprintf(FileStr,"%s\\pcb\\gerber\\%s.drl",DesignPath,FilePart);
    if ((Drillfp=FileOpenWriteUTF8(FileStr))<=0) {
      sprintf(str2,SC(405,"Could not write to file %s"),FileStr);
      MessageBoxOwn(PCBWindow,str2,SC(100,"Error"),MB_APPLMODAL|MB_OK);
      return -2;
    }
    sprintf(str,"G90");
    WriteLn(Drillfp,str);
    ...
    sprintf(FileStr,"%s\\pcb\\gerber\\%s.ncd",DesignPath,FilePart);
    if ((GerberInfo.GerberNumberMode & 8) == 0) { // INCH
      switch (GerberInfo.GerberNumberMode & 7) {
        case 2:  // 2 2
          sprintf(str,"\"%s.ncd\" FSLI22",FilePart);
          break;
        case 3:  // 2 3
          sprintf(str,"\"%s.ncd\" FSLI23",FilePart);
          break;
        case 4:  // 2 4
          sprintf(str,"\"%s.ncd\" FSLI24",FilePart);
          break;
        case 5:  // 2 5
          sprintf(str,"\"%s.ncd\" FSLI25",FilePart);
          break;
        case 6:  // 2 6
          sprintf(str,"\"%s.ncd\" FSLI26",FilePart);
          break;
      }
    } else {
      switch (GerberInfo.GerberNumberMode & 7) {
        case 2:  // 3 2
          sprintf(str,"\"%s.ncd\" FSLM32",FilePart);
          break;
        case 3:  // 3 3
          sprintf(str,"\"%s.ncd\" FSLM33",FilePart);
          break;
        case 4:  // 3 4
          sprintf(str,"\"%s.ncd\" FSLM34",FilePart);
          break;
        case 5:  // 4 2
          sprintf(str,"\"%s.ncd\" FSLM42",FilePart);
          break;
        case 6:  // 4 3
          sprintf(str,"\"%s.ncd\" FSLM43",FilePart);
          break;
        case 7:  // 4 4
          sprintf(str,"\"%s.ncd\" FSLM44",FilePart);
          break;
      }
...
{
  int32  cnt,cnt2,cnt3,cnt4,MemPos,ShapeNr,mirror,hulp,fp=0,result,fp2=0,
         LastCompNr,MinObjectNr,MinNr,LastNr,LengthStr,NrRefNums,NrRefCodes,
         NewMode,NrCompProperties,NotPlaced=0,Fiducial,
         FoundFiducials,BottomComponents;
  CompRecord *Comp;
  ...
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],
        ...
        RefCodes[100][12],FileStr[MAX_LENGTH_STRING],FileStr2[MAX_LENGTH_STRING],
  ...
  result=OwnDialogBox(PCBClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_COMP_POS_OUPUT), PCBWindow,
  ...
  if ((fp2=FileOpenWriteUTF8(FileStr2))<=0) {
    sprintf(str2,SC(405,"Could not write to file %s"),FileStr2);
    MessageBoxOwn(PCBWindow,str2,SC(100,"Error"),MB_APPLMODAL|MB_OK);
    return -2;
  }
  FoundFiducials=0;
    ...
    switch (NewMode) {
      case 0:
        if (FoundFiducials) {
          ...
          WriteLn(fp,"# Ref  Position X Y          Rotation Comp value            SMD/Through hole   NP");
        }
//          WriteLn(fp,"# ------------------------------------------------------------------------------------");
        break;
      case 1:
...
//          WriteLn(fp,"# ------------------------------------------------------------------------------------");
      ...
      case 2:
        if (BottomComponents) {
          ...
          WriteLn(fp,"# Ref   Position (    X         Y )  Rot  Comp value          SMD/Through hole   NP");
        }
//          WriteLn(fp,"# ------------------------------------------------------------------------------------");
        break;
      case 3:
        WriteLn(fp2,"######    Top side     #####");
        WriteLn(fp2,"Ref,Position X,Y,Rotation,Comp value,SMD/Through hole,NP");
        break;
      case 4:
        if (BottomComponents) {
...
//            Rotation=115.67;
        ...
        while ((cnt3<NrRefCodes)
              &&
              (stricmpOwn(RefCodes[cnt3],str)!=0)) cnt3++;
        if (cnt3>=NrRefCodes) {
          cnt4=0;
          while ((cnt4<NrRefCodes)
                &&
                (stricmpOwn(RefCodes[cnt4],str)<0)) cnt4++;
          if (cnt4<NrRefCodes) {
            ...
            switch (CompPosOutput.SmdThroughHole) {
              ...
              case 1: // SMD and through hole
                if ((Shape->Info & SMD_DEVICE) == SMD_DEVICE) {
                  ...
                  strcpy(str5,SC(955,"Through hole"));
                }
                break;
            }
  ...
  //          RotateFlipPoint(&ix,&iy,x1,y1,Rotation);
            ...
            if (NewMode<3) {
              switch (CompPosOutput.Units & 3) {
                case 0:
                  sprintf(str,"%-5s   %10.2f%10.2f  %6.2f  %-21s %-18s",
                          Comp->Name,ix/2540,iy/2540,Rotation,str4,str5);
                  break;
                case 1:
                  sprintf(str,"%-5s   %10.3f%10.3f  %6.2f  %-21s %-18s",
                          Comp->Name,ix/100000,iy/100000,Rotation,str4,str5);
                  break;
                case 2:
                  sprintf(str,"%-5s   %10.5f%10.5f  %6.2f  %-21s %-18s",
                          Comp->Name,ix/2540000,iy/2540000,Rotation,str4,str5);
                  break;
              }
              ...
              switch (CompPosOutput.Units & 3) {
                case 0:
                  sprintf(str,"%s\t%.2f\t%.2f\t%.2f\t%s\t%s",
                          Comp->Name,ix/2540,iy/2540,Rotation,str4,str5);
                  break;
                case 1:
                  sprintf(str,"%s\t%.3f\t%.3f\t%.2f\t%s\t%s",
                          Comp->Name,ix/100000,iy/100000,Rotation,str4,str5);
                  break;
                case 2:
                  sprintf(str,"%s\t%.5f\t%.5f\t%.2f\t%s\t%s",
                          Comp->Name,ix/2540000,iy/2540000,Rotation,str4,str5);
                  break;
              }
              if ((CompPosOutput.NotPlaced)
  ...
  sprintf(str,"The following files are generated:\r\n\r\n");
  sprintf(FileStr,"%s\\pcb\\comp_pos.txt",DesignPath);
  sprintf(FileStr2,"%s\\pcb\\comp_pos.csv",DesignPath);
  sprintf(str2,"%s  :  Output component positions as text lines\r\n",FileStr);
  strcat(str,str2);
  sprintf(str2,"%s  :  Output component positions to be used in a spread sheet\r\n",FileStr2);
  strcat(str,str2);
...
{
  int32  cnt,cnt2,cnt3,cnt4,ShapeNr,hulp,
         LastCompNr,MinObjectNr,MinNr,LastNr,LengthStr,NrRefNums,NrRefCodes,
         RefNums[16384],Index[100];
  CompRecord *Comp;
  char  str[MAX_LENGTH_STRING],RefCodes[100][12];
  LPSTR  RefName;

  WriteLn(Filefp,SC(956,"# Components"));
...

{
  ...
  sprintf(FileStr,"%s\\pcb\\gerber\\layers.txt",DesignPath);
  ...
  for (cnt3=0;cnt3<NrGerberLayers;cnt3++) {
    ...
    switch (Layer) {
      case DRILL_LAYER:
        ...
        WriteLn(fp,"---------------------------------------------------------------------");
        if (GerberInfo.DrillOutputOption==0) {
          WriteLn(fp,SC(962,"Drill files :"));
          WriteLn(fp,"---------------------------------------------------------------------");
          sprintf(str,"%s.drl",FileName);
          strcat(str,"                                          ");
          str[42]=0;
          strcat(str,SC(963,"Drill file"));
          WriteLn(fp,str);
          strcpy(str,SC(964,"drills"));
          strcat(str,".txt                                         ");
          str[42]=0;
          strcat(str,SC(965,"Drill tool file"));
          WriteLn(fp,str);
        } else {
          WriteLn(fp,SC(963,"Drill file :"));
          WriteLn(fp,"---------------------------------------------------------------------");
          sprintf(str,"%s.ncd",FileName);
          strcat(str,"                                          ");
          str[42]=0;
          strcat(str,SC(963,"Drill file"));
          WriteLn(fp,str);
  ...
  if (GerberInfo.OutputNeutral==1) {
    ...
    WriteLn(fp,"---------------------------------------------------------------------");
    WriteLn(fp,"");
    sprintf(str,"%s.neu",FileName);
    strcat(str,"                                 ");
    str[36]=0;
    strcat(str,SC(972,"Neutral_file (PCB testing)"));
    WriteLn(fp,str);
  }
  WriteLn(fp,"");
  strcpy(str,"PCB");
  ...
  WriteLn(fp,str);
  sprintf(str,SC(975,"%s: %10.3f  ,%10.3f   mm"),str2,(maxx-minx)/100000,(maxy-miny)/100000);
  WriteLn(fp,str);
  WriteLn(fp,"");
  ...
  WriteLn(fp,str);
  sprintf(str,SC(975,"%s: %10.3f  ,%10.3f   mm"),str2,Design.BoardOriginX/100000,Design.BoardOriginY/100000);
  WriteLn(fp,str);
  WriteLn(fp,"");
  WriteLn(fp,"");
  ...
  strcat(str,"                                      ");
  str[36]=0;
  sprintf(str2,SC(982,": %.0f mils (%.4f mm)"),
               Design.StandardTraceWidth/2540,Design.StandardTraceWidth/100000);
  strcat(str,str2);
  WriteLn(fp,str);
  ...
  str[36]=0;
  sprintf(str2,SC(982,": %.0f mils (%.4f mm)"),
               Design.StandardClearance/2540,Design.StandardClearance/100000);
  strcat(str,str2);
  WriteLn(fp,str);
  FileClose(fp);
  if (WriteLnError!=0) {
...

{
  ...
  int32           Count,cnt2,cnt3,PinOffset,MemPos,cnt,
                  NrPins,PinNr,NetCount,Layer2,PinsMemPos,MemPosComp,
                  ShapeInfo,NrPinShapes,ShapeType,ShapeNr,NetNr,fp;
  double          x1,y1,x2,y2;
  int32            Found;
  CompRecord      *Comp;
  ObjectRecord    *Object;
...
//        PinNr++;
  ...
  if (AllocateMemNetItems(Count)==-1) return -1;
  NrNetItems=Count;
...
//      }
  ...
  if ((mode & 1)==0) {
    ...
    sprintf(str,"BOARD %s OFFSET x:%.1f y:%.1f ORIENTATION    0",
                str2,
                Design.BoardOriginX/2540.0,
                Design.BoardOriginY/2540.0);
  } else {
    WriteLn(fp,"B_UNITS Mm");
    sprintf(str,"BOARD %s OFFSET x:%.4f y:%.4f ORIENTATION    0",
                str2,
                Design.BoardOriginX/100000.0,
          ...
          if (Object->NetNr==cnt2) {
            if (NetItem->PinNr==Object->PinNr) {
              switch (Object->ObjectType) {
                case PIN_PUT_THROUGH_ROUND:
                  if ((mode & 1)==0) {
                    sprintf(str,"N_PIN %s %7.1f %7.1f %03.0frd%03.0f    0",
                                str3,Object->x1/2540,Object->y1/2540,
                                Object->x2/2540,Object->y2/2540);
                  } else {
                    sprintf(str,"N_PIN %s %8.4f %8.4f %03.0frd%03.0f    0",
                                str3,Object->x1/100000,Object->y1/100000,
                                Object->x2/1000,Object->y2/1000);
                  }
                  WriteLn(fp,str);
                  break;
                case PIN_PUT_THROUGH_SQUARE:
                  if ((mode & 1)==0) {
                    sprintf(str,"N_PIN %s %7.1f %7.1f %03.0fsq%03.0f    0",
                                str3,Object->x1/2540,Object->y1/2540,
                                Object->x2/2540,Object->y2/2540);
                  } else {
                    sprintf(str,"N_PIN %s %8.4f %8.4f %03.0fsq%03.0f    0",
                                str3,Object->x1/100000,Object->y1/100000,
                                Object->x2/100000,Object->y2/100000);
                  }
                  WriteLn(fp,str);
                  break;
                case PIN_PUT_THROUGH_POLYGON:
                ...
                case PIN_SMD_ROUND:
                  ...
                  if ((mode & 1)==0) {
                    sprintf(str,"N_PIN %s %7.1f %7.1f %03.0frd       %i",
                                str3,Object->x1/2540,Object->y1/2540,
                                Object->x2/2540,Layer2);
                  } else {
                    sprintf(str,"N_PIN %s %8.4f %8.4f %03.0frd       %i",
                                str3,Object->x1/100000,Object->y1/100000,
                                Object->x2/1000,Layer2);
                  }
                  WriteLn(fp,str);
                  break;
                case PIN_LINE_HOR:
                  ...
                  if ((mode & 1)==0) {
                    sprintf(str,"N_PIN %s %7.1f %7.1f %03.0frd       %i",
                                str3,(Object->x1+Object->x2*0.5)/2540,Object->y1/2540,
                                Object->y2/2540,Layer2);
                  } else {
                    sprintf(str,"N_PIN %s %8.4f %8.4f %03.0frd       %i",
                                str3,(Object->x1+Object->x2*0.5)/100000,Object->y1/100000,
                ...
                case PIN_LINE_VER:
                  ...
                  if ((mode & 1)==0) {
                    sprintf(str,"N_PIN %s %7.1f %7.1f %03.0frd       %i",
                                str3,Object->x1/2540,(Object->y1+Object->x2*0.5)/2540,
                                Object->y2/2540,Layer2);
                  } else {
                    sprintf(str,"N_PIN %s %8.4f %8.4f %03.0frd       %i",
                                str3,Object->x1/100000,(Object->y1+Object->x2*0.5)/100000,
                                Object->y2/1000,Layer2);
                  }
                  WriteLn(fp,str);
                  break;
                case PIN_LINE_DIAG1:
                  ...
                  if ((mode & 1)==0) {
                    sprintf(str,"N_PIN %s %7.1f %7.1f %03.0frd       %i",
                                str3,(Object->x1+Object->x2*0.5)/2540,(Object->y1-Object->x2*0.5)/2540,
                                Object->y2/2540,Layer2);
                  } else {
                    sprintf(str,"N_PIN %s %8.4f %8.4f %03.0frd       %i",
                                str3,(Object->x1+Object->x2*0.5)/100000,(Object->y1-Object->x2*0.5)/100000,
                                Object->y2/1000,Layer2);
                  }
                  WriteLn(fp,str);
                  break;
                ...
                case PIN_ARC:
                  ...
                  if ((mode & 1)==0) {
                    sprintf(str,"N_PIN %s %7.1f %7.1f %03.0frd       %i",
                                str3,x1/2540,y1/2540,
                                Object->Thickness/2540,Layer2);
                  } else {
                    sprintf(str,"N_PIN %s %8.4f %8.4f %03.0frd       %i",
                                str3,x1/100000,y1/100000,
                                Object->Thickness/1000,Layer2);
                  }
                  WriteLn(fp,str);
                  if ((mode & 1)==0) {
                    sprintf(str,"N_PIN %s %7.1f %7.1f %03.0frd       %i",
                                str3,x2/2540,y2/2540,
                                Object->Thickness/2540,Layer2);
                  } else {
                    sprintf(str,"N_PIN %s %8.4f %8.4f %03.0frd       %i",
                                str3,x2/100000,y2/100000,
                                Object->Thickness/1000,Layer2);
                  }
                  WriteLn(fp,str);
                  break;
                case PIN_SMD_RECT:
                  Layer2=Design.NrBoardLayers-Object->Layer;
                  if ((mode & 1)==0) {
                    sprintf(str,"N_PIN %s %7.1f %7.1f smd_%03.0fx%03.0f %i",
                                str3,Object->x1/2540,Object->y1/2540,
                                Object->x2/2540,Object->y2/2540,Layer2);
                  } else {
                    sprintf(str,"N_PIN %s %8.4f %8.4f smd_%03.0fx%03.0f %i",
                                str3,Object->x1/100000,Object->y1/100000,
                                Object->x2/1000,Object->y2/1000,Layer2);
                  }
                  WriteLn(fp,str);
                  break;
                case PIN_SMD_POLYGON:
                  Layer2=Design.NrBoardLayers-Object->Layer;
                  if ((mode & 1)==0) {
                    sprintf(str,"N_PIN %s %7.1f %7.1f smd_%03.0frd%03.0f    %d",
                                str3,Object->x1/2540,Object->y1/2540,
                                16.0,16.0,Layer2);
                  } else {
                    sprintf(str,"N_PIN %s %8.4f %8.4f smd_%03.0frd%03.0f    %d",
                                str3,Object->x1/100000,Object->y1/100000,
                                0.4e3,0.4e3,Layer2);
                  }
                  WriteLn(fp,str);
...
// Insert vias
      for (cnt3=0;cnt3<Design.NrVias;cnt3++) {
        ...
        if (((Via->Info & (OBJECT_NOT_VISIBLE)) == 0)
           ...
           (Via->NetNr==cnt2)) {
          if ((mode & 1)==0) {
            sprintf(str,"N_VIA                  %7.1f %7.1f %03.0frd%03.0f    1    %d",
                        Via->X/2540,Via->Y/2540,
                        Via->ThickNess/2540,Via->DrillThickNess/2540,Design.NrBoardLayers);
          } else {
            sprintf(str,"N_VIA                  %8.4f %8.4f %03.0frd%03.0f    1    %d",
                        Via->X/100000,Via->Y/100000,
                        Via->ThickNess/1000,Via->DrillThickNess/1000,Design.NrBoardLayers);
          }
          WriteLn(fp,str);
        }
      }
...

{
  ...
  int32 ok,cnt,cnt2,cntx,cnty,cnty2,Divisions,count,Start,End,NrTempObjects4,
        DrillApertureIndex[512],DrillApertureCount[512];
  ...
  Objects5=(ObjectArray *)&((*Objects4)[NrObjects4]);
  for (cnt=0;cnt<512;cnt++) DrillApertureCount[cnt]=0;
  for (cnt=0;cnt<NrObjects4;cnt++) {
    Object4=&((*Objects4)[cnt]);
    if ((Object4->Info2!=-1)
       &&
       (Object4->Info2<NrDrillAperTures)) {
...
typedef struct {
  ...
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
        str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING];
  ...
  if ((fp=FileOpenWriteUTF8(str))<=0) {
    sprintf(str2,SC(405,"Could not write to file %s"),str);
    MessageBoxOwn(PCBWindow,str2,SC(100,"Error"),MB_APPLMODAL|MB_OK);
    return;
  }
...
//      WriteLn(fp,";");
    ...
    switch (AperTure->Info) {
      case GERBER_TRACE:
        strcpy(str4,SC(984,"Round"));
        strcat(str4,"           ");
        str4[11]=0;
        sprintf(str3,"D%-3i    %s%7.2f            0.0           %i",
      ...
      case GERBER_PAD_RECT:
        strcpy(str4,SC(985,"Rectangle"));
        strcat(str4,"           ");
        str4[11]=0;
        sprintf(str3,"D%-3i    %s%7.2f         %7.2f          %i",
                             AperTure->AperTureCode,
                             str4,
                             AperTure->x/2540.0,
                             AperTure->y/2540.0,
...
//  while ((cnt=FindMinApertureDrill())!=-1) {
    ...
    if (mode==0) {
      ...
      strcat(str3,"           ");
      str3[12]=0;
      sprintf(str2,"  T%-3i       %s%6.3f   %6.2f       %5i",
                   DrillAperTure->AperTureCode,
                   str3,
                   (DrillAperTure->x+500)/100000.0,
                   DrillAperTure->x/2540.0,
                   DrillAperTure->Used);
      WriteLn(fp,str2);
    } else {
      if ((GerberInfo.GerberNumberMode & 8) == 0) { // INCH
