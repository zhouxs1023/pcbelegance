...
double  LineCrossX,LineCrossY,DrawPolygonLineThickNess,TempAreaFillClearance,
        ShiftX,ShiftY,OldShiftX,OldShiftY,OldX,OldY,CurrentX,CurrentY,
        NewAreaFillClearance;
...

{
  ...
  while ((NrTries)
        &&
        (((RandValue=(((double)rand()/RAND_MAX)))<0.1)
        ||
        (RandValue-OldValue2<0.0005)
        ||
...
//      sprintf(&str,"%i",cnt);
//      DrawStr(x1,y1,PixelToReal(16),0,1,&str);
...

{
  ...
  for (cnt=0;cnt<count;cnt++) {
    ...
    if ((Xmax>=ViewMinX)
      ...
      sprintf(str,"%i",cnt);
//      DrawStr(x1,y1,PixelToReal(16),0,1,0,str);
      SetROP2(OutputDisplay,R2_COPYPEN);
...

{
  ...
  for (cnt=0;cnt<count;cnt++) {
    ...
    if ((Xmax>=ViewMinX)
      ...
      sprintf(str,"%i",cnt);
//      DrawStr(x1,y1,PixelToReal(16),0,1,0,str);
      TextOut(OutputDisplay,MultX(x1)+2,MultY(y1)+7,str,strlen(str));
      SetROP2(OutputDisplay,R2_COPYPEN);
...
// PolygonObjectType
  ...
  if ((InRange(NewObjectLine.X1,NewObjectLine.X2))
     &&
     (InRange(NewObjectLine.Y1,NewObjectLine.Y2))) {
    ok=1;
  }
...
//    FirstY=NewObjectLine.Y1;
  ...
  DrawLine(MultX(NewObjectLine.X1),MultY(NewObjectLine.Y1),
           MultX(NewObjectLine.X2),MultY(NewObjectLine.Y2));
...

{
  switch (ObjectType) {
    ...
    case OBJECT_RECT:
      if (Units==0) {
        sprintf(str," [ %.1f x %.1f ]",AreaFillObjectRect.Width/2540.0,
                                       AreaFillObjectRect.Height/2540.0);
      } else {
        sprintf(str," [ %.4f x %.4f ]",AreaFillObjectRect.Width/100000.0,
                                       AreaFillObjectRect.Height/100000.0);
      }
      break;
    case OBJECT_CIRCLE:
      if (Units==0) {
...

{
  if ((!ShiftPressed)
     ||
     (PolygonDrawObjectType==OBJECT_POLYLINE)) {
...
{
  int32   cnt,cnt2,cnt3,ok,InsertMode,OkToAdd,Mode2,count,Mode3,
          CurrentAreaFillNr,AreaFillDeletionCount,count2;
...
//  OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_POPUP_ESCAPE,SC(493,"Escape"));
  ...
  switch (Mode & 0x0f) {
    ...
    case 1:
      ...
      if (CurrentAreaFillNr==-1) return -1;
      break;
    case 2:
      if ((CurrentAreaFillNr=GetPowerPlaneByLayer(Layer))==-1) return -1;
      break;
  }
  OldValue2=0.1123;
        ...
        case PIN_LINE_VER:
          ...
          switch (ObjectType) {
            case OBJECT_RECT:
              ...
              if ((InRange(AreaFillObjectRect.Width,0.0))
                 ||
                 (InRange(AreaFillObjectRect.Height,0.0))) {
...
//    if (RightButtonPressed) {
      ...
      switch (ObjectType) {
        case OBJECT_POLYLINE:
          PopUpMenu=CreatePopupMenu();
          if (DrawPolygon2->NrVertices>1) {
            OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_FINISH_POLYGON,SC(990,"Finish"));
          }
          ...
          if (DrawPolygon2->NrVertices>0) {
            OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_TRACE_BACKWARDS,SC(993,"Goto previous point"));
          }
          if (Mode==0) {
            OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_STRING,ID_CLEARANCE_WIDTH_DIALOG,SC(1274,"Change drawing thickness"));
          }
          if (CrossHairType==1) {
            OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_BITMAP,ID_VIEW_CROSS_TYPE1,(LPSTR)BitMapCross1);
          } else {
            OwnAppendMenu(PopUpMenu,MF_ENABLED|MF_BITMAP,ID_VIEW_CROSS_TYPE2,(LPSTR)BitMapCross2);
          }
          break;
        case OBJECT_RECT:
        case OBJECT_CIRCLE:
...
//      RightButtonPressed=0;
      CheckInputMessages(0);
      if (!SelectionEsc) DrawTryingPolygonObject(CurrentX,CurrentY,Mode2);
      ok=1;
    }
// ****************************************************************************
...
//            if (LineInputDialogLong(TextLine,"Add polygon points x1,x2,y2,x3,y3,x4,y4, .... )")==1) {
              ...
              while ((*TextP!=0)
                ...
                if (*TextP!=0) {
                  ...
                  if (*TextP==',') TextP++;
                  if (*TextP=='\r') TextP++;
                  if (((NrParams=ScanParameters(-1,str,0))>=2)
                     &&
                     ((NrParams & 1) == 0)) {
...

{
  ...
  for (cnt=0;cnt<NrCrossPoints;cnt++) {
    ...
    while ((cnt2>0)
          &&
          (CrossPoints[cnt].LineNrPolygonB<Sort[cnt2-1])) cnt2--;
    if (cnt2<cnt) memmove(&Sort[cnt2+1],&Sort[cnt2],(cnt-cnt2)*sizeof(int32));
    Sort[cnt2]=CrossPoints[cnt].LineNrPolygonB;
  }
  cnt=0;
  while ((cnt<NrCrossPoints)
        &&
        (Sort[cnt]!=Nr)) cnt++;
  if (cnt<NrCrossPoints) {
    if (cnt<NrCrossPoints-1) return Sort[cnt+1];
    else return Sort[0];
  } else {
    ok=1;
  }
...

{
  ...
  while ((cnt<NrCrossPoints)
        ...
        (Sort[cnt]!=Nr)) cnt++;
  if (cnt<NrCrossPoints) {
    if (cnt>0) return Sort[cnt-1];
    else return Sort[NrCrossPoints-1];
  } else {
    ok=1;
    return 0;
  }
...

{
  ...
  while ((cnt<NrCrossPoints)
        &&
        (CrossPoints[cnt].LineNrPolygonB!=Nr)) cnt++;
  if (cnt<NrCrossPoints) return cnt;
  else return 0;
...

{
  ...
  for (cnt=0;cnt<NrCrossPoints;cnt++) {
    ...
    while ((cnt2>0)
          &&
          (CrossPoints[cnt].LineNrPolygonA<Sort[cnt2-1])) cnt2--;
    if (cnt2<cnt) memmove(&Sort[cnt2+1],&Sort[cnt2],(cnt-cnt2)*sizeof(int32));
...

{
  ...
  while ((cnt<NrCrossPoints)
        &&
        (CrossPoints[cnt].LineNrPolygonA!=Nr)) cnt++;
  if (cnt<NrCrossPoints) return cnt;
  else return 0;
...
{
  int32  cnt4,cnt3,cnt2,cnt,count,count2,res,NrCrosses,InsertPos,Pos,
         CountOnSameLine,Found,NrTries,LineCnt2,LineCnt3,LineCnt2a,LineCnt3a,
         DisabledPos,NextPoint;
  double  *x1,*y1,*x2,*y2,*FirstX,*FirstY,x3,y3,x4,y4,
          MinDist,DivX,DivY,cx,cy,prevx,prevy;
  ...
  while ((Changed)
    ...
    if (NrCrosses>MAX_NR_CROSSES) return NrCrosses;
    ...
    if ((NrCrosses & 1) == 1) {
      ...
      for (cnt=0;cnt<NrCrosses;cnt++) {
        ...
        sprintf(str,"Cross nr %i = %.5f,%.5f\n",cnt,
                            CrossPoints[cnt].CrossX,CrossPoints[cnt].CrossY);
        OutputDebugString(str);
#endif
        LineCnt2=CrossPoints[cnt].LineNrPolygonA;
        LineCnt2a=LineCnt2;
        if (LineCnt2a<count-1) LineCnt2a++;
        else LineCnt2a=0;

#ifdef _DEBUG
        sprintf(str,"   line 1 %.5f,%.5f to %.5f,%.5f\n",
                            ResultPolygon->Points[LineCnt2].x,
                            ResultPolygon->Points[LineCnt2].y,
                            ResultPolygon->Points[LineCnt2a].x,
                            ResultPolygon->Points[LineCnt2a].y);
        OutputDebugString(str);
        DrawLineRed(PolygonObject->Points[LineCnt2].x,
                    PolygonObject->Points[LineCnt2].y,
                    PolygonObject->Points[LineCnt2a].x,
                    PolygonObject->Points[LineCnt2a].y);
#endif

        LineCnt3=CrossPoints[cnt].LineNrPolygonB;
        LineCnt3a=LineCnt3;
        if (LineCnt3a<count2-1) LineCnt3a++;
        else LineCnt3a=0;
#ifdef _DEBUG
        sprintf(str,"   line 2 %.5f,%.5f to %.5f,%.5f\n",
                            PolygonObject->Points[LineCnt3].x,
                            PolygonObject->Points[LineCnt3].y,
                            PolygonObject->Points[LineCnt3a].x,
                            PolygonObject->Points[LineCnt3a].y);
        OutputDebugString(str);
        DrawLineGreen(PolygonObject->Points[LineCnt3].x,
                      PolygonObject->Points[LineCnt3].y,
                      PolygonObject->Points[LineCnt3a].x,
                      PolygonObject->Points[LineCnt3a].y);
    ...
    if ((NrCrosses & 1) == 1) return NrCrosses;
...
// Find first cross closest to start of line
          ...
          while (cnt4<CountOnSameLine) {
            if ((DistanceOnSameLine[cnt4].Test==0)
               &&
               (DistanceOnSameLine[cnt4].Distance<MinDist)) {
              Found=cnt4;
...
//      return 6;
    ...
    if (mode==1) return NrCrosses;
  }
  for (cnt=0;cnt<NrCrosses;cnt++) {
    if ((CrossPoints[cnt].LineNrPolygonA>10000)
       ||
       (CrossPoints[cnt].LineNrPolygonA<-10000)
       ||
       (CrossPoints[cnt].LineNrPolygonB>10000)
       ||
       (CrossPoints[cnt].LineNrPolygonB<-10000)) {
#ifdef _DEBUG
...
int32 GetPolygonInsertPointsForAppendDelete(PolygonRecord *CopyPolygon,
                                            PolygonRecord *AppendPolygon,int32 mode)


{
  int32 NrVerticesCopyPolygon,NrVerticesAppendPolygon,NextCrossNr,CurrentPos,cnt,
        NextNr,LineNrPolygonA,LineNrPolygonB,PolygonCount,StartNr,TotalPolygons,
        NextPolygonALineNr,NextPolygonBLineNr,res,count;
...
//        } VerticeCopy2Record;
  ...
  while (PolygonCount<NrCrossPoints) {
    ...
    if ((res & 1) == mode) {  // Point PolygonBLineNr+1 outside PolygonA (mode = 0) Append
...
// *********************************************************************************************
    ...
    if ((res & 1) == 0) {     // Point PolygonALineNr+1 outside PolygonB
      NextNr=FindNextNrPolygonA(LineNrPolygonA);
      NextCrossNr=FindCrossIndexNrPolygonA(NextNr);
      if ((count=NextNr-LineNrPolygonA)<0) count+=NrVerticesCopyPolygon;
      VerticeCopy2.LineNrPolygon[PolygonCount]=NextPolygonALineNr;
      VerticeCopy2.Count[PolygonCount]=count;
    } else {                  // Point PolygonALineNr+1 inside  PolygonB
      NextNr=FindPreviousNrPolygonA(LineNrPolygonA);
      NextCrossNr=FindCrossIndexNrPolygonA(NextNr);
      if ((count=LineNrPolygonA-NextNr)<0) count+=NrVerticesCopyPolygon;
...
// Find next unused cross nr
        ...
        while ((cnt<NrCrossPoints)
              &&
              (CrossPoints[cnt].Test==1)) cnt++;
        if (cnt<NrCrossPoints) {
          StartNr=cnt;
          CurrentPos=cnt;
        }
      }
    } else CurrentPos=NextCrossNr;
  }
  for (cnt=0;cnt<NrCrossPoints;cnt++) {
    if ((VerticeCopy2.Count[cnt]>10000)
       ||
       (VerticeCopy2.Count[cnt]<-10000)) {
      res=1;
    }
  }
...
void IncludePolygonInPolygon2(PolygonRecord *StartPolygon,
                              PolygonRecord *AppendDeletePolygon)

{
  int32 NrVerticesStartPolygon,NrVerticesAppendDeletePolygon,ResultP,NextCrossNr,
        WorkPolygonNr,cnt,res,count;
  PointRecord *StartPolygonPoints,*AppendDeletePolygonPoints,*ResultPolygonPoint,
              *CrossPoint;

  NrVerticesStartPolygon=StartPolygon->NrVertices;
  NrVerticesAppendDeletePolygon=AppendDeletePolygon->NrVertices;
  if ((NrVerticesStartPolygon==0)
     ||
     (NrVerticesAppendDeletePolygon==0)) {
    res=1;
  }
...
// ***********************************************************************************************
  ...
  NextCrossNr=0;
  for (cnt=0;cnt<4;cnt++) WorkPolygon[cnt]->NrVertices=0;
...
// Insert first cross point
  ...
  // Insert points polygonA ()
    ...
    count=VerticeCopy2.Count[NextCrossNr];
    if (count>0) {
      AppendVertices(VerticeCopy2.LineNrPolygon[NextCrossNr],count,NrVerticesStartPolygon,
                     StartPolygonPoints,ResultPolygonPoint,0);
    } else {
      count=-count;
      AppendVertices(VerticeCopy2.LineNrPolygon[NextCrossNr],count,NrVerticesStartPolygon,
                     StartPolygonPoints,ResultPolygonPoint,1);
    }
    ResultPolygonPoint+=count;
    ResultP+=count;
  ...
  // ***********************************************************************************************
    ...
    if ((NextCrossNr<NrCrossPoints)
       &&
       (VerticeCopy2.Mode[NextCrossNr-1]!=VerticeCopy2.Mode[NextCrossNr])) {
      WorkPolygonNr++;
      ResultP=0;
      ResultPolygon=(PolygonRecord *)WorkPolygon[WorkPolygonNr];
      ResultPolygonPoint=(PointRecord *)&(WorkPolygon[WorkPolygonNr]->Points[0]);
...

{
  ...
  for (cnt=0;cnt<NrPolygons;cnt++) {
    ...
    Area=(WorkPolygon[cnt]->maxx-WorkPolygon[cnt]->minx)*
         (WorkPolygon[cnt]->maxy-WorkPolygon[cnt]->miny);
    if (Area>MaxArea) {
      MaxArea=Area;
      MaxNr=cnt;
    }
...
*/
  ...
  if (mode==0) {   // Check smaller polygon is inside a big polygon
    ...
    if (FirstPolygonCheckOk) { // The polygon with the max NrVertices encloses the other polygons
      CopyPolygonToPolygon(WorkPolygon[MaxNr],NewPolygonObject);
      if (NrPolygons==1) return 0;
      else return 1;
    } else {  // Find the polygon which encloses the others
      res=1;
      switch (NrPolygons) {
        case 1:
          break;
...
{
  int32  cnt2,count,NrCrosses,TotalCount,res,NrPolygons,ok,
         VerticesCount,MemSizeAreaFill,PolygonResult,Mem2,
...
// PolygonType bit 2 = 1  -> polygon deletion because of user deletion
  ...
  for (cnt2=0;cnt2<TempAreaFill->NrPolygons;cnt2++) {
    if (cnt2==0) {
      if ((PolygonObject->maxx+100<SubPolygon->minx)
         ||
         (PolygonObject->minx-100>SubPolygon->maxx)
         ||
      ...
      if ((SubPolygon->PolygonType & 8) == 0) {
        if ((PolygonObject->maxx+100<SubPolygon->minx)
           ...
           (PolygonObject->maxy+100<SubPolygon->miny)
           ||
           (PolygonObject->miny-100>SubPolygon->maxy)) OverlappedPolygon=1;
      }
    }
...
*/
        ...
        if ((ExtraPolygon->maxx+100<NewPolygon->minx)
           ||
           (ExtraPolygon->minx-100>NewPolygon->maxx)
           ||
           (ExtraPolygon->maxy+100<NewPolygon->miny)
           ||
           (ExtraPolygon->miny-100>NewPolygon->maxy)) {
          NrCrosses=0;
        } else {
...
// ********************************************************************************************
          ...
          if (NrCrosses!=0) {
            if (NrCrosses==-1) return -10;
            if (NrCrosses==-2) return -11;
            return -20;
          }
        }
// ********************************************************************************************
...
//      NrCrosses=CollectCrossesPolygons2(NewPolygon,ExtraPolygon,0);
      if (NrCrosses==-1) return -12;
      if (NrCrosses==-2) return -13;
      return -60;
    }
  }
...
#endif
  if (CheckPolygonInsidePolygon(NewPolygon,SurroundPolygon)==1) { // NewPolygon in SurroundPolygon
...
// ********************************************************************************************
    ...
    if (SubPolygonChanged) { // Copy the SubPolygons (Test=0) to the new AreaFill
...
int32 MakeNewAreaFill(int32 NetNr,int32 Layer,
                      int32 Info,
                      double ThickNess,
                      double ThermalReliefThickness,
                      double ThermalReliefDistance,
                      double Clearance,int32 mode)
...
//  CopyPolygonToPolygon(DrawPolygon,CurrentPolygon);
  ...
  count=DrawPolygon2->NrVertices;
  if (!CheckNoCrossesInPolygon(DrawPolygon2)) return 0;
  for (cnt=0;cnt<count;cnt++) {
...
//  memmove(&((*NewAreaFill).StartPolygon),&((*DrawPolygon2).Points),
//          min(200*8,count*8));
#ifdef _DEBUG
...
// Make AreaFill with a smaller StartPolygon
    ...
    if (!CheckNoCrossesInPolygon(SmallerPolygon)) return 0;
/*
    StartDrawingEditingWindow();
...
//                              ((TryCount+RandValue)*45.34),Found,0);
          MakePolygonFromObject(Object,PolygonObject,Clearance,
...
*/
        ...
        if (((NewAreaFill->Info & (AREAFILL_WITH_THERMAL_RELIEF|POWERPLANE)) == AREAFILL_WITH_THERMAL_RELIEF)
           ...
           &&
           (NotInRange(NewAreaFill->ThermalReliefThickness,0.0))
           &&
           ((Object->Info2 & 1) == 0)) {
          ...
          for (cnt5=0;cnt5<4;cnt5++) {
            if (CopyThermalReliefInPolygon(Object,PolygonObject,
                NewAreaFill->ThermalReliefThickness,(cnt+1),Found,cnt5)==0) {
              memmove(AreaFillMemTemp2,NewAreaFill,NewAreaFill->MemSize);
// PolygonType bit 0 = 0  -> polygon deletion because of copper
// PolygonType bit 0 = 1  -> polygon deletion because of user deletion/thermal relief
// PolygonType bit 2 = 0  -> polygon deletion because of thermal relief
...
{
  int32  ok,cnt,cnt2,count,cnt5,UsePolygon,mc,StartPinObjects,TryCount,res,
         MemSizePolygonObject,Found,Layer,NetNr;
  double RandValue,x1a,y1a,x1,y1,x2,y2;
  PolygonRecord  *AreaFillPolygon,*SubPolygon,*NewSubPolygon,*PreviousPolygon,
                 *ObjectDrawPolygon,*PolygonObject;
  ObjectRecord   *Object,ErrorObject;
...
// ****************************************************************************
  ...
  if ((fp=FileOpenWriteUTF8(str))<=0) return -5;
#endif
...
//    if ((Object->x1>
  ...
  //    Comp=(CompRecord *)Object->TextP;
      ...
      if ((InRange9(Object->x1,170.5e5))
         &&
         (InRange9(Object->y1,73.0e5))) {
        res=1;
      ...
      if (((NewAreaFill->Info & (AREAFILL_WITH_THERMAL_RELIEF|POWERPLANE)) == AREAFILL_WITH_THERMAL_RELIEF)
        ...
        for (cnt5=0;cnt5<4;cnt5++) {
          ...
          while ((res<0)
            ...
            if (CopyThermalReliefInPolygon(Object,PolygonObject,
                                           NewAreaFill->ThermalReliefThickness,
                                           (cnt+1+RandValue+(TryCount*10.0)),
                                           Found,cnt5)==0) {
              memmove(AreaFillMemTemp2,NewAreaFill,NewAreaFill->MemSize);
// PolygonType bit 0 = 0  -> polygon deletion because of copper
// PolygonType bit 0 = 1  -> polygon deletion because of user deletion/thermal relief
    ...
    for (cnt2=0;cnt2<AreaFillToChange->NrPolygons;cnt2++) {
      if (cnt2>0) {
        ...
        if (((SubPolygon->PolygonType & 8)==8)   // User deletion polygon that crosses the
           ||                                    // main polygon
           ((SubPolygon->PolygonType & 5)==5)) { // User deletion polygon encapsulated in
                                                 // the main polygon
          UsePolygon=1;
          if (PreviousPolygon) {
            if (PreviousPolygon->NrVertices==SubPolygon->NrVertices) {
              if ((InRange((PreviousPolygon->minx+PreviousPolygon->maxx)*0.5,
                           (SubPolygon->minx+SubPolygon->maxx)*0.5))
                 &&
                 (InRange((PreviousPolygon->miny+PreviousPolygon->maxy)*0.5,
  ...
  if (!FoundError) {
    ...
    if (!SelectionEsc) {
      ...
      if ((ProblemPolygon1)
         &&
         (ProblemPolygon2)) {
        strcpy(str,SC(352,"Do you want to copy the problem polygon outlines to layers INFO3,4"));
        if (MessageBoxOwn(PCBWindow,str,SC(100,"Error"),MB_APPLMODAL|MB_YESNO)==IDYES) {
...
void RotateObjectPolygon(ObjectPolygonRecord *ObjectPolygon,double CentreX,double CentreY,
...

{
  ...
  count=ObjectPolygon->NrVertices;
  for (cnt3=0;cnt3<count;cnt3++) {
    RotatePointFromOtherPoint2(&(*ObjectPolygon).Points[cnt3].x,&(*ObjectPolygon).Points[cnt3].y,
                               CentreX,CentreY,Rotation);
  }
}
...
int32 PlaceMovedAreafill(int32 AreafillNr,
...
int32 PlaceMovedAreafills(double CurrentX,double CurrentY,
                          double CurrentX2,double CurrentY2,
                          double CentreSelectedX,double CentreSelectedY,
                          double Rotation,uint8 *SelectedNetsForMove,
                          int32 RepaintMode)
...

{
  ...
  if (IsLayerPowerPlane(AreaFill->Layer)) return -1;
...
*/
  if (((mode & 2)==0)
     &&
     (CheckPolygonOverlapAreaFill(DrawPolygon2,AreaFill)==0)) {
    return 0;
  }
...
#endif
  if (res==-5) {
    MessageBoxOwn(PCBWindow,SC(1002,"To complex"),SC(100,"Error"),MB_APPLMODAL|MB_OKCANCEL);
  }
  if (res<0) {
    FoundError=1;
  }
...

{
  ...
  if (res>=0) {
    ...
    memmove(&NewObjectPolygon.Points,&((*AreaFillPolygon).Points),AreaFillPolygon->NrVertices*sizeof(PointRecord));
    if (AddObjectPolygon(&NewObjectPolygon)) {
      ...
      ObjectPolygon2=(ObjectPolygonRecord *)&(ObjectPolygonMem[(*ObjectPolygons)[Design.NrObjectPolygons-1]]);
    }
    RePaint();
  }
...

{
  ...
  AreaFillRecord *AreaFill;
  uint8   *PolygonPos,*NewPolygonPos;
  PolygonRecord *FirstPolygon,*AreaFillPolygon,*SubPolygon,
                *DrawPolygon2,*NewSubPolygon;
  ...
  if (CommandAddPolygonLines(0.0,-1,0,OBJECT_POLYLINE)==-1) return 0;
...

{
  ...
  if (!OkToDrawAreaFills) return;
  if ((Found=GetPowerPlaneByLayer(Layer))==-1) return ;
  AreaFill=(AreaFillRecord *)&(AreaFillMem[(*AreaFills)[Found]]);
  if (mode==0) {
    NetNr=AreaFill->NetNr;
    if (AreaFill->NetNr==-1) {
    ...
    if (!CheckNoCrossesInPolygon(BiggerPolygon)) return;
    for (cnt=0;cnt<count;cnt++) {
    ...
    if (DeleteFromAreaFill(Found,1)==0) return;
/*
    AreaFill=(AreaFillRecord *)&(AreaFillMem[(*AreaFills)[Found]]);
    StartDrawingEditingWindow();
...

{
  ...
  if (CheckIfLayerIsAPowerPlane(Layer)) {
    ...
    if (mode==0) {
      if (AreaFillDialog(DefaultAreaFill,0)==-1) return 0;
    } else {
    ...
    if (mode==0) {
      if (AreaFillDialog(DefaultAreaFill,1)==-1) return 0;
    } else {
      DefaultAreaFill->NetNr=-1;
    }
    ThickNess=0.0;
//    ThickNess=DefaultAreaFill->Clearance;
  ...
  if (NetNr>=0) {
    ...
    Net=&((*Nets)[NetNr]);
    CurrentDrawingNetNr=NetNr;
    if (PowerPlaneNetNr!=-1) AddPowerPlaneObjects3(PowerPlaneNetNr,0);
    strcpy(InfoStr,Net->Name);
    RedrawInfoStr(1);
  } else {
    DefaultAreaFill->Clearance=0.0;
  }
  if (CommandAddPolygonLines(ThickNess,-1,0,OBJECT_POLYLINE)==-1) return 0;
  if (NetNr>=0) {
...
//  DrawTryingPolygon(1); DrawPolygon
  if (MakeNewAreaFill(DefaultAreaFill->NetNr,Layer,
                      DefaultAreaFill->Info,
                      DefaultAreaFill->SurroundThickNess,
                      DefaultAreaFill->ThermalReliefThickness,
                      DefaultAreaFill->ThermalReliefDistance,
                      DefaultAreaFill->Clearance,0)==1) {
    AreaFill=(AreaFillRecord *)&(AreaFillMem[(*AreaFills)[Design.NrAreaFills-1]]);
    AreaFill->FillType=2;
    if (AreaFill->NetNr!=-1) {
      ReCalcConnectionsNet(AreaFill->NetNr,0,1);
...
// ****************************************************************************
      if ((OkToIncludeObject)
         &&
         (PolygonObject->minx<AreaFill->maxx)
         &&
...
// ****************************************************************************
          if ((Object->ObjectType!=VIA_PUT_THROUGH_ROUND)
            ...
            for (cnt2=0;cnt2<4;cnt2++) {
              if (CopyThermalReliefInPolygon(Object,PolygonObject,
                  NewAreaFill->ThermalReliefThickness,RandValue,1,cnt2)==0) {
                NewAreaFill=(AreaFillRecord *)AreaFillMemTemp;
                TempAreaFill=(AreaFillRecord *)AreaFillMemTemp2;
                memmove(AreaFillMemTemp2,NewAreaFill,NewAreaFill->MemSize);
                res=MergePolygon(PolygonObject,1);
...

{
  ...
  if (AreaFillDialog(BoardAreaFill,2)==-1) return;
  ...
  switch (res) {
    case 0:
      ...
      break;
    case -1:
      strcpy(str,SC(1003,"Board outline (Not existing/errors) will not be used for calculating the powerplane.\n\n"));
      strcat(str,SC(1004,"Instead the initial PCB size will be used"));
      if (MessageBoxOwn(PCBWindow,str,SC(102,"Warning"),MB_APPLMODAL|MB_OK)!=IDOK) {
        DeAllocateMemTemp();
        return;
      }
...
//  AreaFill->AddNr=0;
  ...
  if ((AreaFill->NetNr>=0)
     &&
     ((Net->Info & CONNECTIONS_DISABLED) == 0)) {
      ...
      if ((FirstPolygon->PolygonType & 2) == 2) {
        ...
        if (res==0) {
          if (res2==1) {
            ...
            AreaFill->Info|=(AreaFill2->Info & (AREAFILL_WITH_THERMAL_RELIEF|AREAFILL_WITH_NO_VIA_THERMAL_RELIEF));
          }
          if ((!IsLayerPowerPlane(AreaFill->Layer))
             &&
             (RebuildAreaFill(AreaFill,0)==1)) {
...

{
  ...
  int32  cnt,cnt2,res,NetNr,Layer,count,NrAreaFills,pos,pos2,
         ok,ThermalReliefs,MergeAreaFillNrs[100];
  AreaFillRecord *AreaFill,*AreaFill3;
  double minx,maxx,miny,maxy,x1,y1,Clearance=-100.0,ThermalReliefThickness=-100.0,
         ThermalReliefDistance=-100.0;
      ...
      if ((FirstPolygon->PolygonType & 2) == 2) {
        if (NetNr!=-1) {
          if (NetNr!=AreaFill->NetNr) {
            MessageBoxOwn(PCBWindow,SC(1005,"Different nets areafills"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return -1;
          }
        } else {
          NetNr=AreaFill->NetNr;
        ...
        if (Layer!=-1) {
          if (Layer!=AreaFill->Layer) {
            MessageBoxOwn(PCBWindow,SC(1006,"Different layers areafills"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return -1;
          }
        } else {
        ...
        if (Clearance<0.0) {
          ...
          if (NotInRange(Clearance,AreaFill->Clearance)) {
            if (MessageBoxOwn(PCBWindow,SC(1007,"Different clearances areafills\n\nOk to continue"),SC(1,"Message"),MB_APPLMODAL|MB_OKCANCEL)==IDCANCEL) {
              return -1;
            }
          }
        }
        if (ThermalReliefs!=-1) {
          if (((AreaFill->Info & AREAFILL_WITH_THERMAL_RELIEF) == AREAFILL_WITH_THERMAL_RELIEF)
             &&
             (ThermalReliefs==0)) {
            strcpy(str,SC(1008,"Different thermal reliefs areafills\r\n\r\nUse with thermal reliefs (YES), or no thermal reliefs (NO) ?"));
            if ((res=MessageBoxOwn(PCBWindow,str,SC(1,"Message"),MB_APPLMODAL|MB_YESNOCANCEL))==IDCANCEL) {
              return -1;
            }
            if (res==IDYES) {
        ...
        if ((AreaFill->Info & AREAFILL_WITH_THERMAL_RELIEF) == AREAFILL_WITH_THERMAL_RELIEF) {
          if (ThermalReliefThickness<0.0) {
            ...
            if (NotInRange(ThermalReliefThickness,AreaFill->ThermalReliefThickness)) {
              strcpy(str,SC(1292,"Different thermal reliefs areafills\r\n\r\nUse the largest thermal relief thickness?"));
              if ((res=MessageBoxOwn(PCBWindow,str,SC(1,"Message"),MB_APPLMODAL|MB_YESNOCANCEL))==IDCANCEL) {
                return -1;
              }
              if (res==IDYES) {
          ...
          if (ThermalReliefDistance<0.0) {
            ...
            if (NotInRange(ThermalReliefDistance,AreaFill->ThermalReliefDistance)) {
              strcpy(str,SC(1293,"Different thermal reliefs areafills\r\n\r\nUse the largest thermal distance ?"));
              if ((res=MessageBoxOwn(PCBWindow,str,SC(1,"Message"),MB_APPLMODAL|MB_YESNOCANCEL))==IDCANCEL) {
                return -1;
              }
              if (res==IDYES) {
...
// ****************************************************************************
  ...
  for (cnt2=0;cnt2<NrAreaFills;cnt2++) {
    ...
    if (FoundError) {
      MessageBoxOwn(PCBWindow,SC(1010,"Areafills to complex"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
      ok=1;
    }
  }
  if (SelectionEsc) {
...
// ****************************************************************************
  ...
  if (NewAreaFill->NrPolygons>2) {
    MessageBoxOwn(PCBWindow,SC(1011,"Areafills do not overlap"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
    DeAllocateMemPolygons();
    DeAllocateMemAreaFills();
    return -1;
  }
  ...
  for (cnt=0;cnt<Design.NrAreaFills;cnt++) {
    ...
    if ((AreaFill->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      if ((AreaFill->Info & (POWERPLANE)) == 0) {
        ...
        if ((FirstPolygon->PolygonType & 2) == 2) {
          sprintf(str3,SC(1094,"Areafill\tLayer %s\tNet %s\tNr vertices %i"),str2,Net->Name,FirstPolygon->NrVertices);
          ...
          for (cnt2=1;cnt2<AreaFill->NrPolygons;cnt2++) {
            if ((DrawPolygon->PolygonType & 2) == 2) {
              ...
              sprintf(str3,SC(1095,"Areafill cut out\tLayer %s\tNet %s\tNr vertices %i"),str2,Net->Name,DrawPolygon->NrVertices);
...

{
  ...
  int32   cnt,cnt2,cnt3,ok,InsertMode,OkToAdd,Mode2,count,
          FoundPolygonNr,count2,CircleRoundings=24;
  double  NewX,NewY,x1,y1,OldShiftX,OldShiftY,
          x5,y5,hoek,RandValue;
  char    TextLine[2048],str[500],str3[MAX_LENGTH_STRING];
  LPSTR   TextP,TextP2;
  int32    FirstShift=1,LeftButtonFinishPolygon=0;
  PolygonRecord *DrawPolygon2;
...
//      ellips2(MultX(x22),MultY(y22),Mult(ThickNess),Mult(ThickNess),255);
      ...
      if ((res=CheckLineCrossesWithAreaFill(x11,y11,x22,y22,AreaFill2,&xx1,&yy1,&xx2,&yy2,
                                            ThickNess*0.99,0))!=-1) {
        if (res==0) {
          xx1=x11;
          yy1=y11;
...
//        ellips2(MultX(xx2),MultY(yy2),Mult(ThickNess),Mult(ThickNess),255);
      ...
      if (res!=0) {
        if ((res2=CheckLineCrossesWithAreaFill(x22,y22,x11,y11,AreaFill2,&xx1,&yy1,&xx2,&yy2,
                                              ThickNess*0.99,0))!=-1) {
          if (res2==0) {
            xx1=x11;
            yy1=y11;
...

{
  ...
  if (AreafillNrToMirror==-1) return;
