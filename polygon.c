...
int32  ExtraDeletionPolygonPos[32],NrExtraDeletionPolygons,ExtraDeletionPolygonsMemSize,
       GlobalNrCrosses,StartCnt,ok,FinishPolygon,TestDraw,
       PolygonObjectType,ObjectNr,NrCrossPoints,PowerPlaneNetNr,StartPolygon;
...

{
  ...
  while (((RandValue=((rand()/RAND_MAX)))<0.1)
        ||
...

{
  ...
  if (CheckPolygonCompleetlyOutsidePolygon(Polygon,&SearchPolygon)!=1) return 1;
  return 0;
}
...

{
  ...
  double *x1,*y1,*x2,*y2,*x3,*y3,*x4,*y4,
         *FirstX,*FirstY,*FirstX2,*FirstY2;

  if ((PolygonObject->maxx+100<BigPolygon->minx)
     ||
     (PolygonObject->minx-100>BigPolygon->maxx)
     ||
     (PolygonObject->maxy+100<BigPolygon->miny)
     ||
     (PolygonObject->miny-100>BigPolygon->maxy)) return -2;
...
*/
  ...
  if ((ObjectPolygon->NrSubPolygons==0)
    ...
    PolygonMappable=0;
    cnt=0;
    while ((cnt<NrMappableObjectPolygons)
          &&
          (!PolygonMappable)) {
      CheckObjectPolygon=(ObjectPolygonRecord *)&(ObjectPolygonMem[(*ObjectPolygons)[cnt+MappableObjectPolygonStart]]);
      if (CheckObjectPolygon->NrVertices==count) {
        ...
        PolygonMappable=1;
          ...
          if ((NotInRange2(divx,divx2))
             ||
             (NotInRange2(divy,divy2))) {
            PolygonMappable=0;
          }
        }
      }
      if (PolygonMappable==0) cnt++;
    }
    PolygonMappable=0;
    if (PolygonMappable) {
...

{
  ...
  if ((FoundPolygon->NrSubPolygons>0)
     &&
     (FoundPolygon->NrVerticesMainPolygon>0))  {
...
int32 GetBiggerSmallerPoints(double x1,double y1,
                             double x2,double y2,
                             double x3,double y3,
...
{
  double a1,a2,b1,b2,b1a,b2a,AddNum,
         xx1,yy1,xx2,yy2,xx3,yy3,xx4,yy4,diva,suma,
         OldXA,OldYA,x2a,y2a,x2b,y2b,x3a,y3a,temp,x1a,y1a,x1aa,y1aa;
...
//  ThickNess+=0.001;
  ...
  if (Vert12) {
    OldXA=OldX;
    if (y2>y1) OldYA=OldY-AddNum;
    else OldYA=OldY+AddNum;
  } else {
    if (fabs(a1)<1) {
      if (x1>x2) {
        OldXA=OldX+AddNum;
...
// **************************************************************************************
      ...
      *NewX1=xx1;
      if (LineCrosses(x2,y2,x3a,y3a,xx1,yy1,xx2,yy2)==1) {
        if (fabs(yy1-OldYA)<fabs(yy2-OldYA)) { // Shortest length y1 -> (yy1,yy2)
          *NewY1=yy1;
        } else {
          *NewY1=yy2;
        }
      } else {                // Largest length y1 -> (yy1,yy2)
...
// **************************************************************************************
      ...
      if (LineCrosses(x2,y2,x3a,y3a,xx1,yy1,xx2,yy2)==1) { // Shortest length y1 -> (yy1,yy2)
      ...
      if (LineCrosses(x2,y2,x3a,y3a,xx3,yy3,xx4,yy4)==1) {  // Shortest length y1 -> (yy1,yy2)
...
#define    InRange15(x1,x2) ( (((x1>x2-10000.0) && (x1<x2+10000.0))) ? (1) : (0) )
  ...
  CntMax=0;
  count=Polygon->NrVertices;
  if ((Polygon->NrSubPolygons>0)
     &&
     (Polygon->NrVerticesMainPolygon>0))  {
  ...
  if ((mode & 1)==0) {   //  Make bigger polygon
    if (c1a+c2a!=1) {
      ok=1;
      OldX=x2;
      OldY=1e9;
      res=GetBiggerSmallerPoints(x3,y3,x2,y2,x1,y1,OldX,OldY,
                                 &NewX1,&NewY1,&NewX2,&NewY2,&OldX2,&OldY2,Thickness,0);
...
//    (*BiggerPolygon).Points[cnt2].y=NewY;
    ...
    if ((InRange4(x1,13347992.1143873450))
       &&
       (InRange4(y1,9296956.7928441465))) {
      ok=1;
    }
...

{
  ...
  double  x1,y1,x2,y2,x3,y3,x4,y4,x5,y5,x6,y6,dikte,diktex,diktey,
          Angle,Angle1,Angle2,StartAngle,Length1,Length2,Width,Height,Width2,Height2,
          sinx,sinx2,Length,
          AngleInc,AngleInc2,Thickness;
  ...
  switch (Object->ObjectType) {
    ...
    case OBJECT_LINE:
      ...
      if (NotInRange(x5,0.0)) {
        ...
        if (StartAngle==0.0) {
          if (x5<0.0) StartAngle+=PI;
        } else {
          if (StartAngle<0.0) StartAngle+=PI;
        }
        if (y2<y1) StartAngle+=PI;
      } else {
        if (y2>y1) {
          StartAngle=PI*0.5;
        } else {
    ...
    case OBJECT_ARC:
      ...
      if ((x2>(100*2540))
         ||
         (y2>(100*2540))) NrSegments=64;
      if ((x2>(200*2540))
         ||
         (y2>(200*2540))) NrSegments=128;
      x3=Object->x3;
      y3=Object->y3;
      x4=Object->x4;
      y4=Object->y4;
...
void InsertVertice(int32 ResultStart,PolygonRecord *ResultPolygon,
                   PointsArray *InsertPoints)
...
void AppendVertices(int32 InsertStart,int32 Count,int32 NrVerticesInsert,
                    PointRecord *InsertPoints,PointRecord *ResultPoints,
                    int32 mode)
...

{
  int32  cnt4,cnt3,cnt2,cnt,count,count2,res,NrCrosses,InsertPos,Pos,
         CountOnSameLine,Found,NrTries,LineCnt2,
         LineCnt3,LineCnt2a,LineCnt3a,
  ...
  while ((Changed)
    ...
    if (NrCrosses>MaxNrCrosses) return NrCrosses;
...
{
  int32  cnt,cnt2,cnt4,count,NrCrosses,TotalCount,NrPolygons,ok,
         VerticesCount,MemSizeAreaFill,Mem2,MaxNr;
  double  Area,MaxArea;

  PolygonRecord *SubPolygon,*SpecialPolygon,
                *AreaFillPolygon,*TempPolygon;
  int32  Changed,AppendingPolygonChanged,SubPolygonInsidePolygonObject,
        InsideAppendingPolygon,OverlappedPolygon,DeletionPolygonChanged,
        AppendingPolygonInsideDeletionPolygon;
...
// Check if a SubPolygon is inside PolygonObject
  ...
  PolygonPos=(uint8 *)SubPolygon;
  for (cnt2=0;cnt2<TempAreaFill->NrPolygons;cnt2++) {
    if ((SubPolygon->PolygonType & 8) == 0) { // Deletion polygon
            ...
            for (cnt4=0;cnt4<NrPolygons;cnt4++) {
              if (cnt4!=MaxNr) {
                ...
                if (ExtraDeletionPolygonsMemSize+Mem2>MaxTempMemory) {
                  if (AllocateMemTemp(ExtraDeletionPolygonsMemSize+Mem2+16384)!=0) return -1;
                }
...
// PolygonObject will not be included
  ...
  if ((InsideAppendingPolygon)
     &&
     (!SubPolygonInsidePolygonObject)
     &&
     (!AppendingPolygonChanged)
     &&
     (!AppendingPolygonInsideDeletionPolygon)
     &&
     (!DeletionPolygonChanged)) {
    return 3;
  }
...
// and none of the deletion polygons is inside NewPolygon then append NewPolygon
   ...
   if ((!SubPolygonInsidePolygonObject)
     &&
     (!AppendingPolygonChanged)
     &&
     (!DeletionPolygonChanged)) {
...
{
  int32 ok,cnt,count,Found,Found2,res,MemSize,NewPolygonInside,
        SubPolygonSize,NewObjectPolygonLength;
  double x1,y1;
...
//  AreaFill=(AreaFillRecord *)&(AreaFillMem[(*AreaFills)[AreaFillNr]]);
    ...
    if ((Object->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
      if (FoundLayer==-1) {
        ...
        if (Object->Layer!=FoundLayer) {
          MessageBoxUTF8(GEOMWindow,"Selected objects should be on one layer only","Error",MB_APPLMODAL|MB_OK);
          return -1;
        }
      }
      if (Object->PinNr!=-1) {
        if (FirstPinNr==-1) {
          ...
          if (Object->PinNr!=FirstPinNr) {
            MessageBoxUTF8(GEOMWindow,"Selected objects should have the same pinnumber","Error",MB_APPLMODAL|MB_OK);
            return -1;
          }
        }
      }
  ...
  switch (FoundLayer) {
    ...
    case POWER_PAD_LAYER:
      MessageBoxUTF8(GEOMWindow,"Can not merge objects one this layer","Error",MB_APPLMODAL|MB_OK);
      return -1;
  }
  ...
  if (NewAreaFill->NrPolygons!=2) {
    MessageBoxUTF8(GEOMWindow,"Selected objects should overlap each other","Error",MB_APPLMODAL|MB_OK);
    return -1;
  }
  FilledArea=0.0;
  ...
  for (cnt=0;cnt<NewAreaFill->NrPolygons;cnt++) {
    ...
    AreaFillPolygon=(PolygonRecord *)AreafillPos;
    FilledAreaTemp=(AreaFillPolygon->maxx-AreaFillPolygon->minx)*
                   (AreaFillPolygon->maxy-AreaFillPolygon->miny);
