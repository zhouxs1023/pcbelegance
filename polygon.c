	...
	for (cnt = 0; cnt < NrObjects; cnt++)
	{
		...
		if ((Object->Info & (OBJECT_NOT_VISIBLE | OBJECT_SELECTED)) == OBJECT_SELECTED)
		{
			...
			else
			{
				...
				{
					MessageBoxUTF8(GEOMWindow, "Selected objects should be on one layer only", "Error",
					               MB_APPLMODAL | MB_OK);
					return -1;
				}
			}
			...
			if (Object->PinNr != -1)
			{
				...
				else
				{
					...
					{
						MessageBoxUTF8(GEOMWindow, "Selected objects should have the same pinnumber", "Error",
						               MB_APPLMODAL | MB_OK);
						return -1;
					}
				}
