...
//    if (RightButtonPressed) {
			...
			case OBJECT_POLYLINE:
				...
				if (DrawPolygon2->NrVertices > 0)
				{
					OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_TRACE_BACKWARDS,
					              SC(993, "Goto previous point"));
				}

				if (Mode == 0)
				{
					OwnAppendMenu(PopUpMenu, MF_ENABLED | MF_STRING, ID_CLEARANCE_WIDTH_DIALOG,
		...
		if ((AreaFill->Info & (OBJECT_NOT_VISIBLE | POWERPLANE)) == 0)
		{
			...
			if ((FirstPolygon->PolygonType & 2) == 2)
			{
				if (NetNr != -1)
				{
					...
					{
						MessageBoxOwn(PCBWindow, SC(1005, "Different nets areafills"), SC(100, "Error"),
						              MB_APPLMODAL | MB_OK);
						return -1;
					}
				}
				...
				if (Layer != -1)
				{
					...
					{
						MessageBoxOwn(PCBWindow, SC(1006, "Different layers areafills"), SC(100, "Error"),
						              MB_APPLMODAL | MB_OK);
						return -1;
					}
				}
				...
				else
				{
					...
					{
						if (MessageBoxOwn
						        (PCBWindow, SC(1007, "Different clearances areafills\n\nOk to continue"), SC(1, "Message"),
