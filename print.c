...
double    PrintOffsetX,PrintOffsetY,TempScale,
          PDFMultFactor,PDFMultFactor2,PDFPageOffsetX,PDFPageOffsetY,
          PDFStartX,PDFStartY,PDFNewLineWidth,PDFLineWidth;
int32     PDFReverse,PDFCurrentColor,StringCount,PrintToBitmap,ok,
          CurrentPlotThickNess,BrushType,PixelsPerInchY,RoundingNr,RoundingCnt,PolygonNr,
          StartPdfPos,pdffp,NrPDFObjects,*PDFObjectPos,Plotfp;
...
extern    double  TextMinX,TextMinY,TextMaxX,TextMaxY,
                 LastGerberX,LastGerberY,CurrentPlotPenSize,
...
int32 WritePlotText(int32 Layer,double BoardMinX,double BoardMinY,
                    double BoardMaxX,double BoardMaxY,int32 mode);
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDD_SCALE1X1               ,SC(306,"Scale 1X"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(185,"Scale factor"));
      SetWindowTextOwn(Dialog,SC(301,"Scale"));
    ...
    case WM_COMMAND:
      switch (WParam) {
        case IDD_SCALE1X1:
          ...
          return about;
        case IDOK:
          if ((GetDialogFloatValue(Dialog,IDC_EDIT1,&value)==0)
             ||
             (value<0.1)
             ||
             (value>10.0)) {
            MessageBoxOwn(PCBWindow,SC(1020,"Scale factor between 0.1 .. 10"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          TempScale=value;
          EndDialog(Dialog, 1);
...
{
  int32  PixelsPerInchX,PixelsPerInchY,res2,PageX,PageY,minx,maxx,miny,maxy,
         PagePixelsY,OldDrawWindowMinX,OldDrawWindowMinY,OldDrawWindowMaxX,OldDrawWindowMaxY;
  double   Xoffset2,Yoffset2,Factor2,MinX,MaxX,MinY,MaxY,Factor1x,Factor3,
           TempFactor,FactorX,FactorY;
  int32      res;
...
// ********************************************************************************************************
    ...
    if (OkToDrawConnections) DrawConnections(0);
    DrawErrorObjects();
    if (DrawDrillMode>0) {
      DrawObjectArcs(0,1);
      DrawDrillsComps(0);
...
//          Mult2(Design.BoardWidth),Mult2(Design.BoardHeight));
    DrawLine(Mult2(-Xoffset)+DrawWindowMinX,
          DrawWindowMaxY-Mult2(25400.0-Yoffset)-1,
          Mult2(-Xoffset)+DrawWindowMinX,
          DrawWindowMaxY-Mult2(-25400.0-Yoffset)-1);
          DrawLine(Mult2(-25400.0-Xoffset)+DrawWindowMinX,
          DrawWindowMaxY-Mult2(-Yoffset)-1,
          Mult2(25400.0-Xoffset)+DrawWindowMinX,
          DrawWindowMaxY-Mult2(-Yoffset)-1);
...
{
  int32  cnt,cnt2,cnt3,res,count,NrLayerPolygons,
         NrDeletionPolygons,PowerPlaneNr,LayerPolygonsInfo[256],
         DeletionPolygonsInfo[256];
  double xmin,xmax,ymin,ymax,x11,y11,Xmin,Ymin,Xmax,Ymax;
  PolygonRecord *LayerPolygons[256],*DeletionPolygons[256],
                *DeletionPolygon,*DrawPolygon,*PowerPlanePolygon,*TempPolygon;
  ...
  for (cnt=0;cnt<Design.NrAreaFills;cnt++) {
    ...
    if ((cnt!=PowerPlaneNr)
       &&
       ((AreaFill->Info & (OBJECT_NOT_VISIBLE)) == 0)
       &&
...

{
  ...
  for (cnt=0;cnt<Design.NrVerTraces[Layer];cnt++) {
    ...
    if ((Trace->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      if ((xx1+yy2a>=ViewMinX)
         &&
         (xx1-yy2a<=ViewMaxX)) {
        if ((yy2+yy2a>=ViewMinY)
           &&
           (yy1-yy2a<=ViewMaxY)) {
          if ((Mode & 1)==0) {
            ...
            if ((PDFLineWidth==0.0)
               ||
               (NotInRange2(PDFNewLineWidth,PDFLineWidth))) {
  ...
  for (cnt=0;cnt<Design.NrHorTraces[Layer];cnt++) {
    ...
    if ((Trace->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      dikte2=Trace->ThickNess;
      if ((yy1+yy2a>=ViewMinY)
         &&
         (yy1-yy2a<=ViewMaxY)) {
        if ((xx2+yy2a>=ViewMinX)
           &&
           (xx1-yy2a<=ViewMaxX)) {
          if ((Mode & 1)==0) {
            x1=Mult2(xx1-PrintOffsetX);
  ...
  for (cnt=0;cnt<Design.NrDiag1Traces[Layer];cnt++) {
    ...
    if ((Trace->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      if ((xx2+yy2a>=ViewMinX)
        ...
        if ((yy1+yy2a>=ViewMinY)
           &&
           (yy1-(xx2-xx1)-yy2a<=ViewMaxY)) {
#ifdef _DEBUG
          if ((InRange9(xx1,166.7e5))
             &&
             (InRange9(yy1,128.2e5))) {
            ok=1;
          }
#endif
          if ((Mode & 1)==0) {
  ...
  for (cnt=0;cnt<Design.NrDiag2Traces[Layer];cnt++) {
    ...
    if ((Trace->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      if ((xx2+yy2a>=ViewMinX)
         &&
         (xx1-yy2a<=ViewMaxX)) {
        if ((yy1+(xx2-xx1)+yy2a>=ViewMinY)
           &&
           (yy1-yy2a<=ViewMaxY)) {
#ifdef _DEBUG
          if ((InRange9(xx1,50.72e5))
             &&
             (InRange9(yy1,198.1e5))) {
            ok=1;
          }
#endif
          if ((Mode & 1)==0) {
  ...
  for (cnt=0;cnt<Design.NrVias;cnt++) {
    ...
    //  Drill=Via->DrillThickNess;
      ...
      if ((Xmax>=ViewMinX)
        ...
        if ((Mode & 1)==0) {
          if (!ReverseY) {
            ellips2(Mult2(Via->X-PrintOffsetX),Mult2(Via->Y-PrintOffsetY),
            ...
            if (!ReverseX) {
              ...
              InitDrawingPlot(1);
              InitDrawingSolidBrushBlack();
              ellips2(DrawWindowMaxX-Mult2(Via->X-PrintOffsetX),DrawWindowMaxY-Mult2(Via->Y-PrintOffsetY)-1,
                      Mult2(Via->ThickNess),Mult2(Via->ThickNess),255);
              if ((Mode & 2)==0) {
                InitDrawingSolidBrushWhite();
                y2=min(Via->DrillThickNess,MIN_PLOT_DRILL);
...
/*
  if ((mode & 1)==1) {
    ...
    FontSize=Size*1.25*PDFMultFactor*PDFMultFactor2;
    AddToMessageBuf("BT");
    sprintf(PDFstr,"  /F1 %.3f Tf",FontSize);
    AddToMessageBuf(PDFstr);
    x1=PDFConvX(x);
    y1=PDFConvY(y);
    sprintf(PDFstr,"  %.3f %.3f Td",x1,y1);
    AddToMessageBuf(PDFstr);
    sprintf(PDFstr,"  (%s) Tj",str);
    AddToMessageBuf(PDFstr);
    AddToMessageBuf("ET");
    return;
  }
*/
  ...
  for (cnt2=0;cnt2<LineSegments;cnt2++) {
    if ((mode & 1)==0) {
      ...
      SegmentCount++;
      if (ReverseY) yy1=DrawWindowMaxY-Mult2(LineBuf[SegmentCount]-PrintOffsetY)-1;
      else yy1=Mult2(LineBuf[SegmentCount]-PrintOffsetY);
      SegmentCount++;
      ...
      SegmentCount++;
      if (ReverseY) yy2=DrawWindowMaxY-Mult2(LineBuf[SegmentCount]-PrintOffsetY)-1;
      else yy2=Mult2(LineBuf[SegmentCount]-PrintOffsetY);
      SegmentCount++;
...
{
  double xx1,yy1,xx2,yy2,xx3,yy3,xx4,yy4,xx2a,yy2a,incX,incY,
         x11,y11,x22,y22,xxx2,yyy2,yyy3,Rotation,LineBuf[4096],
         Angle1,Angle2,Length1,Length2;
  int32  x3,y3,x4,y4,cnt2,cnt3,cnt4,Layer,SegmentCount,LineSegments,ok,
         MaxCountX,Thickness,Mirror,NrLines,FontNr,count,
         cnt5,cnt6,code,DrawChar,lengte,TextAlignment,x1a,y1a,ArcMode;
  PolygonRecord *DrawPolygon;
  ...
  if ((ViewMaxX<Object->minx)
     ||
     (ViewMinX>Object->maxx)
     ||
     (ViewMaxY<Object->miny)
     ||
  ...
  switch (Object->ObjectType) {
    ...
    case PIN_LINE_HOR:
      if ((Mode & 1)==0) {
        ...
        InitDrawingSolidBrushBlack();
        if (!ReverseX) {
          DrawHorLine(Mult2(xx1-PrintOffsetX),Mult2(yy1-PrintOffsetY),
                      Mult2(xx1+xx2-PrintOffsetX),Mult2(yyy2),0);
        } else {
    ...
    case PIN_LINE_VER:
      if ((Mode & 1)==0) {
        ...
        InitDrawingSolidBrushBlack();
        if (!ReverseX) {
          DrawVerLine(Mult2(xx1-PrintOffsetX),Mult2(yy1-PrintOffsetY),Mult2(yy1+xx2-PrintOffsetY),Mult2(yyy2),0);
        } else {
          DrawVerLine(DrawWindowMaxX-Mult2(xx1-PrintOffsetX),Mult2(yy1-PrintOffsetY),Mult2(yy1+xx2-PrintOffsetY),Mult2(yyy2),0);
        }
      } else {
        if (PDFCurrentColor==0) {
          AddToMessageBuf("0 g");
    ...
    case PIN_LINE_DIAG1:
      if ((Mode & 1)==0) {
        ...
        InitDrawingSolidBrushBlack();
        if (!ReverseX) {
          DrawDiag1Line(Mult2(xx1-PrintOffsetX),Mult2(yy1-PrintOffsetY),
                        Mult2(xx1+xx2-PrintOffsetX),Mult2(yyy2),0);
        } else {
    ...
    case PIN_LINE_DIAG2:
      ...
      if ((Mode & 1)==0) {
        if (!ReverseX) {
          DrawDiag2Line(Mult2(xx1-PrintOffsetX),Mult2(yy1-PrintOffsetY),
                        Mult2(xx1+xx2-PrintOffsetX),Mult2(yyy2),0);
        } else {
    ...
    case OBJECT_RECT:
      if ((Mode & 1)==0) {
        ...
        if (Object->Thickness==0.0) {
          ...
          PrintPDFLineFromFloat(xx1-xx2*0.5,yy1-yy2*0.5,xx1+xx2*0.5,yy1-yy2*0.5,Object->Thickness);
          PrintPDFLineFromFloat(xx1+xx2*0.5,yy1-yy2*0.5,xx1+xx2*0.5,yy1+yy2*0.5,Object->Thickness);
          PrintPDFLineFromFloat(xx1+xx2*0.5,yy1+yy2*0.5,xx1-xx2*0.5,yy1+yy2*0.5,Object->Thickness);
          PrintPDFLineFromFloat(xx1-xx2*0.5,yy1+yy2*0.5,xx1-xx2*0.5,yy1-yy2*0.5,Object->Thickness);
        }
        ...
        if (Object->Thickness==0.0) {
          ...
          if ((Angle2-Angle1<ANGLE_90)
            ...
            if (((abs(x3-x4)<2)
               ...
               (abs(y3-y4)<2))
               ||
               (Angle2-Angle1<ANGLE_CONVERT(10.0))) {
              ArcMode=0;
            } else {
              ArcMode=1;
            }
          } else {
            if ((Angle2-Angle1>0.0)
               &&
               (Angle2-Angle1<ANGLE_CONVERT(3.0))) {
              ArcMode=0;
            } else {
              ArcMode=1;
            }
          ...
          if (ArcMode==0) {
            ...
            if (!ReverseX) {
                SpecialArc(x1a,y1a,
                           Mult2(xx2)+1,Mult2(yy2)+1,
                           Mult2(xx1+xx3-PrintOffsetX),DrawWindowMaxY-Mult2(yy1+yy3-PrintOffsetY)-1,
                           Mult2(xx1+xx4-PrintOffsetX),DrawWindowMaxY-Mult2(yy1+yy4-PrintOffsetY)-1);
              } else {
                SpecialArc(x1a,y1a,
                           Mult2(xx2)+1,Mult2(yy2)+1,
                           DrawWindowMaxX-Mult2(xx1+xx4-PrintOffsetX),DrawWindowMaxY-Mult2(yy1+yy4-PrintOffsetY)-1,
                           DrawWindowMaxX-Mult2(xx1+xx3-PrintOffsetX),DrawWindowMaxY-Mult2(yy1+yy3-PrintOffsetY)-1);
              }
            } else {
              SpecialArc(x1a,y1a,Mult(xx2)+1,Mult(yy2)+1,
                         Mult2(xx1+xx3-PrintOffsetX),Mult2(yy1+yy3-PrintOffsetY),
                         Mult2(xx1+xx4-PrintOffsetX),Mult2(yy1+yy4-PrintOffsetY));
            }
          }
        }
      } else {
...
/*
        ...
        if ((Mode & 1)==0) {
          ...
          for (cnt3=0;cnt3<count;cnt3++) {
            if (!ReverseY) {
              y2a=Mult2((*ObjectPolygon).Points[cnt3].y-PrintOffsetY);
            } else {
              y2a=DrawWindowMaxY-Mult2((*ObjectPolygon).Points[cnt3].y-PrintOffsetY)-1;
            }
            if (!ReverseX) {
              x2a=Mult2((*ObjectPolygon).Points[cnt3].x-PrintOffsetX);
            } else {
              x2a=DrawWindowMaxX-Mult2((*ObjectPolygon).Points[cnt3].x-PrintOffsetX);
            }
            if ((x2a<-32768)
               ||
               (x2a>32767)
          ...
          for (cnt3=0;cnt3<count;cnt3++) {
            x11=PDFConvX(ObjectPolygon->Points[cnt3].x);
            y11=PDFConvY(ObjectPolygon->Points[cnt3].y);
            sprintf(str,"%.3f %.3f",x11,y11);
            if (cnt3==0) {
              strcat(str," m");
            } else {
...
/*
  ...
  y4=0.0;
  NrSegments=8;
  if (Object->x2>(200000)) NrSegments=16;
  if (Object->x2>(600000)) NrSegments=32;

  if ((mode & 1)==0) {
    InitDrawingPlot(Mult2(Object->Clearance)|1);
    InitDrawingNewEmptyBrush();
  } else {
    PDFNewLineWidth=Object->Clearance*PDFMultFactor*PDFMultFactor2;
    if ((PDFLineWidth==0.0)
       ||
       (NotInRange2(PDFNewLineWidth,PDFLineWidth))) {
      PDFLineWidth=PDFNewLineWidth;
  ...
  for (cnt3=0;cnt3<4;cnt3++) {
    ...
    for (cnt=0;cnt<NrSegments;cnt++) {
      ...
      if ((mode & 1)==0) {
        if (!ReverseX) {
          x1a=Mult2(x3-PrintOffsetX);
          x2a=Mult2(x4-PrintOffsetX);
        } else {
          x1a=DrawWindowMaxX-Mult2(x3-PrintOffsetX);
          x2a=DrawWindowMaxX-Mult2(x4-PrintOffsetX);
        }
        if (!ReverseY) {
          y1a=Mult2(y3-PrintOffsetY);
          y2a=Mult2(y4-PrintOffsetY);
        } else {
          y1a=DrawWindowMaxY-Mult2(y3-PrintOffsetY)-1;
          y2a=DrawWindowMaxY-Mult2(y4-PrintOffsetY)-1;
        }
        DrawLine(x1a,y1a,x2a,y2a);
      } else {
        x11=PDFConvX(x3);
...

{
  ...
  if (InRange(x1,x2)) return 0;
  if (InRange(y1,y2)) return 0;
  if (InRange(fabs(x1-y1),fabs(x2-y2))) return 0;
  ...
  if ((rico>0.9)
     &&
     (rico<1.1)) return 0;
//  MessageBoxOwn(PCBWindow,"Plotting line",SC(100,"Error"),MB_APPLMODAL|MB_OK);
  ok=1;
...

{
  ...
  for (cnt=0;cnt<8;cnt++) {
    if (GerberInfo.TextLine[0]!=0) FoundText=1;
  }
...
{
  int32  cnt,cnt2,cnt4,ViaType,Found,ViaInfo,Layer2,
         Mirror,PowerPlaneLayer,OkToAddObject;
    ...
    if ((Layer>0)
      ...
      for (cnt=0;cnt<Design.NrVias;cnt++) {
        ...
        if ((Via->Info & (OBJECT_NOT_VISIBLE)) == 0) {
          ...
          if ((PowerPlaneLayer)
             &&
             (ViewMaxX>=Object4->minx)
             &&
             (ViewMinX<=Object4->maxx)
             &&
             (ViewMaxY>=Object4->miny)
             &&
             (ViewMinY<=Object4->maxy)) {
            ...
            for (cnt2=0;cnt2<Design.NrAreaFills;cnt2++) {
              ...
              if (((AreaFill->Info & (OBJECT_NOT_VISIBLE)) == 0)
                 ...
                 (AreaFill->Layer==Layer)
                 &&
                 (AreaFill->maxx>=Object4->minx)
                 &&
                 (AreaFill->minx<=Object4->maxx)
                 &&
                 (AreaFill->maxy>=Object4->miny)
                 &&
                 (AreaFill->miny<=Object4->maxy)) {
...
// ****************************************************************************
              ...
              default:
                if (Object->Layer==Layer) {
                  if ((Object4=GetNewObject4())==NULL) return -1;
...
// ****************************************************************************
          if ((Layer!=SILKSCREEN_BOTTOM)
            ...
            if ((((Layer==SILKSCREEN_TOP_REFS) || (Layer==SILKSCREEN_BOTTOM_REFS))
               &&
               ((Comp->TextVisibility & 0x100) == 0))
               ||
               (((Layer==SILKSCREEN_TOP_VALUES) || (Layer==SILKSCREEN_BOTTOM_VALUES))
               &&
               ((Comp->TextVisibility & 0x200) == 0))) {
              if ((Layer==SILKSCREEN_TOP_REFS)
                 ||
                 (Layer==SILKSCREEN_BOTTOM_REFS)) {
...

{
  ...
  char   str[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],
         str5[MAX_LENGTH_STRING],str6[MAX_LENGTH_STRING],
...
{
  int32  cnt3,ObjectCount,CompOutlineLayers,res2,PageX,PageY,
         OldDrawWindowMinX,OldDrawWindowMinY,OldDrawWindowMaxX,OldDrawWindowMaxY,Layer,
         PixelsPerInchX,PagePixelsY,PagePixelsX;
  ObjectRecord  *Object,*Object4,MemObject;
  char  str[MAX_LENGTH_STRING];
  PolygonRecord  *PolygonObject;
  uint8  PolygonBuf2[10240];
  char  InfoCopy[MAX_LENGTH_STRING];
  double  Factor2,Factor300,Factor360,Factor600,Factor720,Factor1200,
          MinX,MaxX,MinY,MaxY,MinY2;
  int32      res;
...
#ifdef _DEBUG
    sprintf(str,"%s\\pcb\\plot.txt",DesignPath);
...
//      EndDoc(Pd.hDC);
      ...
      sprintf(InfoStr,SC(406,"Plot %s"),str);
...
*/
  ...
  int32  result,fp,Layer,cnt,cnt2,cnt3,cnt4,cnt5,NrBitmapColors,
         ColorIndex,ColorPixelCount,NrCodedBytes,NrBytesBitmapData,
         cntx,cnty,SectionsX,SectionsY,
         OldDrawWindowMinX,OldDrawWindowMinY,CompOutlineLayers,
         OldDrawWindowMaxX,OldDrawWindowMaxY,
         MemPos,MaxScreenX,MaxScreenY,res,
         LinesToMove,LineBytesToMove,PixelsX,PixelsY,NrBytesSubBitmap,
         SubBitmapMemsize,BitmapMemsize,PowerPlaneLayer;
  ObjectRecord  *Object,*Object4,MemObject;
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],FileStr[MAX_LENGTH_STRING],
        InfoCopy[MAX_LENGTH_STRING];
  PolygonRecord  *PolygonObject;
  uint8  PolygonBuf2[10240],BitmapInfoMem[1024],*LineBytes,CodedLine[4096];
  double  hulp,Factor2,Xoffsets[32],Yoffsets[32],Resolution,
          MinX,MaxX,MinY,MaxY,MinY2,ScreenX,ScreenY;
  ...
  Object=NULL;
  ReverseY=1;
  if (ExportBitmapDialog(0)==2) return -1;


  res=FindMinMaxBoard(&MinX,&MinY,&MaxX,&MaxY,1);
  hulp=MaxX-MinX;
  if (hulp<80.0e5) MaxX=MinX+80.0e5;
  switch (res) {
    case 2:
      break;
    case 3:
...
// ****************************************************************************
        ...
        if (cntx<SectionsX-1) {
          ...
          if (LineBytesToMove==0) LineBytesToMove=NrBytesSubBitmap;
        }
        if (ReverseY) {
          if (cnty<SectionsY-1) {
            LinesToMove=PixelsY;
            ...
            if (LinesToMove==0) LinesToMove=PixelsY;
          }
      ...
      if ((GerberInfo.BitmapExportSaveMode & 1)==1) {
        ...
        for (cnt=0;cnt<LinesToMove;cnt++) {
          ...
          for (cnt2=0;cnt2<MaxScreenX;cnt2++) {
            if (BIT_ACTIVE(LineBytes,cnt2)) { // White
              TempColor=0xffffff;
              cnt3=1;
            } else {
              TempColor=0;
            ...
            if (CurrentColor==(uint32)-1) {
              ...
              if (CurrentColor==TempColor) {
                ColorPixelCount++;
              } else { // Found a different color, store the previous color pixels
...
// ****************************************************************************
  ...
  strcpy(InfoStr,InfoCopy);
  RedrawInfoStr(1);
  sprintf(str,"Bitmap output ready\n\nOutput files are located in dierctory %s\\pcb\\bmp",DesignPath);
...
{
  double     LineWidth,PageSizeX,PageSizeY,dx,dy,NewLineWidth,
             PageSizeX2,PageSizeY2,hulp,MinX,MaxX,MinY,MaxY,MinY2,
             AnnotationsAdded,hulpx,hulpy,DisplX,DisplY;
  int32      res,fp,cnt,cnt2,Layer,cnt3,hulp3,
             PageSizeUnitsX,PageSizeUnitsY,CompOutlineLayers,
             pos,PageCount,NrPages,CurrentColor,PageObjectNr,
             NrPagesWritten,WritePageObjects,
             PageObjectStartNr[64];
  char       str[4096],str2[4096],PageTitles[64][MAX_LENGTH_STRING],
             FileStr[MAX_LENGTH_STRING],InfoCopy[MAX_LENGTH_STRING];
  ...
  if (mode==0) {
    if ((res=(ExportPDFDialog(0)))==2) return -1;
...
// 0.35277777
  ...
  if (FileExistsUTF8(FileStr)==0) {
    sprintf(str2,SC(370,"File %s exist overwrite ?"),FileStr);
  ...
  if (fp<=0) {
    ...
    MessageBoxOwn(PCBWindow,str,SC(100,"Error"),MB_APPLMODAL|MB_OK);
    return -1;
  }
  pdffp=fp;
  SetWaitCursor();
...
// *******************************************************************************************************
  ...
  DeAllocateMemTemp();
// StringCount
  sprintf(str,"PDF output ready\n\nOutput file is %s",FileStr);
  MessageBoxOwn(PCBWindow,str,SC(1,"Message"),MB_APPLMODAL|MB_OK);
...
{
  int32  OldDrawWindowMinX,OldDrawWindowMinY,OldDrawWindowMaxX,OldDrawWindowMaxY,
         FoundPolygonNr,PixelsX,PixelsY,NrBytesSubBitmap,
         SubBitmapMemsize,cnt,count,x,ok;
...
//  Rectangle(OutputDisplay,0,0,PixelsX,PixelsY);
  ...
  for (cnt=0;cnt<PixelsY;cnt++) {
    for (x=0;x<NrBytesSubBitmap;x++) {
      ...
      if ((value & 0x40) == 0) count++;
      if ((value & 0x20) == 0) count++;
      if ((value & 0x10) == 0) count++;
      if ((value & 0x08) == 0) count++;
      if ((value & 0x04) == 0) count++;
      if ((value & 0x02) == 0) count++;
      if ((value & 0x01) == 0) count++;
    }
  }
