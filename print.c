...
int32     CurrentPlotThickNess,BrushType,PixelsPerInchX,PixelsPerInchY,
          RoundingNr,RoundingCnt,PolygonNr,ReverseX,Printing,
          Font90,Plotfp;
        ...
        case IDOK:
          if (SendDlgItemMessage(Dialog,IDC_EDIT1,WM_GETTEXT,20,(LPARAM)str)>0) {
            if ((sscanf(str,"%f",&value)!=1)
               ...
               ||
               (value>100.0)) {
              MessageBoxUTF8(GEOMWindow,SC(409,"Scale factor between 0.1 .. 10"),SC(1,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
            TempScale=value;
            EndDialog(Dialog, 1);
...
{
  int32  res2,PageX,PageY,minx,maxx,miny,maxy,PagePixelsY,
         OldDrawWindowMinX,OldDrawWindowMinY,OldDrawWindowMaxX,OldDrawWindowMaxY;
