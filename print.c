...
typedef struct {
        ...
        int32        CompressionType,
                     BitmapDataSize,
                     HResolutionInPixelsPerMeter,
                     VResolutionInPixelsPerMeter,
                     NrColors1,
                     NrImportedColors;
    } BmpHeaderRecord;
...
double    PDFMultFactor,PDFMultFactor2,PDFPageOffsetX,PDFPageOffsetY,
          PDFStartX,PDFStartY,PDFNewLineWidth,PDFLineWidth,PDFDefaultLineWidth;
...
int32 PrinterDialog(int32 DefaultPaperSize,int32 *NewPaperSize,int32 PaperOrientation,
                    LPSTR PrinterName,int32 mode)

{
  ...
  res=0;
  SystemPaperSize=0;
  if ((mode & 1)==1) { // Force paper settings from the function parameters
    switch (*NewPaperSize) {
      case PAPERSIZE_A3:
...
#define     PRINT_TOLERANCE             0.06
  ...
  char      PrinterFontText[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],NewPrinterName[MAX_LENGTH_STRING];

  int32      res;

  TEXTMETRIC  PrinterFontMetrics;
  int32     h,h2,FontSize,x1,y1,x2,y2,CharSize,hoek,PixelsPerInch,
            OldDrawWindowMinX,OldDrawWindowMinY,NewPaperSize,NewOrientation,
            OldDrawWindowMaxX,OldDrawWindowMaxY,res2,ok,fp;
  double    MinX,MaxX,MinY,MaxY,FactorX,FactorY,Width,Height,
            PrintPageX,PrintPageY,PageX2,PageY2,ExtraPageOffsetX,ExtraPageOffsetY,
            PageOffsetX,PageOffsetX2,PageOffsetY,PrintWidth,PrintHeight,DivX,DivY,
  ...
  if (!EditingSymbol) SetBoardPosInstances();
...
//    Beep(2000,200);
    ...
    if (Color==0) {
      PrintingThickness=1;
      if (PixelsPerInch>500) PrintingThickness=2;
      if (PixelsPerInch>1000) PrintingThickness=4;
    } else {
      PrintingThickness=2;
      if (PixelsPerInch>500) PrintingThickness=3;
      if (PixelsPerInch>1000) PrintingThickness=6;
    }
...
//    if (CheckSpecialVersion(0)) {
    if (0) {
      ...
      sprintf(str,SC(411,"Page offsetx,offsety maxx,maxy  = %d,%d  %d,%d\r\n"),
                  (int32)PageOffsetX,(int32)PageOffsetY,(int32)PrintPageX,(int32)PrintPageY);
      strcat(str2,str);
...
//    CreateObjectColorsPrint(0);
    ...
    PrinterFont=CreateFont(FontSize,0,0,0,0,0,0,0,ANSI_CHARSET,
                           OUT_TT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,
                           FIXED_PITCH,Font1Str);
    PrinterFont_90=CreateFont(FontSize,0,hoek*10,hoek*10,0,0,0,0,ANSI_CHARSET,
                              OUT_TT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,
                              FIXED_PITCH,Font1Str);
    PrinterFont2=CreateFont(FontSize*2,0,0,0,0,0,0,0,ANSI_CHARSET,
                            OUT_TT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,
                            FIXED_PITCH,Font1Str);
    PrinterFont2_90=CreateFont(FontSize*2,0,hoek*10,hoek*10,0,0,0,0,ANSI_CHARSET,
                               OUT_TT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,
                               FIXED_PITCH,Font1Str);
    PrinterFont3=CreateFont(FontSize*3,0,0,0,0,0,0,0,ANSI_CHARSET,
                            OUT_TT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,
                            FIXED_PITCH,Font1Str);
    PrinterFont3_90=CreateFont(FontSize*3,0,hoek*10,hoek*10,0,0,0,0,ANSI_CHARSET,
                               OUT_TT_PRECIS,CLIP_DEFAULT_PRECIS,PROOF_QUALITY,
                               FIXED_PITCH,Font1Str);
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      for (cnt=0;cnt<NrBitmapExportResolutions;cnt++) {
        ...
        sprintf(str,SC(418,"%d dpi"),BitmapExportResolutions[cnt]);
      ...
      if (Found!=-1) {
        ...
        if (TempUnits==0) {
          ...
          SendDlgItemMessageUTF8(Dialog,IDC_EDIT7,WM_SETTEXT,0,(LPARAM)(LPSTR)SC(420,"mm"));
        }
      }
      return about;
    case WM_MOVE:
      ...
      switch (LOWORD(WParam)) {
        case IDOK:
          if (SendDlgItemMessage(Dialog,IDC_RADIO3,BM_GETCHECK,0,0)==1) {
            ...
            if ((value1<(double)1.0)
               ||
               (value1>(double)100000.0)) {
            ...
            if (res==-1) {
              MessageBoxUTF8(SCHWindow,SC(422,"A value should be selected"),SC(38,"Error"),MB_APPLMODAL|MB_OK);
              return about;
            }
        ...
        case IDC_RADIO3:
          ...
          switch (TempUnits) {
            ...
            case 2:
              value1/=100.0;
              SendDlgItemMessageUTF8(Dialog,IDC_EDIT7,WM_SETTEXT,0,(LPARAM)(LPSTR)"\xc2\xb5m");
        ...
        case IDD_UNITS:
          ...
          switch (TempUnits) {
            ...
            case 2:
              value1/=100.0;
              SendDlgItemMessageUTF8(Dialog,IDC_EDIT7,WM_SETTEXT,0,(LPARAM)(LPSTR)"\xb5m");
              SetDialogFloatValue(Dialog,IDC_EDIT2,value1,3);
              break;
          }
          break;
...
#define BIT_ACTIVE(LineBytes,x) (((LineBytes[((x) >> 3)] & ((0x80 >> ((x) & 7)))) != 0) ? 1 : 0)
  ...
  char       PrinterFontText[MAX_LENGTH_STRING],DirPart[MAX_LENGTH_STRING],FileName[MAX_LENGTH_STRING],Name[MAX_LENGTH_STRING];
  BITMAPV4HEADER *BitmapHeader;
  double     Xoffset2,Yoffset2,Factor2,Xoffsets[16],Yoffsets[16],MinX,MaxX,MinY,MaxY,
             ScreenX,ScreenY;
  int32      fp,result,PixelsX,PixelsY,
             cnt,cnt2,cnt3,cnt4,cntx,cnty,hoek,res,
             CharSize,FontSize,NrBitmapColors,
             OldDrawWindowMinX,OldDrawWindowMinY,
             OldDrawWindowMaxX,OldDrawWindowMaxY,
             NrBytesSubBitmap,SectionsX,SectionsY,
             MemPos,MaxScreenX,MaxScreenY,NrBytesBitmapData,
             LinesToMove,LineBytesToMove,NrCodedBytes,
             ColorIndex,ColorPixelCount,
             SubBitmapMemsize,BitmapMemsize;
  ...
  res=DialogBox(SCHClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_BITMAP_OUTPUT), SCHWindow, (DLGPROC)ExportBitmapDialogBody);
  if (res==2) return;
...
// ****************************************************************************************************
  ...
  DeleteGraphicObjects();
  PrintingThickness=1;
  if (2540000.0/BitmapExportResolution>200.0) PrintingThickness=2;
  if (2540000.0/BitmapExportResolution>500.0) PrintingThickness=4;
  if (2540000.0/BitmapExportResolution>1000.0) PrintingThickness=7;
...
// ****************************************************************************************************
  ...
  if (GetDirFromFileName(DirPart,EditFile)!=0) return;
...

{
  ...
  BITMAPV4HEADER *BitmapHeader;
  double     Xoffset2,Yoffset2,Factor2,MinX,MaxX,MinY,MaxY,
             ScreenX,ScreenY;
  int32      fp,result,PixelsX,PixelsY,ok,
             cnt,cnt2,cnt3,cnt4,res,NrBitmapColors,
             OldDrawWindowMinX,OldDrawWindowMinY,
             OldDrawWindowMaxX,OldDrawWindowMaxY,
             NrBytesSubBitmap,MaxScreenX,MaxScreenY,NrBytesBitmapData,
             NrCodedBytes,ColorIndex,ColorPixelCount,
             ColorsUsed[256],NrUsedColors,SubBitmapMemsize,
...
//        TempColor=*memp & 0xffffff;
        if (CurrentColor==TempColor) {
          ...
          cnt3=0;
          while ((cnt3<NrUsedColors)
                &&
                (ColorsUsed[cnt3]!=CurrentColor)) cnt3++;
          if ((cnt3==NrUsedColors)
             &&
             (cnt3<255)) {
...
#define    InRange20(x1,x2) ( (((x1>x2-1.0) && (x1<x2+1.0))) ? (1) : (0) )
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetWindowTextUTF8(Dialog,SC(431,"PDF output"));
...
*/
  ...
  if (Rotation==0) {
    ...
    if (InRangeSpecial(Rotation,90.0,0.01)) {
      ...
      x11=PDFConvX(x);
      y11=PDFConvY(y);
      sprintf(PDFstr,"  %.4f %.4f %.4f %.4f %.3f %.3f Tm",
              cos(ANGLE_CONVERT(Rotation)),
              sin(ANGLE_CONVERT(Rotation)),
              -sin(ANGLE_CONVERT(Rotation)),
              cos(ANGLE_CONVERT(Rotation)),
              x11,y11);
      AddToMessageBuf(PDFstr);
...

{
  ...
  int32  cnt,cnt2,cnt3,cnt4,TextAlignment,TextRotation,BusConnectionType,AddMode,
         ok,NrLines,lengte,ConnectionType,CircleMode,TextMode,Alignment,Length2,
         MemPos,SymbolNr,SegmentCount,LineSegments,InstanceRotation,
         ObjectMirrorX,ObjectMirrorY,NrProperties;
  double Angle,Length;
  ...
  char   PinBusStrings[16][64],TextStr[1024],TextStr2[1024],
         NewPinBusStr[512],str[MAX_LENGTH_STRING];
  int32  LinePos[16];
  LPSTR  LabelText;
...
{
  int32  cnt,cnt2,cnt3,cnt4,res,PageSizeUnitsX,PageSizeUnitsY,hulp3,CurrentColor,
         NrPDFObjects,Rotation,NrProperties,
         x1,y1,x2,y2,NrPDFObjectsRootDir,NrIntroBytes,
         WritePageObjects,NrPagesWritten,PageCount,
         NrCompProperties,PDFObjectPos[4096],pos;
  double MinX,MaxX,MinY,MaxY,hulpx,hulpy,x,y,FontSize,
         PageSizeX,PageSizeY,PageSizeX2,PageSizeY2,NewLineWidth,
         dx,dy,hulp,LineWidth,ExtraOffsetAnnotation;
  InstanceRecord *Instance;
  NetLabelRecord *NetLabel;
  ...
  if (mode==0) {
    res=DialogBox(SCHClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_PDF_OUTPUT), SCHWindow, (DLGPROC)ExportPDFDialogBody);
    if (res==2) return -1;
    ...
    if (!EditingSymbol) {
      for (cnt2=0;cnt2<min(MaxNrInstances,Design.NrInstances);cnt2++) {
        ...
        if (((Instance->Info & OBJECT_NOT_VISIBLE) == 0)
          ...
          if (Rotation==0) {
            ...
            x2=(int32)(x1+PDFConv(strlen(str3)*PDFCharWidthFactor*FontSize*DefFontSize+ExtraOffsetAnnotation));
            ...
            y2=(int32)(y1+PDFConv(strlen(str3)*PDFCharWidthFactor*FontSize*DefFontSize+ExtraOffsetAnnotation*2));
          }
      ...
      for (cnt2=0;cnt2<Design.NrNetLabels;cnt2++) {
        ...
        if (((NetLabel->Info & OBJECT_NOT_VISIBLE) == 0)
          ...
          x1=(int32)(PDFConvX(x-ExtraOffsetAnnotation));
          y1=(int32)(PDFConvY(y-ExtraOffsetAnnotation));
          x2=(int32)(x1+PDFConv(strlen(NetLabel->Name)*PDFCharWidthFactor*FontSize*DefFontSize+ExtraOffsetAnnotation));
