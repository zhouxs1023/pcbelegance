...
char  NewPropertyID[MAX_LENGTH_STRING],NewPropertyValue[MAX_LENGTH_STRING],
      CachedPropertyID[32][100],CachedPropertyValue[32][100],
      TempPropertyID[32][100],TempPropertyValue[32][100],
      CachedNetLabelStr[MAX_LENGTH_STRING],TempNetName[MAX_LENGTH_STRING],
      PropertyFileName[MAX_LENGTH_STRING],PropertyFileName2[MAX_LENGTH_STRING];
...

{
  ...
  if ((PropertyBufLength>0)
     &&
     (PropertyFilePos>=PropertyBufLength)) {
    PropertyFilePos=0;
    return -1;
  }
  cnt=1;
  if (PropertyFileCached==0) {
    ...
    if ((FileRead(fp,PropertyBuf,32768,&BytesRead)==-1)
       ||
       (BytesRead==0)) {
      FileClose(fp);
      return -2;
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDC_BUTTON1    ,SC(247,"Delete"));
      SetDialogItemText(Dialog,IDC_STATIC1    ,SC(445,"ID"));
      SetDialogItemText(Dialog,IDC_STATIC2    ,SC(100,"Value"));
      SetDialogItemText(Dialog,IDC_STATIC3    ,SC(451,"Netlabel"));
      SetDialogItemText(Dialog,IDC_STATIC4    ,SC(123,"Properties"));
      SetDialogItemText(Dialog,IDC_BUTTON2    ,SC(457,"Add TRACEWIDTH property"));
      SetDialogItemText(Dialog,IDC_BUTTON3    ,SC(458,"Add CLEARANCE property"));
      sprintf(str,SC(444,"Modify net"));
...
{
  char PropertyID[MAX_LENGTH_STRING],PropertyValue[MAX_LENGTH_STRING],
       LineBuf[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],NetLabelStr[MAX_LENGTH_STRING];
  int32 count,cnt,cnt2,Length,Found,fp,Modification,
        Changed,res;
  ...
  if ((Modification=DialogBox(SCHClass.hInstance,MAKEINTRESOURCE(IDD_DIALOG_PROPERTY),
                              SCHWindow, (DLGPROC)PropertyDialog2))==3) {
    RePaint();
    return 0;
  }
  ...
  while ((Length=ReadLnProp(PropertyFileName,LineBuf))>=0) {
    if ((Length>1)
      ...
      if (stricmpUTF8(TempNetName,NewNetLabelStr)==0) {
        if (stricmpUTF8(NetLabelStr,NewNetLabelStr)==0) {
          if (count==0) {
            switch (Modification) {
              case 1: // Add,replace
                for (cnt=0;cnt<NrProperties;cnt++) {
                  sprintf(str,"%s \"%s\"=\"%s\"",NewNetLabelStr,TempPropertyID[cnt],TempPropertyValue[cnt]);
        ...
        if ((stricmpUTF8(NetLabelStr,NewNetLabelStr)!=0)
           &&
           (stricmpUTF8(NetLabelStr,TempNetName)!=0)) {
