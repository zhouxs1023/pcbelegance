...

{
  ...
  if (CurrentErrorNr>=0) {
    ...
    if ((Object2->ObjectType!=AREAFILL)
       &&
       ((Object2->ObjectType!=OBJECT_LINE)
       ||
       ((Object2->Info2 & 2)==2))) {
...

{
  ...
  for (cnt=0;cnt<NrErrorObjects/2;cnt++) {
    ...
    if ((ErrorObject1->ObjectType==Object1->ObjectType)
       &&
       (ErrorObject2->ObjectType==Object2->ObjectType)) {
      if ((InRange(ErrorObject1->x1,Object1->x1))
         &&
         (InRange(ErrorObject1->y1,Object1->y1))
         &&
         (InRange(ErrorObject2->x1,Object2->x1))
         &&
         (InRange(ErrorObject2->y1,Object2->y1))) {
        ok=1;
        return 1;
      }
    }
    if ((ErrorObject2->ObjectType==Object1->ObjectType)
       &&
       (ErrorObject1->ObjectType==Object2->ObjectType)) {
      if ((InRange(ErrorObject2->x1,Object1->x1))
         &&
         (InRange(ErrorObject2->y1,Object1->y1))
         &&
         (InRange(ErrorObject1->x1,Object2->x1))
         &&
         (InRange(ErrorObject1->y1,Object2->y1))) {
        ok=1;
        return 1;
      }
    }
...
{
  int32 ok,cnt,cnt2,count,Found,res,Layer,TotalFound,Divisions,ObjectNr,
        StartLayer,EndLayer,CountNr,count2,cnt3,res2,MemSize,
        NrErrors,cntx,cnty,NetNr;
  double xx1,yy1,xx2,yy2,x1,y1,x11,y11,x22,y22,
         BoardDivX,BoardDivY,Clearance,MaxClearance,Length,NewLength,
         BoardStartX,BoardStartY,NewClearance,
         DesignBoardWidth,DesignBoardHeight,DesignBoardOriginX,DesignBoardOriginY,
         ObjectMinX,ObjectMinY,ObjectMaxX,ObjectMaxY,ThickNess;
  PolygonRecord  *AreaFillPolygon,*PolygonObject,*BiggerPolygon,*AreaFillPolygon2,
                 *ChangedPolygon,*BoardOutLinePolygon;
  ObjectRecord   *Object,*Object1,*Object2,NewObject,NewObject2,*ErrorObject,*Object5;
  int32  FoundError,PowerPlaneLayer,IncludeError,SurroundingError,Stop,
        FoundAreafillError,OtherErrors,CheckBoardOutline;
                ...
                if ((CheckPolygonInsideAreaFill(PolygonObject,BoardOutlineAreaFill,0)==1)
                  ...
                  if (!ErrorExist(Object,&NewObject,0)) {
                    ...
                    if (NrErrorObjects+2>=MaxNrErrorObjects) {
                      if (AllocateMemErrorObjects(MaxNrErrorObjects+128)!=0) return -4;
                    }
...
// Check objects (No areafills/big polygons) against other objects (No areafills/big polygons)
          for (cnt=0;cnt<NrObjects4;cnt++) {
...
#ifdef _DEBUG
            if ((InRange9(Object->x1,109.6e5))
               &&
               (InRange9(Object->y1,90.2e5))) {
              ok=1;
            }
            if ((InRange9(Object->x1,172.7e5))
               &&
               (InRange9(Object->y1,59.2e5))) {
              ok=1;
            }
            // Layer
            if (Object->NetNr==-1) {
            ...
            CountNr++;
            if (NewObject.NetNr!=-2) { // Non routing keepout objects
            ...
            for (cnt2=0;cnt2<NrObjects4;cnt2++) {
              if (cnt2!=cnt) {
                ...
                if (Object2->NetNr==-2) { // Routing keepout
                  switch (Object->ObjectType) {
                    case DRILL:
                    case DRILL_UNPLATED:
                ...
                switch (Object2->ObjectType) {
                  ...
                  case PIN_PUT_THROUGH_ROUND:
                  case PIN_PUT_THROUGH_SQUARE:
                    if (Object->NetNr==-2) { // Routing keepout
                      ok=1;
                      continue;
                    }
                  case PIN_PUT_THROUGH_POLYGON:
...
#ifdef _DEBUG
                if ((InRange9(Object2->x1,110.5e5))
                   &&
                   (InRange9(Object2->y1,91.57e5))) {
                  ok=1;
                  if (Object->ObjectType==OBJECT_LINE) {
                    ok=1;
                  }
                }
                if ((InRange9(Object2->x1,180.3e5))
                   &&
                   (InRange9(Object2->y1,58.4e5))) {
                  ok=1;
                }
                if (cnt==2) {
                  ok=1;
                ...
                if ((Object2->NetNr!=NetNr)
                   ...
                   (Object2->Info2==1)
                   &&
                   (Object2->minx<ObjectMaxX)
                   &&
                   (Object2->miny<ObjectMaxY)
                   &&
                   (Object2->maxx>ObjectMinX)
                   &&
                  ...
                  if (Object2->Clearance>NewObject.Clearance+10.0) {
                    memcpy(&NewObject2,Object,sizeof(ObjectRecord));
                    if (NewObject2.NetNr!=-2) { // Non routing keepout objects
...
*/
                  ...
                  if (FoundError) {
                    if (!ErrorExist(Object,Object2,0)) {
                      ...
                      ErrorObject=&((*ErrorObjects)[NrErrorObjects]);
                      memmove(ErrorObject,Object,sizeof(ObjectRecord));
                      if (Object->NetNr==-2) { // Routing keepout layer
                      ...
                      if (Object2->NetNr==-2) { // Routing keepout layer
                        ErrorObject->Layer+=ROUTING_KEEPOUT_LAYER;
                      }
                      ErrorObject->Test=0;
                      ErrorObject->Info2=0;
...
/*
          if ((cnt2 % 100) == 0) {
            sprintf(InfoStr,SC(1028,"Areafill layer %i net %s  %i (%i)"),
                                    Layer,Net->Name,cnt2,Found);
            RedrawInfoStr(1);
            CheckForEscape();
            if (SelectionEsc) {
              NrErrorObjects=0;
...
#ifdef _DEBUG
              if ((InRange9(Object->x1,102.4e5))
                 &&
                 (InRange9(Object->y1,61.5e5))) {
/*
...
*/
            ...
            if ((Object->Info2==1)
               ...
               (Object->NetNr==NetNr)) {
#ifdef _DEBUG
              if ((InRange9(Object->x1,171.8e5))
                 &&
                 (InRange9(Object->y1,94.0e5))) {
                ok=1;
              }
#endif
              Clearance=max(AreaFill->Clearance,Object->Clearance)-0.01e5;
    ...
    for (cnt=0;cnt<Design.NrAreaFills;cnt++) {
      ...
      if (((AreaFill->Info & (OBJECT_NOT_VISIBLE)) == 0)
        ...
        for (cnt2=0;cnt2<Design.NrAreaFills;cnt2++) {
          ...
          if (((AreaFill2->Info & (OBJECT_NOT_VISIBLE|POWERPLANE)) == 0)
             &&
             (AreaFill2->Layer==Layer)
             &&
...
/*
        if ((Layer>0)
           &&
           (Layer<Design.NrBoardLayers-1)) {
          for (cnt2=PinObjectsPos;cnt2<Found;cnt2++) {
//            if (cnt2==258) {
//              ok=1;
//            }
            Object=&((*Objects4)[cnt2]);
...
//          CopyPinObjectsFromRectWindowToObjects4(4,Layer);
        FoundError=0;
        cnt2=0;
        while ((!FoundError)
              &&
...
//            DrawTestPolygon(AreaFillPolygon,2);
            NrErrors++;
            if (NrErrorObjects+2>=MaxNrErrorObjects) {
              if (AllocateMemErrorObjects(MaxNrErrorObjects+128)!=0) return -4;
            }
...
//            DrawTestPolygon4(ChangedPolygon,ThickNess,0);
            ...
            for (cnt3=0;cnt3<count2;cnt3++) {
              ...
              if ((res=CheckLineCrossesWithAreaFill(x11,y11,x22,y22,AreaFill,&xx1,&yy1,&xx2,&yy2,
                                                    ThickNess*0.99,0))!=-1) {
                if (res==0) {
                  xx1=x11;
                  yy1=y11;
...
#ifdef _DEBUG
                    if (0) {
                      PolygonToObjectLines(AreaFillPolygon,INFO_LAYER3,ChangedPolygon,INFO_LAYER4,0);
              ...
              if (res!=0) {
                if ((res2=CheckLineCrossesWithAreaFill(x22,y22,x11,y11,AreaFill,&xx1,&yy1,&xx2,&yy2,
                                                      ThickNess*0.99,0))!=-1) {
                  if (res2==0) {
                    xx1=x11;
                    yy1=y11;
...
#ifdef _DEBUG
                    if (0) {
                      PolygonToObjectLines(AreaFillPolygon,INFO_LAYER,ChangedPolygon,INFO_LAYER2,0);
                    }
#endif
                      if (NrErrorObjects+2>=MaxNrErrorObjects) {
                        if (AllocateMemErrorObjects(MaxNrErrorObjects+128)!=0) return -4;
        ...
        if (cnt>0) {
          while (cnt>cnt2) {
            if (ErrorNrs[cnt]==0) {
              ...
              if (cnt2<cnt) {
                XchangeMem((uint8 *)&((*ErrorObjects)[cnt2*2]),(uint8 *)&((*ErrorObjects)[cnt*2]),
                           2*sizeof(ObjectRecord));
                ErrorNrs[cnt2]=0;
                ErrorNrs[cnt]=1;
                cnt2++;
              }
...
//    RePaint();
    ...
    if (FoundAreafillError) {
      if (OtherErrors) {
        ...
        MessageBoxOwn(PCBWindow,str,SC(102,"Warning"),MB_APPLMODAL|MB_OK);
      }
    } else {
