...
char                        CurDir[400];
int32                       SharedMemoryInfo,ok,*ProcessesReady,*ProcessesStart,
                            *ProcessesResultOk,*ProcessesResultError,
                            *FullStop,*ModuleStop,
                            WaitMask,ProcessMask;
...
runcmd d:\projects\pcbeleg35\make_install_mingw.cmd

runcmd -p,m,1   d:\projects\pcbeleg35\runcmd\test2.cmd
runcmd -p,f,1   d:\projects\pcbeleg35\runcmd\test2.cmd
runcmd -p,w,45,20   d:\projects\pcbeleg35\runcmd\test2.cmd
...
{
  char str[400],str2[400],str3[400],StartUpStr[400],*strp,
       ExeFile[400],ExeParams[400],Parameters[400],Options[400],
       Options1[400],Options2[400],Options3[400];
  ...
  if (!CreateProcess(ExeFile,ExeParams,NULL,NULL,1,0,
...
{
  char str[400],str2[400],str3[400],str4[400],StartUpStr[400],*strp,
       ExeFile[400],ExeParams[400],Parameters[400],Options[400],
       Options1[400],Options2[400],Options3[400];
  int32 retcode,result,FileLength,cnt,WriteResult,tel2,tel,KeySize,Length,
        Mask,res,MaxWait,WaitCnt,BuildTime;
  FILE *fp;
...
//  sprintf(Parameters,"-r %s",str4);
  ...
  if (strcmp(Options1,"print")==0) {
    if (SharedMemory) {
      sprintf(str2,"ProcessesReady=0x%x\r\nProcessesStart=0x%x\r\nProcessesResultError=0x%x",
              *ProcessesReady,*ProcessesStart,*ProcessesResultError);
      MessageBox(NULL,str2,"",MB_APPLMODAL|MB_OK);
      return 0;
    }
    return -1;
  ...
  if (strcmp(Options1,"w")==0) {
    ...
    sprintf(str,"Waiting for %d %d %d (mask=%d) (max wait=%d msec)\n",
            *ProcessesReady,*ProcessesStart,SharedMemoryInfo,WaitMask,MaxWait);
    MessageBox(NULL,str,"Message",MB_APPLMODAL|MB_OK);
#endif
    if (MaxWait==0) MaxWait=10000;
    WaitCnt=0;
    if (WaitMask) {
