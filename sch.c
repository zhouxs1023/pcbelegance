...
#define  ScreenPosAbsCursor             10
#define  ScreenPosAbsGridCursor         80
#define  ScreenPosRelGridCursor         80
#define  ScreenPosInfoStr               150
...
HWND                        DialogWindow,SCHFrameWindow,HScrollWindow,MasterWindow,
                            hwndHBar,hwndVBar;
HDC                         DialogDisplay,OutputDisplay2;
HBITMAP                     ViewBitmap;
HRSRC                       FontResourceHandle;
PAINTSTRUCT                 PS;
RECT                        RealWindow,ClientRect,UpdateRect,ScreenRect,RealWindow2,
                            OldClientRect,OldRealWindow,OldRealWindow2;
...
int32                       DrawWindowMinX,DrawWindowMinY,DrawWindowMaxX,DrawWindowMaxY,
                            ...
                            ScreenStartX,ScreenStartY,EditMode,
                            GeomScreenWidth,GeomScreenHeight,
                            HeightScrollBar,WidthScrollBar,
                            GeomStartX,GeomStartY,FirstSize,
                            ScrollBarSizeX,ScrollBarSizeY,
                            MousePosOldX,MousePosOldY,
                            AllScreenX1,AllScreenY1,AllScreenX2,AllScreenY2;
...
char                        IniFile[MAX_LENGTH_STRING],StartDir[MAX_LENGTH_STRING],ExecutableDir[MAX_LENGTH_STRING],
                            PrintingParameters[MAX_LENGTH_STRING],SchExecutable[MAX_LENGTH_STRING],
                            MessageStr[MAX_LENGTH_STRING];
...
extern int32                SymbolDialogInitialX,SymbolDialogInitialY,
                            GeometryDialogInitialX,GeometryDialogInitialY,
...

{
  if (!DCInUse) {
    DCInUse=1;
    if ( Painting ) {
      if (!Printing) OutputDisplay = BeginPaint(SCHWindow, &PS);
    } else {
      if (!Printing) OutputDisplay = GetDC(SCHWindow);
    }
    SetROP2(OutputDisplay,R2_COPYPEN);
    if (!Printing) SetBkMode(OutputDisplay,TRANSPARENT);
...

{
  ...
  EditingRegion = CreateRectRgn((int16)DrawWindowMinX,(int16)DrawWindowMinY,
                                (int16)DrawWindowMaxX,(int16)DrawWindowMaxY);
  if (!Printing) SelectClipRgn(OutputDisplay, EditingRegion);
  SetROP2(OutputDisplay,R2_COPYPEN);
  BackGroundActive=0;
}
...
{
  if (DCInUse) {
    if ( Painting ) EndPaint(SCHWindow, &PS);
    else ReleaseDC(SCHWindow, OutputDisplay);
    OutputDisplay=NULL;
    DCInUse=0;
  }
}
...
//    res=GetDIBits(OutputDisplay,ViewBitmap,0,ViewPixelsY,TempMem,BitmapInfo,DIB_RGB_COLORS);
    ...
    if ( Painting ) EndPaint(SCHWindow, &PS);
    else ReleaseDC(SCHWindow, OutputDisplay2);
    DCInUse=0;
    ViewBitmap=NULL;
    OutputDisplay=NULL;
    OutputDisplay2=NULL;
...

{
  ...
  if (!Printing) SelectClipRgn(OutputDisplay, NULL);
...

{
  ...
  if ( GetUpdateRect(SCHWindow,&UpdateRect,0) ) {
    ...
    if (!PaintBecauseOfResize) {
      ...
      if (UpdateRect.bottom>=(int16)DrawWindowMaxY+HeightScrollBar) RedrawInfoBar();
      if (UpdateRect.left<(int16)DrawWindowMinX) RedrawButtons();
      if ((UpdateRect.bottom>=(int16)DrawWindowMinY)
         &&
         (UpdateRect.top<(int16)DrawWindowMaxY)) {
        RedrawMainWindow();
      }
      if (FastPaint) {
        DoneDeviceContext2();
...
//      Focused=0;
    ...
    case WM_CREATE:
      ...
      break;
//    case WM_COMMAND:
//      if ( HIWORD(LParam)==0 ) SCHCommand(WParam);
//      break;
// /ue:\pcb_elegance /ee:\pcb_elegance /pe:\pcb_elegance\cdio e:\pcb_elegance\cdio\sch\top.sch
    case WM_PAINT:
      if (FirstPaint) {
        FirstPaint=0;
        if (EditingSymbol) {
...
//      DrawInfo=LParam;
    ...
    case WM_TIMER:
      ...
      if ((TimerValue % 10) == 0) {
        ...
        if ((CheckNewProperties(0)==1)
           ||
           (CheckNewUserVars(0)==1)) {
          RePaint();
        }
      }
      if (CheckSchematic) {
...
{
  int32  fp,Length,Value,ok,Key,ParamMode,
...
//  MessageBoxUTF8(SCHWindow,str1,SchNamesId[16],MB_OK);
  ...
  if ((fp=TextFileOpenUTF8(FileName))<0) return;
  ...
  while ((Length=ReadLn(fp,LineBuf))>=0) {
    ...
    if ((Length>1)
      ...
      if (str1[0]=='[') {
        ParamMode=0;
        if (stricmp(str1,"[Files]")==0) ParamMode=0;
        if (stricmp(str1,"[Settings]")==0) ParamMode=1;
...

{
  ...
  if ((fp=FileOpenWriteUTF8(FileName))<0) return;
...

{
  ...
  if ((ID<0)
     ||
     (ID>=MaxNrSchNames)) return -1;
  if (SchNamesPos+Length+2>=SchNamesBufSize) return -1;
...

{
  ...
  char   LineBuf[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str[MAX_LENGTH_STRING],
         LanguageFileName[MAX_LENGTH_STRING],
  ...
  for (cnt=0;cnt<NrParams;cnt++) {
    if (Parameters[cnt].Option[0]!=0) {
      ...
      if (strnicmp(Parameters[cnt].Option,"w",lengte)==0) {
        sscanf((LPSTR)&Parameters[cnt].Parameter[pos],"%x",&MasterWindow);
      }
      ...
      if ((mode==0)
        ...
        if (ConvertFileNameFromCommandLineUTF8(Parameters[cnt].Parameter,EditFile)==-1) {
          MessageBoxUTF8(SCHWindow,Parameters[cnt].Parameter,
                         SC(197,"Error in opening file"),MB_APPLMODAL|MB_OK);
        }
      }
    }
  }
...
//    SCHClass.hbrBackground = GetStockObject(BLACK_BRUSH);
    ...
    if ( ! RegisterClass(&SCHClass) ) exit(255);
  }
...
//  res=GetFilePartFromFileName(str,"c:\\ab\\");
  ...
  if (AddSchLanguageStrings(str)==-2) return -1;
  LoadDesignIniFile();
    ...
    if (res==0) {
      if (DirectPrinting==1) {
        ...
        if (strcmp(str,"$PDF$")) {
          ...
          ExportToPDF(DirectPrintingPaperSize,DirectPrintingOrientation,DirectPrintingFitToPage,PdfObjectStart,1);
          if (ProjectInfo!=NULL) {
            Sleep(200);
            ProjectInfo->PrintingBusy=0;
          }
...
//  MessageBox(NULL,DesignPath,"DesignPath",MB_APPLMODAL+MB_OK);
  ...
  SCHWindow = CreateWindowEx(WS_EX_ACCEPTFILES,
                             SchClassName,
                             NULL,
                             WS_OVERLAPPEDWINDOW,
                             WindowStartX,
...
//  SetSymbolMenu();
  ...
  hwndHBar = CreateWindow ("scrollbar", NULL, WS_CHILD | WS_VISIBLE | SBS_HORZ,
                           0,0,0,0, SCHWindow, (HMENU) 1, hInstance, NULL);
  hwndVBar = CreateWindow ("scrollbar", NULL, WS_CHILD | WS_VISIBLE | SBS_VERT,
                           0,0,0,0, SCHWindow, (HMENU) 2, hInstance, NULL);
