...
//  SetROP2(OutputDisplay,R2_COPYPEN);
  ...
  switch (SelectionMode) {
    case DRAG_TRACES_VIAS_COMPS_MODE:
      ...
      if (OkToDrawVias) SelectViasFromRectWindow();
      SelectConnectionsFromRectWindow();
      if (OkToDrawAreaFills) SelectAreaFillsFromRectWindow(0);
      SelectObjectLinesFromRectWindow(1);
      SelectObjectArcsFromRectWindow(1);
      if (UnselectAll) {
        strcpy(InfoStr,SC(532,"Drag traces/vias/components"));
        RedrawInfoStr(0);
      }
      break;
    case ROUTING_MODE: // Routing
      SelectTracesFromRectWindow();
//      if (UnselectAll) {
//        strcpy(InfoStr,SC(139,"Routing traces"));
//        RedrawInfoStr(0);
//      }
      break;
    case MOVE_ONE_TRACE_MODE: // Moving one trace
      SelectTracesFromRectWindow();
//      if (UnselectAll) {
//        strcpy(InfoStr,SC(331,"Drag one trace"));
//        RedrawInfoStr(0);
//      }
      break;
    ...
    case MOVE_COMPONENT_REFERENCES_MODE:
      if (OkToDrawCompReference) {
        ...
        if (UnselectAll) {
          strcpy(InfoStr,SC(57,"Component references"));
          RedrawInfoStr(0);
        }
      }
      break;
...
*/
    ...
    case AREAFILLS_MODE:
      if (OkToDrawAreaFills) SelectAreaFillsFromRectWindow(0);
      if (UnselectAll) {
        strcpy(InfoStr,SC(531,"Add/change areafills/powerplanes"));
        RedrawInfoStr(0);
      }
      break;
    ...
    case GATE_PINSWAP_MODE:
      if (!UnselectAll) SelectGatePinSwap(0);
      break;
  }
  ExitDrawing();
...
*/
  ...
  if ((FirstProtectedComp)
     &&
     (Selected==0)) {
    sprintf(InfoStr,SC(1299,"%s  %s (Protected)  [ %d selected ]"),FirstProtectedComp->Name,FirstProtectedComp->Value,Selected);
    RedrawInfoStr(0);
  } else {
    if (FirstComps[0]) {
      if (Selected==1) {
        sprintf(InfoStr,SC(1300,"%s  ( %s )  [ %d selected ]"),FirstComps[0]->Name,FirstComps[0]->Value,Selected);
      } else {
        InfoStr[0]=0;
        ...
        sprintf(str,SC(1301,"  [ %d selected ]"),Selected);
        strcat(InfoStr,str);
      }
      RedrawInfoStr(0);
    } else {
          ...
          if (UnselectAll) {
            ...
            if ((RectTestRect2(x1,y1+(lengte/2),dikte,lengte))
              ...
              if ((SearchMaxX>x1)
                 ...
                 (SearchMaxY>y1)
                 &&
                 (SearchMinY<y1)) TraceInfo|=0x0001;
              if ((SearchMaxX>x2)
                 &&
                 (SearchMinX<x2)
                 &&
                 (SearchMaxY>y2)
                 &&
                 (SearchMinY<y2)) TraceInfo|=0x0002;
              if ((TraceInfo & (OBJECT_SELECTED|3)) == OBJECT_SELECTED) TraceInfo|=3;
              ObjectChanged=1;
            }
          }
        }
...

{
  ...
  for (cnt=0;cnt<Design.NrObjectRects;cnt++) {
    ...
    if (((ObjectRectInfo & (OBJECT_NOT_VISIBLE)) == 0)
       &&
       (OkToDrawObjectsLayer(ObjectRect->Layer)==0)) {
    ...
    if ((ObjectArcInfo & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      if (OkToCheck) {
        ...
        if (UnselectAll) {
          ...
          if (((!ActiveCircle)
             &&
             (RectTestArc2(x1,y1,x2,y2,x3,y3,x4,y4,ObjectArc->LineThickNess)))
             ||
             ((ActiveCircle)
             &&
             (RectTestCircle(x1,y1,x2,255)))) {
            if (!ReplaceSelections) {
              if ((ObjectArcInfo & OBJECT_SELECTED) == OBJECT_SELECTED) {
                if (ShiftPressed) {
                  ObjectArcInfo&=~OBJECT_SELECTED;
...
{
  int32  cnt,cnt3,NrLines,ObjectTextInfo,TextMode,TextAlignment,
         MaxCountX,FontNr,Mirror;
  ...
  for (cnt=0;cnt<Design.NrObjectTexts2;cnt++) {
    ...
    if (((ObjectTextInfo & (OBJECT_NOT_VISIBLE)) == 0)
       &&
       (OkToDrawObjectsLayer(ObjectText2->Layer)==0)) {
      x1=ObjectText2->X;
      y1=ObjectText2->Y;
...

{
  ...
  for (cnt=0;cnt<Design.NrObjectPolygons;cnt++) {
    ...
    if (((ObjectPolygonInfo & (OBJECT_NOT_VISIBLE)) == 0)
       &&
       (OkToDrawObjectsLayer(ObjectPolygon->Layer)==0)) {
      ObjectChanged=0;
      if (UnselectAll) {
...

{
  ...
  for (cnt=0;cnt<Design.NrComps;cnt++) {
    ...
    if (((Mirror == 0 ) && (!OkToDrawSilkScreenTop))
       ||
       ((Mirror == 1 ) && (!OkToDrawSilkScreenBottom))) {
      continue;
    }
...
//       ((TextVisibility & 4) == 0)) {
      if (UnselectAll) {
        if (((TextVisibility & 1) == 1)
           ||
           ((Comp->Info & OBJECT_SELECTED) == OBJECT_SELECTED)) {
          TextVisibility&=~1;
          ObjectChanged=1;
        }
      } else {
...
{
  int32  cnt,MemPos,lengte,OldTextVisibility,OldInfo,
         Mirror,CompInfo,ShapeNr,TextVisibility;
...

{
  ...
  for (cnt=0;cnt<Design.NrComps;cnt++) {
    ...
    if (((Comp->Info & (OBJECT_NOT_VISIBLE)) == 0)
       &&
       ((Comp->TextVisibility & 0x10) == 0x10)) count++;
  }
  return count;
}
...
#ifdef _DEBUG
              if (cnt==2) {
                if ((DrawPolygon->PolygonType & 2) == 2) {  // User deletion
                  ok=1;
                }
                if (cnt2==1) {
                  ok=1;
...
//#else
                  ...
                  if (((DrawPolygon->PolygonType & 1) == 1)
                     ||
                     ((DrawPolygon->PolygonType & 8) == 8)) {
                    if (!ReplaceSelections) {
                      if ((DrawPolygon->PolygonType & 2) == 2) {
                        if (ShiftPressed) {
                          DrawPolygon->PolygonType&=~2;
            ...
            if ((AreaFill->maxx>=SearchMinX)
               &&
               (AreaFill->minx<=SearchMaxX)
               &&
               (AreaFill->maxy>=SearchMinY)
               &&
               (AreaFill->miny<=SearchMaxY)) {
              if (!ReplaceSelections) {
                if ((DrawPolygon->PolygonType & 2) == 2) {
                  if (ShiftPressed) {
                    DrawPolygon->PolygonType&=~2;
      ...
      if (ObjectChanged) {
        ...
        sprintf(InfoStr,SC(1035,"AreaFill %s          Net %s"),LayerText,Net->Name);
        RedrawInfoStr(0);
      }
    }
  }
...

{
  if (Units==0) {
    ...
    if (mode==1) strcat(ValueStr,SC(1036,"  mils"));
  } else {
    sprintf(ValueStr,"%.4f",value/100000.0);
    if (mode==1) strcat(ValueStr,SC(1037,"  mm"));
  }
}
...

{
  ...
  if (Layer==0) {
    sprintf(ValueStr,SC(1038,"Trace\tBottom\t"),Layer);
  } else {
    if (Design.NrBoardLayers>1) {
      if (Layer==Design.NrBoardLayers-1) {
        sprintf(ValueStr,SC(1039,"Trace\tTop\t"),Layer);
      } else {
        sprintf(ValueStr,SC(1040,"Trace\tLayer %i\t"),Layer);
      }
    }
  }
  NetNr=Trace->NetNr;
...

{
  ...
  double  x1,y1,x2,y2,dikte,Rotation,Dist1,Dist2,
         Clearance,Angle1,Angle2,Angle,Length;
  char   str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],Val2[MAX_LENGTH_STRING],
         Val4a[MAX_LENGTH_STRING],Val5[MAX_LENGTH_STRING],Val6[MAX_LENGTH_STRING],
         Val6a[MAX_LENGTH_STRING],Val7[MAX_LENGTH_STRING],Val7a[MAX_LENGTH_STRING],
         Val8[MAX_LENGTH_STRING],LayerText[MAX_LENGTH_STRING],ObjectText[MAX_LENGTH_STRING],
  ...
  if (Units==0) {
    ...
    if (((Object->Info & OBJECT_FILLED) == 0)
      ...
      if (Object->Layer==DRILL_LAYER) {
        sprintf(Val2 ,"%.1f  %.1f\t%.1f (%.1f)\t",
                             x1,y1,x2,Clearance/2540.0);
      } else {
        sprintf(Val2 ,"%.1f  %.1f\t%.1f\t%.1f",
                             x1,y1,x2,dikte);
      }
    } else {
      sprintf(Val2 ,"%.1f  %.1f\t%.1f\t\t\t",
                           x1,y1,x2);
    }
    sprintf(Val4a,"%.1f  %.1f\t%.1f\t%.1f\t%i",
                         x1,y1,x2,dikte,rot);
    sprintf(Val5,"%.1f  %.1f\t%.1f\t%.1f\t%.1f",
                         x1,y1,x2,dikte,Rotation);
    ...
    sprintf(Val6 ,"%.1f  %.1f\t%.1f  %.1f\t%.1f\t%.1f  %.1f",
                         x1,y1,x2,y2,dikte,Angle1,Angle2);
    sprintf(Val6a ,"%.1f  %.1f\t%.1f  %.1f\t%.1f (%.1f)\t%.1f  %.1f",
                         x1,y1,x2,y2,dikte,Clearance/2540.0,Angle1,Angle2);
    sprintf(Val7 ,SC(1043,"%.1f  %.1f\t%.1f  %.1f\t%.1f\tLength  %.1f, Angle %.1f"),
                         x1,y1,x2,y2,dikte,Length/2540.0,Angle*180.0/PI);
    sprintf(Val7a,SC(1104,"%.1f  %.1f\t%.1f  %.1f\t%.1f (%.1f)\tLength  %.1f, Angle %.1f"),
                         x1,y1,x2,y2,dikte,Clearance/2540.0,Length/2540.0,Angle*180.0/PI);
    if ((Object->Info & OBJECT_FILLED) == 0) {
      sprintf(Val8 ,"%.1f  %.1f\t%.1f  %.1f\t%.1f",
                           x1,y1,x2,y2,dikte);
    } else {
      sprintf(Val8 ,"%.1f  %.1f\t%.1f  %.1f\t\t\t",
                           x1,y1,x2,y2);
    }
  } else {
    x1=x1/100000.0;
    ...
    if (((Object->Info & OBJECT_FILLED) == 0)
       ...
       (Object->Layer==DRILL_LAYER)) {
      if (Object->Layer==DRILL_LAYER) {
        sprintf(Val2 ,"%.4f  %.4f\t%.4f (%.4f)\t",
                             x1,y1,x2,Clearance/100000.0);
      } else {
        sprintf(Val2 ,"%.4f  %.4f\t%.4f\t%.4f",
                             x1,y1,x2,dikte);
      }
    } else {
      sprintf(Val2 ,"%.4f  %.4f\t%.4f\t\t\t",
                           x1,y1,x2);
    }
    sprintf(Val4a,"%.4f  %.4f\t%.4f\t%.4f\t%i",
                         x1,y1,x2,dikte,rot);
    sprintf(Val5,"%.4f  %.4f\t%.4f\t%.4f\t%.1f",
                         x1,y1,x2,dikte,Rotation);
    if (Object->ObjectType==OBJECT_ARC) {
      ConvertPointToPolar(Object->x3,Object->y3,&Dist1,&Angle1);
      ConvertPointToPolar(Object->x4,Object->y4,&Dist2,&Angle2);
    }
    sprintf(Val6 ,"%.4f  %.4f\t%.4f  %.4f\t%.4f\t%.1f  %.1f",
                         x1,y1,x2,y2,dikte,Angle1,Angle2);
    sprintf(Val6a,"%.4f  %.4f\t%.4f  %.4f\t%.4f (%.4f)\t%.1f  %.1f",
                         x1,y1,x2,y2,dikte,Clearance/100000.0,Angle1,Angle2);
    sprintf(Val7 ,SC(1044,"%.4f  %.4f\t%.4f  %.4f\t%.4f\tLength  %.4f, Angle %.1f"),
                         x1,y1,x2,y2,dikte,Length/100000.0,Angle*180.0/PI);
    sprintf(Val7a,SC(1105,"%.4f  %.4f\t%.4f  %.4f\t%.4f (%.4f)\tLength  %.4f, Angle %.1f"),
                         x1,y1,x2,y2,dikte,Clearance/100000.0,Length/100000.0,Angle*180.0/PI);
    if ((Object->Info & OBJECT_FILLED) == 0) {
      sprintf(Val8 ,"%.4f  %.4f\t%.4f  %.4f\t%.4f",
                           x1,y1,x2,y2,dikte);
    } else {
      sprintf(Val8 ,"%.4f  %.4f\t%.4f  %.4f\t\t\t",
                           x1,y1,x2,y2);
    }
  }
...
{
  int32  cnt,cnt2,Layer,MemPos,NetNr,TraceInfo,ViaInfo,CompInfo,ShapeNr,
         ObjectsSelected,UserCutOuts,UserInternalCutOuts,ThermalReliefCutOuts;
  double Rotation;
  TraceRecord  *Trace;
  CompRecord  *Comp;
  ViaRecord *Via;
  ...
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],
        ...
        str7[MAX_LENGTH_STRING],str8[MAX_LENGTH_STRING],str9[MAX_LENGTH_STRING];
  PolygonRecord *DrawPolygon;
  uint8   *AreaPos,*PolygonPos;
#ifdef _DEBUG
  ...
  if (SelectionMode==DRAG_TRACES_VIAS_COMPS_MODE) return;
  MessageBufPos=0;
  ObjectsSelected=0;
  cnt2=0;
  for (cnt=0;cnt<Design.NrComps;cnt++) {
    ...
    if ((CompInfo & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
      ...
      if ((Comp->CompMode & 8) == 0) {
        sprintf(str,SC(1048,"%s\tTop\t%s\t%s\t%s\t%s\t%s\t%.1f"),str7,str5,str6,str4,str8,str9,Rotation);
      } else {
        sprintf(str,SC(1049,"%s\tBottom\t%s\t%s\t%s\t%s\t%s\t%.1f"),str7,str5,str6,str4,str8,str9,Rotation);
      }
      if (AddToMessageBuf(str)!=0) return;
//      sprintf(str,"\t\t%s,%s\t%i",
//      if (AddToMessageBuf(str)!=0) return;
    ...
    if ((AreaFill->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      ...
      if ((AreaFill->Info & (POWERPLANE)) == 0) {
        ...
        if ((FirstPolygon->PolygonType & 2) == 2) {
          ...
          sprintf(str3,"\t%i\t%i [ User,internal,thermal %d,%d,%d ]  %d",
                       AreaFill->MemSize,AreaFill->NrPolygons-1,
                       UserCutOuts,UserInternalCutOuts,ThermalReliefCutOuts,cnt);
          strcat(str,str3);
          ...
          for (cnt2=1;cnt2<AreaFill->NrPolygons;cnt2++) {
            if ((DrawPolygon->PolygonType & 2) == 2) {
              sprintf(str,SC(1013,"Areafill cut out\t%s\t%s\t%i"),str2,Net->Name,DrawPolygon->NrVertices);
              if ((DrawPolygon->PolygonType & 5) == 1) {
        ...
        if ((FirstPolygon->PolygonType & 2) == 2) {
          ...
          for (cnt2=1;cnt2<AreaFill->NrPolygons;cnt2++) {
            if ((DrawPolygon->PolygonType & 2) == 2) {
              ...
              sprintf(str,SC(1057,"Powerplane cut out\t%s\t%s\t%i"),str2,Net->Name,DrawPolygon->NrVertices);
              if (AddToMessageBuf(str)!=0) return;
            }
            PolygonPos+=MemSizePolygon(DrawPolygon);
            DrawPolygon=(PolygonRecord *)PolygonPos;
