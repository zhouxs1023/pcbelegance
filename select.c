...

{
  switch (Layer) {
    case SOLD_MASK_BOTTOM_LAYER:
      if (!SoldMaskPadsBottomVisible) return 0;
      break;
    case SOLD_MASK_TOP_LAYER:
      if (!SoldMaskPadsTopVisible) return 0;
      break;
    case PASTE_MASK_TOP_LAYER:
      if (!PastePadsTopVisible) return 0;
      break;
    case PASTE_MASK_BOTTOM_LAYER:
      if (!PastePadsBottomVisible) return 0;
      break;
    case SILKSCREEN_TOP_LAYER:
      if (!SilkScreenTopVisible) return 0;
      break;
    case SILKSCREEN_BOTTOM_LAYER:
      if (!SilkScreenBottomVisible) return 0;
      break;
    case PLACEMENT_OUTLINE_LAYER:
      if (!PlacementVisible) return 0;
      break;
    case COMP_OUTLINE_LAYER:
      if (!CompOutlineVisible) return 0;
      break;
    case BOARD_OUTLINE_LAYER:
      if (!BoardOutlineVisible) return 0;
      break;
    case GEOM_NAME_LAYER:
      if (!GeomNameVisible) return 0;
      break;
    case POWER_PAD_LAYER:
      if (!PowerPadsVisible) return 0;
      break;
    case INNER_PAD_LAYER:
      if (NrPadLayers==2) {
        if (!InnerPadsVisible) return 0;
      }
      break;
    case DRILL_LAYER:
      if (!DrillVisible) return 0;
      break;
    case DRILL_UNPLATED_LAYER:
      if (!DrillUnplatedVisible) return 0;
      break;
    case INFO_LAYER:
      if (!Info1Visible) return 0;
      break;
    case INFO_LAYER2:
      if (!Info2Visible) return 0;
      break;
    case INFO_LAYER3:
      if (!Info3Visible) return 0;
      break;
    case INFO_LAYER4:
      if (!Info4Visible) return 0;
      break;
    case POLYGON_DRAW_LAYER:
      break;
    default:
      if (Layer<32) {
        if (!PadsVisible[Layer]) return 0;
      } else {
        if ((Layer>=ROUTING_KEEPOUT_LAYER)
           &&
           (Layer<ROUTING_KEEPOUT_LAYER+32)) {
          if (!RoutingKeepoutVisible[Layer-ROUTING_KEEPOUT_LAYER]) return 0;
        } else {
          return 0;
        }
      }
    ...
    if ((ObjectInfo & (OBJECT_NOT_VISIBLE)) == 0) {
      switch (Object->ObjectType) {
        case OBJECT_RECT:
          ...
          x2=Object->x2;
          y2=Object->y2;
          if (RectTestRect2(x1,y1,x2,y2)) return cnt;
          break;
        case OBJECT_CIRCLE:
          x1=Object->x1;
          y1=Object->y1;
          x2=Object->x2;
          if (RectTestCircle(x1,y1,x2,255)) return cnt;
          break;
      }
    }
  }
...
// ***************************************************************************************************
        ...
        case ID_SELECT_ONLY_PADS_31:
          if (Layer!=Modifier-ID_SELECT_ONLY_PADS) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_ANTI_POWERPADS:
          if (Layer!=POWER_PAD_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_INNERPADS:
          if (Layer!=INNER_PAD_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_INFO1:
          if (Layer!=INFO_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_INFO2:
          if (Layer!=INFO_LAYER2) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_INFO3:
          if (Layer!=INFO_LAYER3) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_INFO4:
          if (Layer!=INFO_LAYER4) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_DRILL:
          if (Layer!=DRILL_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_DRILL_UNPLATED:
          if (Layer!=DRILL_UNPLATED_LAYER) ObjectChanged=1;
          break;
/*
...
*/
        case ID_SELECT_ONLY_SILK_TOP:
          if (Layer!=SILKSCREEN_TOP_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_SILK_BOTTOM:
          if (Layer!=SILKSCREEN_BOTTOM_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_COMP_OUTLINE:
          if (Layer!=COMP_OUTLINE_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_PLACEM_OUTLINE:
          if (Layer!=PLACEMENT_OUTLINE_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_MASK_TOP:
          if (Layer!=SOLD_MASK_TOP_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_MASK_BOTTOM:
          if (Layer!=SOLD_MASK_BOTTOM_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_PASTE_TOP:
          if (Layer!=PASTE_MASK_TOP_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_PASTE_BOTTOM:
          if (Layer!=PASTE_MASK_BOTTOM_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_LINES:
          if (ObjectType!=OBJECT_LINE) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_RECTS:
          if (ObjectType!=OBJECT_RECT) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_CIRCLES:
          if (ObjectType!=OBJECT_CIRCLE) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_ARCS:
          if (ObjectType!=OBJECT_ARC) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_TEXT:
          if (ObjectType!=OBJECT_TEXT) ObjectChanged=1;
          break;
        ...
        case ID_SELECT_ONLY_ROUT_KEEPOUT_31:
          if (Layer!=Modifier-ID_SELECT_ONLY_ROUT_KEEPOUT+ROUTING_KEEPOUT_LAYER) ObjectChanged=1;
          break;
        case ID_SELECT_ONLY_BOARD_OUTLINE:
          if (Layer!=BOARD_OUTLINE_LAYER) ObjectChanged=1;
          break;
// ***************************************************************************************************
        ...
        case ID_UNSELECT_PADS_31:
          if (Layer==Modifier-ID_UNSELECT_PADS) ObjectChanged=1;
          break;
        ...
        case ID_UNSELECT_ROUT_KEEPOUT_31:
          if (Layer!=Modifier-ID_UNSELECT_ROUT_KEEPOUT+ROUTING_KEEPOUT_LAYER) ObjectChanged=1;
          break;
...

{
  ...
  if (lengte+2+MessageBufPos>=MessageBufMemSize) {
    if (AllocateMemMessageBuf(MessageBufMemSize*2)!=0) return -1;
  }
...

{
  ...
  double  x1,y1,x2,x2a,y2,y2a,dikte,cl,Rotation,
          Dist1,Dist2,Angle1,Angle2,lengte;
  PinInfoRecord *PinInfo;
  ObjectRecord  *Object;
  char   str[MAX_LENGTH_STRING],PinStr[MAX_LENGTH_STRING],Val1[MAX_LENGTH_STRING],
         Val2[MAX_LENGTH_STRING],Val2a[MAX_LENGTH_STRING],Val2b[MAX_LENGTH_STRING],
         Val3[MAX_LENGTH_STRING],Val4[MAX_LENGTH_STRING],Val4a[MAX_LENGTH_STRING],
         Val6[MAX_LENGTH_STRING],Val6a[MAX_LENGTH_STRING],Val7[MAX_LENGTH_STRING],
         Val8[MAX_LENGTH_STRING],CircleAngleString[MAX_LENGTH_STRING],
         LayerStr[MAX_LENGTH_STRING],ObjectStr[MAX_LENGTH_STRING];
  ObjectPolygonRecord *ObjectPolygon;
  ...
  for (cnt=0;cnt<NrObjects;cnt++) {
    ...
    if ((ObjectInfo & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED) {
      ...
      if (Rotation>1000.0) Rotation-=2000.0;
      str[0]=0;
      ...
      if (Units==0) {
        ...
        if (PinStr[0]==0) {
          sprintf(Val7,SC(413,"%.2f,%.2f - %.2f,%.2f\tLength %.2f ( Angle %.2f )\t%.2f"),x1,y1,x2,y2,lengte,Angle1,dikte);
          sprintf(Val8,"%.2f,%.2f\t%.2f,%.2f ( %.2f,%.2f )\t%.2f",x1,y1,x2,y2,Angle1,Angle2,dikte);
        } else {
          sprintf(Val7,SC(414,"%.2f,%.2f - %.2f,%.2f\tLength %.2f ( Angle %.2f )\t%.2f\t%.2f"),x1,y1,x2,y2,lengte,Angle1,dikte,cl);
          sprintf(Val8,"%.2f,%.2f\t%.2f,%.2f ( %.2f,%.2f )\t%.2f\t%.2f",x1,y1,x2,y2,Angle1,Angle2,dikte,cl);
        }
      } else {
        if (PinStr[0]==0) {
        ...
        if (PinStr[0]==0) {
          sprintf(Val7,SC(416,"%.4f,%.4f - %.4f,%.4f\tLength %.4f ( Angle %.2f )\t%.4f\t%.4f"),x1,y1,x2,y2,lengte,Angle1,dikte,cl);
          sprintf(Val8,"%.4f,%.4f\t%.4f,%.4f ( %.2f,%.2f )\t%.4f",x1,y1,x2,y2,Angle1,Angle2,dikte);
        } else {
          sprintf(Val7,SC(417,"%.4f,%.4f - %.4f,%.4f\tLength %.4f ( Angle %.2f )\t%.4f"),x1,y1,x2,y2,lengte,Angle1,dikte);
          sprintf(Val8,"%.4f,%.4f\t%.4f,%.4f ( %.2f,%.2f )\t%.4f\t%.4f",x1,y1,x2,y2,Angle1,Angle2,dikte,cl);
        }
      }
...
#ifdef _DEBUG
        sprintf(str,SC(426,"Polygon\t%s\t%s\t%s\t%d vertices (%d)\t\t%s"),
                LayerStr,PinStr,Val1,count,ObjectPolygon->NrSubPolygons,Val2);
#else
...

{
  ...
  for (cnt=0;cnt<NrObjects;cnt++) {
    Object=&((*Objects)[cnt]);
    if ((Object->Info & (OBJECT_NOT_VISIBLE)) == 0) {
      if (GetObjectSize(Object,&MinX,&MinY,&MaxX,&MaxY)) Changed=1;
    }
  }
