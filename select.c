...
//  RedrawInfoStr(0);
	...
	{
		sprintf(InfoStr, SC(1299, "%s  %s (Protected)  [ %d selected ]"), FirstProtectedComp->Name,
	...
	else
	{
		if (FirstComps[0])
		{
			if (Selected == 1)
			{
				sprintf(InfoStr, SC(1300, "%s  ( %s )  [ %d selected ]"), FirstComps[0]->Name, FirstComps[0]->Value,
				        Selected);
			}
			else
			{
