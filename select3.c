  ...
  for (Layer=start;Layer<end;Layer++) {
    ...
    for (cnt=0;cnt<Design.NrVerTraces[Layer];cnt++) {
      ...
      if ((TraceInfo & CompareMask) == CompareValue) {
        ...
        if ((mode & 4) == 4) dikte+=Trace->Clearance*2;
          ...
          if ((mode & 1) == 0) {
            if (NrObjects4<MaxNrObjects4) {
              ...
              if (NrObjects4>=MaxNrObjects4-1) {
                if (AllocateMemObjects4(MaxNrObjects4+1024)==0) NrObjects4++;
              } else NrObjects4++;
            }
          }
          NrFound++;
...
*/
  ...
  for (cnt=0;cnt<Design.NrAreaFills;cnt++) {
    ...
    if (((AreaFill->Info & (CompareMask)) == CompareValue)
       ...
       ((SelectedLayer==-1)
       ||
       (AreaFill->Layer==SelectedLayer))) {
      AreaPos=(uint8 *)AreaFill;
      count=sizeof(AreaFillRecord);
      DrawPolygon=(PolygonRecord *)(AreaPos+sizeof(AreaFillRecord));
      if ((SearchMaxX>AreaFill->minx-100000.0)
         &&
         (SearchMinX<AreaFill->maxx+100000.0)
         &&
         (SearchMaxY>AreaFill->miny-100000.0)
         &&
         (SearchMinY<AreaFill->maxy+100000.0)) {
        if ((mode & 1) == 0) {
          if (NrObjects4<MaxNrObjects4) {
            Object4=&((*Objects4)[NrObjects4]);
            Object4->x1=0.0;
...
*/
  int32               cnt,cnt2,NrFound,CompareMask,CompareValue,
                      TestMode,TestRectangle,InnerLayer;
  double              x1,y1,x2,y2;
...
#ifdef _DEBUG
          if ((InRange9(Object->x1,180.3e5))
             &&
             (InRange9(Object->y1,58.4e5))) {
            ok=1;
          }
#endif
          if ((SelectedLayer==-1)
            ...
            if ((TestRectangle)
               &&
               (RectTestObject(Object,TestMode))) {
              if ((mode & 1) == 0) {
                if (NrObjects4<MaxNrObjects4) {
                  Object4=&((*Objects4)[NrObjects4]);
                  if (InnerLayer) {
...
*/
  int32 cnt,cnt2,cnt4,CompareMask,NrFound,CompareValue,LineSegments,SegmentCount,
        MaxCountX,FontNr,Mirror,Layer2,NrLines;
  ...
  double  x1,y1,x2,y2,x22,y22,x3,y3,x4,y4,Rotation,Xmax,Xmin,Ymax,Ymin,
          LineBuf[4096];
...
*/
  for (cnt=0;cnt<Design.NrObjectLines;cnt++) {
    ...
    if (((ObjectLine->Info & CompareMask) == CompareValue)
       &&
       (CheckIfCopperLayer(ObjectLine->Layer))) {
      if ((SelectedLayer==-1)
         ||
         (ObjectLine->Layer == SelectedLayer)) {
        x1=ObjectLine->X1;
        y1=ObjectLine->Y1;
        x2=ObjectLine->X2;
        y2=ObjectLine->Y2;
        ...
        if ((min(x1,x2)-x22<=SearchMaxX)
           &&
           (max(x1,x2)+x22>=SearchMinX)
           &&
           (min(y1,y2)-x22<=SearchMaxY)
           &&
           (max(y1,y2)+x22>=SearchMinY)) {
          if ((mode & 1) == 0) {
            if (NrObjects4<MaxNrObjects4) {
              Object4=&((*Objects4)[NrObjects4]);
              Object4->x1=x1;
...
// *******************************************************************************************************
  ...
  for (cnt=0;cnt<Design.NrObjectRects;cnt++) {
    ...
    if (((ObjectRect->Info & CompareMask) == CompareValue)
       &&
       (CheckIfCopperLayer(ObjectRect->Layer))) {
      if ((SelectedLayer==-1)
         ||
         (ObjectRect->Layer == SelectedLayer)
         ||
...
// *******************************************************************************************************
  ...
  for (cnt=0;cnt<Design.NrObjectTexts2;cnt++) {
    ...
    if (((ObjectText2->Info & CompareMask) == CompareValue)
       &&
       (CheckIfCopperLayer(ObjectText2->Layer))) {
      if ((SelectedLayer==-1)
         ||
         (ObjectText2->Layer == SelectedLayer)) {
        x1=ObjectText2->X;
        y1=ObjectText2->Y;
...
// *******************************************************************************************************
          if (FontNr==0) {
            for (cnt2=0;cnt2<NrLines;cnt2++) {
              LineSegments=TextStringToLineSegments2(x1,y1,x2,Rotation,0,Mirror,
                                                     TextStrings2[NrLines],(double *)&LineBuf);
              if ((mode & 1) == 0) {
                SegmentCount=0;
...
// *******************************************************************************************************
            ...
            if ((Xmax>=SearchMinX)
               ...
               &&
               (Ymin<=SearchMaxY)) {
              if ((InRangeSpecial(Rotation,0.0,0.1))
                 ||
                 (InRangeSpecial(Rotation,90.0,0.1))
                 ||
                 (InRangeSpecial(Rotation,180.0,0.1))
                 ||
                 (InRangeSpecial(Rotation,270.0,0.1))) {
                if ((mode & 1) == 0) {
                  if (NrObjects4<MaxNrObjects4) {
                    Object4=&((*Objects4)[NrObjects4]);
...
// *******************************************************************************************************
  ...
  for (cnt=0;cnt<Design.NrObjectPolygons;cnt++) {
    ...
    if (((ObjectPolygon->Info & CompareMask) == CompareValue)
       &&
       (CheckIfCopperLayer(ObjectPolygon->Layer))) {
      if ((SelectedLayer==-1)
         ||
         (ObjectPolygon->Layer == SelectedLayer)
         ||
        ...
        if (((mode & 16) == 0)
          ...
          if ((ObjectPolygon->maxx>=SearchMinX)
             &&
             (ObjectPolygon->minx<=SearchMaxX)
             &&
             (ObjectPolygon->maxy>=SearchMinY)
             &&
             (ObjectPolygon->miny<=SearchMaxY)) {
            if ((mode & 1) == 0) {
              Object4=&((*Objects4)[NrObjects4]);
...

{
  ...
  for (cnt=0;cnt<Design.NrConnections;cnt++) {
    ...
    if (((ConnectionInfo & OBJECT_NOT_VISIBLE) == 0)
       &&
       ((ConnectionInfo & OBJECT_HIGHLITED) != 0)) {
...

{
  ...
  if (!OpenClipboard(PCBWindow)) return -1;
  if (!EmptyClipboard()) return -1;
  if ((NewGlobalClipBoardMem=GlobalAlloc(GHND|GMEM_DDESHARE,ClipBoardMemPos+4))==NULL) return -1;
...

{
  ...
  if (!IsClipboardFormatAvailable(ClipID2)) return -1;
  if (!OpenClipboard(PCBWindow)) return -1;
  if ((NewGlobalClipBoardMem=GetClipboardData(ClipID2))==NULL) return -1;
  if ((NewClipBoardMem=GlobalLock(NewGlobalClipBoardMem))==NULL) return -1;
  ...
  if (TotalMemSize>=ClipBoardMemSize) {
    if (AllocateMemClipBoard(TotalMemSize+4096)!=0) return -1;
  }
