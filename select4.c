...
{
  double Length,Angle,Step,Length2,TraceX1,TraceY1,TraceX1a,TraceY1a,
         TraceX2,TraceY2,NewLengthX,NewLengthY,GridStep;
...
//  EndY=43.0e5;
  ...
  while ((!Found)
    ...
    if (FoundObjects>0) {
      ...
      if (res!=-1) { // Found overlapped object -> Zoom in on the current section
#ifdef _DEBUG
        ...
        if (((Angle>ANGLE_45)
           ...
           ((Angle>ANGLE_225)
           &&
           (Angle<ANGLE_315))) { // Mostly vertical line
      ...
      } else { // No overlapped object found
        if (Length2>Length) {
          *NewEndX=EndX;
          *NewEndY=EndY;
          return 1;
...

{
  ...
  if (SearchConnectionNr==-1) {
    ...
    if (SearchConnectionNr==-1) {
      MessageBoxOwn(PCBWindow,SC(1060,"Did not found a connection"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
    }
    return;
  }
...
{
  int32  ok,cnt,cnt2,Layer,CompInfo,Mirror,OkToCheck,Found,NetNr=-1,
         ActiveCircle,NrNetProperties,NrCompProperties,TestResult;
  double x1a,x2a,y1a,y2a,PosX,PosY,x,y,x1=0.0,y1=0.0,x2=0.0,y2=0.0,
         x3,y3,x4,y4,lengte,dikte,ThickNess;
  char  str[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],str3[MAX_LENGTH_STRING],
        str4[MAX_LENGTH_STRING],str5[MAX_LENGTH_STRING],str6[MAX_LENGTH_STRING],
        str7[MAX_LENGTH_STRING],str8[MAX_LENGTH_STRING],str9[MAX_LENGTH_STRING],
        LayerStr[MAX_LENGTH_STRING],
    ...
    if (OkToCheck) {
      ...
      if ((PosX>x1a)
        ...
        for (cnt2=0;cnt2<NrObjects;cnt2++) {
          ...
          if (Object->Layer!=-1) {
            if (Object->Layer==0) {
              if (!OkToDrawBottomPads) OkToCheck=0;
            } else {
              if (Object->Layer==Design.NrBoardLayers-1) {
                if (!OkToDrawTopPads) OkToCheck=0;
              } else {
                if (!OkToDrawInnerPads) OkToCheck=0;
              }
            }
          } else {
            if ((!OkToDrawBottomPads)
        ...
        if (Found!=-1) {
          ...
          if ((Comp->PartNr)
             &&
             (Comp->PartNr[0])) {
            sprintf(str5,SC(1228,"Part number : %s\r\n"),Comp->PartNr);
            strcat(str4,str5);
          }
          ...
          sprintf(InfoStr,SC(1216,"Component pin\r\n\f\r\n%s - %s\r\nPad : %s\r\nClearance : %s\r\nLayer : %s%s\r\n%s"),
...
#ifdef _DEBUG
      if ((InRange9(ObjectArc->CentreX,170.7e5))
         &&
         (InRange9(ObjectArc->CentreY,42.0e5))) {
        ok=1;
      }
#endif
      ...
      if (OkToCheck) {
        ...
        if (((!ActiveCircle)
          ...
          if ((Layer != DRILL_LAYER)
            ...
            sprintf(str,"%s,%s (%s)",str6,str7,str8);
          } else {
            GetUnitsValue2(Units,x1,str6,0);
            GetUnitsValue2(Units,y1,str7,2);
          ...
          if (Layer == DRILL_LAYER) {
            sprintf(InfoStr,SC(1219,"Drill\r\n\f\r\n%s\r\nDiameter : %s\r\nClearance : %s\r\n%s%s"),
                    str,str2,str3,str4,str5);
          }
          if (Layer == DRILL_UNPLATED_LAYER) {
            sprintf(InfoStr,SC(1220,"Unplated drill\r\n\f\r\n%s\r\nDiameter : %s\r\nClearance : %s\r\n"),
                    str,str2,str3);
          }
          if (Layer<32) {
            GetLayerText(Layer,LayerStr,16+4);
            if ((ObjectArc->Info & OBJECT_FILLED) == OBJECT_FILLED) {
              sprintf(InfoStr,SC(1221,"Pad\r\n\f\r\n%s\r\nDiameter : %s\r\nClearance : %s\r\nLayer : %s\r\n%s%s"),
                      str,str2,str3,LayerStr,str4,str5);
            } else {
              sprintf(InfoStr,SC(1222,"Arc trace\r\n\f\r\n%s\r\nTracewidth : %s\r\nClearance : %s\r\nLayer : %s\r\n%s%s"),
                      str,str2,str3,LayerStr,str4,str5);
            }
          }
          DisplayInfoCursorX=MousePosX;
