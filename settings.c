...
//            SetDialogValue2(Dialog,IDC_EDIT1,TempGridSize);
		...
		case IDAPPLY:
			...
			if ((TempGridSize > 0.0) && (TempGridSize < 10))
			{
				MessageBoxOwn(PCBWindow, SC(931, "Size of component grid >=100nm"), SC(100, "Error"),
				              MB_APPLMODAL | MB_OK);
				return about;
			}
			...
			if ((TempGridSize > 0.0) && (TempGridSize < 10))
			{
				MessageBoxOwn(PCBWindow, SC(1199, "Size of areafill grid >=100nm"), SC(100, "Error"),
				              MB_APPLMODAL | MB_OK);
				return about;
			}
...
int32 CALLBACK PcbSettingsChangeDesignRulesDialog(HWND Dialog, UINT Message, WPARAM WParam, LPARAM LParam)
{
	...
	case WM_COMMAND:
		...
		case IDAPPLY:
			...
			{
				MessageBoxOwn(PCBWindow, SC(174, "Wrong value for trace width"), SC(100, "Error"),
				              MB_APPLMODAL | MB_OK);
				return about;
			}
			...
			{
				MessageBoxOwn(PCBWindow, SC(243, "Wrong value for silkscreen width"), SC(100, "Error"),
				              MB_APPLMODAL | MB_OK);
				return about;
			}
			...
			{
				MessageBoxOwn(PCBWindow, SC(244, "Wrong value for board outline keep out"), SC(100, "Error"),
				              MB_APPLMODAL | MB_OK);
				return about;
			}
			...
			{
				MessageBoxOwn(PCBWindow, SC(245, "Wrong value for board outline width"), SC(100, "Error"),
				              MB_APPLMODAL | MB_OK);
				return about;
			}
	...
	for (cnt = 0; cnt < Design.NrNets; cnt++)
	{
		...
		if (Net->Name[0] != 0)
		{
			if (mode == 0)
			{
				...
				if (Net->ViaNr > 0)
				{
					sprintf(str, "%s\t%.4f\t%.4f\t%d\t%d", Net->Name, Net->TraceWidth / UnitValue,
					        Net->TraceClearance / UnitValue, Net->ViaNr - 1, cnt);
				}
				else
				{
			...
			else
			{
				...
				if (Net->ViaNr > 0)
				{
					sprintf(str, "%.4f\t%.4f\t%s\t%d\t%d", Net->TraceWidth / UnitValue, Net->TraceClearance / UnitValue,
					        Net->Name, Net->ViaNr - 1, cnt);
				}
				else
				{
		...
		case IDC_BUTTON2:		// Change trace width/clearance for nets including all traces/via/areafills
			...
			if (NetsSelected > 0)
			{
				...
				*/
				...
				{
					MessageBoxOwn(PCBWindow, SC(175, "Wrong value for clearance"), SC(100, "Error"),
					              MB_APPLMODAL | MB_OK);
					return about;
				}
