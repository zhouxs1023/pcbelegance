...
uint8 *SelectedNets;
int32 ok,TempUnits,GridChanged,DialogMode,
      DesignSettingsNrTabs,SelectedObjectCode,SelectedObjectNr,
      PcbSettingsNrTabs,TraceNumbering,AperTuresChanged,
      NrDrillLayers,SelectedColorNr,PenPlotUnits,
      EditPenPlotDiametersMode,NetInfoMode,
      MousePosX_S,MousePosY_S,
      TotalCompCount,CompSearchCnt,SpecialNetNr,
      DesignSettingsFocusWindowNr,AvailableComps,NetsSelected,
      PcbSettingsFocusWindowNr,*NetInfo,
      NewMouseWheelMessage,TotalNetCount,
      MessageToBeSend,DesignOptionsSave,
      PcbSettingsDialogWidth,PcbSettingsDialogHeight,
      OwnTimerValue;
...
RECT PcbSettingsRect,RealPcbSettingsWindow,RealPcbSettingsWindow2,
     InfoRect;
char SearchString1[200],SearchString2[200],SearchString3[200],CompSearchString[200];
PAINTSTRUCT   PS;

int32 DialogLayerInfoStartX,DialogLayerInfoStartY,
      DialogLayerInfoEndX,DialogLayerInfoEndY,
      DialogLayerInfoPixelsX,DialogLayerInfoPixelsY,
      SearchCnt1,SearchCnt2,SearchCnt3,
      FirstPaintViewLayersWindow,NetDialogMode,
      LayerInfoOffsetY,LayerOffsetNr,PcbSettingsDialogMode,
      LeftButtonPressed_S,RightButtonPressed_S,TextStart,ButtonStartX,
      ActiveLayerOnMouse,FileTypeStartX;
...

{
  ...
  for (cnt=0;cnt<20;cnt++) {
    ...
    memset(&PcbSettingsTabInfo[cnt].DialogControlHandles,0,
           sizeof(PcbSettingsTabInfo[cnt].DialogControlHandles));
    memset(&PcbSettingsTabInfo[cnt].DialogControlIds,0,
           sizeof(PcbSettingsTabInfo[cnt].DialogControlIds));
  }
  PcbSettingsActiveTab=NULL;

  cnt=0;
  for (cnt2=0;cnt2<2;cnt2++) {
    if (((cnt2==0) && (DialogActive!=IDD_DIALOG_COLOR))
       ||
       ((cnt2==1) && (DialogActive==IDD_DIALOG_COLOR))) {
      PcbSettingsTabInfo[cnt].Name="Colors";
    ...
    if (((cnt2==0) && (DialogActive!=IDD_DIALOG_DESIGN_RULES))
       ||
       ((cnt2==1) && (DialogActive==IDD_DIALOG_DESIGN_RULES))) {
      PcbSettingsTabInfo[cnt].Name="Layout options";
    ...
    if (((cnt2==0) && (DialogActive!=IDD_DIALOG_VIA))
       ||
       ((cnt2==1) && (DialogActive==IDD_DIALOG_VIA))) {
      PcbSettingsTabInfo[cnt].Name="Via";
    ...
    if (((cnt2==0) && (DialogActive!=IDD_DIALOG_NETTYPE))
       ||
       ((cnt2==1) && (DialogActive==IDD_DIALOG_NETTYPE))) {
      PcbSettingsTabInfo[cnt].Name="Design rules Nets";
    ...
    if (((cnt2==0) && (DialogActive!=IDD_DIALOG_NETINFO))
       ||
       ((cnt2==1) && (DialogActive==IDD_DIALOG_NETINFO))) {
      PcbSettingsTabInfo[cnt].Name="Nets/connections info";
    ...
    if (((cnt2==0) && (DialogActive!=IDD_DIALOG_COMPINFO))
       ||
       ((cnt2==1) && (DialogActive==IDD_DIALOG_COMPINFO))) {
      PcbSettingsTabInfo[cnt].Name="Component protection";
    ...
    if (((cnt2==0) && (DialogActive!=IDD_DIALOG_VIEW))
       ||
       ((cnt2==1) && (DialogActive==IDD_DIALOG_VIEW))) {
      PcbSettingsTabInfo[cnt].Name="Layers";
    ...
    if (((cnt2==0) && (DialogActive!=IDD_DIALOG_SETTINGS))
       ||
       ((cnt2==1) && (DialogActive==IDD_DIALOG_SETTINGS))) {
      PcbSettingsTabInfo[cnt].Name="Settings";
  ...
  //create dialog box
  PcbSettingsMainDialog = CreateDialog(PCBClass.hInstance,
                                       MAKEINTRESOURCE(IDD_DIALOG_PCB_SETTINGS),
                                       NULL, (DLGPROC)MainPcbSettingsProc);
...
*/
    ...
    switch (msg.message)   {
      case WM_KEYDOWN:
      case WM_KEYUP:
        if (LOWORD(msg.wParam)==9) { // TAB key
          if (msg.message==WM_KEYDOWN) {
            if ((PcbSettingsActiveTab)
               &&
               (PcbSettingsActiveTab->TabProc==PcbSettingsNetInfoDialog)
               &&
               (NetInfoMode==0)) {
              ...
              if (PcbSettingsActiveTab->DialogControlHandles[cnt3]) {
                res=DefDlgProc(PcbSettingsActiveTab->ControlList,WM_NEXTDLGCTL,
    ...
    if ((!IsWindow(PcbSettingsMainDialog))
       ||
       (!IsDialogMessage(PcbSettingsMainDialog, &msg))) {
...
//      OutputDebugString(str);
  ...
  switch (Message) {
    case WM_INITDIALOG:
      ...
      SetWindowText(Dialog,SC(1329,"Pcb settings"));
      SetDialogItemText(Dialog,IDCANCEL              ,SC(1271,"Close"));
      SetDialogItemText(Dialog,IDOK                  ,SC(155,"OK"));
      SetDialogItemText(Dialog,IDAPPLY               ,SC(1320,"Apply"));
//      GetWindowRect(Dialog,&PcbSettingsRect);
...
//      TabCtrl_SetCurSel(Dialog,2);
      SendDlgItemMessage(Dialog,IDC_TAB1,TCM_SETCURSEL,0,0);
//      ViewplotSettingsFocusWindowNr=ViewplotSettingsNrTabs-2;
//      ViewplotSettingsDialogMode=ViewplotSettingsTabInfo[ViewplotSettingsFocusWindowNr].DialogMode;
      PcbSettingsDialogMode=IDD_DIALOG_SETTINGS;


//      ShowWindow(ViewplotSettingsTabInfo[ViewplotSettingsFocusWindowNr].ControlList,SW_SHOW);
//      SendMessage(ViewplotSettingsTabInfo[ViewplotSettingsFocusWindowNr].ControlList,WM_SETFOCUS,0,0);


//      SendDlgItemMessage(Dialog,IDC_TAB1,TCM_SETCURSEL,ViewplotSettingsNrTabs-1,0);
/*
      PcbSettingsDialogWidth=DesignSettingsRect.right-DesignSettingsRect.left-150;
      PcbSettingsDialogHeight=DesignSettingsRect.bottom-DesignSettingsRect.top-150;
*/
...
//        OutputDebugString(str);
    ...
    case WM_SIZE:
    case WM_MOVE:
//      GetWindowRect(Dialog,&RealPcbSettingsWindow2);
      break;
    case WM_COMMAND:
...
void PcbSettingsTabControlSetup(HWND TabControl)
{
  ...
  while (PcbSettingsTabInfo[cnt].Name) {
    ...
    PcbSettingsTabInfo[cnt].ControlList = CreateDialog(PCBClass.hInstance,
                                                       MAKEINTRESOURCE(PcbSettingsTabInfo[cnt].ResourceID),
                                                       TabControl,
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(1100,"Brushes"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(1101,"Second brush color"));
      SetDialogItemText(Dialog,IDC_RADIO17                ,SC(1102,"Background color"));
      SetDialogItemText(Dialog,ID_CHANGE_COLOR            ,SC(880,"Change color"));
      SetDialogItemText(Dialog,ID_CHANGE_COLOR2           ,SC(1103,"Change second color"));
      SendDlgItemMessageOwn(Dialog,IDD_COLOR_LIST1,LB_RESETCONTENT,0,0);
      SetWindowTextOwn(Dialog,SC(881,"Modify colors"));
...
/*
      if (Design.NrBoardLayers>1) {
        res=SendDlgItemMessageOwn(Dialog,IDD_COLOR_LIST1,LB_ADDSTRING,0,ViewLayer2InNetObjectNr);
      }
      if (Design.NrBoardLayers>2) {
        for (cnt=Design.NrBoardLayers-1;cnt>=2;cnt--) {
          res=SendDlgItemMessageOwn(Dialog,IDD_COLOR_LIST1,LB_ADDSTRING,0,ViewLayer1InNetObjectNr+cnt);
        }
      }
      res=SendDlgItemMessageOwn(Dialog,IDD_COLOR_LIST1,LB_ADDSTRING,0,ViewLayer1InNetObjectNr);
*/
      ...
      MoveWindow(Dialog,SpecialWindowInDialogStartX,SpecialWindowInDialogStartY,
                 DialogPixelsX+SpecialWindowInDialogStartX,
                 DialogPixelsY+SpecialWindowInDialogStartY,1);
      return about;
    case WM_DRAWITEM:
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        case IDD_COLOR_LIST1:
          ...
          break;
        case IDC_RADIO1:    case IDC_RADIO2:    case IDC_RADIO3:    case IDC_RADIO4:
        case IDC_RADIO5:    case IDC_RADIO6:    case IDC_RADIO7:    case IDC_RADIO8:
        case IDC_RADIO9:    case IDC_RADIO10:   case IDC_RADIO11:   case IDC_RADIO12:
        case IDC_RADIO13:   case IDC_RADIO14:   case IDC_RADIO15:   case IDC_RADIO16:
          if ((res=SendDlgItemMessageOwn(Dialog,IDD_COLOR_LIST1,LB_GETCURSEL,0,0))!=CB_ERR) {
        ...
        case ID_CHANGE_COLOR:
          if ((res=SendDlgItemMessageOwn(Dialog,IDD_COLOR_LIST1,LB_GETCURSEL,0,0))!=CB_ERR) {
            ...
            if (NewColor!=-1) {
              switch (res2) {
                case ViewLayer1ObjectNr:    case ViewLayer2ObjectNr:
                case ViewLayer3ObjectNr:    case ViewLayer4ObjectNr:
                case ViewLayer5ObjectNr:    case ViewLayer6ObjectNr:
                case ViewLayer7ObjectNr:    case ViewLayer8ObjectNr:
                case ViewLayer9ObjectNr:    case ViewLayer10ObjectNr:
                case ViewLayer11ObjectNr:   case ViewLayer12ObjectNr:
                case ViewLayer13ObjectNr:   case ViewLayer14ObjectNr:
                case ViewLayer15ObjectNr:   case ViewLayer16ObjectNr:
                case ConnectionsObjectNr:   case ViaPinsObjectNr:
                case ViewLayer1InNetObjectNr:  case ViewLayer2InNetObjectNr:
                case ViewLayer3InNetObjectNr:  case ViewLayer4InNetObjectNr:
                case ViewLayer5InNetObjectNr:  case ViewLayer6InNetObjectNr:
                case ViewLayer7InNetObjectNr:  case ViewLayer8InNetObjectNr:
                case ViewLayer9InNetObjectNr:  case ViewLayer10InNetObjectNr:
                case ViewLayer11InNetObjectNr: case ViewLayer12InNetObjectNr:
                case ViewLayer13InNetObjectNr: case ViewLayer14InNetObjectNr:
                case ViewLayer15InNetObjectNr: case ViewLayer16InNetObjectNr:
                case ViaPinsInNetObjectNr:     case ShapePinsTopObjectNr:
                case ShapePinsBottomObjectNr:  case ShapePinsInnerObjectNr:
                  switch (res2) {
                    case ViewLayer1ObjectNr:   case ViewLayer2ObjectNr:
                    case ViewLayer3ObjectNr:   case ViewLayer4ObjectNr:
                    case ViewLayer5ObjectNr:   case ViewLayer6ObjectNr:
...
*/
                  ...
                  switch (res2) {
                    ...
                    case ViewLayer13ObjectNr:  case ViewLayer14ObjectNr:
                    case ViewLayer15ObjectNr:  case ViewLayer16ObjectNr:
                      PCBColors[res3-ViewLayer1ObjectNr+ViewLayer1InNetObjectNr]=(min(cb,255) << 16) + (min(cg,255) << 8) + min(cr,255);
                      break;
                  }
                  break;
                case GridObjectNr:
              ...
              switch (res2) {
                ...
                case ViaPinsInNetObjectNr:
                  ...
                  switch (res2) {
                    ...
                    case ViewLayer13ObjectNr:   case ViewLayer14ObjectNr:
                    case ViewLayer15ObjectNr:   case ViewLayer16ObjectNr:
                      PCBColors2[res3-ViewLayer1ObjectNr+ViewLayer1InNetObjectNr]=(min(cb,255) << 16) + (min(cg,255) << 8) + min(cr,255);
                      break;
                  }
                  break;
                case GridObjectNr:
        ...
        case IDAPPLY:
          ...
          return 1;
//        case IDHELP:
//          Help(IDH_Plot_output_to_gerber_format,0);
//          return about;
        case IDCANCEL:
          SendMessage(PcbSettingsMainDialog,(UINT)WM_CLOSE,0,0);
          return 1;
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDAPPLY       ,SC(1320,"Apply"));
      SetDialogItemText(Dialog,IDCANCEL      ,SC(1271,"Close"));
      SetDialogItemText(Dialog,IDC_STATIC1   ,SC(882,"Other layers"));
      SetDialogItemText(Dialog,IDC_STATIC2   ,SC(883,"Trace layers"));
      SetDialogItemText(Dialog,IDC_STATIC6   ,SC(66,"Solder mask top"));
      SetDialogItemText(Dialog,IDC_STATIC7   ,SC(74,"Paste mask top"));
      SetDialogItemText(Dialog,IDC_STATIC8   ,SC(62,"Solder mask bottom"));
      SetDialogItemText(Dialog,IDC_STATIC9   ,SC(70,"Paste mask bottom"));
      ...
      SendDlgItemMessageOwn(Dialog,IDC_LIST4,LB_ADDSTRING,0,(LPARAM)SC(57,"Component references"));
      SendDlgItemMessageOwn(Dialog,IDC_LIST4,LB_ADDSTRING,0,(LPARAM)SC(58,"Component values"));
      ...
      SendDlgItemMessageOwn(Dialog,IDC_LIST5,LB_ADDSTRING,0,(LPARAM)SC(59,"Board outline"));
      SendDlgItemMessageOwn(Dialog,IDC_LIST5,LB_ADDSTRING,0,(LPARAM)SC(28,"Silkscreen top"));
      SendDlgItemMessageOwn(Dialog,IDC_LIST5,LB_ADDSTRING,0,(LPARAM)SC(32,"Silkscreen bottom"));
      SendDlgItemMessageOwn(Dialog,IDC_LIST5,LB_ADDSTRING,0,(LPARAM)SC(566,"Routing keepout top"));
      SendDlgItemMessageOwn(Dialog,IDC_LIST5,LB_ADDSTRING,0,(LPARAM)SC(567,"Routing keepout bottom"));
      SendDlgItemMessageOwn(Dialog,IDC_LIST5,LB_ADDSTRING,0,(LPARAM)SC(109,"Routing keepout inner"));
      SendDlgItemMessageOwn(Dialog,IDC_LIST5,LB_ADDSTRING,0,(LPARAM)SC(37,"Info layer 1"));
      SendDlgItemMessageOwn(Dialog,IDC_LIST5,LB_ADDSTRING,0,(LPARAM)SC(40,"Info layer 2"));
      SendDlgItemMessageOwn(Dialog,IDC_LIST5,LB_ADDSTRING,0,(LPARAM)SC(43,"Info layer 3"));
      SendDlgItemMessageOwn(Dialog,IDC_LIST5,LB_ADDSTRING,0,(LPARAM)SC(46,"Info layer 4"));
      ...
      SetDialogItemText(Dialog,IDC_CHECK4    ,SC(91,"Vias"));
      SetDialogItemText(Dialog,IDC_CHECK16   ,SC(888,"Errors"));
      SetDialogItemText(Dialog,IDC_CHECK17   ,SC(889,"Warnings"));
      SetDialogItemText(Dialog,IDC_CHECK26   ,SC(1132,"Cross hair"));

      SetDialogItemText(Dialog,IDC_BUTTON3   ,SC(892,"Solder/paste mask pads"));
      SetDialogItemText(Dialog,IDC_BUTTON4   ,SC(346,"Unselect all"));
      SetDialogItemText(Dialog,IDC_BUTTON5   ,SC(160,"Select all"));
      SetDialogItemText(Dialog,IDC_BUTTON6   ,SC(893,"Default"));
      SetDialogItemText(Dialog,IDC_BUTTON7   ,SC(346,"Unselect all"));
      SetDialogItemText(Dialog,IDC_BUTTON8   ,SC(160,"Select all"));
      SetWindowTextOwn(Dialog,SC(894,"Viewable objects"));
      ...
      SendDlgItemMessageOwn(Dialog,IDC_COMBO1,CB_ADDSTRING,0,(LPARAM)SC(900,"All"));
      ...
      SendDlgItemMessageOwn(Dialog,IDC_COMBO2,CB_ADDSTRING,0,(LPARAM)SC(900,"All"));
      ...
      SendDlgItemMessageOwn(Dialog,IDC_COMBO3,CB_ADDSTRING,0,(LPARAM)SC(900,"All"));
      ...
      SendDlgItemMessageOwn(Dialog,IDC_COMBO4,CB_ADDSTRING,0,(LPARAM)SC(900,"All"));
...
*/
        ...
        case IDC_LIST1: // Trace layers
          ...
          if (SendDlgItemMessage(Dialog,IDC_CHECK25,BM_GETCHECK,0,0)==1) { // Only one layer active
            if (res==LBN_SELCHANGE) {
              ...
              found++;
              if (found==Design.NrBoardLayers) found=0;
              for (cnt=0;cnt<Design.NrBoardLayers;cnt++) {
                Layer=Design.NrBoardLayers-cnt-1;
                if (found==cnt) {
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDAPPLY                    ,SC(1320,"Apply"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(927,"Default grid"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(928,"Grid when moving components"));
      SetDialogItemText(Dialog,IDC_STATIC3                ,SC(929,"Grid when drawing traces"));
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(1198,"Grid when drawing areafills"));
      SetDialogItemText(Dialog,IDC_STATIC5                ,SC(95,"Grid"));
      SetDialogItemText(Dialog,IDC_STATIC6                ,SC(1202,"Selection modes"));
      SetDialogItemText(Dialog,IDC_STATIC7                ,SC(1201,"Snap modes"));
      SetDialogItemText(Dialog,IDC_STATIC8                ,SC(1203,"Several settings"));
      SetDialogItemText(Dialog,IDC_STATIC9                ,SC(1205,"Mouse cursor Info display"));
      SetDialogItemText(Dialog,IDC_STATIC10               ,SC(1206,"Sec"));
      SetDialogItemText(Dialog,IDC_STATIC11               ,SC(1206,"Sec"));
      SetDialogItemText(Dialog,IDC_STATIC12               ,SC(1207,"Activate after"));
      SetDialogItemText(Dialog,IDC_STATIC13               ,SC(1208,"Visible for"));
      SetDialogItemText(Dialog,IDC_STATIC14               ,SC(1343,"Pan sensivity"));
      SetDialogItemText(Dialog,IDC_STATIC30               ,SC(1211,"Repeat"));
      SetDialogItemText(Dialog,IDC_STATIC31               ,SC(1341,"Zoom mode"));
      SetDialogItemText(Dialog,IDC_CHECK27                ,SC(1212,"On"));


//      SetDialogItemText(Dialog,IDC_STATIC7                ,SC());
//      SetDialogItemText(Dialog,IDC_STATIC8                ,SC());
      SetDialogItemText(Dialog,IDC_CHECK1                 ,SC(761,"Recalculate ratsnets after move"));
      SetDialogItemText(Dialog,IDC_CHECK2                 ,SC(762,"Intelligent ratsnets dragging"));
      SetDialogItemText(Dialog,IDC_CHECK8                 ,SC(1179,"Do not show areafill thin lines errors"));
      SetDialogItemText(Dialog,IDC_CHECK9                 ,SC(745,"Recalc areafill after inserting an object"));
      SetDialogItemText(Dialog,IDC_CHECK7                 ,SC(1178,"Repaint after every component move"));
      SetDialogItemText(Dialog,IDC_CHECK6                 ,SC(1177,"Repaint after every trace draw"));
      SetDialogItemText(Dialog,IDC_CHECK10                ,SC(790,"View relative position on grid"));

      SetDialogItemText(Dialog,IDC_CHECK4                 ,SC(746,"Snap on for special objects"));
      SetDialogItemText(Dialog,IDC_CHECK5                 ,SC(1110,"Snap on first comp location"));
      SetDialogItemText(Dialog,IDC_CHECK11                ,SC(1209,"Start with maximum view"));
      SetDialogItemText(Dialog,IDC_CHECK13                ,SC(1304,"Move/rotate component with auto zoom"));
      SetDialogItemText(Dialog,IDC_CHECK12                ,SC(1210,"Activated"));

      SetDialogItemText(Dialog,IDC_RADIO1                 ,SC(794,"Replacement"));
      ...
      SetDialogItemText(Dialog,IDC_BUTTON4                ,SC(1149,"Set"));
      SetWindowTextOwn(Dialog,SC(1200,"Options"));
      TempUnits=Units;
...
//            SetDialogValue2(Dialog,IDC_EDIT1,TempGridSize);
        ...
        case IDAPPLY:
          ...
          if ((TempGridSize>0.0)
             &&
             (TempGridSize<10)) {
            MessageBoxOwn(PCBWindow,SC(930,"Size of grid >=100nm"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          ...
          if ((TempGridSize>0.0)
             &&
             (TempGridSize<10)) {
            MessageBoxOwn(PCBWindow,SC(931,"Size of component grid >=100nm"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          ...
          if ((TempGridSize>0.0)
             &&
             (TempGridSize<10)) {
            MessageBoxOwn(PCBWindow,SC(932,"Size of trace grid >=100nm"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          ...
          if ((TempGridSize>0.0)
             &&
             (TempGridSize<10)) {
            MessageBoxOwn(PCBWindow,SC(1199,"Size of areafill grid >=100nm"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          res=SendDlgItemMessage(Dialog,IDC_COMBO1   ,CB_GETCURSEL,0,0);
          if ((res>=0)
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(228,"Board outline keep out"));
      SetDialogItemText(Dialog,IDC_STATIC5                ,SC(237,"Board outline width"));
      ...
      switch (LOWORD(WParam)) {
        ...
        case IDAPPLY:
          if ((value=GetDialogValue(Dialog,IDD_NEW_EDIT5))<100.0) {
            MessageBoxOwn(PCBWindow,SC(174,"Wrong value for trace width"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          NewDesign.StandardTraceWidth=(float)value;

          if ((value=GetDialogValue(Dialog,IDD_NEW_EDIT6))<100.0) {
            MessageBoxOwn(PCBWindow,SC(175,"Wrong value for clearance"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          NewDesign.StandardClearance=(float)value;
          NewDesign.MaximumClearance=(float)value;
          ...
          if ((value=GetDialogValue(Dialog,IDD_NEW_EDIT7))<100.0) {
            MessageBoxOwn(PCBWindow,SC(243,"Wrong value for silkscreen width"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          NewDesign.SilkScreenWidth=(float)value;

          if ((value=GetDialogValue(Dialog,IDD_NEW_EDIT9))<0.0) {
            MessageBoxOwn(PCBWindow,SC(244,"Wrong value for board outline keep out"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          NewDesign.BoardOutlineKeepOut=(float)value;
          if ((value=GetDialogValue(Dialog,IDD_NEW_EDIT8))<0.0) {
            MessageBoxOwn(PCBWindow,SC(245,"Wrong value for board outline width"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          NewDesign.BoardOutlineWidth=(float)value;
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(233,"Pad size"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(234,"Drill diameter"));
      SetDialogItemText(Dialog,IDC_STATIC3                ,SC(211,"Clearance"));
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(235,"Anti power pad"));
      SetDialogItemText(Dialog,IDC_STATIC5                ,SC(236,"Solder mask"));
      SetDialogItemText(Dialog,IDC_RADIO1                 ,SC(238,"Top layer"));
      SetDialogItemText(Dialog,IDC_RADIO2                 ,SC(239,"Bottom layer"));
      SetDialogItemText(Dialog,IDC_RADIO3                 ,SC(240,"Top/bottom layer"));
      SetDialogItemText(Dialog,IDD_VIA_GET_DEF01          ,SC(248,"Get via definition 1"));
      SetDialogItemText(Dialog,IDD_VIA_GET_DEF02          ,SC(249,"Get via definition 2"));
      SetDialogItemText(Dialog,IDD_VIA_GET_DEF03          ,SC(250,"Get via definition 3"));
      SetDialogItemText(Dialog,IDD_VIA_GET_DEF04          ,SC(251,"Get via definition 4"));
      SetDialogItemText(Dialog,IDD_VIA_GET_DEF05          ,SC(1133,"Get via definition 5"));
      SetDialogItemText(Dialog,IDD_VIA_GET_DEF06          ,SC(1134,"Get via definition 6"));
      SetDialogItemText(Dialog,IDD_VIA_GET_DEF07          ,SC(1135,"Get via definition 7"));
      SetDialogItemText(Dialog,IDD_VIA_GET_DEF08          ,SC(1136,"Get via definition 8"));
      SetDialogItemText(Dialog,IDD_VIA_GET_DEF09          ,SC(1137,"Get via definition 9"));
      SetDialogItemText(Dialog,IDD_VIA_GET_DEF10          ,SC(1138,"Get via definition 10"));
      SetDialogItemText(Dialog,IDD_VIA_SET_DEF01          ,SC(252,"Set via definition 1"));
      SetDialogItemText(Dialog,IDD_VIA_SET_DEF02          ,SC(253,"Set via definition 2"));
      SetDialogItemText(Dialog,IDD_VIA_SET_DEF03          ,SC(254,"Set via definition 3"));
      SetDialogItemText(Dialog,IDD_VIA_SET_DEF04          ,SC(255,"Set via definition 4"));
      SetDialogItemText(Dialog,IDD_VIA_SET_DEF05          ,SC(1139,"Set via definition 5"));
      SetDialogItemText(Dialog,IDD_VIA_SET_DEF06          ,SC(1140,"Set via definition 6"));
      SetDialogItemText(Dialog,IDD_VIA_SET_DEF07          ,SC(1141,"Set via definition 7"));
      ...
      switch (DialogMode) {
        case 0:
          SetWindowTextOwn(Dialog,SC(256,"Define/set current via"));
          break;
        case 1:
          SetWindowTextOwn(Dialog,SC(257,"Change vias"));
          break;
      }
      TempUnits=Units;
    ...
    case WM_COMMAND:
      switch (LOWORD(WParam)) {
        ...
        case IDD_VIA_SET_DEF10:
          ...
          value4=GetDialogValue(Dialog,IDC_EDIT7);
          value5=GetDialogValue(Dialog,IDC_EDIT9);
          if ((value1<(float)(2540.0))
             ||
             (value2<(float)(2540.0))
             ||
             (value3<(float)(254.0))) {
            MessageBoxOwn(PCBWindow,SC(246,"Wrong values"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          if (value2>value1+(float)10) {
            MessageBoxOwn(PCBWindow,SC(247,"Drill diameter > via pad"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
            return about;
          }
          DataBaseChanged=1;
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_BUTTON1                ,SC(1326,"Search component"));
      SetDialogItemText(Dialog,IDD_SELECTALL              ,SC(160,"Select all"));
      SetDialogItemText(Dialog,IDD_UNSELECTALL            ,SC(161,"Deselect all"));
      cnt2=0;
      ...
      sprintf(str,SC(180,"Protected components   [%i,%i]"),count,cnt2);
      SetDialogItemText(Dialog,IDC_STATIC1,str);
      TabStops[0]=700;
...

{
  ...
  if (mode==0) {
    ...
    TabStops[2]=175;
    TabStops[3]=500;
    SendDlgItemMessageOwn(Dialog,ListboxHeader,LB_ADDSTRING,0,(LPARAM)SC(1152,"Net name\tWidth\tClear\tVia nr"));
  } else {
    TabStops[0]=30;
    TabStops[1]=60;
    TabStops[2]=175;
    TabStops[3]=550;
    SendDlgItemMessageOwn(Dialog,ListboxHeader,LB_ADDSTRING,0,(LPARAM)SC(1169,"Width\tClear\tNet name\tVia nr"));
  }
  ...
  for (cnt=0;cnt<Design.NrNets;cnt++) {
    ...
    if (Net->Name[0]!=0) {
      if (mode==0) {
        // Sort on netname
        if (Net->ViaNr>0) {
          sprintf(str,"%s\t%.4f\t%.4f\t%d\t%d",Net->Name,Net->TraceWidth/UnitValue,Net->TraceClearance/UnitValue,Net->ViaNr-1,cnt);
        } else {
          sprintf(str,"%s\t%.4f\t%.4f\t\t%d",Net->Name,Net->TraceWidth/UnitValue,Net->TraceClearance/UnitValue,cnt);
        }
      } else {
        // Sort on trace width
        if (Net->ViaNr>0) {
          sprintf(str,"%.4f\t%.4f\t%s\t%d\t%d",Net->TraceWidth/UnitValue,Net->TraceClearance/UnitValue,Net->Name,Net->ViaNr-1,cnt);
        } else {
          sprintf(str,"%.4f\t%.4f\t%s\t\t%d",Net->TraceWidth/UnitValue,Net->TraceClearance/UnitValue,Net->Name,cnt);
        }
      }
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDHELP                     ,SC(156,"Help"));
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDC_STATIC1                ,SC(1151,"Trace/Clearance width"));
      SetDialogItemText(Dialog,IDC_STATIC2                ,SC(1150,"Trace"));
      SetDialogItemText(Dialog,IDC_STATIC3                ,SC(1097,"Selection"));
      SetDialogItemText(Dialog,IDC_STATIC4                ,SC(1145,"Via"));
      SetDialogItemText(Dialog,IDC_STATIC5                ,SC(1146,"Pad size"));
      SetDialogItemText(Dialog,IDC_STATIC6                ,SC(1147,"Drill size"));
      SetDialogItemText(Dialog,IDC_STATIC7                ,SC(1148,"Solder mask"));
      ...
      strcpy(str,SC(1328,"Set net(s) design rule and also\r\nchange all traces/vias/areafills"));
      SetDialogItemText(Dialog,IDC_BUTTON2                ,str);
      SetDialogItemText(Dialog,IDC_BUTTON3                ,SC(1149,"Set"));

      SetWindowTextOwn(Dialog,SC(158,"Net selection dialog"));
      ...
      switch (NetDialogMode) {
        case 0:
          sprintf(str,SC(173,"Trace/clearance widths    [%d nets]"),Design.NrNets);
          SetWindowTextOwn(Dialog,str);
          break;
      }
      SortMode=0;
      SendDlgItemMessageOwn(Dialog,IDC_RADIO1,BM_SETCHECK,1,0);
...
/*
            ...
            strcat(str,SC(176,"Do you also want to change the width/clearance of already existing traces/vias from the selected nets\r\n\r\n"));
            strcat(str,SC(177,"The clearance of the traces/vias/areafills of the selected nets will be modified,\r\n"));
...
/*
          NetsSelected=(int32)SendDlgItemMessage(Dialog,IDC_LIST3,LB_GETSELITEMS,
                                                 Design.NrNets,(LPARAM)((LPINT)NetInfo));
          if (NetsSelected>0) {
            cnt4=SendDlgItemMessageOwn(Dialog,IDC_LIST2,LB_GETCURSEL,0,0);
            NetChanged=0;
            for (cnt=0;cnt<NetsSelected;cnt++) {
              cnt2=NetInfo[cnt];
              SendDlgItemMessageOwn(Dialog,IDC_LIST3,LB_GETTEXT,cnt2,(LPARAM)str);
              cnt3=strlen(str)-1;
              while ((cnt3>0)
                    &&
                    (str[cnt3]!='\t')) cnt3--;
              if ((sscanf(&str[cnt3],"%i",&NetNr)==1)
                 &&
                 (NetNr>=0)
                 &&
                 (NetNr<Design.NrNets)) {
            ...
            if (NetChanged) {
              ...
              for (cnt=0;cnt<NetsSelected;cnt++) {
                cnt2=NetInfo[cnt];
                SendDlgItemMessage(Dialog,IDC_LIST3,LB_SELITEMRANGE,1,MAKELPARAM(cnt2,cnt2));
              }
              SendDlgItemMessage(Dialog,IDC_LIST3,LB_SETTOPINDEX,max(0,NetInfo[0]-18),0);
            }
          }
*/
          break;
...

{
  ...
  res=OwnDialogBoxParam(PCBClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG_NETTYPE), PCBWindow,
...

{
  ...
  switch ( Message ) {
    case WM_INITDIALOG:
      ...
      SetDialogItemText(Dialog,IDCANCEL                   ,SC(157,"Cancel"));
      SetDialogItemText(Dialog,IDAPPLY                    ,SC(1320,"Apply"));
      SetDialogItemText(Dialog,IDC_BUTTON1                ,SC(1321,"Search net"));
      SetDialogItemText(Dialog,IDC_BUTTON2                ,SC(1321,"Search net"));
      SetDialogItemText(Dialog,IDC_BUTTON3                ,SC(1321,"Search net"));
      SetDialogItemText(Dialog,IDD_SELECTALL              ,SC(160,"Select all"));
      SetDialogItemText(Dialog,IDD_SELECTALL2             ,SC(160,"Select all"));
      SetDialogItemText(Dialog,IDD_SELECTALL3             ,SC(160,"Select all"));
      SetDialogItemText(Dialog,IDD_UNSELECTALL            ,SC(161,"Deselect all"));
      SetDialogItemText(Dialog,IDD_UNSELECTALL2           ,SC(161,"Deselect all"));
      SetDialogItemText(Dialog,IDD_UNSELECTALL3           ,SC(161,"Deselect all"));
      switch (NetInfoMode) {
        case 0: // Nets disable,highlight,invisble
          ...
          SetDialogItemText(Dialog,IDC_STATIC2                ,str);
          sprintf(str,SC(1324,"Net connections disabled [%i nets]"),Design.NrNets);
          SetDialogItemText(Dialog,IDC_STATIC3                ,str);
          break;
        case 1: // Unselect traces/vias nets
          sprintf(str,SC(1325,"Selected nets  [%i nets]"),Design.NrNets);
          SetDialogItemText(Dialog,IDC_STATIC1                ,str);
          SetDialogItemText(Dialog,IDC_STATIC2                ,"");
          SetDialogItemText(Dialog,IDC_STATIC3                ,"");
          break;
...
//      DesignOptionsSave=Design.Options;
      cnt2=0;
      for (cnt=0;cnt<Design.NrNets;cnt++) {
        Net=&((*Nets)[cnt]);
        if (Net->Name[0]!=0) {
...
/*
          ...
          res=SendDlgItemMessage(Dialog,IDC_LIST2,LB_SETTOPINDEX,cnt3,0);
          res=SendDlgItemMessage(Dialog,IDC_LIST3,LB_SETTOPINDEX,cnt4,0);
*/
                      ...
                      if (Net->Info & CONNECTIONS_DISABLED) {
                        ...
                        for (cnt2=0;cnt2<Design.NrConnections;cnt2++) {
                          ...
                          if (((Connection->Info & OBJECT_NOT_VISIBLE) == 0)
                            ...
                            if ((Net->Info & OBJECT_HIGHLITED) == OBJECT_HIGHLITED) Connection->Info|=OBJECT_HIGHLITED;
                          }
                        }
                        Net->Info&=~CONNECTIONS_DISABLED;
                      }
...
//          EndDialog(Dialog,1);
        ...
        case IDCANCEL:
/*
          Design.Options=DesignOptionsSave;
          for (cnt=0;cnt<Design.NrNets;cnt++) {
            Net=&((*Nets)[cnt]);
            if (Net->Name[0]!=0) {
              Net->Info&=~(OBJECT_UNHIGHLITED|OBJECT_INVISIBLE);
            }
          }
          RePaint();
*/
