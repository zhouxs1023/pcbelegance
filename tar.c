...

{
  ...
  while (res) {
      ...
      if ((wcscmp(FindFileData.cFileName,L"."))
        ...
        if (!wcscmp(FindFileData.cFileName,L".")) {
          ...
          for (cnt=0;cnt<length;cnt++) {
            if (TarHeader->formated.name[cnt]=='\\') TarHeader->formated.name[cnt]='/';
          }
    ...
    char padding[12];   /* 500-512 */
          ...
          sprintf(TarHeader->formated.size,"%12o%12o        5",
                  0,mktime2(FileDate.wYear,FileDate.wMonth,
                            FileDate.wDay,FileDate.wHour,
                            FileDate.wMinute,FileDate.wSecond)-2);
...
//      printf("Reading %s\\%s\n",Directory,FindFileData.cFileName);
      ...
      if (length<99) {
        ...
        strcpy(TarHeader->formated.gid ,"      0");
        FileTimeToSystemTime(&FindFileData.ftLastWriteTime,&FileDate);
        sprintf(TarHeader->formated.size,"%12o%12o        0",
                FindFileData.nFileSizeLow,mktime2(FileDate.wYear,FileDate.wMonth,
                                                  FileDate.wDay,FileDate.wHour,
                                                  FileDate.wMinute,FileDate.wSecond));
        strcpy(TarHeader->formated.magic,"ustar");
        strcpy(TarHeader->formated.uname,"user");
        strcpy(TarHeader->formated.gname,"group");
        /* Do checksum on headers */
        ...
        sprintf(TarHeader->formated.chksum,"%6o",sum);
        FileWrite(Tarfp,TarHeader,sizeof(TarStruct),&result2);
        Handle=CreateFile(str2,GENERIC_READ,0,NULL,OPEN_EXISTING,
...

{
  ...
  sprintf(str2,"%s.tgz",OdbFile);
  if (FileExistsUTF8(str2)==0) {
    sprintf(str,SC(370,"File %s exist overwrite ?"),str2);
    SetNormalCursor();
