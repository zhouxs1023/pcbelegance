...
//  ny=AdjustToDrawGridDiv2(y);
  if (TraceObject->Layer!=DrawObject->Layer) return 0;
  if (TraceObject->ObjectType!=DrawObject->ObjectType) return 0;
  if (NotInRange(TraceObject->y2,DrawObject->y2)) return 0;
  ...
  switch (TraceObject->ObjectType) {
    case TRACE_HOR:
      if (NotInRange(TraceY1,DrawY1)) return 0;
...
//          DrawObject->x2=0;
      ...
      break;
    case TRACE_VER:
      if (NotInRange(TraceX1,DrawX1)) return 0;
      CountX1=CountNrObjectsEndPoint(TraceObject->Layer,TraceX1,TraceY1,0);
...
//          DrawObject->x2=0;
      ...
      if (DrawY1>TraceY1+TraceX2) return 0;
      if (DrawY1+DrawX2<TraceY1) return 0;
      ...
      return 3;
    case TRACE_DIAG1:
      if (NotInRange(TraceX1+TraceY1,DrawX1+DrawY1)) return 0;
      CountX1=CountNrObjectsEndPoint(TraceObject->Layer,TraceX1,TraceY1,0);
...
*/
  ...
  for (cnt=0;cnt<NrObjects3;cnt++) {
    ...
    if ((DrawTrace->ObjectType==ConnectObject->ObjectType)
       &&
...
//     ((CheckSpecialVersion(0)==0) || (!CtrlPressed))
    ...
    NewY=CurrentDrawY1;
    if (mode==0) {
      if (!FindLimitTrace(StartFirstDrawingTraceX,
                          ...
                          &NewX,&NewY,
                          DrawTrace->NetNr,
                          CurrentDrawingLayer)) return -1;
    } else {
      if (!FindLimitTrace(StartSecondDrawingTraceX,
    ...
    if (((mode==0)
       &&
       ((NotInRange(NewX,StartFirstDrawingTraceX))
       ||
       (NotInRange(NewY,StartFirstDrawingTraceY))) )
       ||
       ((mode==1)
       &&
       ((NotInRange(NewX,StartSecondDrawingTraceX))
       ||
       (NotInRange(NewY,StartSecondDrawingTraceY))))) {
...
{
  int32   cnt,ok,ConnectionObjectNr,CurrentTraceNr,OkToRePaint,
          res,count1,count2,TempLastActionNr;
...
// If DrawTrace1.x2=0 Trace backwards
  ...
  if (EndPointObjectNr!=-1) { // Endpoint reached
    ok=1;
...
// *******************************************************************************
  if (DrawTrace1.x2>0) {
    if ((res=CheckIfTraceFitAndAdjust(&DrawTrace1,0))==-1) return 1;
    if (((TraceDrawingMode==2)
       ||
       (TraceDrawingMode==3))
       &&
       (res==2)) { // Trace arc does not fit
      return 1;
    }
...
// If the DrawTrace overlaps the underlying CurrentWorkingTrace -> delete trace
      if ((CurrentWorkingTrace.ObjectType==DrawTrace1.ObjectType)
         &&
         (InRange(CurrentWorkingTrace.x1,DrawTrace1.x1))
         &&
         (InRange(CurrentWorkingTrace.y1,DrawTrace1.y1))
         &&
      ...
      if (OkToAddSecondTrace) {
        if (res!=-1) {
          ...
          if ((CurrentWorkingTrace.TraceNr!=-1)
             &&
             (res!=CurrentWorkingTrace.TraceNr)) {
            memmove(&DrawTrace1,&CopyDrawTrace1,sizeof(ObjectRecord));
          }
        }
      } else res=-1;
...
// Add new trace1
  ...
  if (DrawTrace1.x2>0) {
    ...
    if (CheckObjectInsideAreaFillPowerPlane(&DrawTrace1)==0) {
      if (!AddTrace(&DrawTrace1)) return 0;
      TracesInserted++;
      switch (TraceDrawingMode) {
        case 0:
          PreviousDirection=GetNewDirection(StartFirstDrawingTraceX,StartFirstDrawingTraceY,
                                            EndFirstDrawingTraceX,EndFirstDrawingTraceY,0);
          break;
        case 1:
          PreviousDirection=GetNewDirection(StartFirstDrawingTraceX,StartFirstDrawingTraceY,
                                            EndFirstDrawingTraceX,EndFirstDrawingTraceY,0);
      ...
      if ((RecalcAreafillAfterInsert)
        ...
        if (!AltPressed) {
          ...
          if (res==1) ObjectInAreaFillInserted=1;
        }
      }
...
//            break;
        ...
        if (ObjectIsTrace(EndPointObject)) {
          ...
          if ((InRange(EndSecondDrawingTraceX,x3))
             &&
             (InRange(EndSecondDrawingTraceY,y3))) {
            ok=1;
          } else {
            if ((InRange(EndSecondDrawingTraceX,x4))
               &&
               (InRange(EndSecondDrawingTraceY,y4))) {
              ok=1;
            } else {
              if ((count1==1)
                 &&
                ...
                if (!CheckObjectOverlapped(&RebuildTraceObject,0)) {
                  ...
                  if (!AddTrace(&RebuildTraceObject)) return 0;
                }
                ok=1;
              } else {
                if ((count1==2)
...
//      EndPreviousTraceY=StartFirstDrawingTraceY;
      ...
      OkToAddSecondTrace=0;
      MessageBoxOwn(PCBWindow,SC(1063,"Can not draw on powerplane"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
      Beep(1000,200);
    }
...
// Add new trace2
    if ((OkToAddSecondTrace)
      ...
      if (CheckIfTraceFitAndAdjust(&DrawTrace2,1)!=-1) {
        ...
        switch (TraceDrawingMode) {
          case 0:
            PreviousDirection=GetNewDirection(StartSecondDrawingTraceX,StartSecondDrawingTraceY,
                                              EndSecondDrawingTraceX,EndSecondDrawingTraceY,0);
            break;
          case 1:
            break;
          case 2:
        ...
        if (CheckObjectInsideAreaFillPowerPlane(&DrawTrace2)==0) {
          ...
          if (!AddTrace(&DrawTrace2)) return 0;
          TracesInserted++;
          if ((RecalcAreafillAfterInsert)
             &&
             (TraceCheck)) {
...
// If thers is a via and trace then the via is favorit
  ...
  if (ConnectionObjectNr==-1) {    // If nothing connected to the deleted trace exit
    ReCalcConnectionsNet(CurrentDrawingNetNr,0,0);
    LastActionNr++;
    return 0;
  }
...
// Trace from a different layer connected to the via
  ...
  if ((ConnectionObjectNr==-1)
     ||
     ((!ObjectIsTrace(ConnectObject))
     &&
     (ConnectObject->ObjectType!=TRACE_ALL_ANGLE)
     &&
...
// Find the starting point of the connected trace
  ...
  if ((InRange(x1,EndPreviousTraceX))
     &&
     (InRange(y1,EndPreviousTraceY))) {
...

{
  ...
  if ((DrawTrace1.ObjectType==0)
     ||
     (DrawTrace1.x2==0)) return 1;

  if ((ConnectionObject2.Layer!=-1)
     &&
     (DrawTrace1.Layer!=ConnectionObject2.Layer)) return 1;

  if ((DoubleTraceRes=CalcDoubleTrace())==0) return 1;
  ...
  if ((DrawTrace1.x2>0.0)
     &&
     (!CheckObjectOverlapped(&DrawTrace1,0))) {
    if (DrawTrace2.ObjectType!=0) {
      if ((DrawTrace2.x2>0.0)
         &&
         (!CheckObjectOverlapped(&DrawTrace2,0))) {
        IncludeTrace1=1;
        IncludeTrace2=1;
      } else IncludeTrace1=1;
    } else IncludeTrace1=1;
  }
  res2=1;
  if (IncludeTrace1) {
...
//  ViaObject.y1=StartSecondDrawingTraceY;
  ...
  if (CheckObjectOverlapped(&ViaObject,0)) return -1;
    ...
    switch (Object3->ObjectType) {
      ...
      case PIN_PUT_THROUGH_SQUARE:
        ...
        if (ObjectsConnected(&CheckObject,Object3)) {
          ok=1;
          if (((InRange(Object3->x1,ViaObject.x1))
             &&
             (InRange(Object3->y1,ViaObject.y1)))) {
            FoundNr=cnt;
          }
        }
        break;
...
*/
  ...
  for (cnt=0;cnt<NrObjects3;cnt++) {
    ...
    if (ConnectObject->ObjectType==VIA_PUT_THROUGH_ROUND) {
      hulp=0;
      if ((InRange(ViaObject.x1,ConnectObject->x1))
         &&
         (InRange(ViaObject.y1,ConnectObject->y1))
         &&
         (ObjectCoveredNr==-1)) {
  ...
  for (Layer=0;Layer<32;Layer++) {
    for (cnt=0;cnt<Design.NrVerTraces[Layer];cnt++) {
      ...
      if ((Trace->Info & (OBJECT_SELECTED|OBJECT_NOT_VISIBLE)) == OBJECT_SELECTED) {
        if (Net1==-1) Net1=Trace->NetNr;
        else
          if (Net2==-1) Net2=Trace->NetNr;
      }
    }
...
*/
  ...
  if ((!OkToSwitchDrawingLayers)
     ||
     (Design.NrBoardLayers==0)) return -1;
  ...
  if (Design.NrBoardLayers==2) {
    ...
    if (!ViewSingleLayer) {
      ...
      while ((cnt<Design.NrBoardLayers)
        ...
        if (NewLayer>=Design.NrBoardLayers) NewLayer-=Design.NrBoardLayers;
        if ((DrawLayerCode[NewLayer]<0x10)
           &&
           (!IsLayerPowerPlane(NewLayer))) {
