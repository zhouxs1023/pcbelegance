...
//           Mult(CurrentX2-Xoffset),DrawWindowMaxY-Mult(CurrentY2-Yoffset)-1);
  if (GetDimensionTextFromLine(CurrentX2,CurrentY2,CurrentX,CurrentY,
                               &NewObjectText2,NewObjectLine.LineMode)==0) {
...

{
  ...
  if ((NewObjectLine.LineMode!=0)
    ...
    if (AddObjectText2(&NewObjectText2)) DrawObjectText2(&NewObjectText2,0.0,0.0,0.0,0);
  }
  ExitDrawing();
  EndDrawingEditingWindow();
}
...
//  CurrentDrawMode=1;
  ...
  while (!SelectionEsc) {
    if (MouseChanged) {
      ...
      if (Mode>0) InitInfoStr(CurrentX,CurrentY,0);
    ...
    if (!Focused) {
      ...
      CheckInputMessages(0);
      ClipMouseCursor();
      if (!SelectionEsc) DrawTryingObjectLine(CurrentX,CurrentY,Mode);
    }
    if ((ZoomActive())
       &&
       (!SelectionEsc)) {
...
//        NewObjectRect.Height=(fabs(CurrentY-CurrentY2)*2);
  ...
  DrawLine(Mult(NewObjectRect.CentreX-Xoffset)+DrawWindowMinX-3,DrawWindowMaxY-Mult(NewObjectRect.CentreY-Yoffset)-1-3,
           Mult(NewObjectRect.CentreX-Xoffset)+DrawWindowMinX+3,DrawWindowMaxY-Mult(NewObjectRect.CentreY-Yoffset)-1+3);
  DrawLine(Mult(NewObjectRect.CentreX-Xoffset)+DrawWindowMinX-3,DrawWindowMaxY-Mult(NewObjectRect.CentreY-Yoffset)-1+3,
           Mult(NewObjectRect.CentreX-Xoffset)+DrawWindowMinX+3,DrawWindowMaxY-Mult(NewObjectRect.CentreY-Yoffset)-1-3);
...
//  CurrentDrawMode=1;
  ...
  while (!SelectionEsc) {
    if (MouseChanged) {
      ...
      if (Mode>0) InitInfoStr(CurrentX,CurrentY,1);
      DisplayCursorPosition();
      MouseChanged=0;
    }
...
//    if (RightButtonPressed) {
    ...
    if (NrFunctionsInBuf>0) {
      ...
      if (SpacePressed) {
        ...
        switch (Mode) {
          case 0:
            if (LineInputDialog(&TypeObject,SC(1069,"Add rectangle (width,height,[CenterX,CenterY])"),0)==1) {
              if ((NrParams=ScanParameters(-1,TypeObject.Text,0))>=0) {
                if (NrParams==2) {
          ...
          case 1:
            if (LineInputDialog(&TypeObject,SC(1070,"Add rectangle (width,height)"),0)==1) {
              if (((NrParams=ScanParameters(-1,TypeObject.Text,0))==2)
                 &&
                 (NotInRange(NewObjectRect.Width,0.0))
                 &&
                 (NotInRange(NewObjectRect.Height,0.0))) {
  ...
  switch (Mode) {
    case 1:
      ...
      if (((NewObjectArc.Info & OBJECT_FILLED) == 0)
         &&
         (NewObjectArc.Layer!=SOLD_MASK_BOTTOM)
         &&
         (NewObjectArc.Layer!=SOLD_MASK_TOP)
         &&
         (NewObjectArc.Layer!=PASTE_MASK_BOTTOM)
         &&
         (NewObjectArc.Layer!=PASTE_MASK_TOP)
         &&
         (NewObjectArc.Layer>=32)) {
...
*/
  ...
  switch (Mode) {
    ...
    case 11:
    case 12:
      DrawLine(MultX(NewObjectArc.CentreX)-3,MultY(NewObjectArc.CentreY)-3,
               MultX(NewObjectArc.CentreX)+3,MultY(NewObjectArc.CentreY)+3);
      DrawLine(MultX(NewObjectArc.CentreX)-3,MultY(NewObjectArc.CentreY)+3,
               MultX(NewObjectArc.CentreX)+3,MultY(NewObjectArc.CentreY)-3);
      break;
    case 2:
    case 3:
      DrawLine(MultX(NewObjectArc.CentreX),MultY(NewObjectArc.CentreY),
               MultX(CurrentX),MultY(CurrentY));
      break;
  }
  DrawCrossHair(8);
  ExitDrawing();
...

{
  ...
  DrawCrossHair(2);
  SetROP2(OutputDisplay,R2_COPYPEN);
  if (AddObjectArc(ObjectArc)) DrawObjectArc(ObjectArc,0.0,0.0,0);
  ExitDrawing();
  EndDrawingEditingWindow();
}
  ...
  while (!SelectionEsc) {
    ...
    if (LeftButtonPressed) {
      ...
      if (CircleMode!=0) {
        ...
        if ((Mode>=3)
          ...
          if ((NotInRange(NewObjectArc.Width,0.0))
             &&
             (NotInRange(NewObjectArc.Height,0.0))) {
            CommandAddTryingObjectArc(&NewObjectArc);
          }
          SelectionEsc=1;
        }
      ...
      if (CircleMode==0) {
        if ((Mode==1)
           &&
           (((InRange(NewObjectArc.Width,0.0))
           ||
           (InRange(NewObjectArc.Height,0.0))))) {
  ...
  */
    ...
    if (NrFunctionsInBuf>0) {
      ...
      if (SpacePressed) {
        ...
        switch (Mode) {
          case 0:
            if (LineInputDialog(&TypeObject,SC(1072,"Add arc (width,height), [CenterX,CenterY], [StartAngle,EndAngle])"),0)==1) {
              if ((NrParams=ScanParameters(-1,TypeObject.Text,0))>=0) {
                if (NrParams==2) {
                ...
                if (NrParams==6) {
                  ...
                  NewObjectArc.StartDiffX=(float)cos(ANGLE_CONVERT(ConvertToUnits(Units,ParamsFloat[4])));
                  NewObjectArc.StartDiffY=(float)sin(ANGLE_CONVERT(ConvertToUnits(Units,ParamsFloat[4])));
                  NewObjectArc.EndDiffX=(float)cos(ANGLE_CONVERT(ConvertToUnits(Units,ParamsFloat[5])));
                  NewObjectArc.EndDiffY=(float)sin(ANGLE_CONVERT(ConvertToUnits(Units,ParamsFloat[5])));
...

{
  StartDrawingEditingWindow();
  DrawCrossHair(2);
  if (AddObjectText2(ObjectText2)) DrawObjectText2(ObjectText2,0.0,0.0,0.0,0);
  ExitDrawing();
  EndDrawingEditingWindow();
}
...

{
  ...
  if ((Layer==0)
     ||
     (Layer==SILKSCREEN_BOTTOM)) NewObjectText2.TextMode=0x10;

  if (TextInputDialog(&NewObjectText2,2)==2) return;
  if (strlen(NewObjectText2.Text)==0) return;
  ...
  while (!SelectionEsc) {
    ...
    if ((ZoomActive())
      ...
      OldX=CurrentX;
      OldY=CurrentY;
      if (!SelectionEsc) DrawTryingObjectText2(CurrentX,CurrentY,Mode);
    }
    if ((PanActive())
       &&
       (!SelectionEsc)) {
