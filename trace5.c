...
static ObjectRecord  MovingTrace,MovingTrace1,MovingTrace2,
                     MovingDrawTrace1,MovingDrawTrace2,MovingDrawTrace3;

static double   MinX,MinY,MaxX,MaxY,Trace1X1,Trace1Y1,Trace1X2,Trace1Y2,
                Trace2X1,Trace2Y1,Trace2X2,Trace2Y2,
                MovingTraceX1,MovingTraceY1,MovingTraceX2,MovingTraceY2,
                MovingTrace1X1,MovingTrace1Y1,MovingTrace1X2,MovingTrace1Y2,
                MovingTrace2X1,MovingTrace2Y1,MovingTrace2X2,MovingTrace2Y2,
                MovingCurrentDrawX1,MovingCurrentDrawY1,MovingCurrentDrawX2,MovingCurrentDrawY2;
static int32    TraceDir,MovingTrace1Dir,MovingTrace2Dir,FoundObject3Nr,
                FoundObject3Nr1,FoundObject3Nr2,Trace1Dir,Trace2Dir,
                MovingTrace1EndPoint,MovingTrace2EndPoint;

static int32     ChangeCenterTrace,
                AdjustTrace1X1Y1,AdjustTrace1X2Y2,
                AdjustTrace2X1Y1,AdjustTrace2X2Y2;
...

{
  ...
  if (mode==0) {
    if ((MovingDrawTrace1.x2+10.0>0.0)
       &&
       (MovingDrawTrace2.x2+10.0>0.0)
       &&
...

{
  ...
  if (OkToDrawSpecialTraces(0.0,0)) {
    ...
    if ((Net->Info & OBJECT_HIGHLITED) != 0) {
      if ((DrawCode<0)
         ||
         (DrawCode>MAX_ACTIVE_DRAWING_LAYERS-1)) return;
    ...
    if (mode==0) {
      if (MovingDrawTrace1.x2>0) DrawObjectTraceXor(&MovingDrawTrace1);
      if (MovingDrawTrace2.x2>0) DrawObjectTraceXor(&MovingDrawTrace2);
      if (MovingDrawTrace3.x2>0) DrawObjectTraceXor(&MovingDrawTrace3);
    } else {
      if (MovingDrawTrace1.x2>0) DrawObjectTrace(&MovingDrawTrace1);
      if (MovingDrawTrace2.x2>0) DrawObjectTrace(&MovingDrawTrace2);
      if (MovingDrawTrace3.x2>0) DrawObjectTrace(&MovingDrawTrace3);
    }
    if ((OkToDrawClearances)
       ||
       (DrawClearanceForTryingTrace)) {
      InitDrawingObject(0,CLEARANCE_LAYER,0,DRAW_WITH_PEN_AND_NOT_FILLED);
      if (MovingDrawTrace1.x2>0) DrawObjectTraceWithClearance(&MovingDrawTrace1);
      if (MovingDrawTrace2.x2>0) DrawObjectTraceWithClearance(&MovingDrawTrace2);
      if (MovingDrawTrace3.x2>0) DrawObjectTraceWithClearance(&MovingDrawTrace3);
    }
    ExitDrawing();
    EndDrawingEditingWindow();
  }
...
//  }
  ...
  while (!SelectionEsc) {
    if (MouseChanged) {
      ...
      switch (TraceDir) {
        case 0:
          if (OldY!=MovingCurrentDrawY2) Changed=1;
...
//             (InRange(OldX-MovingCurrentDrawX2,OldY-MovingCurrentDrawY2))) Changed=1;
          if (OldX!=MovingCurrentDrawX2) Changed=1;
...
//          }
          ...
          if (RecalcAreafillAfterInsert) {
            res2=InsertObjectInAreaFill(&MovingDrawTrace1,MovingDrawTrace1.Layer,MovingDrawTrace1.NetNr,2);
            if (res2==1) AreaFillChanged=1;
          }
        }
...
//            AreaFill=(AreaFillRecord *)&(AreaFillMem[(*AreaFills)[Design.NrAreaFills-1]]);
            res3=InsertObjectInAreaFill(&MovingDrawTrace2,MovingDrawTrace2.Layer,MovingDrawTrace2.NetNr,2);
            if (res3==1) AreaFillChanged=1;
          }
        }
...
//          }
          ...
          if (RecalcAreafillAfterInsert) {
            res4=InsertObjectInAreaFill(&MovingDrawTrace3,MovingDrawTrace3.Layer,MovingDrawTrace3.NetNr,2);
            if (res4==1) AreaFillChanged=1;
          }
        }
        if (!OkToRePaintAfterTraceDrawing) {
          DrawSpecialTrace(1);
...

{
  ...
  for (cnt=0;cnt<NrObjects3;cnt++) {
    ...
    if ((ObjectNet->ObjectType==MovingTrace.ObjectType)
       &&
       (InRange(ObjectNet->x1,MovingTrace.x1))
       &&
       (InRange(ObjectNet->y1,MovingTrace.y1))
       &&
       (InRange(ObjectNet->x2,MovingTrace.x2))) {
      FoundObject3Nr=cnt;
    }
  }
// **************************************************************************************
...
// Find trace on first point MovingTrace
  ...
  for (cnt=0;cnt<NrObjects3;cnt++) {
    if (cnt!=FoundObject3Nr) {
      Object=&((*Objects3)[cnt]);
      if ((ObjectIsTrace(Object))
         &&
         (ObjectsConnected4(Object,&Object2))) {
        ...
        if (Count1==0) FoundObject3Nr1=cnt;
        if (Count1<31) Count1++;
      }
    }
  }
...
// Find trace on second point MovingTrace
  ...
  for (cnt=0;cnt<NrObjects3;cnt++) {
    if (cnt!=FoundObject3Nr) {
      ...
      if ((ObjectIsTrace(Object))
        ...
        if (Count2==0) FoundObject3Nr2=cnt;
        if (Count2<31) Count2++;
      }
    }
  }
  ...
  if ((FoundObjects1[0].ObjectType!=0)
     &&
     (!ObjectIsTrace(&FoundObjects1[0]))) {
    return;
  }
  if ((FoundObjects2[0].ObjectType!=0)
     &&
     (!ObjectIsTrace(&FoundObjects2[0]))) {
    return;
  }
  ...
  for (cnt=0;cnt<NrObjects3;cnt++) {
    if ((cnt!=FoundObject3Nr)
      ...
      if ((Object->ObjectType!=CONNECTION)
         &&
         (ObjectsConnected(&MovingTrace,Object))) {
...
// Check if three traces are connected to each other
  ...
  if (!(((InRange(MovingTraceX1,MovingTrace1X1))
       &&
       (InRange(MovingTraceY1,MovingTrace1Y1)))
       ||
       ((InRange(MovingTraceX1,MovingTrace1X2))
       &&
       (InRange(MovingTraceY1,MovingTrace1Y2))))
       &&
       (((InRange(MovingTraceX2,MovingTrace2X1))
       &&
       (InRange(MovingTraceY2,MovingTrace2Y1)))
       ||
       ((InRange(MovingTraceX2,MovingTrace2X2))
       &&
       (InRange(MovingTraceY2,MovingTrace2Y2))))) {
      return;
    }
...
// **************************************************************************************
  ...
  if (MovingTrace2.TraceNr!=-1) {
    if ((InRange(MovingTraceX2,MovingTrace2X1))
       &&
       (InRange(MovingTraceY2,MovingTrace2Y1))) {
      MovingTrace2EndPoint=0;
    } else {
...

{
  ...
  if ((res=ValueDialog(0))==2) return -1;
...
*/
    for (cnt=0;cnt<Design.NrObjectArcs;cnt++) {
      ...
      if (((ObjectArc->Info & (OBJECT_NOT_VISIBLE|OBJECT_SELECTED)) == OBJECT_SELECTED)
         ...
         &&
         (NotInRange(ObjectArc->Clearance,NewValue.Value))
         &&
...

{
  ...
  if (ClearanceErrorVias>0) {
    ...
    if (AddToMessageBuf(str2)!=0) return;
  }
  if (MessageBufPos!=0) {
    MessageDialog(SC(1,"Message"),0,0);
    DeAllocateMemMessageBuf();
