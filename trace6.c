...

{
  ...
  if (FoundObject.ObjectType!=0) {
    ...
    if ((FoundObject.NetNr<0)
       ||
       (FoundObject.NetNr>=Design.NrNets)) {
      MessageBoxOwn(PCBWindow,SC(1061,"Pin is not connected"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
      Beep(1000,200);
      return;
    }
    ...
    for (cnt=0;cnt<NrObjects5;cnt++) {
      ...
      switch (Object5a.ObjectType) {
        ...
        case TRACE_DIAG1:
        case TRACE_DIAG2:
          if ((CheckObjectInsideAreaFillPowerPlane(&Object5a)==0)
             &&
             (!CheckObjectOverlapped(&Object5a,0))) {
            Object5a.Info=0;
            if ((Net->Info & OBJECT_HIGHLITED) == OBJECT_HIGHLITED) {
              Object5a.Info=OBJECT_HIGHLITED;
            }
            if (!AddTrace(&Object5a)) return;
    ...
    Beep(1000,200);
    MessageBoxOwn(PCBWindow,SC(1082,"Did not found a pin"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
  }
}
...

{
  ...
  double  OldX,OldY,CurrentX,CurrentY,ShiftX,ShiftY,
          ShiftOffsetX2,ShiftOffsetY2,x1,y1,x2,y2;
  ObjectRecord *Object;
...
//  ShiftOffsetY=-(CurrentY-AdjustToDrawGrid(PinPointY));
  ...
  while (!SelectionEsc) {
    ...
    if ((ZoomActive())
      ...
      OldX=CurrentX;
      OldY=CurrentY;
      if (!SelectionEsc) DrawTracesVias(CurrentX,CurrentY,0);
...
//            PlaceMovedObjects(x1,y1,0);
  ...
  if (!ObjectsPlaced) DrawTracesVias(CurrentX,CurrentY,0);
  SystemBusyMode=0;
}
...

{
  ...
  if (Error==1) {
    MessageBoxOwn(PCBWindow,SC(1084,"Selected traces/vias have more then one net"),SC(100,"Error"),MB_APPLMODAL|MB_OK);
    Beep(1000,200);
    return;
  }
  if (NrObjects5>0) {
