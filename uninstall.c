...
char    *p1,*p2,*p3,regel[MAX_LENGTH_STRING],str1[MAX_LENGTH_STRING],str2[MAX_LENGTH_STRING],
        str3[MAX_LENGTH_STRING],str4[MAX_LENGTH_STRING],CurrentDir[MAX_LENGTH_STRING],
        ExecutableName[MAX_LENGTH_STRING],TempDir[MAX_LENGTH_STRING],ExecutableDir[MAX_LENGTH_STRING];
...
{
  char  str[MAX_LENGTH_STRING],InstallDir[MAX_LENGTH_STRING],
        ProjectDir[MAX_LENGTH_STRING],StartMenuPath[MAX_LENGTH_STRING],
        DesktopPath[MAX_LENGTH_STRING];
  int32 res,KeySize;
  HKEY  Key;
  ...
  if ((res=RegOpenKeyEx(HKEY_USERS,str,0,KEY_ALL_ACCESS,&Key))==ERROR_SUCCESS) {
    KeySize=100;
    if ((res=RegQueryValueEx(Key,"InstallDir",0,NULL,(LPBYTE)&InstallDir,(PDWORD)&KeySize))==ERROR_SUCCESS) {
      ok=1;
    }
    KeySize=100;
    if ((res=RegQueryValueEx(Key,"ProjectDir",0,NULL,(LPBYTE)&ProjectDir,(PDWORD)&KeySize))==ERROR_SUCCESS) {
      ok=1;
    }
    if (ProjectDir[0]==0) {
...
//    UNINSTALLClass.lpszMenuName= MAKEINTRESOURCE(IDR_MENU1);
    if ( ! RegisterClass(&UNINSTALLClass) ) exit(255);
  }

  UNINSTALLWindow = CreateWindow(UNINSTALLClassName,
                                 "Uninstall PCB elegance",
