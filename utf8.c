...

{
  ...
  for (cnt=0;cnt<(int32)strlen(str);cnt++) {
    ch=(uint8)*str++ & 0xe0;
    if ((ch==0xc0) || (ch==0xd0) || (ch==0xe0)) return 1;
  }
...

{
  ...
  if ((Msg==WM_SETTEXT)
    ...
    if (Msg==LB_GETTEXT) {
      ...
      if (res==LB_ERR) return res;
      if (UnicodeToUtf8((WCHAR *)TempStrUC,(char *)lParam,MAX_LENGTH_UC_STRING-1)==0) return 0;
      return strlen((LPSTR)lParam);
    } else {
      if (Utf8ToUnicode((char *)lParam,TempStrUC,MAX_LENGTH_UC_STRING-1)==0) return 0;
...
*/
  if (Utf8ToUnicode((char *)lpString,TempStrUC,MAX_LENGTH_UC_STRING-1)==0) return 0;
  ...
  switch (Msg) {
    ...
    case EM_GETLINE:
      ...
      *DestMem16=(uint16)lengte;
      ...
      TempStrUC[result]=0;
      if (UnicodeToUtf8((WCHAR *)TempStrUC,(char *)lParam,lengte)==0) return 0;
      return strlen((LPSTR)lParam);
    case CB_GETLBTEXT:
    case LB_GETTEXT:
      result=SendDlgItemMessageW(hDlg,nIDDlgItem,Msg,wParam,(LPARAM)TempStrUC);
      if (result==0) return 0;
      TempStrUC[result]=0;
      if (UnicodeToUtf8(TempStrUC,(char *)lParam,MAX_LENGTH_UC_STRING-1)==0) return 0;
      return strlen((LPSTR)lParam);
    case WM_GETTEXT:
      result=SendDlgItemMessageW(hDlg,nIDDlgItem,Msg,min(MAX_LENGTH_UC_STRING-1,wParam),(LPARAM)TempStrUC);
      if (result==0) return 0;
      if (UnicodeToUtf8(TempStrUC,(char *)lParam,min(MAX_LENGTH_UC_STRING-1,wParam)-1)==0) return 0;
      return strlen((LPSTR)lParam);
    default:
...
//  uint16 *SrcMem16,*DestMem16;
  switch (Msg) {
    ...
    case WM_SETTEXT:
      ...
      if (Utf8ToUnicode((char *)lParam,(WCHAR *)TempMemUTF8,MaxLengthBigUcString-1)==0) return 0;
...
*/
    case WM_GETTEXT:
      ...
      result=SendDlgItemMessageW(hDlg,nIDDlgItem,Msg,wParam,(LPARAM)TempMemUTF8);
      if (result==0) return 0;
      if (UnicodeToUtf8((WCHAR *)TempMemUTF8,(char *)lParam,wParam)==0) return 0;
      return strlen((LPSTR)lParam);
    default:
      return SendDlgItemMessage(hDlg,nIDDlgItem,Msg,wParam,lParam);
  }
...
{
  WCHAR  TempStrUC[MAX_LENGTH_UC_STRING];
  if (((MenuOptions & (MF_SEPARATOR|MF_BITMAP|MF_OWNERDRAW))!=0)
     ||
     (!Text)
     ||
...

{
  ...
  if (Utf8ToUnicode(Src,(WCHAR *)TempMemUTF8,MaxLengthBigUcString-1)==0) return 0;
  if (Utf8ToUnicode(Dest,(WCHAR *)Temp2MemUTF8,MaxLength2BigUcString-1)==0) return 0;
...

{
  ...
  if (UnicodeToUtf8((WCHAR *)TempMemUTF8,Src,MaxLengthBigUcString-1)==0) return 0;
  return 1;
}
